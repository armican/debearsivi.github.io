---
layout: post
title: 29 Aralık 2019 Ekşi Sözlük Debe
data:
- entry_name: |
    başörtülü kızı sıkıştıran akit muhabiri
  entry_id:  99714145
  entry_writer: yavlum mithat
  entry_fav_count: 1334
  entry_content: |
    sıkıştığı an o ebleh ses tonuyla "müslüman mısın???" diye soru sormaktan öteye gidemeyen bir embesilin gerçekleştirdiği röportaj.<br/>sana ne lan yavşak!! ister allah'a taparım, ister ateşe taparım. sana ne!!!!
  entry_channels: siyaset,tv,haber
- entry_name: |
    atv'de atatürk'ün sansürlenmesi
  entry_id:  99727481
  entry_writer: atraksiyonmemurlari
  entry_fav_count: 1119
  entry_content: |
    kimse bilmez adlı dizinin 24. bölümünde 95. dakikada kapıdaki atatürk silütetinin blurlanarak kapatılması hadisesidir.<br/><br/>ne dense az!<br/><br/><a rel="nofollow noopener" class="url" target="_blank" href="https://streamable.com/z49q9" title="https://streamable.com/z49q9">cam kapının sağ üstü</a><br/><br/>ister blurlayın, ister sansürleyin. ne halt ederseniz edin! silemeyeceksiniz o'nu ne zihinlerimizden, ne kalplerimizden!<br/><br/>(bkz: <a class="b" href="/entry/99714629">#99714629</a>)
  entry_channels: siyaset,tv,tarih,haber
- entry_name: |
    yerli otomobilin tamamen yalan çıkması
  entry_id:  99730297
  entry_writer: zamyatin
  entry_fav_count: 1014
  entry_content: |
    araçlar aynı değil ki. yerlisinin ekranında cumhurbaşkanlığı forsu var.
  entry_channels: siyaset,teknoloji,otomotiv,ekonomi
- entry_name: |
    kadınların yolda yürürken omuzlarını çekmemesi
  entry_id:  99716957
  entry_writer: vikingstekibalta
  entry_fav_count: 548
  entry_content: |
    henüz istisnasını göremediğim genelleme.<br/><br/>içerisinde kadın kelimesi geçen bu tip başlıklara yazmamaya kararlıydım ama bıktırdınız artık.<br/><br/>şekilden şekle giriyoruz çarpmamak için. bazen ağaca dalmak zorunda kalıyoruz bazen duvara yapışıyoruz.<br/><br/>yeter lan. öyle rahat yürüyorlar ki sanki görünmez olmuşuz. biraz sen çek omzunu rahatın bozulacak ama kusura bakma.<br/><br/>en sonunda dayanamayıp omzu koyucam ama tacizci damgasıyla esnaf tarafından linç edilme korkusu var.
  entry_channels: ilişkiler
- entry_name: |
    doğu ekspresinde tüple yemek yapan çift
  entry_id:  99735630
  entry_writer: oles
  entry_fav_count: 411
  entry_content: |
    bu amkodumun trenine binen herkes bi ölümsüzlüğü keşfettim havalarına giriyor. lan altı üstü ankara- kars yataklı vagonunda seyahat ediyorsun ne olum bu tripler? resim asmalar sağa sola, odayı ışıklandırıp kerhane moduna çevirmeler, 3 öğün seçilmiş özel porselenlerle yemek yemek falan hayırdır amk. adam tüple yemek yapıyor lan trenin içinde?? anasnskyim bu hangi beyinin kabullenebileceği bi durum???<br/><br/>gerçekten fevkalede bir beyinsizlik kokusu geliyor şu videoları izleyince.
  entry_channels: haber,yeme-içme,seyahat
- entry_name: |
    starbucks'ta grande boya orta boy diyen insan
  entry_id:  99725405
  entry_writer: beklenmeyenanlar
  entry_fav_count: 369
  entry_content: |
    (bkz: <a class="b" href="/?q=hay%c4%b1rd%c4%b1r+ingiltere+prensiyle+mi+konu%c5%9fuyorum">hayırdır ingiltere prensiyle mi konuşuyorum</a>)
  entry_channels: anket,yeme-içme
- entry_name: |
    baldızın büyüdükçe taş gibi olması
  entry_id:  99724219
  entry_writer: papazi dovdurmeyecektik
  entry_fav_count: 250
  entry_content: |
    tarkovsky bilen, bergman bilen adam kurgucudur. dolayısıyla hikaye uydurmadır. dolayısıyla celallenmeye lüzum yoktur.
  entry_channels: ilişkiler
- entry_name: |
    giresun'da ayı görünce ulhahaha diyen adam
  entry_id:  99736662
  entry_writer: cicilota
  entry_fav_count: 234
  entry_content: |
    adamlar ayıya ateş etmiyor, yavruları görünce şaşırıp video alıyorlar ve zarar vermeden gidelim diyorlar, bence kaliteli insanlarmış, ama sözlükte mal çok anlamazlar
  entry_channels: ilişkiler
- entry_name: |
    afyon'da tren bariyerini söken dolmuşçu
  entry_id:  99723304
  entry_writer: kisa camel
  entry_fav_count: 218
  entry_content: |
    fakirler arasında küçük kuralları ihlal edip avantaj elde etme hastalığı yaygın. bu hayata 1-0 yenik başladıkları düşüncesinden kaynaklanıyor. orada kazandığı 1 dakikayı kendine kar sayarak ölüm riskini alıyor.
  entry_channels: haber
- entry_name: |
    barış özcan'ın yerli arabaya sessiz kalması
  entry_id:  99719163
  entry_writer: p4c
  entry_fav_count: 136
  entry_content: |
    insanları rahat bırakın.
  entry_channels: 
- entry_name: |
    asgari ücretliler togg'u nasıl alacak sorunsalı
  entry_id:  99725448
  entry_writer: deadgodinme
  entry_fav_count: 127
  entry_content: |
    alamayacak. alamadığı gibi döşü kıllı ayılar da kahyve köşelerinde "anaba yabtı" diyecek. alkışlayacaklar, gururlanacaklar...<br/><br/>ama ufacık bir zeka kırıntısı bile ihtiva etmedikleri için, kendilerine sormayacaklar ki "ulan bu arabaya ben niye sahip olamıyorum, sahip olmam için kaç sene çalışmalıyım, devlet neden benden vergi alıyor, vergilendirilmiş kazanç gerçekten kutsal mı yoksa bizi mi sikiyorlar"...<br/><br/>zenginlerin borçları silinirken, birileri saraylarda yaşarken ben neden bu arabayı görüp heyecanlanıyorum yada gururlanıyorum da demeyecekler.<br/><br/>ama olsun araba yabtı
  entry_channels: 
- entry_name: |
    bir insanla ilişiği kesmek için yeter sebepler
  entry_id:  99713244
  entry_writer: nicotortorella
  entry_fav_count: 95
  entry_content: |
    durmadan kötü enerjisi ile sizi boğuyorsa,kaç yıldır tanıdığınızın önemi yok...
  entry_channels: anket,ilişkiler
- entry_name: |
    dağ evi + hatun + internet + 1000 kitap + ps4
  entry_id:  99733212
  entry_writer: terelelli akademisyen
  entry_fav_count: 80
  entry_content: |
    ilgili hatun turk ise,hatunun geriye kalanlarin verecegi butun zevki ,keyfi ve hazzi pic edecegi durumdur.cennetin ortasina niye cehennem diregi dikilmeye calisilmaktadir anlamasi zordur.
  entry_channels: anket,ilişkiler
- entry_name: |
    volkswagen'in togg paniği erken başladı
  entry_id:  99735005
  entry_writer: boylesidahaguzel
  entry_fav_count: 74
  entry_content: |
    yapmayın böyle ataklar ben de panik atak var. -volkswagen.<br/><br/>bu arada haberi okurken kahkaha attım.
  entry_channels: teknoloji,haber,otomotiv,ekonomi
- entry_name: |
    roboski katliamı
  entry_id:  99715512
  entry_writer: zredust
  entry_fav_count: 67
  entry_content: |
    (bkz: <a class="b" href="/?q=12+aral%c4%b1k+2019+silopi+kan+davas%c4%b1+vah%c5%9feti">12 aralık 2019 silopi kan davası vahşeti</a>) gibi rezilliklere göz kapatıp sadece bu olayı gözümüze sokanların ikiyüzlülüğü yine ortaya çıkmıştir. birkaç kürt, 13 yaşındaki kürt çocuğa pusu kurup katledince sesiniz bu kadar gür çıkmıyor. ikiyüzlülük budur.<br/><br/>edit: ekleme<br/>edit2: aşağıdaki müptezel ifade özgürlüğünü kendi tekelinde gören güruhtan olduğu için başkalarının bu hakkı kullanmasına tahammül edememiş ki hakaretle cevap vermiş. kan davasında çocuk öldürenlere sesiniz çıkmaz. evet kürt kürdü öldürünce "bizdendir lo" kafasıyla görmezden geliyorsunuz.
  entry_channels: siyaset,haber
- entry_name: |
    en cool dizi karakteri
  entry_id:  99719261
  entry_writer: robert downey jr jr
  entry_fav_count: 65
  entry_content: |
    (bkz: <a class="b" href="/?q=hank+moody">hank moody</a>)<br/><br/>(bkz: <a class="b" href="/?q=californication">californication</a>)
  entry_channels: anket
- entry_name: |
    27 aralık 2019 yerli otomobilin tanıtımı
  entry_id:  99712089
  entry_writer: decks dark
  entry_fav_count: 64
  entry_content: |
    bir tanıtım. şimdi burada tuhafıma giden ortalama olarak çıkardığım birkaç görüşe değinmek istiyorum:<br/><br/>1-“arkadaşlar bir şeyi de beğenin, çok güzel tasarım olmuş, ba-yıl-dım.”<br/><br/>-tasarım çok güzel olmuş derken yıllardır üzerinde çalışılan yerli ve milli bir üründen mi bahsediyoruz yoksa halihazırda üretilmiş bir modelin katalogdan seçilmesinden mi?<br/><br/>2-“bazıları da gelmiş ‘yok şu parçası yurt dışındanmış da bilmem neymiş’ diyor, arkadaşım günümüz dünyasında %100 yerli diye bir şey yok, bunu o kalın kafana sok!!!”<br/><br/>-e tamam %100 yerli yok da arabanın tanıtımı, “yerli ve milli gururumuz bu arabayı, ülkemizi bölmek isteyen ve bizi çekemeyen haçlı ordusu dış mihraklarla beraber üretiyoruz, artık globalleştik çünkü.” diye mi yapıldı?<br/><br/>3-“arkadaşlar işin içindeyim, şu kadarını söyleyim siyasi rant odaklı bakmayalım, önümüz çok açık, bataryamıza taş değmesin inşaallah.”<br/><br/>-aynı tanıtımı mı izledik bilemiyorum ama siyasi rant için yapıldı mı yapılmadı mı, cevabı ortalama zekası olan birine bırakıyorum. <br/><br/>bir de konudan bağımsız olarak her türlü vasatlığı övüp sonuna da “muhalifspor ;)” yazma geyiği baydı artık.
  entry_channels: haber,otomotiv,ekonomi
- entry_name: |
    erkekte ev araba para tip kariyer aramayan kadın
  entry_id:  99723389
  entry_writer: metaforrr
  entry_fav_count: 61
  entry_content: |
    ananız olan kadındır.
  entry_channels: ilişkiler
- entry_name: |
    masterchef türkiye
  entry_id:  99712444
  entry_writer: 1 days of summer
  entry_fav_count: 49
  entry_content: |
    --- <a class="b" href="/?q=spoiler">spoiler</a> ---<br/><br/>güzide, senin yemeği beğenmedim eti iyi pişmemiş o yüzden normal vereceğim puandan 1 puan kırdım ve 10 üzerinden 11 verdim.<br/>--- <a class="b" href="/?q=spoiler">spoiler</a> ---
  entry_channels: tv,yeme-içme
- entry_name: |
    yerli otomobilin 150 bin tl olacağı gerçeği
  entry_id:  99732282
  entry_writer: cammasaninatinasumuksurenadam
  entry_fav_count: 46
  entry_content: |
    amerikada yerli üretim fort mustang 26.000 dolar iken, bu yerli üretim araç ülkemizde 30.000 dolar dan fazlaya satılacaksa sokayım böyle yerli üretime.
  entry_channels: 
- entry_name: |
    mert armağan
  entry_id:  99717660
  entry_writer: oggiethebrewbuddy
  entry_fav_count: 42
  entry_content: |
    dövmeli bir kızla nişanlanmış ama millete müslümanlık taslıyor çöl kurnazı.
  entry_channels: tv
- entry_name: |
    28 aralık 2019 diyanet'in sigara açıklaması
  entry_id:  99724457
  entry_writer: determonist
  entry_fav_count: 30
  entry_content: |
    olum mal mısınız? sigara tabi ki haram. bunu içen herkes kanser olma ihtimalinin yüksek olduğunu, ömrünün sonundan on sene kestiğini bilerek içer. bileklerini kesip intihar etmekten tek farkı vadesinin uzun süreli olmasıdır. vücuda hiçbir katkısı olmayan, tümüyle israf bir meret, ve neresinden tutarsan tut zararlı. bundan daha haram ne olabilir amk?<br/><br/>diyanet kırk yılda bir doğru bir şey söylemiş ama evet üstteki entry'ler gibi içki ve sigara vergileriyle alınan makam araçları konusunda da ayrı bir fetva bekliyoruz.
  entry_channels: ilişkiler,haber
- entry_name: |
    atatürk'e duyulan sevginin nedeni
  entry_id:  99738558
  entry_writer: zinaedenzidan
  entry_fav_count: 25
  entry_content: |
    arap poposu yalayıp, malum yerlerde badelenen pirimatlara gereken dersi verdiği için.
  entry_channels: siyaset,ilişkiler,tarih
- entry_name: |
    günler 30 saat olsun kampanyası
  entry_id:  99740743
  entry_writer: kazamaranka
  entry_fav_count: 16
  entry_content: |
    ekstra mesai olarak geri döner o 6 saat.
  entry_channels: 
- entry_name: |
    bir kadının asla giymemesi gereken şeyler
  entry_id:  99732253
  entry_writer: getcha pull
  entry_fav_count: 13
  entry_content: |
    ya sizin fikrinizin nasıl bir önemi olabilir ki? aranızda <a class="b" href="/?q=converse">converse</a> diyenler var, töbeler olsun.
  entry_channels: siyaset
- entry_name: |
    en iyi bira mezesi
  entry_id:  99734745
  entry_writer: redkit55
  entry_fav_count: 12
  entry_content: |
    (bkz: <a class="b" href="/?q=biran%c4%b1n+yan%c4%b1nda+iyi+gidenler">biranın yanında iyi gidenler</a>)<br/>daha önce açılmıştı var. aynı başlıkları açmaktan bıkmadınız mı ? <br/><br/>(bkz: <a class="b" href="/?q=ayn%c4%b1+ba%c5%9fl%c4%b1%c4%9f%c4%b1+a%c3%a7mak">aynı başlığı açmak</a>)<br/>(bkz: <a class="b" href="/?q=ba%c5%9fl%c4%b1k+a%c3%a7m%c4%b1%c5%9f+olmak+i%c3%a7in+ba%c5%9fl%c4%b1k+a%c3%a7mak">başlık açmış olmak için başlık açmak</a>)<br/>(bkz: <a class="b" href="/?q=engelle">engelle</a>)(bkz: <a class="b" href="/?q=ba%c5%9fl%c4%b1klar%c4%b1n%c4%b1+engelle">başlıklarını engelle</a>)
  entry_channels: anket,yeme-içme
- entry_name: |
    doğu ekspresinde dans eden türbanlı bacılarımız
  entry_id:  99738902
  entry_writer: inlovewithkafka
  entry_fav_count: 12
  entry_content: |
    türbanlı olmasalardı ne tepki alacaklardı? muhtemelen ne güzel eğleniyorlar diyecektiniz değil mi?<br/>türbanın manasının içinin boşaltıldığına katılıyorum, bilinçsiz türbanlı sayısı bilinçliden çok, ama bu onları bağlayan bir şey. ama bu size bir şeyi sırf türbanlı olduğu için yapmaması gerekirdi diye onu yerme hakkını vermez. herkes aynaya baksın önce.<br/>kendi çapımda inançlıyım ama türban takmıyorum çünkü yaşam standardım onu taşıyabilecek düzeyde değil. ama takana da bir şey deme hakkımız yok.
  entry_channels: haber
- entry_name: |
    atiye (dizi)
  entry_id:  99719485
  entry_writer: zakir
  entry_fav_count: 12
  entry_content: |
    en cok bizim arkeologlarin girdigi tribe guluyorum. hepsinde bi' hazir konu acilmisken ben arkeologum uyandirayim heyy tavri soz konusu. tweetler, storyler vs. yahu kadin diziyi belgeselmis gibi elestiriyor, bu arkeoloji degil diyor, gerceklikten kopuk diyor. agagshdbsbd. istiyor ki yapimci ya da senarist hakemli bir dergiye makale yazar gibi senaryo yazsin, ekranin altindan bkz'lar aksin. bakin bu bir trajedidir. vizyonsuzluk ve kabizliktir. yaziktir.
  entry_channels: tv,teknoloji,spoiler
- entry_name: |
    28 aralık 2019 galatasaray antalyaspor maçı
  entry_id:  99741996
  entry_writer: daha cok beklicez mi abi
  entry_fav_count: 12
  entry_content: |
    şu taylan varken bizi belhanda’ya selçuk’a mahkum edeninin beynini sikeyim.
  entry_channels: spor
- entry_name: |
    bir erkekten soğuma sebepleri
  entry_id:  99740704
  entry_writer: indirkaldirhavalandir
  entry_fav_count: 11
  entry_content: |
    <a class="b" href="/?q=gold+digger">gold digger</a> aglama basligina donusmus sebeplerdir.
  entry_channels: anket,ilişkiler
- entry_name: |
    veda ederken 2019'a bir not bırakmak
  entry_id:  99714442
  entry_writer: tarsusi
  entry_fav_count: 9
  entry_content: |
    huzur içinde uyu baba.
  entry_channels: anket,tarih
- entry_name: |
    star wars serisinin en sevilen karakteri
  entry_id:  99737753
  entry_writer: i was tony montana
  entry_fav_count: 8
  entry_content: |
    (bkz: <a class="b" href="/?q=obi+wan+kenobi">obi wan kenobi</a>)
  entry_channels: anket,sinema
- entry_name: |
    radamel falcao garcia
  entry_id:  99742510
  entry_writer: zalim antuan
  entry_fav_count: 7
  entry_content: |
    getirin ordan dusme hattindaki bir takimi falzao sov yapacak
  entry_channels: spor
- entry_name: |
    taylan antalyalı
  entry_id:  99742235
  entry_writer: the man who fell to arcturus
  entry_fav_count: 6
  entry_content: |
    galatasaray taraftarını her zamanki gibi haklı çıkaran futbolcu. ülkede oyundan, oyuncudan en iyi anlayan taraftar galatasaray taraftarı hakkaten. <br/><br/>not: adana demirspor
  entry_channels: spor
- entry_name: |
    ekşi itiraf
  entry_id:  99713123
  entry_writer: beton baget
  entry_fav_count: 6
  entry_content: |
    dün doğum günümdü ve sadece bir kişi kutladı.
  entry_channels: anket,ilişkiler,ekşi-sözlük
- entry_name: |
    alexander sörloth vs vedat muriqi
  entry_id:  99738765
  entry_writer: ben nereye geldim
  entry_fav_count: 5
  entry_content: |
    arkadaş bir şeyden de zevk almayı bilin ya, ikisi de izlemesi zevkli forvet. yukarıdaki arkadaşa göre rakipler ligin en çok gol atan adamına önlem almıyor demek ki. vedat da hırslı ve iyi bir oyuncu sorloth da, sizin takımda değil diye bok atmaya gerek yok. <br/><br/>tanım: gereksiz bir karşılaştırma.
  entry_channels: spor
- entry_name: |
    üç yıllık profilo buzdolabının çöp olması
  entry_id:  99739958
  entry_writer: isen buga
  entry_fav_count: 4
  entry_content: |
    rezalettir. buzdolabı gibi beyaz eşyaların 10 yıl gitmeleri gerekirken; 3 yıl skandaldır. <br/>geçmiş olsun.
  entry_channels: teknoloji,ekonomi
- entry_name: |
    ümraniye'de noel babaya öfke kusan çarşaflı
  entry_id:  99742508
  entry_writer: maviyi beklerken
  entry_fav_count: 4
  entry_content: |
    bugün sosyal medyada gördüğüm <a rel="nofollow noopener" class="url" target="_blank" href="https://twitter.com/canererdem4/status/1210969635673260037" title="https://twitter.com/canererdem4/status/1210969635673260037">olay.</a> <br/><br/>videoda benim çocuğum bundan etkilenmesin diye bağıran bir çarşaflı var. her sizin istemediğiniz şeyleri engelleyeceksek burası zaten cumhuriyet değil afganistan olur. benim çocuğum da bu çarşaflı kadını görüp etkilenmesini, korkmasını istemiyorum. onu ne yapacağız? kendi düşüncesi dışında yaşayanlardan rahatsız olan biri. eğer bir gün bunların eline kalırsak inandıkları kitapta geçen <a class="b" href="/?q=nisa+suresi+89.+ayet">nisa suresi 89. ayet</a>, <a class="b" href="/?q=tevbe+suresi+5.+ayet">tevbe suresi 5. ayet</a>i uygularlar diye korkuyorum.
  entry_channels: 
- entry_name: |
    antidepresan niyetine izlenecek filmler
  entry_id:  99725492
  entry_writer: fistiginyanindakiayi
  entry_fav_count: 4
  entry_content: |
    <a class="b" href="/?q=tony+gatlif">tony gatlif</a> in çektiği <a class="b" href="/?q=gadjo+dilo">gadjo dilo</a> ısrarla tavsiye edilir.<br/>coşku dolu, hayat dolu, hüzün dolu, eğlence dolu, müzik dolu.. ayrıca muhteşem bir <a rel="nofollow noopener" class="url" target="_blank" href="https://youtu.be/vD_LsHYWMbM" title="https://youtu.be/vD_LsHYWMbM">soundtrack'i</a> vardır.
  entry_channels: anket,ekşi-sözlük,bilim,tarih,sinema,sağlık,haber,yeme-içme
- entry_name: |
    şu an okunan kitap
  entry_id:  99738102
  entry_writer: zu torlak
  entry_fav_count: 3
  entry_content: |
    dönüşüm - franz kafka..
  entry_channels: anket,edebiyat,eğitim
- entry_name: |
    önce arkadaş olalım sonrasına bakarız diyen kadın
  entry_id:  99739861
  entry_writer: eceligelendinazor
  entry_fav_count: 3
  entry_content: |
    aynısı olmasa da geçenlerde çok samimi olduğum bir arkadaşımın başından garip bir olay geçmişti.<br/><br/>ortak arkadaşlarımız dolayısıyla tanıdığımız bir kız vardı. iyisiyle kötüsüyle 4 ay boyunca günaydınlı iyi gecelerli hiç durmadan mesajlaştılar. <br/>arkadaşım (erkek) çok ilgileniyordu. her çarşamba düzenli olarak okulunun çıkış kapısında bekliyor onunla yemek yiyor hesabı arkadaşım ödüyor ve evine birakıyodu. o kadar düzenli bir hal aldı ki kız her çarşamba haber vermemesine rağmen arkadaşımın, onun okulunun önünde olacağını biliyordu. çoğu zaman kapısının önüne sebepsizce sohpet etmeye gidiyordu. <br/>arkadaşım da yapı olarak kendini herkese açmayan bir insandir. bütün gün abartısız tsl, premier lig, laliga , brezilya seri a, ligue 1, euroleague gece de nba izeyen bir insandır ( arkadaşım uni son sınıf) f1, moto gp falan saymıyorum bağımlıyız. <br/><br/>belli bir süre bu böyle devam etti. kız, şakayla karışık bir foto atmıştı arkadaşıma, telefonuna wallpaper olarak yapar mısın yazdığını hatırlıyorum. telefonda 3 ay onun resmiyle gezdi. kız hastalandı, babası doktora götürmedi benim arkadaşım aldı kızı doktora götürdü onun için röntgen de sıra bekledi. kız bir türlü bizim arkadaş grubunun hiçbir zaman öğrenmesini istemiyordu, benim dahi bildiğimi hala bilmiyor kendileri matmazel. <br/>her neyse gelelim son yaşadığımız olaya. dediğim gibi kendisi çok konuşkan bir insan olmadığı için kızdan olumsuz yanıt alacağı korkusuyla kıza ısrarla açılmıyor. ben ise her gün gazlıyorum. - oğlum git konuş artık çok iyisiniz adını koyun falan filan derken bizimki 1 hafta sonra aldı önüne şapkayı düştü yollara. hemen akabinde kıza bir mesaj konuşmamız lazım. arkadaşımın amacı yüz yüze konuşmaktı fakat şuan hatırlamadığım olumsuz şartlar dolayısıyla telefona kaldı konuşma. kızı sormuş ne söyleyeceksin. arkadaşım demiş ya bunu aslında daha önce söylemek isterdim senden hoşlanıyorum . kız hemen tabı r arkadaşlar, ya işte ben öyle hissettirdiysem özür dilerim. ben seni hep arkadaş olarak gördüm. ben sana hiç bir zaman sevgili gibi konuşmadım. sen beni yanlış anladın. bizimki tabi şok. kız bide üstüne diyor ki bu mesajı atmamış sayalım seninle arkadaşlığımız bozulmasın.<br/>tabi ben hemen çakallığı sezince dedim ki; kankaa bu kız çakal, ya senle manıta olsun yada muhttabı kessin yazdırdım. e sonuç olarak ta konuşmaları kesildi ama kız gider ayak bir mesaj attı arkadaşıma, acayip uyuz oldum. her şeyi batırdın ahmet.. (ahmet örnek).<br/>yani arkadaşlar bunu niye yazdım bilmiyorum ama siz siz olun kendinizi kimseye kullandırtmayın. yol yakınken yol verin. yoksa çok benzin yakarsınız.
  entry_channels: anket,ilişkiler
- entry_name: |
    artık eskisi gibi kaliteli filmlerin yapılmaması
  entry_id:  99740033
  entry_writer: atini oksayan kovboy
  entry_fav_count: 3
  entry_content: |
    yeşil yol, esaretin bedeli, titanic gibi insanın içine işleyen filmler artık gelmiyor. varsa yoksa yeşil perdeyle beraber dayıyorlar görsel efekti.
  entry_channels: anket,sinema
- entry_name: |
    ahlaklı vicdanlı dürüst kibar hoşgörülü insanlar
  entry_id:  99740395
  entry_writer: epirusunyayi
  entry_fav_count: 3
  entry_content: |
    kadınların erkeklere “güç, statü, para” olarak, erkeklerin kadınlara “seks objesi” olarak baktığı ülkemizde nesli tükenmekte olan insanlardır. istersen norveç’e filan git abi. sen bilirsin.
  entry_channels: ilişkiler
- entry_name: |
    zenginlerin çirkin olması
  entry_id:  99741807
  entry_writer: yagmurluhavadasusatanadam
  entry_fav_count: 2
  entry_content: |
    çok yanlış.. zengin adamlar çirkin.. kadınlar için bunu söylemek imkansız
  entry_channels: 
- entry_name: |
    en çok küfür yiyen eşyalar
  entry_id:  99738689
  entry_writer: pink strawberry
  entry_fav_count: 1
  entry_content: |
    yemek masamızın sivri köşesi
  entry_channels: anket
- entry_name: |
    sözlükçülerin şu an ihtiyacı olan şey
  entry_id:  99742155
  entry_writer: laughs
  entry_fav_count: 0
  entry_content: |
    karşımdaki adamın susması.<br/>bi sus artık ya, tamam anladık aslansın, kaplansın ama sus lütfen.<br/>bu kadar güzel bir adam, bu kadar boş konuşabilir mi amk ya, ağlayacağım. <br/>halbuki ben böyle hayal etmemiştim bir şeyleri, vay anasını serhat ya <sup class="ab"><a title="(bkz: swh)" href="/?q=swh" data-query="swh">*</a></sup> sinirlerim bozuldu, hayallerim yıkıldı, tansiyonum fırladı ahajahah
  entry_channels: anket,ekşi-sözlük
- entry_name: |
    saniyelik salaklıklar
  entry_id:  99740767
  entry_writer: sadecekitaplar
  entry_fav_count: 0
  entry_content: |
    arabayla eve dönerken ekmek almak için durduk.<sup class="ab"><a title="(bkz: ekmeği alacak şanslı kişi de benim tabi)" href="/?q=ekme%c4%9fi+alacak+%c5%9fansl%c4%b1+ki%c5%9fi+de+benim+tabi" data-query="ekmeği alacak şanslı kişi de benim tabi">*</a></sup> <br/>ekmeği aldım döndüm, bu arada yol kenarında bizim önümüzde duran başka araç yoktu. bu yüzden direkt en öndeki arabaya gittim. kapıyı bir güzel açtım eğildim tam oturucam bana doğru şaşkın bakan bir çift göz. bi baktım babamla alakası olmayan birisi. adam anladı arabaları karıştırdığımı , sonra gülüştük ve kendi arabamıza döndüm. bi baktım bizimkiler arabanın içinde gülmekten ölüyorlar. benim geçip gittiğimi görmüşler hiç engellememişler sağolsunlar. onlara da eğlence çıktı tabi..
  entry_channels: anket
- entry_name: |
    ilk köpeğinizin ismi
  entry_id:  99720771
  entry_writer: sanfransiskolu mufettis
  entry_fav_count: 0
  entry_content: |
    yine milleti gaza getirip kişisel bilgilerini almak için açılmış bir başlık. yarın öbür gün kişisel hesaplarımı nasıl ele geçirdiler diye merak eserseniz cevabı bu ve bunun gibi başlıklarda yazdıklarınız olabilir.
  entry_channels: anket
- entry_name: |
    ekşi sözlük dertleşecek insan veritabanı
  entry_id:  99713069
  entry_writer: zredust
  entry_fav_count: 0
  entry_content: |
    genelde bafileme amacını gizleyip şarap, gezi, dert falan gibi şeylerin arkasına saklanmış müptezellerin toplandığı tabandır. dertleşeceğim len.
  entry_channels: anket,ilişkiler,ekşi-sözlük,oyun,yeme-içme
- entry_name: |
    bir kadına yazılmış en güzel şiir
  entry_id:  99719585
  entry_writer: abinizjo
  entry_fav_count: 0
  entry_content: |
    “senin dizlerinin üstüne çözmen beni yüceltmez, sadece öldürür” <br/>ibrahim tatlıses. demet akalınlı klibin sonunda yazar.
  entry_channels: anket,edebiyat
---
