---
layout: post
title: 06 Eylül 2019 Ekşi Sözlük Debe
data:
- entry_name: |
    5 eylül 2019 yenikapı ibb kiralık araçlar sergisi
  entry_id:  94913200
  entry_writer: mete han turkyilmaz
  entry_fav_count: 2896
  entry_content: |
    muhtemelen <a class="b" href="/?q=ibb">ibb</a> adına kiralanan araçların bugün, yenikapı miting alanında sergilenmesi durumu. başlığı elimden geldiğince güncelliyorum. atladığım bir gelişme olursa haber verebilirsiniz dostlar.<br/><br/>siz burada kıçıkırık <a class="b" href="/?q=vw+polo">vw polo</a>'ya 180 bin verilir mi verilmez mi diye tartışadurun. bakalım ne <a class="b" href="/?q=s%c3%bcmeyye">sümeyye</a>'ler, ne <a class="b" href="/?q=eymen">eymen</a>'ler, ne <a class="b" href="/?q=%c3%b6mer+faruk">ömer faruk</a>'lar 3 bin motor lüks araçlarla fakir fukaranın vergilerini ezmiş.<sup class="ab"><a title="(bkz: kişi adları temsilidir)" href="/?q=ki%c5%9fi+adlar%c4%b1+temsilidir" data-query="kişi adları temsilidir">*</a></sup><sup class="ab"><a title="(bkz: kimseyi kırmak istemem)" href="/?q=kimseyi+k%c4%b1rmak+istemem" data-query="kimseyi kırmak istemem">*</a></sup><sup class="ab"><a title="(bkz: çocuklarımıza öz türkçe adlar veriyoruz kampanyası)" href="/?q=%c3%a7ocuklar%c4%b1m%c4%b1za+%c3%b6z+t%c3%bcrk%c3%a7e+adlar+veriyoruz+kampanyas%c4%b1" data-query="çocuklarımıza öz türkçe adlar veriyoruz kampanyası">*</a></sup><sup class="ab"><a title="(bkz: bunu da hatırlatmış olalım)" href="/?q=bunu+da+hat%c4%b1rlatm%c4%b1%c5%9f+olal%c4%b1m" data-query="bunu da hatırlatmış olalım">*</a></sup><br/><br/>bakalım almanya bizi ne kadar kıskanıyormuş. bakalım allah, kitap diye diye ne mercedes'ler, ne audi'ler götürülmüş.<br/><br/>not: şu hesapta fotolar gelmeye başlamış:<br/><br/>1-<a rel="nofollow noopener" class="url" target="_blank" href="https://twitter.com/barsinyo/status/1169250219621539842" title="https://twitter.com/barsinyo/status/1169250219621539842">https://twitter.com/…nyo/status/1169250219621539842</a><br/>2-<a rel="nofollow noopener" class="url" target="_blank" href="https://twitter.com/MstSelanik/status/1169379928204292103" title="https://twitter.com/MstSelanik/status/1169379928204292103">https://twitter.com/…nik/status/1169379928204292103</a><br/>3-<a rel="nofollow noopener" class="url" target="_blank" href="https://twitter.com/MstSelanik/status/1169380183436144641" title="https://twitter.com/MstSelanik/status/1169380183436144641">https://twitter.com/…nik/status/1169380183436144641</a><br/>4-<a rel="nofollow noopener" class="url" target="_blank" href="https://twitter.com/MstSelanik/status/1169380968517591040" title="https://twitter.com/MstSelanik/status/1169380968517591040">https://twitter.com/…nik/status/1169380968517591040</a><br/>5-<a rel="nofollow noopener" class="url" target="_blank" href="https://twitter.com/MstSelanik/status/1169384384169959426?s=19" title="https://twitter.com/MstSelanik/status/1169384384169959426?s=19">https://twitter.com/…tatus/1169384384169959426?s=19</a><br/>6-<a rel="nofollow noopener" class="url" target="_blank" href="https://twitter.com/MstSelanik/status/1169384384169959426?s=19" title="https://twitter.com/MstSelanik/status/1169384384169959426?s=19">https://twitter.com/…tatus/1169384384169959426?s=19</a><br/><br/>- lüks araçlar gelmeye başlamış:<br/><a rel="nofollow noopener" class="url" target="_blank" href="https://twitter.com/gazetesozcu/status/1169583606966214656" title="https://twitter.com/gazetesozcu/status/1169583606966214656">https://twitter.com/…zcu/status/1169583606966214656</a><br/><br/>- yahu ibb'ye bağlı tüm araçlar gelse belediye nasıl hizmet verecek? oraya getirilen her araç belli ki <a class="b" href="/?q=ihtiya%c3%a7+fazlas%c4%b1">ihtiyaç fazlası</a>.<br/><br/>- <a class="b" href="/?q=renault+clio">renault clio</a>'yu beğenmeyen arkadaşlar, beğenmemekte haklısınız zira siz zaten bu araçlara tenezzül etmeyecek kadar ballısınız. birkaç kurumdan birkaç maaş alsam ben de beğenmem ama şu an bu aracın en boş modeli, 0.9 motor olanı 92.900 türk lirası. asgari ücret 2020 lira. <br/><br/>üstelik sayıştay raporlarına göre bu araçları 100 km'de 60-80 lt arası yakıt tüketiyor diye göstermişsiniz. ulan 0.9 motoru 9.0 motor falan mı sandınız? bu ne?<br/><br/>rapor:<br/><br/><a rel="nofollow noopener" class="url" target="_blank" href="https://www.sayistay.gov.tr/tr/Upload/62643830/files/raporlar/kid/2017/Belediyeler/İSTANBUL SU VE KANALİZASYON İDARESİ GENEL MÜDÜRLÜĞÜ İSKİ.pdf" title="https://www.sayistay.gov.tr/tr/Upload/62643830/files/raporlar/kid/2017/Belediyeler/İSTANBUL SU VE KANALİZASYON İDARESİ GENEL MÜDÜRLÜĞÜ İSKİ.pdf">https://www.sayistay.gov.tr/…nel müdürlüğü İskİ.pdf</a><br/><br/>raporun haberi:<br/><br/><a rel="nofollow noopener" class="url" target="_blank" href="https://www.yenicaggazetesi.com.tr/iskide-buyuk-yolsuzluk-210941h.htm" title="https://www.yenicaggazetesi.com.tr/iskide-buyuk-yolsuzluk-210941h.htm">https://www.yenicaggazetesi.com.tr/…luk-210941h.htm</a><br/><a rel="nofollow noopener" class="url" target="_blank" href="https://www.google.com/amp/s/indigodergisi.com/2018/11/sayistay-iski-araclari-100-km-63-lt-yakit/amp/" title="https://www.google.com/amp/s/indigodergisi.com/2018/11/sayistay-iski-araclari-100-km-63-lt-yakit/amp/">https://www.google.com/…ari-100-km-63-lt-yakit/amp/</a><br/><br/>bu da araçların ne kadar yakıt tükettiğine dair tablo:<br/><a rel="nofollow noopener" class="url" target="_blank" href="https://eksiup.com/p/wx101618ocr8">https://eksiup.com/p/wx101618ocr8</a><br/><br/>siz burada <a class="b" href="/?q=ek%c5%9fi+s%c3%b6zl%c3%bck+yak%c4%b1t+t%c3%bcketimi+veritaban%c4%b1">ekşi sözlük yakıt tüketimi veritabanı</a> diye "yok klimalı kapalı şu kadar, yüksüz bu kadar" diye yazın. adamlar 1 km'de 420 lt motorin tüketmiş. <a class="b" href="/?q=100+km+gitse">100 km gitse</a>42794 litre` motorin tüketecek. aracı mars yüzeyinde bir km havalandırmayı başarsan bunun küsuratı kadar yakamazsın.<br/><br/>- araç toplama işi pazar gününe kadar sürecekmiş. ifşa pazar günü olacak sanırım.<br/><br/><a rel="nofollow noopener" class="url" target="_blank" href="https://twitter.com/MstSelanik/status/1169509031418970112" title="https://twitter.com/MstSelanik/status/1169509031418970112">https://twitter.com/…nik/status/1169509031418970112</a><br/><br/>- <a class="b" href="/?q=sitede+oturan+basketci+genc">sitede oturan basketci genc</a> dostum şu mesajı gönderdi, buraya ekliyorum:<br/><br/>"selam, mesaj atmak icin hesap actim. ibb kasimpasa ek hizmet binasinda calisiyorum ben. o araclarin hepsi calisanlarin evden ise, isten eve gitmek icin kullandigi araclar. herkese ucretsiz servis hizmeti verilmesine ragmen butun vasifsiz memurlara o araclardan tahsis ediliyordu yillardir. 18-20 kisilik servislere max 3-4 kisi biniyordu. yani bunlar hizmet araclari falan degil. mudurun altindaki makamlara verilen sahsi araclar. su an havuz sistemi uygulanmaya baslandi. sahaya cikmasi gereken mudur yardimcisina soyluyor 5-10 dakikada arac tahsis ediliyor.<br/>kasimpasa rk hizmet binasinin otoparkinda su an yenikapidaki kadar arac var en az, bir kismi da mercedes audi. tahmin ediyorum baska hizmet binalarinda da araclar duruyordur henuz.<br/><br/>bunlari bir sekilde yazabilirseniz sevinirim insanlar dogru olani ogrenir sacma sapan bilmeden yorum yapmazlar."<br/><br/><a rel="nofollow noopener" class="url" target="_blank" href="https://eksiup.com/p/dp1015154nga">https://eksiup.com/p/dp1015154nga</a><br/>(bkz: <a class="b" href="/entry/94927199">#94927199</a>)<br/><br/>- <a class="b" href="/?q=blissray">blissray</a> dostum mobese görüntüsünü eklememi istedi:<br/><br/><a rel="nofollow noopener" class="url" target="_blank" href="https://mobile.donanimhaber.com/store/a2/f6/b9/a2f6b956acac1d706072cd67bba1fd06.png" title="https://mobile.donanimhaber.com/store/a2/f6/b9/a2f6b956acac1d706072cd67bba1fd06.png">https://mobile.donanimhaber.com/…72cd67bba1fd06.png</a><br/><br/>- <a class="b" href="/?q=psikopatkoala">psikopatkoala</a> dostum "altı üstü <a class="b" href="/?q=clio">clio</a> yhaa" diyenler için şöyle bir bilgi paylaştı:<br/><br/>"o beğenmedikleri clio için yıllık kiralama fiyatı aldım, aylık 3.150 tl +kdv ve sadece aylık 3 bin km sınırla."<br/><br/>- <a class="b" href="/?q=clochardulan">clochardulan</a> dostum şu mobese görüntüsünü göndermiş iki saat önce. şu an ekleyebildim.<br/><br/><a rel="nofollow noopener" class="url" target="_blank" href="https://i.hizliresim.com/P72kdb.png">https://i.hizliresim.com/p72kdb.png</a><br/><br/>- <a class="b" href="/?q=always+independent">always independent</a> dostum anlık mobese görüntüsünü göndermiş:<br/><br/><a rel="nofollow noopener" class="url" target="_blank" href="https://resimag.com/p1/2e1e32d0b5b.png">https://resimag.com/p1/2e1e32d0b5b.png</a>
  entry_channels: siyaset,haber,otomotiv,ekonomi
- entry_name: |
    imamoğlu'nun araç sergisi aldatmacası
  entry_id:  94924136
  entry_writer: yazarium
  entry_fav_count: 1514
  entry_content: |
    hayırdır genç, çok heyecan yapmışsın??<br/><br/>(bkz: <a class="b" href="/?q=korkma+titre">korkma titre</a>)
  entry_channels: siyaset,tv,haber,otomotiv
- entry_name: |
    uzungöl'ün son hali
  entry_id:  94925494
  entry_writer: oyuncocugu
  entry_fav_count: 873
  entry_content: |
    vay be! bir de şurayı hala görmeye giden var. beton görmek isteyen ankara' ya falan gitsin, en azından daha az arap görür.
  entry_channels: siyaset,seyahat
- entry_name: |
    rte'nin suriyelilere bahçeli ev yapalım önerisi
  entry_id:  94933459
  entry_writer: atcephesi
  entry_fav_count: 806
  entry_content: |
    yazıklar olsun.
  entry_channels: siyaset
- entry_name: |
    bmc'nin ilk yerli pickup'ı
  entry_id:  94927343
  entry_writer: belkisehrebirfilmgelir
  entry_fav_count: 498
  entry_content: |
    başıma bir şey gelmeyecekse beğendiğim bir hamledir. böyle böyle 100 yıl sonra uzay çağını yakalayacakmışız gibime geliyor. <br/><br/>tasarımı bence hoş. pek özgün olmasa bile beğendim yani. tabii özelliklerini de incelemek gerekiyor. ekşi ergenleri genelde gmc veya hummer kullandıkları için sırf yerli diye yermeye başlr az sonra.
  entry_channels: siyaset,haber,otomotiv,seyahat
- entry_name: |
    milli nükleer bombaya isim önerileri
  entry_id:  94911839
  entry_writer: orsidorsi
  entry_fav_count: 392
  entry_content: |
    15 temmuz milli irade nükleer bombası.
  entry_channels: siyaset,anket,bilim
- entry_name: |
    ekrem imamoğlu'nun yine tatile çıkması
  entry_id:  94934510
  entry_writer: emperor of cydonia
  entry_fav_count: 363
  entry_content: |
    döndüğünde 1 posta daha aktroll şutlayıp malum vakıfların arpasını kesmek için enerji topluyordur.
  entry_channels: siyaset,tv,haber,seyahat
- entry_name: |
    a haber ve sabah'ın ağlamaklı israf sergisi haberi
  entry_id:  94934426
  entry_writer: buyuyup buyuk adam olacaktim
  entry_fav_count: 348
  entry_content: |
    "müdür yardımcılarının evde bakım için kullandığı clio araçları makam aracı gibi gösteriyorlar" dedi ya la :)<br/><br/>müdür yardımcısı bizzat evlere cliolarla bakım yapmaya gidiyormuş galiba.
  entry_channels: siyaset,tv,haber
- entry_name: |
    clio model arabaya makam aracı demek
  entry_id:  94938193
  entry_writer: dahaka dahaka
  entry_fav_count: 239
  entry_content: |
    makama tahsis edilen her araç, makam aracıdır.
  entry_channels: siyaset,anket,haber,otomotiv
- entry_name: |
    plajda dikkat edilmesi gereken görgü kuralları
  entry_id:  94927146
  entry_writer: delireaz
  entry_fav_count: 174
  entry_content: |
    -telefondan müzik açılıp saçma sapan şarkılar ile milletin kulaklarına ve keyfine tecavüz edilmez.<br/>-çöpler arkada bırakılmaz.
  entry_channels: anket
- entry_name: |
    5 eylül 2019 fatih terim'in 4 maç ceza alması
  entry_id:  94941406
  entry_writer: muayeneci
  entry_fav_count: 159
  entry_content: |
    rizespor başkanına verilen cezadan sonra dalga mı geçiyorsunuz. boyle ayrım yok formanızı giyin renginiz belli olsun.
  entry_channels: spor,haber
- entry_name: |
    30+ yaşında kapılmamış mükemmel bekar
  entry_id:  94926677
  entry_writer: ariftotales
  entry_fav_count: 123
  entry_content: |
    bi bitmediniz aq izdivaç programı mı lan burası?
  entry_channels: anket,ilişkiler
- entry_name: |
    fenerbahçe şampiyon olsun diye verilen mücadele
  entry_id:  94942840
  entry_writer: quat
  entry_fav_count: 112
  entry_content: |
    her zaman ki gs taktiği. çirkeflik yap ceza alınca fenere saldır. lobini kullan hakemleri etki altına al şampiyon olunca her engellemeye rağmen olduk dersin olamayınca da organize işler dersin.<br/><br/>40 senedir aynı bok. fatih o saçma lafları edince ceza alacağını bilmiyor muydu? eşek gibi biliyordu.
  entry_channels: spor
- entry_name: |
    alkol içenin cennete gidemeyeceği gerçeği
  entry_id:  94912229
  entry_writer: yavlum mithat
  entry_fav_count: 102
  entry_content: |
    bir <a class="b" href="/?q=cennete+gidenin+alkol+i%c3%a7ece%c4%9fi+ger%c3%a7e%c4%9fi">cennete gidenin alkol içeceği gerçeği</a> değildir.
  entry_channels: anket
- entry_name: |
    sözlükte tanışılan kızın güzel çıkması
  entry_id:  94933314
  entry_writer: zekaseksidiraptalioynama
  entry_fav_count: 84
  entry_content: |
    güzel kız olduğuna inanıyorum ama güzel kızın burdan biriyle tanışıp fotoğraf falan atıcağını kendini beğendirmeye çalışacağını sanmıyorum.
  entry_channels: ilişkiler,ekşi-sözlük
- entry_name: |
    hayal edilen vs sahip olunan araç
  entry_id:  94933591
  entry_writer: dewiloguz
  entry_fav_count: 77
  entry_content: |
    hayal edilen : araba<br/>sahip olunan :
  entry_channels: anket,otomotiv,ekonomi
- entry_name: |
    ekrem imamoğlu
  entry_id:  94911184
  entry_writer: zaptir
  entry_fav_count: 55
  entry_content: |
    binali yıldırım gibi bir sürü sene bakanlık ve başbakanlık yapmış, tüm saray ve etrafı tarafından desteklenen çok güçlü bir adaya karşı istanbul'da %55 oy alarak ibb başkanı olmuş kişi. nokta.
  entry_channels: siyaset,haber
- entry_name: |
    5 eylül 2019 türkiye çekya basketbol maçı
  entry_id:  94924052
  entry_writer: caming back
  entry_fav_count: 54
  entry_content: |
    ben türk milli takımını tanıyorsam bu maçı kaybederiz ve eve döneriz. çünkü türklük bunu gerektirir.
  entry_channels: spor
- entry_name: |
    istanbul'da yaşamak
  entry_id:  94917942
  entry_writer: diplomasi
  entry_fav_count: 53
  entry_content: |
    yaşamak denemez aslında, istanbul her ne kadar güzel bir şehir olsa bile 20 milyon nüfusuyla sonuçta kaotik bir şehirdir. insanı bencilleştiren, saygısızlaştıran, kuralları çiğnemeye iten hatta kalabalığıyla daha fazla para kazanmak için aç gözlü yapan bir şehirdir. <br/><br/>yukarıda bir arkadaş demişki, beşiktaş kadıköy sarıyer de oturuyorsan en iyi seçenektir izmir'i övenlere bakmayın ama bu söylenilen semtlerde gerçekten oturmak, kalabalık, curcuna, işten çıkıp bu semtlerin birine gitmeyi hayatında deneyimlememiştir eminim. davulun sesi uzaktan hoş gelir. <br/><br/>belki çalışan olarak istanbulda diğer şehirlere göre daha fazla kazanabilirsiniz ama bir o kadarda fazla harcarsınız. her şey daha pahalıdır, daha kalitesizdir ve daha sağlıksızdır (gdo'ludur, ilaçlıdır).<br/><br/>özetle istanbulda yaşamak, yaşamak değildir. ömrünü harcamaktır.
  entry_channels: ilişkiler,tarih,seyahat
- entry_name: |
    çok param var ama ne yapacağımı bilmiyorum
  entry_id:  94932289
  entry_writer: gitme demiyorum hobi olarak yine git
  entry_fav_count: 48
  entry_content: |
    (bkz: <a class="b" href="/?q=o+kadar+hakl%c4%b1s%c4%b1n+ki">o kadar haklısın ki</a>)
  entry_channels: anket,ilişkiler,ekonomi
- entry_name: |
    kilo vermenin çok zor olması
  entry_id:  94927572
  entry_writer: kodsuzyazilim
  entry_fav_count: 48
  entry_content: |
    yediklerine biraz olsun dikkat ederek hiç de zor olmayandır. asıl zor olan; yediklerine hiç dikkat etmeyip, kilo almayan insanların varlığını kabullenmektir. hayat asla adil bir yer değil.
  entry_channels: anket,sağlık
- entry_name: |
    tff'nin terim'in cezasını 19.05'te açıklaması
  entry_id:  94942582
  entry_writer: brkmu13
  entry_fav_count: 43
  entry_content: |
    kusura bakmayın ama neresinden tutarsanız tutun iyi niyetli olmayan bir açıklama saatidir.<br/><br/>beşiktaş, fenerbahçe, trabzonspor ve diğer futbol takımlarına da yapılmaması gereken bir harekettir.<br/><br/>lütfen empati yapın. takımınızın en önemli oyuncusu veya teknik direktörü(kararın doğru ya da yanlış olmasına girmiyorum) kulübünüzün kuruluş yılını temsil eden saatte ceza alırsa ne düşünürsünüz?<br/><br/>son olarak, ligimizin ve futbol ortamımızın daha kaliteli olması için herkesin taşın altına elini sokması gerekiyor.
  entry_channels: spor
- entry_name: |
    yurtdışında balayı olmazsa nişanı atarım diyen kız
  entry_id:  94943378
  entry_writer: i fink u freeky
  entry_fav_count: 42
  entry_content: |
    `zaten reis faizleri düşürdü bu neyin korkaklığı ya erkek ol azıcık` diyen bir sevgili iceren serin hikaye.
  entry_channels: ilişkiler
- entry_name: |
    bts'yi atatürk'e tercih eden t.c. gençliği
  entry_id:  94938715
  entry_writer: shah caspian
  entry_fav_count: 42
  entry_content: |
    en çok da "atatürk olmasa k pop'u bilemezdik" yorumu güldürmüştür.<br/><br/>ata'm olmasa adları kim jong olacak türk gençliğidir.
  entry_channels: haber
- entry_name: |
    acılı şalgam suyu içmekteki mantık
  entry_id:  94938705
  entry_writer: musclemania
  entry_fav_count: 41
  entry_content: |
    bak bak bak, adanada acılı içmezlermiş. adana, mersin gibi yörelerde şalgam marketten değil sadece bu işi yapan seyyar vs. tezgahlardan alınır. sarı şişeler tatlı kırmızı şişeler acı olur ve her durumda yanına ekstra acısını da verirler.<br/>acılı şalgam oldukça mükemmeldir ve felaket iştah açar, özellikle et,kebap türleriyle içilebilecek en güzel şeydir.
  entry_channels: anket,yeme-içme
- entry_name: |
    beraber içmek istediğiniz hayvan
  entry_id:  94941126
  entry_writer: perfectcu
  entry_fav_count: 36
  entry_content: |
    zaten ben genelde hayvanlarla içiyorum ama hayvan olduklarını sonradan farkediyorum.
  entry_channels: ilişkiler
- entry_name: |
    varoluş sancısıyla başa çıkmanın yolları
  entry_id:  94932816
  entry_writer: veribigket
  entry_fav_count: 33
  entry_content: |
    ayağa kalkmaya mecali olanlar için hareket ve spor en güzel çıkış yoludur. varoluşun sancısı daha çok gelecek kaygısıyla ilintilidir. spor yaparken sadece o anı yaşarsınız ve salgılanan dopamin sizi geleceği umursamadan yaşamanıza katkı sağlar.
  entry_channels: bilim
- entry_name: |
    irlanda denince akla gelenler
  entry_id:  94918170
  entry_writer: motamot portakal
  entry_fav_count: 29
  entry_content: |
    (bkz: <a class="b" href="/?q=irish+luck">irish luck</a>)<br/><br/>bir kolye istedim irlandadan gelen arkadaşımdan. yahu o yoncayı taktığım anda bir işim mi rast gitmez.
  entry_channels: anket,tarih
- entry_name: |
    fatih terim
  entry_id:  94941530
  entry_writer: ak740
  entry_fav_count: 28
  entry_content: |
    - 2 maç versek derbide takımın başında bu adam<br/><br/>+ 3 maç verelim?<br/><br/>- çok belli olmaz mı?<br/><br/>+ o zaman 4 verelim tahkimde 3’e indiririz hem kimse çakozlamaz, heheh.
  entry_channels: spor,siyaset
- entry_name: |
    5 eylül 2019 trabzonspor kulübü şike açıklaması
  entry_id:  94925172
  entry_writer: keskin kurpe
  entry_fav_count: 28
  entry_content: |
    benim bu konuda aklımın almadigi bir husus var. <br/><br/>hem fenerli hem de chp'li bir şahıs, "şike yoktur ama ayakkabı kutuları vardır!" mi diyor şimdi şaka maka?
  entry_channels: spor
- entry_name: |
    yazarların silmek istediği duygusu
  entry_id:  94935884
  entry_writer: spaykymoon
  entry_fav_count: 23
  entry_content: |
    empati duygumu silmek isterdim. bünyeye çok zarar veriyor.
  entry_channels: anket,ekşi-sözlük
- entry_name: |
    çift anadal yapıp yumurta bile kıramayan kadın
  entry_id:  94940554
  entry_writer: firatsuyukanakiyor
  entry_fav_count: 21
  entry_content: |
    öyle bir kadına bu zamana kadar hiç rastlamadım ki kendim de çift anadal yapıyorum ve en az 10 tane çift anadal yapan kadın tanıyorum ve hiçbiri bu tanıma girmiyor. mesela çift anadal yapan bu kadınlardan biri hem çalışır hem okur, hem de arkadaşlarını,dostlarını her fırsatta evine davet eder muhteşem yemekler yapar diğeri ise okuldan fırsat bulduğu her dakikayı engelli kardeşi ile geçirir, hasta babasıyla ilgilenir ve çok da güzel hatay yemekleri yapar. hepsi ile gurur duyuyorum hepsi muhteşem kadınlar.
  entry_channels: ilişkiler,eğitim
- entry_name: |
    4 eylül 2019 erdoğan'ın nükleer füze açıklaması
  entry_id:  94909748
  entry_writer: madonnanin yagli zencisi 2
  entry_fav_count: 18
  entry_content: |
    turkiye'nin de imza attigi <a class="b" href="/?q=non+proliferation+treaty">non proliferation treaty</a> diyor ki "her ulke nukleer silah yapmamak kaydiyla bariscil nukleer teknolojilerden faydalanma hakkina sahiptir". yani ulkende nukleer santral olsun istiyorsan nukleer silah yapmama sozu veriyorsun. keza nukleer silah sahibi ulkeler de bu silahlari anlasmaya taraf ulkelere vermeme sozunu veriyor. yani teknik olarak putin, "genc adamsin, al su nukleer fuzeyi yaninda bulunsun" diyemez.<br/><br/>--- <a class="b" href="/?q=spoiler">spoiler</a> ---<br/>under article ı of the npt, nuclear-weapon states pledge not to transfer nuclear weapons or other nuclear explosive devices to any recipient or in any way assist, encourage or induce any non-nuclear-weapon state in the manufacture or acquisition of a nuclear weapon.<br/><br/>under article ıı of the npt, non-nuclear-weapon states pledge not to acquire or exercise control over nuclear weapons or other nuclear explosive devices and not to seek or receive assistance in the manufacture of such devices. under article ııı of the treaty, non-nuclear-weapon states pledge to accept ıaea safeguards to verify that their nuclear activities serve only peaceful purposes<br/>--- <a class="b" href="/?q=spoiler">spoiler</a> ---<br/><br/><a class="b" href="/?q=akp%27li+de%c4%9filim+ama+devletimin+yan%c4%b1nday%c4%b1m">akp'li değilim ama devletimin yanındayım</a> tayfa..haci size laf yetistirmeyi bile kendime zul sayiyorum.<br/><br/>aslinda muhabbet donup dolasip suna geliyor..<br/>ulkemizin ciddi bir kesmi amerika-turkiye savasinin kacinilmaz oldugunu dusunuyor. hal boyle olunca entegre edemesek de s-400 alalim, cok da hayalet olmasa da su-57 alalim, nukleer doner bicagiyla dalalim abi modunun ciddi bir alicisi var.
  entry_channels: siyaset,teknoloji,haber
- entry_name: |
    czn burak'ın recep tayyip erdoğan sevgisi
  entry_id:  94943184
  entry_writer: gorgiasbenimoglumdu
  entry_fav_count: 16
  entry_content: |
    bu tip gereksiz böcekler ile alakalı başlık açmayın arkadaşım. sen bunu açarak var ediyorsun u prim peşinde koşan zavallı ahmakları. ha dersen ki entry giriyor ve sen de bu amaca hizmet ediyorsun, haklısın derim. sil entry'ni, hepimiz silelim büyümeden yok olsunlar.
  entry_channels: siyaset,yeme-içme,seyahat
- entry_name: |
    sevişmeyeceksek görüşmeyelim erkeği
  entry_id:  94928813
  entry_writer: poligontasi
  entry_fav_count: 15
  entry_content: |
    bunu ahlakla, düzgün olmakla, karakteri bozuklukla eş anlamlı hale getirmek biraz fazla romantik. sizin beklenti ve ihtiyaçlarınızla karşınızdakinin beklenti ve ihtiyaçları örtüşmüyor diye o kişiyi kötü olarak niteleyemezsiniz. tercihtir ve tercihiniz değilse sevişmez siniz. <br/><br/>asıl dikkat etmeniz gereken kitle <a class="b" href="/?q=sevi%c5%9fmek+i%c3%a7in+a%c5%9f%c4%b1k+numaras%c4%b1+yapmak">sevişmek için aşık numarası yapmak</a> kitlesidir. avını gözüne kestirir günlerce haftalarca sabreder ve o başta söylemediği niyeti ortaya çıkar. toplumun bir kesimini bu davranış şekli vicdanen rahat hissettiriyor olmalı ki tercih ediliyor. <br/><br/>sevişmeyeceksek görüşmeyelim erkeğinin de evrimleşeceği nokta burasıdır aslında. bir kaç dürüst girişim sonrası başarısızlığa uğrayınca çareyi aşık numarası yapmakta bulacak. çünkü bedeni o sıvıyı boşaltmaya programlı ve o eylemi bir şekilde yapmalı.
  entry_channels: ilişkiler
- entry_name: |
    arkadaşlığın bir üstü sevgililiğin bir altı
  entry_id:  94943452
  entry_writer: chicalataa
  entry_fav_count: 12
  entry_content: |
    tatlı tatlı flört etmektir. bir süre sonra sıkıcı oluyor tabii. ilerleme olmayınca gerileme oluyor. yani soğumaya başlıyor insan.
  entry_channels: anket,ilişkiler
- entry_name: |
    imamoğlu'nun araç alım ihale şartları
  entry_id:  94943698
  entry_writer: ben buyuyunce melis danismend olcam
  entry_fav_count: 9
  entry_content: |
    sizler şimdi kaynak ister ama verilen kaynağı ya açıp okumaz ya da rağbet etmezsiniz demişsiniz ancak sizler de kaynağın doğruluğundan bir haber entry girersiniz. bahsettiğiniz tüm maddeleri önceki yönetim tarafından belirlenmiş olan bir ihaledir bu, günümüz yönetimi tarafından ihale iptal edilmiştir.<br/><br/><a rel="nofollow noopener" class="url" target="_blank" href="https://eksisozluk.com/entry/94929849" title="https://eksisozluk.com/entry/94929849">şu entry</a>i giren arkadaş hislerime tercüman olmuş.
  entry_channels: siyaset,tv,haber,otomotiv,ekonomi
- entry_name: |
    behzat ç.
  entry_id:  94930041
  entry_writer: sphagetti
  entry_fav_count: 9
  entry_content: |
    --- <a class="b" href="/?q=nolur+nolmaz+spoiler">nolur nolmaz spoiler</a> ---<br/><br/>sezarin hakki sezara. evet bir onceki bölüm cok havada kalmisti ama bu bölüm gayet iyiydi. hem cinayet arastirmasi, hem agir abiler, hem hayalet akbaba muhabbetleri, sevket abinin basina gelenler gayet iyiydi. ızlemeden kotuydu, artik heycanlandirmiyor diyenlere inanmayin. gayet iyi bir bölümdü. ayrica şule'nin mevzusu da cok sevindirdi. son olarak en bastaki behzo, ercu ve basgan muhabbeti ve pirzola cok güzel idi. izleyin<br/><br/>--- <a class="b" href="/?q=spoiler">spoiler</a> ---
  entry_channels: tv,sinema,spoiler
- entry_name: |
    galatasaray'a karşı oluşturulan kutsal ittifak
  entry_id:  94943882
  entry_writer: partizan solcu
  entry_fav_count: 8
  entry_content: |
    geçen sene "başakşehir'i şampiyon yapacağız" diye böğürenlerin de taraf olduğu ittifaktır.
  entry_channels: spor
- entry_name: |
    fatih terim'e verilen cezayı 19.05'te açıklamak
  entry_id:  94942861
  entry_writer: godot is dead
  entry_fav_count: 8
  entry_content: |
    galatasaraylı olmamama rağmen art niyetle yapılmış olduğu gayet açık ve nettir. hatta ve hatta fatih terim'in 4 maçlık ceza almasını gerektirecek bir söylemi de bulunmamaktadır. objektif olarak yaklaşmak gerekirse, şike yapan kulüplere yakınlığı ile bilinen nihat özdemir yönetiminin galatasaray'a meydan okuma çabasından başka bir şey değildir.<br/><br/>not: 2010-2011 sezonu şampiyonu.
  entry_channels: spor,haber
- entry_name: |
    ben gökhan çınar sorularınızı bekliyorum
  entry_id:  94933785
  entry_writer: absolutzer0
  entry_fav_count: 8
  entry_content: |
    ''klinik psikologum'' deyip buradan nasıl sorular gelmesini bekliyorsunuz? insanların dertlerini dinleyip genel-geçer cevaplar vereceksiniz ve bu da bir boka yaramayacak meslek erbabı olarak siz de bunu biliyorsunuz. o zaman buraya gelmenizin amacı ne? pr yapıp seans ücretinizi daha da mı yükseltmek?
  entry_channels: ekşi-sözlük,sağlık
- entry_name: |
    sigara zammı sonrası yazarların aldığı kararlar
  entry_id:  94910266
  entry_writer: circassia
  entry_fav_count: 6
  entry_content: |
      (nokta)mina koyim içmeycem şunu. her defasında bakkalda. bazen para üstü alırken “halk ağır vergiler altında eziliyordu...” demeyi ihmal etmiyorum.
  entry_channels: anket,ekşi-sözlük,sağlık,yeme-içme
- entry_name: |
    5 eylül 2019 mustafa cengiz açıklaması
  entry_id:  94945791
  entry_writer: nick the whooper
  entry_fav_count: 6
  entry_content: |
    ulan adamlar utanmadan 19.05de ceza açıklıyor , düşmanlıklarını gizlemiyorlar bile. buna tepki verince de ağlıyorlar diyorsunuz. silahım olsa hakemi vururdum diyenlere 11 gün ceza verdiler sesiniz çıktı mı ibneler ? sizin insanlığınıza tüküreyim.
  entry_channels: spor,haber
- entry_name: |
    terim'in disiplin kuruluna çağırılması gerekliliği
  entry_id:  94945174
  entry_writer: hayaloglu
  entry_fav_count: 5
  entry_content: |
    istediği her şeyi, istediği zaman; ima ederek de değil, doğrudan söyleyerek hayatını geçirmiş birisi ve destekçisi olan kesime bu durum garip ve yanlış gelebilir. lakin sen istediğin gibi istediğin kişi veya kurumu, dilediğin şekilde yaftaladıktan sonra hiçbir şey olmamış gibi davranamazsın, bu cezayı sen hak ettin, kazandın; genellikle hak etmeden kazanmaya alıştığınız için bu durum garip geliyor olabilir, hak veriyorum.
  entry_channels: spor
- entry_name: |
    5 eylül 2019 pfdk kararları
  entry_id:  94942162
  entry_writer: bize heryer trabzon
  entry_fav_count: 5
  entry_content: |
    fenerbahçe'ye, trabzonspor maçında ettiği küfürlerden dolayı ceza verilmemesi rezaletini barındıran kararlardır.
  entry_channels: spor
- entry_name: |
    zeki uzundurukan
  entry_id:  94944416
  entry_writer: metin oralet
  entry_fav_count: 4
  entry_content: |
    bugünkü yayınını izlemedim ama orospu çocuklarını rahatsız ettiğine göre demek ki doğru konuşmuş.
  entry_channels: spor
- entry_name: |
    izmir'in en iyi üç dondurmacısı
  entry_id:  94923161
  entry_writer: elista
  entry_fav_count: 4
  entry_content: |
    topunu 5 liradan fazlaya satmıyorsa kötü dondurması olduğunu öğrendiğimiz dondurmacılardır.<br/><br/>şekilciliğinizi seveyim.
  entry_channels: yeme-içme,seyahat
- entry_name: |
    5 eylül 2019 galatasaray'ın pfdk kararına tepkisi
  entry_id:  94944713
  entry_writer: zimmermann telegram
  entry_fav_count: 4
  entry_content: |
    geçen yıl 3 kupanın üçünü de almış bir kulübün hebele hübelemesi olacaktır. lig henüz daha doğru düzgün oynanmadan yok hakkımız yendi vs gibi açıklamalar yapılacak, değerli galatasaray taraftarının popülist damarına basılacak vs vs... olacaktır. bir takım yatırım yaptı diye şampiyon olmak zorunda değildir. önce sahada futbol sonra saha dışında demeç lütfen. fatih terim federasyon ile kendi meselesini çözemedi ise bunu galatasaray kimliğinden bağımsız yapmalı. galatasaray camiası zaten arkamda diyerek hoyrat ve hovarda açıklamalar yaptığı için ceza almaması sürpriz olurdu. kendi camiasına zarar veriyor bu şekilde. sonra insanlar da inanıyor "17 takım( hatta belki alt ligler bile) artı federasyon artı mhk galatasaray'a karşı." temcid pilavıdır bu terimin.
  entry_channels: spor
- entry_name: |
    nihat özdemir
  entry_id:  94941896
  entry_writer: everythings on the tabela
  entry_fav_count: 1
  entry_content: |
    ülke futbol tarihinin en yanlı federasyon başkanı, reis’in has adamı. bu devran döndüğünde seninle de görüşücez nihat.
  entry_channels: spor,ekonomi
- entry_name: |
    geceleri deli gibi horladığı düşünülen ünlüler
  entry_id:  94943316
  entry_writer: boylamdevlet
  entry_fav_count: 0
  entry_content: |
    <a class="b" href="/?q=ba%c5%9fak+purut">başak purut</a> <sup class="ab"><a title="(bkz: kanzuk)" href="/?q=kanzuk" data-query="kanzuk">*</a></sup>
  entry_channels: anket,magazin
---
