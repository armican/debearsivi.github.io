---
layout: post
title: 01 Mayıs 2019 Ekşi Sözlük Debe
data:
- entry_name: |
    yurt dışına yerleşip pişman olmak
  entry_id:  89513574
  entry_writer: hand of fate
  entry_fav_count: 535
  entry_content: |
    sakinlikten bunalırsanız kahvaltıda t24 okuyup, akşam haberlerinde fox tv izleyin, yatmadan önce verilmiş sadakam varmış diyerek misler gibi uyursunuz.
  entry_channels: seyahat
- entry_name: |
    coca-cola memleket apartmanı reklam filmi
  entry_id:  89525036
  entry_writer: mikail bey olmez
  entry_fav_count: 525
  entry_content: |
    türbanlı bacının kocası yusuf neden karısının karşı komşuyla konuşmasına izin veriyor o kısmı anlamadım. bir de öğrencilere neden ev vermişler ki, komşunun karısına kızına sarkar.<br/><br/>memleket apartmanı olmaz böyle.
  entry_channels: tv,yeme-içme
- entry_name: |
    30 nisan 2019 alanya müftülüğü paylaşımı
  entry_id:  89530938
  entry_writer: nedstarkolmemeli
  entry_fav_count: 472
  entry_content: |
    alanya müftülüğüne, dinin insan ve yaratan arasında olduğundan dolayı kimsenin hangi dine mensup olduğunu kesin bilemeyeceğimize ayrıca daha önce kendi ifade etse de vefat etmeden önce hangi dine mensup olduğunu tahmin etmemiz mümkün olmayacağından dolayı kendilerine bok yemek düştüğünü belirtirseniz memnun olurum. işlerini yapsınlar diyeceğim ama bilmedikleri bir dinin görevlileri olarak çalışıp para alan insanlardan bu bile beklenemez.
  entry_channels: haber
- entry_name: |
    tcmb'nin enflasyon hedefini %5 olarak açıklaması
  entry_id:  89520653
  entry_writer: bunungibilerdebokyerler
  entry_fav_count: 466
  entry_content: |
    14.6 'dan 1'i at ne kaldı 4.6, ters çevir 6.4<br/>14.6'dan 6.4'ü çıkar kaç kaldı:8.2<br/>8.2'yi ters çevir 2.8. çıkar 8.2'den 2.8'i kaç kaldı: 5.4 işte hesap bu kadar basit. ben ikna oldum
  entry_channels: ekonomi
- entry_name: |
    kadınlarla dolu bir işyerinde çalışmak
  entry_id:  89518176
  entry_writer: oyuncak bulut
  entry_fav_count: 424
  entry_content: |
    yaz kış klima sıcakta çalışır.<br/>koca iş yeri fırına döner ama hâla üşürler.
  entry_channels: ilişkiler
- entry_name: |
    göğüs kıllarını tıraş etmeyen erkek
  entry_id:  89529079
  entry_writer: incilizum
  entry_fav_count: 350
  entry_content: |
    benim gibi barzodur.<br/>barzolar yeşillendirsin batak atak.
  entry_channels: anket,ilişkiler
- entry_name: |
    türk kızlarını hayata küstüren rus hatun
  entry_id:  89532134
  entry_writer: oyuncak bulut
  entry_fav_count: 314
  entry_content: |
    pavyon kafalı tiplerin yoluna güller döktüğü kadındır.<br/>bu ne len?
  entry_channels: anket,ilişkiler
- entry_name: |
    1 yaşındaki kız ile cima yapmadan evlenilebilir
  entry_id:  89530695
  entry_writer: kissofh3ll
  entry_fav_count: 302
  entry_content: |
    ateist mantığıyla yaklaşalım diyelim diyen bir zır deliye ait beyandır. valla ateist mantığıyla yaklaşırsak senle öyle bir cima ederler ki oturamazsın.
  entry_channels: haber
- entry_name: |
    mastürbasyon yaparken eşten başka birini düşünmek
  entry_id:  89526941
  entry_writer: fasulya
  entry_fav_count: 253
  entry_content: |
    eşini düşünüyorsan neden hayalinde yapıyorsun.
  entry_channels: anket,ilişkiler
- entry_name: |
    30 nisan 2019 venezuela darbesi
  entry_id:  89525932
  entry_writer: seryer
  entry_fav_count: 250
  entry_content: |
    10 gün önce ülke başkentinde ana muhalefet lideri diri diri yakılmak istenen ülkede yaşayan orta dogulularin komiklik yaptığı başlık.<br/>7 haziran - 1 kasım ülke tarihinin en karanlık dönemlerinden birini yasamamis gibisiniz.<br/>3 yıl önce kendi elleriyle besledikleri cemaatçi grup tarafından darbe girişimine maruz kalmamış gibi şakalar yapabiliyosunuz.<br/>seçim sonuçlarını kabul etmeyen bir diktatörün ülkesinde yasamiyormuscasina baskalarinin az gelişmisligiyle alay ediyosunuz.<br/>sizin bu ağlanacak halinize gülen şımarıkliginizla kim alay etsin acaba?
  entry_channels: siyaset,haber
- entry_name: |
    oğluna 4 yıl boyunca tecavüz eden baba
  entry_id:  89524760
  entry_writer: normal esteban
  entry_fav_count: 201
  entry_content: |
    iyi hal indiriminin uygulandığı babadır. mahkeme esnasında tecavüze yeltenmemesi iyi hal sayılıyor sanırım.
  entry_channels: haber
- entry_name: |
    istiklal caddesi ruhunun ölmesi
  entry_id:  89531910
  entry_writer: mor g
  entry_fav_count: 181
  entry_content: |
    uzun bir aradan sonra dün bir iş sebebiyle zorunlu olarak gittiğim istiklal caddesinde sonuç olarak vardığım önerme.<br/><br/>şamda bağdatta bir yerde geziyorum sanki , artık mağazalar bile yavaş yavaş arap esnafın eline geçmiş , her 50 metrede bir arapça şarkılar söyleyen sokak grupları da cabası. istanbul'da kesinlikle gidilmeyecek yerler listesine eklenmiştir.
  entry_channels: seyahat
- entry_name: |
    lenovo marka laptopların çöp olması
  entry_id:  89522299
  entry_writer: ebedi edebiyat
  entry_fav_count: 166
  entry_content: |
    2012 yılında aldığım 2.2 ghz 6 gb ram 2 gb nvidia gt 635m core i7 lenovo z580 dizüstüm hâlâ taş gibi çalışarak siz ve sizin gibi düşünenlerle birlikte zamana da meydan okumaktadır.<br/><br/>reddettiğim düşünce.<br/><br/>düzeltme: z580 kullanıcılarından onlarca mesaj geliyor. hepsi aynı fikirde. bir efsanenin dirilişine tanık oluyoruz adeta.<br/><br/>z500 serisinin diğer cihazlarını kullananlardan da birçok mesaj aldım onlar da aynı şekilde düşünüyor.<br/><br/>efendim durduramıyoruz yıllanmış g serisi kullananlardan da memnuniyet mesajları geliyor.
  entry_channels: 
- entry_name: |
    ekrem imamoğlu'nun ibb'ye tc ibaresini eklemesi
  entry_id:  89525096
  entry_writer: mdh
  entry_fav_count: 130
  entry_content: |
    gerçekleştiği taktirde en çok hangi kesimden tepki gelecek ben asıl onu merak ediyorum.<br/>terörist dedikleri hdp'lilerden mi, yoksa vatan millet sevdalısı akp'lilerden mi?<br/>bakalım t.c ibaresi en çok kimi rahatsız ediyor.<br/>bekleyip görelim.
  entry_channels: siyaset,haber
- entry_name: |
    game of thrones
  entry_id:  89511362
  entry_writer: bananzula
  entry_fav_count: 123
  entry_content: |
    --- <a class="b" href="/?q=spoiler">spoiler</a> ---<br/><br/>bu sözlük <a class="b" href="/?q=29+nisan+night+king%27in+%c3%b6l%c3%bcm%c3%bc">29 nisan night king'in ölümü</a> diye başlık açan bir orospu çocuğunu da gördü, sabah sözlüğe girer girmez yedim spoiler'ı.<br/><br/>çünkü amk sözlüğünde moderasyon denen bir sik yok, sen 8 sene diziyi izle ama ergen bir orospu çocuğu yüzünden bütün dizi zevkinin içine edilsin.<br/><br/>misal ben <a class="b" href="/?q=malum+ki%c5%9finin+g%c3%b6t%c3%bcne+soktu%c4%9fu+d%c3%bcr%c3%bcm">malum kişinin götüne soktuğu dürüm</a> diye başlık açsam acaba kaç dakikada silinir entryim?<br/><br/>sikeyim sizin yapacağınız işi. uçuruyorsanız uçurun amına koduklarım.<br/><br/>--- <a class="b" href="/?q=spoiler">spoiler</a> ---<br/><br/>edit1: ''sen de izlemeden girmeseydin sözlüğe'' diyen arkadaşı allaha havale ediyorum.<br/>edit2: yazar bir arkadaşımız kuzeninin projesi ile alakalı anketini paylaşmamı rica etmiş, onu iliştiriyorum buraya.<br/><br/><a rel="nofollow noopener" class="url" target="_blank" href="https://docs.google.com/forms/d/e/1FAIpQLSdq9FnUB7l6oCeo6Qw4dCciWg6jwrrPZe6tGza4wYKKKeFJCg/viewform?vc=0&amp;c=0&amp;w=1" title="https://docs.google.com/forms/d/e/1FAIpQLSdq9FnUB7l6oCeo6Qw4dCciWg6jwrrPZe6tGza4wYKKKeFJCg/viewform?vc=0&amp;c=0&amp;w=1">kısa bir anket</a>
  entry_channels: tv,edebiyat,spoiler
- entry_name: |
    israil'in türkiye'yi 1 günde işgal etmesi
  entry_id:  89508916
  entry_writer: dergul
  entry_fav_count: 112
  entry_content: |
    neden bunu yapsın ki? tarımımızın, onların kısır tohumuna bağımlı olması bile bir tür işgaldir. gelişmiş ülkeler, işgali ekonomik olarak yapmayı tercih ediyor. askeri işgal en seçenek olarak görüyorlar. istediğiniz kadar süslü cümleler kurun ama, ar-ge'ye önem verilmiyorsa, üretmek yerine, satın almak(yada satmakı) tercih ediliyorsa, kaynaklar har vurup, harman savruluyorsa, işgal çoktan başlamış demektir. artık yeni savaş alanları, coğrafi alanlar değil, ticarettir.
  entry_channels: siyaset
- entry_name: |
    bayan kadın sorunsalı
  entry_id:  89534975
  entry_writer: neyazacaktim
  entry_fav_count: 111
  entry_content: |
    1- bayan bir dakika bakar mısınız?<br/>2- kadın bir dakika bakar mısınız?<br/>3- hanımefendi bir dakika bakar mısınız?<br/><br/>bana göre <a class="b" href="/?q=han%c4%b1mefendi">hanımefendi</a> en ideali gibi duruyor ama umarım feminist ceza kanununa göre yargılanmam.<br/><br/>(bkz: <a class="b" href="/?q=t%c3%bcrkiye+i%c3%a7in+adalet+vakti">türkiye için adalet vakti</a>)<br/><br/>edit: öncelikle şu 3 örnek soruyu işin gırgırında olduğumu anlamanız için sorduğumu keşke anlasaydınız. neymiş efendim bayan denilince cinsiyetleri belli olmuyormuş. bu nasıl bir savunma allah aşkına. biz erkeklerin iyi ki böyle takıntısı yok. yaşasın biz!<br/><br/>editsonn : çok mesaj geldi, tamamdır. erkeğe erkek denilen yerde, kadına kadın denilmesi gerekiyor,bu konuda herhangi bir zıt düşüncem yok. ama erkeğe bay denilen yerde de kadına bayan denmesinde bir sakınca göremem,kimse kusura bakmasın.
  entry_channels: anket,ilişkiler
- entry_name: |
    hemen adalet partisi
  entry_id:  89532927
  entry_writer: sikintiya gelebilen adam
  entry_fav_count: 105
  entry_content: |
    “seri adalet getir kardeşim.”
  entry_channels: siyaset
- entry_name: |
    kadıköy hamsi pub rezaleti
  entry_id:  89509243
  entry_writer: kafanerdesinolmmahmah
  entry_fav_count: 104
  entry_content: |
    asıl rezalet benimkisi dediğim olay. bizde buraya 4 arkadaş gittik. hesap 800 kusur lira geldi. 8 bira içtik, 12 yazmışlar. 3 meze söyledik 7 yazmışlar. daha neler neler eklemişler. sisirmislerde sisirmisler. kavga dövüş 700'e düşürdük. lan bir de atar yapıyor garsonlar yanlışlık var deyince hesapta. sabah kafamız açılınca farkettik ki aslında 500'e kadar düşürebilirmisiz. kısacası gitmeyin, uzak durun.
  entry_channels: anket,yeme-içme
- entry_name: |
    night king
  entry_id:  89508987
  entry_writer: hawaii de mojito yudumlayan adam
  entry_fav_count: 103
  entry_content: |
    --- <a class="b" href="/?q=spoiler">spoiler</a> ---<br/><br/>gerizekalı senaristlerin elinde heder olmuş karakter.<br/><br/>son bölümdeki yavanlığın sebebi bu karakterin ölmesi değil, oldu bittiye getirilerek öldürülmesidir.<br/><br/>son 3-4 sezonun neredeyse hepsinde ortak bir tema vardı şöyle ki, "winter is coming" sloganının desteklemesi için finalde white walker'lara bi değinilir, ucundan köşesinden de olsa bi gösterilirler, sahneleri olurdu. sezon ilerledikçe bu sahnelerin dozu ve süresi arttı. bu şekilde heyecan da taze tutuldu. bakın taht oyunları var ama arkada da ölüm kol geziyor ve yaklaşıyor denilerek.<br/><br/>şimdi biz bölümü beğenmedik diye sanılıyor ki, arya stark tarafından öldürülmesi zorumuza gitti. ben bu karakterin arya stark tarafından öldürülmesinde hiçbir sakınca görmüyorum, hatta güzel bile olabilirdi çünkü bu işi yapabilecek belki de tek donanımlı karakter oydu. zaten çoğu kişi de anlatmış neden olduğunu ama hoş olmadı, eğreti durdu neden çünkü oldu bittiye getirdiler, salak yerine koydular. kaç sezondur şişire şişire arşa çıkardıkları night king ve ordusunu bir bölümde hiç ettiler.<br/><br/>altı bölüm değil de sekiz bölüm yapacaktınız ama night king'in hikayesini hakkını vererek işleyecek, king's landing'de yürüdüğünü gösterecektiniz. ondan sonra isterse tombalak sam öldürsün banane. şimdi ne heyecanı kaldı bu işin ?<br/><br/>bundan sonra kalmış üç bölüm iki tane paçoz sonucu bile belli olan(bu senaristlerden sürpriz son falan beklemiyorum artık) bir taht kavgasına girecekmiş pehh... <br/><br/>--- <a class="b" href="/?q=spoiler">spoiler</a> ---
  entry_channels: tv,spoiler
- entry_name: |
    dilber ay
  entry_id:  89512028
  entry_writer: cennet plaji
  entry_fav_count: 92
  entry_content: |
    kendisiyle ilgili bir animi paylasmak istiyorum.<br/>1999 depreminde duzcedeydim. henuz 16 yasinda gitarim omzumda alanya'dan sevdigim kizi gormeye duzce'ye gitmistim. babasinin dedigine gore o da findik toplamaya koye gitmis. eli bos dondum. o gun otobus bulamayinca, gece otelde kaldim, yarin geri donecektim memlekete.<br/>o gece deprem oldu, herkes gibi can havliyle sokaga ciktim. sokaklarda binlerce insan, inonu parkinin onune toplanmis. herkes apartmanlara bakiyor. bazi kisilerde balkondan asagidaki kalabaligi izliyor. bu arada bir kac kisi grup olmus, gunesle ayin ciftlesmesine ciplak gozle baktik, allah da bizi cezalandirdi minvalinde tartisiyor. ben de muhabbete dahil oldum. derken sirtimdaki gitari sordu abinin biri. muzisyenmis, onunla muhabbeti koyulastirdik. bana dilber ay'in nisanlisi oldugunu soyledi. dilber ay kim ya dedim. nasil bilmezsin diye kizdi. sabah olunca dilber ay yanimiza geldi. basimdan gecen tum maceralari ona anlattim, kadin yavrum benim sevenleri kavusturmak gerek, ben sana yardim ederim dedi. sonra bunlar parkta kilim attilar, bana tost ismarladilar. kadin paran var mi diye sordu. cebime harclik koydu. ustelik beni hic tanimiyor. akcakoca'da yazliklari varmis. beni de goturmek istediler. ben yok gitmeliyim dedim. sonra vedalasip ayrildik, o bana sanki ailesinden biri gibi sahip cikti. ustelik hic tanimadan. o yuzden onun yeri kalbimde hep ayridir. bugun burda vefat ettigini ogrendim. mekanin cennet olsun guzel ruhlu insan.
  entry_channels: müzik,sinema,magazin,spoiler
- entry_name: |
    30 nisan 2019 şenol güneş-hüsnü güreli toplantısı
  entry_id:  89533977
  entry_writer: cigarettesmokingman
  entry_fav_count: 68
  entry_content: |
    galatasaray'a daha iyi hizmet etmek için tff'deki istifamı geri alıyorum" diyen gs'li tff görevlisi ali dürüst'ün de hazır bulunduğu toplantı. bizimkilerin de yaptığı iş değil, sen beşiktaş'ı şampiyon yapmak için toplantı düzenle o toplantıya ali dürüst'ü de çağır. <br/><br/>(bkz: <a class="b" href="/?q=galatasarayl%c4%b1lar%c4%b1n+niye+b%c3%b6yle+olmas%c4%b1">galatasaraylıların niye böyle olması</a>)<br/><br/>(bkz: <a class="b" href="/?q=galatasarayl%c4%b1+a%c4%9flakl%c4%b1%c4%9f%c4%b1">galatasaraylı ağlaklığı</a>)
  entry_channels: spor
- entry_name: |
    yüksek lisans yaparken çalışan insan
  entry_id:  89533697
  entry_writer: dont waste it taste it
  entry_fav_count: 65
  entry_content: |
    yüksek lisans yaparken aynı zamanda formasyon da alıyor ve bir dil kursunda öğretmen olarak çalışıyordum. sabahları formasyon derslerine gidiyor, öğleden sonradan akşam on bire kadar kıytırık bir paraya öğretmenlik yapıyor, gece de tezimi yazmaya çalışıyordum. ekmeğimi kazanabilmek için ekstradan çeviri yapıyor ve özel dersler de veriyordum hafta sonları. ilk entryde bahsedilen lakaytlığı yapma keyfiyetim olmadı hiç. şimdi bakınca o günkü koşullarımdan sağ çıkabilmiş olmayı mucize gibi görüyorum.
  entry_channels: eğitim
- entry_name: |
    jorah mormont
  entry_id:  89511037
  entry_writer: entryrush
  entry_fav_count: 59
  entry_content: |
    <a class="b" href="/?q=spoiler">spoiler</a><br/><br/><a class="b" href="/?q=spoiler">spoiler</a><br/><br/>khalesi'nin efendi adam jorah yerine piç jon tercihi<br/><br/><a class="b" href="/?q=spoiler">spoiler</a>
  entry_channels: tv,edebiyat
- entry_name: |
    her şeyi açıklayan en kısa söz
  entry_id:  89535350
  entry_writer: notre
  entry_fav_count: 55
  entry_content: |
    kasapların tartışmasında, koyunların taraf tutması, koyunların kaderini değiştirmez.
  entry_channels: anket
- entry_name: |
    en sinir olunan imla hatası
  entry_id:  89509763
  entry_writer: galdorisilra
  entry_fav_count: 47
  entry_content: |
    noktalama işaretlerinden önce bırakılan boşluk. <br/>noktalama işaretlerinden sonra bırakılmayan boşluk.
  entry_channels: ekşi-sözlük,edebiyat,eğitim
- entry_name: |
    30 nisan 2019 eminevim rezaleti
  entry_id:  89530997
  entry_writer: albipacino
  entry_fav_count: 46
  entry_content: |
    şirketin seni bekletmesi veya sana eksik bilgi vermesi değil, senin hala sözleşmeyi okumadan imzalayanlar arasında olman bence rezalettir.<br/><br/>rezalet puanım: 3/10<br/><br/>t: rezaletimsi.<br/><br/>edit: imla
  entry_channels: ekonomi
- entry_name: |
    samwell tarly
  entry_id:  89525053
  entry_writer: 14 love
  entry_fav_count: 39
  entry_content: |
    silah arkadaşlarının bedduasını alan adam.
  entry_channels: anket,tv,sanat,sinema,edebiyat,spoiler
- entry_name: |
    hiç yüzünden öldüresiye dövülen 11 yaşındaki çocuk
  entry_id:  89508919
  entry_writer: herhangi1adam
  entry_fav_count: 38
  entry_content: |
    herseferinde yapılması gerekenin yapılması için kamuoyu oluşturmaya çalışmak üzücü.
  entry_channels: haber
- entry_name: |
    demet evgar
  entry_id:  89536660
  entry_writer: vodkaredbullyadatekilalimon
  entry_fav_count: 35
  entry_content: |
    ne zaman kadınsı ne zaman seksi olduğu belli olmayan ayarsız oyuncu.
  entry_channels: tv,sinema
- entry_name: |
    2019 yılında hala windows 7 kullanmak
  entry_id:  89528718
  entry_writer: yakisiklisisman
  entry_fav_count: 34
  entry_content: |
    yeniliklerden hoşlanmadığı için değil mevcut sistemleri, programları windows 7 ile entegredir ve bunu değiştirmekte çok maliyetli olduğu için mecburdur kullanmaya.<br/><br/>en güzeli örneği de yurtdışındaki hava alanında çıkan arıza. adamlar yer ışıkları ve hava sistemleri için hala windows 3.1 kullanıyor. hava alanı gibi 7/24 işleyen bir yerde sistem değişikliği kolay olmaz.<br/><br/>kaynak: <a rel="nofollow noopener" class="url" target="_blank" href="https://news.vice.com/en_us/article/7xakd9/windows-31-is-still-alive-and-it-just-killed-a-french-airport" title="https://news.vice.com/en_us/article/7xakd9/windows-31-is-still-alive-and-it-just-killed-a-french-airport">link</a>
  entry_channels: programlama,teknoloji
- entry_name: |
    bardakilerin arya'ya tepkisi
  entry_id:  89537516
  entry_writer: s10
  entry_fav_count: 33
  entry_content: |
    ulan ne güzel şöyle anlar yaşamak. keşke biz de böyle topluca bir organizasyon düzenleseydik. <br/><br/>sanırım ülkecek bu kadar mutlu olduğumuz an ilhan'ın gol attığı maçtı. çok nadir gelen şeyler bunlar ve kaçırmamak gerekiyor.<br/><br/>tanım: hepimizin içinde olmak isteyeceği ortam ve göstereceği tepki.
  entry_channels: tv
- entry_name: |
    ekşi itiraf
  entry_id:  89524022
  entry_writer: dreamfactory
  entry_fav_count: 32
  entry_content: |
    artık yaşım ilerlediği için ve beyazlarımı kapatmakla uğraşmak istemediğim için saçlarımı komple beyaza boyattım. beyaz saçlarımın bana çok cool, marjinal ve sofistike bir hava verdiğini düşünürken çocuğun teki "babaaannneeee" diye koşup bacağıma sarıldı. silkeleyemedim eşoleşeği, mecbur biraz sevdim.<br/>simply granny.
  entry_channels: anket,ilişkiler,ekşi-sözlük
- entry_name: |
    canı istemiyorsa telefona yanıt vermeyen insan
  entry_id:  89535278
  entry_writer: auspiciousness
  entry_fav_count: 25
  entry_content: |
    telefonla konuşmaktan nefret ettiği için genellikle yanıt vermeyen,bu durumu kabullenerek trip atmayan yakınlarını ise çok seven insan olabilir.ben gibi
  entry_channels: ilişkiler,teknoloji
- entry_name: |
    kendine saygı nasıl arttırılır
  entry_id:  89538625
  entry_writer: the point
  entry_fav_count: 19
  entry_content: |
    instagramda herhangi bir mizah sayfasına girerek ne kadar zeki olduğunuzu görebilir , ekşide kendini zeki sananları görüp ne kadar alçak gönüllü olduğunuzu fark edebilir , haberleri izleyip ne kadar ahlaklı olduğunuzu kendinize hatırlatabilir ve bu entry’i okuyarak ne kadar mütevazı olduğunuzu hissedebilirsiniz...
  entry_channels: ilişkiler
- entry_name: |
    yazarların bir daha gitmem dediği mekanlar
  entry_id:  89531217
  entry_writer: colorblind
  entry_fav_count: 16
  entry_content: |
    (bkz: <a class="b" href="/?q=k%c4%b1z%c4%b1lkayalar">kızılkayalar</a>)<br/>(bkz: <a class="b" href="/?q=mado">mado</a>)<br/>(bkz: <a class="b" href="/?q=dorock+xl">dorock xl</a>)
  entry_channels: anket,yeme-içme,ekonomi,seyahat
- entry_name: |
    30 nisan 2019 tottenham ajax maçı
  entry_id:  89526635
  entry_writer: jukka
  entry_fav_count: 15
  entry_content: |
    bu ajax totnımı da eler finalda barça bunlara çakar. klasik senaryo böyle sürpriz takımlar finalde klasik mala bağlıyorlar 3-4 tane yiyip çimlere yatıp ağlıyorlar.
  entry_channels: spor
- entry_name: |
    mülakatlarda işe alınmama sebepleri
  entry_id:  89536302
  entry_writer: miss desidero
  entry_fav_count: 12
  entry_content: |
    etek giymek! üstelik mini falan da değil bildiğiniz dizde, iş için gayet uygun bir etek ! ama neymiş erkeklerin ağırlıkta olduğu bir şirketmiş etek giymek uygun değilmiş bla bla şirketinize sokayım saygılar.
  entry_channels: anket,ekonomi
- entry_name: |
    istanbul havalimanı
  entry_id:  89509527
  entry_writer: yaziliyorum
  entry_fav_count: 10
  entry_content: |
    otoparkının ilk 15 dakikası ücretsiz(!)<br/><br/>dünyanın en büyük havalimanının otoparkına aracınızı bırakıp yolcunuzu uğurlayıp&amp;karşılayıp 15 dk da geri gelip çıkabilirseniz bedava imiş.
  entry_channels: haber,ekonomi,havacılık,seyahat
- entry_name: |
    düşün ki o bunu okuyor
  entry_id:  89517643
  entry_writer: nilufer
  entry_fav_count: 8
  entry_content: |
    “tüm muhteşem hikayeler iki şekilde başlar; ya bir insan bir yolculuğa çıkar ya da şehre bir yabancı gelir.''
  entry_channels: ilişkiler,edebiyat
- entry_name: |
    öldürülmek için kiralık katil tutan öğretmen
  entry_id:  89537787
  entry_writer: baygezen
  entry_fav_count: 7
  entry_content: |
    bana pek inandırıcı gelmemiştir, mesela şu haber sitesi olayın böyle olduğunu söylemiyor: <a rel="nofollow noopener" class="url" target="_blank" href="https://www.haberler.com/ilkay-ogretmen-sevgilisinin-kursunlari-ile-can-12000334-haberi/" title="https://www.haberler.com/ilkay-ogretmen-sevgilisinin-kursunlari-ile-can-12000334-haberi/">https://www.haberler.com/…-ile-can-12000334-haberi/</a><br/><br/>muhtemelen katil kendini aklamak için böyle bir senaryo uydurmaya çalışmış.
  entry_channels: haber
- entry_name: |
    erkek yazarlardan kadın yazarlara sorular
  entry_id:  89509180
  entry_writer: o o
  entry_fav_count: 6
  entry_content: |
    bıktınız di mi şu ezik hırbolardan?
  entry_channels: anket,ilişkiler,ekşi-sözlük
- entry_name: |
    yazarların otomobil seçme kriterleri
  entry_id:  89537992
  entry_writer: tinkerminkerbell
  entry_fav_count: 4
  entry_content: |
    eskiden olsaydi sadece hp’sine ve motor hacmine bakardim.<br/>kalkis hizina, atikligine.<br/>tabii bir de agresif durup durmadigina<br/>simdiyse motor hacmine, dizel olup olmadigina, bagaj hacmine ve de kac koltuklu olduguna bakiyorum.<br/>ozellikle de arkaya 2 tane oto koltugu sigip sigmadigina.<br/>suan bu entryim smiley ile bitmeyi hakediyodu.
  entry_channels: anket,otomotiv,motosiklet
- entry_name: |
    canan kaftancıoğlu'nun kut'ül amare zaferi mesajı
  entry_id:  89539346
  entry_writer: bythem
  entry_fav_count: 4
  entry_content: |
    sorsan bunlar ingiliz emperyalizmine karşıdır. ama işin içine islam girince ingiliz uşağı olmaktan çekinmezler.
  entry_channels: 
- entry_name: |
    yaş aldıkça kolaylaşan şeyler
  entry_id:  89540339
  entry_writer: daginikligimlameshurum
  entry_fav_count: 3
  entry_content: |
    kilo almak.
  entry_channels: anket
- entry_name: |
    zincirlikuyu metrobüs durağındaki kız
  entry_id:  89538937
  entry_writer: lipsofanangel
  entry_fav_count: 2
  entry_content: |
    (bkz: <a class="b" href="/?q=iyi+denemeydi+montaigne">iyi denemeydi montaigne</a>)<br/>lütfen bir daha yapma.
  entry_channels: 
- entry_name: |
    kıskanılan meslekler
  entry_id:  89509116
  entry_writer: sanal gezgin
  entry_fav_count: 2
  entry_content: |
    son ütücü.<br/>yarın ne yapacağını biliyorsun.<br/>bunun verdiği hafiflik insana yeter.
  entry_channels: anket,eğitim
- entry_name: |
    30-40 yaş erkek ve kadın yazarları evlendiriyoruz
  entry_id:  89517581
  entry_writer: sehege
  entry_fav_count: 1
  entry_content: |
    ah be bi seneyle kaçırdım. neyse seneye katılırım.<br/><br/>herkese hayırlı kısmetler.<br/><br/>(bkz: <a class="b" href="/?q=s%c3%b6zl%c3%bck+bana+bunlarla+gel">sözlük bana bunlarla gel</a>)
  entry_channels: ilişkiler,ekşi-sözlük
- entry_name: |
    sözlükçülerin en uzun yürüyüşleri
  entry_id:  89539199
  entry_writer: pul
  entry_fav_count: 0
  entry_content: |
    eylül 2016, kurban bayramı ama biz havalimanı projemizi açıp, sorunsuz teslim etmek için evimizden binlerce km uzaktayız. <br/>günde ortalama 18 saat sahadayız, herkesin adımsayarları açık, kampa dönerken arabada bakıyoruz benimki 40-42 bin adım arasında değişiyor. çıktığım kat da 40-50 arası...<br/>son gece güvenlik taraması için tüm binayı boşaltıyorlar. sadece ismi alınan kişiler kara tarafı ana terminal girişinde bekleyecek. ben de hava tarafından sorumlu kişi olarak listedeyim. <br/>karşılıklı 15’erden 30 gate var ve en uçta da bir otel. geceyarsına doğru adım sayıma bakıyorum 42 bin, çıktığım kat 51. <br/>ayağıma bir numara büyük aldığım ayakkabılar artık sıkmaya başlamış, bacaklarım taşlaşmış, omuzlarımı biraz geriye atmaya çalıştığımda acı çekiyorum, boynum sağa sola dönmüyor, gözlerim kan çanağı ve hep yanıyor. ama iyiyim. çünkü tam da o sıralar özel hayatımda bir yıkım var ve ben hem bedenen hem kafa olarak o kadar yoruluyorum ki o yıkımı düşünmeye mecalim kalmıyor. daha da güzel o anda yıkmıyorum, yapıyorum. <br/><br/>birazdan işimiz bitecek ve uyumaya gideceğiz diye düşünürken adım anons ediliyor. otel bloğunda bir sorun olduğu ve oraya gitmem gerektiği söyleniyor. yani binanın en ucuna. güvenlik taramasından geçmiş, temiz denilen bölüme geçeceğim için iki kgb polisi ve iki k9 bana eşlik edecek. üstümü aramak istiyorlar ama kadın polis olmadığı için vazgeçiyorlar. sorun olan yere varıyoruz, sorun falan yok. yorgunluktan sinirlenememek diye bir şey varmış, o gece tecrübe ettim.<br/>aynı yolu tekrar dönüyoruz. o gece nasıl bitti, kampa neyle döndük, yatağa nasıl gittim hatırlamıyorum ama uyandığında bakıyorum 52 bin adım atmışım yani yaklaşık 40 km yürümüşüm. <br/><br/>çok yorulduk ama açılışı yaptığımızın ertesi günü yaptığımız havalimanından uçmak harika bir duyguydu. sırf o his için aynı yolları tekrar yürümem gerekse hiç tereddüt etmem, yine yürürüm.
  entry_channels: spor,anket,ekşi-sözlük
- entry_name: |
    bira içmelik arkadaş veritabanı
  entry_id:  89538132
  entry_writer: camel icen deve
  entry_fav_count: 0
  entry_content: |
    baştan aşağı kadıköy entryleriyle dolu olan veritabanı. bir kişi kadıköy yazdıktan sonra neden diğer kadıköylüler o kişiye mesaj atıp buluşmak yerine ısrarla kendileri de entry girip boş kalabalık yapıyorlar? bu başlığa bayburtlu biri yazsa anlarım ki "bayburtta bira içmelik birini bulamamış, sözlükten biri çıkar umuduyla yazıyor" diye. ama allasen kadıköyde nasıl bir asosyalliktesiniz?
  entry_channels: anket,ilişkiler,ekşi-sözlük
---
