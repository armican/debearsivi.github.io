---
layout: post
title: 25 Nisan 2018 Ekşi Sözlük Debe
data:
- entry_name: |
    milli ve yerli para ile kur oyununu bozacağız
  entry_id:  76304267
  entry_writer: mdh
  entry_fav_count: 585
  entry_content: |
    kendisi hep dolarla iş yaptığı için bizim cebimizdeki para hakkında bir fikir sahibi değil herhalde.<br/>biz halen lira kullanıyoruz reis.<br/>milli ve yerli olmayan kişi beyanı.
  entry_channels: siyaset,ekonomi
- entry_name: |
    deniz simidiyle kuzey atlantik okyanusu'nu geçmek
  entry_id:  76307567
  entry_writer: urubitinga
  entry_fav_count: 504
  entry_content: |
    <a rel="nofollow noopener" class="url" target="_blank" href="https://n11scdn4.akamaized.net/a1/1024/spor-outdoor/deniz-aksesuarlari/56581-golgelikli-bebe-flotoru-8166__0398546641040074.asp" title="https://n11scdn4.akamaized.net/a1/1024/spor-outdoor/deniz-aksesuarlari/56581-golgelikli-bebe-flotoru-8166__0398546641040074.asp">bununla</a> git de başına güneş geçmesin.
  entry_channels: seyahat
- entry_name: |
    kılıçdaroğlu genel müdürken 1 galoş 25bin liraydı
  entry_id:  76311203
  entry_writer: veniceviolins
  entry_fav_count: 387
  entry_content: |
    ekmek de 100bin liraydı. şeytan da henüz pabuçlarını düz giyiyordu. taa ki insanlık sizin gibi yeni bir türle karşılaşana kadar.<br/><br/>(bkz: <a class="b" href="/?q=nas%c4%b1l+%c5%9feytan+oldum">nasıl şeytan oldum</a>)
  entry_channels: siyaset,tarih,sağlık,ekonomi
- entry_name: |
    rte seçimi kaybettikten sonraki ilk gün
  entry_id:  76309657
  entry_writer: sortayms
  entry_fav_count: 280
  entry_content: |
    ülkede büyük bir kaos çıkacak, yüzlerce insan ölecek, kanlar akacak...<br/><br/>yok lan şaka. paşa paşa gidecek. zaten şimdiden kendini hazırlıyor gibi.<br/><br/>edit:<br/>• 1. turda %50’yi bulan aday olmayınca seçim 2. tura kalmıştır. <br/>• 2. turda birleşen muhalif cephenin adayı %52 ile önde gitmektedir. saat 22:30... artık sonuçlar kesinleşmiş gibi. açılan sandık sayısı %95’e takıldı kaldı, yeni bilgiler bi türlü gelmiyor. herkes bundan sonra ne olacağını merak ediyor.<br/>• tüm bürokrasiyi ve medyayı elinde bulunduran iktidar ysk’ya kesin sonuçları açıklamaması için baskı yapıyor. muhalif adayların destekçileri ysk önünde protestolara başladı. <br/>• ertesi gün akşam saatlerinde rte canlı yayında açıklama yapıyor. seçimlerin şaibeli olduğunu ve sonuçlar düzeltilene kadar ysk’nın açıklama yapmayacağını söylüyor. <br/>• ysk başkanı sadi güven sorun üzerinde çalışıldığını en kısa sürede kamuoyunu aydınlatacaklarına ilişkin bilgi veriyor. <br/>• tepki 2. günde çığ gibi büyüyor. protestolar artarak devam ediyor. halk kendi iktidarını ve demokrasiyi korumak için sokağa dökülüyor. <br/>• artan baskılar üzerine ankara’dan bir uçak havalanıyor gece saatlerinde. o günün sabahında ysk açıklamayı yapmak zorunda kalıyor. hayat bayram oluyor…
  entry_channels: siyaset,anket,haber
- entry_name: |
    24 nisan 2018 beşiktaş jk kamuoyu açıklaması
  entry_id:  76316207
  entry_writer: guzelceli
  entry_fav_count: 278
  entry_content: |
    beşiktaş seviyeyi düşürmedikçe daha çok delirecekler.
  entry_channels: spor
- entry_name: |
    yazarların şimdiye dek gördükleri ülke sayısı
  entry_id:  76309692
  entry_writer: birmilyonunvarmi
  entry_fav_count: 255
  entry_content: |
    anamur'dan kıbrıs'ı görmüştüm. sayılıyor mu?
  entry_channels: anket,ekşi-sözlük,seyahat
- entry_name: |
    gelmiş geçmiş en iyi araba reklamı
  entry_id:  76300664
  entry_writer: demerg
  entry_fav_count: 198
  entry_content: |
    bence budur<br/><a rel="nofollow noopener" class="url" target="_blank" href="https://youtu.be/3L77ZV0ghn8" title="https://youtu.be/3L77ZV0ghn8">peugeot 206</a>
  entry_channels: anket
- entry_name: |
    muslera vs fabri
  entry_id:  76307733
  entry_writer: all son da all ur
  entry_fav_count: 190
  entry_content: |
    ikiside kainat yerinden oynasa kıçıyla top tutmaya niyet bile etmeyecek karakterdedir. o yüzden kimseyi evden aldırmazlar,milli takımlarını yarıda bırakmazlar,gazeteci dövdürmezler ve malum takımın kalesinde bu tip kalecileri göremezsiniz...
  entry_channels: spor
- entry_name: |
    24 nisan 2018 rte bülent arınç görüşmesi
  entry_id:  76311364
  entry_writer: geri donen mektup
  entry_fav_count: 186
  entry_content: |
    (bkz: <a class="b" href="/?q=hi%c3%a7bir+korkuya+benzemez+halk%c4%b1n%c4%b1+satan%c4%b1n+korkusu">hiçbir korkuya benzemez halkını satanın korkusu</a>)
  entry_channels: siyaset,haber
- entry_name: |
    güzel bir kadını çirkin gösteren şeyler
  entry_id:  76294530
  entry_writer: chuckly
  entry_fav_count: 178
  entry_content: |
    ayak baş parmağının ve ikinci parmağının boyutunu kadınlar doğarken kendileri seçmedigi için güzel kadını çirkinleştiren şey diye bir yorum yapılamaz, yapılmamalı.
  entry_channels: anket,ilişkiler,moda
- entry_name: |
    dünya bir ülke olsaydı başkenti neresi olurdu
  entry_id:  76301632
  entry_writer: aynen coni vaynen
  entry_fav_count: 178
  entry_content: |
    içinde dünyanın bütün senatörlerine yetecek kadar odası olan saray mevcut olduğu için ankara.
  entry_channels: anket
- entry_name: |
    bordo kravat giyip seyirciyi tahrik ediyor
  entry_id:  76307690
  entry_writer: bythem
  entry_fav_count: 148
  entry_content: |
    bir aziz yıldırım vecizesi;<br/><br/><a rel="nofollow noopener" class="url" target="_blank" href="https://twitter.com/ntvspor/status/988744545432203266" title="https://twitter.com/ntvspor/status/988744545432203266">https://twitter.com/…spor/status/988744545432203266</a><br/><br/>ahahaha, oğlum bu seyirci boğa mı renkten tahrik oluyor, adamın bahanesine bak, devreler komple yanmış.
  entry_channels: spor,haber,moda
- entry_name: |
    atom eskir mi
  entry_id:  76314191
  entry_writer: drummerlizard
  entry_fav_count: 146
  entry_content: |
    hayır, eskimez. <br/><a rel="nofollow noopener" class="url" target="_blank" href="https://education.jlab.org/qa/radelement_03.html" title="https://education.jlab.org/qa/radelement_03.html">kaynak</a><br/>edit: çok güzel bir sorudur. bilimin temelinde sormak vardır. bilmemek değil öğrenmemek ayıptır.
  entry_channels: anket,bilim
- entry_name: |
    ittifaka oy vermek farzdır
  entry_id:  76312484
  entry_writer: neanderthalensis
  entry_fav_count: 121
  entry_content: |
    ne zaman birileri çıkıp bu gibilere "çüüüş, bostana girdin" diyecek. resmen dinimize sövüyor münafık şerefsizler. sözde islamcılardan hiç ses seda yok. yerin dibine batsın muhafazakarlığınız takunya kafalar.
  entry_channels: siyaset,haber
- entry_name: |
    evli değilmiş gibi yapan evli erkek
  entry_id:  76308657
  entry_writer: karanliktan korkan vampir
  entry_fav_count: 116
  entry_content: |
    boşandığım eski kocamdı bu adam. <br/><br/>yüzüğünü çogu zaman takmazdı. şüphelenip neden takmadın diye sorunca "unuttum aşkım. o kadar yogunum ki fırsat olmadı" derdi. ben de tabi hemen "adam haklı. saçmaladım, çok art niyetliyim" diye kendimi suçlardım.<br/><br/>inanmak istedim hep. çok seviyordum, çok seviyor gibiydi. kabullenmek istemedim. şimdi düşünüyorum da tam bir zavallılık. <br/>sonradan ögrendim birçok defa aldatmış beni. öğrendikten sonra canımı iyice acıtmak için seni sevmiyorum, sevsem seninle olurdum dedi. sonrasında da boşanmamak için çok fazla diretti. <br/><br/>üstelik aldattığı kadınlar eski eşimin evli oldugunu biliyorlarmış. ama ben hiçbir zaman o kadınları suçlamadım. kocam; beni, minik kızımızı, yuvamızı, kendini düşünmüyorsa o kadınlar ne yapsın? umrunda olmaz kimsenin, yuvamız mı dağılmış, kızım babasız mı büyümüş, ben depresyona girip gece gündüz ağlamışım falan. kimsenin derdi degil bunlar. ne yaşadığımı ancak ben bilirim. vicdan meselesini herkes kendi içinde yaşar. benim gözümde o kadınlar suçlu değil ki, bir zamanlar o çok sevdiğim, kitapsız eski kocam suçlu.<br/><br/>edit; "kezban olmasaydın bunları yaşamazdın.yalnız öl. " diye mesaj atan insanlar var. yahu siz ne yaşıyorsunuz da bu kadar kötü kalpli olabiliyorsunuz.kocamın aldatması benim suçummuş. yapma ya, bak senn şu işe. <br/><br/>evli oldugumuz dönemde şu an olduğu gibi çalışan, kendi ayakları üzerinde duran bir kadındım. ayrıca iyi bir egitim almış, kendi işini kurmuş biriyim. aylık gelirim eşimin 3 katı falandı. yazdıgınız gibi akşam kocaya sarıp, dırdır yapma durumu yok. adamın karakteri bozuksa ben ne yapabilirim? karakter meselesi. <br/><br/>ayrıca şu an 2. kez evlendim. (eski eşim hala yalnız)
  entry_channels: anket,ilişkiler
- entry_name: |
    anneler yazar olsa açacakları başlıklar
  entry_id:  76310508
  entry_writer: clintindubloru
  entry_fav_count: 111
  entry_content: |
    hamiyet'in kızının hala evlenmemesi
  entry_channels: anket,ekşi-sözlük
- entry_name: |
    babalar yazar olsa açacakları başlıklar
  entry_id:  76311873
  entry_writer: fikirtepeli
  entry_fav_count: 110
  entry_content: |
    (bkz: <a class="b" href="/?q=b%c3%bct%c3%bcn+iyi+park+yerlerinin+kap%c4%b1lm%c4%b1%c5%9f+olmas%c4%b1">bütün iyi park yerlerinin kapılmış olması</a>)
  entry_channels: anket,ekşi-sözlük
- entry_name: |
    akşener'in emre uslu ile yazışmalarını silmesi
  entry_id:  76314672
  entry_writer: exfreak
  entry_fav_count: 98
  entry_content: |
    işin daha komiği akşener "<a class="b" href="/?q=bitsin+bu+hasret">bitsin bu hasret</a>" filan da diyordu.<br/><br/>"<a class="b" href="/?q=ne+istediler+de+vermedik">ne istediler de vermedik</a>" diyen de akşener'di.<br/><br/>yol yabdı.<br/><br/><a class="b" href="/?q=ek">ek</a>: <a class="b" href="/?q=aktroll">aktroll</a> lafını önce <a class="b" href="/?q=ok+troll">ok troll</a>, sonra <a class="b" href="/?q=iyi+troll">iyi troll</a> olarak çevirip kullanmaya çalışan gerzekleri görmek muazzam. kendi cahil kitlesi zamanında sosyal platformları kullanma özürlü olduğundan maaşlı kadrolar oluşturan bu hödük kafa her zamanki gibi kendisini eleştiren herkesi kendi yaptıkları şeylerle eleştirmeye çalışıyor. terörist de, vatan haini de, kuduz köpek gibi salya saça saça anır koduğumun çomarı ama israil'e van münüts denirken yıllarca jet yakıtını neden sattığını izah dahi edeme.<br/><br/>ayrıca, fetö denen kuduz köpeği yıllarca besleyip büyüten sizsiniz, ortadan kaldıransa vatansever polis ve askerler. bir başkasıyla olan iktidar ve sidik yarışının sonucu.
  entry_channels: siyaset,haber
- entry_name: |
    altay tankı
  entry_id:  76311391
  entry_writer: konus sen dinliyorum
  entry_fav_count: 84
  entry_content: |
    ortalarda dolanan haberlere bakılacak olursa seri üretimi alman kökenli rbss/rheinmetall ve katar ortaklığı bulunan <a class="b" href="/?q=bmc">bmc</a> ‘ye verilmiş gözüküyor. <br/><br/>ilgili firmanın tankı üretecek fabrikası olup olmadığı, ortaklığındaki ülkelerin milli tankımızın teknik detaylarına sahip olup olmayacağı, her şeyden öte türk silahlı kuvvetlerinin mevcut tank ihtiyacının kaç sene daha öteleneceği sorularının cevabını; vatanını, milletini seven ve bu doğrultuda çalışan bir türk mühendis olarak merak ediyorum. <br/><br/>burayı okuyan meslektaşlarım anlayacaklardır yukarıda neyi kastettiğimi. vicdanınız gerçekten rahat mı?<br/><br/>hiç mi sevmiyorsunuz ulan bu ülkeyi?
  entry_channels: siyaset,teknoloji
- entry_name: |
    beşiktaş'ın hükmen 3-0 mağlup sayılması
  entry_id:  76306222
  entry_writer: deniz fenerine asik olan gemi
  entry_fav_count: 82
  entry_content: |
    (bkz: <a class="b" href="/?q=buram+buram+maydanoz+kokan+ba%c5%9fl%c4%b1klar">buram buram maydanoz kokan başlıklar</a>)
  entry_channels: spor
- entry_name: |
    22 aralık 2016 ışid'in 2 askerimizi şehit etmesi
  entry_id:  76298288
  entry_writer: leki
  entry_fav_count: 79
  entry_content: |
    seçim döneminde unutulmasın, unutana unutturana yazıklar olsun
  entry_channels: siyaset,tarih,haber
- entry_name: |
    aykut kocaman
  entry_id:  76290739
  entry_writer: kevd7
  entry_fav_count: 78
  entry_content: |
    vardar'a elendikten sonra basın toplantısına çıkmaya korkan, takımı ligin başında şampiyonluktan kopunca telefonunu kapatıp bolu'ya kaçan ve kariyeri boyunca sayısız kere istifa edeceğim deyip edemeyen teknik direktör. umarım futbolu bırakma konusunda aldığı kararın üzerine gider kendi adıma aykut kocamansız bir türk futbolu beni oldukça mutlu eder açıkçası.
  entry_channels: spor
- entry_name: |
    ben ilker canikligil sorularınızı cevaplıyorum
  entry_id:  76314600
  entry_writer: edgarallanpoenunkuzgunu
  entry_fav_count: 75
  entry_content: |
    saygıdeğer hocam ilker bey, hoşgeldiniz.<br/><br/>1995 yılında beyoğlu'ndaki akbank sanat merkezi'nde karşılaşmıştık sizinle. köşedeki bir koltukta tek başınıza dergi okuyordunuz. ben sinema öğrencisi olduğumu söylediğimde zamanınızı ayırıp senaryo uygulamaları hakkında değerli sohbetinizle beni bilgilendirmiştiniz. 19 yaşında heyecanlı bir genç adam olduğumdan dolayı o yıl çekeceğiniz "uçmak istiyorum" adlı kısa film hakkında fikir belirtme cüretini gösterebildim. önerim, dev bir mavi tavşanın filmin sürreal gücünü artıracağıydı. sizin terslememenizden cesaret alarak ertesi gün tavşan kostümümle stüdyonuza geldim, siz de şaşkınlığınızı attıktan sonra hevesimi kırmayıp kameranızla ayaküstü birkaç çekim yaptınız ve gönderdiniz beni. <a class="b" href="/?q=sorum">sorum</a> <a class="b" href="/?q=%c5%9fu">şu</a>: o kısmı filme neden eklemediniz ilker bey? filmi izlerken kendimi göremeyince çok üzülmüştüm. üstelik ortada koskoca bir donnie darko örneği var, bence tavşan gayet güzel olabilirdi.<br/><br/>teşekkürler ve saygılar.
  entry_channels: anket,tv,ekşi-sözlük,sinema
- entry_name: |
    24 nisan 2018 aziz yıldırım'ın basın toplantısı
  entry_id:  76307088
  entry_writer: mimmm
  entry_fav_count: 74
  entry_content: |
    aykut kocaman: "arsız bir insan olsam, maçın bizim lehimize hükmen sonuçlanması lazım derim çünkü beşiktaş sahayı terk etti".<br/><br/>şekip mosturoğlu: "bizce bu maç beşiktaşlı oyuncuların sahayı terkmesi nedeniyle 3-0 lehimize hükmen sonuçlanmalıdır".<br/><br/>olm karar verin hanginiz arsız? hanginiz yüzsüz?
  entry_channels: spor,haber
- entry_name: |
    atatürk'le ilgili olumsuz tweet atmayın
  entry_id:  76315984
  entry_writer: diego alejandro fuentes medina
  entry_fav_count: 71
  entry_content: |
    ak'ların derken amına koduklarımın demek istemiş sanırım.
  entry_channels: siyaset
- entry_name: |
    anthony hopkins'in paylaştığı video
  entry_id:  76299509
  entry_writer: giotean
  entry_fav_count: 71
  entry_content: |
    emekli maasina zam geliyor haberini duyan amcalara benziyor diyecegim de inandirici olmaz bu memlekette.
  entry_channels: magazin
- entry_name: |
    010100110110000101100010011010010111001001100101
  entry_id:  76307990
  entry_writer: zwiegesprach
  entry_fav_count: 65
  entry_content: |
    01100001 01101110 01101011 01100001 01110010 01100001 00100000 00110001 00101110 00100000 01110011 01110101 01101100 01101000 00100000 01100011 01100101 01111010 01100001 00100000 01101000 01100001 01101011 01101001 01101101 01101100 01101001 01100111 01101001 00100111 01101110 01101001 01101110 00100000 01110011 01100001 01100010 01101001 01110010 01100101 00100000 01101101 01100101 01101100 01110100 01100101 01101101 00100000 01100010 01100001 01101110 01101011 01101111 00100111 01101110 01110101 01101110 00100000 01110100 01100001 01101100 01100101 01100010 01101001 00100000 01110101 01111010 01100101 01110010 01101001 01101110 01100101 00100000 01110110 01100101 01110010 01100100 01101001 01100111 01101001 00100000 00110011 00110000 00101110 00110000 00110100 00101110 00110010 00110000 00110001 00111000 00100000 01110100 01100001 01110010 01101001 01101000 00100000 01110110 01100101 00100000 00110010 00110000 00110001 00111000 00101111 00110100 00110011 00110001 00110111 00100000 01100100 00101110 01101001 01110011 00100000 01110011 01100001 01111001 01101001 01101100 01101001 00100000 01101011 01100001 01110010 01100001 01110010 01101001 00100000 01110101 01111001 01100001 01110010 01101001 01101110 01100011 01100001 00100000 01100010 01110101 00100000 01100010 01100001 01110011 01101100 01101001 01101011 01110100 01100001 00100000 01111001 01100101 01110010 00100000 01100001 01101100 01100001 01101110 00100000 01101001 01100011 01100101 01110010 01101001 01101011 01101100 01100101 01110010 01100101 00100000 01100101 01110010 01101001 01110011 01101001 01101101 01101001 01101110 00100000 01100101 01101110 01100111 01100101 01101100 01101100 01100101 01101110 01101101 01100101 01110011 01101001 01101110 01100101 00100000 01101011 01100001 01110010 01100001 01110010 00100000 01110110 01100101 01110010 01101001 01101100 01101101 01101001 01110011 01110100 01101001 01110010 00101110
  entry_channels: siyaset,ekşi-sözlük,programlama,teknoloji,haber
- entry_name: |
    cerrahpaşa tıp fakültesi'nin iü'den ayrılması
  entry_id:  76293195
  entry_writer: utter
  entry_fav_count: 48
  entry_content: |
    fakültenin ismi değişiyor tüm eski yayınlardan gelen isim değeri ve diğer fakültelerle yapılan erasmus öğrenci değişimi anlaşmaları gibi anlaşmalar bozuluyor. uluslararası akreditasyonu bozuluyor. mezunlar yurtdışında hiçbir yere görevli veya öğrenci olarak gidemiyor. hocaların kendisi tanınmıyorsa uluslararası kongrelere dahi kabul alması engelleniyor. kovulamayan bazı hocaların yeni üniversiteye alınmayarak de facto kovulması sağlanıyor.<br/><br/>üstüne, sadece istanbul üniversitesinden kopmakla kalmayıp yeni kurulan üniversitenin merkezine yani silivri'ye yapılacak bir hastaneye taşınıyor. cerrahpaşanın yeni hastanesinin de ismi cerrahpaşa olmuyor.
  entry_channels: siyaset,haber,eğitim
- entry_name: |
    güzelliğiyle dikkat çeken özbek piyanist astanova
  entry_id:  76311813
  entry_writer: a rainy day in falkreath
  entry_fav_count: 41
  entry_content: |
    (bkz: <a class="b" href="/?q=g%c3%b6t%c3%bcyle+dikkat+%c3%a7eken+amerikal%c4%b1+%c5%9fark%c4%b1c%c4%b1+nicki+minaj">götüyle dikkat çeken amerikalı şarkıcı nicki minaj</a>)<br/><br/>açacağın başlığı... onedio mu lan burası?
  entry_channels: müzik,magazin
- entry_name: |
    pkk'nın cumhur ittifakına karşı mücadele çağrısı
  entry_id:  76307223
  entry_writer: pardus m
  entry_fav_count: 30
  entry_content: |
    iftira bünyelerine sinmiş bu akgezenlerin. feto pisliğinden zamanında çok şey öğrenmişler.<br/><br/>al sana ittifak:<br/><a rel="nofollow noopener" class="url" target="_blank" href="https://picload.org/view/dorrldgi/tayyip1.png.html" title="https://picload.org/view/dorrldgi/tayyip1.png.html">1</a><br/><a rel="nofollow noopener" class="url" target="_blank" href="https://picload.org/view/dorrldgw/tayyip2.png.html" title="https://picload.org/view/dorrldgw/tayyip2.png.html">2</a>
  entry_channels: siyaset,haber
- entry_name: |
    çaylak onay listesi
  entry_id:  76290882
  entry_writer: yito
  entry_fav_count: 30
  entry_content: |
    ilk başvurduğumda 20 yaşında bir ergendim. simdi 32 yaşındayım ne yazayım ki bilemedim.
  entry_channels: ekşi-sözlük,haber,eğitim
- entry_name: |
    rte'nin seçimi kaybetmesi halinde yapacağı meslek
  entry_id:  76316102
  entry_writer: prandelli
  entry_fav_count: 28
  entry_content: |
    merak edilendir. şöyle ki sayın cumhurbaşkanımız seçimi kaybederse cumhurbaşkanı olamıyor. cumhurbaşkanı adayları aynı anda milletvekili adayı olamayacağı için milletvekili de olamıyor. iki şekilde de etkin siyasete dahil olamıyor. böyle bir durumda 24 bin lira emekli cumhurbaşkanı maaşı alacak ancak bu devirde bu parayla kim geçinebilir ki. 2019'da herhangi bir şehirden rahatlıkla belediye başkanı olabilir. o zamana kadar ise partiden kendisine maaş bağlanıp yazı işlerine falan bakabilir diye düşünüyorum. ya da askerde kazandığı tecrübeyle kantincilik yapabilir. diğer düşündüğüm olasılıklar ise a haberde veya survivorda sunuculuk, emine erdoğanın da desteğiyle çanta dükkanı işletmeciliği, başakşehir futbol klübünde teknik direktörlük, mevlütlerde ve cenazelerde hocalık, üniversite diplomasının bulunması halinde ise eğitimini alıp gemi kaptanlığı yapabilir.
  entry_channels: siyaset,anket
- entry_name: |
    ilk buluşmaya tayt giyip gelen kız
  entry_id:  76317101
  entry_writer: bir yalnizlik isareti
  entry_fav_count: 26
  entry_content: |
    ilk buluşmaya eşofmanla gitmiş birisi olarak yadırgamayacağım kızdır. <br/>hem ilk buluşmalar hep risklidir. güzel geçse ayrı dert kötü geçse apayrı dert. <br/>en güzeli hiç buluşmamak, herkes otursun evinde.
  entry_channels: ilişkiler,moda
- entry_name: |
    eşekherif
  entry_id:  76291057
  entry_writer: dynamico
  entry_fav_count: 23
  entry_content: |
    yukarıda 7 senedir game of thrones'u onunla izliyoruz diye fanatiklik yapmış birileri<br/>çok üzüldüm, 7 senede insan nasıl ingilizce öğrenemez, bi gıdım geliştirmez kendini?
  entry_channels: tv,sinema,edebiyat
- entry_name: |
    sözlükte akp'lileri aşağılamıyoruz kampanyası
  entry_id:  76304916
  entry_writer: tscy
  entry_fav_count: 22
  entry_content: |
    engelli vatandaşla dalga geçen mi var lan?
  entry_channels: siyaset,ekşi-sözlük
- entry_name: |
    abdullah gül
  entry_id:  76291176
  entry_writer: yillarsonra
  entry_fav_count: 21
  entry_content: |
    saadet partisi adayı olabilir ona mani olan yok. çatı aday olursa sikerler öyle muhabbeti, aynı yere iki kere yıldırım düşmez. önce ilk tur adayı bi olsun ikinci tura kalabilirse isteyen gider atar. benden siyasal islamcılara oy yok.hepsine oy yok. karşı karşıya kalsalarda oy yok. böyle salakça bi tezgahı yerse iyi parti ve chp kilit vursunlar parti kapılarına uğraşmasınlar bu işlerle.
  entry_channels: siyaset
- entry_name: |
    aktroll yerine isim önerileri
  entry_id:  76318067
  entry_writer: sadan
  entry_fav_count: 15
  entry_content: |
    (bkz: <a class="b" href="/?q=orospu+%c3%a7ocu%c4%9fu">orospu çocuğu</a>)
  entry_channels: 
- entry_name: |
    yazarların en sevmediği huyları
  entry_id:  76310080
  entry_writer: girl with no name
  entry_fav_count: 14
  entry_content: |
    kendimi anlatma çabam. <br/><br/>bir şey söyleyecek olsam duyguyu en iyi şekilde karşı tarafa geçirmeye çalışıyorum. kendimi insanlara açıklamak istiyorum. sevilme çabasından başka bir şey değil. asla değişmeyeceğim. iflah olmaz bir gevezeyim. bir de her konuda mutlaka bir görüşüm var. baya itici bir insanım galiba.
  entry_channels: anket,ekşi-sözlük
- entry_name: |
    cengiz ünder
  entry_id:  76317565
  entry_writer: bana lukslerimi verin
  entry_fav_count: 10
  entry_content: |
    cengiz'in kadroda olduğu maç. <br/>1 golü var yazın kenara.<br/>atamazsa da entry'i silerim. öyle de yavşak biriyim.
  entry_channels: spor
- entry_name: |
    eşek arısının kopan kafasını alıp uçması
  entry_id:  76317683
  entry_writer: moonrise
  entry_fav_count: 7
  entry_content: |
    o nasıl bir organizmadır arkadaş? kafa yerinde yok hala uçma derdinde... bunların insana yakın ebatta olmasını düşünemiyorum, şerefsizleri silahla öldüremezdik. deadpool gibi birşey olurlardı. bir de bize bak parmağın kanasa yara bandıyla gezersin...
  entry_channels: bilim
- entry_name: |
    bitcoin
  entry_id:  76312414
  entry_writer: amcayanickinigostermeyencocuk
  entry_fav_count: 6
  entry_content: |
    refçi indikatörü al sinyali veriyor.
  entry_channels: programlama,teknoloji,haber,eğitim,ekonomi
- entry_name: |
    türk kadınlarının orgazm olamaması
  entry_id:  76315333
  entry_writer: violent
  entry_fav_count: 4
  entry_content: |
    başlık altındaki mevcut teoriler (en azından bir kısmı) şunlar:<br/><br/>- erkeğin kompleksi<br/>- kadında bir araz var<br/>- kadın seksi erkeği ödüllendirmek için kullandığından durum böyle<br/>- kadının kötü anıları<br/>- erkeğin becerisizliği<br/>- partnerden partnere<sup class="ab"><a title="(bkz: partner emel)" href="/?q=partner+emel" data-query="partner emel">*</a></sup> değişir<br/>- toplum baskısı<br/>...<br/><br/>"çamaşır makinesi su almıyor" gibi bir soru değil arkadaşlar bu. zart diye bir cevabı olamaz. bir sürü şeyin bir kokteyli ve bu durumdaki her kadında özgün, öznel bir yön vardır.<br/><br/>bu durumdaki kadınlardan öğrendiklerime istinaden ben de birkaç şey yazayım.<br/><br/>bir sebep ve aslında sanırım en yoğun sebep, anatomik bir tür hamlık olabiliyor. baskılanan cinsellik, cinselliğin açığa çıkma yollarını unutmasına, onlardan uzak kalmasına sebebiyet verebiliyor kadınların. masturbasyon yapan kadınlar ekseriyetle daha rahat orgazm olabiliyor, çünkü vücudun böyle bir alışkanlığı var, daha önce yapmış. bugün squad yapın 2 tane desem çoğunuz düzgün yapamazsınız. orgazm da böyle işte, antrenmanın yolunu kapatırsan zorlaşır işin.<br/><br/>bir grup cinsellikten utanan ve geldiği noktada o kalıplarını yıkmış gibi görünse de yılların yargılarını taşıyan kadın da var. cinsellik mesela hala ayıp onun için. ne kadar artık bunu aşmış gibi görünse de aslında hala çekinceleri var. ablasıyla veya annesiyle konuşabileceği bir şey değil bırakın abisini, arkadaşlarını. bu da kadını geriyor. gerilmekte haklı. erkek de benzer sebeplerle çok geriliyor. onu da erken boşalmanın anatomisi isimli yedi ciltlik eserimde detaylarıyla anlattım.<br/><br/>beklentinin endişeye, endişeninse başarısızlığa dönüştüğü durumlar da var. cinsellik o kadar mistifiye ediliyor ve bazen o kadar yüceltiliyor ki kadın sürekli o "ulu" anı bekliyor sevişme boyunca. kendini rahat bırakamıyor, bir türlü orada olamıyor ve o beklenen an da sırf beklendiği için gelmiyor. doyacağınız anı düşünerek yemek yemeye çalışın mesela, bir türlü tam o an gelmez. halbuki masadan çoğu kez kalktığınızda zaten doymuşsunuzdur. evet doymak ve cinsel haz tam aynı şey değiller, cinsel hazın doyum noktasına eşlik eden bir takım fizyolojik süreçler de var lakin temelde benzer sonuçları oluyor o anı beklemenin de.<br/><br/>partnerle uyum. bazı kadınlar zordur ve bazı erkek türleriyle bir araya geldiklerinde iş daha da zorlaşır. kendi cinselliğimizi keşfetmek yerine porno izlediğimiz veya türk dizilerindeki öpüşme sahnesi sonsuza kadar sürse dediğimiz için eksik kalıyoruz, öğrenemiyoruz hem erkek hem kadın olarak. birçok kadın neden zevk aldığından, nasıl orgazma ulaşacağından bihaber. bu meyve sevdiğini bilmek ama hangi meyveyi sevdiğini tam bilmemek gibi. sevdiğimiz meyveyi keşfetmek bireysel görevimiz.<br/><br/>erkeğe de kadına da düşen iş aslında bunları sorgulamak, burada kavramsal bir zemin üretmeye çalışmak. korkular varsa bunları anlamaya çalışmalıyız. kaygıları, endişeleri, ön yargıları. bir günde çözülecek iş olmuyor çoğu zaman. bazen birlikte aşılabilecek bir şeyken bazen kadının kendi yolculuğunda kendi çözmesi gereken bir şey haline geliyor. türlü türlü iş var anlayacağınız.<br/><br/>not: bunları deli sikmiş gibi niye yazdığımı düşündüm ve fark ettim ki burayı cidden okuyanlar ve kimseden bulamayacakları (aslında kimseye soramayacakları için alamayacakları) cevaplara dair bir şeyler arayanlar var. bir kaynak olur, bir özlü söz olur, benzeri bir anı olur. içerik üretmek çok kolaylaştığı için içerik üretmenin değerini unuttuk.
  entry_channels: ilişkiler,sağlık
- entry_name: |
    donovan mitchell
  entry_id:  76298505
  entry_writer: bomonti seven fakir
  entry_fav_count: 3
  entry_content: |
    roty çekişmesini mutlu sonla sonuçlandırmak için nba yönetimi bi adamla anlaşıp <a class="b" href="/?q=malcolm+brogdon">malcolm brogdon</a>un evine girip geçen seneki ödülü çaldırmalıdır ki birini mitchell a diğerini simmons a versinler.<br/>çok büyük oynayan oyuncu.<br/><br/>edit:michael yazmışım mesajınız için teşekkürler
  entry_channels: spor
- entry_name: |
    nehirin ve ırmak'ın planetdp'den ayrılması
  entry_id:  76315941
  entry_writer: forrestgump
  entry_fav_count: 2
  entry_content: |
    planetdp'nin yaptığı dangalaklığın doğal bir sonucudur. <br/><br/>eşeğin tekiyle olan ahbap-çavuş ilişkilerini sitenin 1 numaralı kuralının önünde tutarsan böyle tekmeyi yersin işte...<br/><br/>planetdp dizinin başlığını ve çevirmenlerin açıklamasını da kaldırmış, bundan sonra altyazılar şurdan indirilebilir:<br/><br/><a rel="nofollow noopener" class="url" target="_blank" href="http://www.turkcealtyazi.org/mov/0475784/westworld.html" title="http://www.turkcealtyazi.org/mov/0475784/westworld.html">http://www.turkcealtyazi.org/…475784/westworld.html</a>
  entry_channels: tv
- entry_name: |
    şu anda çalan şarkı
  entry_id:  76292139
  entry_writer: ucazugasibere
  entry_fav_count: 1
  entry_content: |
    <a rel="nofollow noopener" class="url" target="_blank" href="https://www.youtube.com/watch?v=dHYDFClkqHw" title="https://www.youtube.com/watch?v=dHYDFClkqHw">goodnight moon</a><br/><br/>ruhumu dinlendiriyor.
  entry_channels: anket,ekşi-sözlük,sanat,müzik,edebiyat
- entry_name: |
    oğuzhan'ın tolga zengin'i tribünlere saldırtması
  entry_id:  76318183
  entry_writer: cok canim acidi ceksene elini
  entry_fav_count: 1
  entry_content: |
    bakın oğlum tolga zengin deseki "baktım takım 10 kişi kupa gidecek şanpiyonluk yarışında taraftar psikolojik okarak yara alacak ben de provakasyon için taraftara hareket yaptım amacım onları kışkırtmaktı", şenol hoca "kafama çakmak ve ponpon geldi ama yarılma yok o zımba tellerini de takımın içeri kaçması sonucu hükmen üç sıfır yenilmemiz için taktırdım. normalde bir bok olmadı kafama" dese buradaki beşiktaşlılar yine takımlarının şerefli olduğunu kendilerinin çok dürüst ve namuslu olduklarını iddia ederler.<br/><br/>mahallenin, her türlüğü itliği yapıp adamım diye gezen yavşak tipleri gibidir bunlar. bu herifler sözlükte de çoğunluktadırlar o yüzden bok gibi çullanırlar beşiktaş karşıtı bir şey yazdınız mı. yüzleri olsa şunlardan dolayı kızarırdı<br/>(bkz: <a class="b" href="/?q=fikret+orman%27%c4%b1n+karaman+valisi+ziyareti">fikret orman'ın karaman valisi ziyareti</a>)<br/>(bkz: <a class="b" href="/?q=fikret+orman%27%c4%b1n+izmir+mar%c5%9f%c4%b1+politiktir+beyan%c4%b1">fikret orman'ın izmir marşı politiktir beyanı</a>)<br/>(bkz: <a class="b" href="/?q=r%c4%b1za+zarrafa+loca+sat%c4%b1l%c4%b1phaciz+kald%c4%b1r%c4%b1lmas%c4%b1">rıza zarrafa loca satılıphaciz kaldırılması</a>)<br/>(bkz: şenol güneş'in evindeki içki şişelerinin sansürlenmesi)<br/>(bkz: <a class="b" href="/?q=fiko%27nun+g%c3%b6ztepe+ma%c3%a7%c4%b1+%c3%b6ncesi+demba+ba+a%c3%a7%c4%b1klamas%c4%b1">fiko'nun göztepe maçı öncesi demba ba açıklaması</a>)
  entry_channels: spor,haber
- entry_name: |
    kılıçdaroğlu'na açık mektup
  entry_id:  76317183
  entry_writer: insanfaktoru
  entry_fav_count: 1
  entry_content: |
    katıldıgım görüşlerdir.<br/>ecevit vatanseverliği su götürmeyen alçakgönüllü sakin bir adamdı. ancak sosyal demokrat siyasi çizgisinden asla ödün vermedi. seçmenlerine de mide bulandırıcı sağ görüşleri aday olarak dayatmadı.<br/>kılıcdaroğlu, ecevit'i örnek almalı.<br/>kılıçdaroglunun liderlik edecek vasfı ya da cesareti olmayabilir.<br/>ama chp seçmeninin gözü arkada kalmadan oy verebileceği yılmaz büyükerşen, ümit kocasakal, emine ülker tarhan hatta özgür özel gibi ulusalcı isimler var.<br/>bunlar dururken sağcı ve liboşların önümüze atılmasını hakaret kabul ediyorum. bu hakaret devam ederse kılıçdaroglu'nun bize dayattıgı sağcılardansa gider gerçek sağcılar'a oy veririm. evet. madem ki karşılıksız olarak iyi partinin yolunu açacak kadar aptalsınız, madem ki önümüze ulusalcı aday koyamayacak kadar satılmışsınız ben de tokadımı iyi parti ve akşener'e oy vererek atarım.
  entry_channels: siyaset
- entry_name: |
    hintli kokusu
  entry_id:  76317051
  entry_writer: umberto d
  entry_fav_count: 1
  entry_content: |
    (bkz: <a class="b" href="/?q=hint+kokusu">hint kokusu</a>) <br/><br/>türklü, italyanlı, ruslu diyor musunuz?
  entry_channels: ilişkiler,sağlık
- entry_name: |
    ekşi sözlük dertleşecek insan veritabanı
  entry_id:  76295614
  entry_writer: iskelealamamki
  entry_fav_count: 1
  entry_content: |
    iyi forumlar nejat, iyi forumlar kostik, iyi forumlar huseyin.
  entry_channels: anket,ilişkiler,ekşi-sözlük
- entry_name: |
    ekşi itiraf
  entry_id:  76302851
  entry_writer: gundelikyasaminhickiriklari
  entry_fav_count: 0
  entry_content: |
    sözlük nickim insanlara hüzünlü bir kadını çağrıştırıyor ama 190 boyunda döşü kıllı 120 kilo bir erkeğim.<br/><br/>ilgililere duyrulur.
  entry_channels: anket,ilişkiler,ekşi-sözlük,edebiyat
---
