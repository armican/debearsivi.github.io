---
layout: post
title: 06 Şubat 2019 Ekşi Sözlük Debe
data:
- entry_name: |
    akp'li bakanın ales'te sonuncu olan akrabası
  entry_id:  86330090
  entry_writer: saintly
  entry_fav_count: 1556
  entry_content: |
    haberlere göre akp'li bakan <a class="b" href="/?q=mevl%c3%bct+%c3%a7avu%c5%9fo%c4%9flu">mevlüt çavuşoğlu</a>'nun ales'te sonuncu olan akrabası <a class="b" href="/?q=%c5%9feyda+%c3%a7avu%c5%9fo%c4%9flu">şeyda çavuşoğlu</a>'dur. yüksek lisansı kazanmıştır ve mahkeme puan listesine erişimi engellemiş.<br/><br/>"dışişleri bakanı mevlüt çavuşoğlu’nun akrabasının ales ve yabancı dil sınavlarında son sıralarda yer almasına rağmen yüksek lisansa kabul edildi. üniversite yönetimi önce internet sitesinde puanlara ve sonuçlara ilişkin duyurulan listeyi yayından kaldırdı. daha sonra üniversite rektörlüğünün talebi üzerine mahkeme puanların gösterildiği listeye erişim yasağı getirdi."<br/><br/><a rel="nofollow noopener" class="url" target="_blank" href="http://www.cumhuriyet.com.tr/m/haber/turkiye/1231337/Bakan_akrabasina_torpil.html" title="http://www.cumhuriyet.com.tr/m/haber/turkiye/1231337/Bakan_akrabasina_torpil.html">link</a><br/><br/>siyasal islamcılar, 'kul hakkı yeme'<sup class="ab"><a title="(bkz: kendi tanımlamaları)" href="/?q=kendi+tan%c4%b1mlamalar%c4%b1" data-query="kendi tanımlamaları">*</a></sup> konusunda çıtayı everestlere çıkardılar maşallah. bu rezilliklere rağmen hâlâ seçimlerde, "küstüm oy kullanmıyorum" diyenler varsa tekrar düşünür umarım. bu karanlığın zayıflaması, güç kaybetmesi lazım.<br/><br/>ayrıca; (bkz: <a class="b" href="/entry/85611165">#85611165</a>)
  entry_channels: siyaset,haber,eğitim
- entry_name: |
    sözlüğün rayından çıkması
  entry_id:  86346855
  entry_writer: viva forza
  entry_fav_count: 980
  entry_content: |
    artık yeter...<br/><br/>edit: mesaj kutum “siktir git” le doldu. aynen devam gençler.<br/><br/>bugünkü sol frame’de görülen başlıklar ve girdilerin artık mide bulandırması sonucu vardığım kanı.<br/>(bkz: <a class="b" href="/?q=o%c4%9flunun+cenazesinde+dalin+ile+du%c5%9f+alan+baba">oğlunun cenazesinde dalin ile duş alan baba</a>)<br/>(bkz: <a class="b" href="/?q=y%c3%bcz%c3%bcne+bo%c5%9faltt%c4%b1rmayan+bayan+iticili%c4%9fi">yüzüne boşalttırmayan bayan iticiliği</a>)<br/>(bkz: <a class="b" href="/?q=eskort+fiyatlar%c4%b1ndaki+inan%c4%b1lmaz+art%c4%b1%c5%9f">eskort fiyatlarındaki inanılmaz artış</a>)<br/>(bkz: <a class="b" href="/?q=sevi%c5%9ftikten+sonra+yap%c4%b1lanlar">seviştikten sonra yapılanlar</a>)<br/>(bkz: <a class="b" href="/?q=eskortluk+ge%c3%a7mi%c5%9fi+olan+bir+kad%c4%b1nla+evlenmek">eskortluk geçmişi olan bir kadınla evlenmek</a>)<br/>(bkz: <a class="b" href="/?q=kad%c4%b1nlar%c4%b1n+en+ba%c5%9ftan+%c3%a7%c4%b1kar%c4%b1c%c4%b1+hareketleri">kadınların en baştan çıkarıcı hareketleri</a>)<br/><br/>oysaki ne güzel siyaset yapar, türk olsak, kürt olsak, eşcinsel olsak vs, işin sonu tatlıya bağlanır, aydınlanır, yeni şeyler öğrenmiş olurduk. artık yok, engellediğim eski nesil yazarların engellerini kaldırır oldum. (bkz: <a class="b" href="/?q=gelen+gideni+arat%c4%b1r">gelen gideni aratır</a>).<br/><br/>temel sebebi son dönemlerdeki yazar alımının artması. moderatörlere selam olsun...<br/><br/>20’li yaşlarında, ergenlikten yeni çıkmış ve dahi çaylaklıktan yeni çıkmış olan yazarımsı tiplerin salya sümük entarileleri artık sözlüğü çekilmez hale getirdi. patronun bu çirkin girdileri okuyarak hoşlandığını düşünür olduk.<br/><br/>beni yok sayın, ben artık yokum...<br/><br/>bozuk para gibi harcadınız eski nesil yazarları, yazık, çok yazık...<br/><br/>(bkz: <a class="b" href="/?q=ek%c5%9fis%c3%b6zl%c3%bck+%c3%b6z%c3%bcne+d%c3%b6n">ekşisözlük özüne dön</a>)
  entry_channels: ekşi-sözlük
- entry_name: |
    iş bankası hazine'nin malı olacak
  entry_id:  86334870
  entry_writer: illa ki sistem
  entry_fav_count: 463
  entry_content: |
    hazinenin malı olacak ki satıp paralarını cukkalayasın. <br/>bir zorbanın malı olan ülkede gerçekleşen bir beyandır.
  entry_channels: siyaset,haber,ekonomi
- entry_name: |
    yüzüne boşalttırmayan kadın iticiliği
  entry_id:  86344517
  entry_writer: zargonya
  entry_fav_count: 420
  entry_content: |
    (bkz: <a class="b" href="/?q=derdine+bo%c5%9falay%c4%b1m+butonu">derdine boşalayım butonu</a>)<sup class="ab"><a title="(bkz: yeni buton)" href="/?q=yeni+buton" data-query="yeni buton">*</a></sup>
  entry_channels: ilişkiler
- entry_name: |
    550 çeşit yemek var seçmesi bile zor vakit alıyor
  entry_id:  86334790
  entry_writer: aslagelmeyecektelefonubekleyenadam
  entry_fav_count: 348
  entry_content: |
    <a class="b" href="/?q=%c3%a7ocu%c4%9fuma+pantolon+alam%c4%b1yorsam+niye+ya%c5%9f%c4%b1yorum">çocuğuma pantolon alamıyorsam niye yaşıyorum</a><br/><br/>dipnot: (bkz: <a class="b" href="/entry/86333229">memişoğlu tarım ürünleri/#86333229</a>)<br/><br/>dipnot 2: (bkz: <a class="b" href="/?q=%c3%a7orum%27lu+i%c5%9fsiz+m%c3%bchendisin+hazin+sonu">çorum'lu işsiz mühendisin hazin sonu</a>)
  entry_channels: siyaset,haber,yeme-içme,ekonomi
- entry_name: |
    bugün maaşlar yatarsa alacağım ben kızıma
  entry_id:  86337585
  entry_writer: xylvr
  entry_fav_count: 326
  entry_content: |
    <a class="b" href="/?q=d%c4%b1%c5%9far%c4%b1dayken+etraftan+duyulan+%c3%bczen+diyaloglar">dışarıdayken etraftan duyulan üzen diyaloglar</a> başlığına sadece bir entry olarak girecekken, günümüz ekonomik koşullarını düşününce vazgeçtiğim; bugün bir markette kulağıma çalınan cümle. <br/><br/>öğlen markete kadar çıktım. öğleden sonra çalıştığım iş yerine gideceğim, gitmeden alışverişi halledeyim dedim. ama ayın 5'i ve maaş henüz yok. bugün yatar mı, yatmaz mı derken, kredi kartına dönmek durumunda kaldım. aç kalacak değiliz ya? hem en fazla birkaç gün gecikir! nedir ki yani! <br/><br/>ben alışveriş yaparken, hemen yan reyonda rafları dizen, üzerinde marketin üniforması/sweatshirt'ü olan bir hanımın, ilkokul 1'de olduğunu tahmin ettiğim kızı geldi "anne!" diye sarıldı! okulda yüzünü boyamış, yanağında tükenmez kalemle kocaman bir kalp. nasıl da tatlı sıpa! o sırada annesiyle konuşuyorlar; <br/><br/>-beslenmeni yedin mi? <br/>-yedim annecim. <br/>-yumurtanı, peynirini? <br/>-yedim annecim. <br/><br/>ama çocuk işte, o sırada annesinin rafa dizdiği çikolataya takıldı gözü; <br/><br/>-bundan alabilir miyim annecim? <br/><br/>raftaki çikolatalara baktım; 65 kuruşluk. 1 tl bile değil. ama kadın birden durdu ve lanet olsun ki ben o duruşu, o an suratından geçeni biliyorum! biliyorum ulan işte! hesap yaptı! bildiğin hesap yapmak için duraksadı kadın. zaten muhtemelen asgari ücretli bir çalışan. ardından da başlıktaki cümle döküldü ağzından; <br/><br/>-bugün maaşlar yatarsa alacağım ben kızıma, söz! <br/><br/>...<br/><br/>bu başlığı bu yüzden ayrıca açtım, belki denk gelirsiniz diye; bugün salı. bugün mecliste parti grup toplantıları var. bütün liderleri birer kere gösterecek o canına yandığımın televizyonları. hepsine bakın! ne olursunuz bakın! oy verdiğiniz/vermediğiniz, sevdiğiniz/nefret ettiğiniz, lider dediğiniz/küfrettiğiniz... ali, veli, ahmet, mehmet, ayşe, fatma... hepsine bakın! <br/><br/>sonra bir de en yakın aynaya bakın! <br/><br/>bu mudur hak ettiğiniz/ettiğimiz?
  entry_channels: ekonomi
- entry_name: |
    eskort fiyatlarındaki inanılmaz artış
  entry_id:  86340927
  entry_writer: seviyorsevmiyorseviyorsevmiyorseviyor
  entry_fav_count: 317
  entry_content: |
    (bkz: <a class="b" href="/?q=eskort+depolar%c4%b1na+bask%c4%b1n">eskort depolarına baskın</a>)
  entry_channels: ilişkiler,haber,ekonomi
- entry_name: |
    kocasından fazla maaş alan kadın
  entry_id:  86336756
  entry_writer: entellektuel nick
  entry_fav_count: 316
  entry_content: |
    kızı istemeye gittiğimizde öğrenciydim. onun aylık geliri 3500tl gibiydi. 2013'te büyük para. ailesine "bu kızı öğrenciyle mi evlendirdiniz?" demişler. sonra ben de işe girdim 2000tlye. o benden 1.5 kat fazla alıyordu. evlilik tarihimiz belli oldu. hemen öncesinde iş değiştirdim. bu kez ben onun 2 katını alıyorum. kız aynı kız. ben aynı ben. yani bu dengeler değişken. yarın işsiz kalırım, hanımın maaşı ile geçiniriz. büyük konuşmamak lazım.
  entry_channels: ilişkiler,ekonomi
- entry_name: |
    oğlunun cenazesinde dalin ile duş alan baba
  entry_id:  86345243
  entry_writer: bucamania
  entry_fav_count: 279
  entry_content: |
    çok güldük! şimdi o iğrenç esprinizi iki lopunuzun arasına sokup başka gezegene yol alabilirsiniz.<br/><br/>not: bu sözlük yönetiminin gerçekten bu herifleri sözlükten ilelebet uçurması lazım! ileri de bu kalitesizlik sözlüğün kendi reklam gelirlerini azaltır. kanzuk bey ister misiniz iki tane çomar yüzünden gelirleri kaybetmek! <br/><br/>para diyorum, zenginlik diyorum kapiş!
  entry_channels: sağlık
- entry_name: |
    çiğköfteci ali ustanın ufak çocuğa bağırması
  entry_id:  86349667
  entry_writer: neyzenruhat
  entry_fav_count: 180
  entry_content: |
    böyle cahil bir boz ayıyı popüler eden kişiliksizlikler utansın. ülke psikolojisinin özetidir bu adam ve yancıları.
  entry_channels: yeme-içme
- entry_name: |
    insanlık tarihinin en önemli iki olayı
  entry_id:  86331320
  entry_writer: melro
  entry_fav_count: 178
  entry_content: |
    yazının icadı<br/>gıdaları pişirerek yemeye başlamak.<br/>yazının icadı soyut düşünceyi ve bugünün insanını inşa ederken, gıdaların pişirilerek yenmesi besin çeşidini çoğaltmıştır. bu ikisi olmasaydı diğer gelişmelerin hiçbiri olmayacaktı.
  entry_channels: anket,tarih
- entry_name: |
    eskortluk geçmişi olan bir kadınla evlenmek
  entry_id:  86342585
  entry_writer: kuyruk yagi
  entry_fav_count: 176
  entry_content: |
    markete gidip ucu yenmiş ekmek almak gibidir. <br/><br/>bunun geçmişiyle alakası yok. para karşılığı kendini satmış bir insana güvenmem ben. şartlar ne olursa olsun.
  entry_channels: anket,ilişkiler
- entry_name: |
    şeyda çavuşoğlu
  entry_id:  86331929
  entry_writer: mikadonun pelerini
  entry_fav_count: 170
  entry_content: |
    oğlum takılmayın lan böyle şeylere, kabullenin artık bu durumu..lütuf gösterip, kalkıp gelip sınava girmiş bu bile bir şeydir bence.. yani bu bile büyük olay çünkü bir telefon bile yeterdi işini hallettirmek için..o sizin dediğiniz eşik aşılalı yıllar oldu, sadece artık her şeyden haberimiz oluyor çünkü kimsenin saklamak gibi bir korkusu kalmadı.. onun için takmayın, boşverin bu mevzuyu,geriye dönmez artık bu..
  entry_channels: siyaset,haber,eğitim
- entry_name: |
    sakalsız erkekler istiyoruz
  entry_id:  86332950
  entry_writer: gaftici fethi
  entry_fav_count: 146
  entry_content: |
    erkeklik, kimseye muhtaç olmadan, işinde gücünde , ailesine, büyüklerine, küçüklerine maddi manevi destek, koruyucu, kollayıcılıkla, bir uzmanlık sahibi olmakla, saygıdeğer bir hayat yaşamakla olur. annesinin arabasını kullanıp babasının kredi kartı ile caddede nişantaşında cafede laklak yapan 1 karış sakalla gördüğünüz tipler en fazla oğlan çocuğu olur. sakalla kafayı yemiş, sakalım da sakalım diye konuşan bebe en fazla liseli ergendir.
  entry_channels: ilişkiler,moda
- entry_name: |
    metrobüs durağında görülen astronot
  entry_id:  86337475
  entry_writer: nickbulamadimnolcak
  entry_fav_count: 141
  entry_content: |
    düşün ki içerisi ne kadar havasız? adam astronot elbisesi giyme riskini göze alarak sırtında oksijen tüpüyle geliyor.<br/><br/>t: metrobüs için doğru ekipman seçmiş astronot.<br/><br/>edit: imla.
  entry_channels: haber
- entry_name: |
    hacettepe üniversitesi asistan alım rezaleti
  entry_id:  86339504
  entry_writer: gregorsamsamsi
  entry_fav_count: 140
  entry_content: |
    iyiniyetliücüncüsahis2 nickli çaylağın isteği üzerine açılmış başlıktır. başlıkta herhangi bir fikri tasarrufum yoktur. başlığın açılmasını isteyen çaylak arkadaşın fikirleridir hepsi.<br/><br/>hacettepe üniversitesi hukuk fakültesi medeni hukuk anabilim dalına alınacak olan araştırma görevlisi sınavında büyük bir rezalet gerçekleşmiştir.sınava giren adaylar arasında ales puanı bazında sondan 2.olan,lisans mezuniyeti bazında yine sondan ikinci olan adayın(gülin süleymanlar) mülakat puanına 90 verilerek sınavı kazanma hadisesidir.sınavı kazanma durumunda kesin gözüyle bakılan ales puanı 85,lisans mezuniyeti 95,8 ve yabancı dil puanı 98 olan adayın(mehmet emir göka) mülakat puanına 40 verilerek sınavda en yüksek puan alan ikinci kişi olmasıdır.kadro 1 kişilik olup asli kazanan gülin süleymanlar,yedek ise mehmet emir göka olmuştur.<br/><br/><a rel="nofollow noopener" class="url" target="_blank" href="http://www.hukukfakultesi.hacettepe.edu.tr/sonucarş.pdf" title="http://www.hukukfakultesi.hacettepe.edu.tr/sonucarş.pdf">ilan edilen puanlama</a><br/><br/><a rel="nofollow noopener" class="url" target="_blank" href="https://www.antalya.edu.tr/uploads/staff/gulin-suleymanlar_1531910198.pdf" title="https://www.antalya.edu.tr/uploads/staff/gulin-suleymanlar_1531910198.pdf">kazanan arkadaşın cvsi merak eden için</a>
  entry_channels: eğitim
- entry_name: |
    kız kulesi değil kadın kulesi
  entry_id:  86348583
  entry_writer: no one knows who i am
  entry_fav_count: 132
  entry_content: |
    (bkz: <a class="b" href="/?q=bayan+kulesi">bayan kulesi</a>)
  entry_channels: ilişkiler
- entry_name: |
    yassine benzia
  entry_id:  86338080
  entry_writer: kurtcobaince
  entry_fav_count: 120
  entry_content: |
    iki maçtır iyi olması koca sezonun amq koyduğu gerçeğini değiştirmiyor.kadro dışı ya da fesh olarak tüm yaptırımları destekliyorum.
  entry_channels: spor
- entry_name: |
    askerde tekli oda isteyebilir miyiz sorunsalı
  entry_id:  86346675
  entry_writer: loosey
  entry_fav_count: 111
  entry_content: |
    kesinlike sorunsal degildir. tamamen alay ya da tugay komutaniyla kuracaginiz iliskiye baglidir. <br/><br/>birlige teslim oldugunuz gun hic vakit kaybetmeden tugay komutaniyla konusmak istediginizi soyleyin. sebebini soran olursa konunun cok onemli oldugunu, ancak tugay komutaniyla paylasabileceginizi dile getirin. asla ama asla, yuzbasi, binbasi ve hatta yarbay gibi dusuk rutbelerle tartismaya girmeyin; bu sadece sureci uzatacaktir. <br/><br/>tugay komutaninin odasina girdiginizde sakin kalin, derdinizi duzgun cumlelerle anlatmaya calisin. kogus arkadaslarinizdan bazilarinin cok horladigini, bundan dolayi uyumakta zorlandiginizi soyleyin. sonucta tugay komutaninin oncelikli gorevi sizin askerde gecirdiginiz sureyi olabilecek en iyi sekilde gecirmenizi saglamak... <br/><br/>bazi durumlarda biraz daha israrci olmaniz gerekebilir elbette, sonucta iclerinde anlayissiz insanlar da olabiliyor. bu durumda siz de sesinizi biraz yukseltebilirsiniz. boyle bir durumla karsilasirsaniz sert ancak nezaketi de elden birakmayacak sekilde tugay komutanina bir ust paragrafta da belirttigim uzere, gorevinin sizi rahat ettirmek oldugunu hatirlatin. bu konuda son sozun sizde oldugunu ve kendinize ait bir odada uyumanin kirmizi cizginiz oldugunu vurgulayin. sorun kesinlikle cozulecektir.<br/><br/><a class="b" href="/?q=hayirli+tezkereler">hayirli tezkereler</a>
  entry_channels: anket
- entry_name: |
    80'li yıllardaki zenginlik belirtileri
  entry_id:  86329823
  entry_writer: x uzeri y nin z ye gore turevi
  entry_fav_count: 105
  entry_content: |
    (bkz: <a class="b" href="/?q=maymunlu+lc+waikiki">maymunlu lc waikiki</a>)
  entry_channels: anket
- entry_name: |
    yazarların favori atasözü
  entry_id:  86322206
  entry_writer: piano sonata
  entry_fav_count: 90
  entry_content: |
    <a class="b" href="/?q=sekreterini+siken+%c3%a7ay%c4%b1n%c4%b1+kendi+al%c4%b1r">sekreterini siken çayını kendi alır</a>
  entry_channels: anket,edebiyat
- entry_name: |
    herkesin inandığı en büyük yalan
  entry_id:  86337152
  entry_writer: tornafreze
  entry_fav_count: 66
  entry_content: |
    türkiye laik bir hukuk devletidir. yönetim şekli cumhuriyettir.
  entry_channels: anket
- entry_name: |
    mike pompeo'nun türkiye'yi bölünmüş göstermesi
  entry_id:  86350315
  entry_writer: kurtz
  entry_fav_count: 65
  entry_content: |
    (bkz: <a class="b" href="/?q=kanal+ankara">kanal ankara</a>)
  entry_channels: 
- entry_name: |
    8/5 çalışmak vs 9/6 çalışmak
  entry_id:  86330550
  entry_writer: kendimettimkendimbuldum
  entry_fav_count: 59
  entry_content: |
    ikisi de özendiğim çalışma türü, son zamanlarda 9-9 çalıştığım için, evden çıkışım varışım da 7-10 olduğu için, ikisinin de her türlü gideri var benim açımdan.
  entry_channels: anket,ekonomi
- entry_name: |
    soğan %231 biber %155 pırasa %116 patates %111
  entry_id:  86340055
  entry_writer: insan donuna girmis telli turna
  entry_fav_count: 51
  entry_content: |
    gida malzemeleri ve bu gidalara son 1 yil icerisinde gelen <a class="b" href="/?q=zam+oranlari">zam oranlari</a>.<br/><br/>"akp'li kardeşlerime sesleniyorum. her yetkiyi verdiniz yetmedi. bana yetki verin tek adam olayım her şey düşecek dedi; verdiniz yine yetmedi. kalkmış şimdi esnafı, çiftçiyi suçluyor. sanki bunlar uyuşturucu satıyormuş gibi baskın yapıyorlar. değerli arkadaşlarım ne oldu da bu zamlar geldi? bakın bir yıllık değişimi veriyorum size. bir yılda ne oldu? kuru soğanda artış yüzde 231. biberde artış yüzde 155, pırasada yüzde 116, patateste yüzde 111. böyle gidiyor liste."<br/><br/><a rel="nofollow noopener" class="url" target="_blank" href="https://odatv.com/chpden-flas-transfer-05021949.html" title="https://odatv.com/chpden-flas-transfer-05021949.html">https://odatv.com/…pden-flas-transfer-05021949.html</a>
  entry_channels: ekonomi
- entry_name: |
    toronto'da ev alınacak en uygun muhit
  entry_id:  86340015
  entry_writer: pimp of pleasure
  entry_fav_count: 45
  entry_content: |
    (bkz: <a class="b" href="/?q=toki">toki</a>)<br/><br/>toronto konut idaresi..
  entry_channels: anket,ekonomi,seyahat
- entry_name: |
    çocuğunuzun ateşinin kimseyi ilgilendirmemesi
  entry_id:  86329767
  entry_writer: a kompleman
  entry_fav_count: 40
  entry_content: |
    maksadını aşan bir beyan ama hakaret etmenin de anlamı yok. kısmen de olsa var böyle bir durum maalesef. bizim toplum çocuk çok sever ama o çocuk kendisinin olursa. insanlar bazen iyi niyetle ama çoğunlukla anlamsızca çocuklarını hayatın merkezine koyup, çocukla ilgili gereksiz detayları (anne-baba için çok önemli olabilir) sürekli anlatıp, herkesden aynı ilgiyi bekleyebiliyor. insanlar biraz dikkatli olmalı. elbette yeri geldiğinde herkes çocuğundan belli ölçülerde bahsedebilir ama karşı tarafın bundan mutlu olamayabileceğini ya da dinleyenler arasında çocuk sahibi olamayan birilerinin de olabileceğini düşünmeli. not: çocuğum var.
  entry_channels: ilişkiler,sağlık
- entry_name: |
    sedat peker'den meral akşener'e imalı tweetler
  entry_id:  86348517
  entry_writer: atina gore kisneyen sahip
  entry_fav_count: 24
  entry_content: |
    kendini ihbar ettiği tweetlerdir.
  entry_channels: siyaset,haber
- entry_name: |
    1 ak partiliyi ikna et
  entry_id:  86349783
  entry_writer: hawt
  entry_fav_count: 22
  entry_content: |
    bunu yapabilen biri var mı gerçekten çok merak ediyorum. ben beceremiyorum usta. sonra stres olup kendi motivasyonumu kaybediyorum.
  entry_channels: siyaset
- entry_name: |
    sol framede görme hayali kurulan başlık
  entry_id:  86349894
  entry_writer: apoolat
  entry_fav_count: 20
  entry_content: |
      (nokta).... vefatı
  entry_channels: anket,ekşi-sözlük
- entry_name: |
    kanser olmak isteyenlere tavsiyeler
  entry_id:  86351028
  entry_writer: ikitekeryedinota
  entry_fav_count: 15
  entry_content: |
    her boku kafanıza takarak başlayabilirsiniz.
  entry_channels: anket
- entry_name: |
    metallica
  entry_id:  86348472
  entry_writer: broken beat scarred
  entry_fav_count: 13
  entry_content: |
    amcalar ve teyzeler dışında kimler dinliyormuş bir bakalım.<br/><a rel="nofollow noopener" class="url" target="_blank" href="https://www.youtube.com/watch?v=Ax8UcJuEiAY">https://www.youtube.com/watch?v=ax8ucjueiay</a><br/><br/>buna da bakalım isterseniz. <a class="b" href="/?q=foo+fighters">foo fighters</a> konserinde <a class="b" href="/?q=dave+grohl">dave grohl</a> sahneye bir çocuk alıyor ve bilin bakalım çocuk ne çalıyor?<br/><a rel="nofollow noopener" class="url" target="_blank" href="https://www.youtube.com/watch?v=s-NkVcJiqPo">https://www.youtube.com/watch?v=s-nkvcjiqpo</a><br/><br/>metallica 7'den 70e herkesi etkilemiş ve etkilemeye devam eden bir gruptur.
  entry_channels: tv,sanat,müzik
- entry_name: |
    almanya'nın çıplak sauna kültürü
  entry_id:  86349600
  entry_writer: faith no more
  entry_fav_count: 13
  entry_content: |
    var böyle birşey, evet. <br/><br/>almanya'ya gittiğimde bir büyüğümüz bizi havuzlu saunalı bir tesise götürmüştü. tabii götürmeden önce böyle bir zorunluluk olduğunu söylemedi. önce havuzda biraz çimdik, sonra saunaya çıktık. o sırada büyüğümüz bombayı patladı: "beyler buradan sonrasında çıplak olmamız gerekiyor!"...<br/><br/>beynim amcıkladı. gerçekten amcıkladı. şaka da değildi, herkes çıplaktı. millet çırılçıplak geziyordu, kadın erkek farketmeksizin, gayet de rahatlardı.<br/><br/>mayolarımızı dolaplara bıraktık, havluya izin vardı. tesisler arasında dolanırken beline havluyu dolayıp, durumu kurtarabiliyordun fakat saunaya ve termal havuzlara girişte el mahkum havluyu bırakman ya da altına sermen gerekiyordu. yani büyüğümüze ayıp olmasın diye tesislere girdik ama o çıplaklık utancını aşmak mümkün değildi. bir noktadan sonra bende "götü umuma açtık bari malı elden geldiğince kurtaralım." hissiyatı oluştu, artık kurtarabildiğim kadar...<br/><br/>tabii almanlar için bu durum normalleşmiş. saunaya giriyorsun, herkes yan yana çırılçıplak oturuyor. kimse kimseye bakmıyor, etmiyor. üst katta kafe alanına çıkıyorsun, millet eşiyle çocuğuyla şezlonglarda çırılçıplak uzanmış, bira içiyor, sohbet ediyor. bizim için imkansız, çok aşırı kareler...<br/><br/>bir süre meme, pipi, kuku ve popo görmek istememiştim, o derece...
  entry_channels: sağlık,haber
- entry_name: |
    yeni sevgilide olması istenen özellikler
  entry_id:  86321331
  entry_writer: morkedili
  entry_fav_count: 11
  entry_content: |
    eski hayatında kedi olması
  entry_channels: anket,ilişkiler
- entry_name: |
    anne mi önce gelir yoksa eş mi sorunsalı
  entry_id:  86351943
  entry_writer: mr frodo
  entry_fav_count: 10
  entry_content: |
    kişinin kendisi önce gelir. ikisi de kişiyi mutsuz ediyorsa ikisiyle de ilişki kesilebilir.
  entry_channels: anket
- entry_name: |
    survivor 2019 türkiye-yunanistan
  entry_id:  86326720
  entry_writer: clinomanistikwoman
  entry_fav_count: 8
  entry_content: |
    acun bu yarışma yüzünden iki devlet arasinda savaş çıkartıp kanalında savaş spikerliği yapsa şaşırmam
  entry_channels: tv
- entry_name: |
    özel sektör 2019 maaş zam oranları
  entry_id:  86345192
  entry_writer: komutan bey
  entry_fav_count: 7
  entry_content: |
    masallah en az zam alan yuzde otuz almis. la kriz basliginda yok patlican 20 lira olmus, yok carliston biber 35 lira diye aglayanlar kim oyleyse!? o zaman dans.
  entry_channels: anket,haber,ekonomi
- entry_name: |
    antidepresan etkisi gösteren şeyler
  entry_id:  86347098
  entry_writer: defoe
  entry_fav_count: 6
  entry_content: |
    beğenilmek, ilgi görmek.
  entry_channels: anket,sağlık
- entry_name: |
    nilhan osmanoğlu
  entry_id:  86337366
  entry_writer: scorpioraptor
  entry_fav_count: 5
  entry_content: |
    bu karıya birinin ayar vermesi gerek derhal. basbaya türkiye cumhuriyeti düşmanlığı yapıyor salak karı.<br/><br/>bıraksak geçecek dolmabahçede oturacak neyin kafasını yaşıyor siz düşünün.<br/><br/>allahım sen bize sabır ver bunların devranı çok uzadı yeter artık!
  entry_channels: siyaset,magazin
- entry_name: |
    5 şubat 2019 trabzonspor ümraniyespor maçı
  entry_id:  86350074
  entry_writer: malachi constant
  entry_fav_count: 5
  entry_content: |
    spikerin yanındaki yorumcuya kanalın trabzonspor kanalı olmadığını birinin hatırlatması gereken maç. sanki yabancı takımla oynuyorlar gibi trabzon merkezli yorumlar yapmakta.
  entry_channels: spor
- entry_name: |
    zeki uzundurukan
  entry_id:  86351402
  entry_writer: srfydn
  entry_fav_count: 4
  entry_content: |
    futbol kurallarını bilmeyen sözde futbol yorumcusu.
  entry_channels: spor,tv
- entry_name: |
    solosu mükemmel parçalar
  entry_id:  86350702
  entry_writer: fakeleri olan unlu adam
  entry_fav_count: 4
  entry_content: |
    tabii ki;<br/><br/><a rel="nofollow noopener" class="url" target="_blank" href="https://youtu.be/xnKhsTXoKCI?t=210" title="https://youtu.be/xnKhsTXoKCI?t=210">master of puppets</a>
  entry_channels: müzik
- entry_name: |
    şener özbayraklı
  entry_id:  86332983
  entry_writer: qatal
  entry_fav_count: 4
  entry_content: |
    tolgay takasında beşiktaş'ın önerdiği parayı beğenmeyerek bu takıma gitmemek kendi kararıydı. bu sebeple kimse suçlayamaz kendisini. yalnız eğer bir karar alıyorsan da bunun sonuçlarına katlanacaksın. fenerbahçe'nin yaptırımı da kendi açısından haklı. futbol piyasasına çıkardığı, yıllarca iyi ücret ödediği futbolcudan içinde bulunduğu sıkıntılı durumdan kurtulmak için bir fedakarlık istedi ve futbolcu bunu yapmadı. bu saatten sonra futbolcu ile kulübün normal ilişkilerine devam etmeleri mümkün değil. <br/><br/>galatasaray, beşiktaş ve başakşehir'in şener'in mevkisinde oyuncuları var. şener'in bu takımlarda oynayan oyunculardan bariz artıları bulunmuyor. bu sebeple şener'i as takımda düşünüp, ona göre bir kontrat vereceklerini düşünmüyorum. ben şener'in yerinde olsam hali hazırda bir teklif varken, o teklif de beşiktaş gibi büyük bir takımdan gelmişken (çok da düşük bir teklif değil bu) bunu kabul ederdim. bu işin sakatlığı var, kadro dışısı var, ekonomik krizi var, ffp'si var... serbest kaldığında istediği paraların kendisine önerileceğinin bir garantisi yok. hatta başakşehir ve galatasaray kendisini hiç düşünmeyebilirler de. çünkü ikisinin de o mevkileri dolu gözüküyor. beşiktaş ortadaki tek alıcı olabilir. onlar da istedikleri gibi fiyatı aşağı çekebilirler. <br/><br/>bu arada beşiktaş'ın yönetim seçimi var, teknik direktörünün değişmesi olasılığı var, yeni yönetim, teknik adam şener'i hiç düşünmeyebilirler de. ben olsam beşiktaş'a gider, gökhan'dan formayı alır, gökhan ortadan çekilince de maaşıma zam isterdim. fenerbahçe'de aldığım rakamlara ulaşabilirdim belki o zaman. bilemiyorum şener, bence çok yanlış yaptın.
  entry_channels: spor
- entry_name: |
    ronaldo luis nazario de lima
  entry_id:  86348293
  entry_writer: tovbeli korsan
  entry_fav_count: 3
  entry_content: |
    futbolu en az 6 yaşımdan beri takip ederim. envai türlü fm, fifa, pes oyunu oynadım. dünya futboluna çok küçük yaşlardan beri hakimim. yaşım da 30’ları geçti ha yanlış anlaşılma olmasın. ronaldo (no9) hayranlık duyduğum bir adamdı ama messi ve cr7 ile karşılaştırılması ve daha iyi bulunması ya marjinal olup dikkat çekme amaçlıdır ya da eski günlere duyulan özlemdir.
  entry_channels: spor
- entry_name: |
    organize işler sazan sarmalı
  entry_id:  86322672
  entry_writer: ekoow
  entry_fav_count: 1
  entry_content: |
    yılmaz erdoğan, şu ana kadar filmini izleyen <a class="b" href="/?q=1+milyon">1 milyon</a> kişiye instagram’da teşekkür etmiş.<br/><br/>sanırım ekşi ile boykot geçiyor!
  entry_channels: sinema,spoiler
- entry_name: |
    geceye bir 2023 yılı efsanesi bırak
  entry_id:  86351021
  entry_writer: sayim listesi
  entry_fav_count: 1
  entry_content: |
    (bkz: <a class="b" href="/?q=s%c3%bcper+g%c3%bc%c3%a7+olucaz">süper güç olucaz</a>)
  entry_channels: anket
- entry_name: |
    ekşi sözlük dertleşecek insan veritabanı
  entry_id:  86325485
  entry_writer: 1 618 gr
  entry_fav_count: 1
  entry_content: |
    bir şey merak ediyorum, siz gerçekten her gece birbirinize derdinizi anlatıyor musunuz lan? <br/><br/>(bkz: <a class="b" href="/?q=derdini+sikeyim+butonu">derdini sikeyim butonu</a>)
  entry_channels: anket,ilişkiler,ekşi-sözlük
- entry_name: |
    çorum'lu işsiz mühendisin hazin sonu
  entry_id:  86353096
  entry_writer: başlık taşıyan
  entry_fav_count: 0
  entry_content: |
    (bkz: <a class="b" href="/?q=%c3%a7orumlu+i%c5%9fsiz+m%c3%bchendisin+hazin+sonu">çorumlu işsiz mühendisin hazin sonu</a>)
  entry_channels: siyaset,haber,ekonomi
- entry_name: |
    başlıklar yayınlanmadan moderatör denetlesin
  entry_id:  86351589
  entry_writer: beyazbirsayfaactim red
  entry_fav_count: 0
  entry_content: |
    1984 e hoş gelmemize vesile olacak abuk taleptir.<br/><br/>kaç tane moderatör var ki, açılan binlerce başlığı denetleye bilsin.<br/><br/>en doğrusu, yazar olma kriterlerinin daha sıkı ve etkili bir şekilde uygulanmasıdır.
  entry_channels: ekşi-sözlük
- entry_name: |
    anksiyete bozukluğu
  entry_id:  86348837
  entry_writer: atcephesi
  entry_fav_count: 0
  entry_content: |
    muazzam zorlayıcı bir rahatsızlık. hiçbir şeyden emin olamama, uykuya geçememe (deli gibi uykunuz olduğu halde 4-5 saat yatakta dönüp durmanıza sebep olabilir). doktor kontrolünde ilaç tedavisi, ilacı bırakmak ise rahatsızlıkla başabaş gelir. öyle sarsıcı. velhasılı kelam canıma tak etti ilacı bıraktım. bırakma yan etkilerini halletmek için de melisa çayı kullandım. 1 senedir ses yok. geçmiş midir kuluçkada mıdır bilmem ama iyi ki varsın melisa çayı (bkz: <a class="b" href="/?q=anneanne+tavsiyeleri">anneanne tavsiyeleri</a>) (bkz: <a class="b" href="/?q=kocakar%c4%b1+ila%c3%a7lar%c4%b1">kocakarı ilaçları</a>)
  entry_channels: bilim,sağlık
---
