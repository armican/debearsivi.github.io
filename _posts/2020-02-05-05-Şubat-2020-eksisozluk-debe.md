---
layout: post
title: 05 Şubat 2020 Ekşi Sözlük Debe
data:
- entry_name: |
    sedat peker'in yurt dışına kaçması
  entry_id:  101574982
  entry_writer: kalemi kilicindan keskin olan adam
  entry_fav_count: 503
  entry_content: |
    eğitim için gittiğini söylemiş.<br/>türkiye'de üniversitelere botokslular giremez diye bir kaide yok.<br/>kaçmak yakışıyor mu senin gibi bir mafyahahahaahaaa.<br/>hayır bir de tehdit etmiş. hakkımda ileri geri konuşanlara onu yaparım bunu yaparım demiş.<br/>ya sedatcığım siktir et onu bunu, beykoz'daki evini satıyor musun onu söyle sen.<br/>hani şu apar topar çıktığın evden bahsediyorum.<br/>evet evet, ana kapısından çıkamadığın evden.
  entry_channels: siyaset,magazin
- entry_name: |
    trabzon rize artvin üçlüsü
  entry_id:  101573214
  entry_writer: menaceofvanity
  entry_fav_count: 471
  entry_content: |
    bu iki ille artvin'i yanyana koyan insanların genel kültür seviyesi yerin dibindedir. yine bu low iq arkadaşlar için tek temennim olacak; umarım trabzonlu veya rizeli biriyle izdivaç edersiniz.<br/><br/>not:ekşisözlük artvinlileri koruma ve yaşatma spor.
  entry_channels: tarih,seyahat
- entry_name: |
    63 bin kişilik çamlıca camii'nin bomboş olması
  entry_id:  101544445
  entry_writer: maya190309
  entry_fav_count: 471
  entry_content: |
    fabrika yapılsa, 63.000 kişi iş sahibi olabilirdi. madem atıl duruyor, sokakta kalanlara açsınlar camiyi, boşuna sıcak ve ışıklar yanıyor. israf olmasın hiç yoktan.
  entry_channels: siyaset,haber,ekonomi
- entry_name: |
    kanser ilaçlarının devlet tarafından karşılanması
  entry_id:  101559380
  entry_writer: civil engineer 26
  entry_fav_count: 456
  entry_content: |
    insanlar 30-40 sene sağlık sigortası ödüyorlar devlete. sonra kansere yakalanıyorlar ve bu hastalıkta faydası kanıtlanmış kişiyi ölümden kurtarabilecek ilacın parasını sigorta karşılamıyor. <br/><br/>bu insanlar sana 20-30 liralık grip ilacını, boğaz antibiyotiğini ücretsiz alayım diye mi onca yıl para ödedi, gençliğini verdi.<br/><br/>(bkz: <a class="b" href="/?q=road+to+american+capitalism">road to american capitalism</a>)
  entry_channels: siyaset,bilim,sağlık,haber
- entry_name: |
    hiç kimsenin coco star yememesi
  entry_id:  101577317
  entry_writer: nobees
  entry_fav_count: 381
  entry_content: |
    ben,<br/>üçlü beşli alıyorum hemde :)
  entry_channels: yeme-içme
- entry_name: |
    kimse yaşattığını yaşamadan bu dünyadan ayrılmaz
  entry_id:  101561484
  entry_writer: sutlabi
  entry_fav_count: 370
  entry_content: |
    geri zekalı avuntusundan başka bir şey değildir.
  entry_channels: anket,ilişkiler
- entry_name: |
    koca adam olup hala yapılan şeyler
  entry_id:  101565796
  entry_writer: eagle is free
  entry_fav_count: 347
  entry_content: |
    pudingi hala çay kaşığıyla yiyorum. çabuk bitmesin mantığı hala devam ediyor. yaş:32.
  entry_channels: anket
- entry_name: |
    15 bin tl birikimle evlenmeye teşebbüs eden erkek
  entry_id:  101560201
  entry_writer: verikontrol
  entry_fav_count: 305
  entry_content: |
    - düğün yapmamalı.<br/>- eşya vs abartmamalı - ki ileride aldığın şeylerin ne kadar gereksiz olabileceğini görüryor insan.<br/>- mütevazı bir ev tutmalı. <br/>- cidden aklı başında bir kadın bulmalı. (bunu bulunca, gerisi zaten geliyor.)<br/><br/>edit : ısrarla "mütevazı" yı "mütevazi" diye düzeltmeye çalışan tdk özürlü yazar arkadaşlara istinaden editleme gereği duydum. bilmiyorsan bir aç bak be arkadaşım.<br/><br/><a rel="nofollow noopener" class="url" target="_blank" href="https://sozluk.gov.tr/?kelime=mütevazi" title="https://sozluk.gov.tr/?kelime=mütevazi">mütevazi</a><br/><a rel="nofollow noopener" class="url" target="_blank" href="https://sozluk.gov.tr/?kelime=mütevazı" title="https://sozluk.gov.tr/?kelime=mütevazı">mütevazı</a><br/><br/>edit 2 : ne mütetaviymiş arkadaş. beni kendimden soğuttu.
  entry_channels: anket,ilişkiler,ekonomi
- entry_name: |
    gamze özçelik
  entry_id:  101576966
  entry_writer: shadow of death
  entry_fav_count: 228
  entry_content: |
    açıkmış kapalımış çok önemli değilde afrikada su kuyuları açıp her kuyuya şehitlerimizin isimlerini vermesi takdire şayan.
  entry_channels: tv,moda,magazin
- entry_name: |
    4 şubat 2020 erdoğan'ın virüs yorumu
  entry_id:  101575903
  entry_writer: boyle bir nick zaten var
  entry_fav_count: 191
  entry_content: |
    koronavirüs ile ilgili tedbirler neler olacak? bundan sonra bu tür salgın hastalıklara karşı türkiye’nin tedbirleri neler olacak? sizin virüse/virüslere karşı özel bir önleminiz var mı ?<br/>rte: <br/>biz ilk işimiz olarak dedik ki bu kadar insan çin’de sıkıntıyı yaşıyor, orada bizim vatandaşlarımız da var, onları bir defa kurtaralım. şimdi arkadaşlarıma dedim ki “biz 20 yıl önce böyle bir uçağı çin’e gönderebilir miydik? veya böyle bir uçağımız var mıydı?” şimdi ise hamdolsun bu uçağı tamamen hastane haline dönüştürdük, içine yataklar yerleştirdik. oksijen, serum vesaire her şeyi sağlık ekipleri uçağa yüklediler ve doktorlar, hemşireler çin’e gittiler. şu anda da ankara’da bunlara komple bir hastaneyi tahsis ettik. bu hastanede her biri bir odada yalnız yatıyor. bunların yanında da 7 azeri, 3 gürcü, bir de arnavut getirdik. şimdi tabi dünyada ses getirdi. <br/><br/>ama benim bir tavsiyem var. tabii bunu sağlıkçılar da söylüyor. yine de üşütmeyin. aman ateşe falan dikkat edin. tabi bunlar belirtileri. hepsinden öte kendinize güvenin. gıdalarınıza dikkat edin. bizim özel tedbirlerimiz var mı derseniz inanın yok. bazı arkadaşlar ara sıra dut pekmezi yollar. her sabah bir kaşık alırım, kan yapar.<br/><br/>güldürmüştür, güldürürkende düşündürtmüştür. :)<br/><br/><a rel="nofollow noopener" class="url" target="_blank" href="https://twitter.com/sputnik_TR/status/1224685229157371904?s=19" title="https://twitter.com/sputnik_TR/status/1224685229157371904?s=19">link</a><br/><br/>edit: açıklamanın tamamı eklendi.<br/>edit 2: bir başlık için üzdün beni moderatör.
  entry_channels: siyaset,haber
- entry_name: |
    vücudunda hiç dikişi kırığı yanığı olmayan insan
  entry_id:  101572736
  entry_writer: baska bir kafa mumkun
  entry_fav_count: 152
  entry_content: |
    valla eğer böyle biri varsa annesi onu gerçekten pamuklara sarmıştır. ben resmen savaş gazisi gibiyim o konuda:)
  entry_channels: sağlık
- entry_name: |
    4 şubat 2020 manisa depremi
  entry_id:  101545959
  entry_writer: asbe
  entry_fav_count: 138
  entry_content: |
    kimin hissettiği kimin hissetmediği inanılmaz mühim olan deprem. herkes tek tek yazsın
  entry_channels: bilim,haber
- entry_name: |
    dövüşmelik arkadaş veri tabanı
  entry_id:  101574308
  entry_writer: natural born gamer
  entry_fav_count: 82
  entry_content: |
    75 kiloluk adam da gelmiş 2v1 kapışalım diyo, haddini siktimin vitaminsiz köpeği biz 75 kilo ile göğüs basıyoruz senin neyini döveyim ben, yatırır sikerim en fazla.
  entry_channels: anket,ekşi-sözlük
- entry_name: |
    çin'de idrarda pişmiş yumurta yenmesi
  entry_id:  101569473
  entry_writer: tohumunaparamisaydim
  entry_fav_count: 72
  entry_content: |
    tam kaynayınca mikroplar ölüyor diyecektim ki, bu ibneler kaynayan idrarı döküp yenisini koyuyorlarmış. yetmezmiş gibi, bir de kabuğunu soyup öyle taze idrarın içine koyuyorlarmış. o da yetmez, hemen koyup çektik olmaaaz, en az 24 saat o idrarda kabuğu soyuk şekilde bekleyecek.<br/><br/>niye tuvalet gibi bi kavramları var, onu anlayamadım yalnız. direkt birbirlerinin ağzına işeyip sıçsınlar, iş gücünden, emekten tasarruf etsinler!
  entry_channels: sağlık,haber,yeme-içme,seyahat
- entry_name: |
    15 bin tl istanbul vs 3 bin 500 euro maaş berlin
  entry_id:  101580199
  entry_writer: nenay
  entry_fav_count: 72
  entry_content: |
    15000 lira günümüz kuru ile 2274 euro yapmakta. <br/><br/>berlin'de kira hariç aşağı yukarı her şey istanbul ile aynı ya da daha ucuz. <br/><br/>özellikle elektronik ürünler, otomobil, ıvır zıvır tüketim malzemeleri daha ucuz. <br/><br/>aradaki 1226 euro fark istanbul'daki kira farkının üzerine gitse, ki o kadar fark olmaz. <br/><br/>aynı paraya ben berlin'de olmayı tercih ederim. <br/><br/>hem daha insancıl çalışma saatleri ile kendime ayıracak zamanım olur.<br/><br/>edit: çoluk çocuğa berlin'de ücretsiz aldırabileceğim eğitimin kalitesi ile istanbul'da ücretli ya da ücretsiz her şekilde aldırabileceğim eğitim kalitesi arasında dağlar kadar olan farkı düşününce. kesin berlin.
  entry_channels: anket
- entry_name: |
    kızların bir kez evleniyorum şımarıklığı
  entry_id:  101561339
  entry_writer: elindo
  entry_fav_count: 67
  entry_content: |
    sanki biz erkekler 3. karıyı alıyoruz . bizde bir kez evleniyoruz ama hiç böyle bir şımarıklığımız yok.
  entry_channels: ilişkiler,ekonomi
- entry_name: |
    sadece türkiye'de dinlenen yabancı sanatçılar
  entry_id:  101579445
  entry_writer: rrrekt
  entry_fav_count: 63
  entry_content: |
    (bkz: <a class="b" href="/?q=inna">inna</a>)
  entry_channels: anket,müzik
- entry_name: |
    sevgilinizle telefonları değişseniz ne olurdu
  entry_id:  101572886
  entry_writer: bloooom
  entry_fav_count: 59
  entry_content: |
    eşimle sıkça yaptığımız eylem. dışarı çıkan hangi telefonun şarjı çoksa onu alıyor.
  entry_channels: anket,ilişkiler
- entry_name: |
    tinder'da tanışıp evlenen çiftler
  entry_id:  101572051
  entry_writer: blacktreacle
  entry_fav_count: 58
  entry_content: |
    en yakın arkadaşım yaptı bunu.<br/><br/>ama şaka maka kız eğlencesine yeni kayıt olmuş ve bir tek arkadaşımla konuşmuş. yersen.
  entry_channels: ilişkiler,teknoloji
- entry_name: |
    ibb'den 4 aylık firmaya 10 milyon tl bedelli iş
  entry_id:  101583186
  entry_writer: draklayn
  entry_fav_count: 53
  entry_content: |
    bundan önceki 25 senede de ihaleleri böyle takip ettin mi mayış güzeli?<br/><br/>not: evet, adhominem yapıyorum. zira bunlar başka türlüsünden anlamıyor.<br/><br/>t: ibişoğlu ibişin birini rahatsız eden iş.
  entry_channels: haber
- entry_name: |
    evlendikten 6 ay sonra osurmaya başlayan eş
  entry_id:  101544970
  entry_writer: cptay
  entry_fav_count: 49
  entry_content: |
    o kişi doğduğundan beri osuruyordu, siz sonradan hayatına girdiniz. haddinizi ve yerinizi biliniz.
  entry_channels: ilişkiler
- entry_name: |
    osmanlı devleti nin yaptığı en büyük hata
  entry_id:  101576023
  entry_writer: beauty in the beast
  entry_fav_count: 47
  entry_content: |
    amerika'nın kolonizasyonu sırasında dönemin en güçlü imparatorluklarından biri olarak kolonizasyona katılmak yerine, başıboş kalan avrupa'ya fetihler düzenlemesidir. bu sırada amerika'dan getirilen değerli madenlerin, osmanlı'nın bölgedeki ticaret hacmini küçülteceğini öngörememesidir. sanayi devrimi vs. derken hiçbir yeniliğe dahil olamayıp, önünde sonunda batıya yenik düşmesidir.
  entry_channels: siyaset,anket,tarih
- entry_name: |
    yaparken insanı zengin gibi hissettiren şeyler
  entry_id:  101560542
  entry_writer: umursamazgenc
  entry_fav_count: 43
  entry_content: |
    (bkz: <a class="b" href="/?q=depoyu+fullemek">depoyu fullemek</a>)
  entry_channels: anket,ekonomi
- entry_name: |
    açılacak söğüşçü için isim önerileri
  entry_id:  101582665
  entry_writer: hellfida
  entry_fav_count: 43
  entry_content: |
    söğüş kulübü.
  entry_channels: anket,yeme-içme
- entry_name: |
    ayda 50 tl doğalgaz parası ödenip ısıtılabilen ev
  entry_id:  101582470
  entry_writer: spaykymoon
  entry_fav_count: 41
  entry_content: |
    evde bazen kombi ile göz göze geliyoruz, o anda bile faturaya 50 tl ekleniyor!
  entry_channels: ekonomi
- entry_name: |
    doğu perinçek
  entry_id:  101544721
  entry_writer: tucek
  entry_fav_count: 36
  entry_content: |
    abdulhamid ve menderes'i sevmez. hele savunanlari hic sevmez. cokta şaaparsaniz ağzınızın uzerine bir tane çarpabilir.<sup class="ab"><a title="(bkz: swh)" href="/?q=swh" data-query="swh">*</a></sup>
  entry_channels: siyaset,haber
- entry_name: |
    4 şubat 2020 beşiktaş'ın kural hatası açıklaması
  entry_id:  101572612
  entry_writer: dede kartal
  entry_fav_count: 32
  entry_content: |
    tinerci, tinerci demekten başka bir tezi olmayan futbol çomarları toplanmış hemen. kural hatası değil bile diyemiyorlar, varsa yoksa tiner. tineri biz çekiyoruz bunların kafalar gidiyor, yüssüz olmaları da cabası..
  entry_channels: spor
- entry_name: |
    counter-strike nick'leri
  entry_id:  101559118
  entry_writer: ismini vermek istemeyen mahmut
  entry_fav_count: 27
  entry_content: |
    (bkz: <a class="b" href="/?q=bombac%c4%b1+m%c3%bclayim">bombacı mülayim</a>) <br/>(bkz: <a class="b" href="/?q=marmara+kaz%c4%b1m">marmara kazım</a>)<br/>(bkz: <a class="b" href="/?q=ali+r%c4%b1za+denen+pezevenk">ali rıza denen pezevenk</a>)<br/>(bkz: <a class="b" href="/?q=fuat+abiniz">fuat abiniz</a>) gardırop fuatın elinde hıyarla fotoğrafı
  entry_channels: anket,oyun
- entry_name: |
    4 şubat 2020 akp'den ilker başbuğ'a suç duyurusu
  entry_id:  101585452
  entry_writer: kalemi kilicindan keskin olan adam
  entry_fav_count: 23
  entry_content: |
    akp sözcüsü ömer çelik tarafından yapılan <a rel="nofollow noopener" class="url" target="_blank" href="https://halkweb.com.tr/omer-celikten-ilker-basbuga-tepki-suc-duyurusunda-bulunacagiz/" title="https://halkweb.com.tr/omer-celikten-ilker-basbuga-tepki-suc-duyurusunda-bulunacagiz/">açıklamadır</a><br/><br/>geçtiğimiz günlerde ilker başbuğ çok önemli açıklamalar yapmıştı.<br/>tabii sözlük ahalisi seksten fırsat bulup, başlığa ilgi göstermemişti.<br/>işte başbuğ'un o sözlerine ilişkin suç duyurusunda bulunacağını açıkladı çelik.<br/><br/>başbuğ'un suç duyurusuna sebep olan açıklamaları:<br/>(bkz: <a class="b" href="/entry/101255148">#101255148</a>)<br/>bu arada başbuğ'un işaret ettiği isimler:<br/>bekir bozdağ, mustafa elitaş, ahmet aydın, mehmet ceylan, yahya doğan, a. müfit yetkin, ahmet iyimaya. <br/>bu isimler söz konusu <a class="b" href="/?q=bkz">bkz</a>'daki yasa değişikliğini teklif eden ve onaylayan şahıslar.<br/><br/>başbuğ delillerle konuştu. detaylarına kadar anlattı.<br/>bunun karşılığında suç duyurusunda bulunacakmış akp.<br/>fetöden şikayet edip, fetönün siyasi ayağını açıklayan hakkında suç duyurusunda bulunmak nedir?<br/>tabii. ne istediyse veren, ergenekonun savcısı olan, gel sevişelim artık diye meydanlarda fetö piçini ülkeye davet eden masum. <br/>o hariç, hepiniz hepimiz fetöcüyüz. o ise fetö ile mücadele eden yılmaz nefer.
  entry_channels: siyaset,ilişkiler,haber
- entry_name: |
    john obi mikel
  entry_id:  101559473
  entry_writer: isaac khan
  entry_fav_count: 21
  entry_content: |
    <a rel="nofollow noopener" class="url" target="_blank" href="https://www.milliyet.com.tr/skorer/chelseali-mikel-e-iftiradan-3-mac-1638361" title="https://www.milliyet.com.tr/skorer/chelseali-mikel-e-iftiradan-3-mac-1638361">https://www.milliyet.com.tr/…ftiradan-3-mac-1638361</a><br/><br/>premıer lig ekiplerinden chelsea’nin orta sahası john obi mikel, hakem mark clattenburg’a yönelttiği ırkçılık suçlamalarının asılsız çıkmasından dolayı 3 maç ceza aldı
  entry_channels: spor
- entry_name: |
    izmir'de yaşayan yazarlar veritabanı
  entry_id:  101545659
  entry_writer: tsayir
  entry_fav_count: 21
  entry_content: |
    herkes bostanlıdan mavişehirden güzelbahçeden alaçatıdan yazıyor. bu gültepede basmanede kadifekalede kimler oturuyor? yazın beni de (bkz: <a class="b" href="/?q=tilkilik">tilkilik</a>)den. afgan dostlarla suriyeli dostlarla bekleriz. mülteci muhabbetinin dibine vururuz.<br/><br/>yalnız bir eliyle çükünü karıştırırken diğer eliyle şemiklerde oturup mavişehir yazan, nergizde oturup bostanlı yazan sen yavaşça telefonu yere bırak ve sakin ol.
  entry_channels: anket,ekşi-sözlük
- entry_name: |
    site güvenliği mesaj attı diye yaygara çıkarmak
  entry_id:  101556117
  entry_writer: sacrilegious
  entry_fav_count: 20
  entry_content: |
    olayda gözden kaçan bir nokta var: yanlış anlamadıysam mesaj atılan kadın pilot. yani eve giriş çıkış saatleri belli değil. kadının 3 parça kargosunun kendisine teslim edilememe sebebi de bu. ihtimal o ki; güvenlik elemanları kadının giriş çıkış saatlerini bilmediklerinden normal bi zaman aralığında denk düşememişler ve kadının kargoları adamların nöbet tuttuğu kulübede birikmiş. <br/><br/>bu adam o an bir şekilde ışıklarının açık olduğunu gördüyse “kadın şu an evde” diye düşünüp haber vermek istemiş olabilir. her şeyi kötüye yormaya ne kadar meraklısınız ya. ha pardon herkes sizin peşinizde koşuyor, herkes size yoklama çekiyor!.. kusura bakmayın ama bence o adam sadece işini yapmaya çalışıyor. sizin sikik kargolarınızın göt kadar kulübede yığılı durmasından usanmıştır belki? neymiş 4.25’te mesaj atmış!!!1 üzgünüm kezbansınız; ve eğitimli olmanız bu gerçeği değiştirmiyor... <br/><br/>önemli not: kadınım.<br/>önemli not2: herkesin her an peşinizde olduğunu düşünmenizden ve bunu her ortamda bikbik dile getirmenizden gerçekten sıkıldım.
  entry_channels: magazin
- entry_name: |
    4 şubat 2020 tsk'nın suriyeli generali vurması
  entry_id:  101584648
  entry_writer: hep gel
  entry_fav_count: 20
  entry_content: |
    (bkz: <a class="b" href="/?q=tabi+l%c3%bctf%c3%bcc%c3%bc%c4%9f%c3%bcm">tabi lütfücüğüm</a>)<br/><br/>(bkz: <a class="b" href="/?q=ibocum+seni+kim+pa%c5%9fa+yapt%c4%b1">ibocum seni kim paşa yaptı</a>)
  entry_channels: siyaset
- entry_name: |
    bıyık denilince akla gelen ilk isim
  entry_id:  101586439
  entry_writer: hem siz ne bilirsiniz ki
  entry_fav_count: 19
  entry_content: |
    salvador dali
  entry_channels: anket,tv,sinema,moda,magazin
- entry_name: |
    4 şubat 2020 trabzonspor bb erzurumspor maçı
  entry_id:  101588020
  entry_writer: thorolf
  entry_fav_count: 17
  entry_content: |
    trabzonspor’umuzun son derece sinir bozucu bir takım haline geldiği maç. erzurumlu topçular büyük efor sarf ediyor, koşuyor, pres yapıyor; trabzonspor hiçbir çaba sarf etmeden skoru 3-0’a getiriyor. gerçekten çok kıl bir takım olduk
  entry_channels: spor
- entry_name: |
    kürt olmanın avantajları
  entry_id:  101584564
  entry_writer: long alone
  entry_fav_count: 9
  entry_content: |
    soru eki kullanmamak
  entry_channels: 
- entry_name: |
    kandilli rasathanesi vs ekşi sözlük
  entry_id:  101587467
  entry_writer: ref034
  entry_fav_count: 8
  entry_content: |
    buradaki sismograf yazarlar kandilli’yi ezer geçer dediğim versus.
  entry_channels: 
- entry_name: |
    dindarların çocuklarını nasıl kurtarabiliriz
  entry_id:  101578326
  entry_writer: neyazacaktim
  entry_fav_count: 8
  entry_content: |
    bir babanın kızına karşı hissettiği duygulardan bile şüphe duyulması islam dinine yakışır bir hareket değil. bu düşüncelerin hepsi cemaatlerin doğru bulduğu şeyler. bu tür babalar bilmem ne cemaatinde sohbetlere katılıyorlar. hoca denilen hokkabazların fetvalarına inanıp, çoluk çocuklarının hayatlarını engelliyorlar. bilmiyorlar ki zorla öğretilen dinden hayır gelmez. <br/><br/>ama başlığı ben de hatalı buldum. genelleme yapılmış gibi. ailesi dindar olup, bu ülkede çok iyi yerlerde olan özgür insanlar var. her dindar gaddar ve ortadoğulu kafasında değildir.
  entry_channels: siyaset,eğitim
- entry_name: |
    dincilerin istismarcı olması
  entry_id:  101584642
  entry_writer: resneliniyazi
  entry_fav_count: 7
  entry_content: |
    (bkz: <a class="b" href="/?q=kur%27an+kurslar%c4%b1nda+istismar">kur'an kurslarında istismar</a>)<br/><br/>istanbul ümraniye'de <a class="b" href="/?q=f%c4%b1k%c4%b1h-der">fıkıh-der</a>’e ait yatılı kurstaki 6 öğrenci cinsel istismara maruz kaldıkları nedeniyle şikeyetçi oldular.
  entry_channels: siyaset
- entry_name: |
    psikiyatr diyalogları
  entry_id:  101586283
  entry_writer: dedi kuzgun
  entry_fav_count: 6
  entry_content: |
    tam ben bir şeyler anlatırken psikiyatristin aklına benden önceki hastayla ilgili bir şey gelir. telefon rehberinden numarayı bulup arar. hastaya vasi olunması gerekmektedir, yakınıyla hararetli tartışma yapar, telefonu kapatır ve bağırarak bana döner:<br/><br/>+ yaa görüyorsun, bakım evine yerleştirmemiz lazım annesi bile ilgilenmiyor?!?<br/>- evet hocam duydum.<br/>+ kaç aydır söylüyorum artık yeter ya!??<br/>- evet hocam haklısınız.<br/>+ hayır çocuğu için ben koşturuyorum burada!!<br/>- evet anlıyorum.<br/>+ ben olmasam perişan olacaklar, kimse ilgilenmiyor!??<br/>- biraz sakin mi olsanız.<br/>+ yeter artık ama yaa!!!?<br/><br/>sonuç: doktora 1mg xanax yazdım. sinirler hep perişan olmuş, damar damar üstüne binmiş garibimin..
  entry_channels: ilişkiler,sağlık
- entry_name: |
    okuduğunuz en iyi roman
  entry_id:  101584185
  entry_writer: sabahattin ali desidero
  entry_fav_count: 5
  entry_content: |
    20 yilda bu başlığı açmak kimsenin aklıma gelmemişti. tebrikler.<br/><br/>(bkz: <a class="b" href="/?q=s%c3%b6zl%c3%bck%c3%a7%c3%bclerin+en+iyi+5+roman+listesi">sözlükçülerin en iyi 5 roman listesi</a>)<br/><br/>(bkz: <a class="b" href="/?q=aramaya+inanmak">aramaya inanmak</a>)
  entry_channels: anket,edebiyat
- entry_name: |
    f16'lar bitince ne olacak sorunsalı
  entry_id:  101586880
  entry_writer: non e possibile
  entry_fav_count: 5
  entry_content: |
    "f 35 almak için götümüzü açıp amerika'ya mı domalalım? ne yapalım?"<br/><br/>demiş bir yazar. amerikalı rahibi seve seve vip uçakla ülkesine gönderirken, uçağını düşürdüğün rusya'dan telefonla özür dilerken kime doğru eğiliyordun acaba?<br/><br/>hava kuvvetleri yana yana uçak istiyor çünkü elimizde cidden azaldı muharip uçaklar. elin abd'lisinin bir uçak gemisine doldurup hürmüz boğazına getirdiği uçak kadar uçağımız yok inanın. o yüzden o uçaklar şart. yerli ve milli araba yapamamışlardan yerli uçak beklemek zaten avelliktir kendinizi küçük düşürtmeyin.<br/><br/>edit: fransa değil uçak çöp vermez sana arkadaşım. sen hiç gündeme bile geldiğini duydun mu ? yok. nedeni basit kardeşim. fransa'yı unut. seçenekler belli. 3 tercih hakkın var. buyur al birinden
  entry_channels: 
- entry_name: |
    tutunamayanlar (dizi)
  entry_id:  101587111
  entry_writer: hayatin acemisi
  entry_fav_count: 4
  entry_content: |
    - sen sanattan anlamazsın.<br/>- sözlükte dört tane hesabım var, aha dayıya sor!
  entry_channels: tv
- entry_name: |
    7 500 000 000 insandan birisin
  entry_id:  101576902
  entry_writer: pxq
  entry_fav_count: 4
  entry_content: |
    türk kızları: amum var.<br/>konu kilit.
  entry_channels: ilişkiler
- entry_name: |
    özel sektör 2020 maaş zam oranları
  entry_id:  101559067
  entry_writer: ressamadam
  entry_fav_count: 2
  entry_content: |
    %30 küsür büyüyüp, %14 almışız. ananızı sikeyim. artık işçilerin açlıktan işverenleri çiğçiğ yiyeceği günleri beklemekten başka yapacak birşey yok.
  entry_channels: anket,ekonomi
- entry_name: |
    ekşi itiraf
  entry_id:  101547049
  entry_writer: alaturco
  entry_fav_count: 2
  entry_content: |
    sevdiğim kadından ayrılalı 1 yıl oluyor ondan ayrıldıktan sonra oturup onun için bir kez bile delice içmedim, bir kere bile mesaj atmadım ama hep özledim çok özledim ve bu canımı acıtan durumları hep sonraya attım bu yolda kendimi kandırmama en çok destek işlerim oldu amansızca çalıştım 8 saat ortalama uyku süremi 6'ya düşürdüm ve amansızca çalıştım dış görünüşüm kilom kıyafetim hiçbir şeyi düşünmedim sadece işime baktım, bu zaman süreci beni çok güzel yerlere getirdi ama bir şeyler eksik, sanırım bugün patlak verdi, dayanılmaz hale geldi. acı çekmek istemiyorum fakat kendimi oyalayacak bir şey kalmadı, kitap okumaya çalıştım, steam kütüphanemde bütün oyunları oynadım işime bakmaya çalıştım ama sıfır. duygularım çok karmaşık ve korku kapladı içimi çok korkuyorum kendimi kandırmak istiyorum, geçmişimi istemiyorum onun beni ele geçirmesini istemiyorum, ne yapacağımı bilmiyorum sözlük bu gece çok çaresizim.
  entry_channels: anket,ilişkiler,ekşi-sözlük
- entry_name: |
    yazarların araba kullanmayı öğrendiği ilk araba
  entry_id:  101545097
  entry_writer: ventolin
  entry_fav_count: 1
  entry_content: |
    renault 12 sw, murat 131 ve tofaş şahin. ilkini çalıştırıp az götürüp durdurdum, ikincisiyle boş arazide tur attım, üçüncüsüyle de trafiğe çıktım.
  entry_channels: anket,ekşi-sözlük,otomotiv,eğitim
- entry_name: |
    şu an okunan kitap
  entry_id:  101589330
  entry_writer: ejiko
  entry_fav_count: 0
  entry_content: |
    stephen king - misery (110.sayfada falanım)<br/>.<br/>acayip sinir oluyorum ama sarıyor kitap. paul'un arada salak hareketleri var. ben onun yerinde olsam annie'nin çok daha suyuna gidip bir an önce kaçmak için planlar yapmaya başlardım.
  entry_channels: anket,edebiyat
- entry_name: |
    sinirlendiğinde çok kırıcı olabilen insan
  entry_id:  101587121
  entry_writer: kidemli bok basi
  entry_fav_count: 0
  entry_content: |
    burclarin zerre kadar etkisi altinda olmadan boyle davranan insan.<br/><br/>not: burclarin hicbir seye etkisi yok ;) ethem derman hoca'nin dedigi gibi "sizin doğumunuzu sağlayan ebenin kütlesini ve ebe ile sizin aranızdaki uzaklığı bildiğimiz için onun etkisine bakıyoruz ve anlıyoruz ki ebenin etkisi satürn'ün etkisinden fazla".
  entry_channels: ilişkiler
- entry_name: |
    kalbi kırılmış bir kadının gidebileceği yer
  entry_id:  101579200
  entry_writer: seryer
  entry_fav_count: 0
  entry_content: |
    annesine tabi ki başka kime gidecek. yargılamadan anlayacak yegane kişi annedir çünkü.<br/>annesi hayatta değilse ya da narsist anlayışsız bir annneyse, abla/kız kardeş veya anne gibi gördüğü teyze/hala olur.<br/>bir ihtimal birbirlerinin her şeyini bildikleri çocukluk arkadaşı olabilir.
  entry_channels: anket,ilişkiler
---
