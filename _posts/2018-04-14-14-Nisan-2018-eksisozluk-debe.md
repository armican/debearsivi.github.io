---
layout: post
title: 14 Nisan 2018 Ekşi Sözlük Debe
data:
- entry_name: |
    şevket altuğ
  entry_id:  75970097
  entry_writer: marconair
  entry_fav_count: 965
  entry_content: |
    sormuşlar, "neden televizyonlarda yoksunuz?" diye. adam da yanıtlamış;<br/><br/>--- <a class="b" href="/?q=spoiler">spoiler</a> ---<br/><br/>“türk toplumunun değerleri değişti. türk toplumuna sunulan işlerin içerikleri değişti. yani ben şu andaki içeriklerle hiçbir dizinin içinde olamam. eleştiri olarak kabul etsinler, biraz da yaşlılığıma versinler… bütün yapılan işlerde tabanca, tüfek, millet birbirini öldürüyor. bütün erkekler sakallı. bizim zamanımızda sakal rol gerekirse bırakılırdı. bu ortamda ben olamam. çünkü biz yaptığımız işlerde topluma sevgiyi, hoşgörüyü, toleransı, birlikte yaşamayı, dayanışmayı öğretmeye çalıştık.böyle bir senaryo ile karşılaşırsam yaşıma rağmen hâlâ oynayabilirim. ama karşılaşacağımı da pek zannetmiyorum”<br/><br/>--- <a class="b" href="/?q=spoiler">spoiler</a> ---<br/><br/><a rel="nofollow noopener" class="url" target="_blank" href="https://www.sozcu.com.tr/hayatim/magazin-haberleri/sevket-altug-ben-su-andaki-iceriklerle-hicbir-dizinin-icinde-olamam/" title="https://www.sozcu.com.tr/hayatim/magazin-haberleri/sevket-altug-ben-su-andaki-iceriklerle-hicbir-dizinin-icinde-olamam/">kaynak</a><br/><br/>benim gözümde gerçek bir sanatçıdır. <br/>seviyoruz seni fiko.
  entry_channels: tv,sanat,sinema
- entry_name: |
    ev de yanlızım gelsenya diye mesaj atan kız
  entry_id:  75979970
  entry_writer: seovi
  entry_fav_count: 897
  entry_content: |
    türkçe hocasını da ben öpeyim bari o kızın.
  entry_channels: ilişkiler
- entry_name: |
    ormanların 5 yıllığına satışa çıkarılması
  entry_id:  75970403
  entry_writer: ginc
  entry_fav_count: 880
  entry_content: |
    ne ceket kaldı ne metelik cebinde ceketin<br/>kurtaracağız diye geldiler içine sıçtılar memleketin
  entry_channels: siyaset,haber,ekonomi
- entry_name: |
    boşanan kadının çocuğuna soyadını verebilmesi
  entry_id:  75975636
  entry_writer: omuriliksorunsali
  entry_fav_count: 740
  entry_content: |
    bir erkek olarak katılıyorum velayet kimde ise soyadı hakkı da onun olsun. babalık yapmayan babalara da ders olsun.<br/><br/>edit: bana mesaj atıp saçma sapan ithamlarda bulunan gerzekler şu entry yi bir okuyun lütfen (bkz: <a class="b" href="/entry/75980191">#75980191</a>)
  entry_channels: ilişkiler,haber
- entry_name: |
    tecavüz ve sapıklığın arkasında deizm vardır
  entry_id:  75971787
  entry_writer: thor x
  entry_fav_count: 497
  entry_content: |
    (bkz: <a class="b" href="/?q=entry+nick+uyumu">entry nick uyumu</a>)
  entry_channels: siyaset
- entry_name: |
    dümdüz siyah tişört giyen erkek
  entry_id:  75972268
  entry_writer: allahdiyenallah
  entry_fav_count: 416
  entry_content: |
    yıllardır dümdüz siyah tişört giyerim. herhangi bir amacım yok. çok seviyorum. size ne amk.
  entry_channels: moda
- entry_name: |
    alman polisi tarafından dövülen almancı çomar
  entry_id:  75981049
  entry_writer: o is oyle olmaz koc
  entry_fav_count: 282
  entry_content: |
    dön gel lan türkiye'ye madem. ne işin var elin gavur ülkesinde.
  entry_channels: siyaset,haber
- entry_name: |
    d&amp;r'ın turkuvaz grubuna satılması
  entry_id:  75972260
  entry_writer: maymuncuk95
  entry_fav_count: 278
  entry_content: |
    medya ayağını demirören grubuna satan aydın doğan, <a class="b" href="/?q=d%26r">d&amp;r</a>'ın 440 milyon liraya turkuvaz grubuna satışı için ön protokol imzaladı.<br/><br/>üzücü. bundan sonra <a class="b" href="/?q=tvek">tvek</a> (adı bile çakma) mağazalarına dönüşecek mağazalardaki en çok satanlar köşesinde <a class="b" href="/?q=nutuk">nutuk</a> yerine <a class="b" href="/?q=recep+tayyip+erdo%c4%9fan">recep tayyip erdoğan</a> biyografileri, <a class="b" href="/?q=ilber+ortayl%c4%b1">ilber ortaylı</a> kitaplarının yerini ise <a class="b" href="/?q=kadir+m%c4%b1s%c4%b1ro%c4%9flu">kadir mısıroğlu</a>'nun maraş dondurması tariflerini içeren kağıt parçaları alacaktır.
  entry_channels: siyaset,haber,ekonomi
- entry_name: |
    zülfü livaneli'nin romanını birebir kopyalamak
  entry_id:  75973972
  entry_writer: tovbebismillah
  entry_fav_count: 266
  entry_content: |
    esinlenmiş olabilir. hikayenin sadece %98'ini kopyalamış.
  entry_channels: edebiyat,haber
- entry_name: |
    müzik dinlerken bateri çalar gibi yapan tip
  entry_id:  75975871
  entry_writer: stukatr
  entry_fav_count: 228
  entry_content: |
    air drum da çalarım, dans da ederim. sizin veba yüklü ölü suratlarınızı mı çekeceğiz ulan sabahtan akşama kadar? solo da atarım göbek de atarım.<br/><br/>dans ulan, mutluluk demek.
  entry_channels: müzik
- entry_name: |
    tecavüz eden kayınpederini öldüren gelin
  entry_id:  75970019
  entry_writer: zlmka
  entry_fav_count: 224
  entry_content: |
    kendisine defalarca cinsel saldırıda bulunan kayınpederini öldüren ve 8 ay tutuklu kaldıktan sonra tahliye olan esma yeşilbaş davasında emsal bir karar çıktı. savcı esma yeşilbaş’ın “ağır haksız tahrik altında kasten öldürme” suçundan cezalandırılmasını istedi. talihsiz kadın “sürekli tehdit ederdi beni. gidecek bir yerim yoktu” diyerek yaşadıklarını anlattı. mahkeme, eylemin meşru müdafaa sınırları kapsamında kaldığını belirterek, oy çokluğuyla yeşilbaş’ın beraatına hükmetti.<br/><br/>eltisi de benzer ifadeler vermiş, kan donduran bir olay, kim bilir nerelerde neler yaşanıyor sorularını da beraberinde getiriyor.<br/>birinin ölmesinden zevk alır, beraat eden kadına sevinir olduk, sanki onu dışarıda harika bir hayat bekliyormuş gibi.<br/><br/>kaynak : <a rel="nofollow noopener" class="url" target="_blank" href="https://www.sozcu.com.tr/2018/gundem/mahkemeden-hukuk-dersi-2346592/" title="https://www.sozcu.com.tr/2018/gundem/mahkemeden-hukuk-dersi-2346592/">tecavüz eden kayınpederini öldüren gelin</a><br/><br/>edit : arkadaşlar beraat kararını ben vermedim,ne hakimim,ne avukat, vicdanen tecavüze uğrayan mağdurun yanındayım. saçma sapan erkek egolarınızı kendinize saklayın.
  entry_channels: haber
- entry_name: |
    1878 yılında çekilen panoramik istanbul fotoğrafı
  entry_id:  75973276
  entry_writer: mictian
  entry_fav_count: 217
  entry_content: |
    1878 yılında germain-ferdinand sengés tarafından <a class="b" href="/?q=beyaz%c4%b1t+kulesi">beyazıt kulesi</a>'nden çekilen, istanbul'a ait çoğu ikonik yapının görülebileceği panoramik <a rel="nofollow noopener" class="url" target="_blank" href="http://www.eskiistanbul.net/resimler/beyazit-kulesinden-panorama-germain-ferdinand-senges-1878.jpg" title="http://www.eskiistanbul.net/resimler/beyazit-kulesinden-panorama-germain-ferdinand-senges-1878.jpg">fotoğraf</a>tır.
  entry_channels: sanat,tarih
- entry_name: |
    13 nisan 2018 ohal'in uzatılması
  entry_id:  75977185
  entry_writer: siparisi koreden 75 gunde gelen adam
  entry_fav_count: 211
  entry_content: |
    dolar bunu beğendi
  entry_channels: siyaset,haber
- entry_name: |
    motosikletleri fark edin
  entry_id:  75970859
  entry_writer: penguent
  entry_fav_count: 206
  entry_content: |
    özellikle de kaldırımda gidenlerini.
  entry_channels: ilişkiler,haber,otomotiv,motosiklet
- entry_name: |
    13 nisan 2018 çiftçinin banka önünde isyanı
  entry_id:  75983823
  entry_writer: buzzle
  entry_fav_count: 195
  entry_content: |
    demirören'e doğan medya'yı satın alması için 700m$ kredi veren devlet bankasının önünde gerçekleşmiştir.
  entry_channels: haber,ekonomi
- entry_name: |
    nurşen karayanız
  entry_id:  75974367
  entry_writer: unknownmaninchains
  entry_fav_count: 172
  entry_content: |
    kendisi bana ilham kaynağı olmuştur.<br/><br/>yazmak istediğim kitaplar;<br/><br/><a class="b" href="/?q=das+kap%c4%b1dan">das kapıdan</a>- almanya'nın doğu almanya ile kapıları yıktığı zamandan beri liberalist ekonomi modeline katkılarından bahsedeceğim.<br/><br/><a class="b" href="/?q=otomatik+k%c3%b6rtopal">otomatik körtopal</a> - gözleri görmeyen ve yürüme engelli birinin topluma ayak uydurması için devletin ne kadar canhiraş davranması gerektiğini anlatacağım bu kitapta allahın izni ve inayetiyle.<br/><br/><a class="b" href="/?q=homo+papiens">homo papiens</a> - papiens isminde bir trans bireyin doğduğundan beri başından geçenleri anlatacağım.<br/><br/><a class="b" href="/?q=ferrasini+satan+simge">ferrasini satan simge</a> - simge isimli zengin genç kızın başından geçenleri anlatmak istiyorum.<br/><br/><a class="b" href="/?q=k%c3%bcrk+mantolu+maradona">kürk mantolu maradona</a> - bu kitap bir biyografi olacak inşallah.<br/><br/><a class="b" href="/?q=laleler+ve+insanlar">laleler ve insanlar</a> - bu lale devrinde sistem eleştirisi olacak.<br/><br/><a class="b" href="/?q=wa%c5%9fington+portakal%c4%b1">waşington portakalı</a> (ciltsiz) - brezilya kökenli bir portakalın serüveni.<br/><br/><a class="b" href="/?q=kimyac%c4%b1">kimyacı</a> - bir çobanın endülüsten çıkıp kimyacı olmaya karar vermesini anlatıyor.<br/><br/><a class="b" href="/?q=yayvan+%c3%a7iftli%c4%9fi">yayvan çiftliği</a> - bu kitapta yayvan yayvan konuşan tiplerin toplum düzenine etkisini inceleyeceğim.<br/><br/><a class="b" href="/?q=menemenler">menemenler</a> - menemen denemelerimi kaleme alacağım.<br/><br/><a class="b" href="/?q=mi+-ci+-qi">mi -ci -qi</a> - polisiye olsun istiyorum bu, filmini falan çekeriz sonra.
  entry_channels: edebiyat
- entry_name: |
    galatasaray'ın boğaziçi temalı 18-19 forması
  entry_id:  75970810
  entry_writer: turcos
  entry_fav_count: 169
  entry_content: |
    hemen fenerbahçeliler olarak odtü temalı forma yapmalıyız.
  entry_channels: spor
- entry_name: |
    her cuma namazı çıkışında para toplanması
  entry_id:  75971693
  entry_writer: bilimyap
  entry_fav_count: 162
  entry_content: |
    ulan arkadaş o vaazın,hutbenin her ne neyse arasına para istemeyi de sıkıştırıyor ya nasıl deliriyorum kafayı yiycem ya.<br/>1-)ilçe müftülük binası yenilenecek<br/>2-)kuran kurslarımıza yardım<br/>3-)camimizin ihtiyaçları<br/><br/>bakın ben ve çevremdeki bir sürü insanı bu şekilde dinden soğutuyorlar cumaya gitmekten soğudum bunlar yüzünden şu ana kadar hiçbir cuma çıkışı para istenmediğine şahit olmadım illa bir şey buluyorlar <br/><br/>diyanete 2017 yılında 6 milyar 517 milyon tl bütçe ayrılmış ve bu doyumsuzlar bu paranın 4 milyar 939 milyon tl’sini 8 ay içinde kullanmışlar.gösterişli cami yapacağınıza var olan camilere klima takın amına koyayım bu kadar bütçe alıyorsunuz cemaat birkaç ay sonra 40 derece sıcakta ayaklı pervanelerle serinlemeye çalışacak bakın çoğu yerde klima var tamam ama olanların da yarısı ya çalıştırılmıyor ya da bozuk amk <br/><br/>baktılar doymuyor bunlar 2018 yılında da 7 milyar 774 milyon tl bütçe ayırmışlar ve geçen hafta gittiğim cumada adamlar caminin her bir çıkış kapısına adam koymuşlar bi tane de kutu bereket versin.birkaç saat sonra cuma var ve gitmiycem çünkü biliyorum ki yine bir şeyer gevelenecek ve çıkışta para toplanacak <br/><br/>çok sinirliyim çok doldum sözlük
  entry_channels: anket
- entry_name: |
    trump'ın fahişelerle grup seks ve işeme kaseti
  entry_id:  75982806
  entry_writer: ourgodgivenrighttobeararms
  entry_fav_count: 160
  entry_content: |
    (bkz: <a class="b" href="/?q=two+girls+one+trump">two girls one trump</a>)
  entry_channels: siyaset,ilişkiler,haber
- entry_name: |
    türk tarihindeki en kötü devlet liderleri
  entry_id:  75980279
  entry_writer: icimdeki sey gibi bilmem ki ney gibi
  entry_fav_count: 136
  entry_content: |
    recep tayyip erdoğan'dır.<br/><br/>niye korktunuz ki? bu ülkede rte'ye rte denir.
  entry_channels: anket
- entry_name: |
    izmir'in en güzel ilçesi
  entry_id:  75977534
  entry_writer: dandikistan
  entry_fav_count: 114
  entry_content: |
    manisa.
  entry_channels: anket,seyahat
- entry_name: |
    futbolda izlenilen oyunculardan kurulu en iyi 11
  entry_id:  75968884
  entry_writer: duvar adam
  entry_fav_count: 108
  entry_content: |
    -----------------o.kahn<br/><br/>c.puyol -----p.maldini----- l.thuram <br/><br/>--------g.gattuso----z.zidane<br/>--------(e.davids)<br/>l.messi------- g.hagi ------ronaldinho<br/><br/>-------- ronaldo---- c.ronaldo<br/><br/>(bkz: <a class="b" href="/?q=ebesinin+am%c4%b1+ali+sami">ebesinin amı ali sami</a>)<br/><br/>edit: g.gattuso'nun default sarı kartı nedeniyle ikinci yarı e.davids değişikliği gerçekleşecektir.
  entry_channels: spor,anket
- entry_name: |
    beşşar esed
  entry_id:  75967828
  entry_writer: basabbasbas
  entry_fav_count: 104
  entry_content: |
    şu biadçı köpeklerdeki esed dilini gördükçe tiksiniyorum.<br/>ulan sanki dün adama kardeşim esad derken demokrasi çocuğuydu da, bugün orospu çocuğu oldu. adam ne zaman az sallansa, ağzı salyalı, köpüklü itler gibi kuduruyorsunuz. senin ülkene bugün bir saldırı olsa hepiniz şebbihaya dönüşür ortada ne kürt, ne laik ne de alevi bırakırsınız. sizin ruhunuz yavşak da hazımsızlığınız doğru söyleyene.
  entry_channels: siyaset
- entry_name: |
    karşı cinste tahammül edilemeyen özellikler
  entry_id:  75983358
  entry_writer: kalyanschik
  entry_fav_count: 53
  entry_content: |
    klişe ve alıntı olacak ama;<br/><br/>sizi siz yapan özelliklere aşık olup, sonra bunları elinizden almaya çalışması. <br/><br/>defalarca yaşanmıştır.
  entry_channels: anket,ilişkiler
- entry_name: |
    kimyasal saldırıyı ingiltere yaptı
  entry_id:  75985682
  entry_writer: yoldakisubirikintisi
  entry_fav_count: 47
  entry_content: |
    "eğer bir nehirde iki balık kavga ediyorsa, bilin ki oradan az önce uzun bacaklı bir <a class="b" href="/?q=ingiliz">ingiliz</a> geçmiştir."
  entry_channels: siyaset,haber
- entry_name: |
    anıtkabir'in dozer ve kepçelerle yıkılacağı gün
  entry_id:  75964013
  entry_writer: entryrush
  entry_fav_count: 46
  entry_content: |
    işte o zaman karşınızda "gerçek türkleri" görürsünüz, yan sanayi arapları değil.
  entry_channels: siyaset,anket
- entry_name: |
    benzinin 6 tl olması
  entry_id:  75983323
  entry_writer: franzprometh
  entry_fav_count: 43
  entry_content: |
    özgürlüğün en büyük düşmanı, halinden memnun kölelerdir.<br/><br/>che guevara<br/><br/>velhasıl dostlar ümmet halinden memnun.
  entry_channels: ekonomi
- entry_name: |
    orhan veli
  entry_id:  75964338
  entry_writer: turkish wagnerian
  entry_fav_count: 37
  entry_content: |
    <a rel="nofollow noopener" class="url" target="_blank" href="https://www.google.com.tr/webhp?hl=tr&amp;ictx=2&amp;sa=X&amp;ved=0ahUKEwihrt6n0rXaAhXGaVAKHQyQDTEQPQgD" title="https://www.google.com.tr/webhp?hl=tr&amp;ictx=2&amp;sa=X&amp;ved=0ahUKEwihrt6n0rXaAhXGaVAKHQyQDTEQPQgD">google</a> tarafından 104. yaş günü <a class="b" href="/?q=istanbul%27u+dinliyorum+g%c3%b6zlerim+kapal%c4%b1">istanbul'u dinliyorum gözlerim kapalı</a> doodle'u ile kutlanan çok genç yaşta kaybettiğimiz değerli şairimiz.<br/><br/>zülfü livaneli'nin güzel sesinden o muhteşem şiirini güzel bir şarkı eşliğinde dinleyelim ve iyi ki doğdun büyük şair diyelim. <br/><a rel="nofollow noopener" class="url" target="_blank" href="https://www.youtube.com/watch?v=gRWIQN_8sNA" title="https://www.youtube.com/watch?v=gRWIQN_8sNA">istanbul'u dinliyorum gözlerim kapalı</a>
  entry_channels: edebiyat
- entry_name: |
    kızların sözlüğü erkek düşürmek için kullanması
  entry_id:  75970904
  entry_writer: senin pezevenk enisten
  entry_fav_count: 37
  entry_content: |
    (bkz: <a class="b" href="/?q=nereye+d%c3%bc%c5%9f%c3%bcyoruz">nereye düşüyoruz</a>)
  entry_channels: anket,ilişkiler,ekşi-sözlük
- entry_name: |
    eski sevgiliyle yeniden başlamak
  entry_id:  75976518
  entry_writer: erken oten civciv
  entry_fav_count: 37
  entry_content: |
    her ne kadar ilk başlarda bi huzur, bi mutluluk, efendime söyliyeyim bi halt becermiş hissi verse de; yeniden <a class="b" href="/?q=eski+sevgili">eski sevgili</a> ünvanına kavuşacağı için, çok da anlam yüklenmemesi gereken başlangıç.<br/><br/>ayrıca (bkz: <a class="b" href="/?q=eski+sevgilinin+yeniden+sevgili+olmas%c4%b1">eski sevgilinin yeniden sevgili olması</a>)
  entry_channels: anket,ilişkiler
- entry_name: |
    eşcinselliğe saygı duyma zorunluluğu
  entry_id:  75985321
  entry_writer: 2x3
  entry_fav_count: 36
  entry_content: |
    eşcinselliğe saygı duymaktan ziyade, kişinin özgür ve hür iradesine saygı duymaktır.<br/><br/>kime ne kişinin cinsel kimliğinden?
  entry_channels: 
- entry_name: |
    çalıntı kitap için isim önerileri
  entry_id:  75978499
  entry_writer: penne rigate
  entry_fav_count: 35
  entry_content: |
    (bkz: <a class="b" href="/?q=sekizinci+dahiliye+ko%c4%9fu%c5%9fu">sekizinci dahiliye koğuşu</a>)
  entry_channels: anket
- entry_name: |
    14 nisan 2018 benzin zammı
  entry_id:  75971613
  entry_writer: erennyildizz
  entry_fav_count: 25
  entry_content: |
    sapppplaaa reis doymadık.<br/><br/>bisiklet alın lem aq <br/>motor alın lem aq
  entry_channels: ekonomi
- entry_name: |
    cinsel organına kokain süren fantastik doktor
  entry_id:  75964637
  entry_writer: her zaman her yerde
  entry_fav_count: 23
  entry_content: |
    ben annemin getirdiği çayı pöfürdüterek içiyorken sizler nasıl sikinize kokain sürüp bununla insan öldürüyorsunuz? üstümde abimin eskisi yelek var galiba bu yelek yüzünden kaybediyorum amk yarın ilk iş bir tane deri ceket alacağım belki hayatımda bir değişiklik olur.
  entry_channels: ilişkiler,sağlık,haber
- entry_name: |
    12 nisan 2018 diyanet'in deizm açıklaması
  entry_id:  75964045
  entry_writer: siradan cinko karbon pil
  entry_fav_count: 20
  entry_content: |
    buram buram islam hoşgörüsü kokan açıklamadır.<br/><br/>''insanların inançlarını sorgulamak veyahut aşağılamak haddimiz değil lakin ben gençlerimizin ateizm'e ya da deizm'e kaydığını düşünmüyorum'' demek bu kadar zor olmamalı.<br/><br/>deistlerin ne sapıklığı kalmış, ne pezevenkliği. orospu çocuğu deseydin kardeş.
  entry_channels: siyaset,haber
- entry_name: |
    ali erbaş'a deizm davası açılsın kampanyası
  entry_id:  75985698
  entry_writer: ismin e hali
  entry_fav_count: 19
  entry_content: |
    diyanet işleri başkanı olan zat, biliyorsunuz deizm inancı sapıklıktır dedi.<br/>toplumsal sözleşme ile güvence altına alınmış inanç ve vicdan özgürlüğüne ters görünüyor. <br/>bu ülkenin anayasasında laiklik bir madde ise bu adamın böyle bir açıklama yapma hakkı yok. doğrudan anayasaya karşı bir açıklama. <br/>gerekli davanın açılması konusunda bizleri bilgilendirecek hukukçular vardır, lütfen yardımcı olun arkadaşlar.<br/>gündemde tutmanızı rica ediyorum.<br/>öyle kolay kurtulmak yok bu işten.
  entry_channels: 
- entry_name: |
    çilek vs yeşil erik
  entry_id:  75983668
  entry_writer: tutsky yanchek
  entry_fav_count: 17
  entry_content: |
    zengin yeşil erik sever, fakir ise çilek.<br/><br/>tabii bu eskiden böyleydi. çünkü o zamanlar erik ucuz, çilek pahalıydı. şimdi erik pahalı, çilek ise hiç yok. <br/><br/>pazarlarda, marketlerde satılan; kırmızı, üstü siyah çekirdekli, yumruk büyüklüğündeki şeyler çilek değil, <a class="b" href="/?q=yossi+cohen">yossi cohen</a>'in daşşağı.
  entry_channels: anket
- entry_name: |
    ata binmede ve ok atmada zirvede olacağız
  entry_id:  75984048
  entry_writer: earendill
  entry_fav_count: 17
  entry_content: |
    buradan anlıyoruz ki yerli araba olayında çuvallıyoruz ve s400'leri satmaktan da vazgeçtiler.
  entry_channels: spor,haber
- entry_name: |
    28 şubat davası
  entry_id:  75984668
  entry_writer: akil taker mind fucker
  entry_fav_count: 15
  entry_content: |
    hakimleri, iddianame savcilari fetoden tutuklu bulunan dava.<br/><br/>ve davada hala ceza cikiyor.<br/><br/>feto kumpaslarinin devami. bakalim buna ne zaman uyanacaklar
  entry_channels: siyaset,haber
- entry_name: |
    ekşi itiraf
  entry_id:  75965882
  entry_writer: flortozhatun
  entry_fav_count: 10
  entry_content: |
    hobilerim arasında internetten nevresim takımı bakmak var, ama almıyorum bakmak hoşuma gidiyor. ve hep tek kişilik. ne zaman çift kişilik bir hayat hayal edeceğim acaba.<br/><br/>her yıl kira zammı dönemi geldiğinde evlenmeye karar veriyor ama ilk zamlı kiradan sonra normal halime dönüyorum. bu seneyi de atlattım neyse ki. <br/><br/>bir şeyler yazmaya karar verdim, aşk hikayesi olacaktı, feminist manifestoya dönüştü. aşk hikayelerinde feminist bir karakter olamayacağını hatırladım, vazgeçtim. <br/><br/>alt kat komşum anlamadığımı zannederek arkamdan konuşuyor. insanlardaki özgüvene hayranım. ne de güzel iş çeviriyorlar, başlarına alacakları belayı hiç düşünmeden. ne mi yapacağım? güvercinler balkonumdaki çiçeklerin toprağını onun balkonuna taşıyor diye saksıları içeri almıştım, bugün hepsini geri kenara koydum. temizlesin şimdi hiç umurumda değil. ben de bu kadar kötü olabiliyorum işte. n’apıyım. konuşmaya devam ederse üç erkek yeğenimi bir hafta misafir etmeyi düşünüyorum. ama buna ben de katlanamayabilirim iyi düşünmem lazım. <br/><br/>sarı renge değişik bir sevgi besliyorum. ama ilginçtir ki sarışınları hiç sevmem (jude law hariç). <br/><br/>iki gündür işe gitmedim, ders çalışıyorum. ne çalıştığıma ve neden çalıştığıma dair hiçbir fikrim yok. üniversitede, ev arkadaşlarım sabah sınava giderken elime not tutuştururdu, hangi ders olduğunu bilirdim en azından. şu an durum tabula rasa. hayırlısı bakalım. <br/><br/>evimde, çerçeveli yalnızca iki fotoğraf var. ikisinin de düğün fotoğrafı olduğunu bugün fark ettim. çerçeveler düğün salonlarındandır kesin. gelin,damat ve bir adet ‘benim burada ne işim var’ ifadeli kadından oluşan bir fotoğraf için çerçeve alacağımı sanmıyorum. <br/><br/>evde bir günden fazla zaman geçirince kendimi cam silerken ya da ocak çitilerken buluyorum. ev hanımlarının damacanaya kıyafet giydirmesine şaşmamalı. kafa gidiyor demek ki bir süre sonra. <br/><br/>yaklaşık bir hafta önce çiçek tohumu ekmiştim, bugün saksıda kafasını topraktan çıkarmış minicik yeşil bir şey gördüm. en son ne zaman böyle mutlu olduğumu hatırlamıyorum bile. doğaya dönün diyen büyüklerimiz boşa konuşmuyor vesselam. <br/><br/>bir ay sonra rize’ye gideceğim. bir buçuk yıl önce gitmiştim, doğu karadeniz bünyede ihtiyaca dönüşüyor anladığım kadarıyla. nurdoğan’ın yerinde kahvaltı edecek, cahce tepesinde gürcü şarabı eşliğinde kamp yapacağım. hayali bile o kadar heyecanlı ki kuzey ışıklarını gördüm ama o tepede hissettiklerimi hissedemedim. <br/><br/>ağustos 2017’de ‘umarım kuzey ışıklarını görürüm’ yazmışım buraya. hiçbir şekilde planlarımda yokken hem de. yazdım oldu. yeni notum; kapadokya’da sabahın dördünde kalkar balona binerim umarım.
  entry_channels: anket,ilişkiler,ekşi-sözlük,edebiyat
- entry_name: |
    ismail kahraman
  entry_id:  75986498
  entry_writer: supr3
  entry_fav_count: 8
  entry_content: |
    geberdiği zaman mezarına zevkle tüküreceğim onursuz bir ihtiyar.
  entry_channels: siyaset,tarih,haber
- entry_name: |
    deizm
  entry_id:  75964020
  entry_writer: kaidesini bozan istisna
  entry_fav_count: 5
  entry_content: |
    şu sıralar hakkında yazmayanı, konuşmayanı dövdükleri izm.<br/><br/>geçecek bu günler de geçecek. islam hepten gariplere kalacak. lan zaten garip olmayanın dinle imanla işi ne.
  entry_channels: bilim,tarih
- entry_name: |
    bitcoin
  entry_id:  75982008
  entry_writer: atobaba
  entry_fav_count: 5
  entry_content: |
    konunun ilk entarisini"son iki haftadir paritesi 6-8 dolar arasinda seyreden..." diye açan arkadaşın şu anki maddi durumunu hep merak etmişimdir.
  entry_channels: programlama,teknoloji,haber,eğitim,ekonomi
- entry_name: |
    at edebiyatının bayması
  entry_id:  75982932
  entry_writer: gundi blues
  entry_fav_count: 5
  entry_content: |
    birtakım başka şeylerden rahatsızlık duyduğunda,<br/>başına iş açacaksa.<br/>başına iş açmayacak olan şeyden rahatsızlık duyman yaşama tutunabilme çabandır.<br/><br/>kolpa mizahi unsur olan ata öfke duyan postmodern yüzyıl insanı.
  entry_channels: tv
- entry_name: |
    alkol suç kapsamına girecek
  entry_id:  75986454
  entry_writer: chloee
  entry_fav_count: 5
  entry_content: |
    yaşlı ve yorgun bi kadınım ama..<br/>işte o gün ayaklanırım, aktivizt olurum.<br/>fena olurum.
  entry_channels: haber,yeme-içme
- entry_name: |
    13 nisan 2018 akhisarspor beşiktaş maçı
  entry_id:  75971714
  entry_writer: ensalak
  entry_fav_count: 5
  entry_content: |
    hem deplasman hem akhisar hem cuma hem de ayın on üçündedir. <br/><br/>allah yardımcımız olsundur.
  entry_channels: spor
- entry_name: |
    türkiye'de oynamış oyunculardan kurulu en iyi 11
  entry_id:  75986646
  entry_writer: guest22474
  entry_fav_count: 4
  entry_content: |
    tuncel kurtiz - haluk bilginer - şener şen - kemal sunal - beren saat (bence iyi oyuncu) - demet akbağ - murat arkın- şevket altuğ - halit ergenç - toprak sergen - çetin tekindor
  entry_channels: spor,anket
- entry_name: |
    çaylaklardan mesaj bekleyen yazarlar veritabanı
  entry_id:  75979949
  entry_writer: thetrid
  entry_fav_count: 2
  entry_content: |
    (bkz: <a class="b" href="/?q=abaza+yazarlar+veri+taban%c4%b1">abaza yazarlar veri tabanı</a>)'na dönmüş başlıktır.
  entry_channels: anket,ekşi-sözlük
- entry_name: |
    üstteki yazarın nickinin düşündürdüğü
  entry_id:  75970658
  entry_writer: sarsak elek
  entry_fav_count: 0
  entry_content: |
    cad operatörü.
  entry_channels: anket,ekşi-sözlük
- entry_name: |
    ilk görüşte aşık olunan şehirler
  entry_id:  75981450
  entry_writer: in between dreams
  entry_fav_count: 0
  entry_content: |
    (bkz: <a class="b" href="/?q=prag">prag</a>)<br/><br/>(bkz: <a class="b" href="/?q=paris">paris</a>)<br/><br/>(bkz: <a class="b" href="/?q=tokyo">tokyo</a>)
  entry_channels: anket,seyahat
---
