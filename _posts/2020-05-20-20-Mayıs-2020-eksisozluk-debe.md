---
layout: post
title: 20 Mayıs 2020 Ekşi Sözlük Debe
data:
- entry_name: |
    ege ve akdeniz bölgesinde kürt mafya yapılanması
  entry_id:  107434485
  entry_writer: nerelerdengeldik
  entry_fav_count: 1423
  entry_content: |
    5 yıldır bodrumda geçirmiş biri olarak söyleyebilirim ki, var olan bir yapılanmadır.<br/><br/>adam akıllı esnaflık yapan, türkiye'nin ve kendisinin kalkınmasına hizmet eden, işinde gücünde kürt yurttaşlardan bahsetmiyorum. onlar bizim canımızdır. çünkü bu kürt mafya yapılanmalarından bu kardeşlerimiz de mağdurdur. yerel esnafı tehdit ve şantajla sindirip, haraç kesmeye çalışan, şehrin göbeğinde eşkıyalık yapan kürt mafya yapılanmasından bahsediyorum. gelen turistlere sarkıntılık olarak turizmi baltalamaya çalışmaları da cabasıdır. barzoların, yaşlı turist kadınlarıyla yatmak için yaptığı şaklabanlıkları görseniz, mideniz yerinden kalkar. tek başlarına pısırık, korkaktırlar. 10-15 kişi bir araya gelince şehrin göbeğinde eşkıyalık yapmaya kalkarlar. ve ayrıca uyuşturucu trafiği de bu çetelerin elindedir.<br/><br/>terör örgüt pkk'nın batı'da kundakladığı ormanlık alanların yakılmasında bu kürt mafya yapılanması çetelerin de önemli bir payı vardır.<br/><br/>bu mafya yapılanmaları doğrudan pkkyla bağlantılıdır. bölücü örgüte sıcak para aktarırlar. uyuşturucu ticaretinden milyon doları kıran kanlı terör örgütü pkk, bu mafyatik çeteler sayesinde ticaretini yapar.<br/><br/>türkiye cumhuriyeti devletinin, tatil yörelerinde yuvalanan bu çetelere artık bir el atması lazımdır.
  entry_channels: siyaset,seyahat
- entry_name: |
    diyanet'in 4 ayda 3.8 milyar lira harcaması
  entry_id:  107432890
  entry_writer: kante
  entry_fav_count: 1094
  entry_content: |
    yeni türkiye'de normal olan olay. 7 bakanlıktan daha çok harcamış diyanet. içişleri, dışişleri, enerji ve tabii kaynaklar, kültür ve turizm, sanayi ve teknoloji, çevre ve şehircilik ile ticaret bakanlığı. <br/>bir ülke düşünün, açlıktan insanlar ölürken, <a class="b" href="/?q=diyanet%27in+yoksulluk+fetvas%c4%b1">diyanet'in yoksulluk fetvası</a> ile millete açlığın sınama olduğunu söylüyor. ama aynı zamandan 7 bakanlıktan daha çok harcıyor.<br/><a rel="nofollow noopener" class="url" target="_blank" href="https://tr.sputniknews.com/turkiye/202005191042071229-kurumlarin-butceden-yaptiklari-harcamalar-netlesti-en-cok-maliye-bakanligi-harcadi-diyanet-7/" title="https://tr.sputniknews.com/turkiye/202005191042071229-kurumlarin-butceden-yaptiklari-harcamalar-netlesti-en-cok-maliye-bakanligi-harcadi-diyanet-7/">kaynak</a>.<br/><br/>edit: tahmin ettiğim gibi pınar altuğ'un dansı kadar ilgi göremeyecek bir başlıktır. ne diyeyim ki artık.<br/><br/>edit 2: kaynağı beğenmeyenler olmuş, sabah yazdı da ben mi kaynak olarak koymadım.<sup class="ab"><a title="(bkz: swh)" href="/?q=swh" data-query="swh">*</a></sup><br/><br/>edit 3 : hâlâ personel maaşı diyen var, <a class="b" href="/?q=insan+ger%c3%a7ekten+hayret+ediyor">insan gerçekten hayret ediyor</a>.
  entry_channels: siyaset,haber,ekonomi
- entry_name: |
    misvak dergisinin 19 mayıs paylaşımı
  entry_id:  107447797
  entry_writer: bu havalarda gelme bana
  entry_fav_count: 538
  entry_content: |
    ilk defa komik bir karikatür yapabilmişler. sırf bundan dolayı kutluyorum. ben güldüm.
  entry_channels: siyaset,haber
- entry_name: |
    10 gençten 6'sı yurtdışında yaşamak istiyor
  entry_id:  107441048
  entry_writer: kore gazisi yavuz dede
  entry_fav_count: 538
  entry_content: |
    ucuz <a class="b" href="/?q=et">et</a> ve <a class="b" href="/?q=bira">bira</a>. sansürsüz internet. ifade özgürlüğü. <a class="b" href="/?q=liyakat">liyakat</a>. uygun fiyata kaliteli binek otomobiller. ne isteyecek gençler <a class="b" href="/?q=konya">konya</a> mı isteyecek amk.
  entry_channels: siyaset,haber,eğitim,ekonomi,seyahat
- entry_name: |
    19 mayıs'a hakaret eden trabzonlu doktor
  entry_id:  107446928
  entry_writer: kabatasliozgur
  entry_fav_count: 405
  entry_content: |
    trabzonluyum, ktü den mezun oldum.<br/><br/>o doktorun anasını sikeyim.
  entry_channels: haber
- entry_name: |
    starbucks
  entry_id:  107435798
  entry_writer: sutlabi
  entry_fav_count: 395
  entry_content: |
    sokakta mendil satan çocuğun karnını doyurmak salaklık ve yanlış bir hareket olduğu için white chocolate mocha alınıp tatlı tatlı gömülmelidir.<br/><br/>edit: ya ne güzel iki kelime ile durumu anlatmıştım ama çok mesaj geliyor neden salaklık, neresi yanlış diye. ben ayrı ayrı yazdım diye ayrı açıklama yapamaya gerek yok yanlışsa salaklıktır ama neden yanlış olduğunu açıklayayım. o çocuklar öyle ya da böyle bir sebeple zorlanıyor olabilirler, dilendiriliyor olabilirler bilmiyorum. hangi amaçla orada olduklarından ziyade net olan orada olmamaları gerektiğidir. eğer siz onların oradaki olumsuz şartlarını ortadan kaldırırsanız onun durumunu pekiştirmiş olursunuz. yani ortamdaki olumsuz durumu kaldırarak sokağı "karnını doyurabileceği bir yer" haline getirirsiniz. bu da sokağa çıkmasını daha tercih edilir yapar. <br/><br/>ayrıca siz kimsiniz de tanımadığınız çocuklara yiyecek falan veriyorsunuz. ne sikim sokum insanlarsınız! geçen de yazdım buradan siktirin gidin vicdan masturbasyonunuzu başka şekillerde yapın.(onu okumamış olabilirsiniz<sup class="ab"><a title="(bkz: swh)" href="/?q=swh" data-query="swh">*</a></sup>) tanımadığınız çocuklara daha doğrusu sizi tanımayan çocuklara siz onlar yabancı konumunda iken yardım ederseniz çocuk için: "yabancılar iyi insanlardır" algısını oluşturursunuz. anneler babalar boşuna mı götünü yırtıyor olm? çocuklarına "tanımadığınız insanlarla konuşmayın, onlarla gitmeyin, verdikleri yiyecekleri almayın" derken. alemin akıllısı siz misiniz? yabancılar her türlü çocuk için kötüdür. olmak zorundadır. sikecem tutarsız romantik vicdani eylemlerinizi. bir boka yaramayan aksine zarar veren. eve gidip "ooo ne büyük yardım yaptım" diye dört köşe uyuyorsunuz. <br/><br/>zorla ağzımı bozduruyorsunuz bana bir de. biri de: "sen ne yaptın sokak çocukları için?" diye sormuş. mallık derecesi öyle ileri bir noktaya taşınmış ki insan ne cevap vereceğini şaşırıyor. <br/><br/>aşağıda arkadaşın biri yazmış ne yapılması gerektiğini 7-8 enrty falan aşağıda. okuyun.
  entry_channels: ekşi-sözlük,yeme-içme
- entry_name: |
    erdoğan'ın atatürk'ten daha çok sevilme sebebi
  entry_id:  107452119
  entry_writer: zinaedenzidan
  entry_fav_count: 376
  entry_content: |
    kudur kudur, hatta git sinirden yorgan kemir. hahhahaha siz çıldırdıkça biz daha mutlu oluyoruz. mustafa kemal’in adı bile düm düz ediyor, sinirden kıvranıyorsunuz.
  entry_channels: siyaset
- entry_name: |
    vahdettin'in kaçarken 140 milyar servet götürmesi
  entry_id:  107446244
  entry_writer: girabic
  entry_fav_count: 223
  entry_content: |
    bugünkü hesaplamalara göre ülkeye 500 milyar usd (145 milyon osmanlı altın lirası) borç bırakan padişah vahdettin, ingiliz gemisiyle maltaya kaçarken yanında 140 milyar tl değerinde olduğu tahmin edilen ülke varlığını yanında götürmüştür.<br/><br/>belirtilen rakam o kadar büyük ki padişah hazretleri, kaçtıktan sonra da saray yaşantısını geliri olmadan da belli bir süre devam ettirebilmiş. <br/><br/>kaynak1:<a rel="nofollow noopener" class="url" target="_blank" href="https://www.sozcu.com.tr/2019/yazarlar/sinan-meydan/vahdettinin-kacisi-5456491/" title="https://www.sozcu.com.tr/2019/yazarlar/sinan-meydan/vahdettinin-kacisi-5456491/">https://www.sozcu.com.tr/…hdettinin-kacisi-5456491/</a><br/><br/>kaynak2:<a rel="nofollow noopener" class="url" target="_blank" href="http://www.mahfiegilmez.com/2011/12/osmanlidan-devraldigimiz-borclar.html?m=1" title="http://www.mahfiegilmez.com/2011/12/osmanlidan-devraldigimiz-borclar.html?m=1">http://www.mahfiegilmez.com/…gimiz-borclar.html?m=1</a><br/><br/>edit:belirtilen rakam bugünkü değerlenmeye göre sinan meydan tarafından yapılmıştır. <br/>kaynak1 vahdettin'in yanında götürdüğü servetten bahsetmektedir. hazineden para çaldı gibi bir iddia değildir. <br/>kaynak2 ise osmanlıdan kalan toplam borçla ilgili turkiye'nin değerli ekonomist ve bürokratlarından mahfi eğilmez'in yazısıdır.<br/>edit2:o kadar para götürdü de neden borç içinde öldü diye soranlar, hazıra dağ dayanmıyor diye boşa demiyorlar. etrafında parayı çarcur eden çok kişi varmış. ticaret yapmamış, bir işte çalışmamış, saray standartlarında yaşamaya devam etmiş.<br/>almanya'ya gidip üç asgari ücretle çalışanlar bile para biriktirip ev alıyorsa, borçlu ölmesi, ilaçlarını alamaması vahdettin'in finans bilgisinin olmadığının kanıtı.<br/>düşünün bu adam bir de koskoca imparatorluk yönetecek ülkeyi düşmandan kurtaracaktı öyle mi?
  entry_channels: siyaset,tarih,haber,ekonomi
- entry_name: |
    amerika'da 100 dolar'a alım gücünü sınayan genç
  entry_id:  107454147
  entry_writer: girdapmc5
  entry_fav_count: 209
  entry_content: |
    12'li tuvalet kağıdı(orta kalite):30tl<br/>1.5 kilo kıyma:67.5tl(en ucuzu)<br/><br/>kalan para:2.5 tl<br/><br/>etinizi yiyip sıçtıktan sonra kıçınızı temizleyebilirsiniz.bu ülke vatandaşına çok bile.<br/><br/>edit:kıymanın daha pahalı olduğuna dair gelen tepkiler üzerine kıyma fiyatına "(en ucuzu)" eklemesi yapıldı.daha pahalı yazsaydım vatan haini ilan edilebilirdim,anlayışla karşılayacağınızı umuyorum.
  entry_channels: haber,ekonomi
- entry_name: |
    çocukken baba eve getirdiğinde mutlu olunan şeyler
  entry_id:  107432100
  entry_writer: sessiz sakin suskun adam
  entry_fav_count: 164
  entry_content: |
    (bkz: <a class="b" href="/?q=kar">kar</a>)<br/><br/>odaya girdiği zaman oleyy diye sevinçle koşup babamın omzundan karları alırdım. uzun bir palto giyerdi. zaten sürekli onu giyerdi. sonra avcunu açar ellerine suyunu damlatırdım. annem "yapma şunu silkelen de öyle gel" diye babama kızsa da o hiç annemi dinlemezdi. ne zaman kar yağsa babamla bu oyunu oynardık. işten yorgun gelse bile o uzun paltosuyla dağ gibi karşımda dururdu. karları alınca da sobanın yanındaki sandalyeye oturup avucunu açardı. ilk zamanları sobaya damlatıyordum ama cıbs cıbs diye annemin kafasını çok şişirdigim için babam sonradan hep ellerini açtı. "karışma hanım o gülünce benim yorgunluğum gidiyor" derdi.<br/><br/>lan yaşım 30'u geçti. halen dertli olup çıkmaza girdiğim zaman kar yağınca doğru babamın yanına giderim. alırım toprağının üstündeki karları tek tek mezarına damlatırım. ruhuma çok iyi geliyor, umarım ona da geliyordur.
  entry_channels: anket,ilişkiler
- entry_name: |
    arda turan'ın 19 mayıs'ı çocuk bayramı sanması
  entry_id:  107459991
  entry_writer: theanothereconomist
  entry_fav_count: 164
  entry_content: |
    trt ile ölümüne yarışır, cahillikte.
  entry_channels: spor,magazin
- entry_name: |
    suriye'de uzman çavuşu döven paşa
  entry_id:  107454169
  entry_writer: jasonmranda
  entry_fav_count: 143
  entry_content: |
    ercan paşa kimmiş diye baktım rütbesi tuğgeneral. koskoca generalin adam döveceğine inanmıyorum. 30 ağustos yaş yaklaşırken karakter suikastı mı acaba? olayın videosu var mı? yaycı paşa'yı geçen gün istifa ettiren kumpasçıların yeni bir oyunu sanki.
  entry_channels: siyaset
- entry_name: |
    zeynep bastık'ın arkadaş ortamlarında osurması
  entry_id:  107456292
  entry_writer: hep gel
  entry_fav_count: 124
  entry_content: |
    muhtemelen başkası osurmuş, <br/><br/>zeynep osuruğun coverını yapmıştır.<br/><br/>yalnız başına osuracak kabiliyeti olduğunu sanmıyorum.
  entry_channels: ilişkiler,müzik,magazin
- entry_name: |
    trt'nin 19 mayıs'ı cumhuriyet bayramı sanması
  entry_id:  107444764
  entry_writer: birliktelik
  entry_fav_count: 123
  entry_content: |
    (bkz: <a class="b" href="/?q=19+may%c4%b1s+cumhuriyeti+anma+ve+spor+salonu+bayram%c4%b1">19 mayıs cumhuriyeti anma ve spor salonu bayramı</a>)<br/><br/>edit: fotoşop diyenler, iptv'niz varsa (tivibu vs) sabah 6daki programa girip 2. dakikada kontrol edebilirsiniz. resim gerçek<br/><br/>ekleme: bu da benden gelsin. yukarıda bahsettiğim yayından çektim resmi. görüldüğü gibi spiker farklı. hatanın uzun süre devam ettiği anlaşılıyor.<br/><a rel="nofollow noopener" class="url" target="_blank" href="https://eksiup.com/p/qi3863448mni" title="https://eksiup.com/p/qi3863448mni">tıkıtıkıtıktık</a>
  entry_channels: tv,tarih,haber
- entry_name: |
    abazaların yüzünden kız bulamamamız
  entry_id:  107443488
  entry_writer: cemoless
  entry_fav_count: 119
  entry_content: |
    üst edit: başlık dikkat çeksin amacıyla bu şekilde açılmıştır çok da itibar etmeyiniz.( abaza ırkı neden alındıysa sizlik bir durum yok. abazanlar olacaktı.)<br/><br/>bu başlık düzgün ilişki kurmak isteyen kardeşlerimizin sesidir.<br/><br/>günümüzün en büyük problemi budur. bu konu hakkında aşırı dolu ve sinirliyim. <br/><br/>sevgili erkekler!<br/>öncelikle orta güzellikte hatta güzel değil de tatlı diyebileceğimiz kızların mesaj kutusunu görseniz erkekliğinizden utanırsınız. öyle prokaryot, tek hücreli canlılar var ki. kızları sürekli rahatsız etmekteler. yaşlılar da ağırlıklı bu arada.<br/>bir çok örnek var atmama gerek yok aşağı yukarı zaten biliyorsunuzdur. sürekli iyi akşamlar yazmalar. benimle evlen diyen amcalar var lan. <br/>neyse hepsinin amına koyayım. pedofili sapık yobazların.<br/>diğer tarzlarda ayrı salak. kıza arama gönderiyor, sürekli selam yazıyor, anla işte dönmüyor sana kız amın evladı bırak artık. öyle bir azmış ki sabah akşam yazıyor. durmuyor. sonrada “hyrdr yni havan kime” tarzı kendi kendine konuşuyor bu şizofrenler.<br/><br/>sonuç olarak bu durumun sonunda kızlarımıza gelen özgüvenle, vazgeçilmezim hissiyle ki bu his eninde sonunda oluyor inkar edemezsiniz. böylece biz konuşarak, anlaşarak güzelce tanışmak isteyen kişilere ne mi oluyor? mesajlarımızı görmüyorlar bile. görse de onlardan sanıyorlar. konuşsa da belli bir yerden sonra bir çok alternatifim var düşüncesiyle en ufak şeydi siktir edebililiyorlar. birinden gerçekten hoşlanılmışsa onla ilişkiye geçmenin tek yolu neredeyse yüz yüze konuşmaktan öteye gitmiyor. fakat bu durum yakışıklılar için geçerli değil. onu da söyleyelim. yakışıklıysanız anında dönüyorlar.<br/><br/>edit: netflix’ten kız düşürmeye çalışan var bu ülkede. değişik durumlardayız. işe de yaramıyor amk. ıslarla deniyorlar. ben milyonda bir yazarım yazarsam. o da selam falan diyerek değil. direk bir konudan girerim konu devam eder. ama selam da yazamayacak duruma geldik sinirim ona.<br/><br/>edit2: ekşi’de de herkes alfaymış ya la. helal şaşırmadım. şey dimi aşırı sosyal ortamlar falan böyle devam.<br/><br/>edit3: günümüzde sosyal medya büyük bir güç. sanki herkes bardan tanışıyor. çoğunluk sosyal medya sayesinde evliliğe kadar varan ilişkilere giriyor. havalı gözükmek için saçma saçma şeyler söyleyip durmayın.<br/><br/>edit4: dengesizin biri de işi gelip tipe bağlamış. bak güzelim. şuana kadar çevremdekilerden yakışıklı olduğumu söylediklerini işiterek büyüdüm. kendimi öyle bulmam orası ayrı. olay yakışıklılık falan değil. ortak bir sorundan bahsediyoruz. <br/><br/>edit5: olgun abiler de 40’lık ablalara yazan 20’liklerden şikayetçi. başka coğrafyada yaşayamam.
  entry_channels: ilişkiler
- entry_name: |
    köpektaparın yaşlı bir kadını darp etmesi
  entry_id:  107452328
  entry_writer: moyes
  entry_fav_count: 76
  entry_content: |
    <a class="b" href="/?q=k%c3%b6pektapar">köpektapar</a> yerine <a class="b" href="/?q=k%c3%b6pe%c4%9fi+olan+biri">köpeği olan biri</a> dense, ve <a class="b" href="/?q=ba%c5%9f%c3%b6rt%c3%bcs%c3%bc">başörtüsü</a> işe karıştırılmasa belki inanacaktım. ama böyle yazınca konunun provokasyon amaçlı olduğu düşünülüyor. alakasız bir konuda mağdur olurken bile siyaset yapılması garip.
  entry_channels: haber
- entry_name: |
    terminatör'de alyan başlı vida kullanılması
  entry_id:  107447687
  entry_writer: eksimtrack
  entry_fav_count: 74
  entry_content: |
    onlarca yıl sonra bile bağlantı elemanlarının aynı şekilde kaldığını gösterir. şimdi bu makina herif, tonlarca ağırlıktaki bir arabayı kaldırabiliyor ve bunca yükü kolundaki metrik 10 ve metrik 6 vidalar mı tutuyor? ayrıca dikkatimi çekti, hidrolik pistonların yağ giriş ve çıkış hortum bağlantıları yok. <br/><br/><a rel="nofollow noopener" class="url" target="_blank" href="https://soz.lk/i/n7ldyh5b" title="https://soz.lk/i/n7ldyh5b">1. görsel</a><br/><a rel="nofollow noopener" class="url" target="_blank" href="https://soz.lk/i/mgs2dgnm" title="https://soz.lk/i/mgs2dgnm">2. yakın çekim görsel</a><br/><br/>alıntı edit: "sonuçta filmde seri üretimle üretiliyor onlar. seri üretimde standart malzeme kullanmak hem zamandan hem malzemeden tasarruf sağlar.<br/>ki bu da kıyamet sonrası kısıtlı kaynakların bulunduğu bir dünyada oldukça mantıklı.<br/>makina yada insan olmak kaynak kıtlığından etkilemeni pek engellemiyor." demiş arkadaş ve haklı. ayrıca paslanmaz kullanmaları daha mantıklı olurmuş.
  entry_channels: sinema,teknoloji
- entry_name: |
    survivor 2020 ünlüler-gönüllüler
  entry_id:  107410935
  entry_writer: zita
  entry_fav_count: 66
  entry_content: |
    ardahan'ın instagram'dan takip ettiklerine baktım; 1888 kişi takip ediyor, 1500 civarı kadın. sağlam abazasın babuş.
  entry_channels: anket,tv,magazin,spoiler
- entry_name: |
    2020 ekonomik krizi
  entry_id:  107438140
  entry_writer: alemin kizanlari cin gibi
  entry_fav_count: 58
  entry_content: |
    nusret'e gittim herkes et yiyordu, hani nerde ekonomik kriz?<br/><br/>türkiye'yi kendi çevresinden ibaret zanneden muhteşem bir gerizekalı topluluğu var.
  entry_channels: siyaset,anket,haber,ekonomi
- entry_name: |
    deniz mi daha çok huzur verir orman mı sorunsalı
  entry_id:  107442162
  entry_writer: triggernometry
  entry_fav_count: 55
  entry_content: |
    mangalcılar yoksa ikisi de huzur verir.<br/><br/>mangalcılar varsa ikisi de eziyet olur.
  entry_channels: anket,seyahat
- entry_name: |
    bjk'nin twitter'da başlattığı gençlik marşı akımı
  entry_id:  107457442
  entry_writer: sozel mantik
  entry_fav_count: 49
  entry_content: |
    yaratıcılarından birinin galatasaray lisesi mezunu olan marş üzerinden. galatasaray'ı etiketlemeyin fransızları etiketlemeyin len diyen zır cahilleri göstermiştir.<br/><br/>bunu demeyi çok seviyorum. eğitilmezsiniz.
  entry_channels: spor
- entry_name: |
    19 mayıs 2020 covid-19 vakasının 151615'e çıkması
  entry_id:  107450351
  entry_writer: i thank you very many
  entry_fav_count: 49
  entry_content: |
    avm ve berber ile kuaförlerin açılmasının pozitif etkisinin görüldüğü sonuçlar.<br/><br/>şimdi sırada haftasonu yasaklarının kalkması var. böylece sayı daha da düşer.
  entry_channels: siyaset,sağlık,haber
- entry_name: |
    evde spor yapmak
  entry_id:  107451124
  entry_writer: oylesinebirivar
  entry_fav_count: 45
  entry_content: |
    yapıyorum yapıyorum aynıyım sinir krizi geçiriyorum. 2 mekik çekip aynada kendime bakmaya başladığım için olabilir. sırf daha çok yapayım diye crop giyip ayna karşısında yapıyorum bu sefer de yüzümü incelemeye başlıyorum. ben zayıflayamıyorum en iyisi siz kilo alın.
  entry_channels: spor,sağlık
- entry_name: |
    atilla yayla'nın skandal atatürk twiti
  entry_id:  107460052
  entry_writer: robin tut
  entry_fav_count: 42
  entry_content: |
    kesin fetöcüdür.<br/><br/>yine yanılmadık:<br/><a rel="nofollow noopener" class="url" target="_blank" href="https://t24.com.tr/amp/haber/gulen-cemaati-parti-kurmali-cunku,274804" title="https://t24.com.tr/amp/haber/gulen-cemaati-parti-kurmali-cunku,274804">https://t24.com.tr/…aati-parti-kurmali-cunku,274804</a>
  entry_channels: siyaset,tarih,haber,eğitim
- entry_name: |
    gençlik marşına galatasaray'ı etiketlememek
  entry_id:  107458638
  entry_writer: black saint
  entry_fav_count: 38
  entry_content: |
    sanırım bir paris saint germain maçıydı...o zamandan beri avrupa maçlarında dağ başını duman almış söylenir.kargalar kuşlar bilmez.
  entry_channels: spor
- entry_name: |
    19 mayıs 2020 19.19'da istiklal marşı okunması
  entry_id:  107450853
  entry_writer: opridon
  entry_fav_count: 36
  entry_content: |
    ve elbette cami hoparlörlerinin iştirak etmediği eylem.
  entry_channels: haber
- entry_name: |
    seyahat izin belgesi
  entry_id:  107417351
  entry_writer: zhang
  entry_fav_count: 33
  entry_content: |
    sakın kontrol edilmiyor diye sahte belge üretmeyin. 2 gün önce istanbul'dan adana'ya gittim ve geldim, en az 10 kere kontrol noktasında durduruldum, 3 kez belgemin doğruluğu sistem üzerinden kontrol edildi.<br/><br/>enselendiğiniz anda 4-5 farklı suç oluşmuş oluyor. <br/><br/>tutuklanırsınız, net.
  entry_channels: siyaset,seyahat
- entry_name: |
    galatasaray'ın karton taraftar uygulaması
  entry_id:  107437314
  entry_writer: horsy
  entry_fav_count: 28
  entry_content: |
    wining eleven 8 teknolojisiyle.
  entry_channels: spor
- entry_name: |
    güzel kızların çirkin erkeklerle beraber olmaları
  entry_id:  107440626
  entry_writer: andirann
  entry_fav_count: 26
  entry_content: |
    5 üstteki entry anlatmış arkadaşlar. görünüşünden hoşlanmamıştım tam bir hınzır :) gibi cümleler var. şimdi bu cümleyi yorumlamak ve eleştirmek bize düşmez ama ben buradaki çirkin erkeklerin meslek itibariyle nasıl hınzır olabildiklerini görmek gerekir diye düşünüyorum. o çocuk o inşaatta çalışan birisi olsaydı sapık olurdu :))
  entry_channels: anket,ilişkiler
- entry_name: |
    19 mayıs 2020 efsanevi habertürk canlı yayını
  entry_id:  107437177
  entry_writer: siradan cinko karbon pil
  entry_fav_count: 22
  entry_content: |
    adamlar düzenli olarak her hafta fatih altaylı'nın programına katılıyolar zaten. neyi efsanevi onu anlamadım? sanki tv'de bir ilk?
  entry_channels: tv
- entry_name: |
    kavgaya çağrılabilecek dizi karakterleri
  entry_id:  107460185
  entry_writer: balrenkliseyler
  entry_fav_count: 21
  entry_content: |
    kuzey tekinoğlu...
  entry_channels: anket,tv
- entry_name: |
    hiçbir şeyin h ile başlayıp y ile bitmemesi
  entry_id:  107462371
  entry_writer: theanothereconomist
  entry_fav_count: 20
  entry_content: |
    arkadaşı hatay'a alalım. halaya da olabilir. ehehe.
  entry_channels: 
- entry_name: |
    köpektapar
  entry_id:  107453406
  entry_writer: antik sozluk
  entry_fav_count: 15
  entry_content: |
    <a class="b" href="/?q=itperest">itperest</a> daha iyi bir kelime olurdu.
  entry_channels: ilişkiler
- entry_name: |
    kadınları çekici yapan detaylar
  entry_id:  107444413
  entry_writer: askmelovebtl
  entry_fav_count: 15
  entry_content: |
    özgüven<br/>bazı kadınlar vardır “güzel değil ama bir şey var” dersin ya işte o özgüven.
  entry_channels: anket,ilişkiler
- entry_name: |
    türk kızının bugün çok güzelsin'e vereceği cevap
  entry_id:  107458008
  entry_writer: mazda
  entry_fav_count: 13
  entry_content: |
    "teşekkür ederim"
  entry_channels: anket,ilişkiler
- entry_name: |
    hastası olunan sözler
  entry_id:  107419027
  entry_writer: ozgurluk savascisi
  entry_fav_count: 13
  entry_content: |
    "kimlerle vakit geçirdiğinizi iyi seçin. insan, birlikte en çok zaman geçirdiği beş kişinin ortalamasıdır".<br/><br/>"bir aptalı motive ederseniz, motive olmuş bir aptal elde edersiniz".<br/><br/>(bkz: <a class="b" href="/?q=jim+rohn">jim rohn</a>)<br/><br/>hastasıyım bu adamın.
  entry_channels: anket,ilişkiler,sanat,edebiyat
- entry_name: |
    vahdettin
  entry_id:  107433286
  entry_writer: lukretius
  entry_fav_count: 11
  entry_content: |
    son bizans imparatoru bile istanbul'u türklere savaşarak ve ölerek teslim etmiştir; yiğidi öldür, hakkını yeme... bu adam ise istanbul'u ve türk topraklarını ingilize, yunana, fransıza, italyana, ermeniye kendi elleriyle teslim etti. bir insanın vatan haini olması için daha ne yapması lazım? haindir, hem de hainin önde gidenidir.<br/><br/><a rel="nofollow noopener" class="url" target="_blank" href="https://www.youtube.com/watch?v=Qk7egz2MwHY">https://www.youtube.com/watch?v=qk7egz2mwhy</a>
  entry_channels: siyaset,tarih
- entry_name: |
    19 mayıs 2020 beşiktaş covid-19 durumu
  entry_id:  107458458
  entry_writer: paul sahillleri
  entry_fav_count: 11
  entry_content: |
    terim'e tiyatrocu diyen yavşak oğlu yavşaklar alsınlar şimdi bu açıklamayı götlerine soksunlar hadi.
  entry_channels: spor
- entry_name: |
    sözlükçülerin en eften püften başarıları
  entry_id:  107416716
  entry_writer: aysediyebiri
  entry_fav_count: 7
  entry_content: |
    şu an bulunduğum köyden çin’e canlı bağlanıp aktivist olan italyan arkadaşıma korona virüsün türkiye’ye etkilerini anlattım. durum tespit raporu verdim. <br/><br/>ek: canlı yayın esnasında annem babama dönüp;<br/>-aaa ingilizce konuşuyor duydun mu? dedi. <br/>ingilizce öğretmeni olmam dışında sorun yok.
  entry_channels: anket,ekşi-sözlük
- entry_name: |
    ölünce çok üzülünen film karakterleri
  entry_id:  107457611
  entry_writer: ch5006
  entry_fav_count: 6
  entry_content: |
    <a class="b" href="/?q=kabaday%c4%b1">kabadayı</a> filminde, rasim öztekin'in canlandırdığı <a class="b" href="/?q=s%c3%bcrmeli">sürmeli</a> karakteri ölünce çok üzülmüştüm. rahmetli taşaklı adamdı
  entry_channels: anket,sinema,spoiler
- entry_name: |
    kadın budu köftenin içinde kadın budu olmaması
  entry_id:  107462461
  entry_writer: zayiflatilmismikrop
  entry_fav_count: 6
  entry_content: |
    tavuk göğsü tatlısında tavuk olmasından daha garip olmayan durumdur.
  entry_channels: 
- entry_name: |
    sigarayı bırakmanın yan etkileri
  entry_id:  107457394
  entry_writer: wishit
  entry_fav_count: 5
  entry_content: |
    bol terleme, spor ve bol su içme ile atlatılabilen etkilerdir. vücudunuz sizinle oynamayı 1 aya kadar bırakıyor ama beyin öyle değil. beyin aynı oyunları belirli arılıklarla oynamayı sürdürüyor, daha doğrusu simüle etmeyi. <br/><br/>bir örnek vereyim, sigarayı bıraktından ve 1 nefes bile almadan geçen 6 ay sonunda 3 saatlik bir uçak yolculuğu yaptım. uçaktan indiğimde eski bir tiriyaki olarak sigara refleksim inanılmaz sert bir şekilde beynim tarafından devreye sokuldu. havalimanı çıkışında resmen başım dönüyor, terliyor ve sigara isteği ile deliriyordum. 1 saat kadar sürdü. reflekslerden doğan tetiklemelere aldanmayın, yenik düşmeyin.
  entry_channels: anket,sağlık
- entry_name: |
    ilber ortaylı anlatısından hiçbir şey anlamamak
  entry_id:  107460902
  entry_writer: nickle adam olunmuyor
  entry_fav_count: 3
  entry_content: |
    konuyu uzattıkça uzatmasındandır. uykulu uykulu da anlatınca bir süre sonra anlattığı olaylarla ilgili bağlantı kuramıyorsun. gerçekten anlayabilmek için pür dikkat dinlemek gerek.
  entry_channels: 
- entry_name: |
    aydın denince akla gelenler
  entry_id:  107460507
  entry_writer: corsairsgold35
  entry_fav_count: 3
  entry_content: |
    (bkz: <a class="b" href="/?q=ayd%c4%b1n+zeybe%c4%9fi">aydın zeybeği</a>)
  entry_channels: anket,tarih,seyahat
- entry_name: |
    birini unutmanın en iyi yolu
  entry_id:  107460663
  entry_writer: resimci
  entry_fav_count: 1
  entry_content: |
    birini unutmaya çalışmak, birini unutmak için yapılabilecek en acı verici eylemdir. <br/><br/>off kafam nasıl güzel aforizma döktürüyorum
  entry_channels: anket,ilişkiler
- entry_name: |
    antidepresanı bırakmak
  entry_id:  107453451
  entry_writer: tolstoyunpisikleti
  entry_fav_count: 1
  entry_content: |
    aniden bıraktığınız zaman ilk başlarda “niye kullanıyormuşum ki, her şey çok iyi, çok güzel, ben çok iyiyim” dersiniz, bir süre sonra duvara toslarsınız. <br/><br/>doktor kontrolünde gerçekleştirilmesi gereken eylemdir.
  entry_channels: anket,sağlık
- entry_name: |
    ahmet davutoğlu
  entry_id:  107420878
  entry_writer: muhendisolacakliseli
  entry_fav_count: 1
  entry_content: |
    asla ama asla iyi parti ve chp seçmeni ahmet davutoğlu'na veya ali babacan'a oy vermemeli. bırakalım ahmet davutoğlu ve ali babacan sadece cumhur ittifakından oy koparsınlar.<br/>tanım: gelecek partili siyasetçi.
  entry_channels: siyaset,tarih,haber,eğitim,ekonomi
- entry_name: |
    aft acısı
  entry_id:  107457672
  entry_writer: thorolf
  entry_fav_count: 1
  entry_content: |
    <a class="b" href="/?q=kenacort+a">kenacort a</a> ile dindirilebilecek acıdır. tabi aftı iyi takip edip çıkacağı anı sezmeniz gerek. profesyonel aftzedeler bilir; aft çıkmadan hemen önce o bölgede bir kabarıklık olur. işte o an kenacort a’nın basılması gereken andır. o anı kaçırırsanız bu acıyı dindirmenin bir yolu yoktur, geçmiş olsundur.
  entry_channels: sağlık
- entry_name: |
    sözlük yazarlarının evcil hayvanlarının isimleri
  entry_id:  107460140
  entry_writer: zeyneppisii
  entry_fav_count: 0
  entry_content: |
    zeytin idi. işitme engelli bir tekirdi, adını duymak nasip olmadı yavrucuğuma. duymadığı için ses tonunu kontrol edemez acayip çirkin bir ses tonuyla miyavlardı. ondan bir önceki muhabbet kuşunun ismi de figo idi.
  entry_channels: anket,ekşi-sözlük
- entry_name: |
    saçı 3 numaraya vurma nedenleri
  entry_id:  107463278
  entry_writer: doctor who the time lord
  entry_fav_count: 0
  entry_content: |
    tespit yapmak içindir.<br/>gençken sıklaşsın, geç dökülsün diye. yaş kemale erince ne kadar dökülmüş bakıp, çok dökülmemiş diyen berberle moral bulmak için. <br/><br/>neydik ne olduk,<br/>nerden nereye,<br/>eskiden seyreltme makası atarlardı berberler. şimdi 3 numaraya vurup bir bakıyoruz şöyle. berber diyor yok abi sen yine yaşına göre iyisin, en azından açılma yok. hem bana sorarsan kazıtsan çok şahane olur. aynı berber, hatay'dan boğma rakı geldi abi istersen akşam takılalım deyince manidar oluyor. bir gün cesaretimi toplayıp içeceğim o berberle. içirip sarhoş edince soracağım ilk soru, doğru söyle lan, gerçekten yakışıyor mu diye sormak olacak.
  entry_channels: anket,sağlık,moda
---
