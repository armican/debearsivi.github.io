---
layout: post
title: 28 Haziran 2018 Ekşi Sözlük Debe
data:
- entry_name: |
    markette yere düşen cüzdanın üzerine yatan kadın
  entry_id:  78573311
  entry_writer: byzantium
  entry_fav_count: 1398
  entry_content: |
    ne yatması aq gayet ayakta. iki saat kadın nasıl yatacak yere diye bekledim.
  entry_channels: haber
- entry_name: |
    mecliste ne dersek o olacak rte'yi biz kurtardık
  entry_id:  78562114
  entry_writer: tetikci muhabir
  entry_fav_count: 837
  entry_content: |
    tayyip sizi öyle bir sikecek ki haberiniz olmayacak
  entry_channels: siyaset,haber
- entry_name: |
    27 haziran 2018 viskilere %70 vergi konulması
  entry_id:  78561858
  entry_writer: bluemeth
  entry_fav_count: 591
  entry_content: |
    ben hep tek buzlu içtiğimden beni hayvan gibi ilgilendiren konu lan bu neymiş.
  entry_channels: haber,yeme-içme,ekonomi
- entry_name: |
    türkiye'nin suriye'den patates ithal etmesi
  entry_id:  78578295
  entry_writer: pizza life
  entry_fav_count: 562
  entry_content: |
    "vatandaşını ithal etmişiz, bi patatesini mi ithal etmeyeceğiz?" dedirten...
  entry_channels: siyaset,haber,yeme-içme,ekonomi
- entry_name: |
    diyarbakır'da açılan millet kıraathanesi
  entry_id:  78578284
  entry_writer: blindbandit
  entry_fav_count: 489
  entry_content: |
    (bkz: <a class="b" href="/?q=100+bine+yak%c4%b1n+kitap">100 bine yakın kitap</a>) <br/>at yalanı ..... .......
  entry_channels: siyaset,haber,yeme-içme
- entry_name: |
    uzun dönem asker erkek aldatılmaya mahkumdur
  entry_id:  78572518
  entry_writer: paslanmazman
  entry_fav_count: 418
  entry_content: |
    bakın kızlar, dişil varlıklar..<br/><br/>ne yaparsanız yapın, sevgilinizi asla ama asla askerde terketmeyin. <br/><br/>geçenlerde bir dişi arkadaşımız böyle birşeyden bahsetmişti; sevgilim askerde ama ben başka biriyle aldattım şimdi hemen terketmeli miyim?<br/><br/>askerliği anlatmak ofsaytı anlatmaktan daha zor. gözler terkedilip gece hüngür hüngür ağlayan o kadar çok "çocuk" gördü ki askerde. ayıptır, günahtır. <br/><br/>dünyanın neresinde olursa olsun asker adamın yanında moral de beraber gider. ne cihan devletleri moralli askerler sayesinde kurulmuştur bir bilseniz. sen bunu o adamdan aldığında, o adam artık yoktur. askerde zaten birey olunmaz. herkes askerdir. sen çocuğu aldatıyorsun, terkediyorsun, sonra o çocuk on ay hakkari'ye dağıtıma gidiyor. bizim ordu norveç ordusu gibi harlem shake yapmıyor dağda bayırda.<br/><br/>böyle bir vebalin altından çok ama çok zor kalkılır.
  entry_channels: ilişkiler
- entry_name: |
    1 temmuz 2018 motorin zammı
  entry_id:  78582756
  entry_writer: cok bilinmeyenli denklemdeki mechul x
  entry_fav_count: 330
  entry_content: |
    10 tl yapmayanın anasını bacısını sikeyim.
  entry_channels: haber,otomotiv,ekonomi
- entry_name: |
    ak parti seçmeninin deli gibi silahlandığı gerçeği
  entry_id:  78582133
  entry_writer: kindaasshole
  entry_fav_count: 288
  entry_content: |
    eğer devlet o gece o videoda görülen ve görülmeyen ama mobeselerden saptanabilen tüm kişileri en kısa zamanda birer birer içeri almazsa hem kendimin yapacağı hem de çevremdekilere yaptıracağım eylem. sonuçta silah alırken kimse hangi partiye oy verildiği sorulmuyor. ben canımı sokakta bulmadım kardeşim, sokakta da kaybetmem. eli silahlı bir grup insan bana göz dağı vermeye kalkarsa cevabını anladıkları dilden de alacaktır bundan sonra.
  entry_channels: siyaset
- entry_name: |
    renault toros ile toroslara gitmek
  entry_id:  78583814
  entry_writer: mahoaganinkuluyam
  entry_fav_count: 244
  entry_content: |
    <a class="b" href="/?q=citroen+saxo+ile+...">citroen saxo ile ...</a>
  entry_channels: anket,otomotiv
- entry_name: |
    recep tayyip erdoğan'ın artık sempatik gelmesi
  entry_id:  78587996
  entry_writer: silayor
  entry_fav_count: 224
  entry_content: |
    benim için artık öyledir. <br/><br/>sik bunları reis, acıma, kulak arkalarını bile es geçme. ben razıyım yeter ki bunların acı acı inlediklerini, ağladıklarını göreyim. ben en kötü özel zevklerimden, biraz da özgürlüğümden kaybederim.<br/><br/>hitler'e sivil halk ölüyor artık teslim olalım dediğinde; "bu, onların tercihiydi, bizi onlar seçti. elbette ölecekler, bedelini ödeyecekler." demişti adolf hitler. <br/><br/>aynen böyle reis, yapıştır. artık bizim taramızdan seviliyorsun, senin hiçbir suçun yok. onlar seçti, onlar istedi.<br/><br/>edit; mesajla uyardılar, o sözü ex aşkum goebbels söylemiş.
  entry_channels: siyaset
- entry_name: |
    tarihin en zor öldürülen 7 kişisi
  entry_id:  78571939
  entry_writer: starwarden
  entry_fav_count: 214
  entry_content: |
    adolf hitler. suikastlerin tamamından sağ kurtulunca s.krim yapacağınız işi deyip kendi işini kendisi bitirmiştir.
  entry_channels: anket,tarih
- entry_name: |
    dijital yazılımla seçim hilesi yaptılar
  entry_id:  78580245
  entry_writer: swollenmember
  entry_fav_count: 189
  entry_content: |
    (bkz: <a class="b" href="/?q=analog+yaz%c4%b1l%c4%b1m">analog yazılım</a>)
  entry_channels: siyaset,haber
- entry_name: |
    çaylaklara mesaj alımını açan yazarın asıl amacı
  entry_id:  78558412
  entry_writer: yazar dedigin benim gibi olur
  entry_fav_count: 140
  entry_content: |
    herkesi sevgiyle kucaklayıp, sevgi ve barışı tüm dünyaya yaymak olabilir. öyle bir tahmin yürüttüm.
  entry_channels: anket,ekşi-sözlük
- entry_name: |
    15 temmuz'da ayin pozu veren esrarengiz genç
  entry_id:  78587741
  entry_writer: evil nymph
  entry_fav_count: 103
  entry_content: |
    bahsi geçen gencin entarisindeki logo kaptan amerika logosudur. davut yıldızı 6 köşeli. of çok güldüm.<br/><br/>edit: daha da eğlenceli hale geldi. editleyip "davut yıldızına benzeyen" yazmış. ya sen hayatında üçgene benzeyen kare, dokuzgene benzeyen çember gördün mü?
  entry_channels: siyaset
- entry_name: |
    deniz gezmiş ve mahir çayan eşkiyaydı
  entry_id:  78581279
  entry_writer: monoditri
  entry_fav_count: 102
  entry_content: |
    haklı beyan.<br/><br/>banka soyup, kamu görevlisi öldürenleri kahramanlaştırmak bir tek bizim kültürümüzde normal karşılanıyordur herhalde.
  entry_channels: siyaset
- entry_name: |
    27 haziran 2018 kemal kılıçdaroğlu konuşması
  entry_id:  78569236
  entry_writer: leblebi tozu
  entry_fav_count: 101
  entry_content: |
    arkadaşlar en dürüst, naif dediğimiz adamlar bile o koltuğa oturunca başka bir boyuta geçiyorlar. bu bir de senelerdir iktidar yüzü görememiş partinin başkanlık koltuğu. bu adam burda bile böyle davranıyorsa, o kadar güç ve yetki zenginliği içindeki erdoğan yine iyi kontrol ediyor kendini. <br/><br/>not: incespor
  entry_channels: siyaset,haber
- entry_name: |
    starbucks'ı tercih etme sebepleri
  entry_id:  78581968
  entry_writer: kibarsosyopat
  entry_fav_count: 97
  entry_content: |
    geçen yaz beşiktaş’ta lüks bir mekanda dj olarak çalışıyordum. haliyle bara ve mutfak kısmına son derece hakimdim. kahve isteyen her müşteriye ister buzlu olsun ister filtre olsun ister başka bir şey, her seferinde nescafe’ye musluk suyu ekleyip gönderiyorlardı. müşteri de afiyetle içiyordu kahveyi. daha yapılan sayısız pislik ve müşteriyi keriz yerine koyma hikayeleri var da bende kalsın. <br/>bu yüzden istediğim kahveyi kaliteli bir şekilde içebileceğim starbucks’ı tercih ediyorum. <br/>not: içtiğiniz fıçı bomonti aslında efes.
  entry_channels: yeme-içme
- entry_name: |
    muharrem ince'nin istanbul bb başkan adayı olması
  entry_id:  78580915
  entry_writer: kermitin dedesi hursit
  entry_fav_count: 84
  entry_content: |
    attan inip eşeğe binmekten farksız olacak durumdur.<br/><br/>sabredip chp genel başkanı, akabinde de erdoğan'ın tekrardan rakibi olmalıdır ve geçen 1.5 ayda kampanyasında yapmış olduğu hatalardan ders alarak iktidarı hedeflemelidir.
  entry_channels: siyaset
- entry_name: |
    seçim gecesi chp'de yaşananlara dair iddialar
  entry_id:  78585417
  entry_writer: bomonti turrret kokusu
  entry_fav_count: 76
  entry_content: |
    ince kazansaydı; ekonomik kriz, chp'nin ve kendisinin üzerine kalacaktı, sittin sene bu yükün altından kalkamazlardı. çok sıkmayın canınızı.
  entry_channels: siyaset,anket,haber
- entry_name: |
    erdoğan'a küfür eden 5 kişinin tutuklanması
  entry_id:  78589677
  entry_writer: once upon a time in sozluk
  entry_fav_count: 75
  entry_content: |
    internet ortamında dolaşan bir kıssadan hisse alamamış kimselerin başına gelendir. <br/><br/><a class="b" href="/?q=---+al%c4%b1nt%c4%b1d%c4%b1r+---">--- alıntıdır ---</a><br/><br/>şili diktatörü pinochet bir gün kılık değiştirip sinemaya gider. salondaki yerine oturur, kimse onu tanıyamamıştır.<br/>derken ışıklar söner, film başlar. filmin bir sahnesinde pinochet’in görüntüsü perdeye gelir. tam o an;<br/>sinemadaki tüm seyirciler ayağa kalkıp alkışlamaya başlar. hatta pinochet lehinde tezahuratlar yapmaya başlarlar.<br/>durumdan çok memnun olan pinochet yayıldıkça yayılır koltuğa. keyfi yerinde büyük bir gururla perdeye bakmaktadır.<br/>yanındaki adam pinochet’in kulağına eğilir: “arkadaşım salon sivil polis dolu. bu it için kendini astırmaya değmez, kalk sen de alkışla!”<br/><br/><a class="b" href="/?q=---+al%c4%b1nt%c4%b1d%c4%b1r+---">--- alıntıdır ---</a>
  entry_channels: siyaset,haber
- entry_name: |
    sevgili bulamama sebepleri
  entry_id:  78565358
  entry_writer: aziraphale
  entry_fav_count: 74
  entry_content: |
    güzel/yakışıklı insanlardan hoşlanmak ve fakat güzel/yakışıklı olmamak.
  entry_channels: anket,ilişkiler
- entry_name: |
    hunharca zikir çeken teyze
  entry_id:  78579973
  entry_writer: said kotan
  entry_fav_count: 67
  entry_content: |
    (bkz: <a class="b" href="/?q=ger%c3%a7ek+islam+bu+de%c4%9fil">gerçek islam bu değil</a>)<br/><br/>lan hakikaten gerçek islam bu değil. hz. muhammed'i getir "bu kadın ne yapıyor?" diye sorar kadın da hz. muhammed'e kendi dinini öğretmeye başlar. işte o anda peygamberler karışır. <br/><br/>hz. muhammed'in ömründe bir kere bile yapmadığı şeyi onun dini gibi göstermek cahillik değilse nedir? işte biz buna <a class="b" href="/?q=cahiliye">cahiliye</a> diyoruz. cahiliye sadece kızları diri diri toprağa gömmek değil demek ki...
  entry_channels: haber
- entry_name: |
    anne babanın çocuğuna bırakacağı en güzel miras
  entry_id:  78564086
  entry_writer: owlowlowl
  entry_fav_count: 66
  entry_content: |
    (bkz: <a class="b" href="/?q=e%c4%9fitim">eğitim</a>)<br/>mal mülk para pul boş çocuklarınızı eğitin
  entry_channels: anket,ilişkiler,eğitim
- entry_name: |
    27 haziran 2018 güney kore almanya maçı
  entry_id:  78583595
  entry_writer: dinazorlar turkmus haci
  entry_fav_count: 64
  entry_content: |
    hala daha teknoloji çomarlığı yapıp "vodoo hokom fotbolo yovoslotoyor" diyenlere sabaha kadar izletilmesi gereken maç. son şampiyon almanya karşısında rakipten giden topa ofsayt verip kore'nin tertemiz golünü engelleyen hakem, belki de almanya'nın tekrar dünya kupası kazanmasına neden olacaktı. <br/><br/>şimdi ne oldu? adalet yerini buldu.
  entry_channels: spor
- entry_name: |
    babanın söylediği unutulmayan sözler
  entry_id:  78562448
  entry_writer: gokcesadik
  entry_fav_count: 52
  entry_content: |
    "ben daha ölmedim kizım, hallederiz"<br/><br/>canım benim..
  entry_channels: anket,ilişkiler,edebiyat
- entry_name: |
    kitlelerin akp'ye oy verme nedeni
  entry_id:  78569366
  entry_writer: avx
  entry_fav_count: 49
  entry_content: |
    kim yazmıştı hatırlamıyorum ama biri senin gibi eğitim göremediği senin bindiğin arabaya binemediği için ve tayyibi senin karşında gördükleri için demişti. bi nevi aşağılık kompleksi. 16 yıldır bu ülkede eğitimli azınlıkla eğitimsiz çoğunluğun didişmesini görüyoruz siyasette. sistem demokrasi olunca çoğunluk dövüyor malesef.
  entry_channels: siyaset
- entry_name: |
    gelmiş geçmiş en iyi protest şarkı
  entry_id:  78571952
  entry_writer: second
  entry_fav_count: 49
  entry_content: |
    (bkz: <a class="b" href="/?q=fele%c4%9fin+%c3%a7emberine+40+kur%c5%9fun">feleğin çemberine 40 kurşun</a>) (bkz: <a class="b" href="/?q=saian">saian</a>)
  entry_channels: anket,müzik
- entry_name: |
    siktir et pizzayı pide söyleyek yarraam diyen kız
  entry_id:  78585608
  entry_writer: dbracgn
  entry_fav_count: 42
  entry_content: |
    yarraaam demesin de pideyi söyleriz yine
  entry_channels: ilişkiler
- entry_name: |
    çalışılan pozisyonlar ve maaşları
  entry_id:  78588332
  entry_writer: keynesinenistesi
  entry_fav_count: 41
  entry_content: |
    düşüren varsa bende yazıcam
  entry_channels: anket,ekonomi
- entry_name: |
    2 yıldır görüşülmeyen arkadaşın mesaj atması
  entry_id:  78573061
  entry_writer: saviela
  entry_fav_count: 41
  entry_content: |
    mega holdings üyesi yapmak istiyordur<br/><br/>edit: başlık sahibi kaçmış
  entry_channels: ilişkiler,teknoloji
- entry_name: |
    27 haziran 2018 istanbul yağmuru
  entry_id:  78562778
  entry_writer: the king of kings
  entry_fav_count: 37
  entry_content: |
    yaz gelmedi diyenler neyin kafasını yaşıyor ben anlamıyorum, antalyaya mı yaz gelmedi? fethiyeye mi yaz gelmedi? evinizde oturduğunuz yerde terlemeye ne kadar meraklısınız amk, ki haftalardır gayet sıcak terletici bi hava vardı istanbulda, sıktı sizin bu yaz nerde muhabbetiniz, siktirin gibi urfaya falan da bi götünüz pişsin.
  entry_channels: haber
- entry_name: |
    40 yaşında problemli bekar müdür kadın
  entry_id:  78560159
  entry_writer: saltin of the saltins
  entry_fav_count: 36
  entry_content: |
    40 yaşının üzerinde şeker gibi bekar kadın yöneticiler tanıyorum. hepsi de kendiyle ve hayatıyla barışık insanlar. bekar olmayı sizin sandığınız kadar büyütmüyorlar hatta çoğu kafaya takmıyor bile. zira öyle de büyük bir şey değil zaten. <br/><br/>bir de 40 yaşının üzerinde evli, problemli erkek ve kadın yöneticiler tanıyorum. bunların da sorunlarını da salt evliliğe bağlayamayız; genel olarak hayatlarından, bulundukları yerden, kariyerlerinden, arabalarının renginden, ne bileyim masalarındaki saksıdan falan memnun değiller. <br/><br/>mevzu evlilik/bekarlıkta değil; insan kendine bir şeyleri dert etmek isteyince, önüne çıkan karıncaya varıncaya kadar takabilir kafayı. her şeyi kocasızlıktan bilmeyin, ne siz sandığınız kadar önemlisiniz ne de kadınlar sandığınız kadar kocaköylü.
  entry_channels: anket,ilişkiler
- entry_name: |
    bedelli askerlik
  entry_id:  78572342
  entry_writer: ogretmenolmayacalisanadam
  entry_fav_count: 33
  entry_content: |
    2, 3 ay önce " sike sike gideceksiniz eheheh" diyen kabzımallar, şimdi çıkacağını anlayınca "inşallah 30 bin olur." diyor. böyle aşağılık bir kitle var işte. bu adamlar azalmadikça bu ülke kurtulmaz.
  entry_channels: siyaset,haber,ekonomi
- entry_name: |
    27 haziran 2018 ruhi çenet videosu
  entry_id:  78587454
  entry_writer: ne dusunuyorsun
  entry_fav_count: 21
  entry_content: |
    hitap ettiği kesimi düşünürsek 2030 seçimleri için ümitlerim artıyor.
  entry_channels: siyaset,teknoloji,ekonomi
- entry_name: |
    survivor 2018 all-star
  entry_id:  78555728
  entry_writer: damdanakan
  entry_fav_count: 20
  entry_content: |
    ihsan'ın suratındaki "çok önemli işler yapıyorum, çok önemli bi insanım" ifadesinden nefret ediyorum ya. veziriazam gibi bi şey sanıyor kendini galiba.
  entry_channels: tv,magazin
- entry_name: |
    ekşi itiraf
  entry_id:  78556887
  entry_writer: acayip bir bas tacisin
  entry_fav_count: 18
  entry_content: |
    baba evinden bu satırlar...<br/>anladım ki her şey çocukken güzelmiş,annenin pişirdiği mis gibi yoğurt çorbası kokusu hani senin ne yaparsan yap kıvamını tutturamadığın. ailene el olmadığın günlerdeymiş mutluluk. <br/>liseden ders bitiminde eve koşarak gelip tertemiz deterjan kokusundaymış huzur. <br/>kışın kardeşlerinle oynadığın karmış saflık,şimdi kar yağınca kat kat giyineceğim diye hayıflanmamakmış.<br/>bayram sabahlarıymış,<br/>limonlu oda kokusu arka fonda şimdiki gibi bozulmamış kral tvden bir şarkı. <br/>sonra yazları tatilmiş,beyaz motel kız kalesi. şimdi çoktan toprak olan <br/>babaannenin nazıymış,anneannenin köyü. <br/>anladım ki büyümek lanetli bir şeymiş. <br/>anladım ki bu masal burda bitmiş...
  entry_channels: anket,ilişkiler,ekşi-sözlük
- entry_name: |
    felsefe sanat kitap belgesel şarap seven kadın
  entry_id:  78565196
  entry_writer: anarchiq
  entry_fav_count: 16
  entry_content: |
    (bkz: <a class="b" href="/?q=trt+2+gibi+kad%c4%b1n">trt 2 gibi kadın</a>)
  entry_channels: anket,ilişkiler
- entry_name: |
    bas dost
  entry_id:  78576093
  entry_writer: kekremsibirtat
  entry_fav_count: 16
  entry_content: |
    olm ben bu adamı zenci, iri yarı bir topçu sandım yıllarca. tam öyle bir isme sahip. resmini bugün gördüm fakat yaşadığım hayal kırıklığını anlatamam.<br/><br/>kimse almasın lan bunu, bundan bir cacık olmaz. adı bas dost ama kendisi bas dost değil.<br/><br/>edit: boyu 196 cm imiş. yazan arkadaşlara teşekkür ediyorum. bu boy ile bas olur ama dost olmaz.
  entry_channels: spor
- entry_name: |
    muğdat çelik
  entry_id:  78588089
  entry_writer: faramir gondor
  entry_fav_count: 15
  entry_content: |
    galatasaray ile anlaştığı söylenen top tepici. bu adamın transferi nasıl oldu aklıma geliyor. aklıma gelen tek senaryo;<br/><br/>(fatih hoca takıma yıldız bir oyuncu almayı düşünmektedir.)<br/>+fatih hocam ozan tufan’ı ve mehmet topal’ı alalım mı?<br/>-oldu aq hatta muğdat’ı da alın.<br/>+baş üstüne hocam.
  entry_channels: spor
- entry_name: |
    herkes sevdiği halde sevilmeyen şeyler
  entry_id:  78589619
  entry_writer: truemykid
  entry_fav_count: 14
  entry_content: |
    gece kuluplerine gitmek
  entry_channels: anket
- entry_name: |
    morg girişine yazılması gereken cümle
  entry_id:  78588776
  entry_writer: corsairsgold35
  entry_fav_count: 10
  entry_content: |
    "asla umudunu kaybetme.morgdan canlı çıkma ihtimalin var.çünkü burası türkiye"
  entry_channels: anket,sağlık
- entry_name: |
    27 haziran 2018 sırbistan brezilya maçı
  entry_id:  78588801
  entry_writer: oliver wendell holmes
  entry_fav_count: 10
  entry_content: |
    neymar’ın millet bahçelerinde nasıl yuvarlanmamız gerektiğini gösterdiği maç.
  entry_channels: spor
- entry_name: |
    rte'nin ölmeden önceki son sözü
  entry_id:  78591407
  entry_writer: plazma
  entry_fav_count: 6
  entry_content: |
    büyük ortadoğu projesi eşbaşkanı olması sebebi ile inşallah yüce rabbim kelime-i şehadet'i nasip eylemez.<br/><br/>.<br/>edit: <br/>mevcut rejim değişikliğinden ötürü bu lafımdan içeri alınacaksam alınayım gitsin. ben bu projeyi de çeşitli ülkelerden bu projeye destek olanları da ölsem bile unutmam. (başta suud ailesindeki yavşaklar! bak bunların soyu kurusun, en beteri bunlar.)<br/><br/>bu nefretimin sebebini merak eden de önce bir haritasına baksın. sonra ırak sınırlarına ve işgaline baksın. sonra suriye sınırlarına ve işgaline baksın. sonra da türkiye'nin sınırlarına bir baksın.
  entry_channels: siyaset,anket
- entry_name: |
    mesut özil
  entry_id:  78584412
  entry_writer: alex xander
  entry_fav_count: 5
  entry_content: |
    kariyerine alman gibi başlayıp türk gibi devam eden oyuncu. 2018 dünya kupası'nın en geberik oyuncusu ödülünü kendisine takdim ediyoruz.
  entry_channels: spor
- entry_name: |
    joachim löw
  entry_id:  78584725
  entry_writer: amandes
  entry_fav_count: 4
  entry_content: |
    leroy sane’nin sende ahı var
  entry_channels: spor,tv,magazin
- entry_name: |
    chp'lilerin erken kutlama görüntüleri
  entry_id:  78591878
  entry_writer: derbender
  entry_fav_count: 4
  entry_content: |
    silahların patlamadığı <a class="b" href="/?q=naif">naif</a> kutlamadır.
  entry_channels: siyaset
- entry_name: |
    sözlük yazarlarının okumakta olduğu kitaplar
  entry_id:  78582170
  entry_writer: skytiret
  entry_fav_count: 3
  entry_content: |
    tüfek mikrop ve çelik.- jared diamond
  entry_channels: anket,edebiyat
- entry_name: |
    ekşi sözlük dertleşecek insan veritabanı
  entry_id:  78562992
  entry_writer: corvi
  entry_fav_count: 2
  entry_content: |
    çay yapmadım diye patronumdan azar yedim (mühendis olarak) istifa edicem iş bulsanıza bana<br/><br/>edit: herkesin içinde azarladı şimdi konuşmak istedim, herkes gitsin sonra dedi. gelişmeleri an an paylaşacağım. elim ayağım titriyor.<br/><br/>edit2: konuştuk herkes dağılınca. sakin sakin " bu şirkette 30 yıldır çay demleniyor sabahları son günlerde bir tavır aldın diye demlemeyecek değilsin yeter dedi. şikayetlerimi anlatmaya çalıştım ama dinlemedi. bende peki o zaman işleri karıştırmanın manası yok o zaman ayrılalım dedim. sen bilirsin dedi.<br/><br/>edit3: patron çıktı şirketten oğlu çağırdı. tüm konuşmalarımıza şahit olmuştu. "ya fevri karar verme oturalım konuşalım seninde şikayetlerini dinleyelim 3ümüz bir konuşalı" dedi. bende " sizinle iletişim kurabiliyorum ama babanızla asla kuramıyorum 2 yıldır buradayım şikayetlerimin bir karşılığı olmuyor uğraşmaya gerek yok ama konuşalım" dedim. bekliyoruz şimdi.<br/><br/>edit 4: sözlük şuan hayretlerim şaşmış beynim uyuşmuş durumda. öğleden sonra patron beni çağırdı ve dedi ki " sabah biraz fevri davrandım tamam şey yapma çok da. hem zaten solar panel işine giriyorum ben istifa edemezsin"<br/>ne?
  entry_channels: anket,ilişkiler,ekşi-sözlük
- entry_name: |
    şu anda çalan şarkı
  entry_id:  78558111
  entry_writer: passion or obsession
  entry_fav_count: 1
  entry_content: |
    <a rel="nofollow noopener" class="url" target="_blank" href="https://youtu.be/iYDYGnBXy7g" title="https://youtu.be/iYDYGnBXy7g">gidelim buralardan - nazan öncel</a><br/><br/>gidelim buralardan<br/>dayanamıyorum<br/>gidelim buralardan<br/>unutamıyorum
  entry_channels: anket,müzik
- entry_name: |
    çaylak onay listesi
  entry_id:  78563294
  entry_writer: damda gezen timsah
  entry_fav_count: 1
  entry_content: |
    teşekkürler sözlük. <br/>ince depresyonundan can vermek üzereyken beni hayata bağladığın için tekrar teşekkürler sözlük.
  entry_channels: ekşi-sözlük
---
