---
layout: post
title: 11 Haziran 2019 Ekşi Sözlük Debe
data:
- entry_name: |
    6 ay verin istanbul'da suriyeli sorunu kalmayacak
  entry_id:  91098050
  entry_writer: kulturshock
  entry_fav_count: 1473
  entry_content: |
    suriyelilere vatandaşlık vererek çözecek sanırım. artık "suriyeli" değil "türkiyeli" sorunu olacaklar. <br/>6 ayda başka türlü çözülemez o sorun.
  entry_channels: siyaset,haber
- entry_name: |
    kız arkadaştan bir anda buz gibi soğutan şeyler
  entry_id:  91100487
  entry_writer: king sized ego
  entry_fav_count: 1370
  entry_content: |
    yeri gelecek doğum yapmak üzere olan eşinizin büyük tuvaletini yapmasına yardımcı olmanız gerekecek, kıldan, b.k’tan sebep kadınlardan soğuyan efendiler! siz isterseniz hiç o topa girmeyin.<br/><br/>belki o sizin altınızı temizlemek zorunda kalacak, kalmayacağının garantisi var mı?
  entry_channels: anket,ilişkiler
- entry_name: |
    15 temmuz'da bizi suriyeliler kurtardı
  entry_id:  91115484
  entry_writer: eeuummm
  entry_fav_count: 1188
  entry_content: |
    yok artık ya bu siyaset değil, buna inanan varsa vay halimize.
  entry_channels: siyaset
- entry_name: |
    izlanda milli takımı için karşılama tavsiyeleri
  entry_id:  91090443
  entry_writer: darbeye kalkisan astegmen
  entry_fav_count: 801
  entry_content: |
    havalimanında çantaları 6 saat boyunca aranıp, bu süre boyunca sürekli <a class="b" href="/?q=ali+ihsan+yavuz">ali ihsan yavuz</a> tarafından “evet çantalarınızda hiçbir şey olmasa da yine de bir şeyler olabilir” açıklamasına maruz bırakmak olabilir.
  entry_channels: spor,anket
- entry_name: |
    nev'in sermiyan midyat'ın kafasına sandalye atması
  entry_id:  91102434
  entry_writer: nick konusunda beceriksizim
  entry_fav_count: 713
  entry_content: |
    nev’i severdim, çok efendi ve çizgisini bozmayan elit bir adamdı gözümde, şuan daha da çok sevdim. eline sağlık nevciğim.
  entry_channels: magazin
- entry_name: |
    16 haziran 2019 imamoğlu yıldırım canlı yayını
  entry_id:  91104874
  entry_writer: bu kuyuya tas atan delidir
  entry_fav_count: 591
  entry_content: |
    ismail küçükkaya çok büyük bir hatadır.<br/><br/>uğur dündar'ın kabul etmeme nedeni zaten bu oyuna gelmemekti ama bizim iso'yu getirmişler oyuna.
  entry_channels: siyaset,tv,haber
- entry_name: |
    300 alperenin izlanda'ya doğru yola çıkması
  entry_id:  91102426
  entry_writer: entellektuel nick
  entry_fav_count: 567
  entry_content: |
    (bkz: 300 alperenin yanlışlıkla irlanda'ya doğru yola çıkması)
  entry_channels: 
- entry_name: |
    milli takımın 3 saat değil 50 dakika beklemesi
  entry_id:  91110369
  entry_writer: nerde o eski gunler
  entry_fav_count: 455
  entry_content: |
    iki ülke arasında krize neden olan, nota'nın falan havada uçuştuğu milli takımın izlanda'ya alınmaması olayı gerçek görünmüyor.<br/><br/>iniş saati: <a rel="nofollow noopener" class="url" target="_blank" href="https://www.flightradar24.com/data/flights/tk3351#20d424a7" title="https://www.flightradar24.com/data/flights/tk3351#20d424a7">22:37 (tsi)</a><br/>burak yılmaz'ın yayın açıp 3 saattir bekliyoruz demesi: <a rel="nofollow noopener" class="url" target="_blank" href="https://twitter.com/Deutsch_Turk/status/1138054375593697280" title="https://twitter.com/Deutsch_Turk/status/1138054375593697280">23:30 gibi.</a><br/><br/>50 dakika sonra 3 saattir bekliyoruz diyor, çıkışın ardından yine aynı şeyi söylüyor, 3 saattir bekliyoruz.<br/><br/>ayıptır.<br/><br/>milyonlarca insan izlerken bile kendisini sürekli yere atarak hakemi aldatmaya yönelik hareketten sürekli ceza alan bir şahsın yine bir aldatma durumu söz konusu gibi görünüyor.
  entry_channels: spor,siyaset,haber
- entry_name: |
    milli takımın izlanda'ya alınmaması
  entry_id:  91087404
  entry_writer: elohir
  entry_fav_count: 408
  entry_content: |
    bence bu sefer tam tersini yapalım. izlanda buraya geldiğinde hiç bekletmeden ülkeye alalım. girişe de medeniyete hoş geldiniz barbarlar pankartı açalım. <br/>hiç bir şey olmazsa havamız olur.<sup class="ab"><a title="(bkz: swh)" href="/?q=swh" data-query="swh">*</a></sup>
  entry_channels: spor,siyaset,haber
- entry_name: |
    caretta caretta'ların insanlara saldırması
  entry_id:  91100463
  entry_writer: juglans nigra
  entry_fav_count: 399
  entry_content: |
    hayvan karaya cıkıyor insan tacizine maruz kaliyor, neyse diyerek tekrar denize dönüyor bu sefer insan orda da tacizde bulunuyor. son derece haklı olan caretta eylemi.<br/><br/>edit: @ucanspagetticanavarininsolkoftesi daha önce caretta caretta koruma programında gönüllü görev aldığını söyleyerek mesajını eklememi rica istedi:<br/><br/>"geçtiğimiz 2 senede 7 aya yakın bir koruma programında gönüllü çalıştım antalya'da belek sahillerinde, sabahın 4ünden başlıyorduk arazilere, 11, 12'ye kadar kalıyorduk. sen kimsin ulan dememeniz için yazıyorum bunları, isterseniz belgeleyebilirim de.<br/><br/>insanların bu konuda en çok yanıldığı nokta şu: carettalar saldırgan bir türdür<br/><br/>carettalar aslında saldırgan sayılabilir amcak insan etiyle ilgilenmiyorlar, burada ısırıldığını zanneden insanların büyük bir çoğunluğu sadece burunla dürtülmüş veya biraz çeneyle bastırılmış oluyor, gece arazilerinde bize söylenen aynen şuydu: elinizi asla hayvanın ağzına sokmayın. bu hayvanın çenesi rahatlıkla ufak boyutlu bir tahta parçasını kibrit gibi kırabiliyor, kemiğin de karşısında pek bir şansı yok.<br/><br/>ikincisi caretta turları. herkes yazlık alanlarda bu olaya hasta ama bu tarz sorunların başlıca sebebi bu turlar. carettaları belli bir bölgede/insanlara yakın tutup müşteri kazanabilmek için dönem dönem gidip besliyorlar ve şartlıyorlar, hayvanlar da beslenmedikleri zaman gördükleri insanların yanına gidip dürtüyor ve yem istiyor, et olur, balık olur. dolayısıyla suç yine otel sahiplerinde, water sport alanlarında.<br/><br/>ayrıca karadayken hayvanları atv'lerle kovalayan, sırtına binip fotoğraf çeken, arkasına takılıp takip eden, hayvanın gözüne flaş patlatan insanlar ve buna izin veren işletme sahipleri var ülkemizde."
  entry_channels: ilişkiler,haber
- entry_name: |
    7400 suriyeli kamuda işe alınacak
  entry_id:  91104009
  entry_writer: david100
  entry_fav_count: 369
  entry_content: |
    <a rel="nofollow noopener" class="url" target="_blank" href="http://www.kamubulteni.com/gundem/iskur-7-bin-400-suriyeliyi-istihdam-edecek-h6444.html" title="http://www.kamubulteni.com/gundem/iskur-7-bin-400-suriyeliyi-istihdam-edecek-h6444.html">öncelikle haber linki</a><br/>ülkemiz de 7, 8 milyon işsiz varken suriyelilerin bu denli korunup kollanması gücüme gidiyor sözlük, artık tamamen kendi öz memleketimden soğudum, imkanım olursa artık gitmek istiyorum bu ülkeden, yeter artık 2, 3 yıldır işsizim, kendi memleketim de yaşamadığım sıkıntı kalmadı, emeği geçen kim varsa allah belasını versin.<br/><br/>edit: bazı şahıslar parasını avrupa veriyor diyor, şunu belirteyim ki türkiye kimsenin bakıcısı değil, bazı avrupa devletleri çok istiyorsa alsın suriyelileri götürüp kendi ülkesin de baksın, benim dedemin şehit olduğu topraklar da, benim dedemi şehit eden araplar, benim dedemin kurduğu resmi dairelerde ça lı şa maz.<br/><br/>edit 2: bu haber ocak, şubat ayların da çıkan haber değil, buğün basına yeni çıkan haberdir, tarihler'e dikkat ediniz.
  entry_channels: siyaset,haber
- entry_name: |
    milyarlarım aksa para vermem denilen şeyler
  entry_id:  91104411
  entry_writer: gabbiadini
  entry_fav_count: 260
  entry_content: |
    (bkz: <a class="b" href="/?q=camiye+yard%c4%b1m">camiye yardım</a>)
  entry_channels: anket
- entry_name: |
    gazipaşa plajının suriyelilere yasaklanma ayıbı
  entry_id:  91118614
  entry_writer: green riddle
  entry_fav_count: 212
  entry_content: |
    tüm ülke geneline yayılmasını dilediğim karardır.<br/><br/>zoruna giden varsa suriyelilerle birlikte siktir olup gidebilir.
  entry_channels: 
- entry_name: |
    9 günlük tatil sonrası ilk pazartesi
  entry_id:  91088084
  entry_writer: bu benim fikrim
  entry_fav_count: 209
  entry_content: |
    son açıklanan işsizlik rakamlarına göre; türkiye genelinde 15 ve daha yukarı yaştakilerde işsiz sayısı 2019 yılı şubat döneminde geçen yılın aynı dönemine göre 1 milyon 376 bin kişi artarak 4 milyon 730 bin kişi oldu. işsizlik oranı 4,1 puanlık artış ile %14,7 seviyesinde gerçekleşti. genç nüfusta (15-24 yaş) işsizlik oranı 7,1 puanlık artış ile %26,1 olurken,15-64 yaş grubunda bu oran 4,1 puanlık artış ile %15,0 olarak gerçekleşti. <br/><br/>yani, sırf bu sabah erken kalkıp gidecek bir işiniz olduğu için bile türkiye'de en az 4 milyon 730 bin kişiden daha şanslısınız. bu kadar ağlamadan sahip olduğunuz şeyin kıymetini bilin istedim. <br/>çalışan arkadaşlara iyi mesailer, hala iş arayan arkadaşlara sabırlar diliyorum.
  entry_channels: anket,yeme-içme,ekonomi
- entry_name: |
    vodafone yanımda bana ne var kepazeliği
  entry_id:  91095718
  entry_writer: kenwakashimazu
  entry_fav_count: 171
  entry_content: |
    dua edin arada sırada çark iflasa denk gelip mevcut paketinizde bulunan interneti yemiyor.
  entry_channels: teknoloji
- entry_name: |
    puanı tıbba yeterken mühendislik yazan insan
  entry_id:  91113503
  entry_writer: deli ali 667
  entry_fav_count: 139
  entry_content: |
    idealisttir.<br/><br/>bizim memlekette ise tıp yazanların çoğu idealist olduklarından değil geçerli meslek, garantisi var diye seçer bunu. sonra da günde bir sürü hastaya bakmaktan insancıl duygularını kaybetmeye başlar bir kısmı bu doktorların.
  entry_channels: anket,eğitim
- entry_name: |
    10 haziran 2019 devlet bahçeli'nin attığı tweet
  entry_id:  91113532
  entry_writer: eksi peace
  entry_fav_count: 130
  entry_content: |
    görüş bildirmeden önce parti liderinden izin almış mı merak ettiğim bahçeli tweet'i.<br/><br/>yayını izlemeyeceğim falan demiş bunlar iddialı sözler.
  entry_channels: siyaset,tv,haber
- entry_name: |
    emre belözoğlu'na bulaşık fırçası uzatılması
  entry_id:  91091559
  entry_writer: teolog35
  entry_fav_count: 123
  entry_content: |
    uzun zamandır magazinsel bir başlığın tüm entrylerini okumuyordum.<br/><br/>bunu okudum ve insanımıza ve insanlığımıza ait ümitlerim iyice azaldı.<br/><br/>milli takımlar saygıdeğerdir ve her yerde saygı görmelidir orası ciddi bir mesele uluslararası arena filan ama bu net ergen şakası elinde de şahsi telefonu var bunun neresini izlanda halkına mal ediyoruz? hangi akıl ve izanla?<br/><br/>olm biz ampute maçında rakibi ıslıkladık (ingiltere maçıydı) bunu afrika’da da, güney amerika’da da yapmaz kimse. milli marş ıslıklama olayımız da özellikle güçlü ülkelere karşı hep oluyor. nedeni de maalesef ezikliğimiz.<br/><br/>valla kusura bakmayalım adamların salaklıkları bireysel oluyor bizimkiler toplumsal.
  entry_channels: spor
- entry_name: |
    ismail küçükkaya
  entry_id:  91105091
  entry_writer: dontyouloveme
  entry_fav_count: 111
  entry_content: |
    fatih portakal gibi muhalif görünerek muhalefeti kontrol altında tutma hareketinin bir diğer aktörü.<br/><br/>ciddi ciddi bu adamlara muhalif falan diyen var. ekrem imamoğlu en az 4 puan önde olduğu bir seçime girecek zaten. karşısındaki adaya mağduriyet yapma fırsatı vermeyecek bir isim olması gerekiyordu ve bu kişi didem arslan yılmaz'dı.<br/><br/>kesinlikle çok yanlış bir seçimdir.
  entry_channels: siyaset,tv,haber
- entry_name: |
    askerlikte ilk gece
  entry_id:  91089071
  entry_writer: porngrinder
  entry_fav_count: 108
  entry_content: |
    (bkz: <a class="b" href="/?q=huzur">huzur</a>)<br/>kendi kendinize kalıp, düşünebileceğiniz, vaktinizi geçirebileceğiniz ve kendinizi gerçekten tanımaya başlayacağınız ilk gecedir.<br/><br/>ne yapmanız gerekeceğini düşünmenize zerre kadar "gerek" yoktur keza birileri sizin için düşünmüştür.<br/><br/>siz sadece kendinizi dinleyecek ve tanıyacaksınız.
  entry_channels: anket
- entry_name: |
    tiktok çeken hakim
  entry_id:  91119691
  entry_writer: i hate you all
  entry_fav_count: 106
  entry_content: |
    şaşırılacak bir durum yok.<br/><br/>özel sektörde giriş seviyesinde dahi çalışamayacak insanları torpille hakim yaparsanız böyle olur.
  entry_channels: ekşi-sözlük
- entry_name: |
    seçime 13 gün kala halen rte'nin miting yapmaması
  entry_id:  91109079
  entry_writer: alt tarafi bi nick yahuu
  entry_fav_count: 96
  entry_content: |
    çünkü vazgeçti, tekrarlanacak seçimin fayda etmeyeceğini, istanbul için artık çok geç olduğunun farkında ve hepimizin hemfikir olduğu üzere kendisi asla bir seçimi kaybetmez kaybederse binali, süleyman hatta damat kaybeder.
  entry_channels: siyaset
- entry_name: |
    bir kadına en çok yakışan hitap şekli
  entry_id:  91105800
  entry_writer: defoe
  entry_fav_count: 90
  entry_content: |
    kendi adıyla hitap etmek.
  entry_channels: anket,ilişkiler
- entry_name: |
    10 haziran 2019 çipras'ın istifa etmesi
  entry_id:  91117408
  entry_writer: galapagosadasindaturubilinmeyensurungen
  entry_fav_count: 79
  entry_content: |
    sorumlusu cehape hatta imamoglu'dur.
  entry_channels: siyaset,haber
- entry_name: |
    eğitimli insanı eğitimsiz gösteren detaylar
  entry_id:  91087261
  entry_writer: nazdrovya baboti
  entry_fav_count: 75
  entry_content: |
    tahsilli olmakla eğitimli olmanın karıştırılmaması gerekir. kendini eğiten kişi eğitimsiz gibi duracak davranışlar sergilemez. burada bahsedilen okul okumaksa üniversitelerin içini bir gezin derim, gerizekalı kaynıyor. konuşmayı, yazmayı bilmeyeni var.
  entry_channels: anket,ilişkiler,eğitim
- entry_name: |
    karslı olmak
  entry_id:  91106316
  entry_writer: zorla yazar yaptiniz beni
  entry_fav_count: 70
  entry_content: |
    nedenini anlayamadigim sekilde kizlara yakistiramadigim ozellik. nerelisin diye sordugum kizin biri karsli'yim deyince farkina vardim bunun. sanki kars'ta tamamen erkekler yasiyormus gibi bir his olusmus bende, anlamlandirilamayan hissiyatlardan biri daha, tuhaf.
  entry_channels: ilişkiler,edebiyat
- entry_name: |
    sözlükteki 30 yaşı aşmış bunak çetesi
  entry_id:  91107659
  entry_writer: sarkinin nakarat kismi
  entry_fav_count: 69
  entry_content: |
    gencolar; sizden evvel dünyaya ve sözlüğe teşrif etmiş olmaktan duyduğum mutluluğu kelimelerle anlatamam.<br/><br/>öyle bir dejenere topluluksunuz ki tüketip tüketip insanlara sarıyorsunuz<br/>konuştuğunuz da kelime olsa çoğunlukla millete hakaret.<br/><br/>unutmadan hatırlatmak isterim, "fazla atar göte batar"
  entry_channels: anket,ekşi-sözlük
- entry_name: |
    didem arslan yılmaz
  entry_id:  91105649
  entry_writer: ex novo
  entry_fav_count: 66
  entry_content: |
    binali yıldırım - ekrem imamoğlu moderatörlüğü konusunda son derece üzücü biçimde hakkı yenen haber spikeri.
  entry_channels: siyaset,tv,haber
- entry_name: |
    tom cruise'un hiçbir filminin iyi olmaması
  entry_id:  91110279
  entry_writer: hamu humu
  entry_fav_count: 51
  entry_content: |
    (bkz: <a class="b" href="/?q=rainman">rainman</a>) izlememiş yazar beyanı. yaşı tutmuyorsa demek ki.
  entry_channels: anket,sinema
- entry_name: |
    pervane pozisyonu
  entry_id:  91114571
  entry_writer: renksiz hayat
  entry_fav_count: 51
  entry_content: |
    hem ayak hem gaita kokusuyla hanımları zevkten bayıltmak için en uygun pozisyon.
  entry_channels: ilişkiler
- entry_name: |
    güne bir görgü kuralı bırak
  entry_id:  91087346
  entry_writer: chemicalengineer
  entry_fav_count: 51
  entry_content: |
    bir insan gunaydin dediyse korkmayin sikmez! sizde gunaydin deyin
  entry_channels: anket,ilişkiler
- entry_name: |
    lazlarla kürtler arasındaki temel farklar
  entry_id:  91112639
  entry_writer: ucazugasibere
  entry_fav_count: 38
  entry_content: |
    lazlar, kürtlerle kıyaslanmayacak derecede az nüfusu olan bir halktır. yerleşik olarak sadece artvin ve rize'nin sahil kesimlerinde yaşarlar. nüfusları toplasan 300 bin etmez.<br/><br/>farklar da say say bitmez, genelde kürtlerin deniz görmüşü derler bizim için ama alakamız bile yok.<br/><br/>genel bir kıyaslama yapacak olursak kürtler daha misafirperver ve güleryüzlüdür, ayrıca dine bize göre çok daha fazla bağlılar. biz ise daha eğlenceli ve yaşamayı bilen, aynı zamanda da daha agresif insanlarız. benim gördüğüm budur.<br/><br/>not: kürtlerin yaşadığı yerde görev yapan bir laz.
  entry_channels: siyaset
- entry_name: |
    mevlüt çavuşoğlu'nun kürdistan şehidi saygı duruşu
  entry_id:  91117003
  entry_writer: huseynick
  entry_fav_count: 34
  entry_content: |
    ortamlarda beka dersiniz kim bilecek duruşudur.<br/><a rel="nofollow noopener" class="url" target="_blank" href="https://twitter.com/TTAgrup/status/1138119408386285570" title="https://twitter.com/TTAgrup/status/1138119408386285570">https://twitter.com/…rup/status/1138119408386285570</a>
  entry_channels: siyaset,haber
- entry_name: |
    hoşlanılan kıza sevişelim mi deme yöntemleri
  entry_id:  91108250
  entry_writer: blacktreacle
  entry_fav_count: 31
  entry_content: |
    “sana yemek yapayım mı” demektir, en basit yöntem olarak.
  entry_channels: anket,ilişkiler
- entry_name: |
    justin bieber'in tom cruise'a meydan okuması
  entry_id:  91113891
  entry_writer: bikereladydiyeydi
  entry_fav_count: 30
  entry_content: |
    bildiğim kadarıyla tom cruise karbon fiberden yapılma , <br/><br/>mi serisi için 10 senedir <a class="b" href="/?q=krav+maga">krav maga</a> eğitimi alıyor. <br/><br/>uçak helikopter çim biçme makinası dahil her türlü ehliyeti var.<br/><br/>aletsiz tırmanış, uçağın dışında iniş, burç halifa dan sallanis gibi hobileri var.<br/><br/>yani sanatçı naifligi olmasa yolda görünce kaldırım değiştirilmesi gereken bi adam. <br/><br/>justin bieber bildiğin kanadalı uncle fucker bir ergen niye dovdurmek istiyor la kendini bu bebe ?
  entry_channels: haber,magazin
- entry_name: |
    suriyeliler iç anadolu ve karadeniz'e yerleşsin
  entry_id:  91104128
  entry_writer: emine winehouse
  entry_fav_count: 28
  entry_content: |
    acilen başlatılması gereken kampanya. istanbul, ankara, izmir, gaziantep, şanlıurfa gibi illerimiz suriyeli yükünü kaldıramamakta. ayrıca <a class="b" href="/?q=turistik">turistik</a> bölgelerde başıboş sığınmacıların gezmesi güvenlik sorunu oluşturuyor. suriyeli kardeşlerimiz acilen <a class="b" href="/?q=konya">konya</a>, <a class="b" href="/?q=%c3%a7ank%c4%b1r%c4%b1">çankırı</a>, <a class="b" href="/?q=%c3%a7orum">çorum</a>, <a class="b" href="/?q=yozgat">yozgat</a>, <a class="b" href="/?q=bayburt">bayburt</a>, <a class="b" href="/?q=rize">rize</a>, <a class="b" href="/?q=trabzon">trabzon</a> gibi illerimize yerleştirilse çok daha iyi olur.
  entry_channels: 
- entry_name: |
    süleyman soylu
  entry_id:  91087773
  entry_writer: merkz
  entry_fav_count: 27
  entry_content: |
    böyle çapsız bir içişleri bakanına sahip bir ülkede, kusura bakmayin hiçbirimiz güvende değiliz. belki de tarihin en basit en düz en sığ içişleri bakanıdır. dosta korku, düşmana güven aşılar böyle bakanlar.
  entry_channels: siyaset
- entry_name: |
    yedi bela hüsnü
  entry_id:  91118480
  entry_writer: ted evelyn mosby
  entry_fav_count: 24
  entry_content: |
    "suya attım bir taş <br/>çıkardı bir ses faş faş<br/>birisi kafama vurunca<br/>gözümden geliyor yaş<br/><br/>hüsniye hüsniye<br/>beni döndürdün deliye<br/>istersen sor beni veliye<br/>diyecek ki hüsnü 12'de binecek gemiye"<br/><br/>-koşarkende seven hüsnü
  entry_channels: sinema
- entry_name: |
    türkiye'nin geri kalma nedenleri
  entry_id:  91096702
  entry_writer: al bundy vs el kaide
  entry_fav_count: 22
  entry_content: |
    din-milliyetcilik ekseninden cikamayan ideolojik siyaset ve bunun sonucu olarak sistematik olarak cagdisi hale getirilmis egitim sistemi ile bagimsizligini kaybetmis adalet sistemi. egitim ve adalet yoksa gerisi hikaye.
  entry_channels: siyaset,anket,bilim,eğitim
- entry_name: |
    corentin siamang
  entry_id:  91109782
  entry_writer: un deux trois
  entry_fav_count: 17
  entry_content: |
    adam diyor ki, “ belçika’ya gidecek uçağı beklerken denk geldi canım bir şaka yapmak istedi sonra bir baktım kendimi cnn türkte buldum. yaptığım bir suç değil ve türk milli takımının gümrükte 3 saat beklemesiyle de bir alakam yok. bir de aynı şakayı belçika milli takımından birine yapsaydım benimle birlikte şakaya katılırlardı”<br/><br/>sakin beyler, biraz sakin...
  entry_channels: spor,haber
- entry_name: |
    10 haziran 2019 bahçeli'nin kürdistan tepkisi
  entry_id:  91114672
  entry_writer: argope8ainw
  entry_fav_count: 10
  entry_content: |
    <a class="b" href="/?q=al%c4%b1nt%c4%b1">alıntı</a>-------<br/><a class="b" href="/?q=d%c3%bcnden+bug%c3%bcne+duru%c5%9fumuz+ayn%c4%b1d%c4%b1r">dünden bugüne duruşumuz aynıdır</a>.<br/><a class="b" href="/?q=al%c4%b1nt%c4%b1">alıntı</a>-------<br/><br/>devlet bahçeli, 10 haziran 2019.
  entry_channels: siyaset,haber
- entry_name: |
    sancak-ı şerif'in ispanyolların eline geçmesi
  entry_id:  91115127
  entry_writer: machete klonu
  entry_fav_count: 6
  entry_content: |
    yahu bu arkadaşın yalan yanlış başlıklarını düzeltmekten bıktık usandık o saçmalamaktan usanmadı. hayır kendi hakkında yazdıklarını okuyunca sempati de duyuyorum ama osmanlı tarihi hakkında kulağına çalınan her saçmalığı kendi nefretiyle birleştirip abartarak buraya salması artık komik bile değil, zavallıca.<br/><br/>tarih boyunca gerçekleşmemiş olaydır. inebahti savaşında kaybedilen sancak kaptan-ı deryanın sancağıdır. orduların sancaklarını kaybetmesi utanç verici ancak sık rastlanan bir durumdur. ali paşa ve leventleri sancağı canları pahasına korumaya çalışmış fakat başarılı olamamışlardır. ruhları şad olsun.<br/><br/>sancak-ı şerif hala topkapı müzesindedir ve ne bu gün ne de muhtemelen o zamanlarda sefere çıkacak durumda değildir.
  entry_channels: siyaset,tarih
- entry_name: |
    sözlük yazarlarının rumuzlarının hikayeleri
  entry_id:  91117514
  entry_writer: kaybolmayan silgi
  entry_fav_count: 5
  entry_content: |
    lisede ve üniversitede silgi kaybetmekten bıkmışımdır ve aklıma süper bir icat gelir.
  entry_channels: anket,ekşi-sözlük
- entry_name: |
    daha yemeğin tadına bakmadan bunu yemem diyen tip
  entry_id:  91116239
  entry_writer: ramenbito
  entry_fav_count: 5
  entry_content: |
    sevmedigi bir sebze meyve veya et varsa haklidir. onyargi baska sey deneyim baska sey.
  entry_channels: yeme-içme
- entry_name: |
    tıp okuyanların çoğunun low tipler olması
  entry_id:  91119852
  entry_writer: gratis
  entry_fav_count: 4
  entry_content: |
    hayatımda gördüğüm en iğrenç başlık. orta ve alt seviye sana ne zarar vermiş ki?çocuk okumuş,çalışmış ve kazanmış. low olur olmaz onun sorunu.tıp okumamama rağmen böyle saçma bir yorum olamaz. adeta eziklik duygusu ile yazılmış veya tıpçı birinden posta yemiş biri böyle yazmış olabilir. rezalet.
  entry_channels: 
- entry_name: |
    ekşi itiraf
  entry_id:  91095409
  entry_writer: istenc
  entry_fav_count: 4
  entry_content: |
    itiraftan ziyade kendini övenlerin yazılarını okuduğumuz başlık. arada bir başka yazarlarca da konunun eleştirildiğini görüyorum; ancak sırf üstte kalsın diye gece saat 12'yi vururken paylaşılan beton gibi yazıların arasından sıyrılıp yeterince dikkat çekemediler gibi hissediyorum.<br/><br/>o kadar iyi bir insanım ki çok çekiyorum...<br/><br/>o kadar dürüstüm ki hep kazık yiyorum...<br/><br/>o kadar muhteşemim ki insanlar beni anlamıyor...<br/><br/>şeklinde vıyvıymış da gıygıymış yazılarınızla içimi bayıyorsunuz sevgili ergenler ve ergen kalmakta ısrarcı olanlar. <br/><br/>sözlük, facebook'a yazamadıklarımızı yazdığımız yer. buraya içinizi dökmenizle ilgili sorunum yok da, sözlüğün en eğlenceli başlığını ağlama duvarına çevirdiniz iyice. her gün en az on tane "ağlak bir kendini övme kompozisyonu" var ki çoğu geçer notu zor alır. bu dandik ötesi yazılarınızı -üç beş fav fazladan almak için- gece 12'yi bekleyip paylaşmanızın eziklik seviyesi ise yürek burkuyor. <br/><br/>üstelik itiraf falan da yok yazılarınızda. bari yalandan da olsa bir itiraf sıkıştırın yazının içine de, içerikle başlık uyumlu olsun. kendi başınıza düşünmek zorunda kalmayın diye amme hizmeti olarak aşağıya örnek bir itiraf bırakıyorum:<br/><br/>geçen gün plajda bir adam gördüm. o gün bu gündür, başka bir şey düşünmem gerekmediği her an herifin götünü dişlediğimi hayal ediyorum.
  entry_channels: anket,ilişkiler,ekşi-sözlük
- entry_name: |
    bir kere gelinen hayatı çocuk yaparak harap etmek
  entry_id:  91104793
  entry_writer: vodkaredbullyadatekilalimon
  entry_fav_count: 3
  entry_content: |
    olm madem yapmak istemiyorsunuz, hazır yapılmışlardan alın bari.
  entry_channels: ilişkiler
- entry_name: |
    bir kediye verilebilecek en güzel isim
  entry_id:  91120132
  entry_writer: siir gibi
  entry_fav_count: 1
  entry_content: |
    (bkz: <a class="b" href="/?q=m%c3%bcnir+bey">münir bey</a>)
  entry_channels: anket
- entry_name: |
    kanser hastasına yardım kampanyası
  entry_id:  91119515
  entry_writer: sahip cikilan dede
  entry_fav_count: 0
  entry_content: |
    up lütfen.
  entry_channels: sağlık,ekonomi
- entry_name: |
    ismail küçükkaya'nın moderatörlük şartları
  entry_id:  91117710
  entry_writer: contik
  entry_fav_count: 0
  entry_content: |
    muhalifler hep linç edecek adam arıyor, ismimi önceden onaylasınlar baştan ona göre demiş.
  entry_channels: siyaset,tv,haber,ekonomi
---
