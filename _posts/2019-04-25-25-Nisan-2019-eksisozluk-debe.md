---
layout: post
title: 25 Nisan 2019 Ekşi Sözlük Debe
data:
- entry_name: |
    ermenilerden özür diliyorum
  entry_id:  89289278
  entry_writer: ebmoldcity
  entry_fav_count: 2641
  entry_content: |
    şahsen bir şey yapmadığım için ben özür dilemiyorum. benim atalarım da özür dilenecek herhangi bir şey yapmadıkları için yine özür dilemiyorum. hoş, yapsalardı da dilemezdim çünkü ben atalarımdan sorumlu değilim. <br/><br/>ayrıca ermenilerden de bu sikimsonik iftiraları için ben özür bekliyorum.<br/><br/>ha bir de ermenilerden özür dileyen türklere de götümle gülüyorum.<br/><br/>edit: konuya ilişkin hukuki analizim: (bkz: <a class="b" href="/entry/86073963">#86073963</a>)
  entry_channels: anket
- entry_name: |
    osman sarıgün'ün ellerinin öpüldüğü fotoğraf
  entry_id:  89296684
  entry_writer: 41 karakterli nick isteyen adam
  entry_fav_count: 755
  entry_content: |
    organize cehaletin bir ulkeyi nasil cokerttigini gosteren fotograftir.<br/><br/>birakin ayni ulkenin vatandasi olmayi, ayni biyolojik turden olmaya dahi utaniyorum bunlari gordugumde.
  entry_channels: haber
- entry_name: |
    emine erdoğan'ın tıp kongresinde konuşması
  entry_id:  89301801
  entry_writer: sonadora
  entry_fav_count: 400
  entry_content: |
    kronik hastalıklar hakkında ne anlatması beklenerek çağrıldı acaba?<br/><br/>en fazla, <a class="b" href="/?q=kayn%c4%b1mda+da+var">kaynımda da var</a> falan diyebilir herhalde.<br/><br/>edit: "geleneksel tıp kongresi" dedikleri, geleneksel olarak düzenlenen kongre değil, "geleneksel tıp<sup class="ab"><a title="(bkz: alternatif tıp)" href="/?q=alternatif+t%c4%b1p" data-query="alternatif tıp">*</a></sup> " kongresiymiş. hacamat, sülük, deve sidiği... falan yani.
  entry_channels: siyaset,bilim,sağlık
- entry_name: |
    34 bry 447
  entry_id:  89303985
  entry_writer: caneraydin0608
  entry_fav_count: 294
  entry_content: |
    yapmayın olum yazıktır. adamın tek suçu arabayı sakin bir yere çekmemesi.
  entry_channels: ilişkiler
- entry_name: |
    24 nisan 2019 ibb uyuşturucu komisyonu reddi
  entry_id:  89303750
  entry_writer: carbonblue
  entry_fav_count: 258
  entry_content: |
    üst edit: <a rel="nofollow noopener" class="url" target="_blank" href="https://www.youtube.com/watch?v=Z9UrwUJ-18g&amp;t=53m55s" title="https://www.youtube.com/watch?v=Z9UrwUJ-18g&amp;t=53m55s">video linki</a><br/><br/>aslında başlığın tam hali "24 nisan 2019 ibb bm'de akp ve mhp oylarıyla uyuşturucu ile mücadele komisyonu ve toplumsal cinsiyet eşitliği komisyonu kurulmasının reddedilmesi" olacaktı ancak malum karakter sınırı. <br/><br/>kaynak soranlar olacaktır. belediye meclisi canlı yayınlarından her hangi birini izlemeniz yeterli. zaten yakında haber sitelerine de düşer. <br/><br/>bu canlı yayınlar sayesinde kimin neye destek olup olmadığını, kimlerin milletin yararına önerilere sırf partizanlık uğruna karşı çıktığını da görmüş oluyoruz.<br/><br/>edit: kaynak eklendi ve reddedilen komisyonun adı düzeltildi<br/><br/><a rel="nofollow noopener" class="url" target="_blank" href="http://www.cumhuriyet.com.tr/haber/turkiye/1361051/Uyusturucu_ile_mucadele_komisyonuna_AKP_oylariyla_ret.html" title="http://www.cumhuriyet.com.tr/haber/turkiye/1361051/Uyusturucu_ile_mucadele_komisyonuna_AKP_oylariyla_ret.html">kaynak</a>
  entry_channels: siyaset,sağlık,haber
- entry_name: |
    mahalle arasında oral seks yapmak
  entry_id:  89300660
  entry_writer: gizli sinefil
  entry_fav_count: 232
  entry_content: |
    (bkz: <a class="b" href="/?q=yerli+ve+milli+sakso">yerli ve milli sakso</a>)
  entry_channels: haber
- entry_name: |
    elimi salladım kılıçdaroğlu'na değmiş
  entry_id:  89304521
  entry_writer: alpican6161
  entry_fav_count: 230
  entry_content: |
    zaytung absürdlüğünün yanında hiç kaldığı bir ifade örneği.
  entry_channels: siyaset,haber
- entry_name: |
    o yumruk rte'ye atılsa idi ne olurdu sorunsalı
  entry_id:  89283728
  entry_writer: buralar eskiden bag bahceydi
  entry_fav_count: 214
  entry_content: |
    milli iradeye yumruk denirdi<br/>erdogan’i yedirmeyiz denirdi<br/>bu milletimize yapilmis bir darbe girisimi denirdi<br/>halk sokaklara cikardi<br/>yumrugu atan muhtemelen orada linc edilip oldurulurdu<br/>bu bir protestodur diyen hapse atilirdi <br/>sevgili vatandaslar tepkinizi gosterdiniz denmezdi mesela<br/>ya da ne isi varmis orda da denmezdi. <br/><br/>yani kisaca su an kilicdaroglu icin soylenen yapilan ne varsa yuzde yuz tersi yapilirdi. <br/><br/>oyle de adaletli bir ulkeyiz yersen.
  entry_channels: siyaset
- entry_name: |
    metrobüs sapığının imamoğlu ile fotoğrafı
  entry_id:  89305261
  entry_writer: gizli sinefil
  entry_fav_count: 200
  entry_content: |
    (bkz: <a class="b" href="/?q=aktroll+ba%c5%9fl%c4%b1klar%c4%b1na+yazm%c4%b1yoruz+kampanyas%c4%b1">aktroll başlıklarına yazmıyoruz kampanyası</a>)
  entry_channels: siyaset,haber
- entry_name: |
    24 nisan 2019 ibb bm toplantısı
  entry_id:  89300331
  entry_writer: gentoure
  entry_fav_count: 188
  entry_content: |
    baştan söyliyeyim büyükşehir belediye meclislerinin yetkisi sadece imar vs için geçerlidir. yani iski'nin başına genel müdür atama, ispark'ın genel müdürünü değiştirme işi tamamen büyükşehir belediye başkanı sorumluluğundadır. ayrıca belediye meclis toplantıları her ayın ilk haftasının salı günü toplanır. şu anki toplantıların sık olma sebebi bazı kurulların seçilmesi nedeniyle. rutine döndüğünde ayda 1 kez toplanır. bu kurul genelde imar amaçlı olan kuruldur. misal veriyorum kağıthane'de bir ana caddede yapılacak bir işlem için kararı büyük şehir meclisi alır. oy çokluğu vs gibi. kısaca büyükşehir belediye başkanı yetkileri inanılmaz fazladır. hiç bir şekilde o meclisin başkanın işine karışma ya da yavaşlatma durumu yok. özellikle belediye iştiraklerinde ki genel müdür tasarrufu tamamen büyükşehir belediye başkanı yetkileri dahilindedir.
  entry_channels: siyaset,haber
- entry_name: |
    23 nisan 2019 metrobüste yaşanan taciz
  entry_id:  89283903
  entry_writer: kaledeki kurban
  entry_fav_count: 185
  entry_content: |
    küçükçekmecede 4 yaşında çocuğa tecavüz ediliyor,metrobüste kadının üstüne boşalınıyor. hafta başında ülkenin en çok oy alan partilerinden birinin lideri linç edilmek isteniyor kadının biri sığındığı evin yakılmasını istiyor ve adama yumruk atan kişinin suçsuz olduğunu söyleyen bir hukuk sistemi var.<br/>sabah da kız diyor ki "ben alman vatandaşı olmak istiyorum ."
  entry_channels: haber,seyahat
- entry_name: |
    24 nisan 2019 hdp'nin 24 nisan 1915 paylaşımı
  entry_id:  89304462
  entry_writer: octave charcot
  entry_fav_count: 144
  entry_content: |
    hdp'lilere hakaret olarak "ermeni" diyen yazarların, türk atalarının asla ermenilere sistematik bir düşmanlık besleyip, bu minvalde harekete geçmiş olamayacağını iddia ederek sövdüğü paylaşım. <br/><br/>sevgili arkadaşlar, az tutarlı olun. birinci dünya savaşında yönetici konumunda bulunan bir grup adamı "atamız/ceddimiz" ortak parantezine alıp, onların asla böyle ırkçı/kötücül şeyler yapmayacak hoşgörü timsali adamlar olduğunu iddia ederken, aynı anda insanlara "ermenisiniz işte" diye hakaret edilmez. <br/><br/>ya "benim atam/ceddim öyle şey yapmaz, bugün de ermeniler kardeşimizdir, bu soykırım tüccarlığı bizim aramıza nifak sokmaktan başka işe yaramaz" deyin<br/><br/>ya da "soykırım bence gerekliydi, iyi ki yapmış atalarımız, bugün olsa yine yapılsın isterim. gidin ötede ağlayın pis ermeni işbirlikçileri" diye meramınızı daha dürüstçe anlatın.<br/><br/>he üçüncü bir yol da var, o bir grup yöneticiyi "atamız/ceddimiz" diye sahiplenmek zorunda değilsiniz.
  entry_channels: siyaset,haber
- entry_name: |
    bella hadid
  entry_id:  89291984
  entry_writer: excelsior was here
  entry_fav_count: 136
  entry_content: |
    sizin ben zevkinize sokayım.<br/><br/>tanım: ilk defa görüp travesti sandığım kadın.
  entry_channels: moda,magazin
- entry_name: |
    playstation 5
  entry_id:  89305492
  entry_writer: ghost writerr
  entry_fav_count: 131
  entry_content: |
    bunlar sony konsollarının çıkış tarihleridir:<br/><br/><a class="b" href="/?q=playstation">playstation</a> = 3 aralık 1994;<br/><br/><a class="b" href="/?q=playstation+2">playstation 2</a> = 4 mart 2000 = ps1'den 1.918 gün (~ 5 yıl);<br/><br/><a class="b" href="/?q=playstation+3">playstation 3</a> = 11 kasım 2006 = ps2'den 2,443 gün (~ 7 yıl);<br/><br/><a class="b" href="/?q=playstation+4">playstation 4</a> = 15 kasım 2013 = ps3'ten 2,561 gün (~ 7 yıl);<br/><br/>konsollar arasındaki ortalama zaman 2.307 gündür. buna dayanarak, sonraki konsollar.<br/>isimler de değişmezse, aşağıdaki tarihlerde ortaya çıkacaktır:<br/><br/><a class="b" href="/?q=playstation+5">playstation 5</a> = 10 mart 2020'de piyasaya sürülecek;<br/><br/><a class="b" href="/?q=playstation+6">playstation 6</a> = 4 temmuz 2026'da;<br/><br/><a class="b" href="/?q=playstation+7">playstation 7</a> = 28 ekim, 2032
  entry_channels: oyun,teknoloji,spoiler
- entry_name: |
    game of thrones
  entry_id:  89285598
  entry_writer: pis sacli alsancak insani
  entry_fav_count: 110
  entry_content: |
    merhaba<br/><br/>--- <a class="b" href="/?q=spoiler">spoiler</a> ---<br/>7 sezon boyunca diyar diyar gezmiş, yeri gelince erkek kılığına girmiş, yeri geldiğinde yüzsüz adamlara karışmış, yeri gelmiş kör olmuş, her gece uyumadan önce ölüm listesini sayıklamış arya'yı;<br/>ölüler ordusunun kapıya dayandığı gece, anadolu'dan istanbul'a okumaya gelmiş üniversiteli kız gibi "kaç kadınla yattın 3 mü 5 mi 10 mu? hıh? benimle de yatmanı istiyorum" diye konuşturan senaristin kalemine sokayım. <br/><br/>güzelim diziyi <a class="b" href="/?q=ay+yap%c4%b1m">ay yapım</a>'a mı devretmişler lan? <br/>ejderha tepesinde kır bayır dolaşıp, şelale önünde yaşlanma hayali kurup cilvelenen iki salak, 7 krallığı yönetecek neredeyse. <br/><br/>şu krallığın başında olmayı hak eden tek adam <a class="b" href="/?q=tywin+lannister">tywin lannister</a>'dı lan. <a class="b" href="/?q=lord+baelish">lord baelish</a> de kral eli olurdu. alemin gördüğü en kral orospu <a class="b" href="/?q=ros">ros</a> da tywin abimizle evlenirdi. mis gibi yaşar giderdik. <br/><br/>şimdi, ortaçağ'da 7 krallığı boşver, 2019 yılında istanbul'da yaşasa 3 gün sonra telefon dolandırıcılarına mal varlığını kaybedecek sümsüklükte olan <a class="b" href="/?q=jon+snow">jon snow</a>'la, <br/>"görümcem beni pek sevmedi galiba:///" diye gezen <a class="b" href="/?q=daenerys+targaryen">daenerys targaryen</a>'in mutlu sonunu bekliyoruz. <br/><br/>inşallah <a class="b" href="/?q=night+king">night king</a> hepsini siker atar. <br/>sonra da, hiçbir badirede yara almamış olan <a class="b" href="/?q=samwell+tarly">samwell tarly</a>, bu badirede de yara almamayı başarıp demir tahta oturur. <br/><br/>--- <a class="b" href="/?q=spoiler">spoiler</a> --- <br/><br/>hoşçakalın.
  entry_channels: tv,sanat,tarih,edebiyat,spoiler
- entry_name: |
    imamoğlu'nun şoförünün mi6'e flash disk vermesi
  entry_id:  89311949
  entry_writer: ibn meymun
  entry_fav_count: 108
  entry_content: |
    savcılardan ziyade psikiyatrların acilen olaya el atması lazım.
  entry_channels: siyaset
- entry_name: |
    evinize suriyeli alır mısınız sosyal deneyi
  entry_id:  89309897
  entry_writer: ideoloji karmasasi yasayan cocuk
  entry_fav_count: 101
  entry_content: |
    (bkz: <a class="b" href="/?q=ahahahahahaha">ahahahahahaha</a>)<br/><br/>dansöz gibi kıvıran rabiacı, ensarcı, siyasal islamcı çomarlarla dolu, yıldızlar geçidi gibi video. <br/><br/>bu arada ilk konuşan dayı din kültürü öğretmenidir. antalya <a class="b" href="/?q=h.+avni+%c3%a7%c3%b6ll%c3%bc">h. avni çöllü</a> ilköğretim okulunda iki yıl din dersime girmiştir. o günlerde de böyle kıvrak bir tipti. hiç değişmemiş. <br/><br/>(bkz: <a class="b" href="/?q=ahahahahahaha">ahahahahahaha</a>)
  entry_channels: siyaset,anket
- entry_name: |
    erzurumda keyif veren ucuz et kuyruğu
  entry_id:  89312310
  entry_writer: viktornoworsky
  entry_fav_count: 101
  entry_content: |
    aslında arka arkaya duruyorlar. ete para vermeseler de olur.
  entry_channels: haber
- entry_name: |
    hatay'ın hak ettiği değeri görmediği gerçeği
  entry_id:  89305510
  entry_writer: pendeltag
  entry_fav_count: 98
  entry_content: |
    hatay devleti olarak kalsaydı 1944'te yeni bağımsızlığını ilan etmiş suriye tarafından ilhak edilip bugün idlib'le beraber htş kontrolünde olurdu.
  entry_channels: 
- entry_name: |
    taciz ve tecavüzün çözümü
  entry_id:  89301750
  entry_writer: bonecrusher
  entry_fav_count: 92
  entry_content: |
    hadım idam felan diyenler bir boktan anlamıyorlar, istedikleri gibi kusura bakabilirler.<br/>tek bir çözüm var:<br/>(bkz: <a class="b" href="/?q=hukuk+devleti">hukuk devleti</a>)<br/>ama gerçekten <a class="b" href="/?q=hukuk+devleti">hukuk devleti</a><br/>öyle tırışkadan adalet felan demiyorum, harbi harbi <a class="b" href="/?q=hukuk+devleti">hukuk devleti</a>
  entry_channels: anket,ilişkiler
- entry_name: |
    yozgatlı replikleri
  entry_id:  89309237
  entry_writer: uff snne be slk
  entry_fav_count: 91
  entry_content: |
    ekmağınan ye.
  entry_channels: 
- entry_name: |
    sokak köpeklerinin çoğunun aşırı mutsuz gözükmesi
  entry_id:  89298343
  entry_writer: tinkerminkerbell
  entry_fav_count: 85
  entry_content: |
    mutsuz ama cok gururlular.bir de cok zekiler.<br/>gecen sene bir sokak kopegi daha sahiplendim.engelli.3 ayagi var.<br/>3 aylikken araba carpmis, vuran kisi onu oylece cope birakmis.gec kalindigi icin ayagini kurtaramamislar.<br/>tabi hem sokak kopegi olup hem engelli olunca sansi cok daha az oluyor.<br/>ben onu bir kere gordum kocaman kulaklarina asik oldum.<br/>eve getirdim diger iki kopegimin yanina koydum.<br/>biri daha sokaktan digeri de cane corso.<br/>o kadar sapsaldi ki cane corso.bahceyi evi koruyacak goya ama hicbiseyden haberi yok.<br/>tripod (engelli olan) bizimkine havlamayi, yasadigi yeri, bahceyi, evi korumayi, akilli olmayi ogretti.<br/>ve ben ona hicbisey ogretmedim.<br/>onu sadece aldim bahceye koydum.yemegini verdim, oynadim.<br/>ve tabii cok sevdim.<br/>benim onu sahiplendigim gibi o da bizi sahiplendi.<br/>digerlerine de ogretti.<br/>o yuzden imkaniniz varsa sahiplenin yoksa onlari sevin.<br/>onlar sizi en iyi anlayanlar.<br/><br/>edit: cok istek geldi o zaman buyrun<br/><br/><a rel="nofollow noopener" class="url" target="_blank" href="https://eksiup.com/d1f06d795294">https://eksiup.com/d1f06d795294</a><br/><br/><a rel="nofollow noopener" class="url" target="_blank" href="https://eksiup.com/ccf33caf6287">https://eksiup.com/ccf33caf6287</a>
  entry_channels: ilişkiler
- entry_name: |
    hiç taciz edilmeyen türk kadınının olmaması
  entry_id:  89289622
  entry_writer: freudiana
  entry_fav_count: 81
  entry_content: |
    30 yaşımda ankara’nın doğusuna geçene kadar bendim bu. bitlis’te çarşının orta yerinde bir primat iftara 5 dk kala herkesin gözü önünde üzerime yürüyüp mememi sıkana kadar.<br/><br/>edit: mesajlara toplu bir cevap olsun. hayır bu kişi çok fena dayak yemedi çünkü orası <a class="b" href="/?q=izmir">izmir</a> değil <a class="b" href="/?q=bitlis">bitlis</a>. kolundan yakalayıp çevreden polis çağırmalarını istedim, kimse çağırmadı. elimden bir şekilde kurtulan adam da kaçtı. primat sürüsü hızlı hızlı dükkanlarına evlerine kaçıştı, görmezden geldiler. kendi çabalarımla polisi zorlayarak yakalanmasını ve ceza almasını sağladım.<br/><br/>taciz o kadar etkilemedi beni açık söylemek gerekirse. travmatize olmuş, tiksinmiş, kirlenmiş falan hissetmiyorum. sığırın birinin yaptığı bir hareket, benim üzülecek bir şeyim yok. cezasını da aldı. beni üzen şey küçük bir şehirde 1 seneden fazla anasına, babasına, bacısına maddi-manevi çok yardımım olan insanların bu olaya seyirci kalması oldu. fiziksel bir yardım da istememiştim sadece iki elimle adamı tuttuğum için polisi arayamamıştım. bu olayı izmir’de yaşasam ilgili sığır sağlam bir meydan dayağı yerdi, ben de unutup gitmiş olurdum herhalde.<br/><br/>bu olay siyasi görüşlerime kadar birçok şeyi değiştirdi hayatımda. o güne kadar çok sevdiğim, her ortamda savunduğum “benim anadolulu insanım”dan, özellikle doğulu olanlarından nefret ediyorum elimde değil. lafa gelince mangalda kül bırakmayan, ahlaklı namuslu geçinen, yüzünüze gülümseyen bu primatların gerçek yüzünü tecrübe ettim. bir daha asla taşraya gitmedim, tatil için bile olsa gitmeyeceğim. “halka inelim, empati yapalım, koyun sürüsü demeyelim” gibi düşüncelere o günden beri katılmıyorum. bazı insanlar, bazı toplumlar kötü kalpi ve kötülük tedavi edilebilir bir şey değil.
  entry_channels: 
- entry_name: |
    istanbullu kız vs izmirli kız vs ankaralı kız
  entry_id:  89285781
  entry_writer: x uzeri y nin z ye gore turevi
  entry_fav_count: 79
  entry_content: |
    üçü de aynı partiye mensup belediye başkanlarından hizmet almaktadır &lt;3
  entry_channels: anket,ilişkiler
- entry_name: |
    osman sarıgün'ün kronik hırsız çıkması
  entry_id:  89311399
  entry_writer: errorla mucadele timi burton
  entry_fav_count: 77
  entry_content: |
    inek hırsızlığından ve son işindende hırsızlıktan dolayı atıldığı iddia edilen durum. <br/><br/>halbuki tip itibari ile at hırsızına daha çok benzemekte kendileri.
  entry_channels: haber
- entry_name: |
    sünnetli penisin daha güzel olması
  entry_id:  89307333
  entry_writer: harika avci
  entry_fav_count: 75
  entry_content: |
    erken boşalma ve alınan hazzın düşmesine sebep olur sünnet. normalde nemli ve korunaklı, 36.5 derecede kalması gereken penis başı, kuru ve sürtünmeye maruz kalarak sıcaklığı da 27 dereceye kadar düşer. bu da hissizliğe ve aynı zamanda cinsel birleşme sırasında ani ısı değişimiyle erken boşalmaya sebep olur.<br/><br/>estetik olabilir, sağlıklı? hiç değil.
  entry_channels: anket
- entry_name: |
    metrobüs tacizcisinin chp'li çıkması
  entry_id:  89298297
  entry_writer: satilik
  entry_fav_count: 69
  entry_content: |
    chp'li olabilir. <br/><br/>mesele bu şerefsize parti ve partililer sahip çıkıyor mu çıkmıyor mu?<br/><br/>ensarcı yavşaklara sahip çıkan tayfa ile karıştırmayalım.
  entry_channels: siyaset,tv,haber
- entry_name: |
    avengers endgame
  entry_id:  89288104
  entry_writer: the prince of heaven
  entry_fav_count: 67
  entry_content: |
    avrupa'da gece itibariyle önvizyona giren, sabah ise ilk seanslarıyla sinema perdesine giriş yapacak 22 filmlik <a class="b" href="/?q=the+infinity+saga">the infinity saga</a> kapanış filmi. türkiye için artık son 24 saat diyebiliriz. gitmeden önce bilinmesi gerekenleri özetledim. sıralamayı da tekrar paylaşayım;<br/>(bkz: <a class="b" href="/entry/80264742">#80264742</a>) 21 filmin özeti: (bkz: <a class="b" href="/entry/80438666">#80438666</a>) <br/><br/><a rel="nofollow noopener" class="url" target="_blank" href="https://ibb.co/qdc6nvt" title="https://ibb.co/qdc6nvt">sonsuzluk taşları</a>ndan başlayalım;<br/><br/><a class="b" href="/?q=tesseract">tesseract</a> – <a class="b" href="/?q=uzay+ta%c5%9f%c4%b1">uzay taşı</a>:<br/><br/>ilk olarak thor filminin son sahnesinde karşımıza çıkan bu sonsuzluk taşı, küp şeklinde bir görünüme sahip bir taşıma kabının içerisindedir ve parlak mavi renktedir. doğru kullanıldığında taşı kullanan kişi evrenin istediği noktasında kendisini bulabilir. asırlar boyu asgard gezegeninde saklı tutulan bu sonsuzluk taşını kral odin, dünya’ya getirmiştir. <br/><br/>captain america: first avenger filminde işlendiği üzere, ikinci dünya savaşı döneminde bu efsanelerin izini süren nazi özel silahlar departmanı olarak da bilinen hydra kumandanı johann schmidt norveç’te tønsberg kasabasında taşın yerini keşfeder ve onu yerinden çıkartır. taşı alp dağlarındaki hdyra araştırma üssüne götüren kumandan orada bilim insanı arnim zola ile beraber taş üzerinde deneyler yapar ve ikisi taşın içindeki enerjiyi silahlara aktarmanın bir yolunu bulurlar. nazi lideri hitler’e de ihanet eden johann schmidt, yeni ismiyle red skull (kızıl kafataşı) dünyayı işgal etme planını hayata geçirir. plan kısa sürede captain america tarafından durdurulur ve taşın gerçek gücünü bilmeyen red skull kendisine uzayın başka bir noktasına geçit açar, ardından yanlışlıkla geçidin içerisine çekilir. okyanusun diplerine düşen sonsuzluk taşını bulan kişi ise stark endüstrinin ceo’su ve tony stark'ın yani iron man'ın babası howard stark olacaktır.<br/><br/>avengers filminde thor’un yenilgiye uğrattığı üvey kardeşi loki, kendisine müttefik ararken dünyada bir sonsuzluk taşı olduğunu öğrenen ve o taşı gerçek kimliğini ortaya çıkartmadan ele geçirmek isteyen thanos ile karşılaşır. dünyayı işgal etmesi için thanos, loki’ye ona itaat edecek olan chitauri ırkından oluşan ordusunu verir. ayrıca beyin yıkama gücü olan scelpter adı verdiği güçlü bir asayı da hediye eder.<br/><br/>elindeki orduya ve asasının gücüne rağmen shıeld lideri nck fury’nin ikna çabasıyla kurulan ıron man, thor, captain america, hulk, hawkeye ve black widow’un yer aldığı avengers ekibi new york’ta loki’yi alt eder. thor, üvey kardeşini asgard’a geri götürür ve tesseract güvende olur lakin thor üçlemesinin son filmde asgard gemisinde thanos saldırısı sonrası tessaracti infinity war'da gördüğünüz üzere kaybederler. <br/><br/><a class="b" href="/?q=scepter">scepter</a> – <a class="b" href="/?q=bilin%c3%a7+ta%c5%9f%c4%b1">bilinç taşı</a>:<br/><br/>organizasyon içerisine zamanında sızmış olan hydra üyelerinin darbesi sonucunda shıeld parçalanınca bu sonsuzluk taşı da bilim insanı wolfgang von strucker’ın eline geçer. sokovia’da bir araştırma üssüne götürülen taş üzerinde araştırmalar yapan hydra bilim insanları insanlar üzerinde deneyler yaparlar. denek olarak da sokovia’da yaşayan insanları kullanırlar. gönüllerin neredeyse hepsi ölür, ama geriye kalan maximoff ikizleri sıra dışı güçler kazanır. pietro, aşırı hıza sahip bir bedene sahip olurken kız kardeşi wanda çevresini manipüle edebilecek güçte zihinsel beceriler edinir.<br/><br/>avengers: age of ultron filmine geldiğimizde ekip, hydra’ya karşı yapılan mücadeleyi uzun uğraşlar sonucunda kazanır. ekibin kulesine götürülen gizemli asa üzerinde tony stark, yani ıron man çalışmaya başlar. thor, asayı güvenliği sebebiyle asgard’a götürmeden önce asanın gizemlerini çözmek isteyen ıron man, hulk’ın bilim insanı kimliği olan bruce banner’ı da ikna eder. ikisi birlikte asanın içerisinde saklı bir yazılım keşfederler ve bu programı ultron adını verdikleri bir yapay zekaya bağlarlar. insanlığı uzaydan gelecek tehlikelere karşı koruması amaçlanan ultron, asıl tehlikenin insanlığın ta kendisi olduğunu düşünür ve ıron man’in demir lejyon’una ait olan robotlarından kendisine bir beden meydana getirir. ultron, asayı kırar ve ona bilinç kazandıran sonsuzluk taşıyla kendisine kusursuz bir beden arayışına çıkar. sadece afrika’nın wakanda ülkesinde bulunan ve kırılmaz bir metal olan vibranium’dan bol miktarda ele geçiren ultron, sonsuzluk taşını ve vibranium metalini kullanarak vision’ı meydana getirir. ama tony stark’ın yapay zekâ programı jarvıs’in de izlerini taşıyan vision, avengers ekibinin yanında yer alır ve ultron’u alt ederler.<br/><br/><a class="b" href="/?q=aether">aether</a> – <a class="b" href="/?q=ger%c3%a7eklik+ta%c5%9f%c4%b1">gerçeklik taşı</a>:<br/><br/>koyu kızıl bir sıvı içerisinde saklı bir şekilde karşımıza çıkan bu sonsuzluk taşı, simbiyotik bir özelliğe sahiptir. gücünü kullanmak isteyen kişiyle sonsuzluk taşı birleşir ve taş, kişiye gerçekliği değiştirme kudreti verir. bu güçle birlikte kişi evreni meydana getiren tüm fizik kurallarına karşı koyabilir. beş bin sene sonrasında thor’un dünyalı sevgilisi jane foster, aether’in saklı tutulduğu bölgeye açılan bir geçit keşfeder ve merakına yenik düşerek aether ile temas kurar. sıvı halindeki aether, jane’in bedenine yerleşir ve sonsuzluk taşı onunla birleşir. thor, odin’in emriyle dünyaya döner ve jane’i asgard’a getirir. malekith, aether’in aktif hale geldiğini hissederek beş bin senenin ardından uykusundan uyanır. asgard’a saldıran malekith, thor’un annesi frigga’nın fedakarlığı ile aether’i ele geçiremeden ordusuyla ayrılmak zorunda kalır.<br/><br/>asgard’tan ayrılması yasak olan thor’a, aether’in etkisinden dolayı ölmek üzere olan jane’i kurtarmak için asgard’daki hücresinden çıkarttığı üvey kardeşi loki yardım eder. loki, thor’u karanlık elflerin diyarı olan svartalfhem’e gizli yollardan ulaştırır. malekith ile tekrardan karşılaştıklarında karanlık elf, jane’in bedeninden aether’i çekip alır ve onu kendi bedeniyle birleştirir. thor, yeni bir convergence anında aether’in gücüne başvurmadan önce malekith’i dünyada yenmeyi başarır. asgardlılar, aether’i uzayın derinliklerinde bir madenci kolonisi olan knowhere’de büyük bir koleksiyonu ve müzesi olan taneleer tivan’a emanet ederler.<br/><br/><a class="b" href="/?q=eye+of+agamotto">eye of agamotto</a> – <a class="b" href="/?q=zaman+ta%c5%9f%c4%b1">zaman taşı</a>:<br/><br/>evren’in haricinde başka boyutların olduğunu keşfeden ve o boyutlardan elde edilen enerjiyle büyü dediğimiz kavramı ortaya çıkartan agamotto, dünyada mistik sanatların üstatları’nı kurar ve diğer insanları da büyü konusunda eğitmeye başlar. karanlık boyut başta olmak üzere dünyaya büyük tehdit oluşturabilecek boyutlara karşı da agamotto dünyanın üç farklı noktasına kalkan görevi görecek sığınak yapar. büyük bilgi birikimi ve deneyimleri sonucunda bir sonsuzluk taşıyla karşılaşır ve onu eye of agamotto adı verdiği bir yadigarın içerisinde saklı tutar.<br/><br/>doctor strange filminde, mistik sanatların yeni öğrencisi dr. stephen vincent strange bu yadigarı eğitim gördüğü kamar-taj’da keşfeder ve onu kullanmayı öğrenir. zamanı manipüle etme yeteneği sunduğu için doğanın dengesini bozacağına inandıklarından büyücü arkadaşları, strange’e bu yadigarın gücüne başvurmaması konusunda tembih ederler. ama agamotto’nun sığınaklarından birinin yer aldığı hong kong’ta strange, bizim evrene geçmek isteyen karanlık boyut’un hükümdarı dormammu’yu alt etmek için yadigarı kullanır. dormammu, kendi boyutuna geri döndükten sonra strange yadigarı kamar-taj’a geri bırakır ve içerisinde bir sonsuzluk taşının saklı tutulduğunu öğrenir.<br/><br/><a class="b" href="/?q=orb">orb</a> - <a class="b" href="/?q=g%c3%bc%c3%a7+ta%c5%9f%c4%b1">güç taşı</a>:<br/><br/>taşın özellikleri: bu sonsuzluk taşı sahibine bir gezegeni bile yok edebilecek çok büyük güç ve enerjiyi manipüle etme imkanı sağlar. orb’u ele geçirmek isteyen thanos, bu iş için ronan’ı görevlendirdi. morag’da bulunan taşı star-lord almak isterken ronan’ın adamları onu yakalamaya çalıştı ama başarısız oldular. star-lord taşı xandar’da satmak istedi ama reddedildi. ronan taşı alması için gamora’yı gönderdi ama gamora’nın planı taşı koleksiyoncu’ya satmaktı. olaylar sonucunda bir takım olan guardians ekipi taşı koleksiyoncu’ya götürdü. koleksiyoncu’nun yardımcısı carina taşa dokundu ve büyük bir patlamaya sebep oldu. <br/><br/>bu sırada taş için nebula’yı gönderen ronan onun sayesinde taşı ele geçirdi. ronan thanos’ ihanet edip taşı kendi amaçları için kullanmaya karar verdi. xandar’a saldıran ronan’ı guardians ve ravagers durdurmaya çalıştı. savaş sırasında star-lord’un eline geçen taş ile guardians ekibi ronan’ı yok etti. daha sonra orb’u güvende olması için nova prime’a teslim ettiler. nova cops'dan ise thanos ele geçirmiştir. <br/><br/><a class="b" href="/?q=the+soul">the soul</a> - <a class="b" href="/?q=ruh+ta%c5%9f%c4%b1">ruh taşı</a>:<br/><br/>bu taşı kontrol eden kişi ya da güç, varlıkların ruhlarını ele geçirip onları ruh dünyası’na atabiliyor. ruhları hapsedebilme, ele geçirebilmekte. <br/><br/>bu, evrende ortaya çıkan son taş. thanos onu almak için sevdiği kişiyi feda etmek zorunda kalacak. avengers-infinity war filminde stard lord'un duygusal davranmasından ötürü thanos, gamora'yı ele geçirip vormir'de onu uçurumdan atarak ruh taşını koruyan kırmızı kafatasından da son taşı thanos böylece alır.<br/><br/>kullanılan kaynaklar: <a rel="nofollow noopener" class="url" target="_blank" href="https://www.journaldugeek.com/dossier/avengers-endgame-quil-faut-retenir-daller-voir-film/" title="https://www.journaldugeek.com/dossier/avengers-endgame-quil-faut-retenir-daller-voir-film/">1</a> <a rel="nofollow noopener" class="url" target="_blank" href="https://frpnet.net/makaleler/marvel-sinematik-evreni-ve-sonsuzluk-taslari-tarihcesi" title="https://frpnet.net/makaleler/marvel-sinematik-evreni-ve-sonsuzluk-taslari-tarihcesi">2</a> <a rel="nofollow noopener" class="url" target="_blank" href="https://www.rtl.fr/culture/super/avengers-4-les-16-details-dont-il-faut-se-souvenir-avant-de-voir-le-film-7797402013" title="https://www.rtl.fr/culture/super/avengers-4-les-16-details-dont-il-faut-se-souvenir-avant-de-voir-le-film-7797402013">3</a> <a rel="nofollow noopener" class="url" target="_blank" href="http://time.com/5227586/mcu-six-infinity-stones/" title="http://time.com/5227586/mcu-six-infinity-stones/">4</a>
  entry_channels: sinema,spoiler
- entry_name: |
    amcayla teyzenin hala &amp; dayıdan daha çok sevilmesi
  entry_id:  89296685
  entry_writer: indiana jones
  entry_fav_count: 67
  entry_content: |
    amca mı?<br/><br/>türkiye sınırlarında amcayla kanlı bıçaklı olmayan var mıdır? milli derdimizdir <a class="b" href="/?q=amcan%c4%b1n+hakk%c4%b1m%c4%b1z+olan+paray%c4%b1+yemesi">amcanın hakkımız olan parayı yemesi</a>.
  entry_channels: 
- entry_name: |
    24 nisan 2019 mars depremi
  entry_id:  89303635
  entry_writer: kulak burun bagirsak
  entry_fav_count: 59
  entry_content: |
    tebrik ediyorum hepiniz geyik yapın. müthiş komiksiniz. ulan bilimsel başlığı bile çöpe çeviriyorsunuz. adamın birisi geliyor "." olarak entry giriyor. hay geyiğinize ya.<br/><br/>tanım: nasa'nın mars'a gönderdiği insight adlı araştırma robotunun sismolojik sensörleri sayesinde insanoğlunun ilk kez başka bir gezegendeki kaydettiği depremdir.
  entry_channels: bilim,teknoloji
- entry_name: |
    en çok fav alan entry
  entry_id:  89312814
  entry_writer: bilgilibiri
  entry_fav_count: 33
  entry_content: |
    eksi butonuna basarak katıldığım başlık.
  entry_channels: ekşi-sözlük
- entry_name: |
    deli mi ne
  entry_id:  89311060
  entry_writer: excelsior was here
  entry_fav_count: 32
  entry_content: |
    adamın başlığında 2. sayfa yok sözlüğe soru yanıtlamaya geliyor. ulan benim nick altımda bile bundan daha çok entry var. nefret eden edene. geleyim sorularını yanıtlayayım aktrollerin.<br/><br/>neden deli mi ne sözlüğe soru yanıylamaya geliyor?<br/>daha önce yasemin mori'nin sözlüğe soru yanıtlamaya gelmesi için <a rel="nofollow noopener" class="url" target="_blank" href="http://tiny.cc/ynko5y" title="http://tiny.cc/ynko5y">şöyle</a> bir entry girmiştim. ben cevabı biliyordum ama ortaya bıraktım bombayı; saolun o başlığın en iyi entry'si seçilmişti. ,<br/><br/>tekrar soruyorum; bu fasilite üzerinden ekşi sözlük para kazanıyor mu?<br/>kazanıyorsa eğer olay ışığı bildiriminin zorunlu olması ne derece etik?<br/>para kazanılıyorsa insanların <a class="b" href="/?q=olan+biten">olan biten</a> başlığını takip etme zorunluluğunu kaldırmayı düşünüyor musunuz?
  entry_channels: teknoloji
- entry_name: |
    24 nisan 2019 dolar kuru
  entry_id:  89293116
  entry_writer: paolno
  entry_fav_count: 31
  entry_content: |
    çocukların bile gitmek istediği ülkede dolar mı kalacaktı amk?
  entry_channels: ekonomi
- entry_name: |
    araba kullanmaya yeni başlayanlara tavsiyeler
  entry_id:  89300624
  entry_writer: detone olan denizli horozu
  entry_fav_count: 30
  entry_content: |
    çok basit bir mantık var trafikte.<br/>tüm kurallara uyun ve diğer tüm sürücülerin hiç bir trafik kuralına uymayacağını hayal edin, ona göre dikkat edin. örneğin size yeşil yanıyor ya da geçiş üstünlüğü sizde diye kavşağa gaza basarak girmeyin, ayağınızı gazdan çekin ve frene basmaya hazır olun.<br/>diğer bir önemli konu ise takip mesafesi. özellikle şehir içinde tampon tampona gitmeyin. çünkü önünüzdeki primat yolu şaşırır, sigarasını kucağına düşürür ya da tanıdık birini görür selam vermek için aniden frene basabilir.
  entry_channels: anket,otomotiv,eğitim
- entry_name: |
    gürültü ile cehaletin doğru orantılı olması
  entry_id:  89285219
  entry_writer: wakana
  entry_fav_count: 29
  entry_content: |
    “uzun zamandır şuna inanıyorum. insanın dayanabileceği gürültü miktarı ile zihinsel yetileri arasında bir ters orantı vardır. kapıyı eliyle yavaşça kapatmak yerine gürültüyle çarpan bir insan yalnızca terbiyesiz değil; aynı zamanda bayağı ve dar görüşlüdür.”<br/>(bkz: <a class="b" href="/?q=arthur+schopenhauer">arthur schopenhauer</a>)
  entry_channels: 
- entry_name: |
    24 nisan 2019 man. united man. city maçı
  entry_id:  89292572
  entry_writer: gokmer77
  entry_fav_count: 26
  entry_content: |
    maç berabere biter, maç sonunda united teknik direktörü solkjaer gururla " belkide bu beraberlikte city 'yi şampiyonluktan ettik " diyip güler.<br/>al sana vizyon.
  entry_channels: spor
- entry_name: |
    aniden gelen saçı üç numaraya vurma isteği
  entry_id:  89307341
  entry_writer: bahtsizmedeni
  entry_fav_count: 23
  entry_content: |
    bir kadın olarak bana gelen istek. saçlarım omzuma kadar. beynimde dolanıyor bir fısıltı "yaptır" diye. şişmanım ve yüzüme gitmez o üç numara saç, en son ilkokulda öyleydi saçlarım ve kötüydü ama çok yoğun yaşıyorum şu an o duyguyu. depresyonun getirileri. <br/><br/>okuyan olursa kadınlardan fikir belirtmenizi isterim.
  entry_channels: anket
- entry_name: |
    23 nisan 2019 küçükçekmece'de çocuğa tecavüz
  entry_id:  89285911
  entry_writer: nickullah entryoglu
  entry_fav_count: 17
  entry_content: |
    bugün itibariyle ancak girebildim sözlüğe, onu da biraz kafam dağılsın diye oluyayım bir şeyler diye açtım, belki iyi bir şeyler okurum derken denk geldim, “türkiye’desin oğlum uyan.” diyerek çimdikledim kendimi.<br/><br/>daha bugün sabah sularında bir aylık kızım vefat etti. tarifi zor bir acı, durup durup gelen acılar yumağı, evladınızı, hiçbir şeyden haberi olmayan o minik meleğin kefene sarılı bedenini gördüğünüz zaman kaskatı kesiliyorsunuz zaten. allah kimseye vermesin.<br/><br/>şimdi böyle bir acım varken, eşim benden de beter durumda, onu teselli etmeye çalışırken, “halimize şükredelim, yavrumuz hiçbir şeyden habersiz vefat etti, çocukları hasta olanlar var, evlatları öldürülen var, tecavüz edilenler var, daha beterleri var, ki bugün evladımızın yanına da 4 tane daha bebek gelecekti, kimisi belki 10 günlük, kimisi 2 aylık.” dedim, sonra da bu haberi okudum. <br/><br/>yazık günah be kardeşim, böyle bir toplumsal yara olamaz, böyle bir akıl tutulması, terbiyesizlik, ahlaksızlık olamaz. bu olaylar, ahlak terbiye ve eğitim, ilk önce de aileden gelecek terbiye ve tevazuya bağlı olmadığı ve bu bilinçsizlik devam ettiği sürece maalesef ki bitmeyecek. çocuğumuz inşallah kendisini toparlar. bu fiili gerçekleştiren insan(!) müsveddesine diyecek hiçbir söz bulamıyorum, kelimeler kifayetsiz kalıyor, yedi ceddine sövsen, bazı organlarını kökünden kessen ne olacak. kanayan yara belli!
  entry_channels: haber
- entry_name: |
    makine mühendisiyim ama okulda 150 kızla seviştim
  entry_id:  89311251
  entry_writer: excelsior was here
  entry_fav_count: 16
  entry_content: |
    milliyetçi bir kitlenin önderiyim 3 tane erkekle seviştim, yalan yok. önden değil arkadan hem de.<br/><br/>tanım: bir beyan.
  entry_channels: ilişkiler
- entry_name: |
    düşün ki o bunu okuyor
  entry_id:  89284275
  entry_writer: berivangcl
  entry_fav_count: 12
  entry_content: |
    s., aka cäsar, aka mein (altes) bärchen, <br/><br/>seni affetmedim, affetmiyorum ve asla affetmeyeceğim. biliyor musun, iyileşiyorum geçen her gün ile. senden hediye kalan kaburgalarımın altındaki morarma izleri de neredeyse tam iyileşti... sadece ruhum vücuduma göre çok daha yavaş toparlanıyor. mesela aradan onca zaman geçmiş olmasina rağmen, kabuslarımda hâlâ senden kaçıyorum. uykularımdan sıçrayarak - ya da boğazıma tıkanmış sessiz çığlıklarla uyanıyorum. kabuslarım sürecek, biliyorum. hiçbir şey olmamış gibi hayatıma devam etmek kolay değil, edemiyorum. <br/><br/>çünkü çok şey oldu; çünkü ben çok yara aldım. <br/><br/>bana yaptıklarını hak etmedim. hak etmedim. hak etmedim. <br/>bu cümleyi sabahlara kadar tekrarladım uykusuz gecelerde. <br/>neyi farklı yapsaydım acaba öfkene yenik düşmezdin diye çok sordum kendime. <br/>sonra tekrarladım,<br/>"hak etmedin, beri. hak etmedin, hak etmedin". <br/>sonra sustum. konuşamadım uzun bir süre, yatağımdan çıkamadım. soyunamadım, vücudumdaki morarmaları görmeyi kendime yediremediğim için. belki artık konuşmazsam yok olurum umuduyla lâl olmayı seçtim - başaramadım. içimdeki acı doldu taştı, günler boyunca ağladım. <br/><br/>biliyor musun, ben seni gercekten sevmeye hazırdım. hayatındaki tüm diğer insanların aksine sana inanmaya hazırdım. arkanda durmaya, seni desteklemeye, kocaman sevmeye. kollarının arasında uyuduğumda kendimi güvende hissediyordum ben. şimdi ise, seninle tekrar karşı karşıya geleceğimiz günden çok korkuyorum. <br/><br/>yalnız olmayacağım bu sefer. bir tarafımda avukatım, diğer tarafımda ailem olacak. aylar sonra ilk defa tekrar gözlerinin içine bakacağım. ve - biliyorum ki - o an tekrar yaşayacağım her şeyi. ellerin yine boğazımı sararak nefesimi kesecek, attığın tokatlardan dolayı zarar alan kulak zarım durmaksızın çınlayacak. <br/><br/>ama sonra,<br/>mahkemeden sonra uzun bir tatile çıkacağım. <br/>bırakacağım kendimi dalgalara;<br/>bırakacağım kendimi akıntıya;<br/>sürüklesin beni istediği kadar. <br/><br/>sonra yine yeni yeniden yeni bir sayfa açacağım kendime.<br/>yepyeni umutlar, yepyeni gülüşler, yepyeni insanlarla...
  entry_channels: ilişkiler,edebiyat
- entry_name: |
    27 yıl komada kalıp uyanan kadın
  entry_id:  89312268
  entry_writer: mazut
  entry_fav_count: 11
  entry_content: |
    uyanma ihtimali varsa cost effectivedir amk bu z kuşağı neden böyle ya?!
  entry_channels: sağlık,haber
- entry_name: |
    rüyada üniversiteye geri döndüğünü görmek
  entry_id:  89307054
  entry_writer: hayvanhaklari
  entry_fav_count: 8
  entry_content: |
    en büyük kabusum.<br/>bugün yine gördüm.<br/>tee ne zamandır görmüyordum ve kurtulduğumu zannetmiştim ancak ne var ki, bu kabus kolay kolay kurtulunabilecek bir kabus değil.<br/><br/>bu sefer sınavdaydım, yanımda da aynı kampüste farklı fak.de okuyan lise arkadaşım vardı.<br/>bir topar sınav kağıdını veriyorlar öndeyim diye arkaya iletmek için. ben hepsini tutuyorum elimde. normal devasa iki sayfadan farklı, daha ziyade kitapçık gibiydi bu kağıtlar.<br/>her neyse...<br/>sınav başlıyor ama ben başka işlerle uğraşıyorum. yazmıyorum hiçbir şey.<br/>arkadaşıma bakıyorum kağıdı doldurmuş, alta tarih yazıp imza atıyor.<br/>sınav süresi sona mı eriyor diye düşünürken birkaç arkadaşın daha bana baktığını görüyorum niye hiç yanıt yazmadın diye.<br/>panikliyorum son anda elime alıp yazacakken birden aslında mezun olduğumu hatırlıyorum.<br/>hoca yaklaşıyor yada asistan her kimse işte sınav görevlisi. en azından ismimi yazıp vermem bekleniyor, ama boş kağıt vermek istemiyorum, hem zaten mezunum.<br/>hocaya ben aslında mezunum niye buradayım bilmiyorum desem embesil gibi görünürüm diye söyleyemiyorum da, en son ben mezunum, arkadaşımın yanına geldim diyerek gülümsüyorum. hoca da hee, tamam o zaman gibi bir şeyler söyleyip gülümseyip gidiyor. sınavdan çıkınca arkadaşlarıma ben mezunum yahu diyorum, diplomam var, onlar da tabi ki, sen mezunsun diploman var diyorlar.<br/>allahım yarabbbim ya, bıktım bu kabustan.<br/><sup class="ab"><a title="(bkz: ühühühühü)" href="/?q=%c3%bch%c3%bch%c3%bch%c3%bch%c3%bc" data-query="ühühühühü">*</a></sup>
  entry_channels: anket,tarih,eğitim
- entry_name: |
    teneffüslerin 15 dakikaya çıkması
  entry_id:  89310874
  entry_writer: falconvirtue
  entry_fav_count: 7
  entry_content: |
    rte'nin 23 nisan müjdesi<br/>--- <a class="b" href="/?q=spoiler">spoiler</a> ---<br/><br/>benim çocuklar da şikayetçiydi, daha uzun teneffüs istiyormuşsunuz, sayın bakan da burda, önümüzdeki yıldan itibaren 15 dakika olacak.<br/>--- <a class="b" href="/?q=spoiler">spoiler</a> --- <br/><br/>artık eğitimde hiç bir problemimiz kalmadı hamdolsun!<br/>hani proje uygulaması vardı teneffüsler 45 dakika olacaktı? o da mı gol değil be !<br/>(bkz: <a class="b" href="/?q=deneme+tahtas%c4%b1+t%c3%bcrk+e%c4%9fitim+sistemi">deneme tahtası türk eğitim sistemi</a>)
  entry_channels: 
- entry_name: |
    japonya
  entry_id:  89308074
  entry_writer: make the world go away
  entry_fav_count: 4
  entry_content: |
    "deytas'in japonya hakkindaki gorusleri" $eklinde degistirilmesi gereken ba$lik. <br/><br/>deytas'in goruslerini okumak isteyenler de o basligi takip eder.<br/><br/>saka yapmiyorum, ciddiyim.<br/><br/>2.5 bin entry'nin 500'e yakinini deytas girmis.
  entry_channels: siyaset,bilim,tarih,teknoloji,seyahat
- entry_name: |
    survivor 2019 türkiye-yunanistan
  entry_id:  89284073
  entry_writer: beat junkie
  entry_fav_count: 3
  entry_content: |
    yemin ediyorum altın şafak partisi propagandası gibi yarışma. türk düşmanı piçler bakıp "bakın, ne kadar da haklıyız" demiyorlarsa adam değilim.<br/>okay hariç adadaki tüm türkler utanç kaynağı.
  entry_channels: spor,tv
- entry_name: |
    jon snow
  entry_id:  89301228
  entry_writer: jupiterianvibe
  entry_fav_count: 3
  entry_content: |
    sevdicegine trip mrip atmakta olmayan adamdir. lavuk halasini siktigini ogrenmis, hayattan tiksinmis. hareketler normal.<br/><br/>sen 1 yildir catir catir siktigin kizin halan oldugunu ogrensen ne yapardin guzel karsim?
  entry_channels: tv,edebiyat
- entry_name: |
    ekşi sözlük dertleşecek insan veritabanı
  entry_id:  89305905
  entry_writer: getcha pull
  entry_fav_count: 1
  entry_content: |
    bana çağrıştırdığı;<br/><br/>(bkz: <a class="b" href="/?q=ek%c5%9fi+s%c3%b6zl%c3%bck+sertle%c5%9fecek+insan+veri+taban%c4%b1">ekşi sözlük sertleşecek insan veri tabanı</a>)
  entry_channels: ilişkiler,ekşi-sözlük
- entry_name: |
    2019 dünya snooker şampiyonası
  entry_id:  89284967
  entry_writer: ignotus peverell
  entry_fav_count: 1
  entry_content: |
    an itibariyle tarihe geçmiş bir maça şahit olduğumuz turnuva. graem dott, stuart bingham'a karşı 8-1 geriden maçı decidera taşımayı başardı. turnuvada inanılmaz bir ilk tur yaşanıyor. bu yüzden harika bir oyun işte snooker.
  entry_channels: spor
- entry_name: |
    şu anda çalan şarkı
  entry_id:  89293096
  entry_writer: ikitekeryedinota
  entry_fav_count: 0
  entry_content: |
    <a class="b" href="/?q=levent+y%c3%bcksel">levent yüksel</a> - <a class="b" href="/?q=sultan%c4%b1m">sultanım</a>
  entry_channels: anket,müzik
- entry_name: |
    gizli çekim videonuz sözlüğe düşse ne yapardınız
  entry_id:  89313476
  entry_writer: snowdiyebiri
  entry_fav_count: 0
  entry_content: |
    "yalnız o 3 değil, 4 posta olacaktı. videoyu kesmişler kıskançlıktan." derdim.
  entry_channels: 
---
