---
layout: post
title: 26 Eylül 2019 Ekşi Sözlük Debe
data:
- entry_name: |
    hem seks sırasında hem trafikte söylenecek cümle
  entry_id:  95772396
  entry_writer: cigkufte
  entry_fav_count: 1117
  entry_content: |
    gel götüme gir
  entry_channels: anket
- entry_name: |
    yemeksepeti'nin kestiği fark
  entry_id:  95782014
  entry_writer: snash
  entry_fav_count: 1030
  entry_content: |
    (bkz: <a class="b" href="/?q=t%c3%bcrk%c3%a7e">türkçe</a>)
  entry_channels: yeme-içme,ekonomi
- entry_name: |
    yusuf yerkel'in oğlunun yoğun bakımda olması
  entry_id:  95782426
  entry_writer: fugiart
  entry_fav_count: 846
  entry_content: |
    umarım günahlarının bedelini o küçücük çocuk çekmez. <br/>geçmiş olsun, allah şifa versin.
  entry_channels: siyaset,sağlık,haber
- entry_name: |
    plazada suyu çömelerek içen embesil
  entry_id:  95786101
  entry_writer: duygu dusunce
  entry_fav_count: 643
  entry_content: |
    istanbul'un en iyi plazalarından birinde en iyi şirketlerin birinde çalışıyorum diye hava atarken; <br/>çalışanların yüzüne konuşamıyorum, arkalarından sözlüğe entry girerek hatta başlık açarak giydiriyorum diye de eklerseniz havanıza hava katabilirsiniz.
  entry_channels: anket,yeme-içme
- entry_name: |
    özel araçlarda sigara yasağı
  entry_id:  95784914
  entry_writer: fuckgloballythinklocally
  entry_fav_count: 581
  entry_content: |
    araç benim. keyif benim. sağlık benim. istersem arabamı ateşe verip aleviyle de sigaramı yakarım. kimse karışamaz.<br/><br/>edit: arabamı yakamıyormuşum. kendimi yakarım o zaman.<br/>edit 2: kendimi de yakamazmışım. ulan ağız tadıyla bi cinnet geçiremedim. ateşi olan var mı?
  entry_channels: siyaset,sağlık,haber,yeme-içme
- entry_name: |
    günlük maliyeti 4.5 milyonu aşan külliye
  entry_id:  95777848
  entry_writer: ratm214
  entry_fav_count: 464
  entry_content: |
    habere gore kulliye'nin maliyeti gunluk 4.5 milyonu gecmis. bunu soyleyen bir muhalefet siyasetcisi degil ha. kerli ferli devlet kurumu sayistay! gunluk 4.5 milyon ulan! ayliga vurdugunuzda 137 milyon lira yapiyor. ne vatan sevgisi ne davaymis arkadas. <br/><br/><a rel="nofollow noopener" class="url" target="_blank" href="https://t24.com.tr/haber/2018-yilinda-cumhurbaskanligi-kulliyesi-nin-gunluk-harcamasi-4-5-milyon-lirayi-asti,840881" title="https://t24.com.tr/haber/2018-yilinda-cumhurbaskanligi-kulliyesi-nin-gunluk-harcamasi-4-5-milyon-lirayi-asti,840881">kaynak</a><br/><br/>edit: yazar arkadasin uyarisiyla kelli felli kerli ferli olarak degistirildi. cok tesekkurler!<br/><br/>edit2: tdk'ya gore kelli felli de kullanimda ve ikisi de ayni anlama geliyor. bu uyariyi yapan yazar arkadasa da cok tesekkur ediyorum.
  entry_channels: siyaset,haber,ekonomi
- entry_name: |
    türkiye'de domuz eti yiyenler var
  entry_id:  95793777
  entry_writer: 33derece
  entry_fav_count: 407
  entry_content: |
    türkiye'de kul hakkı yiyenler de var sayın cumhurbaşkanım, ona da müdahale etmiyorsunuz.
  entry_channels: siyaset,haber,yeme-içme
- entry_name: |
    izmir'de öğrencileri döven müdür yardımcısı
  entry_id:  95769175
  entry_writer: tembel reyiz
  entry_fav_count: 330
  entry_content: |
    bir daha ufuk'a karışmayacaksın diyor. ufuk'ta muhtemelen yanındaki çocuk.<br/>yani müdür yardımcısının dövdüğü çocuk sürekli bu ufuk denen çocuğu hırpalıyor olacak ki bu dayağı o yüzden yiyiyor.<br/>meseleye ufuk tarafından bakılınca, içimin yağlarını eritmiştir.<br/>her firavunun bir musa'sı vardır.
  entry_channels: haber
- entry_name: |
    almanya'da dövmesi var diye okula alınmayan türk
  entry_id:  95783773
  entry_writer: poslwa
  entry_fav_count: 311
  entry_content: |
    muhtemelen komple yalan haber.<br/>oğlan bir şekilde atılmış okuldan, destek görmek için bayrağa oynuyor.<br/>söylediğine göre elindeki türk bayrağı dövmesi yüzünden alınmamış, ama baksanız dövmenin türk bayrağı ile alakası yok.
  entry_channels: siyaset,haber,eğitim
- entry_name: |
    i speech kürsü
  entry_id:  95780914
  entry_writer: nickolantesla
  entry_fav_count: 285
  entry_content: |
    + which kürsü?<br/>- ayetel kürsü. <br/>şeklinde devam filminin çekilmesini çok istediğim kısa metrajlı komedi filmi.
  entry_channels: siyaset,haber
- entry_name: |
    evlatlık alınan çocuğun 22 yaşında cüce çıkması
  entry_id:  95781445
  entry_writer: dukkanin ortasinda bagiran kosnuk
  entry_fav_count: 278
  entry_content: |
    gülersem çarpılırım diye korkuyorum lakin çok komik lan. düşünsene çocuk diye harçlık veriyorsun markete gidip cuğara alıyor, ocaklardan ırak.
  entry_channels: haber,spoiler
- entry_name: |
    real madrid'de terim sesleri
  entry_id:  95776031
  entry_writer: seteris paribus
  entry_fav_count: 275
  entry_content: |
    uno momento sacremento çimento
  entry_channels: spor
- entry_name: |
    lisedeki en arka dörtlünün bugünkü durumu
  entry_id:  95777058
  entry_writer: en buyuk cuce gezegen
  entry_fav_count: 261
  entry_content: |
    iki komiser bir avukat bi de suçlu. <br/><br/>suçlu benim
  entry_channels: anket,eğitim
- entry_name: |
    eğitim fakültelerinin öğrenci almayı durdurması
  entry_id:  95776423
  entry_writer: muzu bicakla soyan adam
  entry_fav_count: 236
  entry_content: |
    sadece eğitim fakültesi için değil bir çok fakülte için yapılması gereken eylemdir. eğitim reformu bu ülkenin olmazsa olmazı artık.
  entry_channels: siyaset,haber,eğitim
- entry_name: |
    mikroişlemci yazılan komutu nasıl anlıyor sorunu
  entry_id:  95790115
  entry_writer: ask ekmek ve uranyum
  entry_fav_count: 232
  entry_content: |
    yazar için ve programlamaya giriş yapmış herhangi bir insan için güzel bir merak konusu olabilecek sorun.<br/><br/>temelde şöyle; mikroişlemciye yazılan kod düşük seviyeli bir dil, ilgili mikroişlemci için üretici tarafından hazırlanmış dar bir komut seti. yani çok anahtar kelime yok.<br/>bu anahtar kelimeler en aşağıya yani donanım seviyesine inene kadar çeşitli aşamalarda decode ediliyor yani çözümleniyor ve en aşağı donanım seviyesine indiğinde 1 ve 0'lara çevriliyor, 1 sinyal var, 0 da sinyal yok demek. <br/><br/>buradan sonrası temel elektronik ve lojik kapı bilgisi gerektiriyor. onu da başka biri anlatır.
  entry_channels: bilim,programlama,teknoloji,eğitim
- entry_name: |
    neşet ertaş
  entry_id:  95768769
  entry_writer: angara beyefendisi
  entry_fav_count: 215
  entry_content: |
    neşet ertaş ankara'da çalışırken, gazinoda şarkı söyleyen leyla'ya<sup class="ab"><a title="(bkz: leyla ertaş)" href="/?q=leyla+erta%c5%9f" data-query="leyla ertaş">*</a></sup> aşık olur. o'na türküler yazar ve hatta evlenir. ancak neşet ertaş'ın babası <a class="b" href="/?q=muharrem+erta%c5%9f">muharrem ertaş</a>, gazinoda çalışan bir kadını oğluna yakıştıramaz ve neşet ertaş'a bu birlikteliği kınayan bir türkü yakar. <br/><br/>bu arada bu türküyü barındıran plak o kadar nadir bir plak ki, internette fotoğrafına bile kolay kolay rastlayamazsınız. "küsmedim neşet'im" adıyla anılıyor ama bu plağı gören olmadığı için ismini de kafadan atmışlar tabii, doğru ismi "<a class="b" href="/?q=ne%c5%9fet+erta%c5%9f%27%c4%b1n+kahr%c4%b1">neşet ertaş'ın kahrı</a>". bu plak elbette bende mevcut ve fondan dinleyerek yazıyorum bu satırarı<sup class="ab"><a title="(bkz: bgv)" href="/?q=bgv" data-query="bgv">*</a></sup> hatta dur <a rel="nofollow noopener" class="url" target="_blank" href="https://streamable.com/u5z7e" title="https://streamable.com/u5z7e">şöyle bir dinleti</a> yapayım ayıp olmasın.<br/><br/>temiz ruhlu, saf kalplisin, şöhretsin<br/>hakkın vardır evlenmeye evladım<br/>mevlam sana yapanları kahretsin<br/>aslı bozuk alma dedim evladım<br/><br/>dokunsalar nazik tene kir gelir<br/>bizden önce ceddimize ar gelir<br/>köle olmak şanımıza zor gelir<br/>aslı bozuk alma dedim evladım<br/><br/>küsmedim neşet'im kahrettim sana<br/>baban değil miydim, sormadın bana<br/>olan olmuş yavrum ne deyim sana<br/>sen aklını yitirmişsin evladım..<br/><br/>"aslı bozuk" lafı, neşet ertaş'ın içine oturur. sevdiği kadına bu yakıştırmaları yapan babasına saygısızlık yapmadan, o meşhur "analar insandır; biz insanoğlu" sözünü barındıran <a rel="nofollow noopener" class="url" target="_blank" href="https://www.youtube.com/watch?v=45CwiaEsvCI" title="https://www.youtube.com/watch?v=45CwiaEsvCI">şu türküyle</a> karşılık verir:<br/><br/>aşkı kimden aldın, sevgiyi kimden<br/>aslı bozuk deme gel şu insana<br/>soracak olursan eğer ki benden<br/>aslı bozuk deme gel şu insana<br/><br/>yazımızı felek yazdı, mevladan değil<br/>senin dediklerin evladan değil<br/>her hata suç bende, leyla'dan değil<br/>aslı bozuk deme gel şu insana<br/><br/>ulu arıyorsan, analar ulu<br/>sevmişiz gönülden, olmuşuz kulu<br/>analar insandır, biz insanoğlu<br/>aslı bozuk deme gel şu insana<br/><br/>seni beni kim getirdi cihana<br/>her oğlu doğurmuştur bir ana<br/>senin fikrin bozuk, dostluk bahane<br/>aslı bozuk deme gel şu insana<br/><br/>bu nahif sözlerle kendini ifade eden neşet ertaş, yıllar sonra leyla ile ayrılır. "yazımı kışa çevirdin, kar yağdırdın başa leyla'm" gibi sitem dolu birçok türkü çığırır ardından, ama hiç kötü bahsetmez.<br/><br/>maalesef bu hikayenin sonunda gülen olmaz. bu yüzden neşet'tir neşet. <a class="b" href="/?q=ne%c5%9fet%27e+sevdi%c4%9fini+verselerdi%3b+ne%c5%9fet%2c+ne%c5%9fet+olmazd%c4%b1">neşet'e sevdiğini verselerdi; neşet, neşet olmazdı</a> diye boşuna dememiştir. 1984 yılında bu dünyadan böyle oğluna dargın gider muharrem usta. neşet ertaş, babasının ardından <a rel="nofollow noopener" class="url" target="_blank" href="https://www.youtube.com/watch?v=8an7w1KKeSA" title="https://www.youtube.com/watch?v=8an7w1KKeSA">deli boran</a> türküsünü havalandırır.<br/><br/>uzak yoldan geldim, hasretim için<br/>hani nerede babam, muharrem nerede<br/>yaralı bülbülüm ses vermez niçin<br/>yüreği yanığım, o kerem nerede...<br/><br/>vasiyetinde "beni ustamın ayak ucuna gömün" der neşet usta. vasiyeti yerine getirilir. mezarları bu şekilde uç ucadır. mekanları cennet olsun, orada kavuşurlar belki.. yalan dünya...
  entry_channels: sanat,tarih,müzik,sinema,edebiyat
- entry_name: |
    25 eylül 2019 fatih terim açıklamaları
  entry_id:  95782784
  entry_writer: juliadreaming
  entry_fav_count: 214
  entry_content: |
    “ali koç'un açıklamalarına istanbul'da bakıp değerse cevap vereceğimi söylemiştim. baktım, değmeyeceğine karar verdim. o nedenle cevap vermedim. eğer cevap vermeye değseydi verirdim." <br/><br/>koskoca koç ailesini madara etmiştir. alicim sen seviyeyi düşürürsün ama kapağı böyle takarlar sana düştüğün yerde kalırsın.
  entry_channels: spor
- entry_name: |
    ilk yoğurdu nasıl yaptılar sorunsalı
  entry_id:  95784583
  entry_writer: volatiliteriter
  entry_fav_count: 201
  entry_content: |
    bir kaç yıl önce amerikadaki bir dokrora öğrencisi kendi vajinasındaki bakteriden yoğurt üretmiş. muhtemelen kuşaklar önce yaşamış ebenin amıyla yaptılar.
  entry_channels: bilim,tarih,yeme-içme
- entry_name: |
    kadınlar parasız erkeği sevmez
  entry_id:  95789799
  entry_writer: chakanga
  entry_fav_count: 178
  entry_content: |
    kadına sormuşlar para mı erkek mi diye? fark etmez ikisini de harcarım demiş.
  entry_channels: anket,ilişkiler
- entry_name: |
    gss prim borcu
  entry_id:  95782393
  entry_writer: kayzersuze
  entry_fav_count: 162
  entry_content: |
    ödeyeni siksinler.. kendi vatanımızda parya ettiler bizi ,elin suriyelisi afganlısı ücretsiz muayene olacak tedavi olacak bizim borcumuz var diye tedavi olamayacağız ve üstüne bu parayı zorla almaya çalışacaklar. böyle bir memleket olabilir mi? ödemeyin toplu olarak direnelim bu haysiyetsizliğe ..
  entry_channels: siyaset,sağlık,ekonomi
- entry_name: |
    gurbetçiler oy kullanmasın kampanyası
  entry_id:  95789018
  entry_writer: plonaj
  entry_fav_count: 144
  entry_content: |
    an itibariyle başlattığım kampanya. hükümet duy sesimizi!<br/>gurbetçi çomarlar, kornacı türkler bizim kaderimizi belirleyecek seçimlerde oy kullanmasın, çok vatan sevdalılarsa temelli dönüş yapıp bol bol oy kullanabilirler!<br/>elbette gurbette çalışan düzgün insanlar da var ama inan’ın bana çoğu ağır çomar!
  entry_channels: siyaset,anket
- entry_name: |
    babadan zengin olmak vs işinde en iyisi olmak
  entry_id:  95795281
  entry_writer: chevron
  entry_fav_count: 138
  entry_content: |
    yıllardır çile çeken biri olarak babadan zenginliği tercih edeceğim karşılaşma.<br/><br/>belki ilerde bir gün zengin olurum ama gençliğim geri gelir mi hayır.<br/><br/>böyle hayatın amına koyim.
  entry_channels: anket
- entry_name: |
    iyi parti
  entry_id:  95787579
  entry_writer: degdiren berber
  entry_fav_count: 136
  entry_content: |
    akp'ye yanlamayan partidir. hdp'ye yanlayanları eleştirmek, biz sizinle bu yolda yokuz demek akp ile iş tutmak değil davaya sahip çıkmaktır, tıpkı ığdır'ı hdp yerine mhp alsın diye adayı çekmek gibi.
  entry_channels: siyaset
- entry_name: |
    bir ölü diriltme hakkı verseler diriltilecek kişi
  entry_id:  95777479
  entry_writer: delidiyez
  entry_fav_count: 127
  entry_content: |
    sorgusuz sualsiz atatürk.
  entry_channels: anket
- entry_name: |
    horizontal ile vertical'ı karıştırmak
  entry_id:  95777941
  entry_writer: tolgatrb
  entry_fav_count: 120
  entry_content: |
    vertical limit diye bir film vardi. dikey limit'ti turkceye cevrilisi ben oyle hafizama kazimistim.<br/><br/>excel kullananlar icin durum daha kolay. vlookup ve hlookup ile kolayca anlasilabilir. vlookup duseyara, hlookup yatayara. <br/><br/>bu entryden sonra karistiran varsa halen normal karsilarim. ben de halen left ve right kelimelerini karisitiriyorum.
  entry_channels: anket,eğitim
- entry_name: |
    mustafa kemal'in askerleriyiz diyen türbanlı
  entry_id:  95790982
  entry_writer: gaflabeyningerisiuremez
  entry_fav_count: 107
  entry_content: |
    mustafa kemal'in askerleriyiz diyen kadın
  entry_channels: 
- entry_name: |
    avrupa vatandaşlığı mı 200 bin tl mi
  entry_id:  95794319
  entry_writer: eksi peace
  entry_fav_count: 104
  entry_content: |
    avrupa vatandaşı olacaksam 200 bin üste ödeme yapabilirim. hazır faizler de düşmüşken kredi falan bir şekilde hallederiz, alt tarafı 32 bin euro.<br/><br/>bir versus.
  entry_channels: anket
- entry_name: |
    bütün iyi erkeklerin kapılmış olması
  entry_id:  95773235
  entry_writer: wowcukiz
  entry_fav_count: 97
  entry_content: |
    armut pis agzima dus isteyen kadinlarin dusuncesidir.<br/><br/>esimle ilk tanistigimda kendisi issiz, konusmayi bilmeyen ve yarim litre vodkayi 10 dakikada sek icebilecek kadar agir alkolik biriydi. ustelik lise mezunu ve lisede okudugu bolum bile issizlik garantili bir iletisim bolumuydu. beraber once alkolu biraktik, sonra kendini gelistirip yukselebilecegi bir is bulduk (isi babasi buldu ama alkolu birakmasaydi bulmazdim dedi<sup class="ab"><a title="(bkz: swh)" href="/?q=swh" data-query="swh">*</a></sup>) iletisim sikintisini da ustun turk gevezeligim ve canayakinligim ile ben cozdum<sup class="ab"><a title="(bkz: kendisi isvecli gunaydin demeyi bile bilmiyordu)" href="/?q=kendisi+isvecli%2c+gunaydin+demeyi+bile+bilmiyordu" data-query="kendisi isvecli gunaydin demeyi bile bilmiyordu">*</a></sup>. sonra evlendik ve ben daha iyi anladim onun hayalimdeki erkek oldugunu. bir erkek dusunun ki o kadar agir alkolik ve sizin icin ickiyi birakiyor, bir daha icmeyecegim diyor ve gercekten de icmiyor. cunku biliyor ki icerse beni kaybedecek. bana kimse bundan daha buyuk bir hediye veremezdi sanirim. iyi ki seni sirf iyi erkek tanimina uymuyorsun diye birakip gitmemis, deyim yerindeyse savasarak kazanmisim. seni cok seviyorum.
  entry_channels: ilişkiler
- entry_name: |
    marketteki saç boyası tüplerini değiştirmek
  entry_id:  95795760
  entry_writer: zlanks the blackboard
  entry_fav_count: 96
  entry_content: |
    oğlum çok büyük yavşaksınız lan.
  entry_channels: 
- entry_name: |
    16 eylül 2019 alfemo mobilya rezaleti
  entry_id:  95794035
  entry_writer: bentorn
  entry_fav_count: 94
  entry_content: |
    arkadaşlar;<br/>konu ile ilgili cidden yardıma ihtiyacım var. yaklaşık 10 gündür hiçbir sonuç alamadım ve artık tek çarem sosyal medya aracılığı ile sesimi duyurmak. bendeniz aylarca dişimden, tırnağımdan arttırıp para biriktirip salonumu yenilemek istedim. ve 27 nisan 2019 tarihinde ıstanbul alfemo sönmezler mobilya'dan bir televizyon ünitesi ve de samsung'dan qled 65" televizyon aldım. ıkisi de 29 nisan'da yetkili servis elemanları tarafından kuruldu. <a rel="nofollow noopener" class="url" target="_blank" href="https://imgyukle.com/i/img-20190917-wa0012.oFSFrN" title="https://imgyukle.com/i/img-20190917-wa0012.oFSFrN">kurulum sonrası görüntü</a><br/>17 eylül pazartesi gecesi salondan çıkıp tam odama giderken evin içinde bir gümbürtü koptu, deprem oldu ve bir yerler yıkıldı sandım. koşarak salona gittim. salona gittiğimdeki görüntüler ise şöyleydi ; <a rel="nofollow noopener" class="url" target="_blank" href="https://imgyukle.com/i/20190917-000347-resized-1.oFuoln" title="https://imgyukle.com/i/20190917-000347-resized-1.oFuoln">salon 1</a>,<a rel="nofollow noopener" class="url" target="_blank" href="https://imgyukle.com/i/20190917-000410-resized-1.oFuuyI" title="https://imgyukle.com/i/20190917-000410-resized-1.oFuuyI">salon 2</a> ve <a rel="nofollow noopener" class="url" target="_blank" href="https://imgyukle.com/i/20190917-000633-resized-1.oFuTRS" title="https://imgyukle.com/i/20190917-000633-resized-1.oFuTRS">salon 3</a><br/>yepyeni televizyonum ise darbeden zarar görmüş ve çalışmıyordu <a rel="nofollow noopener" class="url" target="_blank" href="https://imgyukle.com/i/20190917-000555-resized.oFuHAQ" title="https://imgyukle.com/i/20190917-000555-resized.oFuHAQ">televizyon</a><br/>daha da güzel bir görüntüyü ise duvarı iyice kontrol ettiğimde fark ettim. televizyon ünitesinin parçası olan yaklaşık 10 kg. ağırlığındaki rafını bakın nasıl dübele nasıl vidalarla monte etmişler;<br/><a rel="nofollow noopener" class="url" target="_blank" href="https://imgyukle.com/i/20190917-083058-resized-1.oFuBOY" title="https://imgyukle.com/i/20190917-083058-resized-1.oFuBOY">vida 1</a>, <a rel="nofollow noopener" class="url" target="_blank" href="https://imgyukle.com/i/20190917-083104-resized-1.oFumTx" title="https://imgyukle.com/i/20190917-083104-resized-1.oFumTx">vida 2</a><br/><br/>bütün gece sinirden uyuyamadıktan sonra ertesi sabah alfemo ve samsung servisini aradım, olayın aciliyetini bildirip kayıt açtırdım ve beklemeye başladım. <br/>18 eylül çarşamba günü önce samsung servisi geldi ve darbeden dolayı eğildiğini, ekranın değişmesi gerektiğini bildiren rapor tuttu. hemen akabinde ise alfemo servisi geldi, nasıl montaj yapıldığına baktı, fotoğraf çekti ve hatalı montaj yapıldığını, genel merkeze bildireceklerini söyleyerek çıktılar. fotoğraflarda da göreceğiniz üzere itiraz edebilecekleri hiçbir nokta yoktu. <br/>ve ben o günden bu yana her gün sabırla tüm iyi niyetimle alfemo servisini arayıp, çözüm rica ederek bekledim. ve nihayet 2 gün önce bildirdikleri sorunun tv ünitesini aldığım bayileriyle ilgili olduğu ve sorunu onların çözecekleri idi. <br/>dün ise sönmezler mobilya'yı arayarak "salonu ve tv'yi kullanamadığımı, acilen çözüm istediğimi" bildirdim. ilgili kişi bana "konunun yöneticilerinde oldukları, toplantı sonrası karar verdiklerinde bana dönüş yapacaklarını" söyledi ve telefonu kapattı.<br/>ve tabii ki bütün gün boyunca bir dönüş yapmadılar, bugün tekrar aradığımda ise "yapabilecekleri hiçbir şey olmadığı, tv ünitesi'nin düzgün takıldığı, takıldıktan sonra ertesi gün düşse anlayacaklarını ama sorunun benimle ilgili olduğu ve zararımı karşılamayacakları" idi. bu cevap üzerine güler misiniz ağlar mısınız arkadaşlar? birbirimize girdik telefonda ama ben bu kadar zeytinyağı, müşteri memnuniyetinden anlamayan, geri dönüş yapmayan, kötü ve kalitesiz bir firma görmedim. lütfen bu konunun duyurulması için yardım edin bana<br/>mesajla soranlar için not: bu süre zarfında alfemo bir kez bile beni aramadı ya da dönüş yapmadı. en son aradığımda da müşteri hizmetlerinden "biz hatalı montaj diye bildirmişiz, bizden çıkmış, şikayetinizi bayiiye bildirin, burayı arayıp duruyorsunuz" diye azar işittim üstüne üstlük
  entry_channels: 
- entry_name: |
    fenerbahçe vs galatasaray derbisine isim önerileri
  entry_id:  95788164
  entry_writer: dynamole
  entry_fav_count: 82
  entry_content: |
    şimdi beşiktaşlı biri gelip <a class="b" href="/?q=el+sikko">el sikko</a> yazacak. o yazmadan ben yazayım.<sup class="ab"><a title="(bkz: swh)" href="/?q=swh" data-query="swh">*</a></sup>
  entry_channels: spor,anket
- entry_name: |
    ikinci el otomobil piyasası
  entry_id:  95776699
  entry_writer: tipikbalik
  entry_fav_count: 75
  entry_content: |
    10 gün önce aracımı sattım piyasaya göre gayet uygundu ilanın 2. günü gitti.alan kişi sözde aile arabası yapıcak pikniğe falan gidicekler anlattı da anlattı bir hafta sonra malum sitede 5 bin ekleyerek ilan açmış başka sözüm yok.
  entry_channels: otomotiv,ekonomi
- entry_name: |
    17 yıllık akp hükümetinin en iyi tarafı
  entry_id:  95796488
  entry_writer: kedisevmeyenesaldirdim
  entry_fav_count: 73
  entry_content: |
    ülkeyi bıraktıkları halden sonra siyasal islamın bir daha ülkede asla iktidar olamayacak olması.<br/><br/>(bkz: <a class="b" href="/?q=yeni+nesil">yeni nesil</a>)<br/>(bkz: <a class="b" href="/?q=umut">umut</a>)
  entry_channels: siyaset,tarih
- entry_name: |
    cüneyt çakır
  entry_id:  95792251
  entry_writer: poseidon7
  entry_fav_count: 72
  entry_content: |
    fenerbahçe kazanırsa, galatasaraylılar için eyyamcı; galatasaray kazanırsa, fenerbahçeliler için eyyamcı, eğer maç berabere biterse iki taraf içinde eyyamcı olarak nitelendirilecek hakemdir.<br/><br/>bu arada, az sonra algı operasyonları başlar. (bkz: <a class="b" href="/?q=x+tak%c4%b1m%c4%b1n%c4%b1n+%c3%a7ocu%c4%9fu+c%c3%bcneyt+%c3%a7ak%c4%b1r">x takımının çocuğu cüneyt çakır</a>)
  entry_channels: spor
- entry_name: |
    ermeniler ve rumlar gitmeseydi
  entry_id:  95782474
  entry_writer: aynalikemer
  entry_fav_count: 71
  entry_content: |
    hayatında hiç ermeni veya rumla karşılaşmamış insan beyanı. rusya'da yapılan uluslararası bir spor müsabakasında ermenistan'dan gelen grubun kızlara bakışlarından rahatsız olan asyalı grubun bunlardan kaçacak yer aradıklarına şahit olmuştum, heriflerin tiplerini gece sokakta görseniz yolunuzu değiştirirdiniz. <br/><br/>pembe götlü cihangir solcuları, ermenileri hala osmanlı istanbulu'nda yaşayan şehirli entellektüel kesim olarak görse de bu adamlar fiziksel ve kültürel olarak doğu anadolu insanının hristiyan versiyonudur sadece, çomarlık konusunda eleştirdiğiniz eğitimsiz anadolu insanından hiçbir farkları yoktur. inanmayan erivan'a gidip 2-3 gün geçirmeyi deneyebilir, hayatta kalırsa anılarını burada paylaşır.<br/><br/>ermeni veya rum etnik kimliği ne olursa olsun, modern dünyada yaşamak üzere eğitim almamış kimse toplumsal yaşama adapte olamaz, ülkeye herhangi bir gelişim sağlayamaz.
  entry_channels: tarih
- entry_name: |
    25 eylül 2019 fenerbahçe'nin tweeti
  entry_id:  95796067
  entry_writer: wonderkid91
  entry_fav_count: 51
  entry_content: |
    ibneler güldürdünüz. <br/><br/>böyle şeylerle gelin işte :)<br/><br/>not: 1905
  entry_channels: spor
- entry_name: |
    netanyahu'nun tayyip erdoğan'a verdiği ayar
  entry_id:  95768880
  entry_writer: 604
  entry_fav_count: 42
  entry_content: |
    sen koca birleşmiş milletler toplantısında kendi dertlerini konuşmak yerine ümmetçilik yaparsan elin <a class="b" href="/?q=netanyahu">netanyahu</a>'su da gelir seni senin silahınla vurur.<br/><br/>o toplantıda yapılması gereken konuşma<br/><br/>- türkiye'deki suriyeliler ve suriye'nin toprak bütünlüğü (senin ypg/pkk sorunun dahil)<br/>- ege denizi'nde işgal altında olan adalarımız<br/>- iran ile ticaret yapmamızın engellenmesi<br/>- kktc ve akdeniz'deki yeraltı kaynakları<br/><br/>gibi başlıklar içermeliydi. vakit kalırsa filistin falan filan konuşulabilirdi...
  entry_channels: siyaset,haber
- entry_name: |
    norm ender
  entry_id:  95776849
  entry_writer: mso
  entry_fav_count: 35
  entry_content: |
    şarkılarında büyük atar gider yaptıktan sonra kendini beach partilerine meze yaptırmış rapçi.<br/><br/>hem protest takılıp hem de popülizmden nemalanmak değişik biraz..
  entry_channels: müzik,magazin
- entry_name: |
    acun ılıcalı'nın kanalını izlemiyoruz kampanyası
  entry_id:  95792994
  entry_writer: ortadanicki
  entry_fav_count: 34
  entry_content: |
    izlemiyorduk zaten ne kampanyası diyecektim de survivor yayındayken işi gücü bırakıp buraya survivor bilmemne başlıkları açan ergen tayfa geldi. nasıl olsa onlardan bir fayda gelmez sanki izleseler daha iyi. o saatlerde sokaklar daha bir güvenli sanki.<br/><br/>(bkz: <a class="b" href="/?q=arkada%c5%9flar+kanal%c4%b1ma+ho%c5%9f+geldiniz">arkadaşlar kanalıma hoş geldiniz</a>)
  entry_channels: tv
- entry_name: |
    türkçe'deki en seksi kelime
  entry_id:  95782008
  entry_writer: bu lutfu bana dans eder misin
  entry_fav_count: 33
  entry_content: |
    defalarca.
  entry_channels: anket
- entry_name: |
    hangi burcun kadını ideal
  entry_id:  95768976
  entry_writer: ucan mathilda
  entry_fav_count: 29
  entry_content: |
    başak burcu diyen teröristtir.
  entry_channels: anket,ilişkiler
- entry_name: |
    greta thunberg
  entry_id:  95777284
  entry_writer: jemai
  entry_fav_count: 29
  entry_content: |
    hakkında güzellemeler yapılan, işbu giri yazıldığı tarihte 16 yaşında, zengin isveçli aktivist(!). <br/><br/>bir isveçli'den alıntı yapayım o zaman:<br/><br/>"you f**king idiots. swede here! this girl is literally an autistic 16 year old who is much criticized in sweden for spreading this eco-propaganda which has led to increased gas prices, expensive changes within the electricity infrastructure, and is costing swedish tax payers billions of swedish crowns, while this girl, with a f**king pr corporate behind her, who tell her what to do, is traveling in a 4 million dollar yacht sponsored by big corporates, which costs more in carnon dioxide foot print! this girl has literllay been brainwashed by her mother, who should to be reported to the authorities for abusing her child through munchausen by proxy, has many connections within the one foundation movement, which is funded by, guess who? george f**king soros. this girl has been declared "holier than jesus" by the swedish church. let that sink in for a moment. f**k greta." (sansürlenmiştir<sup class="ab"><a title="(bkz: swh)" href="/?q=swh" data-query="swh">*</a></sup>)<br/><br/>işte her konuyu hep siz iyi biliyorsunuz. çevredir, karbon gazlarıdır vs. ben daha bu vele..., pardon çocuk, dünyada bile yokken türkiye çöl olmasın diye ondan daha küçük yaşta ağaç dikerken, şimdilerde de bir tarafımı yırtıp ülkemin refahı için dünyanın çeşitli ülkelerinde koşturup vergimi öderken bana "how dare you!" (ne cüretle!) nidalarıyla parmak sallayan bir çocuk gitsin kumda oynasın bu bir.<br/><br/>ikincisi yemen'de çocuklar, bebekler açlıktan ölürlerken, afrika'da bu aktivistten(!) daha küçük yaşta çocuklar birbirlerine ak-47 taarruz tüfekleriyle kurşun yağdırırken elin isveçli çocuğu birleşmiş milletler genel kurulu'nda öfke nöbetleri içerisinde insanlara cürretlerini söylememelidir, söylerse yukarıdaki cevapları alır.<br/><br/>suserlar çok güzel bir ifade kullanmışlar, poster çocuğu gibi görünmektedir.<br/><br/>dip not: eyyy greta, sen kimsin ya!<sup class="ab"><a title="(bkz: yapmasam olmazdı)" href="/?q=yapmasam+olmazd%c4%b1" data-query="yapmasam olmazdı">*</a></sup><sup class="ab"><a title="(bkz: swh)" href="/?q=swh" data-query="swh">*</a></sup>
  entry_channels: siyaset,haber
- entry_name: |
    ekşi itiraf
  entry_id:  95768810
  entry_writer: tagu
  entry_fav_count: 16
  entry_content: |
    üç gün önce dedemi; babamın eksikliğini hissettirmeyen adamı, şu zamana kadar sahip olduğum her şeyin sağlayıcısını, ev arkadaşımı, dostumu, kavga ettiğim, şakalaştığım, ağlaştığım, dertleştiğim adamı, koruyucumu, kefilimi ve sırdaşımı kaybettim. bugün onu morgda gördüğüm an, hayatımda kendimi en güçsüz hissettiğim andı. onu toprağa verirken sanki kendim de oraya gömüldüm. beni oğlum diye seven, bu yaşıma kadar her şeyimi düşünen ve beni bu rahatlığa alıştıran adam; onsuz yürümeyi becerememiş bu herifin omzuna tüm ailenin yükünü bıraktı gitti. ailemizin direği, güzel insanlar biriktirdiğimi ve sorumluluklarımı hatırlatıp yıllardır hasretle andığı karısının yanına yattı.
  entry_channels: anket,ilişkiler,ekşi-sözlük
- entry_name: |
    kalp krizi geçiren adam yardım etmeyen cemaat
  entry_id:  95798348
  entry_writer: yuksektansiyon
  entry_fav_count: 14
  entry_content: |
    yanındaki adamların muhabbet belli zaten "allah'ın sevdiği kuluymuş camide aldı." bu arkadaşlar çok güzel konuşurlar allah adına...
  entry_channels: haber
- entry_name: |
    izlenen en berbat film
  entry_id:  95792136
  entry_writer: keyif suruyoruz
  entry_fav_count: 10
  entry_content: |
    abi ben bu özgüvene cidden hayranım, valla bak. 20 senelik bir oluşumda böyle başlığın varlığını sorgulamıyor, aramaya inanmıyor, yetmiyor forum tarzı başlık açıyor. saygıyla eğiliyorum, sikeyim kötü filmleri.<br/><br/>edit: başlığın orijinal hali "izlediğiniz en berbat film" gibi bir şeydi.
  entry_channels: anket,sinema
- entry_name: |
    kısa paça + dar gömlek + nargile + sert bakış
  entry_id:  95797943
  entry_writer: 111222333
  entry_fav_count: 7
  entry_content: |
    daha önce yazılmış ama tekrar edeyim. bunların çoğu gizli eşcinsel ve biseksüeldir. kerimcan durmaz gibi tipler bunların kocalarıdır.
  entry_channels: anket,yeme-içme,moda
- entry_name: |
    twitter'da alkol şirketlerine takip yasağı
  entry_id:  95793721
  entry_writer: sherlucky
  entry_fav_count: 6
  entry_content: |
    vay be. hemen alkolu bıraktım çok etkili oldu bu engelleme. <br/><br/>not: zaten normalde içmiyorum. içenlere de saygı duyuyorum.
  entry_channels: siyaset,teknoloji,haber,yeme-içme
- entry_name: |
    erkeklerin kadınlarda aradıkları
  entry_id:  95795528
  entry_writer: thinkngela
  entry_fav_count: 3
  entry_content: |
    ay n’olur bir şey aramayın. <br/>bi salın artık bizi.
  entry_channels: anket,ilişkiler
- entry_name: |
    kanser hastasına yardım kampanyası
  entry_id:  95784599
  entry_writer: baklava yiyen kedi
  entry_fav_count: 0
  entry_content: |
    elimizden geldiğince yardımcı olmamız gereken hasta için düzenlenen yardım kampanyasıdır. bir an önce tedavisini olabilmesi ve sağlığına kavuşabilmesi dileğiyle.
  entry_channels: sağlık,haber
- entry_name: |
    doğulan ilçeyi 3 kelime ile anlatmak
  entry_id:  95771357
  entry_writer: radyoaktif pokemon
  entry_fav_count: 0
  entry_content: |
    merkezde doğdum aq
  entry_channels: anket,tarih
---
