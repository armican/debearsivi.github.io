---
layout: post
title: 03 Eylül 2019 Ekşi Sözlük Debe
data:
- entry_name: |
    düğününde mekanın sahibini söyleyen gelin ve damat
  entry_id:  94796504
  entry_writer: oyuncak bulut
  entry_fav_count: 1120
  entry_content: |
    düğün salonunun sahibinin adını söyleyecekler sandım lan.<br/>meğer rap şarkısı ismiymiş a.q.
  entry_channels: ilişkiler,haber
- entry_name: |
    bir ailenin çocuğuna yapabileceği en büyük kötülük
  entry_id:  94775766
  entry_writer: detroitli
  entry_fav_count: 381
  entry_content: |
    çocuğunu cemaat ve tarikat yurtlarına yollamak
  entry_channels: anket,eğitim
- entry_name: |
    5 kg'lık dambılla evde vücut yapmaya çalışan hıyar
  entry_id:  94800821
  entry_writer: resimci
  entry_fav_count: 298
  entry_content: |
    profesyonel fitness eğitmeni olarak bunu yapıyorum hıyar olduğumu buradan öğrenmek güzel oldu
  entry_channels: spor,anket
- entry_name: |
    emine erdoğan sevsin diye kuşun gagasını kapatmak
  entry_id:  94798104
  entry_writer: nickolantesla
  entry_fav_count: 224
  entry_content: |
    üst edit: (bkz: <a class="b" href="/entry/94799144">#94799144</a>) burada arkadaş da anlatmış. bu kuşların burun delikleri yok ve gagalarından nefes alıyorlar. <a rel="nofollow noopener" class="url" target="_blank" href="https://evrimagaci.org/pelikanlar-ve-sira-disi-gagalari-hakkinda-bilgiler-3914" title="https://evrimagaci.org/pelikanlar-ve-sira-disi-gagalari-hakkinda-bilgiler-3914">*</a><br/><br/>salda gölü'nde incelemelerde bulunanan emine erdoğan'ın sevmesi için, bir pelikanın gagasının sımsıkı kapatılması olayıdır.<a rel="nofollow noopener" class="url" target="_blank" href="https://pbs.twimg.com/media/EDdTLupXoAAWLSI?format=jpg&amp;name=small" title="https://pbs.twimg.com/media/EDdTLupXoAAWLSI?format=jpg&amp;name=small">*</a> bu hayvan sevgisini görünce ben şahsen emine hanımın çevre duyarlılığı konusunda fevkaladenin fevkinde <a class="b" href="/?q=mutmain">mutmain</a> oldum. salda gölü'nde güzel güzel gezen <a rel="nofollow noopener" class="url" target="_blank" href="https://pbs.twimg.com/media/EDdULyfXoAI33E0?format=jpg&amp;name=small" title="https://pbs.twimg.com/media/EDdULyfXoAI33E0?format=jpg&amp;name=small">şu güzelim canlıya</a> o eziyet niye yapılıyor? çünkü first lady'miz sevecek. <br/><br/>nedir yani; kuş slogan mı atmış, "salda gölü'ne dokunma" diye mi ötmüş... doğa sevginiz sahte ve iğreti, halkı önemsiyor görünmeniz iğreti, incelemeniz iğreti, samimiyet şovlarınız iğreti... bizim paramızla gidip bölgeyi geziyorsunuz, sözde incelemeler yapıyorsunuz, israf yapıyorsunuz, sözde hayvan seviyorsunuz, sözde doğal hayatı önemsiyorsunuz. <a class="b" href="/?q=bezduk+bezduk+da">bezduk bezduk da</a><br/><br/>edit: bazı arkadaşlar, kuşun yabani olması ve kameralar önünde bir aksilik olmaması adına yapılanı normal bulmuş. ya güzel kardeşim, gidip gölde yüzen hayvanı yakalayıp sevme o zaman, uzaktan bakarak sev. niye bunu yapıyorsun hayvana. şu kuşu sevmek için yapılan ile, bir aslanla fotoğraf çektirmek için uyuşturmak arasında bir fark yok.
  entry_channels: siyaset,haber
- entry_name: |
    türkiye'de sıfırdan zengin olmak
  entry_id:  94778043
  entry_writer: kurtarin kendinizi
  entry_fav_count: 222
  entry_content: |
    yasal yoldan olanaksızdır. şans oyunları ve illegal yöntemleri ekarte ederek konuşursak abd'ye göre çok daha düşük bir orandır bu.<br/><br/>çünkü türkiye'de kapitalizm tesis edilememiştir. hantal ve olması gerekenden çok büyük olan ve sık sık politik çıkarlara alet olan bir devlet yapısı var.<br/><br/>kredi kaynaklarına ulaşmak zor ve maliyetli. abd'ye göre hammadde pahalı işçilik görece olarak daha ucuz olsa da ara teknoloji ve arge meselesinde arada 50 yıllık fark var.<br/><br/>türkiye'de stabil bir iktisadi nizam yok. enflasyon, oynak döviz kurları, yüksek vergiler büyük handikap.<br/><br/>tabii abd ile türkiye'yi kıyaslamak çok saçma. en doğrusu türkiye'yi italya ve yunanistan gibi ülkelerle mukayese etmek ancak bilinen bir örnek açısından abd'yi verdim.<br/><br/>~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~<br/><br/>edit : abd'de yasal yoldan zengin olmak çocuk oyuncağıdır, abd'ye giden zengin olur falan demedim. sadece türkiye'ye göre çok daha olasıdır abd'de yasal yoldan sıfırdan zengin olmak.<br/><br/>örnek vermek gerekirse bu ihtimal abd'de %1 ise türkiye'de %0.1 bile değildir. <br/><br/>öyle olmasa her sene milyonlarca insan geçkin yaşına rağmen kpss'de ter dökmez.
  entry_channels: ekonomi
- entry_name: |
    türk kızlarının çoğunun kokması
  entry_id:  94794770
  entry_writer: hobo man
  entry_fav_count: 172
  entry_content: |
    kötü kokan kadına çok az rastladım. belki de benim şansım bilmiyorum.<br/>ama bildiğim bir şey var. türk kadınına hakaret etmeden iki dakika duramıyorsunuz.<br/><br/>edit: bir de şunu söylemeden edemeyeceğim. anası, karısı, kız kardeşi olmasa bokun içinde yaşayacak bana meriç diyen adamlar lütfen siktirsin gitsin.
  entry_channels: ilişkiler
- entry_name: |
    mario lemina
  entry_id:  94770471
  entry_writer: lomayi
  entry_fav_count: 169
  entry_content: |
    26 değil 36 yaşındadır,<br/>iyi olsa premier ligden türkiye’ye gelir miydi?<br/>mustafa sarp’ın 2019 versiyonu.<br/>kiralık topçu iş yapmaz.<br/><br/>orta sahasında 40 yaşında adam oynatan kadıköy yosmaları tarafından bu şekilde eleştirilecek topçudur.
  entry_channels: spor
- entry_name: |
    arif erdem'in fetö'den itirafçı olmak istemesi
  entry_id:  94792911
  entry_writer: bu da gecer ya huu
  entry_fav_count: 168
  entry_content: |
    fetocuları derin düşüncelere düşürmüştür, gerçeklerin bir gün ortaya çıkması gibi bir kötü huyu vardır.
  entry_channels: spor,siyaset,haber
- entry_name: |
    4 dil bilmek
  entry_id:  94798449
  entry_writer: gitmehevesi
  entry_fav_count: 152
  entry_content: |
    bir ekşisozluk klasigini gördüğümüz başlıktır. ayda 100 bin lira kazanan adam başlığında herkes 100 bin kazanıyordur. yakışıklı olmak konusundaki başlıkta herkes yakışıklıdır. sozlukculerin yaptıkları işlere bir bakın elini sallasan ceo ya çarpar.. şimdi bu entry ye de dolarlar 4 dil biliyorum diye. sözlük sen nasıl bir ortamsin yahu..<br/><br/>tanım: kişiyi başka bir kişi yapacak seviyedir
  entry_channels: bilim
- entry_name: |
    yazarların futboldan soğuma nedenleri
  entry_id:  94783122
  entry_writer: cakobo
  entry_fav_count: 138
  entry_content: |
    yıllar önce sinopspor 3.ligdeyken küçücük şehir stadyumuna gidip maçlarını izlerdim. stad o kadar küçüktü ki elini uzatsan yan hakeme dokunabiliyordun.<br/>yine böyle bir gün sinopspor'un maçında holiganların ağırlıkta olduğu kısımda maçı izlerken yan hakeme haksız yere o kadar aşağılık küfürler ediliyordu ki insanlığınızdan utanırsınız. hiç hatasız maç yönetmesine rağmen yan hakeme "akıllı ol adam gibi maç yönet sikeriz belanı" deyip hakem kafasını çevirip tribüne baktığında da ortalığı yıkıp <br/>"ne bakıyon orrospu çocuğu,amk oğlu, senin çoluğunu çocuğunu sikerim" tarzı küfürlerle beynimde şimşekler çaktı. holiganlık gerçekten dünyanın en mal şeyi olabilirdi. hepinizin amına koyim deyip, ortamı terkettim ve bir daha da maç izlemedim.
  entry_channels: spor
- entry_name: |
    yetişkin seviyesindeki en iyi çizgi film
  entry_id:  94802033
  entry_writer: avaremo
  entry_fav_count: 110
  entry_content: |
    (bkz: <a class="b" href="/?q=avatar+the+last+airbender">avatar the last airbender</a>)<br/>general iroh gibi bir filozofu barındırır.
  entry_channels: anket,tv
- entry_name: |
    2 eylül 2019 alpay özalan'ın attığı tweet
  entry_id:  94802603
  entry_writer: smasherz19
  entry_fav_count: 104
  entry_content: |
    alpay bugüne kadar vekil olarak ne yapmış ki milletten hesap sormaya çalışıyor?
  entry_channels: siyaset,haber
- entry_name: |
    gazete kuponuyla alınmış şeyler
  entry_id:  94787214
  entry_writer: sizofrenx
  entry_fav_count: 100
  entry_content: |
    sabah gazetesinin verdiği müzik seti.<br/>müzik seti dediğime bakmayın. devasa şekilde gösterdiği müzik setini komşulardan rica ederek arabayla almaya gittik. elimizde dolmuşla falan getiremeyiz diye.<br/>hesap makinesi gibi bir teyp vermişlerdi orospu çocukları.<br/>amk ne üzülmüştüm ya.<br/><br/>edit: benimle birlikte bu üzücü durumu yaşayan nice yazar varmış. gelin kardeşim sarılalım sizinle:(<br/>bu arada bizim faciamızdan sonra gazete özür babında yeniden müzik seti vereceğini söyledi(daha büyük çift kaset çalarlı) lakin bizi yine keriz gibi silkerler diye girmedik o işe. velhasıl kelam gösterdikleri şeyi bu sefer verdi ipneler.<br/>ama bize hak anasını satayım. millet cilt cilt ansiklopediye, kitaplara yatırım yapmış biz çalgı çengiye. bize müstehak amk.
  entry_channels: anket
- entry_name: |
    evrende yalnız mıyız
  entry_id:  94804528
  entry_writer: minimalistdev
  entry_fav_count: 92
  entry_content: |
    evren epey büyük bir yer. eğer ki sadece biz varsak, bu epey büyük bir israf olurdu.<br/><br/>carl sagan
  entry_channels: bilim
- entry_name: |
    galatasaray
  entry_id:  94770713
  entry_writer: kafkasli17
  entry_fav_count: 88
  entry_content: |
    falcao'nun futbol olarak vereceği katkılar bir tarafa, özellikle önümüzdeki birkaç transfer sezonunda da falcao etkisiyle transfer hamlelerinde eli güçlenecek takımım. falcao ve feguli'nin hücumda, muslera'nın kalede, nzonzi'nin orta sahada olduğu takıma herkes gelmek ister. belhanda ve babel'i saymadım bile, ki beynelminel prestiji olan fatih terim gibi bir değer var takımın başında, ayrıca gomis, drogba, podolski ve sneijder gibi kulübe hala bağlı olan çok güçlü karakterler var ve bunlar adeta menajer gibi çalışıyor halen.. demem o ki maddi sınırlar dahilinde birçok topçuya teklif götürmeli gs, hani "gelmez" denenlere bile. falcao öyle bir değer, herkes onunla oynamak ister.. <br/><br/>transferle ilgili tek olumsuz görüşüm ise elmander kanalını tam kullanamadık. linnes'e bakıyorsun çalışkan, hırslı, tekniği eh ama fiziği gayet sağlam ve yeri geldiğinde gol asist yapabilen bir adam. yani böyle kuzeyli 2-3 adam bulmalıyız her sene düşük maliyetlerle. elmander'i mutlaka kullanmamız gerekir bu konuda..<br/><br/>tanım : dört yıldızlı
  entry_channels: spor
- entry_name: |
    halkımızın mutmain olmasını tavsiye ediyorum
  entry_id:  94793671
  entry_writer: 4grammaton
  entry_fav_count: 83
  entry_content: |
    "su çok mutmain, sen de gelsene..." kadar anlamlı bir cümledir.<br/><br/>(bkz: <a class="b" href="/?q=a%c4%9fl%c4%b1yorum+ama+%c3%bcz%c3%bcnt%c3%bcden+de%c4%9fil">ağlıyorum ama üzüntüden değil</a>)
  entry_channels: siyaset,ilişkiler,haber
- entry_name: |
    masterchef türkiye
  entry_id:  94768768
  entry_writer: kirkalti46
  entry_fav_count: 81
  entry_content: |
    babam 30 yıldır esnaf lokantası işletiyor. daha 1 kez bakla çorbası yapmışlığı yok
  entry_channels: tv,yeme-içme
- entry_name: |
    herkesin baş tacı ettigi sevilmeyen kült filmler
  entry_id:  94788659
  entry_writer: moron5
  entry_fav_count: 78
  entry_content: |
    (bkz: <a class="b" href="/?q=star+wars">star wars</a>)<br/><br/>sevmemek de değil, iğrenmek. ilk taşı günahsız olanınız atsın.
  entry_channels: anket,sinema
- entry_name: |
    erkeklerin ömür boyu sakladıkları kablo dolu kutu
  entry_id:  94796354
  entry_writer: geometri bilmeyen akademos rahibi
  entry_fav_count: 74
  entry_content: |
    adaptörlerin üstünde rahat okunsun diye amper değerlerinin yazılı olduğu etiketler olan, tamamı milimetrik sarılmış, geçmişten bugüne herhangi bir platform için lazım olabilitesi olan her türlü jack, dönüştürücü, kablo ve ölçüm aletlerinin olduğu 2 adet büyük bisküvi kolim bu tanıma uygundur diye düşünmekteyim. bir kablo lazım olduğunda<sup class="ab"><a title="(bkz: elimde olmasına rağmen)" href="/?q=elimde+olmas%c4%b1na+ra%c4%9fmen" data-query="elimde olmasına rağmen">*</a></sup> kutunun düzenini bozmamak için yenisini satın almışlığım çok olmuştur.<sup class="ab"><a title="(bkz: obsession)" href="/?q=obsession" data-query="obsession">*</a></sup><br/><br/>edit : başlık, dirilmek için bana "hdmi to vga adapter" in lazım olmasını bekliyormuş.
  entry_channels: teknoloji
- entry_name: |
    canan kaftancıoğlu'nun türk diyememesi
  entry_id:  94798477
  entry_writer: tintobrass
  entry_fav_count: 61
  entry_content: |
    fransa'da etnik sayımın yasak olduğunu ve etnik açıdan fransa'da yaşayan 5 milyondan fazla <a class="b" href="/?q=br%c3%b6ton">bröton</a> (bretonlar) olduğunu hanginiz biliyor?<br/>veyahut fransa'da yaşayan <a class="b" href="/?q=korsikal%c4%b1">korsikalı</a>ları?<br/><br/>ve hanginiz fransa başkanlarının, siyasetçilerinin " ey fransızlar, brötonlar, korsikalılar" diye lafa başladığını duydunuz?<br/>veya gazetelerde kaçınız "<a class="b" href="/?q=fransa+halklar%c4%b1">fransa halkları</a>" diye haber okudunuz?<br/><br/>bu yüzden lafa "araplar, çerkezler, kürtler, türkler, lazlar " diye başlayan orospu çocukları, lütfen önce yukarıdaki sorulara cevap versin. <br/><br/>ayrıca "araplar, çerkezler, kürtler, türkler, lazlar " diye sayarken türkiye'deki tam 36 etnik unsurun tamamını da sayacaksınız ulan, öyle yok 3-5'ni saymak!
  entry_channels: siyaset,haber
- entry_name: |
    ilk buluşmada sevişmek
  entry_id:  94802673
  entry_writer: maynakk
  entry_fav_count: 58
  entry_content: |
    insanın çekici bulduğu biriyle sevişmesini eskortlukla aynı kefeye koyan insanların olduğunu gördüğümüz durum. <br/><br/>düşünsene, bunun düşüncelerini ciddiye alan falan da vardır.
  entry_channels: ilişkiler
- entry_name: |
    metal müzik tarihinin en hüzünlü şarkıları
  entry_id:  94790731
  entry_writer: ahadbir
  entry_fav_count: 57
  entry_content: |
    <a class="b" href="/?q=pentagram">pentagram</a><br/><a class="b" href="/?q=for+those+who+died+alone">for those who died alone</a>
  entry_channels: anket,müzik
- entry_name: |
    luiz gustavo dias
  entry_id:  94774137
  entry_writer: siyahhuydurbende
  entry_fav_count: 49
  entry_content: |
    ''ne gereği vardı bu kadar paraya, orda bir sürü adamımız var zaten'' diyen arkadaşlar ne kadar öenmli bir adam olduğunu bugunkü trabzon maçıyla anlamışlardır umarım.
  entry_channels: spor
- entry_name: |
    yanyana açılan 3-4 tane eczanelerin amacı
  entry_id:  94800081
  entry_writer: iki saattir seni ariyorum nick
  entry_fav_count: 47
  entry_content: |
    (bkz: <a class="b" href="/?q=barlar+soka%c4%9f%c4%b1">barlar sokağı</a>)<br/>(bkz: <a class="b" href="/?q=kalayc%c4%b1lar+%c3%a7ar%c5%9f%c4%b1s%c4%b1">kalaycılar çarşısı</a>)
  entry_channels: sağlık,ekonomi
- entry_name: |
    norveç insanlarla hayvanların evlenmesine onadi
  entry_id:  94805767
  entry_writer: jebir
  entry_fav_count: 45
  entry_content: |
    peki hayvanların bundan haberi var mı?
  entry_channels: siyaset,ilişkiler,haber
- entry_name: |
    elektrikli süpürge alacaklara altın tavsiyeler
  entry_id:  94771447
  entry_writer: powerslave
  entry_fav_count: 42
  entry_content: |
    tüm başlığı okudum. şu anda da "iyi de neden okudum? benim elektrikli süpürgeye ihtiyacım yok ki" diyerek çıkıyorum başlıktan.
  entry_channels: anket,teknoloji
- entry_name: |
    200 gram nescafenin 54 lira olması
  entry_id:  94800852
  entry_writer: dedi kuzgun
  entry_fav_count: 42
  entry_content: |
    (bkz: <a class="b" href="/entry/80180907">#80180907</a>)<br/><br/>ekleme: geldi gene onu içme bunu içciler. adam diyor ki 200 gr kahve nasıl böyle pahalı olur; bizim ponçikler ba ba ba barista şovvvv!<br/><br/>süt köpüğüyle "ekşi sözlük" yazıyorum kahvemin üstüne, ay siz nasıl içiyorsunuz o kadar özensiz özensiz..
  entry_channels: yeme-içme,ekonomi
- entry_name: |
    instagram'a niçin fotoğraf yüklüyoruz sorunsalı
  entry_id:  94804893
  entry_writer: fistiginyanindakiayi
  entry_fav_count: 28
  entry_content: |
    bir süre önce kendime sordum ve tatmin edici bir cevap bulamayınca kendi fotoğraflarımı yüklemeyi bıraktım.
  entry_channels: anket,ilişkiler,teknoloji
- entry_name: |
    kadını yatakta mutlu etmenin yolları
  entry_id:  94775168
  entry_writer: mersibogu
  entry_fav_count: 27
  entry_content: |
    bir kadın olarak söylüyorum hardcore her zaman işe yaramaz. belli bir ritmi uzun süre tutturmazsan hiçbir kadını mutlu edemezsin. evirip çevirip durmayın hardcore yapacam diye
  entry_channels: anket,ilişkiler
- entry_name: |
    ekşi itiraf
  entry_id:  94769029
  entry_writer: bir damla mutluluk
  entry_fav_count: 23
  entry_content: |
    yapmam dediğim her şeyi yaptım şu hayatta. kınadığım her şey de bir bir geldi başıma. böyleymiş bu dünyanın işleri...
  entry_channels: anket,ilişkiler,ekşi-sözlük
- entry_name: |
    yurt dışında döner yemek
  entry_id:  94768745
  entry_writer: ethrenie
  entry_fav_count: 20
  entry_content: |
    tam bir low hareketi. lan adamın memleketine gitmişsin insan yeni tatlar tanımak istemez mi. phillye gidip philly cheese steak yerine tavuk şiş yiyen gördü bu gözler.
  entry_channels: yeme-içme,seyahat
- entry_name: |
    underrated pc oyunları
  entry_id:  94774172
  entry_writer: kore gazisi yavuz dede
  entry_fav_count: 19
  entry_content: |
    adam <a class="b" href="/?q=gta">gta</a>, age of falan yazmış la. yemin ediyorum insanı yoruyorsunuz siz. lan <a class="b" href="/?q=underrated">underrated</a> diyor adam <a class="b" href="/?q=underrated">underrated</a> . bu yeni neslin beyni beni yıpratıyor yeminlen.
  entry_channels: anket,oyun,teknoloji
- entry_name: |
    anıtkabir'e gitmek siyasi mesajdır
  entry_id:  94806153
  entry_writer: kedisini arayan sahip
  entry_fav_count: 17
  entry_content: |
    "evet, özellikle de senin gibi siyasal islamcı ve fetullahçı kıçı yalamaya bayılan kevaşeler için ince bir mesajdır." şeklinde doğruladığım tespit.
  entry_channels: siyaset,haber
- entry_name: |
    tokatçı
  entry_id:  94804817
  entry_writer: falconvirtue
  entry_fav_count: 16
  entry_content: |
    an itibari ile star tv'de başlamış kemal sunal efsanesi.<br/>açıp da internetten izlemiyoruz böyle tv'de denk gelince nostaljik olduğundan mı özlem duyduğumuz çocukluğumuza götürdüğünden mi bilinmez izlettiriyor kendini.<br/><br/>filmde en çok dikkatimi çeken osman hep hayata karşı pozitif. dilenci kadın enayi yerine koyuyor gülüyor, tüm birikimini trende çarptırıyor farkedince hâlâ pozitif "emine bekle beni" deyip babasını düşürüp gülüyor. hiç vazgeçmiyor yılmıyor zaten filmin sonunda da amacına ulaşıyor.<br/>evet bu bir tek filmlerde olur. ama belki de hayatı film gibi yaşamak gerek...
  entry_channels: sinema
- entry_name: |
    riza durmisi
  entry_id:  94773504
  entry_writer: condenser mikrofona tukuren sarkici
  entry_fav_count: 15
  entry_content: |
    la adam iyi hissediyorum, ne zaman istersen teste hazırım gibisinden bir şeyler yazmış. istediğiniz yerde ekspertize sokarız diyen <a class="b" href="/?q=sahibinden">sahibinden</a> tayfasına benzettim herifi.<sup class="ab"><a title="(bkz: swh)" href="/?q=swh" data-query="swh">*</a></sup> <br/><br/>muhtemel yeni sol bekimiz.
  entry_channels: spor
- entry_name: |
    zekaya aşık olan kadınlar
  entry_id:  94798555
  entry_writer: fraternity
  entry_fav_count: 14
  entry_content: |
    inanmıyorum ben bu kadınların varlığına.<br/>benim zekama aşık olan kadın bana yaşımı, tipimi, maaşımı, boyumu, posumu sormaz.
  entry_channels: ilişkiler
- entry_name: |
    fuat ergin
  entry_id:  94806285
  entry_writer: degisimindengesi
  entry_fav_count: 13
  entry_content: |
    kendisini bimde kasa sırasında beklediğim eski rapçidir. bilen bilir kendisi çapada ikamet etmektedir. önümde baktım iri yarı bir adam. aldığı ürünler geçtikten sonra kasiyere avucundaki bozuklukları uzatıp, “burdan al kanka” demesinden tanıdım. o kanka deyişini unutabilir mi insan?<br/>(bkz: <a class="b" href="/?q=sana+puan%c4%b1m+9+kanka">sana puanım 9 kanka</a>)
  entry_channels: tv,müzik
- entry_name: |
    türk kadınlarının sevişmeden önce ışığı kapatması
  entry_id:  94806998
  entry_writer: 604
  entry_fav_count: 11
  entry_content: |
    saçma bir özelliktir. arkadaş bırak görsel olarak da tatmin olalım. olay bir deliğe girip çıkmak olsa senle sevişmemin kıymeti nedir?<br/><br/>şimdi düşündüm de sadece çirkin erkeklerin başına geliyor olabilir bu durum. kendim çirkin olmamdan yola çıkarak bu cevabı daha mantıklı bulmaya başladım. nasıl bir aydınlanma yaşadıysam...<br/><br/>kadın kişisi muhabbeti sevmiştir, sevişme ihtiyacı da doğmuştur. tipsizsinizdir ve olaylar gelişir.<br/><br/>kafamıza kese kağıdı geçirmek kırıcı olabilir diye ışığı kapamak daha kolay.
  entry_channels: ilişkiler
- entry_name: |
    ilk pringles cipsi star gazetesinden almış nesil
  entry_id:  94804388
  entry_writer: olene kadar mokoko
  entry_fav_count: 11
  entry_content: |
    sabah 4'te kalkıp bütün bakkalların kapısında sıra ile bekledikten sonra "yok kalmadı" sözleri ile karşılaşarak alamadığım cipstir. <br/><br/>ibneler stoklayıp para ile sattı heralde. <br/><br/>ondan sonra küçük esnaf böyle iyi şöyle güzel. nah güzel yaşasın kapital düzen.
  entry_channels: 
- entry_name: |
    volkan demirel
  entry_id:  94802431
  entry_writer: a rainy day in falkreath
  entry_fav_count: 7
  entry_content: |
    (bkz: <a class="b" href="/?q=kad%c4%b1k%c3%b6y%27de+hala+bir+ay%c4%b1+var">kadıköy'de hala bir ayı var</a>)<br/><br/>edit: kulübede.
  entry_channels: spor
- entry_name: |
    yazar nick'inden nasıl öleceğini tahmin etmek
  entry_id:  94793039
  entry_writer: ben buraya niye ciktim
  entry_fav_count: 6
  entry_content: |
    dar bir skinny jean giyip varislerinin pörtlemesi sonucu aramızdan ayrılacak.
  entry_channels: ekşi-sözlük
- entry_name: |
    mutmain olmak deyimini cümle içinde kullan
  entry_id:  94804204
  entry_writer: al2che enon
  entry_fav_count: 4
  entry_content: |
    -ben mutmain gördüm.
  entry_channels: siyaset,edebiyat
- entry_name: |
    mutsuzken yapılanlar
  entry_id:  94774437
  entry_writer: mismiyav
  entry_fav_count: 3
  entry_content: |
    90lar türkçe pop dinliyorum ben. cıstak olanlarından ama^^
  entry_channels: anket,ilişkiler,sağlık
- entry_name: |
    canı istemiyorsa telefona yanıt vermeyen insan
  entry_id:  94790607
  entry_writer: nihilanth
  entry_fav_count: 3
  entry_content: |
    - uyuyor muydun<br/>- hasta mısın, yorgun musun sesin kötü geliyor<br/>- nerdesin, müsait misin<br/><br/>sorularını duymak istemeyen insandır. sevgili, eş dost haricinde telefonla konuşmak acil ve önemli konular dışında çok gereksiz geliyor. bir de iş itibari ile bıkmış durumdayım.
  entry_channels: ilişkiler,teknoloji
- entry_name: |
    bilim adamlarının %93'ünün ateist olması
  entry_id:  94803662
  entry_writer: hopaa
  entry_fav_count: 3
  entry_content: |
    say lan.
  entry_channels: anket,bilim,haber,eğitim
- entry_name: |
    çaylaklardan mesaj bekleyen yazarlar veritabanı
  entry_id:  94773211
  entry_writer: robiji
  entry_fav_count: 2
  entry_content: |
    düşüyor mu böyle cidden? yorulmadınız amk
  entry_channels: anket,ilişkiler,ekşi-sözlük
- entry_name: |
    spotify
  entry_id:  94804511
  entry_writer: travenian
  entry_fav_count: 1
  entry_content: |
    android app si son zamanlarda sık sık saçmalamaya başladı. durup dururken ses kesilmesi , diğer şarkıya geçiş yapmadan çalmaması. ne ararsanız var amk.
  entry_channels: müzik,teknoloji
- entry_name: |
    klimayı 18 derecede açacak kadar kıt akıllı olmak
  entry_id:  94807545
  entry_writer: ortadanicki
  entry_fav_count: 1
  entry_content: |
    güney florida'ya gittim diye başlık açsan daha iyiydi, ben de nereye varacak bu diye boşuna okudum.
  entry_channels: 
- entry_name: |
    falcao'nun 2019-2020 sezonu gol sayısı tahminleri
  entry_id:  94808806
  entry_writer: maverakc
  entry_fav_count: 1
  entry_content: |
    türkiye fizik güce dayalı olduğu için kolay gol atılan bir lig değilmiş. o yüzden falcao'nun işi çok zormuş. öyle yazmış. <br/><br/>diagne: 30 gol<br/><br/>- beyni yok fikri var :)
  entry_channels: spor,anket
- entry_name: |
    yaş ilerledikçe katlanılması zor şeyler
  entry_id:  94805454
  entry_writer: lostparadise
  entry_fav_count: 0
  entry_content: |
    geçimsizlik
  entry_channels: anket
---
