---
layout: post
title: 18 Nisan 2018 Ekşi Sözlük Debe
data:
- entry_name: |
    cehape'nin bir oyu daha azaldı
  entry_id:  76079576
  entry_writer: futuregonnacome
  entry_fav_count: 719
  entry_content: |
    kimlerle muhatap olduğunuzun farkında olun. bunlara hümanizm kasmayın boşuna. hümanizm insan olanlar için. bu çöl faresi bedeviler için değil. ortamına göre özgürlükçü ortamına göre ulvi bir yapıda iyilik meleği timsali olarak takılırlar bu tipler. ancak gerçek yüzleri tam olarak da budur işte kulağınıza küpe olsun. <br/><br/>not: hayatın ona karşı cimri davranıp onu intihara itmesini sağlayan öğretmenimizin ailesine baş sağlığı dilerim. kimler yalakalıkla nerelere gelirken böyle pırıl pırıl toplumda görmek isteyeceğimiz insanlar canlarına kıyıyor.
  entry_channels: siyaset,haber
- entry_name: |
    berat albayrak'ın rte'ye satranç takımı fırlatması
  entry_id:  76098611
  entry_writer: ratm214
  entry_fav_count: 497
  entry_content: |
    berat albayrak’ın renginin attığını bu mesafeden görmek mümkün.
  entry_channels: siyaset,haber
- entry_name: |
    malatya'daki mevcut üniversite adını anmayacağım
  entry_id:  76091448
  entry_writer: condenser mikrofona tukuren sarkici
  entry_fav_count: 422
  entry_content: |
    isabet olmuş dediğim açıklama. keşke atatürk’ün de adını hiç o ağzına almasa. eğreti duruyor zira.
  entry_channels: siyaset,tarih,haber,eğitim
- entry_name: |
    yemeksepeti.com'un yorum manipülasyonu
  entry_id:  76090524
  entry_writer: meth
  entry_fav_count: 370
  entry_content: |
    yemek siparişi verilen bir platformda "sümük gibi" yazıp bu yorumun kabul göreceğini bekleyen, hırsını kaba ve saldırgan bir üslup ile çalışandan çıkaran bir adet tüketici barındırır.<br/><br/>haklı olmayı bile beceremiyorsunuz.
  entry_channels: haber,yeme-içme
- entry_name: |
    17 nisan 2018 bahçeli'nin erken seçim çağrısı
  entry_id:  76087964
  entry_writer: emesdos
  entry_fav_count: 337
  entry_content: |
    bu ülkede seçim tarihlerine sadece devlet bahçeli’nin karar veriyor olması çok ilginç gerçekten.
  entry_channels: siyaset,haber
- entry_name: |
    dört kelimeyle ilk buluşmanı mahvet
  entry_id:  76087507
  entry_writer: zamazingonun tam ortasi amazing
  entry_fav_count: 304
  entry_content: |
    "çabuk olursak; karım bekliyor"
  entry_channels: anket,ilişkiler
- entry_name: |
    bank asya müdür yrd'nın spk'ya başkan olması
  entry_id:  76087005
  entry_writer: zelsuk
  entry_fav_count: 241
  entry_content: |
    gerçekten çok enteresan bir durum. yahu bank asya'da en ufak hesabı olan adamı işinden, evinden ediyorsun. orada 16 yıl yöneticilik yapmış adamı spk'ya başkan atıyorsun. 16 yıl arkadaş 16 yıl. adamın %1 bile olsun fetullahçı olmama ihtimali var mı? <br/><br/>aynı adam herhangi muhalif bir partiye üye olsa bugün tüm gazetelerde "eski fetöcü yönetici x partisine üye oldu, fetö x partisi el ele" diye manşetler çıkar, 45 farklı tartışma programında saatlerce tartışılırdı.<br/><br/>hadi bunları geçtim diyelim ki bu beyefendi tertemiz, pırlanta gibi bir adam, 80 milyonluk memlekette kimse mi kalmadı? spk'ya başkan olabilecek fetullah bağlantısı olmayan ikinci bir aday hiç mi yok? vah ki ne vah...vallahi ayakta alkışlıyorum.<br/><br/>ek: veresiyesatan nickli yazar'ın altta belirttiği üzere beyefendinin kardeşi (bkz: <a class="b" href="/?q=zehra+ta%c5%9fkesenlio%c4%9flu">zehra taşkesenlioğlu</a>)'imiş. ne diyelim sözün bittiği yerdeyiz, türkiye için hayırlı uğurlu olsun.
  entry_channels: siyaset,haber
- entry_name: |
    barış özcan
  entry_id:  76085290
  entry_writer: turkaz
  entry_fav_count: 240
  entry_content: |
    bilginin fetö'su götösu yoktur.doğruysa ve güzelse alınır,kullanılır.henüz ideolojik veya siyasi bir video yayımlamadı.çok yararlı videoları var. başarı var ortada,bok atmayın lütfen.
  entry_channels: siyaset,sanat,teknoloji,haber,eğitim
- entry_name: |
    26 ağustos 2018 cumhurbaşkanlığı seçimi
  entry_id:  76088379
  entry_writer: patryn
  entry_fav_count: 216
  entry_content: |
    bakın erdoğan seçilir seçilmez ya da başka şeyler olur neyse de, bıktık kardeşim. sürekli seçim; sürekli erdoğan posterleri, kafasının üzerinde bir evet. zangır zangır miting arabaları kılıçdaroğlu'nun boydan yururken fotoğrafları... yüzde bir iki alan partilerin yer yer karşılıklı apartmanlara astıkları bayraklar, her akşam ana haberlerde atışma görüntüleri...<br/><br/>ülke koalisyonlar döneminde bile bu kadar seçime gitmiyordu. bu ne yangından mal kaçırma hevesi. tamam yüzde 45 var, bir 5 puan daha bulursunuz alın ne yaparsınız yapın. zaten alamasaniz bile aldı gösteriyorsunuz. <br/><br/>yeter artık seçim seçim...
  entry_channels: siyaset,haber
- entry_name: |
    devlet bahçeli'nin yazacağı kitaba isim önerileri
  entry_id:  76092078
  entry_writer: grashof
  entry_fav_count: 188
  entry_content: |
    (bkz: <a class="b" href="/?q=gavgam">gavgam</a>)
  entry_channels: anket,ekşi-sözlük,edebiyat
- entry_name: |
    seçim pusulasına zeka sorusu koymak
  entry_id:  76093090
  entry_writer: sen ansin yasanan zaman
  entry_fav_count: 183
  entry_content: |
    ciddi ciddi hayata geçirilmesi gereken bir eylem.her önüne gelen ülkenin geleceği ile ilgili kararlar verememeli.mesela 2+2 kaçtır koysan emin ol güzel günler görürüz.
  entry_channels: siyaset,bilim
- entry_name: |
    10 km koşmak
  entry_id:  76087174
  entry_writer: tyrion degil islevi
  entry_fav_count: 178
  entry_content: |
    ortalama tempoda bir saatten biraz fazla süren aktivite. 2 günde bir kendisine 1 saat vakit artıramayan insan yaşamıyordur, farkında değildir.
  entry_channels: spor,sağlık
- entry_name: |
    nisan 2018 siemens rezaleti
  entry_id:  76096700
  entry_writer: sedatnet
  entry_fav_count: 151
  entry_content: |
    3 haftalık süreçte başıma gelen dolandırıcılık olayı. başlığı siemens adına açtım zira ilk muhattap siemens bayii olmasına rağmen, süreçte siemens türkiye'nin de olaya dahil olduğu ve dolandırıcılıkta payı olduğu kanaatindeyim.<br/><br/>süreç 28 mart çarşamba günü atılıp grup resmi adıyla siemens sincan bayii'nden satın aldığım davlumbaz ile başlıyor. bahsi geçen bayii ile daha önceden alışveriş yapmışlığım olduğu için herhangi bir ön araştırma yapmadan ve siemens adına da güvenerek alışveriş yapmayı sorgulamak aklıma gelmedi. ürünü bana 2 nisan pazartesi günü teslim edeceklerini söylediler ancak 2 nisan pazartesi günü ürün teslimatı yapılmadı. mesai saati bittiği için ancak 3 nisan salı günü bayiye ulaşmak için aradım. aradığımda telefona çıkan hanimefendi, durumu not aldığını kendisinin bir malumatı olmadığını ve bize satışı gerçekleştiren atila bey e ileteceğini söyledi. aynı zamanda siemens in 444 lü müşteri hizmetleri hattını arayarak teslimatın yapılmadığını bildirdim.<br/><br/>sonrasında siemens genel müdürlük şikayet ile ilgili geri dönüş yaptı ve satın aldığım ürünün seri numarasını, satın aldığım bayi ismini vs. gibi sorular yöneltti ve durumu araştırıp bana geri dönüş yapacaklarını belirttiler.<br/><br/>bu süreçte bayi den herhangi bir geri dönüş gelmedi ve ben bayii'yi tekrar aradığımda yine atila bey'e ulaşamadım ve yine not bırakmak durumunda kaldım.<br/>3. günün sonunda bayiyi tekrar aradığımda yine atila bey yoktu ama bu sefer daha önceden konuştuğum hanımefendi, ürünün stokta olmadığını ve bu yüzden gecikme yaşandığını belirtti. aynı gün içinde siemens genel müdürlükten de aranarak, ürünün stokta olmadığını ve bayiinin ürün stoğa gelir gelmez teslimatını yapacaklarını söylediler.<br/><br/>aynı zamanda ev tamiratı işleri ile de uğraştığımdan, bu gecikmenin çok da sorun yaratmayacağını, gerekli şikayetleri yaptığımı(durumu genel merkeze bildirdiğimi) de düşünerek 1 hafta boyunca herhangi bir temasta bulunmadım.<br/>1 hafta geçtikten sonra evde tamirat bitmek üzere ve hala ses çıkmadığından bayiye tekrar ulaşmak istedim ve yine atila bey'e ulaşamadım. not bıraktım ve her zamanki gibi atila bey bana geri dönüş sağlamadı. siemens müşteri hizmetlerini tekrar arayarak durumu bildirdim ve bir an önce durumun araştırılıp bana geri dönüş yapılmasını istedim. genel müdürlük ertesi gün beni arayarak bayinin söylediği lafı tekrar etti. ürünün stokta olmadığını ve stoğa girince teslimatının yapılacağını belirtti.(büyük ihtimalle bayiye sorup, bayinin verdiği cevabı papağan gibi aynısını bana söylediler, stok durumunu ve siparişin kayda geçilip geçilmediğini araştırmadılar) <br/>ertesi gün durumdan iyice şüphelenip başka bir siemens bayiinden aynı ürünün genel depo stoklarında olduğu teyitini aldım ve bunun üzerine bayiiye tekrar ulaşmak için telefon açtım. bu sefer atila bey e ulaştım ve bana ürünün stokta olduğunu ve hemen ertesi gün (14 nisan cumartesi) ya da en geç 16 nisan pazartesi günü teslimatının yapılacağını söylediler.<br/>düne kadar yani 16 nisan pazartesi günü akşam saatlerine kadar bekledim ve tahmin edin ne oldu? ürün gelmedi. bunun üzerine tekrar siemens müşteri hizmetlerine 4. kez şikayet bildiriminde bulundum. bir yandan da gün boyu bayiye ulaşmak için telefonu elimden düşürmedim. telefonlarıma cevap gelmeyince hemen bayiiye gittim.(şu ana kadar bayiye çok uzakta olduğu için gitmemiştim. bayinin yerinde yeller esiyordu. daha 3 gün önce konuştuğum mağaza kepenkleri indirmiş, dükkanın içini boşaltmıştı. dolandırılmıştım.<br/><br/>bu olayda bayinin beni dolandırılmasında tepkisiz kalan, sipariş ve stok durumunu araştırmadan bana 'ürününüz stokta yok o yüzden teslimat yapılamıyor' diye yanlış bilgi veren siemens genel merkezi de sorumludur. benim yaptığım şey kapısında siemens bayii yazan bir mağazayea güvenip alışveriş yapmamk istememdi. bunun dışında hiç bir kusurum olmamsına rağmen, hatta teslimat yapılmadığı andan itibaren şüphelenip genel merkezi uyaran kişi olmama rağmen siemens genel merkezi duyarsız kalmış, bayiinin yalan beyanının arkasında durup, bayinin beni dolandırmasına yataklık etmiştir. ben siemens ismine güvenerek o mağazaya girdim ama bundan sonra siemens mağazasının önünden geçmem. <br/><br/>bugün itibarıyla genel merkez beni tekrar arayarak, bayi ile ilişkilerinin bittiğini ve olayın hukuk birimine intikal ettiğini ve şu aşamada birşey yapamadıklarını söylediler. <br/><br/>yeni gelişmeler oldukça güncelleyeceğim.<br/><br/>edit: geçmiş olsun dileğinde bulunan herkese teşekkürler.<br/>edit 2: atila'nın ilk vukuatı değilmiş. siemens bu olayı zamanında araştırsaymış bugünkü olay olmazdı.<a rel="nofollow noopener" class="url" target="_blank" href="https://forum.donanimhaber.com/siemens-sincan-atilim-group-teslimati-yapmiyor-siemens-de-yardimci-olmuyor--128391990" title="https://forum.donanimhaber.com/siemens-sincan-atilim-group-teslimati-yapmiyor-siemens-de-yardimci-olmuyor--128391990">şikayet</a><a rel="nofollow noopener" class="url" target="_blank" href="https://www.sikayetvar.com/atilim-grup" title="https://www.sikayetvar.com/atilim-grup">şikayet 2</a> <a rel="nofollow noopener" class="url" target="_blank" href="https://www.google.com.tr/search?q=atılım+grup&amp;npsic=0&amp;rflfq=1&amp;rlha=0&amp;rllag=39973842,32737246,13480&amp;tbm=lcl&amp;ved=2ahUKEwjl-amzycHaAhXMyaYKHbQbDD8QtgN6BAgAEDE&amp;tbs=lrf:!2m4!1e17!4m2!17m1!1e2!2m1!1e2!2m1!1e3!3sIAE,lf:1,lf_ui:2&amp;rldoc=1#lrd=0x14d33a865259dc31:0xd5a880d46ff3d39d,1,,,&amp;rlfi=hd:;si:15395696976065713053;mv:!1m3!1d51241.407417883274!2d32.737246649999996!3d39.93717099999999!2m3!1f0!2f0!3f0!3m2!1i515!2i201!4f13.1" title="https://www.google.com.tr/search?q=atılım+grup&amp;npsic=0&amp;rflfq=1&amp;rlha=0&amp;rllag=39973842,32737246,13480&amp;tbm=lcl&amp;ved=2ahUKEwjl-amzycHaAhXMyaYKHbQbDD8QtgN6BAgAEDE&amp;tbs=lrf:!2m4!1e17!4m2!17m1!1e2!2m1!1e2!2m1!1e3!3sIAE,lf:1,lf_ui:2&amp;rldoc=1#lrd=0x14d33a865259dc31:0xd5a880d46ff3d39d,1,,,&amp;rlfi=hd:;si:15395696976065713053;mv:!1m3!1d51241.407417883274!2d32.737246649999996!3d39.93717099999999!2m3!1f0!2f0!3f0!3m2!1i515!2i201!4f13.1">şikayet 3</a>
  entry_channels: anket
- entry_name: |
    diziyle makas atan maganda
  entry_id:  76097897
  entry_writer: karaktersiztospa
  entry_fav_count: 143
  entry_content: |
    böylelerinin yaşamaması lazım.<br/>gerçekten tespit edilip yakalanması ve uyutulması yada kısırlaştırılıp üremelerinin engellenmesi gerekli.<br/>elinde tesbih,direksiyonu bırakmış ben izlerken gerim gerim gerildim.<br/>hiç suçu günahı olmayan kendi halinde yoluna giden,tatile giden,ailesi ile keyifli vakit geçirmek için bir yerlere giden insanlara çarpsa ,ekmeğinin peşinde oradan oraya koşturan insanlardan birine çarpsa<br/>onların ardında bırakacağı insanlara yazık.<br/>küfür etmek istemiyorum ama sövülmeyecek gibi değil.<br/>umarım kendi başını yer pezevenk kimseye zarar vermeden.
  entry_channels: haber,otomotiv
- entry_name: |
    seçimde iyi partiye oy vermek
  entry_id:  76100276
  entry_writer: gundelikyasaminhickiriklari
  entry_fav_count: 117
  entry_content: |
    sorsan herkes abi oy verelim de ortada parti yok der.<br/><br/>parti kurarsın muhalefeti bölecek der. <br/><br/>ve dahi ortada seçim dahi henüz olamamıştır
  entry_channels: siyaset,anket
- entry_name: |
    30 yaşından sonra anlaşılan gerçekler
  entry_id:  76095785
  entry_writer: butterflytruelove
  entry_fav_count: 94
  entry_content: |
    hiç kimsenin ilgisine ihtiyaç duymadığın gün olgunlaşırsın. hiç kimseden beklentiye girmediğin gün yara almazsın. hiç kimseye bağımlı kalmazsan kazanırsın. (bkz: <a class="b" href="/?q=osho">osho</a>)
  entry_channels: anket,ilişkiler,eğitim
- entry_name: |
    chp'nin erken seçime hodri meydan demesi
  entry_id:  76088969
  entry_writer: koministimam2
  entry_fav_count: 83
  entry_content: |
    (bkz: <a class="b" href="/?q=ulan+bi+aday%c4%b1+belirleseydiniz+yaa">ulan bi adayı belirleseydiniz yaa</a>)<br/><br/>ıktidar gibi bir dertleri olmadığı için sorun yok! sonuçta her şartta bülenttezcanın koltuğu kendisine tahsisli!<br/>böyle muhalefetin içine sıçıyayım emi!
  entry_channels: siyaset,haber
- entry_name: |
    michael jackson'ın 1992 yılındaki sahne gösterisi
  entry_id:  76097202
  entry_writer: tridine bandim
  entry_fav_count: 79
  entry_content: |
    en önemlisi kimsenin elinde telefon yok ve herkes o anın tadını çıkarıyor.
  entry_channels: sanat,müzik
- entry_name: |
    ingiliz aksanı vs amerikan aksanı
  entry_id:  76081708
  entry_writer: genkei
  entry_fav_count: 71
  entry_content: |
    amerikan aksanı daha kolay anlaşılır. ingiliz aksanı ise kulağa güzel gelir.
  entry_channels: anket
- entry_name: |
    bir erkeğin hazırlanıp evden çıkma süresi
  entry_id:  76096748
  entry_writer: hicbisy
  entry_fav_count: 69
  entry_content: |
    8.30 kalkış.<br/><br/>8.28 de hazırlanıp çıkmış oluyorum.
  entry_channels: anket,moda
- entry_name: |
    angeln cattle'nin sansürsüz meme frikiği
  entry_id:  76101594
  entry_writer: akbaba gorunumlu yilan
  entry_fav_count: 69
  entry_content: |
    ulan nasıl da kandırdın ama hepimizi. nasıl da düştük tufaya elimiz sikimiz de kaldı bak görüyor musun. napçaz şimdi. şu zekaya bakar mısın ya. vay be. helal olsun valla.
  entry_channels: anket,yeme-içme
- entry_name: |
    2018 cumhurbaşkanı adayınız
  entry_id:  76095473
  entry_writer: houston
  entry_fav_count: 64
  entry_content: |
    fikrim hiç değişmedi..<br/><br/>(bkz: <a class="b" href="/?q=cem+uzan">cem uzan</a>)
  entry_channels: siyaset,anket
- entry_name: |
    26 ağustos 2018 cumhurbaşkanlığı seçimi sonuçları
  entry_id:  76092485
  entry_writer: webyusuf
  entry_fav_count: 51
  entry_content: |
    meral ana’nın güneş gibi doğacağı seçimdir.
  entry_channels: siyaset
- entry_name: |
    tayyip erdoğan'ın yazacağı kitaba isim önerileri
  entry_id:  76078604
  entry_writer: giovinco
  entry_fav_count: 47
  entry_content: |
    ben kimim ya!
  entry_channels: anket
- entry_name: |
    iyi parti'nin ağustos 2018 seçimlerine girememesi
  entry_id:  76096040
  entry_writer: pardus m
  entry_fav_count: 47
  entry_content: |
    türk tarihi böyle hain görmedi.<br/><br/>(bkz: <a class="b" href="/?q=devlet+bah%c3%a7eli">devlet bahçeli</a>)<br/><br/>edit: bu iddia doğru mudur, değil midir bilmiyorum. sanırım değilmiş. ancak bahçeli'nin hain olduğu %100 doğrudur.
  entry_channels: siyaset
- entry_name: |
    mesleğinin adını söylemeden anlat
  entry_id:  76086221
  entry_writer: djdigy
  entry_fav_count: 46
  entry_content: |
    ayın 15 i.
  entry_channels: anket
- entry_name: |
    devlet bahçeli
  entry_id:  76088800
  entry_writer: koministimam2
  entry_fav_count: 45
  entry_content: |
    bir parti lideri düşün ki adam ülkedeki karışıklığı çözebilecek tek kişinin başka bir parti lideri olduğunu söylesin ahaagaha:))
  entry_channels: siyaset
- entry_name: |
    yazarlara whatsapp'tan gelen son mesaj
  entry_id:  76095511
  entry_writer: bir yalnizlik isareti
  entry_fav_count: 39
  entry_content: |
    "x hanım istegnz pantul geldi"<br/><br/>sanırım demet akalın ile mesajlaşıyorum.
  entry_channels: anket,ilişkiler
- entry_name: |
    erkeğin en seksi yeri
  entry_id:  76078831
  entry_writer: carriebradshaw
  entry_fav_count: 35
  entry_content: |
    her yeri ya da hiçbir yeridir.<br/><br/>herkes her şeyi öğrendi de sözlük yazarları seksiliğin tavırdan geldiğini öğrenemedi. <br/><br/>erkeğin en seksi yeri kollarıdır diyelim. bir adama aşık oldunuz ama çiroz, adamda kol yok. o zaman o adam size seksi değil mi? <br/>ya da bakışları dediniz. ortamda boş boş konuşan adamın bakışları size ne kadar tesir eder? ne bakıyor bu bana salak salak dersiniz en fazla.<br/><br/>seksilik koldan, bacaktan, popodan gelmez. ya aşktan gelir ya sağlam duruştan, tavırdan. <br/>hadi genellemeden kaçınayım, benim için böyledir diyerek yumuşak kapatayım.
  entry_channels: anket,ilişkiler
- entry_name: |
    renault clio
  entry_id:  76084665
  entry_writer: bahves
  entry_fav_count: 21
  entry_content: |
    clio kullanıcısı değilim, ancak yiğidi öldür hakkını ver, vw grubu gibi insanların kanser olmasına sebep olacak şekilde zehirli gazları limit dışı salınım yaparak değil, düşük salınımlar ile düşük yakıt tüketimi sağlayabilmiş bir markanın aracı. vw ne gık demeyen yavşak gelmiş sert plastik lab lab diyor, yahu diğeri insanları, toplumların kansere yakalanma riskini misli misli arttırdı, arttırıyor, ülkemizde geri çağrılan vw 1 tane yok, sert plastiği pamuk diye bir tarafınıza tıkasınlar
  entry_channels: otomotiv
- entry_name: |
    idam gerektirecek suçlar listesi
  entry_id:  76083036
  entry_writer: burakowski
  entry_fav_count: 19
  entry_content: |
    (bkz: <a class="b" href="/?q=%c3%a7ocuk+istismar%c4%b1">çocuk istismarı</a>)
  entry_channels: siyaset,anket
- entry_name: |
    winamp
  entry_id:  76096619
  entry_writer: koca nickli zenci
  entry_fav_count: 13
  entry_content: |
    20 yılı aşkın süredir kullandığım ve hala hakkında en ufak bir eleştirimin olmadığı aşmış program. bilgisayar alınınca ilk kurulacak programlardandır benim için. asla yarı yolda bırakmaz, az yer kaplar, muhteşem performans verir. daha iyisi elbet vardır ama winamp gibisi yoktur.
  entry_channels: müzik,teknoloji
- entry_name: |
    tüm türkiye'nin yanlış telaffuz ettiği kelime
  entry_id:  76080614
  entry_writer: nickiminadiferiha
  entry_fav_count: 12
  entry_content: |
    vayfi
  entry_channels: anket,edebiyat
- entry_name: |
    son bir kere ak parti'ye oy vermek
  entry_id:  76102350
  entry_writer: feridun abi
  entry_fav_count: 12
  entry_content: |
    son bi lanet olası iş adamım anlıyo musun?
  entry_channels: siyaset
- entry_name: |
    bedelli askerlik
  entry_id:  76097483
  entry_writer: seiryuu
  entry_fav_count: 12
  entry_content: |
    ilk seferde 45 bin lira + sağ böbrek olarak çıkmalı, bedelli yapanlar ömür boyu köle olmayı kabul etmeli, çektikleri kredilerin en az 3 katını ödemeli, otobüslerde arkada ayrılmış koltuklara oturmalı ve ön tarafa asla geçmemeli, yılda bir de generallere sakso çekmeli gibi gerizekalı öneriler sunulan konudur. toplumsal adaletin katliymiş. her şey çok adil ya toplumda bi biz katledicez sanki.
  entry_channels: siyaset,haber,ekonomi
- entry_name: |
    johnny sins'in türk abur cuburlarını denemesi
  entry_id:  76079517
  entry_writer: blue islands
  entry_fav_count: 11
  entry_content: |
    aq keli, bi s.kmediği top kek kalmıştı.
  entry_channels: yeme-içme
- entry_name: |
    sevişme başlatıcı salak diyaloglar
  entry_id:  76083809
  entry_writer: ourgodgivenrighttobeararms
  entry_fav_count: 10
  entry_content: |
    yazılanlardan anladığım kadarıyla çift olunca zeka yaşı 0-6 bandında seyrediyor.
  entry_channels: anket,ilişkiler
- entry_name: |
    #engelleyemezsiniz
  entry_id:  76098560
  entry_writer: tourin
  entry_fav_count: 9
  entry_content: |
    türkiye iyi olacak #engelleyemezsiniz
  entry_channels: siyaset,haber
- entry_name: |
    ingiliz aksanının çok gıcık olması
  entry_id:  76100120
  entry_writer: fenahuyluspazo
  entry_fav_count: 8
  entry_content: |
    ingilizlere bu konuda ben biraz acıyorum. senin anadilini, senin ülkenden başka bir kıt'aya göçüp orada dejenere bir şekilde konuşanlar, politik iklimde kendilerinin baskın hale gelmesiyle, aslında senin olan şeyi kendilerine mâl etmişler.<br/><br/>yani aslında komik ve gıcık olan amerikan aksanıdır, ingilizlerin konuştuğu şekil esastır. ama millet ingilizlere güler. elin nijeryalısı amerika'da tahsil görüp ingiltere'de şekil yapar filan, garip.
  entry_channels: 
- entry_name: |
    ekşi itiraf
  entry_id:  76082837
  entry_writer: anarax
  entry_fav_count: 8
  entry_content: |
    perşembe sabahı 9.30'da beş yıllık meslek hayatımda beni en çok etkileyen dosyanın karar duruşmasına gireceğim ve neredeyse iki haftadır uyku tutmuyor. <br/>yapılması gereken her şey yapıldı, vicdanım son derece rahat. ancak tahliye beklememekle birlikte içimde ufacık da olsa bir umut var. içimi dökmek istedim ama nereden nasıl başlayacağımı beceremedim, bilemedim. yapamıyorum.<br/><br/>bir cuma sabahı, akşam hayatındaki en unutulmaz güne dair kabusu yaşayarak sabah gözaltına alınacağından habersiz şekilde dört yaşındaki kızını öperek, eşiyle rutin haliyle vedalaşarak çıkan genç bi' adam iki yıla yakın süredir tutuklu. medya organlarında bangır bangır vatana ihanet etmekle darbeci diye itham edilen on binlerce askerden birisi, üsteğmen. hikayesini de anlatmaya mecalim yok ancak mesleki tecrübem, mesleğim üzerine konuşarak o kadar eminim ki suçsuz bir adam. normal şartlarda soruşturma aşamasında bile tutuklama talep edilmemesi gerekirken 21 aydır ailesinden çok uzaklarda. kızı cezaevinde camın arkasından babasıyla telefon yardımıyla konuşurken dahi babasını görevde sanıyor...<br/>iki haftada bir eşi, kızı, annesi, babası ve iki kardeşini görebiliyor, haricinde tutuklu arkadaşları dışında her hafta gördüğü tek kişi bendim. her hafta duruşma ile ilgili rutin çalışmalarımız dışında dışarıdaki hayatı anlattım ona. esasında anlatmak içimden gelmese de o soruyordu ve ben de haliyle cevaplamak durumunda kalıyordum. görüşün sonunda "dışarının kıymetini bil" diyerek gözlerime baktığı ve vedalaştığımız anlarda işi goygoya vurup "hee abi hee yaa" diyerek geçiştirmeye çalışsam da cezaevinden çıktıktan sonra her defasında yaklaşık elli kilometrelik yolu türkü dinleyip burnumu silerek gelmemin sebebi budur. yanisi, ne kadar sikindirik şeylere üzüldüğümü idrak etmem, üzerine istisnasız her defasında düşünmem, cuma günü saat 18'den sonra ankara'ya tepeden inerken yaptığım rutin eylem oldu.<br/><br/>kendim için bişi istemiyorum, bu dosyadaki zaman kaybım ve maddi zararımı dahi hesaba katmazsak, neredeyse aldığım iki kuruş ücret için dahi pişmanlık duyacak seviyeye gelmişken akıl sağlığımı kaybetmememin tek yolu bu dosyada müvekkili beraat ettirmemdir. umudum az da olsa istinafta, o da olmadı yargıtay'da beraat etmek durumunda. aksi halde bu mesleğe dair içimde kalan son kırıntılar da böyle gidecek ve işimi iyi yaptığıma gönülden inanmama rağmen paramparça olup bitireceğim.<br/><br/>yapılması gereken her şey yapıldı, bundan sonrasında hem kendim hem müvekkil hem de benzer durumdaki insanlar için tanrıya sığınmaktan gayrısı aklıma gelmiyor.<br/><br/>yardım et, lütfen...
  entry_channels: anket,ilişkiler,ekşi-sözlük,edebiyat
- entry_name: |
    hala evinde televizyon bulunan insan
  entry_id:  76100164
  entry_writer: sag seritten giden solcu
  entry_fav_count: 6
  entry_content: |
    ne yapam filmleri çük kadar ekranda mı izleyem? <br/><br/>ayrıca günümüzde çoğu tv akıllandı. en basitinden netflix diye bir şey var. illa çatıdan gelen yayını alman gerekmiyor.
  entry_channels: anket,tv,teknoloji
- entry_name: |
    erkek yazarlardan kadın yazarlara sorular
  entry_id:  76080490
  entry_writer: delfina
  entry_fav_count: 6
  entry_content: |
    erkek parfümü önerir misiniz? (koklamak bana da iyi gelsin size de)<br/>depresyondan nasıl çıkarsınız?<br/>+terapi alıyorum ben depresyona girdiysem (parfüm bilmem, en güzeli ten kokusudur. bkz: <a class="b" href="/?q=feromon">feromon</a>)<br/><br/>şiir lazım mı?<br/>+arada lazim bence ya<br/><br/>1-istediğiniz şey olmayınca neden çirkefleşiyorsunuz?<br/>+öyle bişey görmedim kendimde de etrafımda da<br/>2- neden alttan alan erkeğe, efendisiymiş gibi davranıyorsunuz?<br/>+bi bok yemiş de ondan alttan alıyorsa, efendisi gibi davranmak olmuyo o. alttan almana neden olacak boku yeme o zaman hacı.<br/>3- futbolda türkiye’de bu sene kim şampiyon olur?<br/>+başakşehir olmasın da. gerisine okeyiz<br/><br/>sekste en çok hangi pozisyonu seviyorsunuz ?<br/>+her birini ayrı seviyoz. <br/><br/>oral seksi partneriniz için mi kendiniz için mi yapıyorsunuz ?<br/>+seksin ruhuna aykırı bi soru. seksi böyle bölmüyoruz, her yol aynı yere birlikte çıkar. iki kisinin tek olmak isteyerek efor sarfettigi bi eylemdir bu. yumurta-tavuk ilişkisi gibi<br/><br/>neden size iyi davranan erkeklere nazaran, size değer vermeyen erkekleri seçiyorsunuz?<br/>+valla öyle seçmiyoruz. ben ve ekurim yani.<br/><br/>eksi sözlüğe çapkınlık yapmak için girermisiniz?<br/>+ahah. sesli güldüm (yaş 37)<br/>gerçi ben 25 yaşından beri buralardayim ama şükür o derece yokluk çekmedim hiç <br/><br/>evli veya sevgilisi olan bir erkeğin size daha cazip gelmesinin mantıklı bir açıklaması var mıdır? <br/>+asla cazip gelmiyor. gelenlere (yani bunu kroniklestirmislere) nasıl geliyor cidden anlamiyorum. <br/>(bilenler <a class="b" href="/?q=daddy+issue">daddy issue</a>diyor bu işe)<br/><br/>çorapsız etek sezonu ne zaman açılıyor ?<br/>+bence mayıs gibi<br/><br/>polisten koca olur mu ?<br/>+bence erkekten koca olmaz total olarak. kocalık (evlilik) muessesine toptan muhalifim<br/><br/>çuval gibi çantalarınızın içinde,aradığınız şeyi şap diye ilk seferde nasıl buluyorsunuz?<br/>+valla bulamıyorum ben<br/><br/>tanışmak için nasıl bir giriş cümlesi güzel olurdu ?<br/>ve reddetmezdiniz ? ( en önemli soru)<br/>+dışarda nasıl tanışılır bilmiyorum. ben tanismam. ama bir ortamda tanışacağınız hatun ilk anda çarptıysa, elini sıkarken kararli bir şekilde tutun ve (başkası tanıştırıyor olsa bile) gözlerinin içine içine bakarak guclu bir sekilde "merhaba ben x" deyin. tanistiran kisi adinizi soylemis olsa bile bunu yapin. güçlü ve kararlı duruş bizde yankı bulan biseydir genel olarak. (sonra da altını doldurabiliyorsanız anlamlı tabii bu)<br/><br/>parfüm/deodorant önerisi?<br/>+valla bilmem.<br/><br/>çekiçi gelen erkek özelliği ?<br/>+yok onun stabil bi formülü lan. valla bak. 10+ yildir kaslı adam severim zannederken, ebeminkini tersten görmeme sebep olacak kadar körkütük aşık olduğum herif 104 kg, hafif göbekli filan.<br/><br/>erkekte sevdiğiniz giyim kombini nedir bu yaz sizin önerilerinizi değerlendireceğim özellikle ekşi sözlük kadınlarının zevklerine güveniyorum.<br/>+klişe ama, gönül sevdiyse çuval giyseniz okeyiz. hatta okeysek bisey giymemenizi tercih ediyoruz. eheh.<br/><br/>arada tazmanya canavarına dönüşmeyi nasıl başarıyorsunuz?<br/>+naapiyosunuz da dönüşüyoruz acaba?<br/><br/>hayatınızda biri varken aldattınız mı.<br/>+ortalama 25 yaş altında yediğimiz boklar hep maalesef. <br/><br/>neden.<br/>-fantezilerimiz uyuşmadığı icin seksüel ilgimi kaybetmiştim adama karşı ben. milyon farklı sebep olabilir. <br/><br/>yakışıklı bi aslan erkeğiyle sevişmek ister misiniz?<br/>+burcuna bu sekilde vurgu yapan hicbir erkek, allame-i cihan olsa çekici gelmez bence bize<br/><br/>herhangi bir erkekte değil, hoşlandığınız/sevdiğiniz/aşık olduğunuz erkekte en itici bulduğunuz nokta nedir?<br/>+cinsiyetçi kafa gorursem buz gibi sogurum bi anda gibi geliyor (ilk aklıma gelendi. bu tip adama ilgi sürmez zaten bu durumda)<br/><br/>cidden merak ettiğim bir diğer nokta sözlük veya internet ortamında dallamalık (sevişmek ister misin, hede hödö) yapan bir tipe sempatik bulup cevap verip vermediğiniz.<br/>+veren olur illaki. ama bence gençtir ve sebepleri başkadır, sempati duyma olayi degil yani (unutmak istediği biri vardır, rövanş almak istediği bi pozisyon vardır, ilgi aranası vardır, vs)<br/><br/>(seçerek şeyetmedim. bugunkulerin hepsini sıradan şeyettim)
  entry_channels: anket,ilişkiler,ekşi-sözlük
- entry_name: |
    beşiktaş'ı sevme nedenleri
  entry_id:  76083487
  entry_writer: mysticwarrior
  entry_fav_count: 5
  entry_content: |
    8 yaşından beri tribünlerde olan bir "deli" fenerliyim ben ama beşiktaş'ı severim..<br/>çünkü o süleyman seba'dır, metin -ali-feyyaz'dır benim için. <br/>çarşı'dır.<br/>ve babamın tuttuğu takımdır.
  entry_channels: spor,anket
- entry_name: |
    lübnan'ın türkiye'den daha önde olduğu konular
  entry_id:  76093754
  entry_writer: intromental
  entry_fav_count: 4
  entry_content: |
    alfabetik isim sırasında türkiye'den öndedir.
  entry_channels: siyaset,anket,seyahat
- entry_name: |
    şahit olmayı çok isterdim denen tarihi olaylar
  entry_id:  76083344
  entry_writer: skarlet ohara
  entry_fav_count: 2
  entry_content: |
    bu dileğiniz gerçek olsa, bir çok olay aslı öyle kitaplarda yazıldığı gibi olmadığı için, şahit olmayı umduğunuz şeye şahit olamayabilirsiniz.
  entry_channels: anket,tarih
- entry_name: |
    rte'ye rakip olacak tek siyasetçi
  entry_id:  76101698
  entry_writer: cocugunuokuldankawasakiylealanadam
  entry_fav_count: 1
  entry_content: |
    bir anlaşılmadı gitti bu ülkede (artık), namuslunun kazanacak rakibi daha namuslu, namussuzun kazanacak rakibi daha namussuz olmalıdır.
  entry_channels: siyaset,anket
- entry_name: |
    ömür boyu nafakanın kaldırılması
  entry_id:  76102644
  entry_writer: yazar dedigin benim gibi olur
  entry_fav_count: 0
  entry_content: |
    hakkaniyet gözetilerek düzenlemek yapılacakmış. <br/><br/>haberde geçen cümlelerden biri:<br/><br/>"bu ölçütler ''evli kalma süresi”, “çocuk olup olmaması”, “boşanan kadının yaşı”, “gelir durumu” ve “kusur durumu” şeklinde belirlenmiş durumda."<br/><br/>olması gereken buydu. burada bunu yorumlarken cinsiyet veyahut taraftarı olunan akımı belirtmeye gerek yok. bu kadar aferin bağımlısı olmayın.
  entry_channels: ekonomi
- entry_name: |
    mehmed bir cihan fatihi
  entry_id:  76099776
  entry_writer: benden sana
  entry_fav_count: 0
  entry_content: |
    final yapacağı açıklandığına göre akp seçmeninin osmanlı'ya sahip çıkmadığını gösterir. hani lan neden izlemediniz?
  entry_channels: siyaset,tv,sanat,tarih,spoiler
- entry_name: |
    kadın yazarların erkek yazarlardan farkı
  entry_id:  76095541
  entry_writer: gunumuz aylagi
  entry_fav_count: 0
  entry_content: |
    entry’lerinde asla ama asla (bkz: <a class="b" href="/?q=y%c3%bcr%c3%bcyed%c3%bcr+%c3%a7ocuk">yürüyedür çocuk</a>) demezler.
  entry_channels: anket,ekşi-sözlük
- entry_name: |
    ekşi sözlük dertleşecek insan veritabanı
  entry_id:  76079806
  entry_writer: ozman
  entry_fav_count: 0
  entry_content: |
    bana lüleburgazı gezdirecek pasif geyler ve/veya bayanlar yeşillendirebilir.
  entry_channels: anket,ilişkiler,ekşi-sözlük
---
