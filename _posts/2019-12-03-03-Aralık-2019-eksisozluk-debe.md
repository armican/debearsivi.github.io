---
layout: post
title: 03 Aralık 2019 Ekşi Sözlük Debe
data:
- entry_name: |
    selahattin demirtaş'ın bilincinin kapanması
  entry_id:  98572127
  entry_writer: bir preposition olarak la
  entry_fav_count: 1056
  entry_content: |
    bir insanın siyasi düşüncesi ne olursa olsun, bu yapılan olacak iş değil.<br/><br/>türkiye insan hakları açısından bunların acısını ileride çok çekecek.<br/><br/>zamanında işkence gayet normal karşılanıyordu bu topraklarda. onların da acısını çok çektik. adam gibi adalet, yargılama ve infaz sisteminin gelmesi gerekiyor.<br/><br/>size pastor nie moeller'in sözünü hatırlatıyorum. adalet herkese lazım.<br/><br/>"önce yahudiler için geldiler, sesimi çıkarmadım, çünkü ben yahudi değildim sonra komünistler icin geldiler, sesimi çıkarmadım çünkü komünist değildim. sonra sendikacılar için geldiler, sendıkacı olmadığım için yine sesimi çıkarmadım<br/><br/>sonra benim için geldiler ses çıkaracak kimse kalmamıştı..."<br/><br/>edit: “adalet” dedim diye linç etmeye çalışan yazarlar var özel mesajla. ben teröristmişim. ananızdır terörist! ben köprüde mehmetçik kafası kesmedim, ben habur’da terörist karşılamadım, karşılayanı da alkışlamadım, siz bedelli yaparken ben askerliğimi delikanlı gibi yaptım, ben megri megri söylemedim. ben “adalet” dedim. biz yaşadık bu adaletsizliği kuddusi okkır’la, türkan saylan’la. ben kişi bağımsız adalet istiyorum, adı umrumda değil. isteğiniz aslında bu topraklarda ölümlerin durmaması. bununla besleniyorsunuz. şehitler üzerinden siyaset yapmak kolayınıza geliyor klavye başında. bu adalet bir gün size de lazım olacak unutmayın. hiç bir zaman atatürk’ün hayal ettiği muasır medeniyetler seviyesine ulaşamamızın sebebi sizsiniz. kalıba göre şekil alıyorsunuz şu anki moda milliyetçilik. nerdeydiniz lan habur rezaleti yaşanırken? siz “hocaefendi” derken ben “fetö” diyordum. o zaman da “hocaefendi” demek modaydı değil mi? omurgasızlar sizi! gelmiş 18 yaşında velet bana burada terörist diyor. ben gözü kapalı g3 söküp takma denetlemesine girerken senin anan senin altını değiştiriyordu lavuk!
  entry_channels: siyaset,sağlık,haber
- entry_name: |
    sabahın altı buçuğunda kalkıp işe giden insanlar
  entry_id:  98559463
  entry_writer: ucikibir
  entry_fav_count: 469
  entry_content: |
    o zaman otuz beş gece kalkayim da köle olduğum anlaşılmasın.
  entry_channels: ilişkiler,ekonomi
- entry_name: |
    ben kimim ve burada ne yapıyorum denilen anlar
  entry_id:  98550564
  entry_writer: prettyreckless
  entry_fav_count: 374
  entry_content: |
    gece yarısı okuldan dönerken <sup class="ab"><a title="(bkz: bitirme projesi)" href="/?q=bitirme+projesi" data-query="bitirme projesi">*</a></sup> bulunduğum dolmuşa kavşakta çöp kamyonu çarptığı için camın içeri, bana doğru patlaması, hastanelik olmam, o hengamede kendimi düşünmeyip içinde proje olan bilgisayarım için ağlamam, bir şeyimin çıkmaması, akabinde o saatte karakola ifade vermeye götürülmem, şekerim düştüğü için balgat'taki zeugma'dan künefe alınması, titrek bir floresan ışığın altında, karşımda suratsız polisler rapor yazarken kaza geçirmiş biri olarak pembe pofuduk beremle ayaklarımı sallayarak künefenin peynirini hüplettiğim an.
  entry_channels: anket,ilişkiler
- entry_name: |
    erdoğan'ın termik santral yasasını veto etmesi
  entry_id:  98577335
  entry_writer: anaximendos
  entry_fav_count: 357
  entry_content: |
    ulan zaten akparti çıkarttı yasayı ak parti genel başkanı da veto etti. madem veto edecektin niye meclise gönderdin, meclisten gönderdin neden veto ettin? arkadaşlar bakın reisin düşen popülüterisini parlatma çalışmaları bunlar <a rel="nofollow noopener" class="url" target="_blank" href="https://twitter.com/ntv/status/1201503297179410432?s=19" title="https://twitter.com/ntv/status/1201503297179410432?s=19">buyrun</a><br/><br/>edit: başlığı biz açtık nasıl düştük aşağı ya? eyyyyyyy sözlük<br/><br/>edit 2 : bu açıklamayı cumhurbaşkanlığı makamı gereği neden ibrahim kalın veya fahrettin altun değil de akp sözcüsü yaptı? olması gereken beştepe'den açıklama yapılmasıydı ama devletimiz devlet olmaktan çıktığı için devlet adabı da kalmadı
  entry_channels: siyaset
- entry_name: |
    migros'ta 169 tl'ye satılan 2014 yılbaşı paketi
  entry_id:  98574363
  entry_writer: 8832ch78
  entry_fav_count: 267
  entry_content: |
    şu an o fiyata sadece sepet alınabiliyor.<br/>içindekileri hayal ediyorsunuz.
  entry_channels: tarih,yeme-içme,ekonomi
- entry_name: |
    2 aralık 2019 c. başkanlığı seçim anketi
  entry_id:  98569657
  entry_writer: remaker
  entry_fav_count: 258
  entry_content: |
    anketleri pek sevmem ama imamoğlunun tayyibin önünde olduğunu sol tarafta akepeli yazarların yana yakıla açtığı propaganda başlıklarından anlıyorum.
  entry_channels: siyaset,anket,haber
- entry_name: |
    aldatmayan erkeğin olmaması
  entry_id:  98567693
  entry_writer: deathfuckingangel
  entry_fav_count: 254
  entry_content: |
    aldatmayan erkek vardır fakat onu seven var mıdır?<br/><br/>edit; imla.
  entry_channels: ilişkiler
- entry_name: |
    5 milyon dolar vs yaşam boyu uçabilme özelliği
  entry_id:  98550242
  entry_writer: sukusukubaba
  entry_fav_count: 247
  entry_content: |
    ömür boyu uçmayı seçsem kesin ibnenin biri keyif için beni havada vurur.en iyisi her zaman ki gibi para.
  entry_channels: anket
- entry_name: |
    marifet sanılan ama marifet olmayan şeyler
  entry_id:  98550462
  entry_writer: keskeozgurolsaydim
  entry_fav_count: 215
  entry_content: |
    psikolojik rahatsızlıklardan herhangi birine sahip olmak. bakın bunlara sahip olmak marifet değildir, insanlar bundan muzdariptir. sagda sola abi anksiyetem var yha diye gezenlerin tam ağzının ortasına vurup ulan kaç kere stresten tüm vücudun kasıldı da yürüyemedin hödük demek istiyorum.
  entry_channels: anket
- entry_name: |
    2 aralık 2019 galatasaray twitter paylaşımı
  entry_id:  98580513
  entry_writer: ahmetkostarika
  entry_fav_count: 215
  entry_content: |
    bunun nesi ağlamak?<br/><br/>çıkarıp şak diye vurmuş masaya admin. <br/><br/>gs'nin yediği golde sörloth arkadan itiyor. itmeyi maç sırasında göstermiyor yayıncı kuruluş. var hakemi gördü mü belli değil.<br/><br/>üstüne bir de telif nedeniyle görüntüyü vermiyor. <br/><br/>galatasaray da ağlamıyor, bir de penaltısının verilmediği bu zor deplasmanda beraberlik golünü atıyor. sonra da ironi içeren bir tweet atıyor. <br/><br/>ağlama görmek isteyen cumartesi akşamına dönsün.
  entry_channels: spor
- entry_name: |
    anasınıfında dini eğitime başlanması
  entry_id:  98572399
  entry_writer: allegro molto
  entry_fav_count: 207
  entry_content: |
    imam hatiplerle istenilen itaat seviyesine ulaşılamadığı görülmüş olacağından başlanılan uygulama. bir de 'dinimi seviyorum, öğreniyorum' denmiş. hangi dinin hangi mezhebi öğretilecek, bu belirtilmemiş. hoş, hepimiz biliyoruz hangisi olduğunu...<br/><br/>yazık şu memlekette doğan çocuklara.
  entry_channels: siyaset,haber,eğitim
- entry_name: |
    asgari ücrette birçok ab ülkesinden daha iyiyiz
  entry_id:  98573335
  entry_writer: y
  entry_fav_count: 175
  entry_content: |
    asgari ucretlilerin tum calisanlar icindeki orani da soylenmezse eksik olan aciklama. avrupada yuzde 5, 10 flndir. asgari ucret dedigin vasifsiz genc bireyin kisa sure icin aldigi maas olmasi gerekirken ulkenin yuzde 60i asgari ucret aliyor asgari ucretin altinda alanlar, memurlar da olmasa yuzde 95 herhalde. yakinda asgari ucrete yuzde 20 geri kalan kesime yuzde 5 zam gele gele ücretli çalışan herkes asgari ucretle calismaya baslicak. maasda komunist fiyatlarda kapitalist ulke oluyoruz.
  entry_channels: siyaset,haber,ekonomi
- entry_name: |
    6 ay askere gitmemek için 36 ay kredi ödemek
  entry_id:  98559930
  entry_writer: kolasinli
  entry_fav_count: 170
  entry_content: |
    4 gün tatile gitmek için 12 ay kredi ödeyenler, 156 gün askere gitmemek için 36 ay kredi ödeyenleri eleştirir olmuş.<br/><br/>not : 307 ksd.
  entry_channels: anket,ekonomi
- entry_name: |
    bıyıksız ve sakalsız da yakışıklı olabilen erkek
  entry_id:  98568453
  entry_writer: kontraktil koful
  entry_fav_count: 139
  entry_content: |
    hem sakallı hem de sakalsız halimle çirkin olduğum için benle alakası olmayan erkektir.
  entry_channels: anket,ilişkiler,moda
- entry_name: |
    muayene edilen hastanın sosyal medyadan yürümesi
  entry_id:  98582538
  entry_writer: lisw
  entry_fav_count: 101
  entry_content: |
    doktorum ben.<br/><br/>konusu bir türlü açılmayınca belirteyim dedim.
  entry_channels: ilişkiler,sağlık
- entry_name: |
    2 aralık 2019 beşiktaş kayserispor maçı
  entry_id:  98550314
  entry_writer: ismini vermek istemeyen anonim
  entry_fav_count: 94
  entry_content: |
    beşiktaş'ımın ibrahim üzülmez'in ayrılışından bu yana çıkacağı 291. lig maçı. <sup class="ab"><a title="(bkz: swh)" href="/?q=swh" data-query="swh">*</a></sup>
  entry_channels: spor
- entry_name: |
    neden tekrar ay'a gitmiyoruz
  entry_id:  98568862
  entry_writer: ridanes
  entry_fav_count: 75
  entry_content: |
    yüksek maliyet ve düşük hatta sıfır getiri yüzünden.<br/><br/>soğuk savaş döneminde gidilme amacı şovdu.
  entry_channels: anket,bilim,teknoloji,sağlık,ekonomi,havacılık
- entry_name: |
    ekrem bey'in alkollü ve şatafatlı sofrası
  entry_id:  98550344
  entry_writer: biladerinho
  entry_fav_count: 69
  entry_content: |
    bu tarz görüntülere devam. sike sike eski yaşantımıza geri döneceğiz.
  entry_channels: siyaset,haber,magazin
- entry_name: |
    2 aralık 2019 yurtiçi kargo açıklaması
  entry_id:  98583564
  entry_writer: dusakabinn
  entry_fav_count: 61
  entry_content: |
    "evde yoksunuz işte amk" politikasından "şubede yeterli adam yok amk" politikasına geçmiş bulunan şirketin güzel açıklamasıdır. kara cuma sadece bize var. çin'den verdiğim sipariş 2 haftada geliyor ancak anadolu yakasından avrupa yakasına sipariş gelemiyor.
  entry_channels: ekonomi
- entry_name: |
    isminden zenci olduğu anlaşılan futbolcular
  entry_id:  98569954
  entry_writer: pastaban
  entry_fav_count: 57
  entry_content: |
    (bkz: <a class="b" href="/?q=sabire+melte+mbanko">sabire melte mbanko</a>)
  entry_channels: spor,anket
- entry_name: |
    ekşi'den kız düşürme yöntemleri
  entry_id:  98579916
  entry_writer: goarrk
  entry_fav_count: 56
  entry_content: |
    evli ve 3 çocuklu güzide yazarımızın, sanal alemde kız düşürme çabalarına karar vermesi ve yol yordam öğrenmeye çalışması <br/><br/>olarak değişebilir mi başlık? <br/><br/>abi uzatma işte senin niyetin, bi hatun düşürüp, yengeye yakalanmadan işi götürmek ama yakalarlar canım abim sen bu sevdadan vazgeç..
  entry_channels: anket,ilişkiler,ekşi-sözlük
- entry_name: |
    ankara soğuğu
  entry_id:  98559852
  entry_writer: mehtapta demlenen hamsi
  entry_fav_count: 51
  entry_content: |
    ölüyü bır daha öldürür amk.
  entry_channels: seyahat
- entry_name: |
    istanbul trafiğinin sebepleri
  entry_id:  98563362
  entry_writer: fenasi kertmen
  entry_fav_count: 50
  entry_content: |
    makasçı orospu çocukları ile, gideceği yönün tersinde olan şeridi boş görüp oraya dalan ve sonra oradan çıkmaya çalışan orospu çocuklarıdır.<br/><br/>koduğumun çocuğu sağ şerit boşsa ileride bir şey var demektir bir sen misin uyanık kancık?<br/>piç, en soldan en sağa dalar sonra yavaş giden kamyon konvoyu ve arızalı araç karşısına çıkınca en sola geçmek için akan trafiği felç eder.
  entry_channels: anket,otomotiv,motosiklet,seyahat
- entry_name: |
    evli kadınlardan bekar kadınlara tavsiyeler
  entry_id:  98581781
  entry_writer: timetable
  entry_fav_count: 49
  entry_content: |
    kusura bakma da sen eğer bu maddeleri kaleme alıp kocişinle beraber bunu noterde onaylatıp, üstüne bir de uygulatıyorsan ben senin kocişinin taa bilmem neresine...<br/><br/>dünyayı kendi etrafında dönüyor zanneden yazar kezbanı evliliği kocasına olup olmadık isteklerini kabul ettirmek zannediyor belli ki. bir de utanmadan buraya maharetmiş gibi bekarlara tavsiye diye yazıyor. %100 de çalışıyormuş bak sen. siz evlilik kurumunu böyle aptal saptal şeylerle ayağa düşürdükçe ömür boyu mutlu olamayın diyesi geliyor insanın tövbe tövbeee...
  entry_channels: ilişkiler
- entry_name: |
    ytü'nün akıllara zarar giriş kapısı
  entry_id:  98583544
  entry_writer: ciel phantomhive
  entry_fav_count: 42
  entry_content: |
    <a class="b" href="/?q=br%c3%bctalizm">brütalizm</a> etkisinde tasarlanmış üniversite giriş kapısı. gayet beğendiğim bir kapı olmuştur ayrıca.<br/><br/>unutulmamalıdır ki adı <a class="b" href="/?q=yt%c3%bc">ytü</a> diye kısaltılan bu üniversite bir teknik üniversitedir. ve mutlak <a class="b" href="/?q=teknokrasi">teknokrasi</a> ile bağdaştırdırdığım bir mimari ile tasarlanmış. son olarak diyebilirim ki minimal ve işlevsel.<br/><br/>(bkz: <a class="b" href="/?q=br%c3%bctalist+mimari">brütalist mimari</a>)
  entry_channels: sanat,eğitim
- entry_name: |
    babadan öğrenilen en kıymetli şey
  entry_id:  98550492
  entry_writer: blue req
  entry_fav_count: 42
  entry_content: |
    babalık yapamayacaksan çocuk yapma.
  entry_channels: anket,eğitim
- entry_name: |
    19. yüzyıldaymış gibi yaşayan çift
  entry_id:  98568133
  entry_writer: akrostisbizimisimiz
  entry_fav_count: 42
  entry_content: |
    videonun altındaki "this is the highest level of hipster ı’ve ever seen in my life" yorumuyla kahkaha attırmıştır.
  entry_channels: ilişkiler
- entry_name: |
    aleyna tilki'nin dahi olduğunu iddia etmesi
  entry_id:  98578848
  entry_writer: eeuummm
  entry_fav_count: 39
  entry_content: |
    madde etkisinde bende aynı hisleri yaşıyorum . sonra düşüş oluyor bütün bu hisler gidiyor hiçlik hissi ortama hakim oluyor.<br/><br/>kokaini azaltmasını tavsiye ediyorum.
  entry_channels: magazin
- entry_name: |
    50 trilyon dolar vs yaşam boyu mutluluk
  entry_id:  98576769
  entry_writer: stajyer epistemolog
  entry_fav_count: 35
  entry_content: |
    50 trilyonla neler yapılabileceğinden haberi olmayan yazarın versusudur.
  entry_channels: 
- entry_name: |
    evlenilecek insan bulamama nedenleri
  entry_id:  98550815
  entry_writer: metsamor magduru
  entry_fav_count: 32
  entry_content: |
    bu kadar mücadele edip savaşıp tırnakların ile kazıyarak sabırla bir yere geldikten sonra öyle bu hayata ortak edecek insan bulmak zor. karşımdaki insan için özellik saymaya gerek yok. ben neysem o da denk olsun yeter.
  entry_channels: anket,ilişkiler
- entry_name: |
    ekşi itiraf
  entry_id:  98550446
  entry_writer: sessiz sakin suskun adam
  entry_fav_count: 30
  entry_content: |
    küçükken iki tur bineyim diye arkadaşın bisikletini almıştım. bisiklet yeni bisikletti ve o gün nasıl olduysa herkese izin verdi. bindim bisiklete daha birinci tur bitmeden hızlıca geri dönüp "al sen bin" diye arkadaşım ekrem'e verdim. ekrem aldı sürdü sürdü sonra yarı yolda inip eliyle tuta tuta geri döndü. bisikletin sahibine "kusura bakma lastiğini patlattım" dedi. ben de sinirlenip ekrem'e bağırdım "ya eko, anasini siktimin yerinde sürmesini bilmiyorsan isteme abi şu bisikleti. nolcak şimdi heh, amina koydun lan bisikletin" dedim.<br/><br/>oysa lastiği ben patlatmıştım lan.<br/><br/>bu olay beni çok üzdü. günlerce kabus gördum. geceleri kan ter içinde uyandım. sonunda vicdanımın sesine dayanamayıp eko'ya tüm gerçeği söyledim demeyi çok isterdim ama mışıl mışıl uydum lan hiç sikimde olmadı.<br/><br/>aradan yıllar geçti bir gün hem icip hem birasına pişti oynarken eko'ya gerçeği anlattım. affet beni eko dedim. hatırlamıyorum ki amk dedi. peki dedim yerden kupa bacağıni çaldım. kestikten sonra sinek ikiliyle pişti yapıp allah ne verdiyse kağıtları şak diye alnına yapıştırdım.<br/><br/>ben götün tekiyim lan.
  entry_channels: anket,ilişkiler,ekşi-sözlük
- entry_name: |
    ateist olunan yaş ve şu anki yaş
  entry_id:  98583606
  entry_writer: honanim daber
  entry_fav_count: 28
  entry_content: |
    ne ateizm, ne deizm, yalnizca sensizm...
  entry_channels: anket
- entry_name: |
    hani ekonomik kriz vardı lan
  entry_id:  98583827
  entry_writer: okubakayim
  entry_fav_count: 26
  entry_content: |
    lan zaten oradan belli değil mi ekonomik kriz olduğu? normal zamanda alınamayacak ürünler için senenin bir haftasını bekliyor millet. benim ihtiyacım olan ve daha önce 1250 liraya satılan ürünü 950 liraya sattı hepsiburada. bu zaman almayacağım da ne zaman alacağım? benim gibi nice hikaye var daha.<br/><br/>ek: alınamayacak kelimesi biraz fazla olmuş olabilir ama troll değilseniz anlamışsınızdır demek istediğimi. kendi siparişime dönecek olursam; 950yi veren pezevenk 1250yi veremez mi? evet verebilir ama niye vereyim ki? kazandığım para zaten pul olmuş, elektriğe, gaza, ota boka köküne kadar zammı geçirmişler yine. giderim aradaki 300 lira ile 1 - 1.5 aylık doğalgaz paramı öderim.
  entry_channels: siyaset,ekonomi
- entry_name: |
    akp'lilerin tasarıyı veto eden rte'ye teşekkürü
  entry_id:  98582063
  entry_writer: klozete sicmayan adam
  entry_fav_count: 26
  entry_content: |
    termik santral tasarısını kabul eden akp milletvekilleri tasarıyı veto eden recep tayyip erdoğan'a teşekkür etmiş. <a rel="nofollow noopener" class="url" target="_blank" href="https://t24.com.tr/haber/filtresiz-termik-santral-tasarisini-kabul-eden-akp-liler-tasariyi-veto-eden-erdogan-a-tesekkur-etti,850554" title="https://t24.com.tr/haber/filtresiz-termik-santral-tasarisini-kabul-eden-akp-liler-tasariyi-veto-eden-erdogan-a-tesekkur-etti,850554">link</a> gerçekten şaka gibi diyeceğim de bu apayrı bir boyut. arkadaş size silah zoruyla mı kabul ettirdiler de şimdi veto etti diye teşekkür ediyorsunuz ? ne kadar gülünç duruma düştüklerini kimse söylemiyor mu bunlara ?
  entry_channels: siyaset
- entry_name: |
    bekar erkeğin yaptığı ev temizliği
  entry_id:  98577968
  entry_writer: bulutlari bekleyen
  entry_fav_count: 25
  entry_content: |
    pasaklılığı, tembelliği mizah ve hatta zeka göstergesi sanan tiplerin garipsediği olay.<br/><br/>temiz olmanın bekar ya da öğrenci olmakla alakası yok, öz saygı ile alakası var.
  entry_channels: anket
- entry_name: |
    en beğenilen silah
  entry_id:  98559423
  entry_writer: protomon
  entry_fav_count: 24
  entry_content: |
    (bkz: <a class="b" href="/?q=caps+lock+tu%c5%9fu">caps lock tuşu</a>)
  entry_channels: anket,teknoloji
- entry_name: |
    bankacı kadınlar
  entry_id:  98577302
  entry_writer: yepisyeniydi
  entry_fav_count: 15
  entry_content: |
    normal, gayet sıradan, bildiğin çalışan türk kadınlarıdır.<br/><br/>eskiden daha güzel ve bakımlıydı bunlar aslında. bankalarda ücret seviyesi ve yan haklar tırpanlandıkça o eski bakımlı bankacı kadınlar başka sektörlere göç etti. açlık sınırının az üstünde, yoksulluk sınırı seviyesinde bir maaşla bu kadar oluyor sanırım. <br/><br/>masa başı olsa da çalışma koşulları zor, hedef ve satış baskısı, müşteri kaprisi, müdür amir baskısı... ülkede bu kadar yaygın eğitimli işsiz varken, ilk kriz kokusunda bankalar çatır çatır şube kapatırken ezilse de örselense de kimse sesini yükseltemez zaten. <br/><br/>geneli mutsuz ve umutsuz kadınlardır.
  entry_channels: anket,ekonomi
- entry_name: |
    michael jordan
  entry_id:  98575064
  entry_writer: botulismus
  entry_fav_count: 13
  entry_content: |
    jordan’ın kıyaslanabileceği oyuncular <br/>michael phelps <br/>michael schumacher<br/>usein bolt <br/>roger federer(gönlümün en iyi tenisçisi)<br/>messi/ronaldo<br/>muhammed ali<br/>bu ve bunun gibi sporcularla dünyanın en iyi sporcusu olarak kıyaslanabilir.<br/><br/>edit: muhammed ali eklendi @balporsugu na teşekkürler
  entry_channels: spor
- entry_name: |
    atm'lerde para bakiyesinin görünmesi saçmalığı
  entry_id:  98580957
  entry_writer: anubismillahirrahmanirahim
  entry_fav_count: 10
  entry_content: |
    haklı bir serzeniştir günümüz türkiyesinde güvenlik sorunun teşkil etmektedir.
  entry_channels: haber,ekonomi
- entry_name: |
    msn + winamp + ultima online + kaşarlı tost
  entry_id:  98570231
  entry_writer: rick wright
  entry_fav_count: 9
  entry_content: |
    telefonumun sağ altında akınsoft belirdi şu an.
  entry_channels: oyun
- entry_name: |
    ekşi sözlük dertleşecek insan veritabanı
  entry_id:  98550287
  entry_writer: russevenadam
  entry_fav_count: 9
  entry_content: |
    anıtkabir’deki ismet inönü'nün mozolesi gibiyim. kimse fark etmiyor beni...
  entry_channels: anket,ilişkiler,ekşi-sözlük
- entry_name: |
    en hüzünlü sezen şarkısı
  entry_id:  98583260
  entry_writer: imlaci
  entry_fav_count: 7
  entry_content: |
    (bkz: <a class="b" href="/?q=seni+kimler+ald%c4%b1">seni kimler aldı</a>)
  entry_channels: anket,müzik
- entry_name: |
    fetöcü denince akla ilk gelen şey
  entry_id:  98578715
  entry_writer: laagland
  entry_fav_count: 4
  entry_content: |
    akp
  entry_channels: siyaset,anket
- entry_name: |
    izmir soğuğu
  entry_id:  98552444
  entry_writer: softshell turtle
  entry_fav_count: 3
  entry_content: |
    yahu 20 senedir izmir'de yaşıyorum. ege bölgesinden hiç dışarı çıkmadınız herhalde izmir'i soğuk sanıyorsunuz.
  entry_channels: anket,sağlık
- entry_name: |
    atiba hutchinson
  entry_id:  98585939
  entry_writer: parliament night blu ray
  entry_fav_count: 2
  entry_content: |
    beşiktaş ilçesinin en işlek yerine <a class="b" href="/?q=%c5%9f%c3%bckr%c3%bc+g%c3%bclesin">şükrü gülesin</a>, <a class="b" href="/?q=r%c4%b1za+%c3%a7al%c4%b1mbay">rıza çalımbay</a> ve <a class="b" href="/?q=ibrahim+%c3%bcz%c3%bclmez">ibrahim üzülmez</a> ile birlikte heykelinin dikilmesi gereken futbolcu. çalışkanlık, takım aidiyeti ve karakter timsali bir adam. gidince çok özleyeceğiz.
  entry_channels: spor,anket,ilişkiler,tv,ekşi-sözlük,bilim,sanat,tarih,programlama,müzik,sinema,edebiyat,oyun,teknoloji,sağlık,haber,yeme-içme,otomotiv,motosiklet,moda,magazin,eğitim,ekonomi,havacılık,spoiler,seyahat
- entry_name: |
    üst komşuya söylenmek istenen şeyler
  entry_id:  98555534
  entry_writer: oooooonur
  entry_fav_count: 1
  entry_content: |
    şimdiki üst komşuyu tanımıyorum. sesi de gelmiyor. <br/><br/>eski üst komşuya ise şunları söylemek ve sormak istiyorum: <br/><br/>tanıdığım kadarıyla sen iyi bir insandın. duygulu biriydin. <br/><br/>seni instagram'dan takip ediyorum. <br/><br/>neden sürekli kedini değiştiriyorsun? <br/><br/>bir tanesini biliyorum. anneliği tatmasını istiyorum dedin. tipini daha çok sevdiğin yavruyu aldın, diğerlerini bir şekilde dağıttın, anne kediyi ise sokağa bıraktın. <br/><br/>sonra bu devran sürdü, durdu.<br/><br/>dinine bağlı olduğunu söylüyorsun. günah değil mi bu yaptığın peki?<br/><br/>seni artık bir arkadaş olarak görmediğimi biliyor musun? <br/><br/>bu tutumunu gören bir çok kişinin senden uzaklaşacağını tahmin edebiliyor musun?
  entry_channels: anket
- entry_name: |
    yazarların en çok korktukları film
  entry_id:  98585073
  entry_writer: ezik
  entry_fav_count: 1
  entry_content: |
    the exorcist
  entry_channels: anket,sinema
- entry_name: |
    ballon d'or 2019
  entry_id:  98580780
  entry_writer: life is just
  entry_fav_count: 1
  entry_content: |
    futbol tarihinin gelmis gecmis ve gelecekteki en iyi futbolcusu <a class="b" href="/?q=lionel+messi">lionel messi</a>'nin kazanacagi odul. <br/><br/>2 aralik 2019 paris saati ile 20:30'da verilecek.
  entry_channels: spor
- entry_name: |
    en son alınan iltifat
  entry_id:  98553608
  entry_writer: hknnl
  entry_fav_count: 0
  entry_content: |
    you killed my pussy
  entry_channels: anket,ilişkiler
- entry_name: |
    bira içmelik arkadaş veritabanı
  entry_id:  98587150
  entry_writer: winteringus
  entry_fav_count: 0
  entry_content: |
    besiktasta olanlar yesillendirebilir <br/><br/>24/f
  entry_channels: anket,ilişkiler,ekşi-sözlük,yeme-içme
---
