---
layout: post
title: 21 Haziran 2019 Ekşi Sözlük Debe
data:
- entry_name: |
    halanın ölmemesi
  entry_id:  91496555
  entry_writer: it oynamis yonca masarasi
  entry_fav_count: 1390
  entry_content: |
    (bkz: <a class="b" href="/?q=herkesin+akl%c4%b1na+ayn%c4%b1+ki%c5%9finin+gelmesi">herkesin aklına aynı kişinin gelmesi</a>)<br/><br/>edit: (bkz: <a class="b" href="/?q=jon+snow%27un+halas%c4%b1">jon snow'un halası</a>)
  entry_channels: anket
- entry_name: |
    ben 15 temmuz'u yaşamış bir başbakanım
  entry_id:  91494815
  entry_writer: can simidini caya batirdim
  entry_fav_count: 912
  entry_content: |
    binali yıldırım "başbakanlık tarafından tahsis edilen makam aracını kullanıyor" iddialarına yanıt: ben 15 temmuz darbesini yaşamış bir başbakanım; bu konuların gündeme getirilmesini kınıyorum.<br/><br/>bizde vatandaş olarak 15 temmuz yaşadık vergi, ötv almayın da dostluğumuz pekişsin.<br/><br/><a rel="nofollow noopener" class="url" target="_blank" href="https://t24.com.tr/haber/yildirim-dan-makam-araci-tepkisi-butun-bu-duzenlemeler-benim-disimda-yapiliyor-istesem-de-reddedemem,826925" title="https://t24.com.tr/haber/yildirim-dan-makam-araci-tepkisi-butun-bu-duzenlemeler-benim-disimda-yapiliyor-istesem-de-reddedemem,826925">link</a>
  entry_channels: 
- entry_name: |
    seçim öncesi ido'nun seferleri iptal etmesi
  entry_id:  91490320
  entry_writer: hoper07
  entry_fav_count: 714
  entry_content: |
    bu halk yüzerek gene gelir amk. ne yapsalar boş kaderin üstünde bir kader vardır. <br/>her şey çok güzel olacak.
  entry_channels: haber
- entry_name: |
    1 ak partiliyi ikna et
  entry_id:  91479042
  entry_writer: obenizimo
  entry_fav_count: 641
  entry_content: |
    akıl sağlığını korumak istiyorsan hiç uğraşma. vicdanı olan zaten göreceğini görmüştür,geri kalan partizan kesim zaten boşuna münakaşaya girersin canın sıkılır.
  entry_channels: siyaset
- entry_name: |
    sakalsız erkekler istiyoruz
  entry_id:  91483760
  entry_writer: hector8
  entry_fav_count: 391
  entry_content: |
    benim sakalım, benim kararım.
  entry_channels: ilişkiler,moda
- entry_name: |
    bisiklet sürerken kask kullanmanın gereksiz olması
  entry_id:  91481211
  entry_writer: welebrity
  entry_fav_count: 390
  entry_content: |
    bu fikri savunan dangalağın bir üst versiyonu da scooter gibi düşük cc'li motosikletlere kasksız biner, sonra 20 ile giderken kafası dağılır beyni akıp geberir gider.<br/><br/>bu fikri savunan herhangi bir gerizekalıyı alıp, 10 kilometre hızla duvara vurun. bakın bakalım amı götü mü yokza ağzı götü mü yoksa götü beyni mi yoksa eli kolu mu dağılıyor.<br/><br/>(not: şehir içinde caddebostan sahilyolunda kilitli pedal ortalama 30 km/sa spor)<br/><br/>(ayrıca bkz: <a class="b" href="/?q=do%c4%9fal+salaksiyon">doğal salaksiyon</a>)
  entry_channels: spor,seyahat
- entry_name: |
    terör örgütlerinin destek verdiği cumhur ittifakı
  entry_id:  91493978
  entry_writer: nickson
  entry_fav_count: 340
  entry_content: |
    (bkz: <a class="b" href="/?q=allah+s%c3%b6yletmi%c5%9f">allah söyletmiş</a>)
  entry_channels: siyaset,haber
- entry_name: |
    önce ev mi yoksa araba mı alınmalı sorunsalı
  entry_id:  91485464
  entry_writer: what makes you think i m not a superhero
  entry_fav_count: 321
  entry_content: |
    rahmetli dedemin lafıdır:<br/><br/>"araba ev aldırmaz ama ev araba aldırır."
  entry_channels: ekonomi
- entry_name: |
    seçilse bile vitrin süsü olacak
  entry_id:  91494636
  entry_writer: anani siken kadi
  entry_fav_count: 269
  entry_content: |
    sadece bu da değil, asıl önemli olan devamı.<br/>ne diyor devamında?<br/><br/>"en ufak yanlışta karşısına belediye meclisi dikilecek"<br/><a rel="nofollow noopener" class="url" target="_blank" href="https://eksiup.com/p/zs36986unjp3">https://eksiup.com/p/zs36986unjp3</a><br/><br/>peki ne demek bu? "akp'li birisini seçerseniz yanlışına göz yumulacak, deveyi hamuduyla götürse bile, istanbulu parsel parsel peşkeş çekse bile kimse ona karışamayacak" demek değil mi?<br/><br/>recep tayyip erdoğan kendi ağzı ile, istanbul'da hak yenmesin, yanlışlara göz yumulmasın, hırsızlık ve talan son bulsun istiyorsanız imamoğlunu seçersiniz demiş. <br/><br/>(bkz: <a class="b" href="/?q=rte%27nin+%c5%9fecaatini+s%c3%b6ylerken+sirkatini+arz+etmesi">rte'nin şecaatini söylerken sirkatini arz etmesi</a>)
  entry_channels: siyaset,haber
- entry_name: |
    konya denince akla gelen üç şey
  entry_id:  91484946
  entry_writer: akintiya karsi yolculuk
  entry_fav_count: 254
  entry_content: |
    bilim<br/><br/>sanat <br/><br/>teknoloji
  entry_channels: anket,seyahat
- entry_name: |
    a haber'in yılın haber kanalı seçilmesi
  entry_id:  91491885
  entry_writer: deadgodinme
  entry_fav_count: 216
  entry_content: |
    yarrakçadır ve angutçadır.<br/><br/>başka küfre de gerek olabilir.
  entry_channels: 
- entry_name: |
    20 haziran 2019 nuri bilge ceylan açıklaması
  entry_id:  91495986
  entry_writer: sinemaci kubrick
  entry_fav_count: 210
  entry_content: |
    türkiyemizin en başarılı yönetmenlerimizden biri olan nuri bilge ceylan, marvel hakkında "çocuklarım yüzünden marvel filmlerini seyretmek zorunda kalıyorum, bu da benim cezam." diyerek ona yüklediğimiz sevgiyi bir anda yerle bir etti. üstünde çok emek harcanan marvel iç... ahahahahahhhhh kahrolsun marvel! böyle güzel bir cümleyi nuri bilge ceylan'dan başka kim yapabilirdi ki? az yapmış. hala marvel filmlerinin oscar kazanması gerektiğini düşünen moronlar üzülsün biraz.<br/><br/>edit:<br/>(bkz: <a class="b" href="/?q=moronlar+ne+seyreder">moronlar ne seyreder</a>)<br/>(bkz: <a class="b" href="/?q=marvel">marvel</a>)
  entry_channels: sinema,haber,magazin
- entry_name: |
    binali yıldırım'ın beyaz tv'de cevapları unutması
  entry_id:  91499639
  entry_writer: xaxxbczczaaxax
  entry_fav_count: 176
  entry_content: |
    sunucuların "bilmediğimiz yeni bir müjde var mı?" dedikten sonra binali yıldırım'dan yanıt gelmeyince müjdeleri şakır şakır sıralamaları gibi bir enstantaneye sahne olmuş komik olay.<br/>tapu tahsis belgesiyle ilgili müjde varmış ya binali bey, niye milleti müjdelemiyorsun?<br/><br/>ek: kadın sunucunun "seçime birkaç gün kala" dedikten sonra sanki hiçbir şey bilmiyormuş gibi yanındaki sunucuya dönüp baktığı an yaşadığım utancı tarif edebilmem mümkün değil.
  entry_channels: siyaset,tv,haber
- entry_name: |
    domuzla da arası iyi
  entry_id:  91495660
  entry_writer: p0ltergei5t
  entry_fav_count: 115
  entry_content: |
    aşırı hızla alçalıyorlar. aman iniş takımlarına dikkat.
  entry_channels: siyaset,haber
- entry_name: |
    tayyip erdoğan için ölürüm
  entry_id:  91487047
  entry_writer: no status
  entry_fav_count: 110
  entry_content: |
    (bkz: <a class="b" href="/?q=geberit">geberit</a>)
  entry_channels: siyaset,haber,magazin
- entry_name: |
    06 bfj 470
  entry_id:  91497616
  entry_writer: tavukbanalop
  entry_fav_count: 96
  entry_content: |
    kadriye çavuş isimli kadını, sehirler arasi otobusten darp ederek kaçıranlarin kullandigi arabanın plakası:<br/><a rel="nofollow noopener" class="url" target="_blank" href="https://twitter.com/metinyoksu/status/1141716447707783169" title="https://twitter.com/metinyoksu/status/1141716447707783169">https://twitter.com/…ksu/status/1141716447707783169</a><br/><br/>iç işleri bakanı belediye seçimleri ile ilgilenmeye ara verip konuyla ilgili twit atmış, adamlar yakalanmış: <a rel="nofollow noopener" class="url" target="_blank" href="https://twitter.com/suleymansoylu/status/1141737290986921986?s=20" title="https://twitter.com/suleymansoylu/status/1141737290986921986?s=20">https://twitter.com/…tatus/1141737290986921986?s=20</a>
  entry_channels: haber
- entry_name: |
    otobüsten dövülerek kaçırılan kadın
  entry_id:  91498046
  entry_writer: chess player
  entry_fav_count: 94
  entry_content: |
    neden engel olmuyorlar diyenler isviçre'de yaşadıklarını zannediyor. adam seni çeker vurur, ya da satırı ense köküne indirir, 3 ay sonra da çıkar içeriden, o kızla da barışır ve sen de 3. dünya ülkesi vatandaşı olarak bok yoluna gittiğinle kalırsın. bir kere de şu ekşicilere denk gelmiyor şu olaylar ahhh ah. ah bir gelse neler yapacaklar.<br/><br/>devletin böyle konularda öyle bir tutumu olacak ki, bunu yaparsam yedi ceddimi sikerler benim diye düşünecek yapacak olan. vatandaş belki bir olaya müdahale edebilir, devlet ise komple önünü alabilir. vergi alıyorsa önünü almak da zorunda en azından. içişleri bakanı twitter'da profiline yıldırım koyup ciuv ciuvvvv yapmakla meşgul olduğu için zor tabi o da.
  entry_channels: haber
- entry_name: |
    sözlük erkeklerinin yakışıklılık dereceleri
  entry_id:  91471115
  entry_writer: bi kere ben de oynayabilir miyim
  entry_fav_count: 83
  entry_content: |
    annem bile beni birine anlatırken çok iyi bir insan diye bahsediyor.
  entry_channels: anket,ilişkiler,ekşi-sözlük
- entry_name: |
    recep tayyip erdoğan
  entry_id:  91471878
  entry_writer: jazz cazz
  entry_fav_count: 74
  entry_content: |
    bu kadar insan kinci, gaddar, kibirli, kaba, haydutsa senin yüzündendir bilesin. insanlara bunu verdin, bu ortamı sağladın. ateşin bol olsun.
  entry_channels: siyaset,haber,ekonomi
- entry_name: |
    avengers endgame'in yeniden gösterime girmesi
  entry_id:  91497573
  entry_writer: okaberintarou
  entry_fav_count: 64
  entry_content: |
    milleti silkmenin yeni metodu. iki üç sahne görülsün diye tüm filmin parasını tekrar verecek kerizlere geçmiş olsun şimdiden.
  entry_channels: sinema,haber,spoiler
- entry_name: |
    meral akşener
  entry_id:  91476726
  entry_writer: seylann
  entry_fav_count: 54
  entry_content: |
    şahsi çıkarlarını değil ülkesini düşünmüştür. alınan bu büyük başarıda aslan payı kendisine aittir. tarih de bunu böyle yazacaktır. biz de çocuklarımıza anlatacağız kendisini.
  entry_channels: siyaset,haber
- entry_name: |
    100 bin liraya tuttuğun takımı değişir misin
  entry_id:  91470967
  entry_writer: cristiano ronaldo dos santos aveiro
  entry_fav_count: 50
  entry_content: |
    ben şahsen (bkz: <a class="b" href="/?q=am%c4%b1na+bile+korum">amına bile korum</a>)<br/><br/>ama soru zor. 100 bin pek de büyük bir para olmayabilir çoğu ekşici için, ne de olsa maaş başlığında hepiniz 20k alıyonuz amk.<br/>futbol ve takım kavramları, şeref ve namus seviyesine yüceltildiği için ülkemizde, 100.000'i elinin tersiyle iter bizim koçlar.
  entry_channels: spor,anket
- entry_name: |
    20 haziran 2019 uygurlar için araştırma önergesi
  entry_id:  91499594
  entry_writer: korhaydut
  entry_fav_count: 44
  entry_content: |
    başlığın tam hali uygur türklerine uygulanan zulmün araştırma önergesi.<br/><br/>teklifi yapan (bkz: <a class="b" href="/?q=hdp">hdp</a>) reddeden (bkz: <a class="b" href="/?q=mhp">mhp</a>)<br/><br/><a rel="nofollow noopener" class="url" target="_blank" href="https://youtu.be/pb8z-3Sw884" title="https://youtu.be/pb8z-3Sw884">youtube linki</a><br/><br/><a rel="nofollow noopener" class="url" target="_blank" href="https://youtu.be/HQKe7GrUz0k" title="https://youtu.be/HQKe7GrUz0k">mercedeslerle istanbulun fethi</a>
  entry_channels: 
- entry_name: |
    migrene iyi gelen şeyler
  entry_id:  91490915
  entry_writer: mina harker
  entry_fav_count: 38
  entry_content: |
    patates.<br/>hep yok daha neler derdim ama en son ağrı kesiciler fayda etmeyince... üzerine bir dünya istifra edip, tülbentle hala sıkılı olan kafanın zonklaması devam edince...<br/>yapıştırdım 3 dilim patates ve yarım saate mis. <br/>dedim psikolojiktir ya olur mu öyle saçma şey.<br/>ikinci krizde yine denedim. yine mis.<br/><br/>bu bakımdan patates bir yana dünya bir yana.<br/><br/>edit: merhaba, yoğun istek üzerine;<br/>evet dostlar daire daire kesin patatesleri ve alnınıza yapıştırıp tülbenti bağlayın sıkıca. ister şakaklara ister alnınızın çatına.. 3-4 adet yerleştiriyorum ben. ağrı uzun sürerse yarım saate bir patatesleri tazeleyin. hepimize geçmiş olsun.<br/><br/>edit 2: yapıştırın yahu yemeyin yemeyiin. yiyin tabi de o karın doyurma niyetine olanı. yoksa dünyanın en güzel nimetlerinden kendisi.<br/><a class="b" href="/?q=parasetamol">parasetamol</a> niyetine alnınıza yapıştırın. kafaya ha. yapıştırın.
  entry_channels: anket,sağlık
- entry_name: |
    ekşi itiraf
  entry_id:  91472650
  entry_writer: dandik tavsan
  entry_fav_count: 37
  entry_content: |
    az sonra tüm samimiyetimle yapacağım itiraftır...<br/><br/>sıkı durun... veya durmayın... evet, yasak olmasına rağmen biraz içtim. bira ve votka kafası var. önümüzdeki haftaya kadar boşum. dört günde alkolü atarım bünyemden. tabii ki, yeniden almazsam... içmek istedim. bugün pek bir işim yoktu. sözlük'te takıldım, haberleri okudum, internet üzerinden bazı işlerimi hallettim. ve tedavi gördüğüm kliniğe gittim. çıkmaz ama raporu sorayım, diye. çıkmış! rapor, yarın çıkacak, diye biliyordum ama çoktan hazırlamışlar. aldım. okudum. bir kez daha okudum. <br/><br/>eve geldim. rakı vardı. dutyfree'den ben de hediye yeni rakı alıp getirmiştim. ona dokunmadım. yarım kalan rakıyı bitirdim. sonra votka aldım. votka ve bira. onları da bitirdim. saat daha akşam beş buçuk bile değil. kafam güzel. <br/><br/>evet... nedir itirafım? şudur...<br/><br/>itiraf ediyorum ki... içim rahatladı.<br/><br/>hayır! iyi bir haber almadım. tersine, durum kritik. yanlış anlamadıysam dört günlük kısa bir kemo veya ilaç tedavisi ve aşı olacak. sonrasında, yeniden tahlilller alınacak. tedavi konusunda nelerin yapılacağına karar verilecek. ilerleme olmuş. pek fazla zamanım yok. ne kadar zamanım kaldı, onu temmuz'a doğru söyleyecekler. bu da güzel. beş yıl bana yeter. daha fazlasını istemiyorum. şimdiki halde onun yarısı kadar bile yok, ama aşı işe yararsa daha çok zaman kazanmış olurum. bakalım doktor abi ve ablalar ne diyecek. daha fazla ayrıntıya girmiyorum. <br/><br/>önce, korktum. irkildim. daha sonrasında mala bağladım. bir şey düşünemedim. ardından, aklıma sevdiklerim ve sevenlerim geldi. yine korktum, çok da üzüldüm. insanca bir duygu bu; daha çok yaşamak istiyoruz...<br/><br/>bugün sözlük'te "ne olursa olsun son güne kadar yaşayacağım ve çalışacağım", diye yazmıştım. aynen de öyle yapacağım. diğer yandan, itiraf etmeliyim ki, insanın kendini alıştırmaya çalıştığı durum ile yüz yüze gelmesi korkunç bir tecrübe.<br/><br/>bu, sağlık durumum ile ilgili son girdim olacak. bundan sonrası hep hayat, hep mutluluk, hep umut.
  entry_channels: anket,ilişkiler,ekşi-sözlük
- entry_name: |
    sevilen kişiden vazgeçme eşiği
  entry_id:  91471570
  entry_writer: merakli159
  entry_fav_count: 31
  entry_content: |
    önemsenmediğini, hiç olmazsa senin onu önemsediğin kadar onun seni önemsemediğini fark edip kendine bu saygısızlığı yakıştırmadığın o an.
  entry_channels: ilişkiler
- entry_name: |
    aleksandar kolarov
  entry_id:  91485166
  entry_writer: tevfikken
  entry_fav_count: 28
  entry_content: |
    türk takımları illa (eskimiş) yıldız almak istiyorsa seçenekler belli:<br/><br/>tartışmasız a sınıfında olanlar: (bkz: <a class="b" href="/?q=roberto+carlos">roberto carlos</a>), (bkz: <a class="b" href="/?q=didier+drogba">didier drogba</a>)<br/><br/>a sınıfı kaliteye sahip ama o mertebede pek kalamamış, ukdesi kalan yıldızlar: (bkz: <a class="b" href="/?q=gheorghe+hagi">gheorghe hagi</a>), (bkz: <a class="b" href="/?q=nicolas+anelka">nicolas anelka</a>), (bkz: <a class="b" href="/?q=wesley+sneijder">wesley sneijder</a>), (bkz: <a class="b" href="/?q=ricardo+quaresma">ricardo quaresma</a>)<br/><br/>a sınıfı takımlarda hatrı sayılır dönem geçirmiş, o takımların havasını suyunu kapmış olan b sınıfı futbolcular: (bkz: <a class="b" href="/?q=dirk+kuyt">dirk kuyt</a>), (bkz: <a class="b" href="/?q=adriano+correia+claro">adriano correia claro</a>), (bkz: <a class="b" href="/?q=gael+clichy">gael clichy</a>), (bkz: <a class="b" href="/?q=mario+gomez">mario gomez</a>)<br/><br/>hal böyleyken kolarov fenerbahçe'ye gelirse 3. gruptan gelmiş olacak (zaten şu anki ekonomik şartlar gereği mümkün olabilecek en yüksek kalite bu). kolarov'u küçümsemek için değil, birazdan bahsedeceğim şeye köprü olsun diye belirttim bu grupları. <br/><br/>kendisi şu an fenerbahçe için 'yıldız oyuncu'dan öte bir fırsat çünkü modern futbolun görüp görebileceği en komple oyunculardan biri. <br/><br/>2000'lerin sonlarına doğru stoper fiziği ve gücünde olup aynı zamanda kanat enerjisine sahip fazla bek oyuncusu yoktu, daha çok dani alves, patrice evra gibi kısa boylu ayağına hakim, pırpır çizgi bekleri moda olmuştu, kısacası sol/sağ defans jokeri diyebileceğimiz oyuncu sayısı azdı. kolarov bu yüzden dikkat çekti, aynı zamanda skor katkısı da yüksek bir bek olduğu için önce lazio'ya oradan da man city'ye gitti. yani yetenek/kalitesinden ziyade çokyönlülüğüyle yükseldi, zaten a sınıf bir yeteneği yok. ama uzaktan şut, frikik, akıllı kademe, oyun kurma gibi mevkiisinin ötesine geçen sivri özellikleri var. bugün yaşı 34 değil de (19-20'yi geçtim) 26-27 olsaydı transfer piyasasını alt üst ederdi. kısacası geleceğin bek modeline (bkz: <a class="b" href="/entry/69230594">#69230594</a>) çok uygun bir tarzı var, tıpkı filipe luis gibi yaşı gereği şanssızca dünyanın en iyisi olma fırsatını kaçıranlardan oldu. kısacası hem ayağına hakim, hem akıllı, cüsseli, aynı zamanda (yaşı gereği hantallaşsa da) enerjisi hala fena değil. bunlar bana birini hatırlatıyor:<br/><br/>(bkz: <a class="b" href="/?q=abd%c3%bclcebrail+akbulut">abdülcebrail akbulut</a>)<br/><br/>bu çocuğa bir şeyler öğretmeyi becersin yeter, oyun stilleri benziyor çünkü. abdülcebrail'in ilerde a veya b sınıf bir bek olması elbette zor, ama birkaç farklı görevi birlikte icra edebilip sahada geniş yer kaplama potansiyeli gerçeğe dönüşsün, modern futbolda karşılığı olsun yeter. bunu da kolarov'dan daha iyi verebilecek birini bulamaz (zaten fenerbahçe getiremez). <br/><br/>ancak bunlar sadece <a class="b" href="/?q=hasan+ali">hasan ali</a>'nin gitmesiyle anlam kazanır. eğer abdülcebrail a takıma çıkartılıp kolarov'un yedeği olacak ve 1-2 sene bir şeyler kapacaksa evet kolarov'a verilen paralar göze batmaz, yok yine hasan ali kalacaksa, kolarov gelmesin o zaman, gerçekten diyorum. fenerbahçe şampiyon olacaksa o parayı başka mevkiiye harcayıp yine olur.
  entry_channels: spor
- entry_name: |
    20 haziran 2019 donald trump tweeti
  entry_id:  91497163
  entry_writer: ben ne diyorum sen ne diyorsun
  entry_fav_count: 23
  entry_content: |
    120 milyon dolarlık ihayı, 1980 öncesi sovyet füzesi ile vurdular. varın s400 ü siz düşünün.
  entry_channels: siyaset,haber
- entry_name: |
    hala ölmemesi 2
  entry_id:  91500926
  entry_writer: foxkids
  entry_fav_count: 22
  entry_content: |
    (bkz: <a class="b" href="/?q=hala+%c3%b6lmemesi">hala ölmemesi</a>) adlı başlığı taşıyıp yok eden yönetime bir cevap olarak kafamda kurguladığım sinema filmi.<br/><br/>hala ölmemesi adlı filmin devamı. filmde birisi var ve ölmüyor. halkına zulmediyor. sonra hakkında başlık açılıyor. ama o kişiden hiç bahsedilmiyor (tıpkı şu anda olduğu gibi). sonra bir yönetim geliyor. başlığı yok ediyor.<br/><br/>güzel bir film.
  entry_channels: anket
- entry_name: |
    bir şirkette en fazla kaç yıl çalışmak mantıklıdır
  entry_id:  91485737
  entry_writer: nicksizolmaz
  entry_fav_count: 22
  entry_content: |
    3 yılı aşmamak iyidir. 3 yıl eşiğini geçince risk almak zorlaşıyor, iş değiştirmekten ve yeni bir işe adapte olmaktan korku duyuluyor. <br/><br/>3 yıl şirketteki geleceğinize, kariyer hedefine ulaşabilme ihtimallerinize dair çok şey gösterir.
  entry_channels: anket
- entry_name: |
    fenerbahçe'nin basketbolda gs'nin moralini bozması
  entry_id:  91500489
  entry_writer: ne koysam ki
  entry_fav_count: 14
  entry_content: |
    (bkz: <a class="b" href="/?q=fenerbah%c3%a7e+taraftar%c4%b1n%c4%b1n+galatasaray+kompleksi">fenerbahçe taraftarının galatasaray kompleksi</a>)
  entry_channels: spor
- entry_name: |
    aliexpress'ten alınan ürünlerin gelmemesi
  entry_id:  91488228
  entry_writer: exgavator
  entry_fav_count: 14
  entry_content: |
    son 2 aydır alınanlar hiçbir şekilde gelmiyor<br/>takipli paketi getiren uçak 2 aydır havada duruyor halen inmedi<br/><br/>bazıları sizin haberiniz bile olmadan geri gidiyor<br/><br/>böyle giderse aliexpress karar alarak türkiyeye gönderimi yasaklayacaktır.
  entry_channels: ekonomi
- entry_name: |
    20 haziran 2019 tv8 fenerbahçe winwin yayını
  entry_id:  91483480
  entry_writer: muhendis kizan
  entry_fav_count: 14
  entry_content: |
    ulan bişey demiycem de bizim bileklik ile beşiktaş'ın feda kampanyasıyla dalga geçen taraftar şimdi kendileriyle dalga geçilmesine kızıyor. adamlar pembe dizi gibi her hafta bir kampanya yapıyor amk :)<br/>tanım: kaçıncı bölümünün olduğunu ve sezon finalinin ne zaman yapılacağını merak ettiğim atraksiyon.<br/>edit:final yapsın ondan sonra izliycem :)
  entry_channels: spor,tv
- entry_name: |
    krize rağmen konut fiyatlarının düşmemesi
  entry_id:  91492407
  entry_writer: hep yer hep bakar
  entry_fav_count: 12
  entry_content: |
    kardeşim enflasyon kaç çıktı yüzde 25 mi ?<br/>evet. <br/>daire fiyatları arttı mı yüzde 25 hayır.<br/>geçene seneden bu yana dolar %60 altın gramı %60 artış gösterdi mi?<br/>evet.<br/>o zaman nasıl düşmemis oluyor bu konut fiyatları.<br/>az daha sabredin sike sike tl bazında da düşecek o fiyatlar.
  entry_channels: ekonomi
- entry_name: |
    kadınlardaki küfür eden erkek hayranlığı
  entry_id:  91494484
  entry_writer: umittaciri
  entry_fav_count: 8
  entry_content: |
    aksine küfür etmeyen aranıyor da bulunamıyor. <br/>çünkü gün gelir üçüncü şahıslara edilen o küfürler, size de edilir. o eski <a class="b" href="/?q=istanbul+beyefendisi">istanbul beyefendisi</a> tadında erkek yok artık.
  entry_channels: ilişkiler
- entry_name: |
    imamoğlu'nun reklam filmlerinin çalıntı çıkması
  entry_id:  91500882
  entry_writer: ckitax
  entry_fav_count: 8
  entry_content: |
    baktılar hiçbir şey bulamıyorlar, reklam filmlerine mi sarmaya başlamışlar.<br/>yazık lan size...
  entry_channels: 
- entry_name: |
    bir kadını fiziği dışında çekici yapan detaylar
  entry_id:  91471539
  entry_writer: explorateur
  entry_fav_count: 8
  entry_content: |
    çok düşündüm aklıma memeden başka bir şey gelmedi.
  entry_channels: anket,ilişkiler
- entry_name: |
    genç kesimin tüm aptallığını iktidara yıkması
  entry_id:  91501114
  entry_writer: foxkids
  entry_fav_count: 6
  entry_content: |
    (bkz: <a class="b" href="/?q=entry+nick+uyumu">entry nick uyumu</a>)<br/><br/>bak hele sen bak. soruyu tersten soralım mı seninle?<br/><br/>genç nesilin aptal olmasının nedeni bu iktidar olmasın sakın?<br/><br/>girişim yapmış mıyız da önümüzü mü kesmişlermiş... kaç tane öğrencinin şahane projesi bu iktidar tarafından reddedilip onun yerine götümüze okunmuş kuru fasülye sokarsak ne olur minvalinde projelere yüzbinlerce dolar verildi biliyor musun sen?<br/><br/><a rel="nofollow noopener" class="url" target="_blank" href="https://eksiup.com/p/jt37102gdbrc?del=AgjOrEEq7kNvmw0o3483mMfb6a1n1yV537103" title="https://eksiup.com/p/jt37102gdbrc?del=AgjOrEEq7kNvmw0o3483mMfb6a1n1yV537103">kaçarsa diye</a>
  entry_channels: siyaset
- entry_name: |
    erkek yazarlardan kadın yazarlara sorular
  entry_id:  91472806
  entry_writer: kediyi severek olduren satanist
  entry_fav_count: 6
  entry_content: |
    yurdum insanının bakış açısını, buraya yazılanları okuyarak rahatlıkla görebilirsiniz. hatta ileriye taşıyorum! bu başlıktaki entryleri inceleyerek erkeklerin kadınlar hakkındaki düşünceleri, cinsellik, hayattan beklentiler, yıkıklık vb. konularda sosyolojik tez yazabilecek kadar toplum hakkında fikir sahibi olabilirsiniz.
  entry_channels: anket,ilişkiler,ekşi-sözlük
- entry_name: |
    alaçatı'da 3 top dondurmanın 24 tl olması
  entry_id:  91481551
  entry_writer: venera
  entry_fav_count: 5
  entry_content: |
    10 lirası dondurma ise geri kalanı <a class="b" href="/?q=instagram">instagram</a> telif hakkıdır.
  entry_channels: seyahat
- entry_name: |
    rusya'daki akıl almaz vantilatör
  entry_id:  91501181
  entry_writer: galiloo
  entry_fav_count: 3
  entry_content: |
    şunu görmesem hayatta ne kaybederdim diye düşünüyorum izlediğimden beri.<br/><br/>tanım: başlık olamayacak bir vantilatördür.<br/><br/>uyarı: birbirini şey eden iki heykelciğin hareketiyle çalışan bir adet vantilatör. seks var yani.
  entry_channels: 
- entry_name: |
    ergin ataman'ın ultraslan'ı final maçına çağırması
  entry_id:  91501560
  entry_writer: biraz uzak
  entry_fav_count: 3
  entry_content: |
    iyi ki fenerbahçe’liyim dedirtmiştir. <br/>çağır gergin herkesi çağır. (bkz: <a class="b" href="/?q=amerikan+ba%c5%9fkan%c4%b1+dahil+herkesi+devreye+sokun">amerikan başkanı dahil herkesi devreye sokun</a>).
  entry_channels: spor
- entry_name: |
    şu anda çalan şarkı
  entry_id:  91475944
  entry_writer: jayhawker
  entry_fav_count: 2
  entry_content: |
    <a rel="nofollow noopener" class="url" target="_blank" href="https://www.youtube.com/watch?v=NGVO0MsNqms" title="https://www.youtube.com/watch?v=NGVO0MsNqms">redd - siktiret boşver</a>
  entry_channels: anket,müzik
- entry_name: |
    olgun kadın genç erkek konulu filmler
  entry_id:  91501772
  entry_writer: cevirimanyagi
  entry_fav_count: 2
  entry_content: |
    (bkz: <a class="b" href="/?q=my+friend%27s+hot+mom">my friend's hot mom</a>)
  entry_channels: sinema
- entry_name: |
    baba olmak istemeyen erkek
  entry_id:  91495492
  entry_writer: kozniku
  entry_fav_count: 2
  entry_content: |
    baba olmak isteyen kadar normaldir.<br/>sonradan pişman olabilir. baba olan da pişman olabilir. bunlar hayatta var. herkes bir değil.<br/><br/>burada çocuk sahibi olmak istemeyenlere ağız dolusu sövenlerin çoğu, çocuklarıyla mutsuz insanlardır. bununla yüzleşmeyi, başka bir alternatifin varlığını kabul etmek istemezler. bunu dile getirenlere sayıp sövmeleri bundan.<br/><br/>onaylanmaya ihtiyaçları vardır ve kendilerinkinden başka tercih bu onaylanma ihtiyaçlarına aykırıdır. düşük olan özgüvenleri daha da örselenir.<br/><br/>çocuk sahibi babalara sayıp söven varsa, onlar da aynı dertten muzdariptir.<br/><br/>bir de, nasıl her baba olanın sorumluluk bilinci çok yüksek değilse, baba olmak istemeyen de illa sorumluluk alamıyor demek değildir. bu bir tercihtir.
  entry_channels: ilişkiler
- entry_name: |
    metalstorm'un en iyi metal albümleri listesi
  entry_id:  91499889
  entry_writer: crowley
  entry_fav_count: 1
  entry_content: |
    12 ve 13. sırada <a class="b" href="/?q=agalloch">agalloch</a> denen grubun albümleri olan liste. şaka mısınız?
  entry_channels: anket,müzik
- entry_name: |
    kanser hastasına yardım kampanyası
  entry_id:  91470964
  entry_writer: neil mccauley
  entry_fav_count: 1
  entry_content: |
    destek olunası kampanya
  entry_channels: sağlık,ekonomi
- entry_name: |
    eljif elmas
  entry_id:  91501458
  entry_writer: dominar rygel xvi
  entry_fav_count: 0
  entry_content: |
    menajerliğini babasının yaptığı velet. bu zamana kadar menajerliğini babasının yaptığı kaç futbolcu tanıdıysam hepsi de balon çıktı. babası saygın ve tanınan bir menajer olmadığı için kimse bu elemanı sallamıyor. babası arıyor basını elif'i inter izliyor, tottenham fiyat sordu, milan takip ediyor tarzı söylenti yayıyor millette inanıyor. tipik balkan kurnazı bunlar. zamanında bize böyle ne balkan genci çaktılar süper yıldız olacak biraz gelişsin avrupa devleri kapınızda yatacak diye. hepsi yalan oldu. <br/><br/>burada çıkan haberlere gülüyoruz diye fenerliler kuduruyor. aslanım siz kendi takımınızın maçından başka maç izlemediğiniz için size böyle haberler gerçek geliyordur. burada çoğu insan avrupa liglerini takip ediyor. kimse bu elemana 2 milyon euro bile vermez bunu kafanıza sokun. sonra hayal kırıklığı yaşamayın. <br/><br/>emre mor olayıyla bu farklı. dortmund emre mor'u hızı, deparları ve çalım yeteneği iyi olduğu için aldı. beyinsiz ama çocuk top sürüyor. belki bende daha gelişir oynatırım diye düşündü. elif elmas'ın 20 milyon edecek hangi özelliği var? mal mısınız nesiniz. adam geçemez, top süremez, uzun şut yok, serbest vuruş yok. uzun pas herkes kadar. hayvan gibi faul yapma özelliği para etse bana göre 30 milyon euro da eder. beşiktaş altyapısında oynayan çocuğa yaptığı insanlık dışı faul hala aklımda.
  entry_channels: spor
- entry_name: |
    ekşisözlük oyun oynayacak arkadaş veritabanı
  entry_id:  91500472
  entry_writer: sekersegololur
  entry_fav_count: 0
  entry_content: |
    mantıklı bir sebepten dolayı açılmış veritabanı.<br/><br/>total war serilerinde beraber grand campaing oynamak isteyen arkadaşlar yeşillendirebilir, opsiyonlu olarak pubg lite'da atarız arada.
  entry_channels: 
- entry_name: |
    ekşi sözlük dertleşecek insan veritabanı
  entry_id:  91501607
  entry_writer: crying phoenix
  entry_fav_count: 0
  entry_content: |
    sık sık insanların dertlerini ve neşelerini gece gündüz demeden dinlediğim veritabanıdır. bir çok iyi insanla tanıştım. herhangi anlatacak bir şeyleri olan yazar ve çaylaklarımızı mesaj kutumu yeşillendirmeye bekliyorum. burada herkese yargılamadan yaklaşılır. <br/>telegram,c2 ve ribony için : cryingphoenix (c ve p büyük harf)
  entry_channels: anket,ilişkiler,ekşi-sözlük
---
