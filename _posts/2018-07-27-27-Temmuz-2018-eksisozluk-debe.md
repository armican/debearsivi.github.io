---
layout: post
title: 27 Temmuz 2018 Ekşi Sözlük Debe
data:
- entry_name: |
    sabirech melt embankoch
  entry_id:  79580937
  entry_writer: ataman bogdan khmelntsky
  entry_fav_count: 1122
  entry_content: |
    avusturya 2.liginde oynayan alman forvet. beşiktaş'ın kendisiyle ilgilendiği konuşuluyor. takımı gazlayabilecek bir oyuncu.
  entry_channels: spoiler
- entry_name: |
    türk bayraklı maketten rahatsız olan öğretmen
  entry_id:  79573712
  entry_writer: oguzbatur
  entry_fav_count: 910
  entry_content: |
    ulan bu devlet seni okutmuş, öğretmen yapmış. gözünle görmüşsün, yaşamışsın ki seni ayırmamış diğer vatandaşlarından.<br/><br/>yok ağa yok bunlardaki nankörlük bitmez. umarım öğretmenlikten ihraç edilir, hak etmiyor bu devletten maaş almayı.
  entry_channels: siyaset,haber
- entry_name: |
    bedelliler olarak rütbemiz ne olacak
  entry_id:  79592736
  entry_writer: batuks
  entry_fav_count: 824
  entry_content: |
    (bkz: <a class="b" href="/?q=genelkurmay+ba%c5%9fkan%c4%b1">genelkurmay başkanı</a>)
  entry_channels: siyaset,anket
- entry_name: |
    trabzon'da 300 işçili fındık fabrikası kapandı
  entry_id:  79573907
  entry_writer: kurremkamerruk
  entry_fav_count: 763
  entry_content: |
    fındık üreticisiyim, hortum ve itfaiye ile geliyorum, benim istediğim kişiler seçilmediği için değil, seçenler yanlış sebeplerle insanları seçtiği için, seçenler seçtiklerine hesap soracaklarına bana saldırdıkları için, ülkede olan zırnık kadar iyi iş için seçtiklerini yere göğe koyamayıp ülkenin anası bellenirken ses çıkartmadıkları için.<br/>su veren itfaiyenin hortumunu sikeyim.<br/>kapanmakla kalmamalı fabrika bir an önce avmye çevrilmeli, o olmuyorsa ederinin onda biri fiyata ferreroya devredilmeli, ha o da olmalı bir a 101 açılmalı oraya.<br/><br/>edit: fabrika ferreronunmuş zaten, tabi hıyarlık bende, para kazanan ne ferreronun değil ki anasını satayım fındık işinde.
  entry_channels: siyaset,haber,ekonomi
- entry_name: |
    twitter'da penisli tacizi teşhir eden kadın
  entry_id:  79585928
  entry_writer: kecicim
  entry_fav_count: 517
  entry_content: |
    anneye bak anneye. ben böyle bir olayın içine girsem boyumun yarısı olan annem beni uykumda tek kurşunla skimden vurur şimdi böyle gez derdi. <br/>anne var anne var.
  entry_channels: ilişkiler,haber
- entry_name: |
    amerika'da 849 dolara satılan araba
  entry_id:  79572758
  entry_writer: balthasar van der ast
  entry_fav_count: 461
  entry_content: |
    asıl senin ülkende bunun +25.000 liraya satılması enteresan.
  entry_channels: 
- entry_name: |
    etek giyen kadınlara küfreden yobaz dede
  entry_id:  79590417
  entry_writer: meksika post
  entry_fav_count: 318
  entry_content: |
    eskinin bu tip adamlarında bir naiflik bir ermişlik olurdu. şimdi rüyamıza ak sakallı dede gelse seni anasını s*ktiğimin yobazı diyip döveriz heralde.<br/><br/>(bkz: <a class="b" href="/?q=dedeler+bi+susun">dedeler bi susun</a>)
  entry_channels: siyaset,haber
- entry_name: |
    yazılımcıların genelde özgüvensiz olmaları
  entry_id:  79580357
  entry_writer: hatunlarin tercih etmedigi efendi adam
  entry_fav_count: 274
  entry_content: |
    işim gereği yazılımcılarla birlikte çalışıyorum = bi yazılım ofisinde ofisboy.
  entry_channels: 
- entry_name: |
    enzo roco
  entry_id:  79583795
  entry_writer: bulletproof
  entry_fav_count: 264
  entry_content: |
    şilili ise iyidir. şilili adam demir siker.
  entry_channels: spor
- entry_name: |
    sabire meltem banko
  entry_id:  79580783
  entry_writer: ataman bogdan khmelntsky
  entry_fav_count: 217
  entry_content: |
    eller ayır<a class="b" href="/?q=sabire">sabire</a> <br/>yollar ayır<a class="b" href="/?q=sabire">sabire</a> <br/>yıllar ayır<a class="b" href="/?q=sabire">sabire</a> <br/>biz ayrılamayız
  entry_channels: magazin
- entry_name: |
    26 temmuz 2018 bahçeli'den askıda ekmek müjdesi
  entry_id:  79596098
  entry_writer: faizlbisi
  entry_fav_count: 202
  entry_content: |
    "aç insanlara yardım ettim beni kahraman ilan ettiler, bu insanlar neden aç diye sorgulamaya başladım beni komünist ilan ettiler"<br/><br/>che guevara
  entry_channels: siyaset,haber,ekonomi
- entry_name: |
    chp'de 59 il başkanının genel merkeze desteği
  entry_id:  79584609
  entry_writer: attirmayin makedonun kafasini
  entry_fav_count: 178
  entry_content: |
    19 ilde bile birinci çıkamamış partinin başarısız il başkanlarının başarısız genel merkeze desteğidir.<br/>(bkz: <a class="b" href="/?q=k%c3%b6rler+sa%c4%9f%c4%b1rlar+birbirini+a%c4%9f%c4%b1rlar">körler sağırlar birbirini ağırlar</a>)
  entry_channels: siyaset,haber
- entry_name: |
    türkiye'nin en sonunda 1923 ruhuna dönmesi
  entry_id:  79598980
  entry_writer: 3lly
  entry_fav_count: 148
  entry_content: |
    göt sıkışınca 1923 atlatınca 2023.
  entry_channels: tv,yeme-içme,moda,magazin
- entry_name: |
    golden gate köprüsünde görüntülenen azrail
  entry_id:  79589485
  entry_writer: anonimyus
  entry_fav_count: 148
  entry_content: |
    montaj olabilir inanmayin
  entry_channels: teknoloji
- entry_name: |
    google'ın telefonları dinlemesi
  entry_id:  79591950
  entry_writer: passenger28
  entry_fav_count: 141
  entry_content: |
    şimdi ister inanırsınız ister inanmazsınız. ben,sözlükte zaten genelde ''romantik edebiyat'' yapan bir adamım.öyle teknolojiden de çok anlamam.ama boş gezenin boş kalfası misali bir test yaptım ve sonucu anlatacağım.<br/>internetten ne zaman bir şey arasam facebook'ta yahut başka sitelerde karşıma sürekli o konuyla alakalı reklamlar çıkıyordu.tamam dedim sonuçta google amcaya yazıyorsun o da yeğenlerine aktarıyor bu bilgiyi eyvallah.<br/>ama bu pazartesi'den başlayarak asla ama asla ne telefondan ne bilgisayardan adını dahi yazmadan fotoğraf makinesi alacağım diyip durdum ki ömrümde internetten fotoğraf makinesi falan da aramışlığım yoktur hiç hatırlamıyorum.<br/>bakın bakın ruh hastasına bakın şimdi, mutfakta makarna haşlıyorum telefon tezgahta ben kendi kendime bağırıyorum ''yaa oğlum hangi fotoğraf makinesini alsam lan. nikon mu canon mu daha iyi'' acaba falan. çay içerken demliğe soruyorum ''ulan ucuza yok mu fotoğraf makinesi'' falan.telefon da sinsi sinsi bize bakıyor.<br/>pazartesi geçti,salı geçti,çarşamba geçti ve bugün oldu arkadaşlar. şimdi facebook'tan,haber sitelerinden,outlook'tan tutun nereye isterseniz her yerde fotoğraf makinesi ve bu makineleri satan site ve firmaların reklamları doldu. bırakın fotoğraf makinesini,''foto'' diye bile aratmadım hiçbir yerde ama sardılar dört bir yanımı.yani demem o ki. buna inanmayan adam varsa hala aranızda tövbe etsin.ve bu bokun çaresi de yok.her haltımız dinleniyor belli ki...
  entry_channels: bilim,teknoloji
- entry_name: |
    en sevilen diziyi bir replikle anlatmak
  entry_id:  79588212
  entry_writer: tompa
  entry_fav_count: 129
  entry_content: |
    (bkz: <a class="b" href="/?q=we+were+on+a+break">we were on a break</a>)
  entry_channels: anket,tv
- entry_name: |
    26 temmuz 2018 trump'ın türkiye'yi tehdit etmesi
  entry_id:  79597644
  entry_writer: fiach mchugh
  entry_fav_count: 105
  entry_content: |
    daha geçen ay kraliçe'nin huzuruna çıkan, iki hafta önce trump'la fotoğraflar çektiren erdoğan'ı desteklemiyoruz, doğru.<br/>yalnız ve güzel ülkemiz tehdit ediliyor, bu da doğru.<br/><br/><a class="b" href="/?q=%c3%bclkemizin+tehdit+edilece%c4%9fini">ülkemizin tehdit edileceğini</a> ve hatta <a class="b" href="/?q=daha+ciddi+sorunlar">daha ciddi sorunlar</a> yaşayacağımızı yıllardır anlatmaktan <a class="b" href="/?q=dilimizde+t%c3%bcy+bitti">dilimizde tüy bitti</a>.<br/><br/>bunları söylediğimiz her an, hakaretlerle susturulduk.<br/><br/><a class="b" href="/?q=bu+y%c3%bczden">bu yüzden</a>, akp'li dostlar kusura bakmasın, <a class="b" href="/?q=s%c3%b6ylediklerimiz+ger%c3%a7ekle%c5%9fti+diye">söylediklerimiz gerçekleşti diye</a> <a class="b" href="/?q=erdo%c4%9fan%27a+ko%c5%9fulsuz+biat+edecek+de%c4%9filiz">erdoğan'a koşulsuz biat edecek değiliz</a>. <a class="b" href="/?q=biz+zaten+do%c4%9fru+yerdeydik">biz zaten doğru yerdeydik</a>, hâlâ doğru yerdeyiz. siz memleketin tek adama peşkeş çekilmesine koşulsuz destek oldunuz. <a class="b" href="/?q=su%c3%a7lu+olanlar+sizsiniz.">suçlu olanlar sizsiniz.</a><br/><br/>biz haklıydık. biz hâlâ haklıyız. <br/><br/>haksız olan ve bunları yaşamamıza sebep olan sizlersiniz. o yüzden lütfen önce dönüp kendi hatalarınıza bakın. hatanın neresinden dönülse kârdır. sorgulayın, bağlantıları kurun ve bugüne dek hakaretlerle susturduğunuz, ötekileştirdiğiniz, küstürdüğünüz muhaliflere kulak verin. 2002'den beri "<a class="b" href="/?q=tehlikenin+fark%c4%b1nda+m%c4%b1s%c4%b1n%c4%b1z">tehlikenin farkında mısınız</a>" dememiz boşuna değildi.<br/><br/>kişiler, partiler, hükumetler gelip geçer. baki olan milli bilincimizdir. türkiye cumhuriyeti devleti'nin yapı taşlarını dinamitleyen, koca devlet geleneğimizi, üç-beş kasaba kurnazının fantezilerine terk edenler akp seçmenidir. kendine gelmesi ve duruşunu netleştirmesi gereken de yine aynı gruptur.<br/><br/>ekleme: şu entryden ülkemizi sevmediğimizi sonucunu çıkartabilecek kadar kansız olan kişi, ne ülkemizi seviyordur ne de milletimizi. net söylüyorum. biz bu ülkeyi öyle çok sevdik ki, sizinkiler habur'da halay çekerken biz hapse düştük. sizinkiler kozmik oda'yı fetö'ye teslim ederken, biz silivri'den haykırdık. sizinkiler ege'deki adalara "kayalık" derken biz ses çıkarttığımız için mahkemelik olduk. 6. filo'yu kıble belleyenler burada gelip vatansever pozu kesmesin bir zahmet.<br/><br/>böyle rezilce bir düşünceyle hipnotize olan bu kişilerin tek derdi de akp'yi haklı göstermeye çalışmaktır. 16 yıldır yapılan hataları görmezden gelmek, 16 yıldır "yapmayın, bakın devlet böyle yönetilmez. üretimi bitirdiniz, liyakati bitirdiniz, hukuku bitirdiniz, ekonomiyi de bitirirseniz bizi kukla yaparlar" diyenlere, bize, yani gerçek vatanseverlere hakaretler etmek, anca bu kansızlara yakışır zaten.<br/>bu fetöcüleri bu ülkede kim istihdam etti? biz bu fetöcülere cayır cayır muhalefet ederken, "batı fetöyü kullanacak, bunları kritik yerlere sokmayın" diye biz bağırırken, akpliler korumadı mı bu adamları? neyin tantanasını yapıyorsunuz? lan siz bozdunuz zaten memleketin ayarını! sizin yüzünüzden bugün buradayız lan!<br/><br/>merak etmeyin. bu toprakların gerçek sahipleri, 1922'de özgürlük için canını verenlerin torunları yine kurtarır bu toprakları. sakarya'da, kıbrıs'ta, kore'de, bosna'da, şırnak dağlarında nasıl destan yazdıysak yine yazarız. ama bu sefer sizin gibi vatansız, paragöz, insanlıktan kopmuş, ali kemal torunu haysiyetsizlere tahammül eder miyiz orası meçhul...<br/><br/>saflarımız iyice belli olsun ki, ülkemizi kurtarmamızın gerekeceği o gün gelince dost-düşman iyice ayırt edelim.
  entry_channels: siyaset,haber
- entry_name: |
    26 temmuz 2018 pence'in türkiye'yi tehdit etmesi
  entry_id:  79595822
  entry_writer: uckuruslukopera
  entry_fav_count: 103
  entry_content: |
    20 yıl öncesinin paradigmalarıyla analiz yapmaya çalışanları göstermiş haberdir. bu konjonktürde türkiye gibi bir müttefik kaybedilmezmiş. özellikle iran’la bu durum varken deniyor. şimdi haberlere geçelim o zaman;<br/><br/>1- türkiye artık abd’nin müttefiki değildir. bu birçok platformda defalarca dile getirilmiştir.<br/>2-türkiye’nin içindeki israil olmasa (ne dediğimi anlayanlar olacaktır) yerimize güneydeki kürt bölgesi çoktan geçecekti.<br/>3- abd türkiye’yi müttefik ve lojistik ortak saysa güney sınırımızın öbür tarafına 5000 tır silah göndermezdi. ikmal dünyanın en pahalı işidir. <br/>4- şu an bu ülke ile aramızda örtülü savaş hukuku vardır. biraz diplomasi bilseydiniz bunu görürdünüz.<br/><br/>edit: bazen buranın eski ekşi olmadığını unutuyorum. şu seviyeye bak ya, insanın içi acıyor. facebook gruplarından beter yorumlar var başlıkta.<br/><br/>edit2: ilginç olan şu ki 2. madde ile ilgili bir sürü insan soru sordu ama o madde lehimize olan tek madde. düşünmeniz gerekenler diğerleri arkadaşlar. yine de gizli saklı bir konudan bahsetmiyorum.
  entry_channels: siyaset,ekonomi
- entry_name: |
    yabancı damat dizisinden akılda kalanlar
  entry_id:  79584731
  entry_writer: ney na na na
  entry_fav_count: 93
  entry_content: |
    şeker hastası olan kahraman ın, bir tepsi baklavayla bilinçli intihar etme sahnesi.<br/><br/><a rel="nofollow noopener" class="url" target="_blank" href="https://m.youtube.com/watch?v=E4TEC9Z4pDo&amp;feature=youtu.be&amp;t=1m28s" title="https://m.youtube.com/watch?v=E4TEC9Z4pDo&amp;feature=youtu.be&amp;t=1m28s">https://m.youtube.com/…pdo&amp;feature=youtu.be&amp;t=1m28s</a><br/><br/>edit:link icin riseofthefuture e teşekkürler
  entry_channels: anket,tv
- entry_name: |
    chp'de 129 milletvekilinin kurultaya karşı çıkması
  entry_id:  79592669
  entry_writer: ecco homoreous
  entry_fav_count: 88
  entry_content: |
    59 il başkanından sonra chp'li 129 milletvekili de "kurultay olmasın" dedi.<br/><br/>chp'de imza gerginliği sürüyor. bugün önce 59 il başkanı kılıçdaroğlu'na destek açıklaması yaptı. ardından ince'yi destekleyen tüzün ve usluer'den "il başkanları delegelerin iradesini temsil etmez" açıklaması geldi. son olarak ise 129 milletvekili ortak açıklama yaparak olağanüstü kurultaya karşı çıktı. açıklamaya, 12 milletvekili ise imza koymadı.<br/><br/>(bkz: <a rel="nofollow noopener" class="url" target="_blank" href="http://www.cumhuriyet.com.tr/haber/siyaset/1037871/CHP_li_129_milletvekili_de__kurultay_olmasin__dedi.html" title="http://www.cumhuriyet.com.tr/haber/siyaset/1037871/CHP_li_129_milletvekili_de__kurultay_olmasin__dedi.html">http://www.cumhuriyet.com.tr/…ay_olmasin__dedi.html</a>)<br/><br/>kılıçdaroğlu'nun çırpınışları sürüyor. chp tüzüğündeki haklarını kullanarak kurultay için imza toplayan delegelerin yeterli sayıyı bulmalarından sonra, birbiri ardısıra salvolar geliyor genel merkezden. bunu en son mhp'de görmüş idik. kılıçdaroğlu ölümü pahasına koltuğu terk etmemek için her yolu deniyor. il başkanları, yetmedi şimdi de milletvekillerini basın açıklamasına zorlamış. basın açıklamasını okuyan aysu bankoğlu adlı körpecik kızcağızın hemen yanında tüm seçim sürecinde muharrem ince'nin yanında olan ve seçim gecesi herhangi bir açıklama yapmasını engelleyen ve "ertesi gün yaparsın açıklmanı!..." diyen engin altay duruyor. ne ironik değil mi? sakın bu levent gültekin'e "muharrem ince, seçim gecesi içti, ayakta duracak hali yoktu!.. " diyen o olmasın... çünkü muharrem ince'nin beyanına göre, o gece yanında sadece yaşar tüzün, aile mensupları ve engin altay vardı swiss otelde...<br/><br/>yahu bu nasıl iş?... adamlar değişim isteyen delegelerin imzalarını toplayıp, legal bir yolla bir süreç başlatmışlar, genel başkan olacak zavalllı da bütün adamlarını ortalığa salıp kurultaya mani olmak için olmadık yollara başvuruyor. daha çok batıyorsun kılışdar, daha çok batıyorsun. bundan kelli ne sana, ne de orada basının karşısına geçirdiğin adamlara oy moy yok, anlamıyor musun?...
  entry_channels: siyaset
- entry_name: |
    cem yılmaz'ın yaz ayında mont giymesi
  entry_id:  79591151
  entry_writer: leland
  entry_fav_count: 72
  entry_content: |
    belki giymeye programlanmıştır.
  entry_channels: moda,magazin
- entry_name: |
    26 temmuz 2018 b36 torshavn beşiktaş maçı
  entry_id:  79571768
  entry_writer: halfpastten
  entry_fav_count: 67
  entry_content: |
    aman aman nereye geldik bi anda. neresi burası lan. daha demin şampiyonlar ligi'ndeydik, oğlum fenalık gecırıyorum lan. <br/>harbi şampiyonlar liginde degiliz lan daha demin bayern'le kapışıyorduk.
  entry_channels: spor
- entry_name: |
    homofobik doktorun hasta haklarımı ihlal etmesi
  entry_id:  79586208
  entry_writer: canonmunikonmu
  entry_fav_count: 65
  entry_content: |
    entryi silen yazar veya arkadaşı okuyorsa eğer, yapılması gereken, olay esnasında doktorun odasını terk ettiğinde, başhekimin yanına giderek durumu bildirmekti. eğer sonuç alamıyorsan bimere yaz, ilgili yerlere başvur.<br/>ama sen olayı sosyal medyaya taşıyıp, açık isim paylaşıp doktoru hedef haline getirip, sonra aynı şeyi doktor yaptığında gizlilik gizlilik diye yırtınamazsın.
  entry_channels: siyaset,ilişkiler,sağlık
- entry_name: |
    bedelli asker saç kestirmemeli
  entry_id:  79594591
  entry_writer: thacker
  entry_fav_count: 61
  entry_content: |
    her gün duş almalı, sabah isterse 11'de kalkabilmeli.
  entry_channels: siyaset,ilişkiler
- entry_name: |
    32 yaşında hala evlenmemiş olmak
  entry_id:  79571392
  entry_writer: mantarlitavuk
  entry_fav_count: 56
  entry_content: |
    fazla kafaya takılmaması gerekendir. evlenirsin, bir yıl sonra çocuk yapmıyor derler, çocuk yaparsın ikinci çocuk yok mu derler.insanlar sürekli başkalarının hayatında dile getirecek eksik bir şeyler bulurlar.kendi hayatınıza bakın, bol bol evli arkadaşlarınızın sorunlarını dinleyip oh be ben ne rahatım diyin.
  entry_channels: anket,ilişkiler
- entry_name: |
    rusya'yla dayanışmamız birilerini kıskandırıyor
  entry_id:  79600066
  entry_writer: pok
  entry_fav_count: 51
  entry_content: |
    birilerini nasıl bir tabirdir ya. facebook manidarı gibi uluslararası ilişkiler.
  entry_channels: siyaset,haber
- entry_name: |
    100 liraya hamile bırakan hoca
  entry_id:  79596192
  entry_writer: dumanizt
  entry_fav_count: 50
  entry_content: |
    doğan çocuklarin da hocaya benzemesi ile sonuçlanan keramettir.
  entry_channels: ilişkiler
- entry_name: |
    angela merkel'in iki araçlık konvoyu
  entry_id:  79599352
  entry_writer: mahoaganinkuluyam
  entry_fav_count: 44
  entry_content: |
    birisi kendi makam aracı diğeri koruma aracı olmak üzere iki araçtan oluşan konvoydur,<br/><br/><a rel="nofollow noopener" class="url" target="_blank" href="https://www.youtube.com/watch?v=1oIi4iDCZGE">https://www.youtube.com/watch?v=1oii4idczge</a><br/><br/><a class="b" href="/?q=bilecik">bilecik</a> valisi'nin bile zırhlı <a class="b" href="/?q=mercedes-maybach+s600">mercedes-maybach s600</a>'e binip <a class="b" href="/?q=2+passat+3+mondeo">2 passat 3 mondeo</a> <a class="b" href="/?q=1+tane+de+trafik+polisi+arac%c4%b1">1 tane de trafik polisi aracı</a> olmak üzere 6 koruma aracıyla yolculuk yaptığı bir ülkenin vatandaşına garip gelebilir.<br/><br/>(kendi vergisiyle alınan lüks araçlarla yapılan konvoyun görüntülerine <a class="b" href="/?q=dombra">dombra</a>, <a class="b" href="/?q=dirili%c5%9f+ertu%c4%9frul">diriliş ertuğrul</a> <a class="b" href="/?q=mehter">mehter</a> müziği ekleyip bunla gururlanan canlı türlerine birşey anlatmaya kalkmayın arkadaşlar.)<br/><br/>aynen kardaşım reis gövde gösterisi yapıyyeahh, <a class="b" href="/?q=mercedeslerle">mercedeslerle</a> <a class="b" href="/?q=bmwlerle">bmwlerle</a> <a class="b" href="/?q=gmclerle">gmclerle</a> <a class="b" href="/?q=volkswagenlerle">volkswagenlerle</a> <a class="b" href="/?q=t%c3%bcrk%27%c3%bcn+g%c3%bcc%c3%bcn%c3%bc+d%c3%bcnyaya+g%c3%b6steriyyeahh">türk'ün gücünü dünyaya gösteriyyeahh</a>,<br/>şimşek püskevitini al ve gururlanmak için ekranın karşısına geç kardaşım,<br/><br/><a class="b" href="/?q=reis+recep+tayyip+erdo%c4%9fan">reis recep tayyip erdoğan</a>ın konvoyu <a class="b" href="/?q=dombra">dombra</a> eşliğinde<br/><br/><a rel="nofollow noopener" class="url" target="_blank" href="https://www.youtube.com/watch?v=zth1csCuTmQ">https://www.youtube.com/watch?v=zth1cscutmq</a><br/><br/><a class="b" href="/?q=korumalar+sirenleri+patlat%c4%b1yor">korumalar sirenleri patlatıyor</a> yollar açılıyor kardaşım<br/><br/><a rel="nofollow noopener" class="url" target="_blank" href="https://www.youtube.com/watch?v=n8sNYj1WwdY">https://www.youtube.com/watch?v=n8snyj1wwdy</a><br/><br/><a class="b" href="/?q=mevl%c3%bct+%c3%a7avu%c5%9fo%c4%9flu">mevlüt çavuşoğlu</a>bakanımızın konvoyu kardaşım (<a class="b" href="/?q=gece+%c3%a7ekimi+m%c3%bcziksiz">gece çekimi müziksiz</a>)<br/><br/><a rel="nofollow noopener" class="url" target="_blank" href="https://www.youtube.com/watch?v=Ng2ZuWgVc3M">https://www.youtube.com/watch?v=ng2zuwgvc3m</a><br/><br/><a class="b" href="/?q=s%c3%bcleyman+soylu+bakan%c4%b1m%c4%b1z">süleyman soylu bakanımız</a> sürmene'ye giriş yapıyor (<a class="b" href="/?q=oooh+%c3%a7ok+g%c3%bczel">oooh çok güzel</a>)<br/><br/><a rel="nofollow noopener" class="url" target="_blank" href="https://www.youtube.com/watch?v=GWxUUWFHLvY">https://www.youtube.com/watch?v=gwxuuwfhlvy</a>
  entry_channels: siyaset
- entry_name: |
    3 ayı zorunlu 6 ayı bedelli askerlik
  entry_id:  79577988
  entry_writer: balkes10
  entry_fav_count: 44
  entry_content: |
    eğer bu sistem gelecekse fırsatı plan bu bedelliden yararlansın, hatta kısa dönem gidecek olanlar gitsin kısa dönem. yoksa 5 ay gitmek varken hem 3 ay gidip hem de günümüz parasıyla yaklaşık 10 bin tl ödemek zorunda kalacaksınız.<br/><br/>yalnız bedelliye ihtiyaç bırakmayacak sistem buysa vay halimize ülkecek :)
  entry_channels: siyaset
- entry_name: |
    26.7.2018 türkiye'nin kredi almasının kısıtlanması
  entry_id:  79600602
  entry_writer: yagsag
  entry_fav_count: 39
  entry_content: |
    <a class="b" href="/?q=erdin%c3%a7+kalu%c3%a7">erdinç kaluç</a>'un dersinden bir alıntı : <br/><br/>"ilk yıl bize ambargoyu koydular. biz o zamana kadar jipti bilmem neydi hep amerikadan alıyoruz tabii. bunlar biraz çalıştı, arıza çıkarmaya başladı. yenileri gerekti. biz de o zaman okul olarak bu projede yer aldık, silahlı kuvvetlere jip yapmaya başladık. ilkin yaptığımız araçların tavanı dalgalı falan oluyordu, kapısı elde kalıyordu. sonra sonra biz bu işi iyice öğrendik, cillop gibi makineler yapmaya başladık. işte o sırada -bir elini yumruk yapıp diğer elini de onun üzerine şaklatarak- gittiler ambargoyu kaldırdılar. yine eski jiplerini hibe etmeye başladılar. bizim jip imalatı da rafa kalktı haliyle."
  entry_channels: haber,ekonomi
- entry_name: |
    bedelli askerlik
  entry_id:  79572260
  entry_writer: cascascas
  entry_fav_count: 36
  entry_content: |
    eğer 21 gün kaldırılmazsa and içiyorum o 21 günlük askerlik anılarımı insanları bezdirip kusturana kadar anlatacağım. <br/><br/>ilk gün sıradayız teçhizat alıyoruz tamam mı? görevli askere dedim ki "kardeşim bu botun bi numara büyüğü yok mu?" dedi ki "var bak ebenin .mnda." ben de dedim ki "sen ne diyosun lan onun bunun çocuğu" çaat bi gömdüm kafayı buna. daha ilk günden diskoya atmazlar mı beni. disko dediysem disiplin koğuşu yanlış olmasın. öyle disko topu falan yok ya.....<br/><br/>aha bak şimdiden kafa sikiyorum.<br/><br/>tehlikenin farkında mısınız sayın .mna koduklarım.
  entry_channels: siyaset,haber,ekonomi
- entry_name: |
    türkiye'nin çin'den 3.6 milyar dolar kredi alması
  entry_id:  79598904
  entry_writer: bilgisayar
  entry_fav_count: 26
  entry_content: |
    bu ayın kuruyemiş parası çıkmış galiba.
  entry_channels: ekonomi
- entry_name: |
    tanıdığın birinin ilk halini özlemek
  entry_id:  79579734
  entry_writer: pismandegilim
  entry_fav_count: 26
  entry_content: |
    cidden keşke öyle kalsaydı diyorsun, özlüyorsun sonra yaşananlar geliyor aklına sonra keşke unutsam diyorsun. unutamıyorsun. geri döndüğünde de istemiyorsun. öyle yaşıyorsun.
  entry_channels: anket,ilişkiler
- entry_name: |
    26 temmuz 2018 mevlüt çavuşoğlu açıklaması
  entry_id:  79599464
  entry_writer: kalemi kilicindan keskin olan adam
  entry_fav_count: 24
  entry_content: |
    an itibarıyla evde aydemir akbaş gibi ezan okuyorum.<br/><br/><a class="b" href="/?q=ek">ek</a>: gün sayın. en hızlı tahliyeye şahit olacaz.
  entry_channels: 
- entry_name: |
    bedelli mobbing görürse hakkını nasıl arayacak
  entry_id:  79597009
  entry_writer: okipita
  entry_fav_count: 17
  entry_content: |
    mobbing mi sjsjsjsjsj. yöneticinize gidin veya ik ile iletişime geçin.
  entry_channels: anket
- entry_name: |
    ekşi itiraf
  entry_id:  79571359
  entry_writer: ay dios mio
  entry_fav_count: 16
  entry_content: |
    dünya fiziktir, insanlarla mesafeli olmak iyidir.<br/><br/>kütle çekim kanununa göre çekim kuvveti iki kütlenin arasındaki mesafenin karesi ile ters orantılıdır, bu sebeptendir ki insanlara yeterince yaklaşırsanız bir o kadar uzaklaşırsınız. <br/><br/>tanıdıkça sevilen insanlar suya kaçmış, su nerde? inek içmiş. inek nerde? dağa çıkmış, dağ nerde? yandı, bitti, kül oldu.
  entry_channels: anket,ilişkiler,ekşi-sözlük
- entry_name: |
    26 temmuz 2018 abd'nin türkiye'ye yaptırım kararı
  entry_id:  79597052
  entry_writer: attirmayin makedonun kafasini
  entry_fav_count: 13
  entry_content: |
    karar değildir.<br/>mynet haberciliği anlayışıyla manşet atmayınız.<br/><br/>tehdittir.
  entry_channels: siyaset
- entry_name: |
    düşün ki o bunu okuyor
  entry_id:  79571897
  entry_writer: antik limon kolonyasi
  entry_fav_count: 12
  entry_content: |
    herşeyi başa almak mümkün olsaydı keşke...
  entry_channels: anket,ilişkiler,edebiyat
- entry_name: |
    sevgilisi ödeyecek diye 2.5tl lik çay içen kız
  entry_id:  79598430
  entry_writer: unakkum enakkum
  entry_fav_count: 11
  entry_content: |
    2.5 liraya nerede çay içiliyor ki ya? 7 liranın altına çay veren mekan var mı ciddi ciddi?<br/><br/>neyse konumuza dönelim. iki üç liranın hesabını yapacaksanız zaten yallah köyünüze gidin. iyice terbiyesizleştiniz artık siz. <br/><br/>iç kız sen de çayını. canın istediyse 1 demlik iç. şu çomarlardan mı çekinicez. hayret bi' şey.
  entry_channels: ilişkiler
- entry_name: |
    mcdonald's çalışanlarından dayak yiyen kız
  entry_id:  79600587
  entry_writer: flakka
  entry_fav_count: 9
  entry_content: |
    sonlara dogru eline gecirdigi sandalyeyi de "oyle olmem, al biraz da bununla vur" dercesine calisana teslim eden cesur kizdir.
  entry_channels: anket,haber
- entry_name: |
    köpeği arabayla sürükleyen şerefsizin görüntüleri
  entry_id:  79595848
  entry_writer: gkhns
  entry_fav_count: 6
  entry_content: |
    korna çalarak arabayı durduran şoför, köpeği bağlayan öküzün ağzını burnunu kırdı.<br/><br/>umarım kamera kapandıktan sonra olay bu şekilde gelişmiştir
  entry_channels: haber
- entry_name: |
    kemal kılıçdaroğlu
  entry_id:  79586845
  entry_writer: galektersiz
  entry_fav_count: 6
  entry_content: |
    kendisine olan saygım ve sevgim artık tamamen bitmiştir. başında olduğu sürece partiye ne genel seçimde ne de belediye seçimlerinde oy vermeyeceğim.<br/><br/>hatta ve hatta önderlik ettiği hiç bir oluşuma da katılmayacağıma namusum ve şerefim üzerine yemin ederim.<br/><br/>rezil olarak mı gitmek istiyorsun, istediğin olacak.
  entry_channels: siyaset,haber
- entry_name: |
    sözlükçülerin abd'de yaşamak istediği eyalet
  entry_id:  79571811
  entry_writer: xme
  entry_fav_count: 5
  entry_content: |
    çoğu yazar hayal kurmuş ama bence en guzel yer sizi seven insanları barındıran yerdir. <br/><br/>hayaller california, hayatlar yozgat boğazlıyan...
  entry_channels: anket,ekşi-sözlük
- entry_name: |
    btcturk ceo'su özgür güneri soruları cevaplıyor
  entry_id:  79597858
  entry_writer: sicarken ilham gelen dahi
  entry_fav_count: 4
  entry_content: |
    merhaba. fiyatlar düşünce sistemi neden kapatıyorsunuz?
  entry_channels: ekşi-sözlük,ekonomi
- entry_name: |
    yayınlandığı döneme damga vuran film
  entry_id:  79596567
  entry_writer: korki48
  entry_fav_count: 3
  entry_content: |
    (bkz: <a class="b" href="/?q=back+to+the+future">back to the future</a>)<br/>sadece yayınlandığı döneme değil, günümüze dahi damgasını vuran film serisidir. arada deniyorum yeni dönem bilim kurgu filmlerini ama yok olmuyor. içerik yok çünkü. süper kahraman filmlerini izliyor yeni gençlik. liseyi yeni bitirmiş birine back to the future izlettim, haberi dahi yokmuş filmden. 1. filmi izlemeye başlarken yattım ben. sabah öğrendim ki hepsini izlemiş çocuk. çok beğenmiş. ve arkadaşlarına haber vermiş.
  entry_channels: 
- entry_name: |
    türkiye'nin abd'ye uygulayacağı yaptırımlar
  entry_id:  79600795
  entry_writer: benjamin breeg
  entry_fav_count: 3
  entry_content: |
    malum papaz olayindan sonra turkiye'nin abd tarafindan maruz kalacagi yaptirimlara tepki olacak olasi yaptirimlardir. basta aklima hicbiseyi gelmedi ta ki dolabi acip icinde iki haftadan fazla duran asidi kacmis coca cola sisesini gorune kadar.<sup class="ab"><a title="(bkz: hinzir gulus)" href="/?q=hinzir+gulus" data-query="hinzir gulus">*</a></sup>
  entry_channels: 
- entry_name: |
    insan neden sanat yapar
  entry_id:  79600220
  entry_writer: bjork
  entry_fav_count: 3
  entry_content: |
    toplum sorunları tartışır, sanat tartışılacak sorunları keşfeder. bunu felsefe bile yapamaz, onun bir tık gerisindedir. <br/><br/>fakat yanlış anlaşılmasın, sanat sorun icat etmez. öyle ya da böyle, karşımıza çıkacak olan sorunları önceden haber verir. bu yüzden çok değerlidir.
  entry_channels: 
- entry_name: |
    geceye bir şiir bırak
  entry_id:  79571961
  entry_writer: basamel
  entry_fav_count: 2
  entry_content: |
    güneş yüzünü gösterir bana<br/>yalnızca pazarları<br/>insan olduğum aklıma gelir<br/>bugün benim günümdür<br/>ve bir gece önceden başlar hazırlığım<br/>cumartesi en güzel gecedir<br/>buzlu rakımı içtiğim.<br/>ertesi sabah sevginin günüdür.<br/>değerliler değerlenir,<br/>karpuzlar şekerlenir<br/>ve öyle günler de mutlaka çay içerim<br/>kahvaltılarımda.<br/>tüm gün gözlüğümü çıkarmam gözümden<br/>bir pandaya dönüşürüm sonrasında.<br/>gözlerim hep beyazdır benim hep beyaz.<br/><br/>e.c.d.
  entry_channels: edebiyat
- entry_name: |
    bedelli askerlik yapan erkekle evlenmem diyen kız
  entry_id:  79600400
  entry_writer: harlemzencisi
  entry_fav_count: 2
  entry_content: |
    onun evlenmedigi erkek bana gelsin dedigim kizidir. cok cocuk istiyosa da evlatlik aliriz.
  entry_channels: ilişkiler
- entry_name: |
    ekşi sözlük dertleşecek insan veritabanı
  entry_id:  79593678
  entry_writer: vaylele
  entry_fav_count: 0
  entry_content: |
    hayatımın her anlamda en kötü günlerini geçiriyorum.uyku düzenim diye bir şey yok günün herhangi bir saatinde uykuda ya da uyanık olabilirim.yaklaşık 22 saattir hiç uyumadım, elimde bira uykumun gelmesini bekliyorum.geçirdiğim sıkıntılı dönemden sonra toparlanmaya çalışıyorum ama ilk defa bunu başaramayacakmışım gibi geliyor artık gittiği yere kadar.kimseyle dertleşmek istemem sadece yazmak istedim.okuyanlara mutlu günler.
  entry_channels: anket,ilişkiler,ekşi-sözlük
---
