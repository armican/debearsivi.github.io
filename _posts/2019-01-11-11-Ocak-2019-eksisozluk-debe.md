---
layout: post
title: 11 Ocak 2019 Ekşi Sözlük Debe
data:
- entry_name: |
    madem solcular çok zeki neden iktidar olamıyorlar
  entry_id:  85330794
  entry_writer: stoudamire
  entry_fav_count: 833
  entry_content: |
    malumunuzdur zeki insanların sayıları azdır.
  entry_channels: siyaset,anket
- entry_name: |
    didem soydan'ın yaşadığı big dick polemiği
  entry_id:  85342953
  entry_writer: chinchanchon
  entry_fav_count: 784
  entry_content: |
    (bkz: <a class="b" href="/?q=stop+making+stupid+people+famous">stop making stupid people famous</a>)
  entry_channels: magazin
- entry_name: |
    sırt çantası takan 30 yaşındaki erkek
  entry_id:  85344482
  entry_writer: headless horseman
  entry_fav_count: 632
  entry_content: |
    kitap falan taşıyor olabilir diyeceğim ama linç edilmekten korkuyorum. <br/>tanım: hayatı anahtar, cüzdan ve telefondan ibaaret olanların anlamayacağı erkektir.
  entry_channels: anket,moda
- entry_name: |
    ali ece'nin takipçisine hakaret etmesi
  entry_id:  85339613
  entry_writer: cammasaninatinasumuksurenadam
  entry_fav_count: 606
  entry_content: |
    (bkz: <a class="b" href="/?q=ali+ece+kim+aq">ali ece kim aq</a>)<br/><br/>not: bunu yazmama kızanlar oldu, ben hepsine yazdığım cevabı buraya da yazayım da tam olsun;<br/><br/>ben gerçekten ali ece'yi tanımıyorum kardeşim. önemli biri olsa haberlerde denk gelirdim, demek ki önemsizmiş ve insanlarda benim gibi düşünmüşler, kendileri de bunu yazacakları için bana onay vermişler. burada atıyorum rte, ali koç vs diye başlık atılmış olsa herhalde böyle yazmazdık.
  entry_channels: magazin
- entry_name: |
    73 61 62 69 72 65 20 6d 65 6c 74 65 6d 20 62 61
  entry_id:  85347438
  entry_writer: lothlorien5
  entry_fav_count: 466
  entry_content: |
    darbenin şifreleri paylaşılıyor 155’i ararın
  entry_channels: teknoloji
- entry_name: |
    5 dakikadan uzun ancak baymayan şarkı yapmak
  entry_id:  85340514
  entry_writer: dstnysdd
  entry_fav_count: 404
  entry_content: |
    (bkz: <a class="b" href="/?q=bohemian+rhapsody">bohemian rhapsody</a>)
  entry_channels: anket,müzik
- entry_name: |
    evi arabası olan 10 bin tl maaşlı bekar erkek
  entry_id:  85352563
  entry_writer: ofis canlisiyim multinetle beslenirim
  entry_fav_count: 391
  entry_content: |
    kendisine talip oldugum erkektir.<br/>sahsiminda erkek olmasi iliskimize engel degildir, en nihayetinde bu kucuk puruzleri bi cay icerek konusulabiliriz.
  entry_channels: ilişkiler
- entry_name: |
    türklerin kullanmayı beceremedikleri şeyler
  entry_id:  85338541
  entry_writer: beni kimse sevmedi
  entry_fav_count: 369
  entry_content: |
    oy pusulası.
  entry_channels: anket
- entry_name: |
    10 ocak 2019 brothers cafe bar rezaleti
  entry_id:  85357131
  entry_writer: inleten nagmeler
  entry_fav_count: 303
  entry_content: |
    "ankara'da olduğun sürece artık ankara'da ben de varım" cümlesinin apaçık bir tehdit olduğunu idrak edemeyenlerin beğenmediği rezalettir.
  entry_channels: yeme-içme
- entry_name: |
    asla mutlu olamayacak insanlar
  entry_id:  85330480
  entry_writer: uaslan
  entry_fav_count: 277
  entry_content: |
    mutsuz bir çocukluk geçiren insanlar.
  entry_channels: anket,ilişkiler
- entry_name: |
    aselsan başlangıç kiti
  entry_id:  85351473
  entry_writer: kissofh3ll
  entry_fav_count: 257
  entry_content: |
    işe başlar başlamaz yöneticilerini yalamaya başlamış tipe ait kittir.
  entry_channels: haber
- entry_name: |
    en güzel çizgi filmler
  entry_id:  85333177
  entry_writer: sinpecado
  entry_fav_count: 201
  entry_content: |
    (bkz: <a class="b" href="/?q=%c5%9firinler">şirinler</a>) <br/>(bkz: <a class="b" href="/?q=casper">casper</a>) <br/>(bkz: <a class="b" href="/?q=tom+ve+jerry">tom ve jerry</a>) <br/>(bkz: <a class="b" href="/?q=jetgiller">jetgiller</a>) <br/>(bkz: <a class="b" href="/?q=ar%c4%b1+maya">arı maya</a>) <br/>cocuklugumu geri verin.
  entry_channels: anket,tv
- entry_name: |
    meb haydi çocuklar camiye yarışması müjdesi
  entry_id:  85345283
  entry_writer: illgnsyyz
  entry_fav_count: 176
  entry_content: |
    kontrol mekanizması her camide bulunan "proje sorumlu"larıymış. sorumlulara imza vererek ilgili namaz katılımı kontrol ediliyormuş. bu işin ucunda da maddi değeri olan ödüller olduğuna göre, bazı proje sorumluları akraba/yakın çocuklarını kollayacaktır. nede olsa: muhakkak ki allah, adaleti, iyiliği, akrabaya yardım etmeyi emreder
  entry_channels: eğitim
- entry_name: |
    43 su markasında ağır metale rastlanması
  entry_id:  85345427
  entry_writer: guneyk
  entry_fav_count: 170
  entry_content: |
    hep 50 liralık espirisi de yapıldı, gülmeniz bittiyse artık hangi şerefsizler bizi zehirliyormuş, marka ve satıldığı yerleri de bilmemiz gerekiyor.<br/><br/>bu işin şakası yok. ciddi ciddi sağlığımızı tehdit edip ceplerini dolduran orospu evlatları açığa çıkmalıdır.
  entry_channels: sağlık,haber,yeme-içme
- entry_name: |
    hesap ödeten kızla ciddi ilişki yaşamak
  entry_id:  85341991
  entry_writer: hayvan adam
  entry_fav_count: 167
  entry_content: |
    az önce işyerindeki kızlarla bu konuyu konuşuyordum. kızlardan birisi 33 yaşında, bekar ve 10lu yaşlarda 2 erkek çocuğu annesi. öteki kız 25 yaşında, bekar ve hayatını yaşıyor. iki kız da abd standartlarına göre 10 üzerinden 8-8.5 gibiler. <br/><br/>bunlar bana arada bi erkeklerle ilgili sorular sorarlar. ilk seks kaçıncı buluşmada olur diye sordular. <br/><br/>dedim; hesapları erkek ödüyorsa, en fazla 3. buluşmada seks. bunlar hemen karşı çıktılar, o çok hızlı falan. biraz daha zaman lazımmış. ama bu aynı kızların one night stand maceralarını da bilirim. o yüzden bu argüman boş. <br/><br/>hesabı erkek ödemiyorsa veya yarı yarıya ödüyorlarsa, ilişki iki taraf için de daha ciddi olduğu için, birbirlerini tanıyana, ne olup bittiğini anlayana kadar bekleyebilirler dedim. <br/><br/>aboooo, bu seferde bunların içindeki kezbanlık bi anda ortaya çıktı. erkek hesap ödemezse seksi unutsun. bi daha kızlarla dışarı da çıkamazmış. böyle cimrilik yapan erkeklerden nefret ediyorlarmış. cimrilik tüm romantizmi bitiriyormuş vs vs. <br/><br/>ulan dedim ipnetorlar, gitsin babalarınız sizi besleyip büyütsün. sizinle bi kan bağı olmayan erkek senin gıda maliyetini niye karşılasın? açsan, paran yoksa başka. yoo öyle bi durumda yok. <br/><br/>baya paranız var ve hesap gelince eliniz cüzdanınıza hiç gitmiyorsa, kusura bakmayın ama, bi zahmet siktir olup gidebilirsiniz. başka kapıya.
  entry_channels: anket,ilişkiler
- entry_name: |
    2019 grip salgını
  entry_id:  85339388
  entry_writer: oric
  entry_fav_count: 157
  entry_content: |
    başa edit: hastalığımızın virüsü <a class="b" href="/?q=h1n1">h1n1</a> - diğer bilinen adıyla <a class="b" href="/?q=domuz+gribi">domuz gribi</a><br/><br/>çevre ülkelerde de etkili<br/><br/>swine flu causes death of two people in past two days - agenda.ge<br/>dec 25, 2018 - swine flu (h1n1) has caused one more death in georgia.<br/><br/>flu in georgia: school semester postponed, 640 being treated in ...<br/>22 hours ago - according to official data 10 have lost their lives to the swine flu. ... been yet confirmed in a lab that the reason of the deaths was also swine flu.<br/><br/>two people died from influenza virus - ajaratv<br/>dec 25, 2018 - yesterday one woman died in batumi. against this ... according to him, the death was caused by the high risk group patients. according to the ...<br/>----------------------------------------------------<br/><br/>şimdi gelelim korunma ve tedavilere<br/><br/>korunmanın iki-üç basit yolu var<br/><br/>1) aksırıp tıksıran insanlardan kendinizi izole edin (havadan bulaşma)<br/>2) elinizi burnunuza ve gözünüze sürmeyin (3'ü yapmamışsanız). açıklama, grip, nezle gibi hastalıkların virüsleri vücut dışında 4-6 saat kadar aktif durumda olurlar. bu süre zarfında hasta birinin bulaştırdığı virüsler elinizle dokunduğunuzda sizin derinizin üzerinde bulaşma potansiyeli ile konaklamaya başlarlar. burun veya göz çevresindeki deriden içeri girerlerse hasta olursunuz. o yüzden hepimizin bilmeden yaptığı elini suratımıza dokundurma hareketi büyük risktir. sabunlu su ile bu virüsler kolayca imha edilir. <br/>3) elinizi sık sık sabunla yıkayın (temasla bulaşma)<br/><br/>vücut direnci için:<br/><br/>kışın d vitamini eksikliği bağışıklığınızı bozar<br/><br/><a class="b" href="/?q=devit-3">devit-3</a> gibi takviyeleri her ay veya 2 ayda bir, birer ampül kırıp içerek halledebilirsiniz. d vitamini sandığımızın aksına c vitamininden daha çok işe yarar. c vitamini tamir edicidir, bu doğrudan duvar örüyor.<br/><br/>ekleme:<br/><br/>eğer serum takıldığında ayağa kalkıyorsanız aslında yeterince iyi beslenemiyorsunuz veya vücudunuz yeterince iyi şekilde besinleri alamıyor demek olabilir. hastaneye kadar gidip sefalet yaşayacağınıza, neticede taktıkları serumda mucize bir ilaç yok, mikropları öldürmüyor, elektrolit dengenizi iyileştiriyor, ağrı kesici eklemişlerse biraz ateş düşürüp ağrı kesiyor)<br/><br/>1. kemik suyuna çorba yaptırın/için (kemikleri 4-5 saat kaynatsınlar sonra o suya sevdiğiniz sebzelerden çorba yaptırın, çok fazla şehriye vs. koydurtmayın)<br/>2. azar azar ceviz, fındık<br/>3. limon suyu ve elma sirkesinden tonik. yarım limon suyu, 2 yemek kaşığı sirke üzerine de 6 bardak su ile karıştırtıp bir tutam tuz ekletin. susadıkça bunu pipetle için<br/><br/>bunları yapın <a class="b" href="/?q=vallahi+oric+do%c4%9fru+s%c3%b6ylemi%c5%9f">vallahi oric doğru söylemiş</a> dersiniz
  entry_channels: sağlık
- entry_name: |
    2008 yılındaymış gibi yazmak
  entry_id:  85350375
  entry_writer: draw
  entry_fav_count: 131
  entry_content: |
    dolar 1.20 olmuş. bilgisayar alacaklar beklesin. bu fiyatlar ile elektronik eşya alınmaz.
  entry_channels: tarih
- entry_name: |
    yiğidi öldür hakkını yeme'nin ingilizcesi
  entry_id:  85351599
  entry_writer: usengecotu
  entry_fav_count: 123
  entry_content: |
    kill yiğit, but do not eat his hakkı.
  entry_channels: anket,eğitim
- entry_name: |
    finlandiya'da bulunan penis şeklindeki nehir
  entry_id:  85352296
  entry_writer: calambura
  entry_fav_count: 119
  entry_content: |
    bence finlandiyada hakkari olması daha enteresan
  entry_channels: seyahat
- entry_name: |
    1.5 milyar ışık yılı uzaktan gelen radyo sinyali
  entry_id:  85330569
  entry_writer: siyamtekizi
  entry_fav_count: 114
  entry_content: |
    1998 yılında ilk radyo sinyali aldık dediklerinde, daha sonra sinyalin yan odada bulunan mikro dalga fırından geldiğini söylemeleri çok uzun sürmemişti.
  entry_channels: ilişkiler,bilim
- entry_name: |
    türkiye'de mimar yok
  entry_id:  85354150
  entry_writer: rev d wayne love
  entry_fav_count: 110
  entry_content: |
    gaudi'yi getirsen imar yönetmeliğinden, güdük imar planlarından, belediyelerde çalışan raportörlerin, şeflerin cehaletinden, koruma kurullarının insiyatif alamamasından dolayı herkesle hemen hemen aynı ürünü vereceği, derdini bir türlü anlatamadığı ortamlarda hevesinin kırılacağı, mimarlar odası'nın kifayetsizliği, müşterinin cehaleti yüzünden üç kuruşa beş liralık projeyi yarı zamanda yapmak zorunda kalacağı, bazen yapı denetim kanunu yüzünden projenin inşa sürecinin tamamen dışında kalacağı bir ülkede anlamsız serzeniştir. <br/><br/>önceki sene şaşırtıcı bir biçimde bu sorunları yaşamadan çok severek bir proje yaptım. özendim de, mal sahibi de açık fikirliydi, parası azdı ama iyi olsun istedim. şansına belediyedeki mimar da gayet keyif aldı süreçten. çok güzel bir ürün çıktı ortaya. sonra mal sahibi işi devretti, kendi de uzaklara bir yere gitti. cephesi için biraz pahalı ama özgün bir sistem bile geliştirmiştik hey gidi... <br/><br/>ruhsatı aldık, müteahhit işe başladı, yapı denetim şirketi işi üzerine aldı, ben süreç dışındayım artık. <br/><br/>el birliğiyle projeyi bir güzel bozmuşlar, tadilat projesiyle falan da düzeltilecek tarafı kalmamış. zaten gerekli ara ruhsatları falan da almadan bodoslama yapmışlar binayı. bana tadilat projesini çiz diye geldiler, ne yaptınız siz dedim, ben tadilat çizersem yıkıp yeniden yapacaksınız binayı. kaba inşaatı bile hatalı. döşemeler hatalı, asansör mevzuata uygun değil, koridor genişliklerini daraltmışlar falan. para vermeyi de düşünmüyorlardı tadilat projesi için. çünkü ne ki yani iki tane çizik atacaksın! fiyat teklifi verdim. çıkacak masrafları söyledim. kalsın biz imar affına başvururuz dediler. <br/><br/>şimdi görseniz o çirkin binanın mimarına sövesiniz gelir biliyorum, ama o bina bir mimarın projesi falan değil. doğaçlama inşaat. en ucuz malzeme. çözülmemiş detaylar. çirkin ötesi bir cephe kaplaması. ve imar affı geldiği için benim bir mimar olarak müdahil olabileceğim hiç bir şey yok, günün birinde o binayı yıkıp yeniden yapmaya kalkarlarsa belki benim arşivdeki projemi kullanırlar.
  entry_channels: 
- entry_name: |
    yeni neslin hiç metalci olmaması
  entry_id:  85330578
  entry_writer: allwithinmyhands
  entry_fav_count: 94
  entry_content: |
    yeni nesil genel olarak müziği sevmiyor. müzik setleri, walkman'ler, mp3'ler artik yok. surekli dikkat kesilmeniz gereken sosyal medya hesaplari ve oyunlar var. bizim neslimizde müzik bir yaşam tarziydi. şimdi sadece ara sira dinlenen şarkilar..
  entry_channels: anket,müzik
- entry_name: |
    palu ailesi
  entry_id:  85335628
  entry_writer: misk kedisi
  entry_fav_count: 93
  entry_content: |
    emine doğru söylüyor. <br/>çok sırlı olaylar var. <br/>enes kayboluyor, o gün hastanede vücudunda iğneler olduğu anlaşılıyor, araştırılmadan aileye geri veriliyor. <br/>recep defalarca evden kaçıyor, araştırılmadan aileye geri veriliyor. <br/>recep sosyal hizmetler himayesine alınıyor ama evde başka çocuk var mıdır bakılmıyor. <br/><br/>benim gözümde asıl katiller o çocukları her defasında deli evine geri gönderenlerdir.
  entry_channels: haber
- entry_name: |
    instagram kullanan pilot
  entry_id:  85354060
  entry_writer: eksimtrack
  entry_fav_count: 77
  entry_content: |
    343 entry'de 90 kere pilot yazmak nasıl bir egodur, nasıl bir yokluktur lan <a rel="nofollow noopener" class="url" target="_blank" href="https://eksiup.com/c32f9c4f2366" title="https://eksiup.com/c32f9c4f2366">ahahah</a> zorlama fazla düşse düşse yüzün düşer. burdaki kızların alayı sapyoseksüeldir, mesleğe bakmaz. <br/>(bkz: <a class="b" href="/?q=pilotlar%c4%b1n+pilot+oldu%c4%9funu+bir+%c5%9fekilde+s%c3%b6ylemesi">pilotların pilot olduğunu bir şekilde söylemesi</a>)<br/><br/>edit akbayram : başlık için yazacak olursam, kimseyi ilgilendirmeyen pilottur.
  entry_channels: teknoloji,havacılık
- entry_name: |
    mabel matiz
  entry_id:  85342712
  entry_writer: rahibeler terasa
  entry_fav_count: 73
  entry_content: |
    barış manço ile ne alakası olduğunu anlayamadığım şarkıcıdır. olm sabah sabah kafanız mı güzel len?
  entry_channels: sanat,müzik
- entry_name: |
    zencilerde eşcinselliğin olmaması
  entry_id:  85360122
  entry_writer: yarasa talag
  entry_fav_count: 65
  entry_content: |
    (bkz: <a class="b" href="/?q=moonlight">moonlight</a>)<br/><br/>hadi evden dışarı çıktığınız yok anladık da film falan da mı izlemiyorsunuz?!
  entry_channels: ilişkiler
- entry_name: |
    rusya'nın dolar bağımlılığını azaltma politikası
  entry_id:  85349217
  entry_writer: olene kadar mokoko
  entry_fav_count: 57
  entry_content: |
    dolar 2 kuruş filan düşer muhtemelen 100 milyarı abd ordusu 30 günde filan yiyor.
  entry_channels: 
- entry_name: |
    video oyunları sanat mıdır tartışması
  entry_id:  85341605
  entry_writer: naturral disasster
  entry_fav_count: 52
  entry_content: |
    (bkz: <a class="b" href="/?q=the+witcher+3+wild+hunt">the witcher 3 wild hunt</a>) oyunda klasik, yöresel müzikten, resim, heykel sanatına, edebiyattan, tiyatroya kadar herşeyi bulabilirsiniz. bize yaşattığı duygulara girmedim bile.<br/><br/>edit: oyun müziklerine bayılanlar için;<br/><br/>(bkz: <a class="b" href="/entry/85139370">#85139370</a>)
  entry_channels: anket,sanat,oyun,teknoloji
- entry_name: |
    tek eşliliğin erkek için işkence olması
  entry_id:  85357522
  entry_writer: uykututmaz
  entry_fav_count: 43
  entry_content: |
    açık söyleyeyim <a class="b" href="/?q=okumad%c4%b1m">okumadım</a><br/>35 yaşındayken kendini delikanlı görüp birden fazla kadınla beraber olmayı kendine hak görüyorsun ya, o pipi her zaman o performansta çalışmayacak bunu unutma. bu da doğa kanunu.
  entry_channels: anket,ilişkiler
- entry_name: |
    seçim bir siyasi faaliyet değildir
  entry_id:  85350519
  entry_writer: anengineer
  entry_fav_count: 40
  entry_content: |
    seçimle siyasetin ne alakası var?<br/><br/>sn. binali yıldırım doğru söylüyor.<br/><br/>seçim dediğin şey, tiyatrodur ülkemizde..
  entry_channels: siyaset,haber
- entry_name: |
    en güzel ps1 oyunu
  entry_id:  85352263
  entry_writer: miyora
  entry_fav_count: 38
  entry_content: |
    (bkz: <a class="b" href="/?q=metal+gear+solid">metal gear solid</a>)
  entry_channels: anket,oyun
- entry_name: |
    yıldızlararası yolculuğun imkansız olması
  entry_id:  85358482
  entry_writer: ekmekarasiwaffle
  entry_fav_count: 37
  entry_content: |
    <a class="b" href="/?q=rumuz+ararken+deliren+adam">rumuz ararken deliren adam</a> uyardı. lorentz daralması ile ilgili kısımda bir çeviri hatası olmuş. entry güncellenecektir.<br/><br/>uzay-zaman düzlemini bükerek bir yerden başka bir yere seyahat etmek henüz bilim çevrelerince yeterince keşfedilemediğinden çokça konuşulan bir konu değil. ancak ışık hızına yakın <a class="b" href="/?q=relativistik">relativistik</a> hızlarda galakside yıldızlararası seyahat etmek bir süredir gündemi meşgul etmekte. sinema filmleri ve popüler bilim yazılarıyla insanların bu konuya ilgisi her geçen gün artmakta. bu konuda <a class="b" href="/?q=quora">quora</a>'da rastladığım bir akademik yayını inceledim. makale <a class="b" href="/?q=john+hopkins+%c3%bcniversitesi">john hopkins üniversitesi</a> ve <a class="b" href="/?q=california+%c3%bcniversitesi">california üniversitesi</a> araştırmacıları <a class="b" href="/?q=william+edelstein">william edelstein</a> ve <a class="b" href="/?q=arthur+edelstein">arthur edelstein</a> tarafından yazılmış. merak edenler için open source <a rel="nofollow noopener" class="url" target="_blank" href="http://dx.doi.org/10.4236/ns.2012.410099" title="http://dx.doi.org/10.4236/ns.2012.410099">link</a> bırakıyorum. makaleye göre, yukarıda bahsettiğim sebeplerden ötürü bu yolculuğa yaklaştık algısı oluşsa da, henüz bu yolculuğa çok, çok ama çok uzaktayız. hatta güneş sisteminin dışına bile çıkmaya çok uzaktayız.<br/><br/>öncelikle 1977'de uzaya gönderilen <a class="b" href="/?q=voyager+1">voyager 1</a> probu şu anda boş uzayda 17 km/s ya da 61.000 km/h hızla hareket halinde ve güneş sisteminin sınırlarına ulaşmış durumda. bu hızlarla hareket edilecek olunursa 4.3 ışık yılı uzaklıktaki en yakın <a class="b" href="/?q=alpha+centauri">alpha centauri</a> yıldız sistemine ulaşması 75.000 yıl sürecektir. bu yüzden yıldızlararası bir yolculuk için relativistik hızlar gerekli olmaktadır. bu hızlarda dünyadaki bir gözlemci için binlerce yıl bile geçse uzay aracındaki bir kişi için belki de birkaç yıl geçecektir. bu zamanın hangi hızda ne kadar yavaşladığını öğrenmek için matematiksel olarak anlaşılması kolay <a class="b" href="/?q=%c3%b6zel+g%c3%b6relilik+form%c3%bclleri">özel görelilik formülleri</a> incelenebilir. örneğin bu formüle göre ışık hızına oldukça yakın belirli bir hızda dünyada geçen 25.000 yıl uzay aracında yaklaşık 20 yıla tekabül etmektedir. ancak burada sadece zamanın değil uzayın da göreliliği söz konusu. uzayın büzülerek seyahat edene göre cisimlerin boylarının kısalması durumu ortaya çıkıyor. bu yüzden halihazırda boşlukta yer alan serbest <a class="b" href="/?q=hidrojen">hidrojen</a> atomları da sıkışarak yoğunlaşmış oluyor. hikaye de aslında tam burada başlıyor.<br/><br/>ortalama bir değer olarak boş uzayda 1.8 atom/cm3 yoğunluğunda hidrojen bulunuyor. relativistik hızlarda bu hidrojen atomlarının yoğunluğu da <a class="b" href="/?q=%c3%bcstel">üstel</a> bir şekilde artıyor ve gemiye doğru gelen hidrojen atomları iyonize olarak ölümcül şiddette radyasyon yaymaya başlıyor. ayrılan proton ve elektronlar birbirlerinden bağımsız halde gemiyle etkileşime giriyorlar ve bu radyasyon sadece yolculuk yapanları saniyeler içinde öldürmekle kalmıyor, seyahat edilen araçtaki elektronik ve sistemsel ekipmanları da anında kullanılamaz hale getiriyor. bu aslında <a class="b" href="/?q=cern">cern</a> deki <a class="b" href="/?q=b%c3%bcy%c3%bck+hadron+%c3%a7arp%c4%b1%c5%9ft%c4%b1r%c4%b1c%c4%b1s%c4%b1">büyük hadron çarpıştırıcısı</a>nda çarpıştırılan protonların çok yoğun halde gemiye çarpması şeklinde düşünülebilir. iki araştırmacı tarafından bu ölümcül bombardımanı önlemek için geminin çevresine daha önce mevcut insanlı görevler için tasarlanmış alüminyumdan oluşan 10 metre çap ve 10 cm kalınlığa sahip bir tank monte edildiği düşünülmüş. ancak bu boyuttaki alüminyum bir gövdenin ağırlığının kabaca 85 metrik ton olacağı, ve protonların bu kalınlıktan hemen hemen hiç enerji kaybetmeden yine direkt olarak gemidekilere geleceği hesaplanmış. bu küre protonları kesebilecek kalınlıkta kurşun bir küre ile değiştirilmiş olsa dahi hem tank belki onbinlerce ton ağırlıkta olacak ve relativistik hızlara ulaşmaya çalışan gemi için gerekli yakıt miktari geminin kendi ağırlığının belki milyonlarca katı olacaktır. ayrıca yüksek enerjili protonların küre üzerinde tutulmasıyla bu ışımadan kaynaklı ısının soğutulmasını da bir uzay gemisinde sağlamak imkansız bir iştir.<br/><br/>şu anda güneşten gelen yüklü parçacıklar dünyanın manyetik alanıyla etkileşerek çoğunluğu dünyamıza uğramadan uzay yolculuğuna devam etmekte. eğer gemide de tıpkı bu şekilde yüklü parçacıkları saptırabilecek bir manyetik alan oluşturulursa gemidekiler de bu bombardımandan kurtulabilecektir. ancak gemiyi koruyabilecek bir manyetik alanın en az 5 km çapa sahip olması gerektiği hesaplanmış. bu büyüklükte bir manyetik alanı da yine çok büyük kütlelerdeki mıknatıslar oluşturabilecektir. bu büyüklükte mıknatısları üretmek hem zor, hem de yukarıdaki kütlesel etmenler yine işin içinde olmaktadır. ayrıca manyetik alanın kusursuz ve gemi içerisinde olmayacak şekilde çalışması gerekir ki <a class="b" href="/?q=eddy">eddy</a> akımları ile oluşan yön değiştirmelerle de parçacıkların gemiye yönelmemesi sağlanabilsin. bu derece kusursuz bir manyetik alanı oluşturmak da oldukça zordur. bu alanın elektromıknatıslar ile oluşturulabileceği düşünülse dahi ısınma ve enerji gereksinimi yine önümüze büyük engel olarak çıkacaktır.<br/><br/>iki araştırmacının yaptığı çalışmaya göre hidrojen bombardımanın etkili olamayacağı hızlar v = 0.5c yani ışık hızının yarısındaki hızlar olmaktadır. ancak bu hızlarda da zamanın göreliliği çok düşük olacaktır. dünyadaki bir gözlemci için yolculuk yapana göre zaman sadece %15 daha hızlı akacaktır. bu da galaksi içerisindeki seyahati dahi imkansız hale getirmekte, sadece birkaç komşu yıldıza ziyareti mümkün kılmaktadır.
  entry_channels: bilim,seyahat
- entry_name: |
    düzenli ilişki dururken one night tercih eden kişi
  entry_id:  85355331
  entry_writer: whisky river
  entry_fav_count: 35
  entry_content: |
    one night nedir amk.tek gece deyince anlaşılmıyor mu?illa bir ingilizce şey sokacaksiniz one night
  entry_channels: ilişkiler
- entry_name: |
    memur olup sıradan bir hayatla geberip gitmek
  entry_id:  85357544
  entry_writer: guvenc11
  entry_fav_count: 32
  entry_content: |
    özel sektördeki her an 'işten çıkarılma' gibi stres ve kaygıyı ensede hissetmektense memur olup gebermenin daha evlâ olduğu durum.
  entry_channels: anket,ilişkiler
- entry_name: |
    ali ece
  entry_id:  85337516
  entry_writer: callecorpac
  entry_fav_count: 31
  entry_content: |
    her çıktığı programı izlediğim yorumcu.<br/><br/>konuşma tarzını,beresini,esprilerini,futbola bakış açısını çok severim.sürekli takip ederim.<br/><br/>ama konu liverpool’a gelmesin lütfen.gerçekten muhabbet çekilmez oluyor.<br/>şu türkiye’deki liverpool abartısından dolayı everton fanatiği olacam amk.
  entry_channels: spor,tv
- entry_name: |
    2000 yılındaymış gibi yazmak
  entry_id:  85352967
  entry_writer: sise icinde sms bekleyen cocuk
  entry_fav_count: 31
  entry_content: |
    x: slm<br/>y: slm<br/>x: asl?<br/>y: 17 m<br/>x: m too bye
  entry_channels: anket
- entry_name: |
    teoman'ın en iyi şarkısı
  entry_id:  85360469
  entry_writer: doremifa
  entry_fav_count: 27
  entry_content: |
    hiç düşünmeden (bkz: <a class="b" href="/?q=mavi+ku%c5%9f+ile+k%c3%bc%c3%a7%c3%bck+k%c4%b1z">mavi kuş ile küçük kız</a>)<br/><br/>biraz düşününce (bkz: <a class="b" href="/?q=%c3%a7%c3%b6lde+%c3%a7i%c3%a7ek">çölde çiçek</a>)<br/><br/>o bitince (bkz: <a class="b" href="/?q=rapsodi+istanbul">rapsodi istanbul</a>)<br/><br/>kararsız kaldım son kararım (bkz: saat 03:00)<br/><br/>yok yok bu sefer kesin. (bkz: <a class="b" href="/?q=ne+ekmek+ne+de+su">ne ekmek ne de su</a>)<br/><br/>hepppsi güzelmiş be. varol teoman.
  entry_channels: anket,müzik
- entry_name: |
    türk sinema tarihinin en iyi filmi
  entry_id:  85360613
  entry_writer: delizekali
  entry_fav_count: 26
  entry_content: |
    benim açımdan hiç şüphesiz "kibar feyzo" dur.<br/><br/>her repliği efsane, hem sosyal mesaj veriyor hem düzeni eleştiriyor hem güldürüyor, oyunculuk mükemmel, senaryo olsun, yönetmenlik olsun her açıdan dört dörtlük bir film.
  entry_channels: anket,tarih,sinema
- entry_name: |
    kediyi canlı canlı yakan cani
  entry_id:  85361864
  entry_writer: super frog
  entry_fav_count: 16
  entry_content: |
    sözlükteki hayvan düşmanı orospu evlatlarını mutlu eden bir piç daha.<br/><br/>açmadım, açmam.
  entry_channels: haber
- entry_name: |
    viskinin yanında iyi gidenler
  entry_id:  85353186
  entry_writer: heartbroker
  entry_fav_count: 13
  entry_content: |
    (bkz: şahsi içiş seklim; buzsuz sek, yanında kola ve çikolata.)<br/><br/>aga bu nedir ahahh allah canını almasın öldüm gülmekten. <br/>sek içiyor arkadaş ahahhah.
  entry_channels: anket,yeme-içme
- entry_name: |
    age of evlerinin elde kalması
  entry_id:  85330607
  entry_writer: erhanmartin
  entry_fav_count: 11
  entry_content: |
    ben bile aoe oynarken evleri bu kadar yan yana yapmıyorum ki saldırıda hepsi hasarlanmasın diye. adam gerçek hayatta sadece ev basmış. <br/><br/>yapsana skirmisher üretim yeri, development center falan. hadi onu geçtim, okçu kuleleri bile yok!
  entry_channels: haber
- entry_name: |
    ekşi yazarıyım dendiğinde gelen tepki
  entry_id:  85330395
  entry_writer: kulup raki beyaz leblebi
  entry_fav_count: 9
  entry_content: |
    liseliler bilmez eskiden "kaçıncı nesil" şeklinde sıkıntılı cevaplar vardı.
  entry_channels: ekşi-sözlük
- entry_name: |
    izlenesi netflix türkiye içerikleri
  entry_id:  85345458
  entry_writer: eleutheiros
  entry_fav_count: 8
  entry_content: |
    (bkz: <a class="b" href="/?q=izlenesi+netflix+t%c3%bcrkiye+i%c3%a7erikleri">izlenesi netflix türkiye içerikleri</a>)
  entry_channels: anket,tv
- entry_name: |
    new york'taki çıplak yoga sınıfı
  entry_id:  85333928
  entry_writer: kelly bundey
  entry_fav_count: 7
  entry_content: |
    ciplaklik modernlik gostergesi degildir ama ciplaklarin hicbir endise etmeden ciplakligini sergileyebilme ozgurlugu modernlik gostergesidir.
  entry_channels: spor
- entry_name: |
    en iyi gitar solosu
  entry_id:  85356154
  entry_writer: seferoglu suphi
  entry_fav_count: 5
  entry_content: |
    bişeyin en iyisini seçebilen insanları kıskanıyorum. <br/><br/>hiç mi aklınız diğerinde kalmıyor ya da başka birine haksızlık ettiğinizi düşünmüyorsunuz lan ?
  entry_channels: anket,müzik
- entry_name: |
    ekşi itiraf
  entry_id:  85342181
  entry_writer: gri balikcil
  entry_fav_count: 5
  entry_content: |
    iş hayatından o kadar sıkıldım ki. bütün o saçmalıklardan, elimin bir ters hareketiyle bütün şirketin batma tehlikelerinden filan. ağzımdan çıkan en ufak yanlış sözle en kıymetli müşterinin siktir olup gidebilme ihtimallerinden. hepinizin canı cehenneme deyip gitmek istiyorum. bir şeyler yetiştirmek, bu beton yığınlarına tıkılmış iğrenç negatif insan enerjisi yerine yaşayan ve büyüyen bir şeylerin güzel enerjisine maruz kalmak istiyorum. insan artık bana enerjiyi içinde kıstırıp örümcek ağı gibi örüp bütün pislikleri o ağa toplayan sonra da kirletip etrafı zehirleyen bir şey gibi görünüyor. oysa o enerjinin içimizden akıp geçmesi ve paylaşılması, gerekirse topraklanması gerekiyor. hasta ruhlara maruz kalmaktan bıktım.
  entry_channels: anket,ilişkiler,ekşi-sözlük
- entry_name: |
    brexit filminin fragmanındaki türkiye nefreti
  entry_id:  85359269
  entry_writer: jotunogullari
  entry_fav_count: 2
  entry_content: |
    türkiye'de nasıl akp ve rte seçimden önce çıkıp eyyy amerika, eyy almanya diye bağırıp oy topluyorsa aynısını brexit için yaptılar. günah keçisi türkiye oldu. çünkü avrupa birliğinde değiliz ama bir ara girme muhabbeti vardı. mesela ingiltere'de polonyalılar, romenler ve bulgarlarda sevilmez ama avrupa birliğinde oldukları için ingiltereye doluştular zaten. eğer onlardan biri avrupa birliğinde olmasaydı brexit kampanyasında bu ülkelerden birine sövdüklerini görebilirdik. hemen akp çomarına bağlayıp türkleri sevmiyorlar tribine girmeyin.<br/><br/>ayrıca ingiltere türkler için avrupadaki en rahat ülkedir. vize kolay verirler. zaten ankara anlaşmasıyla gelen gırla adam var. türklere ırkçılık sıfıra yakındır. ingilizler genel olarak türklere karşı iyidir. diplomaside de dikkat ederseniz hiçbir problemimiz yoktur. hiç reis ingiltereye atar gider yapmaz. bütün ajan politikacılar ingilterede okumuştur zaten. gül, şimşek vb.
  entry_channels: siyaset,sanat,sinema,haber,ekonomi
- entry_name: |
    erkek yazarlardan kadın yazarlara sorular
  entry_id:  85331808
  entry_writer: nutellali balik ekmek
  entry_fav_count: 1
  entry_content: |
    pişi olsa da yesek dediğimde;<br/><br/>"onu yapmakta ne var ki. yağ, un, tuz" diyorsunuz da neden yapıp getirmiyorsunuz minnoşlar?
  entry_channels: anket,ilişkiler,ekşi-sözlük
- entry_name: |
    beşiktaşlı sözlük yazarları
  entry_id:  85354948
  entry_writer: emrngz
  entry_fav_count: 1
  entry_content: |
    şahsım istanbullu bile değil.
  entry_channels: spor,anket,ekşi-sözlük
- entry_name: |
    ekşi sözlük dertleşecek insan veritabanı
  entry_id:  85352485
  entry_writer: facade architecture
  entry_fav_count: 0
  entry_content: |
    bugün bende buralardayım arkadaşlar ağır vakalar dışında elimden geldikçe yardımcı olmaya çalışırım..
  entry_channels: anket,ilişkiler,ekşi-sözlük
---
