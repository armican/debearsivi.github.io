---
layout: post
title: 15 Ocak 2020 Ekşi Sözlük Debe
data:
- entry_name: |
    bekarlardan daha çok vergi alınsın
  entry_id:  100512354
  entry_writer: dar gelirli buyuk is adami
  entry_fav_count: 1326
  entry_content: |
    ödleri kopuyor bir yerlerde birileri güzel hayatlar yaşayacak diye.. herkesi kendileri gibi kokuşmuş hayat yaşamaya mahkum etmektir istedikleri.
  entry_channels: siyaset,ilişkiler,haber,ekonomi
- entry_name: |
    kızını koruyan babaya 12 yıl hapis verilmesi
  entry_id:  100521198
  entry_writer: tounela
  entry_fav_count: 1154
  entry_content: |
    başlığın tam hâli, "kızının psikopat tacizcisini öldüren babaya 12 yıl hapis verilmesi" olacaktı.<br/><br/>haberi okudum ve resmen kanım dondu. ben böyle psikopat böyle orospu çocuğu bir tacizci ilk defa gördüm ya da hepsi genelde böyle oluyor da haberim yok. <br/><br/>kızını uzun zamandır taciz ve tehdit eden hatta ailesini komple taciz ve ölüm tehditi eden bir yaratığa aslan yürekli babanın son noktayı koyması olayıdır. bu babanın kesinlikle hapis yatmaması gerekir. tacizcinin mesajlarını okuduğunuzda siz de sinir krizine girebilirsiniz. bence buradan bir kamuoyu oluşturup babanın daha az ceza almasını sağlayabiliriz. <br/><br/>babaya da buradan iki çift lafım var; aslan yürekli kahraman; bu ülkeden iğrenç psikopat bir tacizci sildiğin için sana teşekkür ederim. <br/><br/><a rel="nofollow noopener" class="url" target="_blank" href="https://bundle.app/SZVs5D6K">https://bundle.app/szvs5d6k</a><br/><br/>edit: insanlarımızın arasında gerçekten vicdan yoksunu, tacizciyi savunan koruyan adamlar var. hayır yaşanan taciz olayı tacizin çok çok ötesinde. nasıl olur da bu olay bu kadar basite indirgenir??! <br/><a rel="nofollow noopener" class="url" target="_blank" href="https://eksiup.com/p/u2324628z7yg">https://eksiup.com/p/u2324628z7yg</a><br/><br/>bence çocuklarımıza ilk vereceğimiz değer vicdan olmalıdır. vicdanlı bireyler yetiştirmeliyiz ki, bu tür olayların bir nebze de olsa önüne geçelim.
  entry_channels: haber
- entry_name: |
    sana vurana sen de vur diyen ebeveyn
  entry_id:  100512388
  entry_writer: 2bira1sigara
  entry_fav_count: 1011
  entry_content: |
    burası türkiye sen de vurmazsan sana daha çok vururlar.
  entry_channels: ilişkiler,eğitim
- entry_name: |
    almanya'nın bekar kızını diyanetin kurdu haberi
  entry_id:  100527698
  entry_writer: imamihanife
  entry_fav_count: 521
  entry_content: |
    10 kere okuyup hiçbir şey anlamadığım başlıktır.<br/><br/>taksim dayı sen misin yoksa?<br/><br/>edit: başlığın gizemi çözüldü arkadaşlar:<br/><br/>almanya'nın - (bkz: <a class="b" href="/?q=almanya%27n%c4%b1n+13+milyar+500+m.+b%c3%bct%c3%a7e+fazlas%c4%b1">almanya'nın 13 milyar 500 m. bütçe fazlası</a>)<br/>bekar - (bkz: <a class="b" href="/?q=bekarlardan+daha+%c3%a7ok+vergi+al%c4%b1ns%c4%b1n">bekarlardan daha çok vergi alınsın</a>)<br/>kızını - (bkz: <a class="b" href="/?q=k%c4%b1z%c4%b1n%c4%b1+koruyan+babaya+12+y%c4%b1l+hapis+verilmesi">kızını koruyan babaya 12 yıl hapis verilmesi</a>)<br/>diyanetin - (bkz: <a class="b" href="/?q=diyanetin+faize+caiz+demesi">diyanetin faize caiz demesi</a>)<br/>kurdu haberi - (bkz: <a class="b" href="/?q=yeni%c5%9fafak%27%c4%b1n+fet%c3%b6%27y%c3%bc+in%c3%b6n%c3%bc+kurdu+haberi">yenişafak'ın fetö'yü inönü kurdu haberi</a>)<br/><br/>gündemdeki başlıklardan oluşturulmuş karma anlamsız söz öbeği.<br/>( <a class="b" href="/?q=marsli+gocmen">marsli gocmen</a> yazardan alıntıdır )
  entry_channels: siyaset
- entry_name: |
    alevilerin pek de hoşgörülü olmamaları
  entry_id:  100519370
  entry_writer: hayaletciniz
  entry_fav_count: 428
  entry_content: |
    ulan geldin açım dedinde soframıza mı buyur etmedik.<br/>insan mı yaktık.<br/>yurtta kalan erkek çocuklarına mı tecavüz ettik.<br/>ne yaptık size orospu çocukları
  entry_channels: siyaset
- entry_name: |
    diyanetin faize caiz demesi
  entry_id:  100511918
  entry_writer: girabic
  entry_fav_count: 379
  entry_content: |
    açıklamanın en önemli noktası şu kısım bence;<br/>"bu itibarla, devlet toki’nin bu uygulamasında başka bir yolla konut alma imkânı tanımadığından, belirtilen niyet ve amaçlar doğrultusunda söz konusu projeden yararlanmak caizdir."<br/><br/>yani sen faizle al senin için caiz kardeşim. çünkü devlet sana başka bir yolla konut alma imkanı tanımıyor, günahı sevabı devletin diyor.
  entry_channels: siyaset,haber,ekonomi
- entry_name: |
    13 ocak 2020 gıdada taklit ve tağşiş listesi
  entry_id:  100505723
  entry_writer: cozulemeyen adam
  entry_fav_count: 297
  entry_content: |
    hepsine tek tek baktım. zannedersem en masumu, "tam yağlı keçi peynirine, inek sütü karıştıran" firmadır.
  entry_channels: sağlık,haber,yeme-içme,ekonomi
- entry_name: |
    almanya'nın 13 milyar 500 m. bütçe fazlası
  entry_id:  100523281
  entry_writer: spaykymoon
  entry_fav_count: 246
  entry_content: |
    başlığın tam hali " almanya'nın 13 milyar 500 milyon euro bütçe fazlası olması" olacaktı ama malum karakter/ harf sınırı..<br/><br/>almanya'da federal hükümetin 2019 yılı bütçesinin 13 milyar 500 milyon euro ile rekor fazla vermesi, mali kaynakların nasıl harcanması gerektiğine ilişkin tartışmalara yol açtı.<br/><br/>vay arkadaş ya! bu bizi kıskanan almanya'nın ne boktan dertleri var. insan gerçekten de kıskanıyor!<br/><br/><a rel="nofollow noopener" class="url" target="_blank" href="https://twitter.com/gercekgundem/status/1217067950609457153?s=19" title="https://twitter.com/gercekgundem/status/1217067950609457153?s=19">link</a>
  entry_channels: haber,ekonomi
- entry_name: |
    suriyeli olduğu için düşük not verilen çocuk
  entry_id:  100528731
  entry_writer: inat edip saunada terlemeyen adam
  entry_fav_count: 181
  entry_content: |
    bu çocuğa üzüldüm. ama 1 kişiye üzüldüğüm için de 5 milyon suriyeli'nin verdiği zararı görmeyecek değilim.
  entry_channels: siyaset,tv,haber,eğitim
- entry_name: |
    eğitim seviyesi arttıkça kişide artan şey
  entry_id:  100528946
  entry_writer: pesimistdoctor
  entry_fav_count: 135
  entry_content: |
    anksiyete ve depresyon ikilisidir.
  entry_channels: anket,eğitim
- entry_name: |
    14 ocak 2020 kadıköy çekici terörü
  entry_id:  100527118
  entry_writer: 3310merkez
  entry_fav_count: 133
  entry_content: |
    <a rel="nofollow noopener" class="url" target="_blank" href="https://twitter.com/Medyanin50Tonu/status/1217094236815929344?s=20" title="https://twitter.com/Medyanin50Tonu/status/1217094236815929344?s=20">anlatmaya gerek yok... görüyorsunuz...</a><br/><br/><a rel="nofollow noopener" class="url" target="_blank" href="https://www.sozcu.com.tr/2020/gundem/bunun-adi-cekici-teroru-yasli-kadin-olumden-dondu-5567695/" title="https://www.sozcu.com.tr/2020/gundem/bunun-adi-cekici-teroru-yasli-kadin-olumden-dondu-5567695/">bu da sözcü linki ve de haberi</a><br/><br/>içişleri bakanlığının genelgesi var.. <br/>araç trafiği engellemiyorsa çek-me-ye-cek-si-niz..<br/>anlaşılan polis bakanı sallamıyor.. <br/>kaşla göz arasında çekiciyi yanaştırıyorlar.. <br/>polis araçtan inmeye bile zahmet etmiyor..<br/>arabanın çekiciye yüklenmesi 30 saniye..<br/>içinde kadın mı var, çocuk mu var;<br/>kim uğraşacak onla..<br/>nasıl olsa araç sahiplerinde para bol.. <br/>gelip parayı bayılıp aracını alacak.. <br/>malı da yediemin otoparkının sahibiyle anlaşmalı olduğu polis ve çekici şoförü götürecek.. <br/>allah sizin belanızı versin..<br/><br/>edit: suç kadında diyen sığırlarda başlığa yazmış.. <br/>kapıyı açmasaymış; <br/>panik yapmasaymış;<br/>atlamasaymış;<br/>falan filan.. <br/>bakalım sen 80 yaşına gelince çişini tutabilecek misin? bay çok bilmiş <br/>wallahi siz eğitilmezsiniz...
  entry_channels: siyaset,haber,otomotiv,ekonomi
- entry_name: |
    genç işsizlik oranı %27 iken tarlaların boş olması
  entry_id:  100530754
  entry_writer: tovbebismillah
  entry_fav_count: 113
  entry_content: |
    tarlayı ekip biçme giderleri, ürünü sattığında elde ettiğin gelirin altında ya da eşit ise ekip biçmezsin. çünkü zarar edersin. anlaşılmayacak bir tarafı yok. 60-70 iq yeterli. hükümet adam gibi tarım politikası üretsin. makam arabasına binip devlet benziniyle göt gezdirmekle olmuyor o işler.<br/><br/>ek:15 senedir tarım politikamız yetersiz gelen ürünü yurt dışından ithal edip yerli üreticinin belini kırmaktan ibaret. <br/><a rel="nofollow noopener" class="url" target="_blank" href="https://www.tarimdanhaber.com/tarimsal-ekonomi/turkiye-hangi-ulkeden-hangi-tarim-urununu-ithal-ediyor-h10265.html" title="https://www.tarimdanhaber.com/tarimsal-ekonomi/turkiye-hangi-ulkeden-hangi-tarim-urununu-ithal-ediyor-h10265.html">https://www.tarimdanhaber.com/…l-ediyor-h10265.html</a>
  entry_channels: anket,ekonomi
- entry_name: |
    14 ocak 2020 fenerbahçe sk'nın attığı tweet
  entry_id:  100511034
  entry_writer: tokmaki
  entry_fav_count: 96
  entry_content: |
    uğur boral ve bekir irtegün'ün fetö itirafçısı olması, sohbetlere emre, gökhan, memeh topal ve alper potuk'un katıldığını anlatması ile balyoz ve ergenokon savcılarının fenerbahçe formasıyla maça çıkmaları ve türkçe olimpiyatları sponsorunun koç grubu olması vs vs vs :) hakikaten gol olmuş :d
  entry_channels: spor
- entry_name: |
    rise of empires ottoman
  entry_id:  100515652
  entry_writer: zhafrus
  entry_fav_count: 69
  entry_content: |
    celal şengör'ü de istanbul'dan kaçan şişman burjuva yapsalardı tadından yenmezdi.
  entry_channels: tv,tarih,teknoloji,spoiler
- entry_name: |
    her gün en az 2 litre su içen insan
  entry_id:  100523056
  entry_writer: martinexz
  entry_fav_count: 59
  entry_content: |
    alışkanlık haline getirdim ama bi faydasını görmedim. (içsel faydaları bilemem).<br/>masabaşında şişen olsun ve büyükçe bir su bardağın olsun. bir dönem sonra ula bu bardak niye buş duruyor deyip dolduruyorsun, madem bardak dolu içeyim deyip içiyorsun böyle bir döngü sürüp gidiyor.
  entry_channels: sağlık,yeme-içme
- entry_name: |
    libido yükselten şeyler
  entry_id:  100528161
  entry_writer: askeriucret750tlolsun
  entry_fav_count: 52
  entry_content: |
    libidomun yükselmesi için herhangi bir olaya gerek yok.
  entry_channels: anket,ilişkiler,sağlık,yeme-içme
- entry_name: |
    güneşin 8 dakika önceki halini görüyor olmamız
  entry_id:  100533527
  entry_writer: aydayasayanaslan
  entry_fav_count: 45
  entry_content: |
    aynaya baktığımda mili saliseler önceki bana baktığımdan daha acıklı değildir.
  entry_channels: bilim
- entry_name: |
    türkiye'nin en yobaz şehri
  entry_id:  100495388
  entry_writer: fcstp
  entry_fav_count: 43
  entry_content: |
    erzurumla bayburt kapisir. kadinlar pazara cikamiyor amk ne konyasi.
  entry_channels: siyaset,anket
- entry_name: |
    kübra par
  entry_id:  100531697
  entry_writer: captain torero
  entry_fav_count: 43
  entry_content: |
    ülkemizi istila esen 1 milyona yakın suriyeli tosunun kendi ülkeleri için savaşmayıp, burada zil zurna sarhoş olup yılbaşını kutlamalarını can siperana savunan haberci (!). bu kadar çok seviyorsan al iki tanesini evine besle samimiyetini görelim.<br/><br/>keşke bu tavrını binali yıldırım'ın, bülent arınç'ın, mahir ünal'ın karşısında da sergileyebilseydin kübra hanım. onların karşısında esas duruşta dururken şimdi omurgası alınmış gibi jölevari bir kıvamda masaya yayılıyorsun. <br/><br/>bakınız bir yazar arkadaş hakkında ne güzel bir tespit girmiş (bkz: <a class="b" href="/entry/71202641">#71202641</a>)<br/><br/>bilmeyenler için söyleyelim, kendisi <a class="b" href="/?q=ahmet+hakan">ahmet hakan</a>'ın kardeşi ile evlidir. bu tür insanlar neden hep birilerinin akrabası olmak zorunda. aynı şekilde <a class="b" href="/?q=selman+%c3%b6%c4%9f%c3%bct">selman öğüt</a> de <a class="b" href="/?q=hilal+kaplan">hilal kaplan</a>'ın kayınbiraderi mesela.
  entry_channels: tv
- entry_name: |
    erken ölmeseydi başka olurdu denilen insanlar
  entry_id:  100536320
  entry_writer: flascry
  entry_fav_count: 36
  entry_content: |
    hmm tam mustafa kemal atatürk diyip fav almalık başlıkmış
  entry_channels: anket
- entry_name: |
    kemalist öğretinin ülkeye çözüm getirmemesi
  entry_id:  100531301
  entry_writer: girabic
  entry_fav_count: 29
  entry_content: |
    1950'lere kadar mukemmel işler yapıldı.<br/>sonrasında da bi bitmediniz ki aynı işler yapılmaya devam etsin. <br/>halka dayatildi dediğiniz şeylerin hepsi halkın faydasına olan şeylerdi. <br/>çok basit bir örnek seçme ve seçilme hakkı da sizin deyiminizle halka dayatılanlardan sadece biri.
  entry_channels: siyaset,tarih
- entry_name: |
    unutulmayan bilgisayar oyunu replikleri
  entry_id:  100495343
  entry_writer: schizop
  entry_fav_count: 26
  entry_content: |
    for mother russia!<br/>kirov reporting
  entry_channels: anket,oyun
- entry_name: |
    hayatından kolaylıkla insan çıkarabilmek
  entry_id:  100535200
  entry_writer: skymakesmeblue
  entry_fav_count: 23
  entry_content: |
    son on yılda kendimi çokça geliştirdiğim yeteneğim. bir kalemde silmek deyimi hayat mottom haline geldi. kimseyi kazanmaya filan çalışmıyorum. baktım ki bana zararı var, negatif enerjisi var kaç yıllık arkadaşımmış diye bakmadan hemen hayatımdan çıkarıyorum.
  entry_channels: ilişkiler
- entry_name: |
    tuğçe kazaz'ın ölümle tehdit edilmesi
  entry_id:  100535322
  entry_writer: aci sevmeyen mazosist
  entry_fav_count: 21
  entry_content: |
    napalım bu konu hakkında mesela?<br/><br/>t: düşünce özgürlüğü adı altında karşıt görüşlere hakaret eden bir kadının kendini acındırmak için paylaştığı gönderi.
  entry_channels: 
- entry_name: |
    ukraynalı kızın direk dansı
  entry_id:  100535375
  entry_writer: zevzek herif
  entry_fav_count: 19
  entry_content: |
    çok merak ediyorum, şu direk dansları erkek hemcinslerimden bi bana mı seksi gelmiyor. olmuyor kardeşim sevemiyorum. çok denedim, canlı izledim vs ama bi hareket olmuyor bende. sıradan bi kadın karşımda dudağını ısırsa daha fazla etkileniyorum. ama bu beni etkilemiyor, bana amerikan futbolu ve beyzbol gibi amerika’nın kültür emperyalizmi gibi geliyor. her filmde gösterip zorla beğendirmeye çalışıyorlar sanki. <br/>tanım; güzel bi hanım kızın değişik hareketler yaptığı videodur.
  entry_channels: 
- entry_name: |
    tutunamayanlar (dizi)
  entry_id:  100535007
  entry_writer: seni beklemez zaman
  entry_fav_count: 18
  entry_content: |
    hay sikeyim leyla ile mecnun fetişinizi ya. kardeşim biz de izledik ama bitti leyla ile mecnun. dizi bitti ama bunların yapmacık romantizmi bitmedi. ulan dizinin oyuncuları bile unuttu artık diziyi. yeter yahu iki entryden biri leyla ile mecnun amk. tamam benzer yönleri var mı var. eee amk? dizinin yayınlandığı her bölüm aynı şeyi mi yazacaksınız? yahu daha 2. bölüm yayınlandı. bi bırakın şans verin karakterler otursun, senaryo ve hikaye otursun, ekip de kendini bulsun.<br/>ekşi sözlük hiçbir siki beğenmeme timi yine iş başında. birisi gelir trt'de yayınlanıyor diye izlemem der. izleme kardeşim. trt'de yayınlanıyor diye izlemiyorsan bi zahmet trt'de yayınlanan diziye de entry girme. dalyarak otorite sanıyor kendini amk. orda yayınlanıyor diye izlemiyormuş. ee bize ne lan?<br/>ikinci bölüm de gayet güzeldi. her hafta üstüne koyarak gideceğini düşünüyorum. henüz üst seviye bi ritim tutturamadı dizi. yer yer yine güzel göndermeler vardı. ayrıca 1 liraya simit mi kaldı be kardeşim? hele de istanbul gibi yerde. saçmalamışlar orda.
  entry_channels: tv
- entry_name: |
    14 ocak 2020 kayserispor fenerbahçe maçı
  entry_id:  100536831
  entry_writer: emiravci
  entry_fav_count: 17
  entry_content: |
    (bkz: <a class="b" href="/?q=mevl%c3%bct+erdin%c3%a7">mevlüt erdinç</a>)<br/>(bkz: <a class="b" href="/?q=gizli+forvet">gizli forvet</a>)<br/><br/>78.dakikada oyundan çıkana kadar kimse tarafından görülmemiştir ama ben tek karede dahi olsam gördüm, şahitlerim var. büyük yetenek cidden süper güç gibi bir şey ya.
  entry_channels: spor
- entry_name: |
    14 ocak 2020 ispanya patlaması
  entry_id:  100536530
  entry_writer: bos gezenin bos ustasi
  entry_fav_count: 17
  entry_content: |
    --- <a class="b" href="/?q=spoiler">spoiler</a> ---<br/><br/>gecmis olsun ikinci evim:(<br/>--- <a class="b" href="/?q=spoiler">spoiler</a> --- <br/><br/>ay götüm. sen bunu yazınca kime ne faydası dokunuyor? niyeti belli de onun o niyetini siksinler.
  entry_channels: haber
- entry_name: |
    mustafa kemal atatürk'ün dediği gibi
  entry_id:  100523139
  entry_writer: ysfkpln
  entry_fav_count: 16
  entry_content: |
    geldikleri gibi giderler
  entry_channels: siyaset,anket,tarih
- entry_name: |
    sukurting
  entry_id:  100533524
  entry_writer: calimera
  entry_fav_count: 14
  entry_content: |
    hayırlı cimalar
  entry_channels: ilişkiler,sağlık
- entry_name: |
    sevgilisi varken başkasına yazan erkek
  entry_id:  100533853
  entry_writer: c est ma nick
  entry_fav_count: 13
  entry_content: |
    ha evliyken yapmış, ha sevgilisi varken. ha kadın yapmış, ha erkek. karaktersiz şahıs davranışı, aynısı kendisine yapılsa çıngar çıkartır. ayrıl ne bok yiyorsan ye bunu yapana kadar, şerefsizliğin anlamı yok.<br/><br/>edit: ne kadın ne erkek çok eşli değildir, yeni moda da bu laf oldu amk. madem çok eşli olduğuna inanıyorsun sevgili yapma, evlenme de. başkasının kanına geçme boşu boşuna, kendin gibi ucuzlarla takıl.
  entry_channels: ilişkiler
- entry_name: |
    ekşi itiraf
  entry_id:  100496345
  entry_writer: zerdust radyosu
  entry_fav_count: 12
  entry_content: |
    bazı şeylerin olması gerekiyor ve oluyor ne kadar çabalasanız da değiştiremiyorsunuz bu hayatta üzülmekten başka şansınız olmuyor
  entry_channels: anket,ilişkiler,ekşi-sözlük
- entry_name: |
    göbekli tepe ermenilerden mi kalma
  entry_id:  100535446
  entry_writer: kitap deniz gunes
  entry_fav_count: 10
  entry_content: |
    bulunamayan antik kentler icin simdiden rezervasyon yapilabiliyor mu acaba?
  entry_channels: tarih
- entry_name: |
    türkiyenin 2.14 milyar dolar cari fazla vermesi
  entry_id:  100537404
  entry_writer: tyler durdenin yedinci packi
  entry_fav_count: 6
  entry_content: |
    52.14 milyar dolarlık toprak sattığımız manasına gelir.
  entry_channels: ekonomi
- entry_name: |
    meslekdaş kişilerin birbiriyle evlenme merakı
  entry_id:  100531634
  entry_writer: henriette kleist
  entry_fav_count: 6
  entry_content: |
    (bkz: <a class="b" href="/?q=meslekta%c5%9f">meslektaş</a>)
  entry_channels: ilişkiler
- entry_name: |
    son 20 yılın en gıcık lafı
  entry_id:  100534849
  entry_writer: acugurlu
  entry_fav_count: 5
  entry_content: |
    (bkz: <a class="b" href="/?q=aynen">aynen</a>)
  entry_channels: anket
- entry_name: |
    14 ocak 2020 fenerbahçe beko asvel maçı
  entry_id:  100535071
  entry_writer: eoamt
  entry_fav_count: 5
  entry_content: |
    entry sayından, fenerbahçe'nin farklı yendiğini anlayabilirsiniz.
  entry_channels: spor
- entry_name: |
    biz bir aileyiz diyen patron
  entry_id:  100523959
  entry_writer: bizde yanlis olmaz
  entry_fav_count: 4
  entry_content: |
    (bkz: <a class="b" href="/?q=inanma+sikecek">inanma sikecek</a>)<br/><br/>meali it gibi çalış ama hakkını arama.
  entry_channels: ilişkiler
- entry_name: |
    14 ocak 2020 iran'ın abd üssünü vurması
  entry_id:  100537456
  entry_writer: kenvsryu
  entry_fav_count: 4
  entry_content: |
    an itibariyle dolar 5.88 lira olduğuna göre gene karavana olan vuruş.
  entry_channels: 
- entry_name: |
    üstteki yazarın eşkalini tarif etmek
  entry_id:  100517858
  entry_writer: o yok bu yok ne var o vakit
  entry_fav_count: 3
  entry_content: |
    pijamasını koltuk altına çeyrek kalaya kadar çeken, beyaz atletli isyankar genç.
  entry_channels: anket,ekşi-sözlük
- entry_name: |
    ramo
  entry_id:  100531519
  entry_writer: godwithuse
  entry_fav_count: 3
  entry_content: |
    çukur ' a alternatif dizi. çukur dizisi rtük tarafindan ceza yedigi zaman<sup class="ab"><a title="(bkz: swf)" href="/?q=swf" data-query="swf">*</a></sup> bu diziyle devam edecekler. <br/><br/>bi on, on bes yil onceki dizilere bakarsak kaygisizlar, cilgin bedis, ruhsar vs. ne dönemlere geldik kamil ya.
  entry_channels: tv
- entry_name: |
    14 ocak 2020 k.irak'taki abd üssüne saldırı
  entry_id:  100537545
  entry_writer: moraigne
  entry_fav_count: 3
  entry_content: |
    patlama sonucu sevinç konvoyu atan 60 iranlı ezilerek öldü.
  entry_channels: siyaset,haber
- entry_name: |
    şu anda çalan şarkı
  entry_id:  100501710
  entry_writer: mariya mandalina
  entry_fav_count: 1
  entry_content: |
    eva cassidy - feilds of gold <br/>bir kaç şarkısını daha dinliyordum severek başka hangi şarkıları var diyerek youtubeda keşfe çıkmıştım. kendi kendime bu kadın neden albüm yapmıyor diye düşündüm. aşağıdaki yorumlara bakınca cilt kanserine yenik düşüp , 96 da vefat ettiğini gördüm. hep hüzünlü bir şeylere yenik bir sesi vardı . bu şarkı senin için eva " batı rüzgarları estiğinde beni hatırlayacaksın ".
  entry_channels: anket,müzik
- entry_name: |
    hekimoğlu (dizi)
  entry_id:  100534969
  entry_writer: crysisanthem
  entry_fav_count: 1
  entry_content: |
    güzel lan.<br/>şimdi denk geldim. esas adam çok kafa. özellikle hamileliği parazit taşıyorsunuz diye betimlemesi bayağı güldürdü. <br/><br/>dobralığı falan çok iyi ya. mucize doktor gibi tutmamasının sebebi sanırım çok fazla drama eğilimleri yok. ki ben de severim mucize doktoru bu otizm muhabbetinden dolayı.. diğer karakterler de bayağı gerçekçi bence. <br/><br/>malum toplum yapısı arabesk. salya sümüklü bir dizi olmazsa tutma ihtimali düşük.<br/><br/>ama ben sevdim. <br/><br/>house ıle benziyor mu benzemiyor mu ona yorum yapamam çünkü hiç izlemedim. izlemeyi düşünmüyorum malesef. zaten bir kaç doktor dizisi daha seyredersem beyaz önlük kusacağım. ööğk!<br/><br/>burada en azından baş roldeki adamda yok.
  entry_channels: tv,spoiler
- entry_name: |
    geçmişe gidebilseydiniz hangi güne giderdiniz
  entry_id:  100536892
  entry_writer: nod33
  entry_fav_count: 1
  entry_content: |
    istemezdim. çünkü, belki değiştireceğim şey zamanda kırılmaya yol açıp ilerde çok kötü sonuçlara vesile olacak.
  entry_channels: anket,tarih
- entry_name: |
    atatürk yarım bir ilahtır türklerin babasıdır
  entry_id:  100496024
  entry_writer: gratis
  entry_fav_count: 1
  entry_content: |
    bu aralar nerelere saldıracaklarını şaşırdı bu troller. bu ülkenin kurucusuna şimdilik laf ediyorsunuz ama elbet bu devran da dönecek. allah ıslah etsin sizler gibileri.
  entry_channels: siyaset
- entry_name: |
    testosteron yükselten şeyler
  entry_id:  100537462
  entry_writer: kobayashisoze
  entry_fav_count: 0
  entry_content: |
    (bkz: <a class="b" href="/?q=squat">squat</a>) (bkz: <a class="b" href="/?q=deadlift">deadlift</a>) ve kavgaya karışmak.
  entry_channels: anket
- entry_name: |
    mehmet öz'ün kahvaltı yasaklanmalı demesi
  entry_id:  100538342
  entry_writer: başlık taşıyan
  entry_fav_count: 0
  entry_content: |
    (bkz: <a class="b" href="/?q=kahvalt%c4%b1+yapmak+yasaklanmal%c4%b1">kahvaltı yapmak yasaklanmalı</a>)
  entry_channels: sağlık,yeme-içme
- entry_name: |
    ekşi sözlük dertleşecek insan veritabanı
  entry_id:  100517299
  entry_writer: hakiki ssg benim
  entry_fav_count: 0
  entry_content: |
    eğlenceli bir hafta ortası muhabbeti arayan kişileri beklerim. yazarlar veya çaylaklar yeşillendirebilir.
  entry_channels: anket,ilişkiler,ekşi-sözlük,yeme-içme
- entry_name: |
    2019 aralık ayı doğalgaz faturaları
  entry_id:  100522421
  entry_writer: ben bunu bildim
  entry_fav_count: 0
  entry_content: |
    çok çirkin bişey yaptım ve komşunun faturasına baktım. bana 480 tl gelmişti, dedim ki acaba bizde mi sorun var komşuya ne kadar geldi dedim. 854 tl’yi görünce ne kadar tasarruflu olduğumu anladım.
  entry_channels: anket,ekonomi
---
