---
layout: post
title: 15 Ağustos 2018 Ekşi Sözlük Debe
data:
- entry_name: |
    amerikan elektronik ürünlerine boykot uygulaması
  entry_id:  80250690
  entry_writer: duygu mazosisti
  entry_fav_count: 847
  entry_content: |
    "onların iphone'u varsa, bu tarafta da samsung var" -rte<br/><br/>15 temmuzda facetime'ı da ben kullandım tabii.
  entry_channels: siyaset,teknoloji,ekonomi
- entry_name: |
    14 ağustos 2018 bedelli askerlik başvuru rezaleti
  entry_id:  80246696
  entry_writer: dont waste it taste it
  entry_fav_count: 779
  entry_content: |
    az önce askerlik şubesi asal şube müdürü tarafından aşağılayıcı tavırlara uğramam rezaleti. bedelli askerlik başvurumu yaptım, imza için bu adama gönderdiler. adamın odasını ararken "gel kardeşim geeelll bu tarafa" küçümseyici tavırlarıyla başladı. belgeleri verdim kendisine ve sandalyeye oturdum. oturur oturmaz çok sert çıkışarak "kalk o sandalyeden, sana kim otur dedi" diye azarladı beni. sandalyeye oturmamın ne sakıncası olduğunu sorduğumda da "ben buranın amiriyim, ben ne dersem onu yapacaksın" diye bağırdı. kendisine buranın bir devlet dairesi olduğunu, devlet dairelerinde vatandaşa oturma yasağının olmadığını söyledim. "ne diyorsam onu yapacaksın" diye devam eden sözlerine küçümseyici yüz mimikleri ve el kol hareketleri de eklenince tepem attı ve ben de kendisine çıkıştım, cimer'e şikayet edeceğimi belirttim. bu sırada komutan da oradaydı ve hiçbir şey yapmadı. "bedelli askerlik başvurusu yaptığım için mi böyle davranıyorsun?" diye sorunca da "al kardeşim, git kanunun verdiği haktan yararlan" dedi. "yararlanacağım tabi ki, sen mi veriyorsun bana bu hakkı!" diye cevap verdim ben de. <br/><br/>öncelikle olayı fotoğraf veya video ile ispat edemem. yazarak da olayın durumunu ve gerçekliğini tam olarak aktarmak mümkün değil. bu adamın tavırları tamamen sırf bedelli askerlik başvurusu yaptığım içindi. beni gördüğü andan odasından kibarca kovuşuna dek her şeyi bedelli askerlik başvurusu yapanlara duyduğu kini gösteriyordu. devlet memuru olduğunu ve herkese eşit davranması gerektiğini unutan bu adamı cimer'e şikayet edecegim.<br/><br/>dipnot: otur demeden oturulmazmış. sebep? devlet dairelerindeki sandalyeler niçin var? işlemler devam ederken vatandaş otursun diye. bankaya gidince izin mi bekliyorsun oturmak için? kaldı ki o bir komutan değil, normal bir memur. ben de henüz asker değilim, askerlik şubesini, yani bir devlet kurumunu kışla ile nasıl bağdaştırabiliyorsunuz?<br/><br/>dipnot2: bedelli askerlikten neden eziklik duyayim? sacmalamayin. memura onu söylemem tamamen davranislarindaki bedelli askerlik öfkesini net olarak onun ağzından duymak istememdi. çıkarsın ağzındaki baklayı, kaçak dövüşmesin.
  entry_channels: haber
- entry_name: |
    14 ağustos 2018 doların düşüş trendine girmesi
  entry_id:  80239127
  entry_writer: der meister
  entry_fav_count: 568
  entry_content: |
    (bkz: <a class="b" href="/?q=pick+noktas%c4%b1">pick noktası</a>)<br/><br/>daha yazılacak çok şey var da şimdi başıma iş almak istemiyorum. başlığı açan şahsın konu hakkındaki bilgisinin ne kadar sağlam (!) olduğu belli oluyor. aynen kanka, düşüş trendine girmiş dolar. her şey bitti, toparlandı ekonomi.<br/><br/>edit: bir sürü mesaj aldım, "peak noktası olmasın o?" diyenler var. ulan şunu da yazdırdınız ya ne diyeyim. biz de ona dikkat çekiyoruz zaten. başlık sahibi ekonomi hakkında ahkam kesip "pick noktası" demiş, ona işaret ediyoruz.<br/><br/>edit2: baslik sahibi hatasini duzeltmis. ilk actiginda "pick noktasi" yazmisti. hala bana "peak noktasi olmasin o?" diye mesaj atan gerizekalilar var. gercekten inanilmazsiniz. baslik sahibinin cakal gibi sessiz sedasiz degistirdigini haber verenlere tesekkur ederim, tek tek donemiyorum cunku ilginc bir sekilde patlamis baslik, 30-40 tane mesaj gelmis.<br/><br/>edit3: hala "peak noktası olmasın o?" diye mesaj atanların ironi yahut şakaçs gülmiksliği yaptığını varsayıyorum. umarım öyledir. yoksa gidişat kötü.
  entry_channels: ekonomi
- entry_name: |
    sevgilisini aldatmayan güvenilir nazik kibar erkek
  entry_id:  80242432
  entry_writer: luckdown
  entry_fav_count: 456
  entry_content: |
    alırım bir dal diyenlerin karşılaştıklarında tercih etmeyecekleri erkektir.
  entry_channels: ilişkiler
- entry_name: |
    erdoğan'ın atatürk'ten daha büyük bir lider olması
  entry_id:  80253744
  entry_writer: eskizozluk
  entry_fav_count: 429
  entry_content: |
    kurucu lider atatürk'e duyduğum sevgiye rağmenden sonra okumayı bıraktığım başlık.<br/>(bkz: <a class="b" href="/?q=a%c3%a7may%c4%b1n+troll">açmayın troll</a>)<br/><br/>edit: mesajlarda küfredenlere sözüm hakkımızda her ne düşünüyorsanız allah size on katını versin. ne mutlu türk'üm diyene!<br/><br/>edit2: bu işler böyle demek ki. ortaya yumurtla sonra kaybol klavye delikanlısı algı operasyoncuları. saflığımıza geldi görüntü almadık, alan paylaşır umarım.
  entry_channels: siyaset,anket
- entry_name: |
    kurban katliamdır
  entry_id:  80256402
  entry_writer: marsli gocmen
  entry_fav_count: 400
  entry_content: |
    <a class="b" href="/?q=k%c3%b6t%c3%bc+niyetli+insan">kötü niyetli insan</a> söylemi.<br/><br/>koskoca bir hayvancılık endüstrisi ortada dururken, fakir zengin her insanın yardımlaştığı bu anlamlı bayrama saldıran bir insan kötü niyetlidir. "parası olmayan et yemesin kardeşim"dir derdi.<br/><br/>"primat, virüs, evren, soykırım, sevgi, barış" gibi havalı kelimeleri kullanarak olduğunuzdan farklı göstermeyin kendinizi. yürüttüğünüz mantık üç yaşındaki çocuk seviyesinde, motivasyonunuz ise nefret ve kötülük.
  entry_channels: siyaset,anket,yeme-içme
- entry_name: |
    onların iphone'u varsa diğer tarafta samsung var
  entry_id:  80250813
  entry_writer: sananebirader
  entry_fav_count: 391
  entry_content: |
    iphone'u abd'den dolar ile alırken samsung'u kore'den öpücük ile aldığımızı öğrenmemize vesile olan açıklama.
  entry_channels: siyaset,teknoloji,haber,ekonomi
- entry_name: |
    bordo sütyen
  entry_id:  80251090
  entry_writer: sutterkane
  entry_fav_count: 315
  entry_content: |
    <a class="b" href="/?q=bordo+memeliler">bordo memeliler</a>in intikal için kullandığı mühimmat
  entry_channels: moda
- entry_name: |
    dolar güvenilirliğini yitirdi
  entry_id:  80260395
  entry_writer: koministimam2
  entry_fav_count: 246
  entry_content: |
    (bkz: <a class="b" href="/?q=dedi+kimsenin+g%c3%bcvenmedi%c4%9fi+adam">dedi kimsenin güvenmediği adam</a>)
  entry_channels: siyaset,haber,ekonomi
- entry_name: |
    maskülen erkekle yattıktan sonra gelen tiksinti
  entry_id:  80267103
  entry_writer: ebmoldcity
  entry_fav_count: 221
  entry_content: |
    (bkz: <a class="b" href="/?q=o%c4%9fronoyorom+%c5%9fo+komok+olmo+%c3%a7obonozdon">oğronoyorom şo komok olmo çobonozdon</a>)
  entry_channels: anket,ilişkiler,sağlık
- entry_name: |
    ilk kez orgazm olan kız
  entry_id:  80261076
  entry_writer: artik yazar yapsaniza
  entry_fav_count: 212
  entry_content: |
    onu ilk orgazma ulaştıran erkeği ikinci erkekte çoktan unutmuştur. kimse unutulmaz değildir. çok anlam yüklemeyin. altı üstü seks yani. ulaşmak da kolay, orgazm olmak da. amma da büyüttünüz ha. büyüdüğünüzde anlayacaksınız anlam yüklemenin saçma olduğunu. çünkü evet hepsi aynı. hiçbir fark yok bebeler.
  entry_channels: anket,ilişkiler,sağlık
- entry_name: |
    kısa boylu iri memeli zayıf kız
  entry_id:  80246011
  entry_writer: bunungibilerevericeksinodunu
  entry_fav_count: 193
  entry_content: |
    başlık altında kapanı kurmuş bekliyor herkes.başlığın altı serengeti sazlıkları gibi
  entry_channels: anket
- entry_name: |
    17'lik bir kızla 37'lik bir kadın arasındaki fark
  entry_id:  80247765
  entry_writer: oh lilian
  entry_fav_count: 163
  entry_content: |
    17'lik kizla 37 yaşında bir kadının herhangi bir kategoride ne işi var?<br/>müsade edin de 17'lik kiz, sınıf arkadaşlarıyla flört etsin. <br/>20 lerinde doğurmuş olduğunu farzedersek, anne /kız tahlili yapıyorsunuz farkında mısınız?<br/>kaldı ki resit bile değil.<br/>oldukça sapkın bir kategori gibi geldi bana.
  entry_channels: anket,ilişkiler,sağlık
- entry_name: |
    gece gece hız motorları ile ses yapan motorcular
  entry_id:  80250114
  entry_writer: men et manus
  entry_fav_count: 161
  entry_content: |
    hız motoruna bile razıyım. külüstür motorlara "patlak egzoz" takıp dddaaaarrrtt diye evin önündeki sokaktan geçen çomarlar var birde. motor çıkardığı ses kadar yol gitse o sürede 1 km uzaklaşması lazım ama bir bakıyorsun evi 10 metre geçmiş geçmemiş. burayı okuyorsanız hepinizin amk.
  entry_channels: anket,motosiklet
- entry_name: |
    14 ağustos 2018 yunan askerlerinin serbest kalması
  entry_id:  80264852
  entry_writer: wasabie
  entry_fav_count: 159
  entry_content: |
    türkiye'nin tipik dış politikasıdır. atarlanır sonra da sessiz sedasız bırakır.
  entry_channels: siyaset,haber
- entry_name: |
    çıplak uyumak
  entry_id:  80249288
  entry_writer: rockcidedeturbesi
  entry_fav_count: 155
  entry_content: |
    çırılçıplak ve üstünde çarşaf olmadan uyuyanlar gece canavar gelse napacak? ne koruyacak onları? tehlikeli harekettir. ayak bile dışarda olmayacak. çünkü canavar gelebilir.
  entry_channels: sağlık
- entry_name: |
    2 yıldır görüşülmeyen arkadaşın mesaj atması
  entry_id:  80245621
  entry_writer: huzursuz ruh sendromu
  entry_fav_count: 148
  entry_content: |
    hukuki danışmaya ihtiyacı vardır. geçen hafta başıma geldi, şaka gibi. o kadar mezun ol, mesleğe başla, buna dair gelişmeleri whatsapp durumundan görsün ama zahmet edip bir "hayırlı olsun" demesin. <br/>sonra bir akşam arasın ve şu diyalog gerçekleşsin:<br/>ben: efendim?<br/>o:alo canım naber?<br/>b: iyidir sağ ol senden?<br/>o: iyi işte. ya sana bir şey sorucam.<br/>b: aslında ben çok d...<br/>o: çok bir şey değil zaten. bizim bir arsa vardı da blablabla...<br/><br/>ben ne yaptım? insanlık edip söyledim yol yordam gösterdim. kapatırken "avukatları da sevmem ama ihi ihi" bir şeyler zırladı.<br/>bundan sonra babamın oğlu gelse nah bilgi veririm. biz bilgimizle para kazanıyoruz. haydi onu geçtim insan nankörlük yapmaz be. senin vefasızlığına rağmen seni insan yerine koyup cevap veren aklıma şaşayım ben.<br/><br/>bak hatırladım sabah sabah sinirlendim yine.
  entry_channels: ilişkiler,teknoloji
- entry_name: |
    ali koç istifa
  entry_id:  80269135
  entry_writer: thehybrid
  entry_fav_count: 137
  entry_content: |
    umarım bu başlığa yazılanlar şaka, troll gibi şeylerdir. gerçekse komik değil yani
  entry_channels: spor
- entry_name: |
    mutsuz çiftlerin 2. çocuğu yapmasının mantığı
  entry_id:  80265611
  entry_writer: larker
  entry_fav_count: 136
  entry_content: |
    "uc kisinin hayatini siktigimiz yetmedi, bari okeye dorduncu bulalim."
  entry_channels: ilişkiler
- entry_name: |
    vestel venus
  entry_id:  80257579
  entry_writer: zalim olma imana gel
  entry_fav_count: 133
  entry_content: |
    twitter'da tt olmuş. sebebi ise apple'ı boykot edip yerli diye bunu kullanma çağrısı. gururla yerliymiş bu öyle diyorlar. bütün parçaları çin'den getirip burada birleştirince yerli oluyor tabii.
  entry_channels: teknoloji
- entry_name: |
    demet akalın'ın korkunçlu eli
  entry_id:  80265193
  entry_writer: alukard
  entry_fav_count: 125
  entry_content: |
    mesele sadece manikür değil, hâlâ <a rel="nofollow noopener" class="url" target="_blank" href="https://eksiup.com/15cf57a9e81" title="https://eksiup.com/15cf57a9e81">anlamadın mı?</a>
  entry_channels: magazin
- entry_name: |
    dolar artarken işyerlerinin son durumu
  entry_id:  80242465
  entry_writer: aprio
  entry_fav_count: 111
  entry_content: |
    kur yüzünden 2 ayda 35 bin lira zarar ettim ve tl de kasıtlı olarak kaldım. bir nevi bile bile lades oldu. 9 çalışanımdan hiç kimseyi işten çıkarmadım. iş durumum 2 elemanı işten çıkarmamı gerektiriyor ama yapmıyorum. <br/>varsın yatsınlar.2 işçi işten çıkararak her ay kazanacağım fazladan 6-7 bin liraya ihtiyacım olsa bile bunu yapmıyorum. <br/>çünkü bazen paradan çok daha önemli şeyler vardır. gerekirse gelirim onlarla aynı seviyeye düşsün, hatta her ay bir miktar zarar edeyim. çok önemli değil. yinede çıkarmayacağım. <br/>bir kaç bin lira paradan daha değerli şeyler olduğuna inanıyorum. başarabileceğimize inanıyorum.
  entry_channels: siyaset,anket,ekonomi
- entry_name: |
    14 ağustos 2018 apple boykotu
  entry_id:  80250846
  entry_writer: barely evil
  entry_fav_count: 109
  entry_content: |
    devlet dairelerinde bütün bilgisayarlar microsoft kullanıyor atın hepsini birden sokağa o zaman, bütün f-16 lar da çöpe madem, neden sadece apple?
  entry_channels: siyaset,teknoloji,ekonomi
- entry_name: |
    adanalı erkeklerde görülen ekstra yakışıklılık
  entry_id:  80258959
  entry_writer: sd kart
  entry_fav_count: 105
  entry_content: |
    tam inanacağım geliyor, sonra aklıma kuran satan adanalı torbacı geliyor, bütün ciddiyet kayboluyor amk.
  entry_channels: ilişkiler
- entry_name: |
    hastane bahçesinde hayatını kaybeden doktor
  entry_id:  80267148
  entry_writer: horty
  entry_fav_count: 98
  entry_content: |
    allah rahmet eylesin.<br/><br/>kemoterapi ilaçları çalınmış. türkiye cumhuriyeti sosyal devlettir cümlesinin ne kadar büyük bir yalan olduğunu acı bir şekilde tekrar gördük. allah kimseyi düşürmesin.<br/><br/>edit: sosyal devlet lafıma takılanlar için. iki ay önce eczaneye gitmiştim. yaşlı bir teyze ve amca geldi, amca bir deri bir kemik saçları dökülmüş belli kanser hastası, amca bir kenarda oturdu teyzeye sıra verdiler. eczacı reçeteyi aldı hesapladi teyze 2970tl(kusurat sallamıyorum, an hep aklımda) tutuyor dedi. teyze tamam kızım dedi reçeteyi alayım o kadar paramız yok. sonra amcanın yanına gitti bey hadi gidelim burada bu ilaçlardan yokmuş dedi.<br/>dünya başıma yıkıldı yardım da edecek durumda değildim.<br/><br/>şimdi o kanser ilaçları niye çalındi biraz daha iyi anlayabilirsiniz.
  entry_channels: sağlık,haber
- entry_name: |
    doğu anadolu'nun paris'i neresidir anketi
  entry_id:  80260290
  entry_writer: ozgh
  entry_fav_count: 93
  entry_content: |
    zihniyet olarak tunceli, kent olarak malatya derim.
  entry_channels: siyaset,anket,tarih
- entry_name: |
    1.75 boyundaki erkek
  entry_id:  80263154
  entry_writer: agresifcan
  entry_fav_count: 92
  entry_content: |
    benim bu. oğlum siz gerçekten kompleksli misiniz lan? kadınlar size söylüyorum. şu topuklu giyince bilmem şey oluyorum diyen güruh var ya, heh siz siz evet!<br/><br/>oğlum ben doğmadan önce bana soruyorlar mı sen hangi boyu seçmek istersin, ten rengin ne olsun diye? ben mi seçiyorum bana bi söyleyin? ya da hangi insana soruyorlar? benim genetiğime bu işlendiyse ve ben bunu seçemiyorsam sen bana bedenimin boyunun uzunluğuna göre hakaret etme lüksünü nereden buluyorsun, kim veriyor lan size bu hakkı?<br/><br/>benim bu. gayet de memnunum. şu zamana kadar hayatın hiçbir yerinde de boyum dolayısıyla elimden bir fırsat kaçmadı. çünkü o bedenin merkezindeki beyin asıl seni sen yapar. ama bu ülke gibi eğitimsiz varoşların kol gezdiği bir ortamda boyunla sınıflandırılırsın.<br/><br/>o topuklunuz bi ayağınızı bilekten kırar da topal gezersiniz inşallah geri kalan hayatınızda.
  entry_channels: anket
- entry_name: |
    yazarların paraları yetmediği için alamadığı şey
  entry_id:  80244557
  entry_writer: kevinelrond
  entry_fav_count: 57
  entry_content: |
    geleceğim.
  entry_channels: anket,ekşi-sözlük,ekonomi
- entry_name: |
    kız arkadaşın başka bir erkeğin evinde kalması
  entry_id:  80268577
  entry_writer: cemal sureya
  entry_fav_count: 57
  entry_content: |
    --- <a class="b" href="/?q=spoiler">spoiler</a> ---<br/><br/>%100 orjinal boynuz bakım seti tek tuşla kapınızda<br/>--- <a class="b" href="/?q=spoiler">spoiler</a> ---
  entry_channels: ilişkiler
- entry_name: |
    türk kadınlarının orgazm olamaması
  entry_id:  80237780
  entry_writer: mukemmelligi hizlandiramayan adam
  entry_fav_count: 47
  entry_content: |
    masters&amp;johnson ve kinsey raporları, daha sonra da shere hite'nin "kadın cinselliği üzerine rapor"u yayınlandıktan sonra dünya şunun farkına vardı; kadınların çoğu orgazm olamıyor. tabi biz erkekler olayı çok yanlış anladık ve bunun sebebi kesinlikle birlikte olduğu erkektir diye düşünüp tasalandık ve bir görev bilinci edindik; "ben diğer erkeklerden farklıyım" dedik. işte burada erkek; kendini diğer erkeklerden farklı bir yere koyarak, narsistik bir tatmin sağlayıp, aynı zamanda da kadınların sorununu sadece erkeğe indirgedi. kadın orgazm olabiliyorsa erkeğin başarısı, olamıyorsa erkeğin başarısızlığı. kadın denklemde yok amına koyayım. işte burada 70'lerin ikinci kuşak feministleri "ben kendi orgazmımdan sorumluyum" diyordu. kastettiğim "türk erkeklerinin tamamı seks machine" anlamı taşımıyor elbette. ve tüm bu orgazma giden yolun, sürekli yarısından dönülmesinin yegâne sebebi kadınlardır da demiyorum. şunu kabul etmeliyiz; cinselliğin tabu sayıldığı hastalıklı bir toplumda yaşıyoruz. yemek yemek kadar, su içmek kadar ve evde uyumak kadar elzem olan bu sevişme arzusu, ülke de sadece erkeklerin tekelindeymiş gibi gösterilirse, burada daha çok ağlarsınız "aşkım yapma dediği için, orama burama dokunma dediği için, ışıkları kapat dediği için" diye.
  entry_channels: anket,ilişkiler,sağlık
- entry_name: |
    dünya hayatının iki kelimelik özeti
  entry_id:  80237963
  entry_writer: ugurlubocek
  entry_fav_count: 46
  entry_content: |
    uyku ve uyanış.<br/>insanlar uykudadır, ölünce uyanırlar.<br/>(bkz: <a class="b" href="/?q=hz.+ali">hz. ali</a>)
  entry_channels: anket
- entry_name: |
    14 ağustos 2018 %20'lik otomobil zammı
  entry_id:  80261953
  entry_writer: hehe
  entry_fav_count: 46
  entry_content: |
    toyota, renault, honda, fiat, dacia gibi otomobil firmalarının sitelerinden duyurdukları zamdır. düne göre %10 ile %20 arasında zam mevcut, çoğu firma da fiyatları kaldırmış güncelleyeceğiz diye not düşmüşler. <br/><br/>dün 90.000 lira olan araçlar bugün 105.000 civarına yükselmiştir. <a class="b" href="/?q=bir+g%c3%bcnde+15.000+lira+zam">bir günde 15.000 lira zam</a>, harikayız türkiye. <br/><br/><a rel="nofollow noopener" class="url" target="_blank" href="https://goo.gl/Ge6Jvu" title="https://goo.gl/Ge6Jvu">örnek:</a>
  entry_channels: siyaset,haber,otomotiv,ekonomi
- entry_name: |
    sözlükçülerin en iyi 5 roman listesi
  entry_id:  80239432
  entry_writer: damdan dama
  entry_fav_count: 40
  entry_content: |
    türk ve dünya edebiyatı olarak ayırmamın sebebi bence muhteva ve romanların karakteristik özelliklerinin apayrı tatlarda olmalarıdır.<br/><br/>türk edebiyatında aslında 2 yazardan 2’şer eser koymak isterdim. lakin bu biraz tutucu bir tavır sergilememe neden olacağından dolayı her yazara 1 hak tanımak istedim. <br/>huzur (bkz: <a class="b" href="/?q=ahmet+hamdi+tanp%c4%b1nar">ahmet hamdi tanpınar</a>)<br/>anayurt oteli (bkz: <a class="b" href="/?q=yusuf+at%c4%b1lgan">yusuf atılgan</a>)<br/>kuyucaklı yusuf (bkz: <a class="b" href="/?q=sabahattin+ali">sabahattin ali</a>)<br/>yalnızız (bkz: <a class="b" href="/?q=peyami+safa">peyami safa</a>)<br/>kara kitap (bkz: <a class="b" href="/?q=orhan+pamuk">orhan pamuk</a>) (sanırım) <br/><br/>dünya edebiyatında ilk beşimi en iyilerden ziyade tekrar okumak için sabırsızlandığım kitaplardan yazmamın daha makul olacağından dolayı bu yönde bir tercihte bulunmak istiyorum.<br/>ayrıca ilk beşimi rus edebiyatından doldurmak istesem bile 2 ile sınırlı kalmak daha iyi olacak.<br/><br/>karamazov kardeşler (ç. ergin altay)<br/>(bkz: <a class="b" href="/?q=fyodor+mihaylovi%c3%a7+dostoyevski">fyodor mihayloviç dostoyevski</a>)<br/>ölü canlar (ç. ergin altay)<br/>(bkz: <a class="b" href="/?q=nikolay+vasilyevi%c3%a7+gogol">nikolay vasilyeviç gogol</a>)<br/>germinal (ç. nesrin altınova)<br/>(bkz: <a class="b" href="/?q=emile+zola">emile zola</a>)<br/>iki şehrin hikayesi (ç. meram arvas)<br/>(bkz: <a class="b" href="/?q=charles+dickens">charles dickens</a>)<br/>uğultulu tepeler (?)<br/>(bkz: <a class="b" href="/?q=emily+bronte">emily bronte</a>)
  entry_channels: anket,ekşi-sözlük,edebiyat
- entry_name: |
    14 ağustos 2018 dolar kuru
  entry_id:  80242275
  entry_writer: sonsuzmavilik
  entry_fav_count: 30
  entry_content: |
    bu gün belçika tv'si haberlerini izlerken içimi acıtan kurdur.<br/>haberlerde tl'nin aşırı değer kaybettiği ve 10 saniye tayyip'in konuşmasından sonra turist bir arap kadını ile lüks bir marka kuyruğunda yapılan reportajda kadın aynen şunları söyledi;<br/>"çok mutluyum, herşey yarı fiyatına geliyor bize, herşey çok ucuzladı, ne bulursak alıyoruz"<br/>yurt dışında yaşamamdan dolayı lehime olan bir durum gibi görünse de ülkemin bu durumda olması ve kıçı boklu bir arabın buna sevinmesi içimi acıtıyor sözlük .
  entry_channels: haber,ekonomi
- entry_name: |
    trump türkiye'ye çok öfkeli
  entry_id:  80270805
  entry_writer: aksi kanitlanmadikca cocuktur
  entry_fav_count: 23
  entry_content: |
    (bkz: <a class="b" href="/?q=komutan+logar+m%c4%b1+onun+ben+am%c4%b1na+koyay%c4%b1m">komutan logar mı onun ben amına koyayım</a>)
  entry_channels: 
- entry_name: |
    kişide kaçma arzusu uyandıran muhabbetler
  entry_id:  80260278
  entry_writer: koministimam2
  entry_fav_count: 23
  entry_content: |
    ben ben ben ben ben ben ben<br/><br/>her sikime ben ile başlayan her cümlede ben ile başlayıp ben ile biten bir anlatım tarzına sahip, 3 cümlenin dördünde kendini anlatan kadın erkek gördün mü kaçacaksın! <br/><br/>ben<br/>benim anam <br/>benim babam hiç öyle bi insan..<br/>bizimkiler ohooo o konularda <br/>biz hayattaaaa, bizde olsa varyaaa...<br/>ben gelemem öyle şeylere ..<br/>zaten benim tarzım ..<br/>benim bi de şöyle bi ..<br/><br/>ağzınıza da sıçayım, senin yedi ceddine de.... sokucam bi sus be, ben mecbur muyum amk seni ve senin ceddini dinlemeye.<br/>bana ne la bana ne ? bakıyorum 3 cümlede tanıyorum artık. eğer üç cümlede kendi ve kendi florası ile ilgili anlatım sergiliyorsa, “kardeş allah yolunu açık etsin” diyor kaçıyorum hemen
  entry_channels: anket,ilişkiler
- entry_name: |
    insanı tüketen şeyler
  entry_id:  80237767
  entry_writer: ereksiyon hakimiyetini kaybeden adam
  entry_fav_count: 23
  entry_content: |
    insanı tüketen; önündeki tırmanılacak dağlar değil, ayakkabısının içindeki çakıl taşıdır.
  entry_channels: anket,ilişkiler
- entry_name: |
    kötü bira yoktur
  entry_id:  80263906
  entry_writer: hassasterazitutanablagelsin
  entry_fav_count: 22
  entry_content: |
    çok vergi vardır
  entry_channels: yeme-içme
- entry_name: |
    14 ağustos 2018 fenerbahçe benfica maçı
  entry_id:  80252013
  entry_writer: kosmayan adam 23
  entry_fav_count: 19
  entry_content: |
    adama kombineni gişe fiyatından alayım diyorum, 20 tl fazlasını isteyip almazsan tribün boş kalır benim için sıkıntı yok diyor. ee ulan fenerbahçe ne olacak ? desteğe ihtiyacı yok mu ? 20 tl'yi fenerbahçe'den üstün gören tipler kombine aldı. ona yanıyorum. haram zıkkım olsun. <br/><br/>fenerbahçe derhal şu kombine devir konusunda bi sisteme geçmeli yoksa 39 bin kombineden gelmeyenlerle çok boş tribün izleriz bu sene.<br/><br/>edit: @barsky sağolsun, gişe fiyatından maçtayız. umarım turlayacağız.
  entry_channels: spor
- entry_name: |
    volkan demirel
  entry_id:  80269039
  entry_writer: johanericcalvinelmander
  entry_fav_count: 18
  entry_content: |
    galatasaray maçında kahramanlık yaparak yine 1 senelik sözleşmeye imza atacaktır.
  entry_channels: spor
- entry_name: |
    14 ağustos 2018 italya'da köprü çökmesi
  entry_id:  80255997
  entry_writer: ugr
  entry_fav_count: 17
  entry_content: |
    (bkz: <a class="b" href="/?q=italyanlar%c4%b1n+t%c3%bcrklere+benzemesi">italyanların türklere benzemesi</a>)
  entry_channels: haber
- entry_name: |
    mucidinin adıyla anılan buluşlar
  entry_id:  80261264
  entry_writer: cadillacman
  entry_fav_count: 15
  entry_content: |
    (bkz: <a class="b" href="/?q=beh%c3%a7et+hastal%c4%b1%c4%9f%c4%b1">behçet hastalığı</a>)<br/>(bkz: <a class="b" href="/?q=mors+alfabesi">mors alfabesi</a>)<br/>(bkz: <a class="b" href="/?q=dizel+motoru">dizel motoru</a>)<br/>(bkz: <a class="b" href="/?q=zeplin">zeplin</a>)<br/>(bkz: <a class="b" href="/?q=geiger+sayac%c4%b1">geiger sayacı</a>)
  entry_channels: anket,teknoloji
- entry_name: |
    giuliano victor de paula
  entry_id:  80268528
  entry_writer: pukkipukki
  entry_fav_count: 9
  entry_content: |
    burda öven arkadaşlara da cevabını sahada veren futbolcu.<br/><br/>‘aykut hoca mevkisinde oynatmıyor, o brezilyalı bir star’ <br/><br/>diyen tüm kardeşlerimizi göt etmiştir.<br/><br/>bu herif çöp, ilk günden beri söylüyoruz. yeteneği sınırlı, çabası yok, fenerbahçe’nin an itibariyle el freni.<br/><br/>gitsin penaltı golü atıp show yapsın.
  entry_channels: spor
- entry_name: |
    bir çaylak parçasıyla mesajlaşacak kadar düşmek
  entry_id:  80266177
  entry_writer: dexmach
  entry_fav_count: 9
  entry_content: |
    (bkz: <a class="b" href="/?q=i%c5%9fte+o+%c3%a7aylak+par%c3%a7as%c4%b1n%c4%b1n+ad%c4%b1+albert+kanzuk+idi">işte o çaylak parçasının adı albert kanzuk idi</a>)
  entry_channels: anket,ekşi-sözlük
- entry_name: |
    ölümden sonra neler oluyor
  entry_id:  80255249
  entry_writer: italiano21
  entry_fav_count: 7
  entry_content: |
    başımız , mezar taşına çarptıktan sonra [herkes gibi öldüğümüzü] anlıyormuşuz. işte ondan sonra başlıyor <a class="b" href="/?q=fragman">fragman</a>. film demiyorum çünkü asıl film kıyamet koptuktan sonra başlıyor.<br/><br/>ardından <a class="b" href="/?q=m%c3%bcnker+ve+nekir">münker ve nekir</a> geliyor. önce hâl hatır soruyorlar, sonra başlıyorlar <a class="b" href="/?q=ahiret+sorular%c4%b1">ahiret soruları</a> sormaya. tabi çok günah işlediğimiz için daha o an başlıyor bi taraflarımızdan alevli ateşler çıkmaya. henüz daha cennet cehennem yok. onlar kıyamet kopunca. <a class="b" href="/?q=k%c4%b1yamet">kıyamet</a>'e kadar günahı çok olanlar için kabir azabı ve sevabı çok olanlar için kabir eğlencesi [dünyevi bir eğlence değil] var. sonra öyle gidiyor işte. <br/><br/>gerisini de siz araştırın. yazmaktan yoruldum.<br/><br/>edit: ...
  entry_channels: anket,bilim
- entry_name: |
    ekşi itiraf
  entry_id:  80266375
  entry_writer: unutulamayanlar
  entry_fav_count: 4
  entry_content: |
    bugün geri geri hamle yaparken dokundum bir arabaya, yurt dışındayım, dil bilmiyorum ama araba kiralık masraf zerre umrumda değil. sakince arabadan indim hatalıyım kabul ediyorum tutanak tutup halledebiliriz dedim. karşı taraf iner inmez kapa çeneni direk polis arıyorum falan diye bağırdı, ağlamaya başladı. ben de uzatmamak için döndüm, arabaları kenara çektik polisi bekledik. <br/><br/>15 dakika içinde polis geldi, polis gelir gelmez iki tarafa da sordu ne oldu diye, ben hatalıyım tutanak tutalım dedim dinlemeden bağırmaya başladı,kavga etmek istemiyorum ama karşı tarafın tutumu rahatsız edici ben sadece sakince evime gitmek istiyorum dedim. polis daha arabaları görmediğinden ambulansı aradınız mı? yaralı var mı diye sordu arabayı gösterip bilmem iç kanamam olabilir arabaların haline baksanıza dedim. güldük ortam yatıştı neyse polis memuru arabalara baktı evet hatalısınız ama tutanak tutmayı kabul ettiğiniz yardımcı olmaya çalıştığınız için ceza kesmiyorum dedi. kadın bu arada sürekli bağırıyor yandan lehçe sanırım nasıl olur falan demeye çalışıyordu. sonra polis kadına neden polisi aradınız karşı taraf sizinle antlaşmayı kabul ediyormuş zaten diye bir kalay çekti. sonra da polisi boş yere yormaktan ceza kesti. <br/><br/>oh olsun amk sarı çıyanı. o 350 zlt götüne girsin.
  entry_channels: anket,ilişkiler,ekşi-sözlük
- entry_name: |
    karşı cins arkadaşla rüyada dudak dudağa öpüşmek
  entry_id:  80270701
  entry_writer: icimdeki sey gibi bilmem ki ney gibi
  entry_fav_count: 3
  entry_content: |
    id'in piçlik peşinde olduğuna delalettir.
  entry_channels: ilişkiler
- entry_name: |
    kahvaltıdaki favori besin
  entry_id:  80239073
  entry_writer: cenkacar
  entry_fav_count: 3
  entry_content: |
    bir egeli olarak salça.
  entry_channels: anket,yeme-içme
- entry_name: |
    vedat milor eve gelse hazırlanacak menu
  entry_id:  80270308
  entry_writer: bir yalnizlik isareti
  entry_fav_count: 1
  entry_content: |
    ana yemek olarak yuvalama, pirinç pilavı, dolma; tatlı olarak da zerde yapardım. hazır bayram yaklaşıyorken gaziantep mutfağını anmadan olmaz.
  entry_channels: anket,yeme-içme,moda,magazin
- entry_name: |
    sözlük yazarlarının satın aldıkları son kitap
  entry_id:  80267380
  entry_writer: billboard
  entry_fav_count: 0
  entry_content: |
    pdf çıktı mertlik bozuldu. 2016 da gogol'un bir delinin hatira defterini almistim.
  entry_channels: anket,ekşi-sözlük,edebiyat
---
