---
layout: post
title: 05 Ekim 2018 Ekşi Sözlük Debe
data:
- entry_name: |
    şl grubundan 14 puan ve namağlup çıkmak
  entry_id:  81940646
  entry_writer: attilanin yigit komutani altar
  entry_fav_count: 491
  entry_content: |
    adam 6 maç oynadıktan sonra kazanılan ünvana şans eseri diyebilecek kadar kudurmuş(: olum bak sabah oldu, git bir elini yüzünü yıka, kendine gel.
  entry_channels: 
- entry_name: |
    adıyaman'ın medeni olduğu 15 saniye
  entry_id:  81954440
  entry_writer: byzantium
  entry_fav_count: 448
  entry_content: |
    <a rel="nofollow noopener" class="url" target="_blank" href="https://www.youtube.com/watch?v=bwVzDbi_1DQ">https://www.youtube.com/watch?v=bwvzdbi_1dq</a><br/><br/>her şeyi video anlatıyor, bir an için yaya geçidinde bekleyen adama yol vermeye karar veren abimiz ve ona teşekkür eden yayamız ile 15 saniye su gibi akıp geçiyor. aklıma behzat ç.'de üst geçit cinayeti geldi. ulan üst geçitten bir kişi geçiyordu, onu da öldürmüşler gibi. <br/><br/>renault marka araba hem gidip hem kapısını açmasını ise anlamadım.
  entry_channels: haber
- entry_name: |
    eşi ve sevgilisi aynı gün doğuran adam
  entry_id:  81941586
  entry_writer: kasabali efsane
  entry_fav_count: 368
  entry_content: |
    ''o bana erkek çocuk verdi'' diyen dillerini siktiğim haysiyetsiz cahili. sanki 1200'lü yıllarda yaşıyoruz da çocuğun cinsiyetinde kadının bir etkisinin olmadığını bilmiyor yavşak.
  entry_channels: ilişkiler,haber
- entry_name: |
    recep tayyip erdoğan'ın en çok sevdiği şey
  entry_id:  81938380
  entry_writer: nickicem ama
  entry_fav_count: 306
  entry_content: |
    (bkz: <a class="b" href="/?q=para">para</a>) diye biliyorum. sen, ben, ülke falan umrunda değiliz.
  entry_channels: siyaset,anket
- entry_name: |
    tinder'da eşleşme sonrası atılacak ilk mesaj
  entry_id:  81936094
  entry_writer: cifte kavrulmus zenci
  entry_fav_count: 299
  entry_content: |
    (bkz: <a class="b" href="/?q=nabuyun+amucuk">nabuyun amucuk</a>)
  entry_channels: anket,ilişkiler,teknoloji
- entry_name: |
    4 ekim 2018 batman saldırısı
  entry_id:  81942048
  entry_writer: nikimi mikimi sorma
  entry_fav_count: 255
  entry_content: |
    başlarını, saklandıkları lağım çukurlarından ve domuz yuvası mağaralarından çıkarmaya cür'et edemeyen, askerimizin karşısına çıkıp onlarla göğüs göğüse çarpışmayı g*tü yemeyen alçaklar sürüsü ancak böyle tuzaklarla varlığını sürdürmeye çalışıyor. abd'nin mayın eşekleri; kaybedeceksiniz. teker teker geberip cehennem çukurlarının odunu olacaksınız.
  entry_channels: haber
- entry_name: |
    bilinen en uzun sıra bekleme kuyruğu
  entry_id:  81943181
  entry_writer: neolduysa3yasimdaoldu
  entry_fav_count: 147
  entry_content: |
    yer: diyarbakır / k.t.m.<br/>olay: içtima<br/>mevcut: 1400 tezkereci<br/><br/>ad soyad, tek tek, megafonla, bağır çağır, ortalama ıq 30...<br/><br/>4 saat sürdü...<br/><br/>edit:te(s)zkere
  entry_channels: anket
- entry_name: |
    bir anda ortadan kaybolan ünlüler
  entry_id:  81947488
  entry_writer: x uzeri y nin z ye gore turevi
  entry_fav_count: 116
  entry_content: |
    (bkz: <a class="b" href="/?q=david+copperfield">david copperfield</a>)
  entry_channels: anket,magazin
- entry_name: |
    küfürbaz olduğu düşünülen hayvanlar
  entry_id:  81950959
  entry_writer: kasmadan
  entry_fav_count: 111
  entry_content: |
    ana bacı yapan bal porsuğu
  entry_channels: anket,ilişkiler
- entry_name: |
    4 ekim 2018 fenerbahçe spartak trnava maçı
  entry_id:  81935712
  entry_writer: turkiyeden bikan pilot
  entry_fav_count: 99
  entry_content: |
    maçı özet geçiyorum:<br/><br/>-ilk 5 10 dakika fenerbahçe baskıyla başlayacak sonra basit bir top kaybı ile kontradan gol yiyeceğiz.<br/>-ilk yarı bitene kadar trnava kapanacak biz de orta saha stoperler ve bekler arasında top çevireceğiz dikine gidemeden.<br/>-ikinci yarı başlayacak cocu oyunu açmak için bir hücumcu daha alacak oyuna ve 60. dakika gibi 2. golü yiyeceğiz. <br/>-sonra maç bitene kadar protesto hadi bakalım bahisler açılsın.
  entry_channels: spor
- entry_name: |
    erdoğancıydım artık erdoğan'a oy moy yok
  entry_id:  81937403
  entry_writer: hatalicool
  entry_fav_count: 89
  entry_content: |
    seçim günü muhtemel diyalog . oy veren kişi ovk, arkadaşı f olsun . <br/><br/>f: "ee ovk ne yaptın kime verdin oyunu? "<br/>ovk: "valla gardaşım, bastım reise geldim.<br/>f: "ee hani vermeyecektin, kızgındın?"<br/>ovk: "valla gardaş kime verelim başka, hep garıştırıyorlar ülkeyi, hem chp'ye verelim de din iman mı gitsin? en iyisi bu, sabredeceğiz allahın izniyle her şey düzelir."<br/>f: 404 not found.
  entry_channels: siyaset
- entry_name: |
    şeyma subaşı
  entry_id:  81950347
  entry_writer: hasnicktirrr
  entry_fav_count: 88
  entry_content: |
    bir gün öğretmen osman’a sorar,<br/>şöyle bakalım osman büyünce ne olmak istersin?<br/>osman: çok zengin adam olmak isterim. kendime ve karıma ayrı ayrı araba almak isterim. yat ve kat almak isterim. karım altın ve incilerle bozuk para gibi oynasın isterim. <br/>aferin osman otur der öğretmen. sonra neriman’a döner ve sen ne olmak istersin? diye sorar. neriman cevap verir:<br/>osman’ın karısı olmak istiyorum öğretmenim. <br/>hayatta tek gayesi neriman gibi olan kadın.
  entry_channels: moda,magazin
- entry_name: |
    doktor maaşının abartılı olduğu gerçeği
  entry_id:  81937112
  entry_writer: kompozisyon yazamayan adam
  entry_fav_count: 88
  entry_content: |
    selam. 15.000 liraya "insanlığı kurtarıyoruz", "dünyaya barış getiriyoruz", "mesleğimiz kutsal" gibi triplere kesinlikle girmeden hastalara hiç göstermediğim yüzümle şunu anlatayım:<br/><br/>1. hiçbir meslek kutsal değildir. biz de para için çalışıyoruz.<br/>2. bazılarımız imkanı olsa iktidar sahibi olmak ve insanları yönetmek istiyor.<br/>3. 15.000 lira kazanan doktor çok fazla çalışıyordur.<br/>4. yıllarca çalışarak doktor olduğumuz için para karşılığı dayak yiyecek değiliz.<br/>5. hiçbir hastaya kötü davranmıyoruz. sorun çıkaran hastalarla ugrasmak bizim görevimiz değil.<br/>6. hastalar günlerce araştırma yaparak da gelse onlardan çok daha fazla şey biliyoruz ve bu durum cahiller tarafından kibir zannediliyor.<br/>7. mevcut iktidarın getirmediği şiddetli cezalar geldiğinde hiçbiriniz böyle davranamayacaksınız.
  entry_channels: anket,sağlık,ekonomi
- entry_name: |
    onlar yedi mehmetimizi aldı biz 700 öldüreceğiz
  entry_id:  81961939
  entry_writer: funkistan
  entry_fav_count: 81
  entry_content: |
    evlatlarını askere göndermemiş bir ebeveyn için iddialı sözler bunlar hep.
  entry_channels: siyaset,haber
- entry_name: |
    beşiktaş taraftarı
  entry_id:  81937864
  entry_writer: balthazar blake
  entry_fav_count: 80
  entry_content: |
    porto'dan küsküğü yiyen fetocuların ağlamak için toplandığı başlıktır.
  entry_channels: spor
- entry_name: |
    4 ekim 2018 ağaoğlu şirketler grubu rezaleti
  entry_id:  81960141
  entry_writer: hakki alkan
  entry_fav_count: 73
  entry_content: |
    aşağıda olan bitenin kayıt altında kalması için bu notları sizlerle paylaşmak istedim dostlar. <br/><br/>dün (3 ekim 2018), akşam saatlerinde eşimden bir telefon geldi. acilen eve (maslak 1453 sitesi) gelmem gerektiğini, bebeği yıkarken banyo camının patladığını ve yaralandıklarını anlattı. heme ofisten fırladım ve hızlıca eve gittim. <br/><br/>daha önce de tamir edilmesi için not bıraktığımız kapıyı, üstün körü bırakıp gitmişler ve bu hale gelmiş. görseli, sosyal medyada paylaştım <br/><br/><a rel="nofollow noopener" class="url" target="_blank" href="https://twitter.com/hakki_alkan/status/1047522487054094337" title="https://twitter.com/hakki_alkan/status/1047522487054094337">https://twitter.com/…kan/status/1047522487054094337</a><br/><br/>şükürler olsun, cam kırıklarından bebeğe zarar gelmemiş ama eşim hafif yaralanmış. <br/><br/>sabah olduğunda bu rezilliğin hesabını sormak için aradım. bana telefonda satış sonrası ile görüşmeniz lazım, bizim konumuz değil vs dediklerinde, çok sinirlendim ve hakaret ederek (doğru değil, farkındayım) kapadım. <br/><br/>bugün akşama doğru ağaoğlu şirketler grubu kurumsal iletişim ekibinden olduğunu söyleyen bir hanımefendi aradı. neden bu saate kadar beklediklerini, bir geçmiş olsun bile demediklerini, söz konusu olan bir bebeğin hayatı olduğunu belirttim. gelen yanıt "haklısınız, normalde o camın parasını almamız lazımdı ama biz özür dilemiş olmak için camın parasını almayacağız" dedi. <br/><br/>"bunca olan bitenden sonra cam parasından mı bahsediyorsunuz hala. allah sizi kahretsin" dedim ve telefonu kapadım. <br/><br/>yarım saat sonra ağaoğlu şirketler grubu yönetim kurulundan adnan uğur kılıç olduğunu belirten kişi tarafından 0532 644 23 ** numaralı telefonla arandım . kendisinin, "sorumlulara hakaret etmişsin, ben en sorumlu kişiyim" dedi. "yaptıkları hatanın bir bebeğin hayatını riske attığını belirttim" ama orda sözümü kesip küfür etmeye başladı. "allah sizi kahretsin" dedim ve telefonu kapattım. <br/><br/>avukatımızı aradım, ne yapabileceğimizi kısaca görüştük. birkaç önerisi oldu, onları yapacağım. <br/><br/>durumlar bu şekilde dostlar. karşı tarafı düşündüğümde, sanırım bu saatten sonra işler daha da çirkinleşecek. olan bitenler de burada kayıtlı olsa iyi olur diye düşündüm. <br/><br/>zaman ayırdığınız için teşekkür ederim.<br/><br/>edit: camın nasıl kırıldığını merak edenler olmuş. duşakabinin menteşeleri sorunluydu. düzgün tamir edilmediği için menteşeden çıkıyor, önce duvara çarpıyor sonra da parçalanarak banyo yaptırılan bebeğin üstüne düşüyor. <br/><br/>neden tamire site yönetimi bakıyor diyen oldu. ilk 2 sene garantili bu kullandığımız parçalar. bir sıkıntı olduğunda site yönetimi tamir ediyor. ücret gerektiren bir tamir olduğunda da aidata ekleniyor.
  entry_channels: haber,ekonomi
- entry_name: |
    mckinsey imf değil duyun-u umumiye hiç değil
  entry_id:  81959101
  entry_writer: viski raki sarap bira
  entry_fav_count: 68
  entry_content: |
    mhp lideri sayın dr. devlet bahçeli'nin açıklamasıdır.<br/>--- <a class="b" href="/?q=spoiler">spoiler</a> ---<br/><br/>mckinsey ımf değil, duyun-u umumiye hiç değil. sorsanız ne ımf’den anlarlar, ne de duyun-u umumiye’yi bilirler. cehalet varsa ihanet tetiktedir, buna da kafaları basmaz. <a rel="nofollow noopener" class="url" target="_blank" href="https://twitter.com/dbdevletbahceli/status/1047854257381265409" title="https://twitter.com/dbdevletbahceli/status/1047854257381265409">.</a><br/>--- <a class="b" href="/?q=spoiler">spoiler</a> ---<br/>amerikan şirketi mckinsey'e karşı çıkanlar ihanet içindeymiş, türkmen beyi öyle diyor. geçen günde abd'yle savaştayız hükümeti desteklemeyen vatan haini diyordu.
  entry_channels: siyaset,haber,ekonomi
- entry_name: |
    4 ekim 2018 beşiktaşlıların kayışı koparması
  entry_id:  81954065
  entry_writer: skumbag
  entry_fav_count: 67
  entry_content: |
    (bkz: <a class="b" href="/?q=futbol+fanatiklerinin+zek%c3%a2+ya%c5%9f%c4%b1">futbol fanatiklerinin zekâ yaşı</a>)<br/><br/>yazık amk, adam kudurup kompozisyon yazmış.
  entry_channels: spor
- entry_name: |
    3 ekim 2018 porto galatasaray maçı
  entry_id:  81933803
  entry_writer: sikekonankebelek
  entry_fav_count: 66
  entry_content: |
    beraberlik için gidip kazanacak gibi oynayıp kaybetmek de bize nasip oldu tabii aq
  entry_channels: spor
- entry_name: |
    doktor maaşı
  entry_id:  81956425
  entry_writer: wasabie
  entry_fav_count: 65
  entry_content: |
    asistan doktorum.<br/>4.985 tl maaş <br/>965 tl döner aldim<br/>şimdi siktirip gidebilirsiniz
  entry_channels: siyaset,sağlık,ekonomi
- entry_name: |
    airbus a330'un suudi arabistan'daki ilk uçuşu
  entry_id:  81953873
  entry_writer: kesesini kaybeden kanguru
  entry_fav_count: 59
  entry_content: |
    zamanında havaalanlarında "dış hatlarda" çalışınca bu arapların ne kadar pis, iğrenç ve mide bulandırıcı olduğunu görünce şaşırmadığım videodur.<br/><br/>restoran bölümlerini lobici kadınlar temizlerken iğrenirdi, tuvaletler deseniz resmen talan edilmiş gibi oluyordu. <br/><br/>aynısı cevahir de sinema salonlarında da mevcutmuş. çalışan arkadaşım bu araplar girince sinema salonu pislikten ve çöpten geçilmiyor diyordu. bunlar kadar medeniyetsiz ve pis bir millet görmedim diye isyan ediyordu.<br/><br/>not: zaten ülkemiz araplaştıkça iyicene kültür, medeniyet, çağdaşlık, temizlik, iman/din gibi kavramlardan koşarak uzaklaştı.
  entry_channels: anket
- entry_name: |
    netflix ücretsiz 3 aylık üyelik
  entry_id:  81946381
  entry_writer: este noyan
  entry_fav_count: 57
  entry_content: |
    beleş mezar bulsa girecek tipler için velinimet sanırım. sanıyor ki biri eline toplamış isteyen herkese dağıtıyor. içerikten, nasıl olduğundan haberi yok, içeriği ile de ilgilenmiyor. biri başlatmış, bana da düşer belki diye yazıyor. eminönü baklava dağıtımlarını anımsatıyor bana. <br/><br/>biz öğrenciyken durumu olmayan kişi elini kaldırıp istekte bulunmazdı. herkeste bir onur ve tevazu vardı. şimdi "ilk mesaj atana 50 tl vereceğim." desem mesaj kutum dolar taşar.<br/><br/>sorsan ülke bu durumda ise nedeni kömürcüler, makarnacılar vb. ithamlarla tanımlanan insanlar. <br/><br/>onun bir tık üstü de sensin, sadece az biraz daha gelişmiş versiyonusun. hayat mottosu nerede beleş oraya yerleş olan insandan bir cacık olmaz.
  entry_channels: tv,teknoloji
- entry_name: |
    mesleğini söyleyince verilen cevaplar
  entry_id:  81954229
  entry_writer: ahmet222
  entry_fav_count: 51
  entry_content: |
    (biri fabrika 3 iş batırıp) saygın bir işadamı geçmişimden sonra;<br/><br/>-ne iş yapıyorsun?<br/>-pazarda perde satıyorum<br/>-ayy yazık <br/><br/>neden yazık ? ekmeğimi helalinden kazanıyorum. kimseye muhtaç olmamaya çalışıyorum. çalmıyorum gasp etmiyorum. çabalıyorum...<br/><br/>tamam pazarcılık zor fakat kendi ayaklarının üzerinde durabilmenin verdiği hazzı verecek başka bir olgu yok...
  entry_channels: anket,ilişkiler
- entry_name: |
    mesleğini bir cümleyle anlatmak
  entry_id:  81954810
  entry_writer: raskolnikovun cebinde kalan son ruble
  entry_fav_count: 47
  entry_content: |
    - erkekten de .... oluyor muymuş yahu?<br/><br/>edit: hemşire değil. hostes değil. dansöz hiç değil amk :) konsomatris tahmininden sonra buraya yazmaya karar verdim. okul öncesi öğretmeniyim arkadaşlar. biraz daha beklesem orospu musun diye soran çıkacaktı. :)
  entry_channels: anket
- entry_name: |
    üniversite mezunlarının işsiz kalma sebebi
  entry_id:  81948427
  entry_writer: camel soft icen adam
  entry_fav_count: 38
  entry_content: |
    biz üniversite okurken lise mezunları bütün mesleklerin ustası oldu bence.
  entry_channels: anket,eğitim,ekonomi
- entry_name: |
    aşırı abartılmış kitaplar
  entry_id:  81936013
  entry_writer: imp
  entry_fav_count: 35
  entry_content: |
    kürk mantolu madonna’dır. çok güzel bir kitaptır ama sanki türk edebiyatının açık ara en eşsiz eseriymiş gibi lanse edilir. şahsen ben içimizdeki şeytan’ı daha çok beğenmiştim.
  entry_channels: anket,edebiyat
- entry_name: |
    4 ekim 2018 dolar kuru
  entry_id:  81947079
  entry_writer: woolrock
  entry_fav_count: 28
  entry_content: |
    en cok da 5.95'e dusunce millete vatan haini yakistirmasi yapan orospu cocugu ak partili comarlari sevindiren kurdur. serefsizler 7 tl olmasini bekliyorlar ki bozdursunlar. 4 tl'den aldiklari dolari 7 tl'den dolari bozdururlar, 6,90'dan alip para kaybedenleri de vatan hainligiyle suclarlar.<br/><br/>dunya insanlik tarihi bunlar kadar yuzsuz yavsakligi gormedi.
  entry_channels: ekonomi
- entry_name: |
    turkcell bip amerika reklamı
  entry_id:  81952189
  entry_writer: skydreams
  entry_fav_count: 26
  entry_content: |
    bu kadar kötü bir metni hangi liseliye yazdırdıklarını merak ettiğim reklam
  entry_channels: teknoloji
- entry_name: |
    doktorların dayak yemesine çözüm
  entry_id:  81934531
  entry_writer: isvicreli
  entry_fav_count: 22
  entry_content: |
    çok para kazanmalarını sağlamak, bizim milletimiz öylesine aziz bir millettir ki zenginden güçlüden kprktuğu kadar ona saygı duyduğu kadar allahtan korkmaz.
  entry_channels: siyaset,anket,sağlık
- entry_name: |
    starbucks'ta sıra beklerken gelen asalet hissi
  entry_id:  81938715
  entry_writer: 1584
  entry_fav_count: 21
  entry_content: |
    migros'tayken gelen laiklik hissini bilirim de bunu ilk kez duyuyorum nasip olmadı daha.
  entry_channels: ilişkiler,yeme-içme
- entry_name: |
    netflix türkiye
  entry_id:  81953099
  entry_writer: yapici
  entry_fav_count: 20
  entry_content: |
    başlığı çöpe dönüşmüş olan oluşum. gerçi sözlük çöpe dönmüş ben kime ne diyorum. anlamıyorum sizi, insanlar buraya bilgi edinmek, fikir paylaşmak için geliyor, karar vermek için geliyor. herif gelmiş bir yıldır kullanıyorum yazmış, eee abi? ne yapalım yıl dönümünü mu kutlayalım ne yapalım amk.
  entry_channels: tv,sinema,teknoloji
- entry_name: |
    kadınlarımız camilere neden giremiyor
  entry_id:  81956323
  entry_writer: gayim ama para bende
  entry_fav_count: 20
  entry_content: |
    ''bana hocalarım öğretmedi'' diyerek beni benden almıştır.<br/><br/>hangi hocanız!
  entry_channels: siyaset,haber
- entry_name: |
    6 kasım 2007 liverpool beşiktaş maçı
  entry_id:  81959362
  entry_writer: ttravis bicklee
  entry_fav_count: 20
  entry_content: |
    8 yiyen kadrodaki forvetimiz bobo idi. 13 numarayı giyerdi. şimdi alanyaspor'da oynuyor. evde hala 13 numaralı bobo formam durur. sırtıma giyer çıkar dolaşırım birazdan sokakta. <br/><br/>peki sen, sen maklubeci, uefa kupasını kazanan takımının forvetlerinin formasını giyerek sokağa çıkabiliyor musun? <br/><br/>bizim başımız daha dik, emin olun buna!
  entry_channels: spor
- entry_name: |
    4 ekim 2018 malmö ff beşiktaş maçı
  entry_id:  81939152
  entry_writer: ruhlar aleminin gece bekcisi
  entry_fav_count: 20
  entry_content: |
    sinirden kendini siken cimbomluları daha şimdiden pusuya yatıran maç. olm alt tarafı yenildiniz lan fazla kafaya takmayın. biz de yenilebiliriz yani ne olmuş. hayat devam ediyor, yarın bir gün çiçekler de açar bahçelerde. ya da boşverin lan kendinizi sikmeye devam edin. mına koduğumun maklubecileri sizi :))))
  entry_channels: spor
- entry_name: |
    ben oğuz aksaç sorularınızı yanıtlıyorum
  entry_id:  81959364
  entry_writer: oguz aksac
  entry_fav_count: 19
  entry_content: |
    merhaba, ben <a class="b" href="/?q=o%c4%9fuz+aksa%c3%a7">oğuz aksaç</a>.<br/><br/>bugün terletici sorular bekliyorum sizden.<br/><br/>sevgiler.<br/><br/><a rel="nofollow noopener" class="url" target="_blank" href="https://instagram.com/p/Bog9Z5fB7AT/" title="https://instagram.com/p/Bog9Z5fB7AT/">kanıt</a><br/><br/>sorularınıza verilmiş yanıtları görmek için şu bağlantıyı kullanabilirsiniz: (bkz: <a class="b" href="/?q=ben+o%c4%9fuz+aksa%c3%a7+sorular%c4%b1n%c4%b1z%c4%b1+yan%c4%b1tl%c4%b1yorum%2f%40oguz+aksac">ben oğuz aksaç sorularınızı yanıtlıyorum/@oguz aksac</a>)<br/><br/>not: soru cevap etkinliğini mobil ve web tarayıcınızdan takip edebilirsiniz.
  entry_channels: ekşi-sözlük,müzik
- entry_name: |
    korunmasız seks
  entry_id:  81945055
  entry_writer: amkya
  entry_fav_count: 18
  entry_content: |
    (bkz: <a class="b" href="/?q=hiv">hiv</a>) <br/>(bkz: <a class="b" href="/?q=hepatit+b">hepatit b</a>) <br/>(bkz: <a class="b" href="/?q=frengi">frengi</a>) <br/>(bkz: <a class="b" href="/?q=hpv">hpv</a>) <br/><br/>istersen bidaha düşün.
  entry_channels: ilişkiler,sağlık
- entry_name: |
    türk kızının hayattaki üç amacı
  entry_id:  81934860
  entry_writer: burasi kasiyo msn var mi
  entry_fav_count: 16
  entry_content: |
    hayattaki üç amacı bmw almak, sosyal medyadan kız düşürmek ve uçaktan ilk önce inebilmek olan türk erkeği ile kıyaslarsak, bana fazlasıyla anlamlı ve rasyonel gelen amaçlardır. zengin biriyle evlenmek dahil.
  entry_channels: anket,ilişkiler
- entry_name: |
    ateist olmamak için yapılması gerekenler
  entry_id:  81960711
  entry_writer: allah diye kukreyen wookie
  entry_fav_count: 14
  entry_content: |
    "düşünmemelisin" şeklinde yanıt verilmesi gereken soru cümlesi.
  entry_channels: anket
- entry_name: |
    yüksek sesle konuşan insan
  entry_id:  81952359
  entry_writer: finesettimana
  entry_fav_count: 10
  entry_content: |
    (bkz: <a class="b" href="/?q=a%c4%9fz%c4%b1na+k%c3%bcrekle+vurulas%c4%b1+insanlar">ağzına kürekle vurulası insanlar</a>)<br/>(bkz: <a class="b" href="/?q=ne+ba%c4%9f%c4%b1r%c4%b1yosun+lan+ay%c4%b1">ne bağırıyosun lan ayı</a>)<br/>t: asla katlanamadığım insanlar.
  entry_channels: ilişkiler
- entry_name: |
    bir erkeğe verilecek en güzel hediye
  entry_id:  81936496
  entry_writer: contravener
  entry_fav_count: 10
  entry_content: |
    saat değil. <br/><br/>erkeğe saat almak damadın gelinliği kıza sormadan alıp gelmesi gibi bir şey.
  entry_channels: anket,ilişkiler
- entry_name: |
    bebek arabası terörü
  entry_id:  81946728
  entry_writer: elestiri
  entry_fav_count: 10
  entry_content: |
    bebek arabası kullanın kullanmayın demiyorum da içinde bebek olsun bari! bakıyorsun askerlik çağı gelmiş çocuğu bebek arabasıyla gezdiriyorlar.
  entry_channels: ilişkiler
- entry_name: |
    sözlük yazarlarının okumakta olduğu kitaplar
  entry_id:  81934059
  entry_writer: hemingilizceogretmenihemfelsefeci
  entry_fav_count: 7
  entry_content: |
    engin geçtan-insan olmak
  entry_channels: anket,edebiyat
- entry_name: |
    yazarların favori film soundtrack'leri
  entry_id:  81957034
  entry_writer: gelince yaz
  entry_fav_count: 6
  entry_content: |
    kesinlikle <a rel="nofollow noopener" class="url" target="_blank" href="https://youtu.be/UDVtMYqUAyw" title="https://youtu.be/UDVtMYqUAyw">interstellar</a>
  entry_channels: anket,ekşi-sözlük,müzik,sinema
- entry_name: |
    ekşi itiraf
  entry_id:  81933787
  entry_writer: mjorate
  entry_fav_count: 6
  entry_content: |
    herkes diyârında muhabbetinde, bilmem beni ne civâra yazmışlar<a rel="nofollow noopener" class="url" target="_blank" href="https://www.youtube.com/watch?v=vw4brZ_7Rxc" title="https://www.youtube.com/watch?v=vw4brZ_7Rxc">?</a>
  entry_channels: anket,ilişkiler,ekşi-sözlük,sanat,sağlık,eğitim
- entry_name: |
    ciddi ciddi çaylaklara mesaj özelliğini açmak
  entry_id:  81935288
  entry_writer: mucmell
  entry_fav_count: 6
  entry_content: |
    büyük bir risk, ama aldım ben de bu riski. daha yazar olmamışken zeka seviyesi düşük olanları kendini belli ediyor ve hemen engelliyorum. keşke şikayet etme ve puan düşürme gibi şeyler de olsa da sıranın sonuna gönderebilsek bunları ey sözlük yöneticileri duyun bu isteğimi.<br/><br/>edit: yönetim duydu sesimi kanzuk favorilemiş <sup class="ab"><a title="(bkz: swh)" href="/?q=swh" data-query="swh">*</a></sup>
  entry_channels: ekşi-sözlük
- entry_name: |
    parasına kesinlikle değen şeyler
  entry_id:  81955879
  entry_writer: elferon
  entry_fav_count: 4
  entry_content: |
    kesinlikle (bkz: <a class="b" href="/?q=amazon+kindle+paperwhite">amazon kindle paperwhite</a>)
  entry_channels: anket,ekonomi
- entry_name: |
    mancınıkla adam fırlatma
  entry_id:  81962032
  entry_writer: komik nickli yazar
  entry_fav_count: 3
  entry_content: |
    (bkz: <a class="b" href="/?q=ek%c5%9fi+s%c3%b6zl%c3%bck%27%c3%bcn+%c5%9faka+maka+gerizekal%c4%b1+kaynamas%c4%b1">ekşi sözlük'ün şaka maka gerizekalı kaynaması</a>)
  entry_channels: 
- entry_name: |
    evli ve mutsuz olup boşanmayı istememek
  entry_id:  81962604
  entry_writer: would you like some company
  entry_fav_count: 3
  entry_content: |
    <a class="b" href="/?q=cicim+aylar%c4%b1">cicim ayları</a> dışında evli olup da mutlu olanını tanımadığımdan evli çiftlerin büyük çoğunluğunun bu dertten muzdarip olduğunu düşünüyorum.<br/><br/><a class="b" href="/?q=stockholm+sendromu">stockholm sendromu</a> mu desem, <a class="b" href="/?q=%c3%b6%c4%9frenilmi%c5%9f+%c3%a7aresizlik">öğrenilmiş çaresizlik</a> mi desem, ne desem bilemiyorum bu duruma.<br/><br/>ha çoğunluk çocuğu bahane eder ama çocuksuz hallerini de biliyoruz o ayrı mesele.<br/><br/>etrafıma bakıyorum evli olup da mutlu olan bir allahın kulu yok. kadını da aynı erkeği de. iş boşanmaya gelince hemen geri vites. <br/><br/>götleri yemiyor ellaam.
  entry_channels: 
- entry_name: |
    bir şiirin tamamını anlatan tek mısrası
  entry_id:  81933850
  entry_writer: absum
  entry_fav_count: 3
  entry_content: |
    iyi ki şairler böyle bir gaflete düşüp bizi aforizmalarla başbaşa bırakmamışlar. böyle bir dize yoktur. şiiri özetleyebilecek tek bir dize yoktur iyi ki yoktur. <br/><a class="b" href="/?q=sevilen+%c5%9fiirin+en+vurucu+dizesi">sevilen şiirin en vurucu dizesi</a> vardır en fazla. şiiri özetlemeyen böyle bir dize için önerim ise,<br/><a class="b" href="/?q=bir+b%c3%bcy%c3%bck+s%c4%b1r+s%c3%b6yleyece%c4%9fim+sana+zaman+sensin">bir büyük sır söyleyeceğim sana zaman sensin</a> dir benim için, ama bence siz tamamını okuyun : (bkz: <a class="b" href="/?q=b%c3%bcy%c3%bck+s%c4%b1r">büyük sır</a>)
  entry_channels: anket,edebiyat
- entry_name: |
    hayatında hiç suşi yememiş insan
  entry_id:  81955654
  entry_writer: benjamin breeg
  entry_fav_count: 0
  entry_content: |
    2018 dünyasında özentilik falan değildir mk herkes herşeye artık daha rahat erişebiliyor isteyen yer isteyen yemez . <br/><br/>benim diyeceğim ise, ilk kez mezun olduğum sene çalıştığım şirketin iş gezisinde fransa'da japon bi restoranda denedim yedi yıl kadar önce. çok hoşuma gitmişti sonra o zamanki patron ve diğer müşterilerle ne zaman yemek olayı olsa suşi restoranı seçiyorum. yemek yeri ayarlama olaylarını bana paslamislardı ben de kırk yıllık ninja gibi paso suşi restoranları seçiyordum. neyse bigün patron bi de saşimi denesene dedi. ben de denerim tabi dedim. içimden de saşimi ne la mk dedim. sonra geldi bi tabakta. suşi gibi iste çiğ somon tuna falan doldurmuşlar tabağı suşiden tek farkı pilav yok bunda. neyse ben bunu yedim ve hiç hoşuma gitmedi. sonra yine suşi istedim. yani suşi ile saşimi aynı şey ama içinde pilav olmadığı için saşimiyi sevemedim suşiyi sevdim demek içinde pilav olmasa suşiyi de sevmezdim. yani meğerse ben aslında pilav seviyormuşum mk haberim yokmuş. bunu öğrenince bi aydınlanma geldi tabi. istanbuldaki günlerimde yediğim tavuklu pilavlar aklıma geldi para olmadığı zaman sadece pilav alıp üzerine hunarca ketçap sıktığım günler geldi. demek ki tüm olay pilavda imiş. burdaki suşiciler kandırmayın kendinizi siz de mk alayınız tavuk pilavcı biliyoruz.
  entry_channels: anket,yeme-içme
---
