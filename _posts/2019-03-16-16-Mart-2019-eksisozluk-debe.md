---
layout: post
title: 16 Mart 2019 Ekşi Sözlük Debe
data:
- entry_name: |
    camiye saldıran şerefsizin erdoğan düşmanı olması
  entry_id:  87765552
  entry_writer: eksi peace
  entry_fav_count: 584
  entry_content: |
    50 can gitmişken hala siyasi propaganda derdine düşmüş vicdansızları ortaya çıkarmış gereksiz bilgi.<br/><br/>siz ahlaksız siyasal islamcıların derdinin ölen müslümanlar veya cami olmadığını çok iyi biliyoruz.
  entry_channels: siyaset,tv,tarih,haber
- entry_name: |
    100 bin kredi çekerek 60 ayda 40 bin kar etmek
  entry_id:  87757341
  entry_writer: a1465
  entry_fav_count: 524
  entry_content: |
    “çalışmadan, yorulmadan ve üretmeden, rahat yaşamak isteyen toplumlar; evvela haysiyetlerini, sonra hürriyetlerini daha sonra da istiklal ve istikballerini kaybetmeye mahkumdurlar.”<br/>mustafa kemal atatürk
  entry_channels: ekonomi
- entry_name: |
    yeni zelanda'da camiye saldırı
  entry_id:  87752417
  entry_writer: yemek yapmayi seven adam
  entry_fav_count: 472
  entry_content: |
    avusturalya'da 2013 yilindaki federal secimleri liberaller kazandilar. liberal partinin o zamanki lideri tony abbott gayet trump'dan hallice, inanilmaz populist ve bagnaz bir adamdi.<br/><br/>2007'den beri ısci partisi tarafindan yonetilen avusturalya'nin o zamanki parasi mesela amerikan dolari'ndan daha degerliydi. ulke ısci partisi yonetiminde 2008 krizine yakalanmayan tek gelismis ulke olma ozelligini koruyordu. yine burada belirtmeyecegim bircok ozellik, liberallerin ulke yonetimini devralmasiyla bas asagi gitmeye basladi. bu ortamda ne yazik ki tony abbott ve ondan sonra gelen basbakanlar, ve ozellikle de gocmen bakani peter dutton, hukumete geldiklerinden beri toplumu inanilmaz geren, ayrimci, neredeyse nefret sucu sayilabilecek soylemlerine devam ettiler, ve ne yazikki bugun gelinen noktada hersey ortada.<br/><br/>ısin ilginci, halen avusturalya gazeteleri bu olayi "australian gunman" olarak vermekteler, "australian terrorist" diyen gazete hemen hemen yok gibi.<br/><br/>yeni zelanda, muslumanlarin da huzur icinde yasadigi muthis bir ulke. ozellikle banglades kokenli cok yeni zelandali var ve camiilerine de gidiyorlar, ulkenin ekonomisine de muthis bir katki yapiyorlar (software engineers, vs).<br/><br/>bu yasanan saldiri nereden baksan tum bolgenin dengesini alt ust edecek. muslumanlar cok kizgin, anglo-sakson kokenli yeni zelandalilar cok kizgin. yeni zelanda, avusturalya'nin aksine, gercekten cok kaliteli, populist olmayan politikacilara sahip, ozellikle de suanki basbakan hanimefendi. umuyorum bu olay bolgede ki ulkelere, ozellikle de avusturalya'nin siyasilerine ders olur ve bu nefret soylemini artik birakirlar.
  entry_channels: siyaset,haber
- entry_name: |
    a haberin ssg'yi hedef göstermesi
  entry_id:  87761491
  entry_writer: skumbag
  entry_fav_count: 461
  entry_content: |
    ahaber'in nasıl bir bok çukuru olduğunu bir kez daha gösteren olay.<br/><br/>lan facebook'ta biri kufur edince zuckerberg'e mi sallıyorsunuz?<br/><br/>sozlugun maaşlı aktrollü bile anlam verememiş allahım nasil bir şeyle sınanıyoruz...
  entry_channels: siyaset,ekşi-sözlük,haber
- entry_name: |
    iphone x'in 2000tl olması
  entry_id:  87774128
  entry_writer: tomsaywer
  entry_fav_count: 454
  entry_content: |
    çoluk çocuğu yazar yaparsanız aha böyle olur, çok görmemek lazım.
  entry_channels: anket
- entry_name: |
    mukemmeligindayanilmazhafifligi
  entry_id:  87755088
  entry_writer: imamihanife
  entry_fav_count: 375
  entry_content: |
    yeni zelanda'daki cami saldırısı için:<br/><br/>'' videoyu izlerken keşke türkiyeye gelip bu cuma günü camileri temizlese diye iç geçirdim '' diyen vicdan yoksunu müslüman düşmanı kişidir.<br/><br/>soruyorum sana: acaba senin işid zihniyetinden ne farkın var?<br/><br/>edit: bunun ifade özgürlüğü ile alakası yok. suçsuz günahsız insanların canice öldürülmesini istemek ifade özgürlüğü değildir. daha ifade özgürlüğün ne olduğunu bilmeden onu savunmak ya art niyet işi ya da cahil işidir. senin ifaden benim canıma kastediyorsa sence bu ifade özgürlüğü müdür yoksa nefret söylemi mi lütfen önce bunu düşünün.
  entry_channels: ekşi-sözlük
- entry_name: |
    misvak dergisinin tarih böyle yazılır tweeti
  entry_id:  87770259
  entry_writer: vitamineral
  entry_fav_count: 310
  entry_content: |
    terörist ile sidik yarışı yaparak saçmaladıklarını tescillemişler.
  entry_channels: siyaset,haber
- entry_name: |
    kimseye çaktırmadan para basıp piyasaya sürmek
  entry_id:  87764363
  entry_writer: kasmadan
  entry_fav_count: 290
  entry_content: |
    selam, tcmb'dan arıyoruz ama telefonunuz hep meşgul. muhtemelen bu entry sebebiyle bir çok kurum ve kuruluş tarafından aranıyorsunuz. müsait olunca bize dönün lütfen. kaçırmayalım sizin gibi muhteşem bir dehayı.
  entry_channels: ekonomi
- entry_name: |
    mansur yavaş'ın şantaj sms'leri
  entry_id:  87756617
  entry_writer: gayim ama para bende
  entry_fav_count: 268
  entry_content: |
    mehmet özhaseki acaba mesajları fetullah gülen hoca efendisinden mi almıştır.<br/>malum fetöş hala bunları destekliyor.
  entry_channels: siyaset,haber
- entry_name: |
    devletin dağıtacağı prezervatife isim önerileri
  entry_id:  87770394
  entry_writer: angutbeyin
  entry_fav_count: 236
  entry_content: |
    ak-takke
  entry_channels: anket
- entry_name: |
    araç kullanırken gelen uykuyu kaçırma yöntemleri
  entry_id:  87762824
  entry_writer: nickbulamadimnolcak
  entry_fav_count: 189
  entry_content: |
    arabayı müsait ilk yerde kenara çekip dinlenmek dışındaki hiç birinin denenmemesi gereken yöntemlerdir. ne kahvesi, ne iğne batırması gerizekalı mısınız olum? hadi kendiniz kaza yapın geberin gidin neyse de, ya o araçta ya da kazaya karışması muhtemel diğer araçtaki/araçlardaki insanların suçu ne? sen uykun açılsın diye ibinelikler yapasın diye mi ölecek ve/veya yaralanacak diğer insanlar?
  entry_channels: sağlık,otomotiv
- entry_name: |
    emre belözoğlu'nun milli takıma dönmesi
  entry_id:  87765978
  entry_writer: bedenci
  entry_fav_count: 183
  entry_content: |
    genç futbolculardan oluşan bir takımda, ağırlığı olan, herkesçe kabul görmüş, kariyerli bir oyuncu yer alırsa; gençlerin performansları, ciddiyetleri artar. maça çıktıklarında da o sporcu takımın motivasyonunu arttırır. varlığıyla gençlere güven verir ve onları sakinleştirir.<br/><br/>önemli olan doğru oyuncuyu seçmek. bence emre doğru tercih. o isim arda asla değil, en azından ona eminim.<br/>edit: imla ve bozuk cümle.
  entry_channels: spor,haber
- entry_name: |
    erkeklerin %96.5'i aynı anda üç kızı idare ediyor
  entry_id:  87756123
  entry_writer: a sehrinden b sehrine
  entry_fav_count: 156
  entry_content: |
    %3,5 kısımda olarak kendimle gurur duydum
  entry_channels: ilişkiler
- entry_name: |
    sakız çiğniyorum diye dayakla tehdit edilmem
  entry_id:  87769386
  entry_writer: pathaamax
  entry_fav_count: 123
  entry_content: |
    hanzoya diklenmemekle doğru yapan ekşicidir. ağamsın paşamsın deyin geçin. sizin hayatınızdan mühim değil.<br/>bu arada ben de kıl olurum sesli sakız çiğneyene ama yakın tanıdığım değilse kimseye müdahale de etmem. bu adamlar bu özgüveni cehaletlerinden alıyor olsa gerek.
  entry_channels: ilişkiler,seyahat
- entry_name: |
    istanbul'dan tersine göçün başlaması
  entry_id:  87767191
  entry_writer: tunguska11
  entry_fav_count: 115
  entry_content: |
    sen de (bkz: <a class="b" href="/?q=az+g%c3%b6%c3%a7veren">az göçveren</a>) değilsin ankara, izmir.
  entry_channels: ekonomi
- entry_name: |
    güzel kızların çirkin erkeklerle beraber olmaları
  entry_id:  87753733
  entry_writer: ayi yogi diyete girmis
  entry_fav_count: 89
  entry_content: |
    çok yakın arkadaşım gitti, aşırı aşırı çirkin biriyle evlendi.<br/><br/>kız da taş gibiydi, ulan taş ne kelime. üstelik zenginlerdi, tamam çocuğun durumu çok daha iyiydi ama kızın da ailesi fakir değildi. herkesin içinden geçirdiği ama kimsenin soramadığı o soruyu sordum; "ayşe neden, cidden neden?"<br/><br/>"yakışıklı biriyle evlenip üç beş sene sonra ayrılmak mı, çirkin biriyle evlenip ömür boyu gözlerini benden alamaması mı?" dedi. <br/><br/>yakışıklı biriyle evlenip üç beş sene sonra ayrılmak dedim. çirkinler aldatmaz diye bir kaide mi var?
  entry_channels: anket,ilişkiler
- entry_name: |
    hıristiyan terörü
  entry_id:  87759751
  entry_writer: jewfro
  entry_fav_count: 88
  entry_content: |
    bireysel olduğu için tam da başlıkta olduğu gibi hristiyan terörü şeklide vuku bulmuyor. müslüman veyahut islam terörü dediğin zaman oldukça organize ve örgütlü büyük ölçekli gruplar var. misal ne zaman hıristiyanlar da bir <a class="b" href="/?q=el+kaide">el kaide</a> gibi, <a class="b" href="/?q=boko+haram">boko haram</a> gibi ya da <a class="b" href="/?q=%c4%b1%c5%9fid">ışid</a> gibi örgütlü olur işte o zaman tam olarak hıristiyan terörü diyebilirsin.
  entry_channels: siyaset,tarih
- entry_name: |
    slim fit giymeyen erkek
  entry_id:  87769970
  entry_writer: egzntksftk
  entry_fav_count: 81
  entry_content: |
    fit değildir.
  entry_channels: moda
- entry_name: |
    musevilerin türkiye'yi hızla terk etmesi
  entry_id:  87764929
  entry_writer: hidiv kayzeri
  entry_fav_count: 77
  entry_content: |
    sadece museviler değil. <br/><br/>2017'de 6 bin türk milyoner ülkeyi terk etti.<br/><br/>özellikle eğitimli kesim ülkeyi terk ediyor.<br/><br/>amerikan new york times gazetesi, rekor rakamlara ulaşan “türkiye’den yurt dışına göç” konusu ile ilgili haberinde, göçün başlıca nedenleri için “kayırmacılık ve artan otoriterleşmeyi” gösterdi.<br/><br/>“erdoğan’ın vizyonuna hararetle karşı çıkan türkler, yeteneklerini ve varlıklarını da alarak, sürüler halinde ülkeyi terk ediyor” başlığı ile yayımlanan haberde, ülkeden ayrılanların profillerine dikkat çekildi.<br/><br/>“ülkenin kuruluşundan bu yana ilk kez, kültür ve iş hayatına yön veren üst sınıf ailelerin türkiye’yi terk ettiği” de belirtildi.<br/><br/>haberde çok sayıda kişinin de yunanistan, portekiz ve ispanya’dan mülk alma yoluyla göç yolunu kullandığı da kaydedildi.<br/><br/>biliyorsunuz geçtiğimiz aylarda sabancılar bile malta vatandaşlığına geçtiler. <br/><br/>(bkz: <a class="b" href="/?q=%c3%a7ifte+vatanda%c5%9fl%c4%b1k">çifte vatandaşlık</a>)
  entry_channels: siyaset,haber
- entry_name: |
    bilgisayar oyunlarının yasaklanması gerektiği
  entry_id:  87774388
  entry_writer: shinji91
  entry_fav_count: 71
  entry_content: |
    oynama sen de amına koyim
  entry_channels: oyun
- entry_name: |
    türkiye a milli futbol takımı
  entry_id:  87763691
  entry_writer: kadran
  entry_fav_count: 54
  entry_content: |
    herkes bu işi çok iyi biliyor beşiktaş’ta oynamış 3 forveti çağırmışmış. diagne’yi mi çağırsın geri zekalı, yoksa slimani mi? şu anda sakat olmayan ve cenk tosun’la burak yılmaz’dan iyi forvet sayın bakalım. daha iyi değil yakın sayın hatta. eldekilere göre iyi bir kadrodur. kadroya alınmayan, kayıp bir oyuncu yok.
  entry_channels: spor
- entry_name: |
    türk televizyon tarihinin en kaliteli dizisi
  entry_id:  87744797
  entry_writer: hellboy7
  entry_fav_count: 49
  entry_content: |
    kesinlikle 'ezel'. her bölüm ayrı bir heyecandı. hayatımıza bir çok karakter kattı. sadece birini diyorum bilen bilir zaten. eyşan. ah eyşan ah.
  entry_channels: anket,tv
- entry_name: |
    tuttuğun takımı söylemeden anlat
  entry_id:  87745114
  entry_writer: annebenmanyagim
  entry_fav_count: 43
  entry_content: |
    taraftarını kanser eder
  entry_channels: spor,anket
- entry_name: |
    çıplak fotoğraf gönderen kadın
  entry_id:  87771252
  entry_writer: stilic
  entry_fav_count: 42
  entry_content: |
    özelini sizinle paylaşan kadındır. özellikle söz konusu fotoğrafı gönderen kadını tanıyorsanız güzeldir. diğer türlüsü internete "çıplak kadın" yazıp çıkan fotoğraflardan farksızdır.
  entry_channels: anket,ilişkiler
- entry_name: |
    pewdiepie
  entry_id:  87767004
  entry_writer: onlinethemeseus
  entry_fav_count: 41
  entry_content: |
    yeni zelanda'da yaşanan katliam ile alakası olmayan youtuber. <br/><br/>saldırganın, işi gücü meme üretmek ve shitpost atmak olan çeşitli sitelerde aktif olduğu görülüyor. "subscribe to pewdiepie" da internet aleminin güncel memelerinden bir tanesi. ve meselenin bu olayla hiçbir alakası yok. zira söz konusu geyik, youtube'un en çok aboneye sahip iki kanalı arasındaki bir çekişmeden ibaret. diğer kanal t-series isminde bir hint kanalı. <br/><br/>bu katliamı pewdiepie'ın üzerine yıkmaya çalışmak saçmalıktır. kendisinin herhangi bir azmettirici eylemi yoktur.
  entry_channels: oyun,teknoloji
- entry_name: |
    ak parti'yi destekleyen aydınlara satılmış demek
  entry_id:  87777337
  entry_writer: saintly
  entry_fav_count: 32
  entry_content: |
    "ak parti'yi destekleyen aydın" demek.<br/><br/>(bkz: <a class="b" href="/?q=d%c3%bcnyan%c4%b1n+en+k%c4%b1sa+f%c4%b1kras%c4%b1">dünyanın en kısa fıkrası</a>)
  entry_channels: siyaset,ilişkiler
- entry_name: |
    istanbul'un 5'te 4'ü yeşil
  entry_id:  87769299
  entry_writer: katmandulubuda
  entry_fav_count: 31
  entry_content: |
    10 senedir iki günde bir istanbul'u havadan görüyorum, böyle yalan duymadım.
  entry_channels: siyaset
- entry_name: |
    bilal erdoğan için karşılama düzenlemesi
  entry_id:  87774675
  entry_writer: saintly
  entry_fav_count: 30
  entry_content: |
    öncelikle; <a class="b" href="/?q=hahahahahahahahaha">hahahahahahahahaha</a>. <br/><br/>cumhurbaşkanı tayyip erdoğan'ın oğlu olmak dışında hiçbir resmi ve kamusal sıfatı bulunmayan bilal erdoğan'ın adıyaman'da üst düzey kamu görevlileri tarafından karşılanması <a class="b" href="/?q=rezalet">rezalet</a>idir.<br/><br/>vali, rektör, milletvekilleri, belediye meclis üyeleri hazır ola geçmiş durumda reis'in oğlunu bekliyorlar. ilkokul öğrencilerine dönmüşler. ahaha.<br/><br/>(bkz: <a class="b" href="/?q=aile+devleti">aile devleti</a>)<br/><br/><a rel="nofollow noopener" class="url" target="_blank" href="https://twitter.com/afunsal02/status/1106507783787163648?s=19" title="https://twitter.com/afunsal02/status/1106507783787163648?s=19">link</a>
  entry_channels: siyaset,haber
- entry_name: |
    behzat ç.
  entry_id:  87747968
  entry_writer: cigarettesmokingman
  entry_fav_count: 29
  entry_content: |
    yeniden ekranlara dönecek olan efsane dizi. netflix filan deniyor da umarım bunlar harun'la hayaleti filan seviştirmezler. ülkece hazır değiliz böyle bir duruma.<br/><br/>(bkz: <a class="b" href="/?q=netflix+dizilerindeki+e%c5%9fcinselli%c4%9fin+b%c4%b1kt%c4%b1rmas%c4%b1">netflix dizilerindeki eşcinselliğin bıktırması</a>)
  entry_channels: tv,sinema,edebiyat,spoiler
- entry_name: |
    15 mart 2019 fenerbahçe sivasspor maçı
  entry_id:  87777066
  entry_writer: daha cok beklicez mi abi
  entry_fav_count: 29
  entry_content: |
    diabate gole giderken düdüklü ilk yarıyı bitiriyor ve dünya 15 dakikalığına fenerliler için güzelleşiyor.
  entry_channels: spor
- entry_name: |
    içki içen insanların ahirette vereceği hesap
  entry_id:  87772729
  entry_writer: guzellige inancimi sarsan simgeler var
  entry_fav_count: 24
  entry_content: |
    ahiret diye bir yer olmadığı için kafasına takmasına gerek yoktur.
  entry_channels: anket,ilişkiler,yeme-içme
- entry_name: |
    haşmet babaoğlu'nun şaşırtan yeni zelanda önerisi
  entry_id:  87772446
  entry_writer: 126
  entry_fav_count: 22
  entry_content: |
    bu da bunadı iyice.. önceden ne güzel adamdı.. filmlerden, kitaplardan bahseder vatan'da yazardı..
  entry_channels: siyaset,haber
- entry_name: |
    sonsuza kadar devam edecekmiş hissi veren şey
  entry_id:  87745072
  entry_writer: fightclup
  entry_fav_count: 20
  entry_content: |
    akp yönetimi
  entry_channels: anket
- entry_name: |
    devlet bahçeli'nin bir akşamı nasıl geçiyor
  entry_id:  87773407
  entry_writer: huseynick
  entry_fav_count: 19
  entry_content: |
    çay,ferdi tayfur ve temel matematik çalışmaları ile geçmektedir.
  entry_channels: siyaset,anket,magazin
- entry_name: |
    boyundaki morluk için üretilebilecek bahaneler
  entry_id:  87774051
  entry_writer: nedovic
  entry_fav_count: 18
  entry_content: |
    yılan soktu batuhan zehri emdi
  entry_channels: anket,ilişkiler,sağlık
- entry_name: |
    suriyelilere 40 milyar dolar harcanması
  entry_id:  87777846
  entry_writer: 3310merkez
  entry_fav_count: 16
  entry_content: |
    suriye'de iç savaş başladığından bu yana 8 koca yıl geçti.<br/>esad ülkesindeki savaşı kazandı..<br/>bunu düşmanları da söylüyor.<br/>idlib dışında bir problemi yok<br/>orayı da rusya ile birlikte türkiye'nin onayıyla vuruyor <br/>nisan mayıs gibi idlib'i alması kesin <br/>idlib'de ne kadar insanın yaşadığı net değil<br/>işin kötüsü hangisinin cihatçı hangisinin oranın yerlisi olduğu da bilinmiyor <br/>4 milyon suriyeliye bakıyoruz..<br/>büyük ihtimal idlib'den kaçıp gelenlerin adresi de burası olacak <br/>gelenlerin çoğunun hiçbir vasfı, mesleği yok..<br/>tek ortak noktaları sünni olmaları <br/>ve emin olun geri dönmeyecekler..<br/>üstüne üstlük çoğalıyorlar..<br/>doğum hızları bizden kat be kat yüksek <br/>bunu siz de caddede sokakta kolayca gözlemleyebilirsiniz<br/>milli savunma bakanı hulusi akar suriyelilere toplam harcanan paranın miktarını açıkladı <br/>tamı tamına 40 milyar dolar <br/>40 milyar dolar 220 milyar türk lirası ediyor <br/>220 milyar tl'yi nüfus olan 82 milyona bölelim<br/>kişi başı 2 bin 700 lira gibi bir para çıkıyor <br/>yani her birimizin cebinden 2 bin 700 lira alınarak suriyelilere verildi<br/>ne uğruna..<br/>kim uğruna <br/>harcanan paranın neredeyse tamamı bizim vergilerimizden toplanan para..<br/>avrupa'dan ve abd'den bir miktar para gelse de toplam harcanan paranın yanında devede kulak kalır..<br/>ak partili arkadaşım, kardeşim<br/>o parayla kaç köprü, kaç havalimanı yapılırdı<br/>belki de geçmediğimiz köprüye ödeme yapmak zorunda kalmazdık..<br/>eyt'liler her gün sokakta<br/>reis-i cumhur, kaynak yok diye her defasında kestirip atıyor <br/>ülkesinin insanına bulunamayan o kaynak suriyelilere nasıl ve neden hemen bulunuyor? <br/><br/>--- <a class="b" href="/?q=spoiler">spoiler</a> ---<br/>milli savunma bakanı hulusi akar: 40 milyar dolar civarında bir parayı suriyeli kardeşlerimiz için harcamış bulunuyoruz. <br/>--- <a class="b" href="/?q=spoiler">spoiler</a> ---
  entry_channels: siyaset,ekonomi
- entry_name: |
    geceye bir 90'lar şarkısı bırak
  entry_id:  87744594
  entry_writer: isveperver
  entry_fav_count: 16
  entry_content: |
    nilüfer örer - mevsim bahar
  entry_channels: anket,müzik
- entry_name: |
    ekşi itiraf
  entry_id:  87744622
  entry_writer: deep love
  entry_fav_count: 12
  entry_content: |
    düşünüyorum da zaman gerçekten insanda çok şeyi değiştiriyor. <br/><br/>benim gibi analitik düşünen bir insan için bunu kabullenmek zor oldu biraz. ama insan gerçekten yaşadıklarıyla şekilleniyor. beynimiz yaşadıklarımızla şekillenip “rasyonel” olduğunu iddia ettiği seçenekleri tercih ediyor. <br/><br/>kendimi tükenmiş hissediyorum. artık hiçbir şey heyecanlandırmıyor. insanlarla muhabbet etmeyi bile gereksiz buluyorum. nefes alıp ağzımdan kelimeler çıkarmak, bir şeyler anlatmak o kadar zor geliyor ki artık. eskiden böyle insanları hiç anlamazdım. şimdi çok iyi anlıyorum sözlük. beynim artık bu düşünce yapısını çok daha gerçekçi hissettiriyor bana. <br/><br/>hiçbir şeyi yapmaya bir enerjim kalmadı. bunları yazmak bile zor geliyor. ifade edemiyorum da zaten. <br/><br/>evet ifade edemiyorum. aslında anlatmak istediğim şey yazdıklarımın hiçbiri değil. anlatamıyorum. yoruluyorum.
  entry_channels: anket,ilişkiler,ekşi-sözlük
- entry_name: |
    kamyon'la nusret'e girememe rezaleti
  entry_id:  87777951
  entry_writer: syncho
  entry_fav_count: 11
  entry_content: |
    (bkz: <a class="b" href="/?q=millet+ne+sigaralar+i%c3%a7iyor">millet ne sigaralar içiyor</a>)<br/><br/>edit: rezalet olabilecekken olamamış olay.
  entry_channels: 
- entry_name: |
    e-devletten kredi puanı sorgulama
  entry_id:  87771744
  entry_writer: m orak
  entry_fav_count: 8
  entry_content: |
    acilen tramer sorgusunada gelmesi gerek, biz arabanın tramerini öğrenmek için neden para ödüyoruz mantıklı bir durum varmı?
  entry_channels: teknoloji,ekonomi
- entry_name: |
    ayasofyanın hemen yarın ibadete açılması
  entry_id:  87775399
  entry_writer: paranoyak robot
  entry_fav_count: 8
  entry_content: |
    teröristle sidik yarıştıran kaale alan seviyesine düşenlere yakışır. açcaksan aç da, teröristi sikliyorsan onun kadar aklın vardır.
  entry_channels: 
- entry_name: |
    türkiye'nin en büyük spor kulübü
  entry_id:  87774588
  entry_writer: sacmalayanadaminklavyesii
  entry_fav_count: 4
  entry_content: |
    galatasaray mı fenerbahçe mi diye yıllardır tartışılan bir konu.<br/><br/>çünkü türkiye'de 2 büyük vardır.<br/><br/>kimse de çıkıp ciddi ciddi trabzon, bjk, bursaspor en büyüktür demez.
  entry_channels: spor,anket
- entry_name: |
    sadık çiftpınar
  entry_id:  87775295
  entry_writer: sokolsuz pasha
  entry_fav_count: 4
  entry_content: |
    maç esnasında kendini tarlası yanan köylü gibi yerden yere çarpması hakikaten çok rahatsız edici olan futbolcu. sanırım pofuduk saçlarını da aldığı darbelerin şiddetini daha yüksek göstermek için kesmiyor. benim bildiğim fenerliler rakip topçuları biçen çirkef ve sert stoperleri sever. sadık'ın bu huyu yakın gelecekte fenerlileri de rahatsız edecektir.
  entry_channels: spor
- entry_name: |
    kek yapacaklara tavsiyeler
  entry_id:  87749510
  entry_writer: moon grass
  entry_fav_count: 4
  entry_content: |
    kek yaparken tariflerde gördüğümüz şekerleri kendi damak tadımıza uydurmak için azaltmak ya da arttırmak istiyoruz ya, işte onu +- %20'den fazla yapmamak iyidir. şeker sadece ne kadar tatlı olacağına etki eden bir malzeme değil, aynı zamanda keke nem veren bir malzemedir ve sonra -fazla azaltma durumunda- sası/kuru bir kek elde ihtimalimiz yüksek olur. şekeri arttırırsak ne olur? arttırmayın, zararlarından bahsetmeye gerek yok. -tadından değil yapısından bahsedecek olursak- daha yapış yapış bir kek olur; hani elinize aldığınızda dışı elinize yapışıyor gibi hissettirir ya, işte öyle olur.<br/><br/>afiyet olsun.
  entry_channels: anket,yeme-içme
- entry_name: |
    brenton tarrant manifestosu
  entry_id:  87776112
  entry_writer: borellus666
  entry_fav_count: 4
  entry_content: |
    4chan'in fazla etkisinde kalmış gerizekalı bir deus vult'çu zırvası. bu malların ulaşabileceği en üst seviye bir silah ile sivil insanları vurmaktır. bu 4chan'in avrupa yarrağı müritlerinin yapabileceği en büyük şey budur.
  entry_channels: siyaset,haber
- entry_name: |
    sahip olunan ilk forma
  entry_id:  87749815
  entry_writer: geliyorlar
  entry_fav_count: 3
  entry_content: |
    pazardan alınmış, inzaghi forması,milan. dikey çubuklu kırmızı-siyah.
  entry_channels: spor,anket,tarih
- entry_name: |
    geceye bir galatasaray futbolcusu bırak
  entry_id:  87770067
  entry_writer: shimmyshimmyshimmy
  entry_fav_count: 3
  entry_content: |
    henüz gece olmadığı için bırakmayacağım futbolcudur.<br/><br/>gece olsun, o zaman bırakırım.
  entry_channels: spor,anket
- entry_name: |
    kapı koluna dokununca gelen kirlenmişlik hissi
  entry_id:  87775081
  entry_writer: dort yanlis bir dogruyu goturur
  entry_fav_count: 2
  entry_content: |
    kapı koluna dokunulmasından ziyade, ıslak elle dokunuyorlarya işte o en kötüsü bence.
  entry_channels: sağlık
- entry_name: |
    ekşi sözlük dertleşecek fuckbuddy ilanları
  entry_id:  87770388
  entry_writer: internettinamca
  entry_fav_count: 1
  entry_content: |
    içimdeki dertleri “boşaltabilmemi “ sağlayacak ilanlar. çok dertliyim sözlük :(
  entry_channels: ilişkiler,ekşi-sözlük
- entry_name: |
    benata
  entry_id:  87770641
  entry_writer: yuzuk kardesligi
  entry_fav_count: 0
  entry_content: |
    bugün ispanya'ya göç eden yahudi olarak tanıdım, hoşuma gitti uygur türkü asıllı kürt olması, çorumdaki müezzinliği dönemi çok keyifli geçmiş ama...
  entry_channels: ekşi-sözlük
---
