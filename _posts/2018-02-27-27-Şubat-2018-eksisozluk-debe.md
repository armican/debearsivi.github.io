---
layout: post
title: 27 Şubat 2018 Ekşi Sözlük Debe
data:
- entry_name: |
    reddedildikten sonra whatsapp'tan engelleyen erkek
  entry_id:  74528201
  entry_writer: asimpleman
  entry_fav_count: 738
  entry_content: |
    yaptığı şey gayet net ve modern olan biridir kanımca.<br/><br/>şöyle bir ifade dile getirebilirim. bir erkek geldi, size karşı olan hislerini, düşünce ve duygularını dile getirdi ve ortak bir payda bulamadınız, reddettiniz veya olamayacağını dile getirdiniz. bu adam da sizi malum platformlardan engelledi. sonrasında ise hiç bir şey olmamış gibi hayatına devam etti.<br/><br/>bu; görüp görebileceğiniz en net, en temiz ifadedir. en modern yaklaşımdır.<br/><br/>çünkü adam; ne kendisini ezdirmiş, ne egolarınızı tatmin etmenize fırsat vermiş ne de durumu rahatsız edici bir boyuta getirmiştir.<br/><br/>hiç bir şey olmamış gibi hayatına devam etmiştir. sarhoşken de aynı durum geçerlidir. iletişim kurmak istememiştir, keza sizin de günün birinde kurmak istemenizi istememiştir.<br/><br/>tavsiyem ise bu erkeklerin neden böyle yaptıklarını düşünmek, buna kafa patlatmak yerine sizin de bir şekilde hayatınıza devam etmeniz olacaktır.
- entry_name: |
    uber'i türkiye'den kovacağız
  entry_id:  74532572
  entry_writer: kazaryasa
  entry_fav_count: 647
  entry_content: |
    acaba kaç tane plakası var?, sorusunu akla getiren beyan.
- entry_name: |
    bedelli askerlik yapmış biriyle sevgili olan kadın
  entry_id:  74526124
  entry_writer: olsunnapalim
  entry_fav_count: 497
  entry_content: |
    "askerliğini dağda bayırda sırtta 100 kiloluk çanta, elde g3 ile yapmış, savaşmış, kan görmüş" ibaresi ile beni benden almıştır.<br/><br/>işte bu çok televizyon izlemenin zararları.
- entry_name: |
    erkeklerin evlenmek istememe nedenleri
  entry_id:  74519787
  entry_writer: kulube i ahzan
  entry_fav_count: 496
  entry_content: |
    (bkz: <a class="b" href="/?q=kaynana+ile+perde+se%c3%a7mek">kaynana ile perde seçmek</a>)<br/>(bkz: <a class="b" href="/?q=h%c3%b6d%c3%bck+kay%c4%b1nbaba">hödük kayınbaba</a>)<br/>(bkz: <a class="b" href="/?q=abart%c4%b1l%c4%b1+d%c3%bc%c4%9f%c3%bcn+masraflar%c4%b1">abartılı düğün masrafları</a>)<br/>(bkz: <a class="b" href="/?q=d%c3%bc%c4%9f%c3%bcn%c3%bcn+gereksiz+bir%c5%9fey+olu%c5%9fu">düğünün gereksiz birşey oluşu</a>)<br/>(bkz: <a class="b" href="/?q=k%c4%b1z+isteme+adetleri">kız isteme adetleri</a>)<br/>(bkz: <a class="b" href="/?q=gereksiz+akrabal%c4%b1k+ili%c5%9fkileri">gereksiz akrabalık ilişkileri</a>)<br/>(bkz: <a class="b" href="/?q=ev+d%c3%b6%c5%9femek">ev döşemek</a>)<br/>(bkz: <a class="b" href="/?q=ta%c5%9f%c4%b1nma+i%c5%9fkencesi">taşınma işkencesi</a>)<br/>(bkz: <a class="b" href="/?q=kar%c4%b1+koca+yap%c4%b1%c5%9f%c4%b1k+ikiz+gibi+ya%c5%9famak">karı koca yapışık ikiz gibi yaşamak</a>)<br/>(bkz: <a class="b" href="/?q=bo%c5%9fanman%c4%b1n+zorluklar%c4%b1">boşanmanın zorlukları</a>)<br/>(bkz: <a class="b" href="/?q=nafaka">nafaka</a>)
- entry_name: |
    kadını aşağılayan kelimelerin tdk'dan çıkarılması
  entry_id:  74518565
  entry_writer: medeniyet
  entry_fav_count: 467
  entry_content: |
    yanlış karardır. <br/><br/>tdk, bu kelimeleri subjektif bir değerlendirmeyle tanımlamıyor. o kelime toplumda nasıl kullanılıyorsa o şekilde tanımlıyor. <br/><br/>oynak, aşufte, yollu gibi kelimeler aşağılayıcı kelimeler olabilir ama nihayetinde bu kelimeler günlük konuşmada kullanılıyor mu, kullanılmıyor mu? <br/><br/>bizim feministler istiyor ki “kaşar” kelimesini tdk’dan aratınca sadece “bir peynir türü” tanımı çıksın. ancak malumunuz, bu kelime sadece peyniri ifade etmiyor ve günlük konuşmada kullanılıyor. evet hoş değil ama kullanılıyor işte. tdk, bilimsel bir çerçevede bu kullanımları aktarıyor. kaşarın peynir dışında argodaki karşılığının da yazılması tdk’nın kadın düşmanı olduğunu veya bu kelimenin argodaki gibi algılanmasını arzuladığı anlamına gelmez. bu ayrıma varmak lazım. burada eleştirilmesi gereken tdk değil, toplumun çarpık cinsiyet anlayışıdır. <br/><br/>bu kelimelerin veya am göt meme’nin cinsiyetçi ifadelere aracılık etmesi onları literatürden sürgün etmeyi gerektirmiyor. aksine bu kelimelerin derinine inip toplumsal bilinçaltındaki çarpıklıkları ifşa etmek gerekir. ama bizim cihangir erbabı havadan nem kaptığı için böyle abuk subuk sonuçlar ortaya çıkıyor. devlet de sözde toplumsal baskıyı görmüş de geri adım atmış gibi bir görüntü ortaya çıkıyor. feministler de mutlu oluyor.
- entry_name: |
    cinsel ilişki teklifini reddeden erkek
  entry_id:  74533848
  entry_writer: dostmahallenizinkocaoglani
  entry_fav_count: 427
  entry_content: |
    bunu garip bulmak anlamsız, her erkeği yüzük arayan gollum gibi delik arıyor zannetmeyin
- entry_name: |
    30 yaşında 50 ülke gezmiş 1000 kitap okumuş olmak
  entry_id:  74517340
  entry_writer: huaaa allah ne kadar super lan
  entry_fav_count: 417
  entry_content: |
    1 ülke ve 5 kitapla katıldıgım elit gruptur.<br/><br/>+ amk beni yine tanıdılar siz kaçın beyler.
- entry_name: |
    recep tayyip erdoğan'a mareşal unvanı verilmesi
  entry_id:  74520608
  entry_writer: victory90
  entry_fav_count: 378
  entry_content: |
    yetmez. "gazi mareşal başkomutan solotürk türk yıldızları ak gandalf ak saruman 15 temmuz şehitleri isildur'un varisi recep tayyip erdoğan" olmalı kendisi. reisimize bu yakışır.
- entry_name: |
    kadıköy'de minibüsçü terörü
  entry_id:  74523118
  entry_writer: syhngr
  entry_fav_count: 374
  entry_content: |
    <a rel="nofollow noopener" class="url" target="_blank" href="http://www.hurriyet.com.tr/gundem/kadikoyde-minibuscu-dehseti-bir-kadina-bu-yapilir-mi-40753970" title="http://www.hurriyet.com.tr/gundem/kadikoyde-minibuscu-dehseti-bir-kadina-bu-yapilir-mi-40753970">http://www.hurriyet.com.tr/…-bu-yapilir-mi-40753970</a><br/><br/>kadıköy’de yolcu almak için aniden duran minibüse çarpmamak için ani fren yapan motosikletli çift seren okur ve samet yüksel, dört minibüs şoförü tarafından feci şekilde dövüldü. şoförlerden şikâyetçi olan seren okur, “dört erkek beni yerde tekmeledi. acıdan bayılmışım. motosikletimi yere devirdiler. bir kadına bu yapılır mı?” dedi.<br/><br/>başımdan daha yeni geçen bir olayı aktarayım:<br/>yaklaşık bir ay önce işyerimden çıktım ve hemen yakındaki kavşağa geldim. ama kavşak kilit adım atmıyor. arkamda da hatlı bir minibüs. kavşağa girersem daha fazla tıkanacaktı ve kavşak açılsın diye 3-4 arabanın geçmesine izin verdim. bir anda minibüs şoförü kornaya basmaya başladı hiç durmadan. neyse kavşağa girdim bu da yanıma geldi ve pencereyi açtı (hla trafik yürümüyor)25-26 yaşında genç, bende açtım camı, hayırdır birader dedim trafik felç nereye gideyim? başladı ana avrat küfür etmeye. arabadan indim dedim ne küfür ediyorsun bu da indi küfür etmeye devam ederek arabamın sağ tarafına yumruk attı. bir baktım kaporta yamulmuş. lan dedim sen benim malıma nasıl verirsin bende bastım küfürü. baktım üstüme geliyor geri adım yok ağzının ortasına geçirdim yumruğu. sonra beklemediğim birşey oldu bu minibüse koştu. sopa çıkardı üstüme saldırdı tekrar. ben birşey alacağını tahmin ettiğim için peşinden gitmiştim kafama vurmaya çalışırken sopayla elini tuttum yere yuvarlandık. bir şekilde sopayı aldım elinden araya girdiler. beni tutarlarken bu atladı minibüse kaçtı... ama eminim yakında başka minibüsçü olsaydı olayı bilmeden onlar da saldırırlardı bana... <br/>minibüsçü terörü heryerde. kafasına göre durur, yolcu alır, yolcu indirir kavga eder. nasıl bunun önüne geçilebilir hiç bir fikrim yok...
- entry_name: |
    26 şubat 2018 erdoğan'ın doğum günü fotografı
  entry_id:  74528467
  entry_writer: kendinibilmeyenbilgenintakendisi
  entry_fav_count: 196
  entry_content: |
    ben şeyi merak ediyorum ya sanatçılar yan odadı mı yaşıyor napıyor lan?<br/><br/>iftar olur aynı ekip damlar, doğum gününde aynı ekip anında toplaniyor. <br/>ne bileyim sarayda böyle <a class="b" href="/?q=sanat%c3%a7%c4%b1lar">sanatçılar</a> adı altında bir salon var bunlar orada mı yaşıyor? <br/>acil durumlarda sanatcilar hazırlansin diyorlar, bunlar da otuziki kısım tekmili birden sıralamayı bile bozmadan poza mı duruyor dsfasdfsdf.<br/><br/>yavuz bingol, orhan gencebay falan isi bitince kravatini gevsettigi takim elbisesi ile yan odada bir sonraki etkinligi mi bekliyor?
- entry_name: |
    izmir'in bittiği gerçeği
  entry_id:  74528886
  entry_writer: hidrolikrotrecan
  entry_fav_count: 173
  entry_content: |
    artık bütün türkiye bitti dersek daha doğru olur kanımca.
- entry_name: |
    25 şubat 2018 beşiktaş fenerbahçe maçı
  entry_id:  74509743
  entry_writer: soner ilkay
  entry_fav_count: 170
  entry_content: |
    objektif olup, fener'in futbol doğrularını da konuşmalıyız. bugün 2 noktada fener'i maça iyi hazırlanmış gördüm.<br/><br/>1) çok üst düzey, profesyonel seviyede zaman geçirdiler, kameni sarı almadan zaman geçirmede iyiydi.<br/>2) fernandao'nun gol sevinci iyi çalışılmış, etkileyici bir organizasyondu. sanırım fener'in maçtaki tek organizasyonu da buydu.
- entry_name: |
    öğrencilere ücretsiz ulaşım için kanun teklifi
  entry_id:  74526769
  entry_writer: hayhaymatlos
  entry_fav_count: 161
  entry_content: |
    almanya'da her eyaletin kendi öğrencisine verdiği haklar vardır. bunlar biri de üniversite öğrencilerine her türlü ulaşımın ücretsiz olması. keza (ahanda bomba geliyor) öğrencinin yanında bir kişiye daha ücretsiz olması. bu ne demek sen öğrencisin sana zaten tren, tramvay, otobüs falan ücretsiz, diyelim ki annen de seni ziyarete geldi o haftasonu ve beraber gezmek istediniz şehri annene de ücretsiz tüm ulaşım. al sana sosyal devlet al sana muasır medeniyet seviyesinde bir devlet al sana devlet gibi devlet...<br/><br/>edit: imla
- entry_name: |
    çarşı'nın aykut kocaman'a köpek demesi
  entry_id:  74509821
  entry_writer: neuroeconomist
  entry_fav_count: 120
  entry_content: |
    golden sonra fenerbahçeli futbolcuları sakin olun diye uyaran, onlardan tribünleri tahrik etmemelerini isteyen hoca için yazılmıştır.
- entry_name: |
    scottish fold
  entry_id:  74516460
  entry_writer: kasvetlikutlama
  entry_fav_count: 116
  entry_content: |
    alacağım, satacağım vs. havalarda uçuşan, haysiyet yoksunlarının elinde peluş oyuncak olmuş zavallı kedi cinsi.
- entry_name: |
    atatürk'ün orgeneral olmadan mareşal olması
  entry_id:  74534614
  entry_writer: 1971 2007
  entry_fav_count: 113
  entry_content: |
    millet diploma olmadan cumhurbaşkanı oluyor aktrollün derdine bak.
- entry_name: |
    uyandıktan sonra kurulan ilk cümle
  entry_id:  74516964
  entry_writer: 31feetunder
  entry_fav_count: 100
  entry_content: |
    <a class="b" href="/?q=efendim">efendim</a>? <a class="b" href="/?q=yapar%c4%b1m">yaparım</a>! <a class="b" href="/?q=oduncu">oduncu</a>.<br/><br/>alarma tepki<br/>o sırada bizden beklenen<br/>meslek<br/><br/>(bkz: <a class="b" href="/?q=age+of+empires+k%c3%b6yl%c3%bcs%c3%bc">age of empires köylüsü</a>)
- entry_name: |
    ananın ve kız kardeşinin kolu bacağı tahrik eder
  entry_id:  74525548
  entry_writer: garipbasci
  entry_fav_count: 93
  entry_content: |
    kolun bacağı tahrik etmesi gibi bir anlam çıkıyor ki o bile kastedilen anlamdan daha mantıklı.
- entry_name: |
    beşiktaş
  entry_id:  74509721
  entry_writer: castor
  entry_fav_count: 84
  entry_content: |
    safları sıklaştırın.<br/><br/>bu akşam eze eze kazandığımız derbi sonrası tüm spor programlarında beşiktaş’ın üçüncü golü öncesi faul var mı, quaresma atılmalı mıydı, penaltı olmayabilir goy goyu yapıldı. ofsayttan yediğimiz gol, verilmeyen penaltımız kadar konuşulmadı; hatta aspor'da merdiven boşlukları niye dolu diye soruldu, forza beşiktaş hesabının paylaşımları konuşuldu. <br/>hakkımız olan penaltıya bile acaba diyecek kadar ipleri başkasının elinde olan basın ile bu takımın işi geçen iki sene olduğundan çok daha zor. sahada hakem, dışarıda basın, masa başında federasyon ile mücadele etmek zorundayız. 2004'te başardılar ama bu kez beşiktaş’ onlara yedirmeyeceğimiz. önce hocamızı, sonra oyuncularımızı önümüze atacaklar, oyunlarına gelmeyeceğiz, sahip çıkacağız.<br/>dağılmayın, safları sıklaştırın, tüm hesapları sezon sonuna bırakın. alacak bir şampiyonluk var.<br/><br/>tanrı beşiktaş'ımızı korusun ve yüceltsin!
- entry_name: |
    içinde tren geçen filmler
  entry_id:  74524075
  entry_writer: hukukcuyum gomutan
  entry_fav_count: 82
  entry_content: |
    bir kişi de <a class="b" href="/?q=snowpiercer">snowpiercer</a> yazmamış. içinde tren geçen daha ala bir film mi var?<br/>filmin içinde tren geçmiyor. trenin içinde film geçiyor. <sup class="ab"><a title="(bkz: swh)" href="/?q=swh" data-query="swh">*</a></sup>
- entry_name: |
    amazon.com
  entry_id:  74534444
  entry_writer: exerbo
  entry_fav_count: 77
  entry_content: |
    yıllardır paraya para katmak haricinde bir tane bile müşteriyi nasıl memnun ederim icraati olmayan çakma yerli muadillerinin kökünü kurutması dileğiyle. bizimkiler refurbished ürünü sıfır diye çakarken bu arkadaşlar hayvan gibi arge yapıp insanlığa yeni teknolojiler kazandırmaya kasıyorlardı. bir de kendi kargo firmasını kurarak geliyormuş. ayvayı yediniz oğlum. beter olun.
- entry_name: |
    kadınların erkeklerde aradıkları şeyler
  entry_id:  74521150
  entry_writer: bezdarniy
  entry_fav_count: 74
  entry_content: |
    afedersiniz ama <a class="b" href="/?q=yarrak">yarrak</a> arıyorlar. yukarda o kadar şey sayılmış ve çoğu özellikler bende var ama hatun beni değil de gidip tek özelliği yarrağı olan birini seçiyor. sonra gelip bana dert yanıyor. <br/>o yüzden kadınların ne istediğine çok takılmayın gençler. siz kadından ne bekliyorsunuz ona karar verin ve hakkettiği gibi davranın. ne fazla ne eksik.
- entry_name: |
    kapüşonunun kenarı kürkle çevrilmiş mont
  entry_id:  74532373
  entry_writer: beeyore
  entry_fav_count: 73
  entry_content: |
    en sevdiğim mont tipi. <br/><br/>benim yok. hafta içi kaşmir, hafta sonu polyester spor montlar giymeyi sevdiğim için almadım ama millette görünce çok beğeniyorum. kadına da erkeğe de yakışıyor. <br/><br/>gayet güzel bence. subjektif yorumlarınızı sanki doğa kanunu gibi yazmayın lan. salak görünüyor.
- entry_name: |
    tinder
  entry_id:  74516508
  entry_writer: beyazmantoluadam
  entry_fav_count: 72
  entry_content: |
    aga sol frame de görünce tekrar yükledim belki durum düzelmiştir diye. efendim direk tinder gold'a yükselttim üyeliğimi ve ava başladım.<br/>1 tane boost kullandım , 1 saat de 4 şehir değiştirdim toplam 7 match geldi amk.<br/><br/>sonra dedim lan bence tipim harbi o kadar kötü değil, biz erkekler nasıl çoğu kızı kaydırıyorsak bir de topi top olarak deniyim dedim uygulamayı.<br/><br/>aramayı erkek yaptım, lokasyon san francisco.<br/>şaka yapmıyorum<br/><br/>1 saat de 96 tane erkekle matchleştim hepsini bile kaydırmadım.<br/><br/>aralarında google da çalışan tut bir sürü havalı tip de var. yakışıklı da bol.<br/>kendimi birden herkesin beni sikmeye çalıştığu türk bir kezban gibi hissettim neyse moralim iyi olarak ben yatıyorum izninizle.<br/><br/>hetero erkekler olarak harbi işimiz zor aga.<br/>top olmak varmış bu dünyada :d<br/><br/>hala likelar yağmaya devam ediyor arkadaşlar, telefonun sesini kapattım. çoğu erkek ama :(<br/><br/>cute guy<br/>sexy boy tarzı bir sürü mesaj geliyor toplardan efendim götüm çok kalktı<br/>gaylerin neden çok yüksek oranda hıv kaptığını şimdi anladım aga, adamlar için sex bakkaldan su almaktan daha kolay amk
- entry_name: |
    içine girildiğinde çıkılmak istenmeyen yerler
  entry_id:  74523612
  entry_writer: drink je verdriet weg
  entry_fav_count: 57
  entry_content: |
    ev.<br/><br/>bu nedenle biriyle is cikisi bulusmuyorsam, bulusamiyorum.
- entry_name: |
    galatasaray
  entry_id:  74510833
  entry_writer: n8
  entry_fav_count: 53
  entry_content: |
    2017 2018 sezonunda gerek haftaici ic saha maclarina 30000'in üzerinde taraftar gelmesiyle, gerekse tarihinin en centilmen takimiyla sampiyonlugu en cok hakeden takimdir.<br/><br/>tek yapmasi gereken deplasmanda galibiyetlere baslamasidir, ic sahadaki maclari zaten final maclari ama o maclara dahi puan avantajiyla cikacagiz.<br/><br/><a class="b" href="/?q=sen+sampiyon+olacaksin">sen sampiyon olacaksin</a>
- entry_name: |
    ekşi itiraf
  entry_id:  74509929
  entry_writer: golgem ve ben
  entry_fav_count: 49
  entry_content: |
    daha yedi yaşındayken notumu verdiler benim; bu kız okumaz. abim kimsenin yardımı olmadan kendi başına okuma yazmayı öğrenince ben ne yapsam yaranamadım. okuma yazma okulda mı öğrenilir oğlum. bu nasıl bi ezikliktir! <sup class="ab"><a title="(bkz: evet o ezik benim)" href="/?q=evet+o+ezik+benim" data-query="evet o ezik benim">*</a></sup><br/><br/>aslında öyle kötü bi öğrenci de değildim. ödevlerini zamanında yapan, boş zamanlarında kitap okuyan, hep takdir belgesi getiren bi öğrenciydim. ama abisine sınıf atlattırılması düşünülen bi çocuğa göre hep 'eh, idare eder' oldum. <br/><br/>ortaokul bitince ailemin benim hakkımda "okumayacak bu kız, acaba meslek lisesine mi versek? hem meslek sahibi olur hemen." diye konuşulduğunu duymuşluğum bile var. artık nasıl dert ettiysem bu konuşmayı, lisede okul birincisi oldum. bu başarı aileme yetti sanırım. inandırdık onları okuyabileceğime ama bitmiyordu ki sıkıntılarım. okulda kimse okul birincisi olduğuma inanmak istemiyordu. kendi hocalarım bile "bu dönemlik bu, seneye olamazsın" diyordu. bu öğrenciyi ben yetiştirdim demek yerine herkes ayağımı kaydırmaya çalışıyordu. diğer dönem de okul birincisi olunca, sana kolay hoca denk gelmiş diye bir sonraki dönem sınıfımı değiştirip daha zor hocaya verdiler. okulun sıfırcı matematik hocasından bile 100 almam bile yetmedi. <br/><br/>üç sene burslu olarak farklı dersanelere gittim. hepsinde öncelikle kendimi kanıtlamam gerekiyordu. herkesin öylesine girdiği denemelere ben hep stresle giriyordum. henüz türev, integral konularını görmeden sorularını çözmem bekleniyordu. yapamayınca da "heh işte bak ben demiştim" deniyordu. <br/><br/>işin tuhaf yanı şu; bana inanmayan tüm bu insanlarla aynı fikirdeydim. bu sınavdan kesin kötü alırsın, çok zor dediklerinde "bence de, zaten geçen sınavda sorular kolaydı, ondan yüksek aldım" derdim. fen lisesinden bi arkadaşım "biz parabol sorularını kalem kullanmadan çözüyoruz dediğinde, "ben kalem kullanıyorum, üniversite benim neyime" demişliğim var. başarısızlıklarımı kendime, başarılarımı da hep hoca kolay sormuş gibi bahanelerle dış faktörlere bağlıyordum.<br/><br/>türkiye'deki iyi üniversitelerden birine gittim. gidene kadar inanamadım kendime. hatta başarı bursu aldım ama burs mülakatında bile rakiplerimi tanıdıktan sonra "yok ya ben hak etmiyorum bu bursu, onlar alsın" demişliğim var. lisans bitti, yüksek lisansı başlar. başka üniversiteden geldin, hadi kendini kanıtla derler. yine sil baştan başlar her şey. şimdi doktora için iyi bir üniversitedeyim. ama ben bile hala içten içe "ulan bu adamlar beni ne yapsın" diyorum. yurtdışından bir hoca ile mailleşiyorum qualified biri olduğumu söylediklerinde inanamıyorum. ben bile kendime güvenmezken insanlar bana neden güvensin ki? <br/><br/>bu kadar özgüvensiz biri olarak en nefret ettiğim sorulardan biri tabii ki mülakatların vazgeçilmezi olan "neden sizi tercih edelim?" sorusu. ne bileyim oğlum ben. sormayın böyle zor sorular bana.
- entry_name: |
    geceye bir leyla ile mecnun repliği bırak
  entry_id:  74538082
  entry_writer: ebedi edebiyat
  entry_fav_count: 42
  entry_content: |
    -ardına bakma mecnun.<br/>-kader almaya mı geldi beni benden?<br/>-bunu bir daha sorma mecnun.<br/>-neden at mı var arkamda?
- entry_name: |
    hocalı katliamı
  entry_id:  74515930
  entry_writer: kor nokta
  entry_fav_count: 40
  entry_content: |
    dünyanın 26 yıldır umrunda olmayan katliamdır, soykırımdır. ölenler; bizimle aynı dili, tarihi, kültürü paylaşan, bizimle aynı millet olan azerbaycan türkleri.
- entry_name: |
    26 şubat 2018 m.başakşehir gençlerbirliği maçı
  entry_id:  74537753
  entry_writer: cy
  entry_fav_count: 38
  entry_content: |
    beş on kişi "bu taraftar için saldırın" tezahüratına başladıktan sonra "bunlar için saldırılır mı be" diye düşündü herhalde başakşehirli futbolcular; ipin ucunu hepten bıraktılar.
- entry_name: |
    bir kediyle yapılabilecek en güzel aktivite
  entry_id:  74538102
  entry_writer: yagmur mry
  entry_fav_count: 33
  entry_content: |
    saklambaç oynamak. şapşikler çok güzel saklanıyor ama kuyruklarını hep unutuyorlar ehehe
- entry_name: |
    çukur (dizi)
  entry_id:  74533268
  entry_writer: byhasret
  entry_fav_count: 32
  entry_content: |
    not; 17. bölüme kadar izlemediysen aşağıyı okuma!<br/><br/>bu dizinin anasını nasıl sikebilirler diye düşünüyordum hele şükür 17. bölümü izlemiş olmakla birlikte çok düşünmemek gerektiğini anladım.<br/><br/>nedir lan o amir karakteri? ne sikim mal senaristlersiniz lan siz? herif iki metreden bazuka ile araba patlatıyor, güya klasik müzik dinliyor güya piyano çalıyor filan? ha yamaç karakteri kimyager madem bu herifi atom fizikçisi olma gibi hünerler yükleyip süper hero özelliklerle daha da donatın isterseniz. hatta ölümsüz filan olsun, gözlerinden ateş saçsın filan ne bileyim uçsun hatta, bir yerden bir yere ışınlanarak gitsin. zaten bir fantastik ögeler eksikti aq dizisinde. <br/><br/>bir dizi nasıl sikilir bir karakter nasıl oluşturulmaz içine sıçılır görmüş olduk. <br/><br/>kardeşinin olduğu restoranda ses sistemi kurup oyuncak ork çalmalar filan yahu onu içeri sokup kurarken nasıl fark edilmedin diye merak ediyor insan, ha öyle çokta değil iki üç masa ilerisi. <br/><br/>şöyle klasik müzik dinleyen ama aşırı zeki oturaklı, aşırı kibar ama gerektiğinde çukuru sikertebilecek özelliklerini iyi kamufle eden bir karakter sokamadınız aq dizisine. anca süper hero tipler. yamaç karakterine bir şey demiyorum diyen demiş zaten. <br/><br/>ulan bize polat alemdar'lardan kına getirdiniz derken bunu kastetmiyorduk. daha insancıl daha gerçekçi daha olağan karakterlerden bahsediyorduk. fakat dizi senaristleri bu kadar avengers izleyince demek ki diziye bir iki fantastik öge sokmaktan kendilerini alamıyorlar. <br/><br/>lan bu diziyi çoğu kişinin izleme sebebi ercan kesal'dır. idris baba karakteridir ama 17 bölüm olmuş ilk bölümde eve çürük meyve götüren idris babadan eser yok hatta hiç yok. herif sanki figüran gibi arada görünüyor filan derinlik yok bişey yok.<br/><br/>sizin o kaleminizi sikeyim ben amına koyduklarım, ızdırabını, aşortmanını siktiklerim...<br/><br/>bilmem ne amirmiş amına kodumun amiri, 25 yaşında ergenin altında hummer jeep var bir de ne ara amir olmuş ne ara doğuya sürgün yiyip geri gelmiş bu kadar olayı hayatının hangi siktiğim dönemine sığdırmış bana biri anlatsın. <br/><br/>lan oğlum şu klavyeyi uzak tutun benden gözüm seyiriyor hala çok pis küfürler edip çok şeyler yazasım var alamıyorum kendimi.
- entry_name: |
    kokusu kendinden güzel olan yiyecekler
  entry_id:  74534878
  entry_writer: cahilim cahilsin cahiller
  entry_fav_count: 29
  entry_content: |
    (bkz: <a class="b" href="/?q=kahve">kahve</a>)
- entry_name: |
    samsung galaxy s9
  entry_id:  74511191
  entry_writer: onuraydin
  entry_fav_count: 28
  entry_content: |
    en güzel yanı, s8 almayı planlayanlar için güzel bir model s9. fiyatları düşecek çünkü. gerisinde tabiki çok farkı yok bir önceki modelinden. iphone'da da yoktu.<br/><br/>ancak bazı alanlar var tabi geliştirilecek olanlar, ama samsung ve apple sanki aralarında konuşup anlaşmış gibi frenliyorlar önümüzdeki yıllarda yavaş yavaş bunları sunmak için.<br/><br/>ne mi onlar?<br/><br/>misal 8gb ram. lan 1400tl'lik zte nubia'da var 6gb ram, koskoca s9'a koya koya aynısını mı koyuyorsunuz? oneplus 5t, razer phone, xiaomi mi mix 2s, asus zenfone ar hepsinde var 8gb, inatla 2-3 yıla tahminen saçma sapan uygulamalarla yetmeyecek kapasitelere koşturuyorlar. <br/><br/>misal 64gb depolama. lan 1000tl'lik xiaomi'lerde var 64gb. flagship yapıyorsun, 5-6k'lık telefon satacaksın, koy yahu 256gb'ı. lan gene 1400tl'lik nubia'da 128gb depolama var ya la. <br/><br/>kameraya da gelelim, iki tarafa da çift kamera koymasını beklerken, s9 plus'da sadece iki kamera var arka tarafta. lan casper'da bile var, geliştirilmiş aperture kısıp açma gene bir nebze, millet 16mp+20mp kameralar kullanıyor, hala 12mp'lere bel bağlamak nedir. arkasındaki teknolojiyi daha derliyorsun, ama gene bir sonraki modele ekleyecek şey olsun diye burada da frenliyorsun kendini. ön kamera da 8mp ulan, yahu paranız mı yetişmedi nedir bu saçmalık dünyanın en pahalı telefonlarından birisin ya la. <br/><br/>en beter durumdan bahsedelim, o da pili. lan ben sizin cibiliyetinizi sikeyim 3000mah nedir lan? taşak mı geçiyorsunuz? millet 5000mah üzerinde uçuyor, başka markaları incelemeye bile gerek yok, yahu galaxy a9'da bile 5000mah var. ızdırap mısınız lan siz insanlara? biz niye her yere yanımızda şarj aleti, taşınabilir şarj aletleriyle gezip duruyoruz? sabah işe giderken trafikte kullansan ve öğlen arasında kullansan şarj bitiyor lan, hala inatla nedir bu bataryayı yetersiz yapma hevesi. <br/><br/>piyasada arkaya ekran koyan var, sanki samsung bu ürünü "lan 100 dolara ürettiğimiz telefonu bakalım kaç keko 1000 dolara alıcak, bu da satarsa 10 dolarlık telefonu 2000 dolara satarız" anlayışıyla mı çıktılar, ne yaptılar pek anlayamadım.
- entry_name: |
    otizmli çocuklara bağış için oyna projesi
  entry_id:  74533747
  entry_writer: highwaytohell
  entry_fav_count: 25
  entry_content: |
    otizme farkındalığı arttırmak amacıyla <a rel="nofollow noopener" class="url" target="_blank" href="https://www.instagram.com/thegamefbk/" title="https://www.instagram.com/thegamefbk/">the game</a> ile <a class="b" href="/?q=tohum+otizm+vakf%c4%b1">tohum otizm vakfı</a>'nın birlikte hayata geçirdiği bağış için oyna adlı projedir.<br/><br/>the game instagram sayfasında projeyi şu şekilde paylaştı;<br/><br/>"uzun zamandır üzerine çalıştığımız bağış için oyna projemizin bugün itibariyle duyurularına başladık. otizmli çocukların eğitimine destek olmak ve otizme farkındalığı arttırmak amacıyla üretilen bu atarilerde oynayarak çocuklara destek olabilirsiniz. şimdilik kolektif house levent ve bahçeşehir üniversitesi beşiktaş kampüsü'nde olan cihazları pek yakında avm'lerde görebileceksiniz. projeye destek vermek için ve cihazlarımızı kurumunuzda konumlandırmak için bizlere ulaşabilirsiniz :) "<br/><br/>açıklama ve proje tanıtım videosu <a rel="nofollow noopener" class="url" target="_blank" href="https://www.instagram.com/p/BfqRdFiglVX/" title="https://www.instagram.com/p/BfqRdFiglVX/">linki</a>
- entry_name: |
    la casa de papel
  entry_id:  74510051
  entry_writer: we fucking played great the whole season
  entry_fav_count: 24
  entry_content: |
    dizinin izleyicisinde bıraktığı bir his için; <a rel="nofollow noopener" class="url" target="_blank" href="https://i.hizliresim.com/nObWBN.jpg">https://i.hizliresim.com/nobwbn.jpg</a>
- entry_name: |
    milliyetçiliğin bittiği an
  entry_id:  74536791
  entry_writer: hakantheturk
  entry_fav_count: 18
  entry_content: |
    eski bir milliyetçi olarak şöyle söyliyeyim yaşanacak ilk büyük savaştan sonra bu ülkede milliyetçiler milliyetçi olduğunu, imam hatipliler imam hatipli olduğunu, rize'liler de rize'liliğini gizleyecek.<br/><br/>savaşa kadar milliyetçilik tam gaz sürer. sonrasını göreceğiz.<br/><br/>yemen ve çanakkale cephelerindeki şehit sayıları askerden kaçanların oranını zirveye taşımıştı. modern çağın silahlarının neler yapabildiğini görürsek eğer (bkz: <a class="b" href="/?q=melhame-i+k%c3%bcbra">melhame-i kübra</a>) at üstünden putine meydan okuyan, elde balta diriliş izleyen vatan sevdalılarının ne kadar milliyetçi kalacağını görürürüz.<br/><br/>yeryüzü vatanım, insanlar kardeşim.
- entry_name: |
    karizmatik ölen film karakterleri
  entry_id:  74537765
  entry_writer: rock n roll junkie
  entry_fav_count: 16
  entry_content: |
    (bkz: <a class="b" href="/?q=y%c4%b1k%c4%b1lmayan+adam">yıkılmayan adam</a>) (bkz: <a class="b" href="/?q=c%c3%bcneyt+ark%c4%b1n">cüneyt arkın</a>)<sup class="ab"><a title="(bkz: swh)" href="/?q=swh" data-query="swh">*</a></sup>
- entry_name: |
    tıp bilimi sayısal mı yoksa sosyal bilim mi
  entry_id:  74534744
  entry_writer: poturgilinpotur
  entry_fav_count: 11
  entry_content: |
    <a class="b" href="/?q=tababet">tababet</a>, (tıp, tabip aynı kökten) bir <a class="b" href="/?q=zanaat">zanaat</a> 'tir, bilimsel yöntemi temel alabilir ama amacı hayatı, canlılığı, biyolojik sistemleri açıklamak anlamak değil, hastalıkları, yaralanmaları tedavi etmek, önlemek, insan hayatının sağlıkla ilgili standardını <a class="b" href="/?q=afiyet">afiyet</a> ini artırmaktır. o yüzden bu anlamda hekimlik, tabiplik sanatı tabiri vardır. amacı açıklamak ve anlamak olan bilgi disiplini bilimdir. amacı bir işi başarmak, bir sorunu çözmek, çözümü geliştirmek olan disiplin teknik veya değil, bir zanaat'tir. şimdi kansere çare olarak bir yöntem bulunsa, bunun etkisi, faydası, yan etkilerinin kabul edilebilirligi kontrol grupları vs ile bilimsel yöntemle gösterilse ancak bu yöntemin kanseri nasıl tedavi ettiği engellediği bilinemese açıklanamasa bu yine tıbbi bir başarı olur ancak bilimsel anlamda bir bilgi artışı olmaz. bir şeyleri araştırmak için iyi bir ipucu ortaya çıkmış olur.
- entry_name: |
    recep tayyip erdoğan
  entry_id:  74515941
  entry_writer: yazarolaydimiyidi
  entry_fav_count: 11
  entry_content: |
    sevdalılarından biri <br/>"ruh hastası mısınız nesiniz arkadaş madem adamı sevmiyorsun ondan nefret ediyorsun görmeye tahammülün yok adını anmak bile sana zulüm, ne diye sürekli onunla ilgili şamata yapıyorsun? hayatına işine gücüne baksana gelip başlığında tatava yapacağına."<br/>şaka yapmıyor anladığım kadarıyla gayet ciddi yazmış. ülkede uçan kuşun bile sakata gelmeden geçeyim şu ülkeden deyip g.tünü kolladığı bir dönemde işimize bakacakmışız. hayatımızın tam ortasına çöreklendiği için olabilir mi bu kadar isyan acaba ? veya defolup hitmeli miyiz beğenmiyorsak ? <br/><br/>kafaya bak ya. herkes emir eri mi olacak reisinizin ? askeriye mi la burası ?
- entry_name: |
    starbucks ve türevlerinde televizyon olmaması
  entry_id:  74534669
  entry_writer: frkinhere
  entry_fav_count: 8
  entry_content: |
    eksi sözlük 40 yaş üstü yazarlar<br/><br/>televizyon mu?
- entry_name: |
    bir erkekle yapılabilecek en güzel aktivite
  entry_id:  74534243
  entry_writer: diego alejandro fuentes medina
  entry_fav_count: 5
  entry_content: |
    sözlük kadınlarından çok sözlük erkeklerinin yazacağı başlıktır.<br/><br/>not: şu üstte benim yazdığım cümle çaylak yapılma sebebiydi.
- entry_name: |
    allianz'ın depresyona sokan reklamı
  entry_id:  74534342
  entry_writer: camus hocam cay koy da icelim
  entry_fav_count: 4
  entry_content: |
    reklamın iyisi kötüsü olmaz diye çekmişler. yalnız gerçekten kötü olmuş hacı. duygusallık verelim derken dozunu kaçırmışsınız. hem mantık hatası var. eğer araç sigortası söz konusuysa kızın kazası ve adamın sevgisiyle bağlantısız. eğer hayat sigortası ise konu olan, kız öldükten sonra adam napsın parayı. kısaca mal sevgisi için yapılan sigortayı ,can sevgisi ile kotarmaya çalışmışsınız olmamış. şimdi gidin düzgün bir şey çekin.
- entry_name: |
    türkülerin çoğunun tırt olması
  entry_id:  74538163
  entry_writer: sonsuz sevgilerimle
  entry_fav_count: 3
  entry_content: |
    bu başlığı çoğunlukla türkü sevenler dolduracağı için ben dünya güzel bir türkü atıp kaçayım: <br/><a rel="nofollow noopener" class="url" target="_blank" href="https://www.youtube.com/watch?v=4ai3myMdPYY" title="https://www.youtube.com/watch?v=4ai3myMdPYY">çayın öte yüzünde</a>.<br/>nasıl bir şey allahım ya. <br/>her bir notası vücudumda farklı bir hücreye dokunuyor.
- entry_name: |
    çaylaklardan mesaj bekleyen yazarlar veritabanı
  entry_id:  74511429
  entry_writer: oktayozgul
  entry_fav_count: 3
  entry_content: |
    içeriğinin pek yakında <a class="b" href="/?q=ek%c5%9fi+s%c3%b6zl%c3%bck+fuck+buddy+aran%c4%b1yor+ilanlar%c4%b1">ekşi sözlük fuck buddy aranıyor ilanları</a> formatına geçeceğini düşündüğüm veritabanı. atın fava bekleyin.
- entry_name: |
    hoşlanılan kızın 04.47'de whatsapp'ta görülmesi
  entry_id:  74540295
  entry_writer: dedicado a deus
  entry_fav_count: 2
  entry_content: |
    onun da birinden hoşlandığına delalettir.
- entry_name: |
    galatasaray fenerbahçe rekabeti
  entry_id:  74531416
  entry_writer: alihandro
  entry_fav_count: 2
  entry_content: |
    her türlü rekabetcige yegdir.fenerbahce ile yaristigimiz zamanlar en güzel zamanlardir.elbette problemler oluyor ancak bu mümkün mertebe birbirine bel alti vurmadan oluyor.son yillarda ortaya cikan bazi takim ve taraftarlarinin kendilerini rekabete dahil etmeye calismasi ise ancak bizi güldürüyor.2 senede kendilerini büyük sanmaya baslamalari cok afacanca.keratalar sizi.
- entry_name: |
    the walking dead
  entry_id:  74518538
  entry_writer: ikitekerlibisiklet
  entry_fav_count: 1
  entry_content: |
    --- <a class="b" href="/?q=spoiler">spoiler</a> ---<br/><br/>walking dead sevenleri tatmin edecek bir bolum oldu 809. helal olsun adim adim birlesmeye hazirliyorlar bizi bu sezon. gelecek sezon negan-rick kardesligini yaratacak ideolojinin tohumlari carl tarafindan babanin aklina yerlesti. flash forward sahneleri efsane. ben o sahnelerin carlin hayalinden ibaret oldugunu dusunmuyorum, oyleyse bile dizinin sonunu kimin gorecegini az cok caktik gibi. herneyse.<br/><br/>ben twd evrenine hayranim. benim gibi arkadaslara da web serilerini de izleyin oneririm (twd machete yada passage gibi)<br/><br/>ha bide transformers sevenleri tatmin etmeyecek bir bolum olmus:( <br/><br/>ilerleyen gunlerde bu basliga gelip mac yorumu gibi doserler, atanamamis aksiyon avcilari.<br/><br/>--- <a class="b" href="/?q=spoiler">spoiler</a> ---
- entry_name: |
    2017-2018 sezonu süper lig sıralaması tahminleri
  entry_id:  74540234
  entry_writer: etornietlo
  entry_fav_count: 1
  entry_content: |
    1- ligin en iyi futbolunu oynayan takimi<br/>2- teknik direktörü faktoru ile yarista kalan takim<br/>3- basarisiz sampiyonluk proje takimi<br/>4- td ve baskanina sampiyonluk yakismayacak takim
- entry_name: |
    çocuk yaşta izlenmiş travmatik diziler ve filmler
  entry_id:  74539512
  entry_writer: pompa panter
  entry_fav_count: 0
  entry_content: |
    trt de ziyaretçiler vardi. diana karisi koca fareyi yerdi lan o neydi yav. bi dizi daha vardi unuttum. uzay gemisinde miydi neydi adam yere dusmus bagirsaklariyla dolaniyordu. baslangic jeneriginin bir parcasiydi heralde. hatirlayan var mi lan.<br/><br/>ha ha evet o mkun eli unutulmaz filmdi.
---
