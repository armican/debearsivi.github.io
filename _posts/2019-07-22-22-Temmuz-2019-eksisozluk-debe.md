---
layout: post
title: 22 Temmuz 2019 Ekşi Sözlük Debe
data:
- entry_name: |
    20 temmuz 2019 ibb atamaları
  entry_id:  92882940
  entry_writer: molasar
  entry_fav_count: 1459
  entry_content: |
    ozlenen, istenen goruntudur. umarim sonunda aranan baslica kriterin imam hatip mezunu oldugu gunler geride kalir ve liyakat sistemi genele yayilir.<br/><br/>ulaşımdan sorumlu genel sekreter yardımcılığı'na raylı sistem uzmanı orhan demir atandi. orta doğu teknik üniversitesi mimarlık fakültesi şehir ve bölge planlama bölümü'nden 1979 yılında mezun.stfa'da calismis, 1999 yılından bu yana mimar sinan güzel sanatlar üniversitesi mimarlık fakültesi şehir ve bölge planlama bölümü'nde yarı zamanlı öğretim görevlisi olarak ulaşım planlaması konusunda dersler veriyor.<br/><br/>raylı sistemler daire başkanlığına, çok iyi seviyede japonca ve ingilizce bilen doç. dr. pelin alpkökin atandi. kendisi istanbul teknik üniversitesi inşaat mühendisliği bölümü mezunu, 1998 yılında ulaştırma mühendisliği yüksek lisans programını bitirmis. kendisinin doktorasi japonya'dan.<br/><br/>park bahçe ve yeşil alanlar daire başkanlığı'na da yüksek mimar prof. dr. yasin çağatay seçkin atandı. kendisi mimar, sırasıyla istanbul teknik üniversitesi mimarlık tarihi programı'ndan yüksek mimar ve kentsel tasarım programı'ndan da doktor unvanlarını almis. su an da universitelerde dersler veriyor.<br/><br/><a rel="nofollow noopener" class="url" target="_blank" href="https://www.sozcu.com.tr/2019/gundem/son-dakika-ekrem-imamoglundan-ibbde-3-atama-birden-5241290/" title="https://www.sozcu.com.tr/2019/gundem/son-dakika-ekrem-imamoglundan-ibbde-3-atama-birden-5241290/">kaynak</a>
  entry_channels: siyaset,haber
- entry_name: |
    bir bilgisayar oyununda yapılmış en zalimce şey
  entry_id:  92879999
  entry_writer: lorensoth
  entry_fav_count: 790
  entry_content: |
    sims de (bkz: <a class="b" href="/?q=the+sims">the sims</a>) evin üst katını halı veya kilimle dolduruyoruz üst üste koyabilirsiniz. daha sonra parti verip mahallede kim var kim yok çağırıyoruz. yapabildiğiniz en güzel yemeği evin üst katına koyuyoruz.gelelim kilit noktaya. bir tane havai fişek itemi vardı havaya kadar gidip çoluk çocuk eglendiriyordu.herkes evin üst katında toplanmışken bunu üst katta ateşleyip alt kata kaçıyoruz. ve anında merdivenleri satıyoruz<sup class="ab"><a title="(bkz: swh)" href="/?q=swh" data-query="swh">*</a></sup> uzaktan komşuların pişmesini izleyip geriye kalan mezar taşlarıyla bahçemizi dekore ediyoruz.iyi eğlenceler<sup class="ab"><a title="(bkz: swh)" href="/?q=swh" data-query="swh">*</a></sup>
  entry_channels: anket,oyun
- entry_name: |
    internette her şey mevcut iken netflix alan tip
  entry_id:  92888936
  entry_writer: one does not simply walk into eksisozluk
  entry_fav_count: 646
  entry_content: |
    bak önce 15 sn dedi sonra torrent dedi. ne dediğinin farkında değil üstüne gitmeyin, cevap vermeyin.
  entry_channels: tv,sinema
- entry_name: |
    galatasaray'ın başarılarında gülen'in etkisi oldu
  entry_id:  92895320
  entry_writer: quatermain
  entry_fav_count: 602
  entry_content: |
    15 temmuz 2016'dan sonra galatasaray kaç kupa aldı fener kaç kupa aldı gördük.<br/><br/>edit=başlık sahibi haksız olduğunu bildiği için dayanamadı,kaçtı
  entry_channels: spor
- entry_name: |
    eljif elmas
  entry_id:  92884813
  entry_writer: ffmvain
  entry_fav_count: 314
  entry_content: |
    kıskanç eziklerin hakkında artık yalan söylediği oyuncu. eljif as oyuncu falan değil diyorlar şimdi de. ki ilk 11'e oynayıp oynamaması önemli değil de, bu çocuk bu sene 40 resmi maça çıktı kulüp ile. bu sene 2500 dakika süre aldı. kendi milli takımında 9 maç oynadı. yani 49 maç oynadı. <br/>daha ne oynayacak?<br/><br/>adam 19 yaş ve altında avrupa'da en fazla oynayan 12. oyuncu. <br/><br/><a rel="nofollow noopener" class="url" target="_blank" href="https://www.transfermarkt.com.tr/meisteeinsaetze/gesamteinsaetze/statistik/2018/plus/1/galerie/0?saison_id=2018&amp;wettbewerb_id=profi&amp;land_id=0&amp;altersklasse=u19&amp;yt0=Göster" title="https://www.transfermarkt.com.tr/meisteeinsaetze/gesamteinsaetze/statistik/2018/plus/1/galerie/0?saison_id=2018&amp;wettbewerb_id=profi&amp;land_id=0&amp;altersklasse=u19&amp;yt0=Göster">https://www.transfermarkt.com.tr/…se=u19&amp;yt0=göster</a><br/><br/>napoli başkanının eljif'i yerine aldığı oyuncu olan eljif'ten 4 yaş büyük marko rog 20 milyon euro'ya frankfurt'a gidecek. bakalım marko rog kaç maç oynamış?<br/><br/><a rel="nofollow noopener" class="url" target="_blank" href="https://www.transfermarkt.com.tr/marko-rog/leistungsdaten/spieler/317482/plus/0?saison=2018" title="https://www.transfermarkt.com.tr/marko-rog/leistungsdaten/spieler/317482/plus/0?saison=2018">https://www.transfermarkt.com.tr/…lus/0?saison=2018</a><br/><br/>keza daha geçen hafta benfica'ya aynı paraya sattıkları 25 yaşındaki vinicius'a bakalım. <br/><br/><a rel="nofollow noopener" class="url" target="_blank" href="https://www.transfermarkt.com.tr/carlos-vinicius/leistungsdaten/spieler/525287/plus/0?saison=2018" title="https://www.transfermarkt.com.tr/carlos-vinicius/leistungsdaten/spieler/525287/plus/0?saison=2018">https://www.transfermarkt.com.tr/…lus/0?saison=2018</a><br/><br/>yetmedi mi? transfer dönemi başında eljif'ten 2 yaş büyük ve 21 milyon euro'ya sattıkları diawara'ya bakalım. <br/><br/><a rel="nofollow noopener" class="url" target="_blank" href="https://www.transfermarkt.com.tr/amadou-diawara/leistungsdaten/spieler/355501/plus/0?saison=2018" title="https://www.transfermarkt.com.tr/amadou-diawara/leistungsdaten/spieler/355501/plus/0?saison=2018">https://www.transfermarkt.com.tr/…lus/0?saison=2018</a><br/><br/>size göre benfica ve roma da şike peşinde koşuyor. 'oyun büyük ey aptallar' diyebiliriz bu durumda değil mi? <br/>gördünüz mü büyük resmi? tüm dünya seferber olmuş eljif'e para verip fenerbahçe'ye para kazandırmak için ne transferler yapıyor. <br/><br/>şu transfer bitse de kurtulsak. bu kadar akılsız gençliğin yetiştiği gerçeği ülke hakkında umutları kırıyor.
  entry_channels: spor
- entry_name: |
    mirasın tümünün çocuğa kalması saçmalığı
  entry_id:  92877580
  entry_writer: betveren
  entry_fav_count: 285
  entry_content: |
    ben ölünce paramin insanlara dagitilacagini bilsem tamamini harcarim ölünce mezar tasimi ölmezsem sikimin basini alirlar...<br/><br/>tanim : saçma sapan bir sistem istegidir...<br/><br/>edit : özel mesaj yoluyla insanlara kalsa neyin eksilir diyenler olmus, daha birkac gün önce (bkz: <a class="b" href="/?q=paras%c4%b1n%c4%b1+kaybeden+turist">parasını kaybeden turist</a>) basliginda yurtdisinda parasini kaybedip yardimci olunmasini isteyen elemani bir sikmediginiz kaldi, ben insanlara para mi birakirim amk... siktirin gidin...
  entry_channels: anket,ekonomi
- entry_name: |
    ata demirer vs cem yılmaz vs şahan vs şafak sezer
  entry_id:  92886988
  entry_writer: orhan abi halk show
  entry_fav_count: 229
  entry_content: |
    ata demirer: taklit<br/>cem yılmaz: zeka<br/>şafak sezer: mahalle kültürü<br/>şahan gökbakar: recep ivedik
  entry_channels: anket,tv,sinema
- entry_name: |
    9.5 + 9.5 toplamının 19 olması
  entry_id:  92889274
  entry_writer: 42 kilometre
  entry_fav_count: 200
  entry_content: |
    31+69=100 denklemi kadar tatmin edici değildir.
  entry_channels: bilim
- entry_name: |
    çocukluktan kalma yara izi
  entry_id:  92869918
  entry_writer: sn3x
  entry_fav_count: 187
  entry_content: |
    bizim neslin en çok dizlerinde bulunur.
  entry_channels: anket
- entry_name: |
    100 km'de 5 litre akaryakıt yakan araç
  entry_id:  92888744
  entry_writer: defoe
  entry_fav_count: 185
  entry_content: |
    renault 1.5 motorunu kullanan herhangi bir otomobildir.
  entry_channels: anket,otomotiv,ekonomi
- entry_name: |
    antilobun aslanı bakkala göndermesi
  entry_id:  92897533
  entry_writer: carbonifera
  entry_fav_count: 182
  entry_content: |
    aslanın <a class="b" href="/?q=ben+zaten+ona+gitmiyodum+ki">ben zaten ona gitmiyodum ki</a> ortadan yok oluşu.
  entry_channels: haber
- entry_name: |
    netflix'in 1 haftada 28 milyar $ değer kaybetmesi
  entry_id:  92871452
  entry_writer: bol
  entry_fav_count: 165
  entry_content: |
    üyeliğimiz var. nikahımıza almadık. <br/><br/>batarsa iran sinemasını över o an ki en iyi stream servise geçeriz.
  entry_channels: tv,teknoloji,ekonomi
- entry_name: |
    en sadık erkek burcu
  entry_id:  92879957
  entry_writer: umittaciri
  entry_fav_count: 118
  entry_content: |
    (bkz: <a class="b" href="/?q=o%c4%9flak+burcu+erke%c4%9fi">oğlak burcu erkeği</a>) <br/>harcadığı ve harcayacağı para ile emeği hesaplayacağından aldatmaya <a class="b" href="/?q=%c3%bc%c5%9fenir">üşenir</a>. <br/>bu yüzden evde beslenebilecek en iyi erkektir.
  entry_channels: anket,ilişkiler
- entry_name: |
    lotr'u netflix çekse hangi karakter gay olurdu
  entry_id:  92887101
  entry_writer: nedennilmiyorum
  entry_fav_count: 111
  entry_content: |
    aslında başlığı lotr’u netflix çekseydi hangi karakter hem zenci hem ibne olurdu diye açacaktım lakin malum başlık kısıtlamaları. <br/><br/>gecen gün aklıma takılan soru. bence gay olan kesin madenci cuce olurdu. legolas da hem zenci hem gay olur böylelikle aralarındaki ask icin de uygun ortam oluşurdu.
  entry_channels: anket,tv
- entry_name: |
    fb-gs youtube 1 milyon abone savaşı
  entry_id:  92870256
  entry_writer: jeksparow
  entry_fav_count: 93
  entry_content: |
    (bkz: <a class="b" href="/?q=fb-gs+sidik+yar%c4%b1%c5%9f%c4%b1">fb-gs sidik yarışı</a>)
  entry_channels: spor
- entry_name: |
    bülent ecevit'in fetöcü olma ihtimali
  entry_id:  92893948
  entry_writer: tembel fare
  entry_fav_count: 87
  entry_content: |
    <a class="b" href="/?q=diriyi+siktiniz+s%c4%b1ra+%c3%b6l%c3%bcye+mi+geldi">diriyi siktiniz sıra ölüye mi geldi</a> denesi iddia.<br/><br/>önce yaşayanları temizleyin sonra sıra ecevit'e gelir.<br/><br/>ek olarak bir ipucu vereyim:<br/>ecevit fetöcü olsa <a class="b" href="/?q=ne+istediniz+de+vermedik">ne istediniz de vermedik</a> açıklamasına kadar arkasından atıp tutan bir rte göremezdiniz. adamın hastalığından, ambargo sırasında takındığı dik duruşa kadar sakız olmazdı.
  entry_channels: siyaset,anket,tarih,haber
- entry_name: |
    s-400 filosu için özel tasarlanan arma
  entry_id:  92883156
  entry_writer: ibrayun
  entry_fav_count: 85
  entry_content: |
    el silahı ile kahramanlık yazan arkadaşlar, destanı silah değil onları kullanan millet yazar. o silahlar sadece araçtır. <br/><br/>kurtuluş savaşı destanını yazan bu millet sadece yerli üretim silah mı kullandı? <br/><br/>harika olmuş logodur.
  entry_channels: haber
- entry_name: |
    ay'a nasıl gidildiğini unuttuk
  entry_id:  92897149
  entry_writer: aynshinqaf iki
  entry_fav_count: 79
  entry_content: |
    ekşicilerdeki okuduğunu anlamama sonucu çarpıtılan ifade. adam diyor ki o dönemin teknolojisini birebir uyarlayamayız. sıfırdan almak lazım ve bu maliyetli. ama bu sıfırdan almanın motivasyonu yok. o dönem sovyetler ile yarış hali vardı. şimdi aya gitmenin bir faydası yok. o kadar zaman, para ve beyin gücü harcamanın manası yok. <br/><br/>(bkz: <a class="b" href="/?q=ek%c5%9fi%27de+s%c4%b1radan+bir+g%c3%bcn">ekşi'de sıradan bir gün</a>)
  entry_channels: bilim,teknoloji
- entry_name: |
    norm ender
  entry_id:  92869093
  entry_writer: derbder01
  entry_fav_count: 75
  entry_content: |
    old-school seven biri oladak hayranı ve fanı olduğum rapten sırf bu yeni jenerasyon yüzünden 2010'dan sonra soğudum.<br/>artık yeni jenerasyon rap konusu "sikişiyorum moruq, esrar hap bali ektazi eroin çekiyorum moruqh, moruq içki alkol sigara moruq, gangta moruq, polisler bizi arıyor moruq, gayrimeşruyuz moruq" tarzlarında rap yapmalari eski kuşağı ve old-school sevenleri sinir etme derecesine getirmiştir.<br/><br/>asıl komik olan şu ; şu polisten kaçıyorum mevzusu o kadar saçma. sen bir azınlık değilsin, seni ezmeye çalışan bir polis yok burası amerika veya fransa değil ve zaten türk poliside öyle abd ve fransadaki gibi robot değildir.<br/><br/>eski kuşak ve eski jenerasyon önceden ülkenin sorunlarına değinirken yeni tür rapçiler sadece uyuşturcu para cinsellik alkol sigara ve gayrimeşruluğu anlatmıştır.<br/>rap dediğin şeyin amacı o alt mahallelerdeki veya ülkedeki sorunları dile getirmektir lakin yeni jenerasyon bunu yapmadı.<br/><br/>ışte kuşak çatışması bu yüzden başlamaya o kadar müsaitken, norm ender resmen bunun fitilini ateşleyerek kuşak savaşını başlattı.<br/>yeraltına gömülmüş biz old-schoollar yavaş yavaş ortaya çıkmaya başladı.<br/>(bkz: <a class="b" href="/?q=eski+ve+yeni+jenerasyon+rapcilerin+anlasamamasi">eski ve yeni jenerasyon rapcilerin anlasamamasi</a>)<br/>attığı disleride epey olumlu buluyorum epey iyi yapmış.
  entry_channels: müzik,magazin
- entry_name: |
    şafak sezer in komik olmadığını iddia eden insan
  entry_id:  92888439
  entry_writer: feyk eczaci
  entry_fav_count: 67
  entry_content: |
    mizahtan anlayan insandır.
  entry_channels: tv,sinema
- entry_name: |
    starbucks kahvesinin diğerlerinden farkı
  entry_id:  92896053
  entry_writer: klostrofobik kaplumbagaa
  entry_fav_count: 67
  entry_content: |
    nasıl bir algı abi bu kendin elit hissetmek için starbucksa gitmeler falan. ulan en ucuz kahve starbucksta şu an resmen, yarım litre filtre kahveyi sekiz liraya alabileceğiniz ve bu kahvenin sert ve kıvamında olduğu garantisini veren tek yer. moc'ta aynı kahvenin yarısı 12 lira bilmemnerede 15 lira olmuşken hala elit hissetmek için starbucks'a gidip elimizde birer meşale gibi taşıdığımızı savunanlar var ki beş liraya sütlü nescafe sanırım beklentiler. yanlış anlaşılmasın bayılırım sütlü nescafeye ama bir kahve değildir iki kanserojenin allahıdır. teşekkürler.
  entry_channels: anket
- entry_name: |
    sinirli kadını sakinleştirmek
  entry_id:  92888071
  entry_writer: taijin kyofusho
  entry_fav_count: 65
  entry_content: |
    saçından bahsetmeyin.<br/>eğer kadın kaosla besleniyorsa ve kavga etmeyi seven bir manyaksa asla bugün saçın ne güzel olmuş, sinirlenince çok seksi oluyorsun gibi dangalakça şeyler söylemeyin. götünüze girmesin. <br/><br/>gerçekten sinirlenmişse ve bunu çözmek istiyorsa tutup çekip öpmeyin de. bu işi daha da karmaşıklaştırır. çünkü onun dediği şeye önem vermediğinizi, sikinize takmadığınızı düşünür ve daha çok sinirlenir. <br/><br/>anlamaya çalışın. bir süre sonra sizin sakinliğiniz onu etkileyecektir zaten ama ateşi harlamaktan da vazgeçin. sadece dediği şeye kulak verin. <br/>bu sırf kadınlar için geçerli değil. insan ilişkilerinin temel taşı. onu dinleyin. neden sinirlendi, sinirlenmesinin sebebi ne, aslında size ne anlatmak istiyor bunu düşünün. kışkırtmak yerine dinleyin.
  entry_channels: anket,ilişkiler
- entry_name: |
    futbolun boş iş olması
  entry_id:  92887151
  entry_writer: samimi panda
  entry_fav_count: 55
  entry_content: |
    akademik başarıdan, zekadan bağımsızdır. tam bir davar işidir. futbolu deli gibi seven insanların bomboş tipler olduğunu anlamanız için 3-4 dakika muhabbet etmeniz yeterlidir. <br/><br/>hatta muhabbet bile etmeyin. futbolu seviyorsa hayatınızın 4 dakikası boşa gitmesin.
  entry_channels: spor,anket
- entry_name: |
    ekşici kadınların ekseriyetle çirkin olması
  entry_id:  92893563
  entry_writer: nick konusunda beceriksizim
  entry_fav_count: 44
  entry_content: |
    bu entrylerin sahipleri hep reddedilen yazarlar.. bazen buraya profil fotoğrafı koyan veya instagram adreslerini paylaşanları gördüm, hatta çok yakışıklı olduğunu iddia edip de öyle fotoğraflar atanlar oldu. yolda görsem suratlarına tükürmezdim, öyle apaçi dolu burası. sizde sadece kompleks var dostum. ciğere ulaşamayan kedi misali :-* <br/>tabi ben sizin gibi genelleme yapmıyorum eli yüzü düzgünler de gördüm ama 10 da 1 diyeyim. ve kadın yazarlarla da konuştum, gördüklerim oldu, cidden güzellerdi.
  entry_channels: anket,ilişkiler,ekşi-sözlük
- entry_name: |
    muhabbet kuşunu elektrikli süpürge ile çekmek
  entry_id:  92897097
  entry_writer: axasking
  entry_fav_count: 43
  entry_content: |
    orospu çocuğudur efendim. <br/>kamuoyu oluşturup acilen kuşun kurtarılmasını temenni ettiğimdir.
  entry_channels: haber
- entry_name: |
    kısa paça + dar gömlek + nargile + sert bakış
  entry_id:  92897479
  entry_writer: kirmiziiiruj
  entry_fav_count: 37
  entry_content: |
    seri köz getirciler... ankara çukurambar semtinin adeta bitki örtüsü bunlar
  entry_channels: anket,yeme-içme,moda
- entry_name: |
    hindu kadınların penise tapınması
  entry_id:  92903865
  entry_writer: gaiusjuliuscaesar
  entry_fav_count: 33
  entry_content: |
    inanmıyorsan bile saygı duy terbiyesiz!<br/><br/>bu burda dursun ben zil almaya gidiyorum.
  entry_channels: ilişkiler,haber,seyahat
- entry_name: |
    cep telefonuna sevgili ismini kaydetme biçimleri
  entry_id:  92900014
  entry_writer: dear god make me a bird
  entry_fav_count: 30
  entry_content: |
    annem ve babam hariç herkesi telefon rehberime ad-soyad şeklinde kaydederim ki elimden gelse tc numaranızla kaydederdim. kusura bakmayın, kurumsallıktan ödün veremem.
  entry_channels: anket,ilişkiler,teknoloji
- entry_name: |
    avusturya'da tarihi bina alıp bordo maviye boyamak
  entry_id:  92903827
  entry_writer: danspistindekicocuk
  entry_fav_count: 27
  entry_content: |
    takim sevdasinin ulke sevdasini gecmemesi gerektigini vurgulayacagimiz bir olay. <br/><br/>bu arada bu kadar sevdaliysa buyursun trabzon'a gelsin yasasin.
  entry_channels: spor,haber
- entry_name: |
    aktör denince akla gelen ilk aktör
  entry_id:  92891489
  entry_writer: feanorr
  entry_fav_count: 25
  entry_content: |
    (bkz: <a class="b" href="/?q=c%c3%bcneyt+ark%c4%b1n">cüneyt arkın</a>) komedi, dram, aksiyon, romantik ne ararsan oynamış. gelmiş geçmiş en iyi jön
  entry_channels: anket,tv,sinema
- entry_name: |
    sütun gibi bronz bacaklar + mini şort + halhal
  entry_id:  92900663
  entry_writer: du kannst mich mal am arsch lecken
  entry_fav_count: 22
  entry_content: |
    en kisa zamanda <a class="b" href="/?q=nafaka">nafaka</a> basliginda entry girmen dilegiyle.yengenin ihtiyaci olursa ekran goruntusunu iletirim.
  entry_channels: ilişkiler
- entry_name: |
    en güzel bira
  entry_id:  92902673
  entry_writer: nereden baksan tutarsizlik
  entry_fav_count: 22
  entry_content: |
    beleş biradır.
  entry_channels: anket,yeme-içme
- entry_name: |
    kaş (ilçe)
  entry_id:  92901723
  entry_writer: ghergedan
  entry_fav_count: 20
  entry_content: |
    abi gitmeyin lütfen, bırakın orası güzel kalsın yea. he at kafası, senden başka kimse gitmesin. senelerdir aynı terane. kaş süper ama siz gelmeyin. herkes gidince instagram’da etiket yapmanın anlamı kalmıyor, onun tantanası. sik kafalı japon askerleri sizi. salda’nın da anasını sizin gibi bencil, ilgi orospuları sikti. elin ingilizi, rusu, çekik gözlüsü, arabı gelsin; yesin içsin sıçsın, biz gelmeyelim. he paşam, her yere sen git. babanın malı ya. sikik.
  entry_channels: tarih,seyahat
- entry_name: |
    la casa de papel
  entry_id:  92868676
  entry_writer: lesimi
  entry_fav_count: 12
  entry_content: |
    tanım: gelecek sene başlayacak 4.sezonu iple çekiyoruz<br/><br/>--- <a class="b" href="/?q=spoiler">spoiler</a> ---<br/>alicia’nın osman demesi...<br/><br/>rio’nun işkence gördüğü yerde camiden gelen ezandan bahsetmesi..<br/><br/>lan.. yoksa!!<br/>--- <a class="b" href="/?q=spoiler">spoiler</a> ---
  entry_channels: tv,spoiler
- entry_name: |
    evlenmeden önce yapılması gereken şeyler
  entry_id:  92903345
  entry_writer: bilir isi kisisi
  entry_fav_count: 10
  entry_content: |
    'evlenme olm salak mısın' diyen arkadaşlarınıza kulak verin.<br/>şaka zannediyorsunuz ama adamlar çok ciddi.
  entry_channels: anket,ilişkiler
- entry_name: |
    bmw almaya korkar olduk
  entry_id:  92902988
  entry_writer: cathars
  entry_fav_count: 8
  entry_content: |
    bu konuyla ilgili şöyle bir şey var kesinlikle yardımcı olacaktır: <a rel="nofollow noopener" class="url" target="_blank" href="https://pbs.twimg.com/media/DqTAR0LWkAAXjc1?format=jpg&amp;name=small" title="https://pbs.twimg.com/media/DqTAR0LWkAAXjc1?format=jpg&amp;name=small">https://pbs.twimg.com/…aaxjc1?format=jpg&amp;name=small</a>
  entry_channels: otomotiv
- entry_name: |
    yunan hayranlığı vs arap nefreti
  entry_id:  92885922
  entry_writer: vykintas
  entry_fav_count: 7
  entry_content: |
    türk-yunan ortak kültürü hayranlık değildir. yaklaşık 800 yıl beraber yaşamışısız, yok aslında birbirimizden farkımız. araplarla din dışında pek bir ortak noktamız yok. dininde uygulaması çok farklı. son olarak iki ülkeye de gittim riyad’da yabancı atina’da kendi evimde gibiydim.
  entry_channels: siyaset,anket
- entry_name: |
    herkes sevdiği halde sevilmeyen şeyler
  entry_id:  92870302
  entry_writer: bydreams
  entry_fav_count: 7
  entry_content: |
    la casa de papel. 2 bölüm dayanamadım, neden bu kadar olay oldu anlayamıyorum.
  entry_channels: anket
- entry_name: |
    fiyatı korkunç bir şekilde artan ürünler
  entry_id:  92902493
  entry_writer: defoe
  entry_fav_count: 7
  entry_content: |
    insanca yaşamamıza yardım eden her şey.<br/>teşekkürler siyasal islam teşekkürler aptal ve satılmış yöneticiler.<br/>akp döneminde zengin fakir uçurumu inanılmaz derinleşti.<br/>düzeleceği yok. <br/>düzüleni çok!
  entry_channels: anket,ekonomi
- entry_name: |
    çürüğü dolgusu olmayan diş çektirmemiş insan
  entry_id:  92899693
  entry_writer: edrf
  entry_fav_count: 6
  entry_content: |
    ocağıma incir ağacı dikmeye çalışan insandır.<br/><br/>not:diş hekimiyim.
  entry_channels: sağlık
- entry_name: |
    netflix
  entry_id:  92892661
  entry_writer: ghostronaut
  entry_fav_count: 6
  entry_content: |
    2 temel sebepten üyeliğimi sonlandırdığım oluşum.<br/><br/>1- zencisi gayi ibnesi hiç umurumda değil, angels in amerika'yı keyifle izlemiş adamım. herhalde en gayi odur aralarında. <br/><br/>ama mesela ortaçağ slavik atmosferde yazılmış fantastik kurgu işine kitapta ve oyunda karakterler çok net belirtilmişken sen oraya zorla etnik, zenci vs sokarsan sikerim öyle işi der kapatırım üyeliği. adam zaten xenophobia'yı işlemiş eserinde, sen kim oluyorsun da bozup kafana göre değiştiriyorsun? kitabın yazarını geçtim, eseri bağrına basmış insanlara da büyük saygısızlık. <br/><br/>en sevmediğim olay bir şeylerin bana zorla kabul ettirilmeye çalışılması. forced political correction dediğin şey bildiğin nazilik. olayın doğal işleyişine müdahale edip doğru olduğunu düşündüğün bir ideolojiyle ırklara müdahale etme hakkını kendinde buluyorsun. hadi biri bana açıklasın neden ırkçılığın dik alası olmadığını? <br/><br/>2- içerik. ülkeye göre içerik sağlanmasından hoşlanmıyorum. sevdiğim nordik diziler var ve bulunduğum ülkeden izleyemiyorum. sikerim öyle platformu.
  entry_channels: tv,sinema,spoiler
- entry_name: |
    ilk buluşma için en uygun mekan
  entry_id:  92871928
  entry_writer: sanat tarihi okudukta ne oldu
  entry_fav_count: 6
  entry_content: |
    canlı müzik olmayan bir yer.<br/>öyle mekanlarda düşüncemi duyamıyorum, karşımdaki kişiyi anlamak imkansız oluyor.<br/>haa konuşurken ağzının içine düşeyim derseniz o zaman canlı müzik iyidir<sup class="ab"><a title="(bkz: swh)" href="/?q=swh" data-query="swh">*</a></sup>
  entry_channels: anket,ilişkiler,yeme-içme
- entry_name: |
    yazarların bu akşamki alkol tercihi
  entry_id:  92870293
  entry_writer: babakinskywalker
  entry_fav_count: 5
  entry_content: |
    ulan hani boykot yapıyoduk dalyaraklar
  entry_channels: anket,ekşi-sözlük,yeme-içme
- entry_name: |
    ekşi itiraf
  entry_id:  92871592
  entry_writer: vardevela
  entry_fav_count: 4
  entry_content: |
    acı eşiğim yüksek sanırdım. ama canım harbi tatlıymış! <br/><br/>yine yoğurt yapmaya kalktım. <br/><br/>yine sütün içinde parmağımı 8 saniye bekletebileceğim kadar ılımasını bekledim. <br/><br/>yine mayalanma sıcaklığının altına düştüm. <br/><br/>yine ortaya cılk bir şey çıktı.
  entry_channels: anket,ilişkiler,ekşi-sözlük
- entry_name: |
    devlet bahçeli'nin f-35 için biz de yaparız demesi
  entry_id:  92904242
  entry_writer: beter mi
  entry_fav_count: 4
  entry_content: |
    fantastik bir beyan. şimdi diyeceksiniz ki, <a class="b" href="/?q=sebahattin+sen+yine+de+%c3%bclkemizi+o+kadar+k%c3%bc%c3%a7%c3%bcmseme">sebahattin sen yine de ülkemizi o kadar küçümseme</a> ama şöyle de bir gerçek var; tank palet fabrikasını satan bir ülkenin kalkıp da f-35 hayali kurması gerçekten çok enteresan. gerçi yaparsınız, yerli araba gibi gider alır boyarsınız, sonra gider yerli ve milli diye a haber kitleye yutturursunuz. <br/><br/><a rel="nofollow noopener" class="url" target="_blank" href="https://www.independentturkish.com/node/53446/haber/bahçeli’den-f-35-tepkisi-biz-de-yaparız?fbclid=IwAR1ntoRMLd34OkgLx3ZC4MaBXcXilxHbE6B6vriiGOafg1kmvFFClt50G2o#.XTSpWCVYX2k.twitter" title="https://www.independentturkish.com/node/53446/haber/bahçeli’den-f-35-tepkisi-biz-de-yaparız?fbclid=IwAR1ntoRMLd34OkgLx3ZC4MaBXcXilxHbE6B6vriiGOafg1kmvFFClt50G2o#.XTSpWCVYX2k.twitter">kaynak</a>
  entry_channels: 
- entry_name: |
    modern insanın en büyük problemi
  entry_id:  92901952
  entry_writer: and so forth
  entry_fav_count: 2
  entry_content: |
    kendi küçük sorunlarına hapsolması
  entry_channels: anket,ilişkiler,edebiyat
- entry_name: |
    ekşi sözlük'teki erkeklerin tipsiz olması
  entry_id:  92889321
  entry_writer: 42 kilometre
  entry_fav_count: 2
  entry_content: |
    herkesi kendi gibi sanan bir suserin sıçtığı tespit. üzücü yanı, benim için de geçerli olması.
  entry_channels: ilişkiler,ekşi-sözlük
- entry_name: |
    geceye bir şarkı bırak
  entry_id:  92873283
  entry_writer: celocel
  entry_fav_count: 1
  entry_content: |
    onur akın - asi ve mavi<br/><a rel="nofollow noopener" class="url" target="_blank" href="https://www.youtube.com/watch?v=m-0C7aZC5wk" title="https://www.youtube.com/watch?v=m-0C7aZC5wk">burdan</a>
  entry_channels: anket,müzik
- entry_name: |
    sözlükçülerin en sevmedikleri ses
  entry_id:  92905623
  entry_writer: alwaysbetter
  entry_fav_count: 0
  entry_content: |
    hızlı sürtünme sonucu çıkan ıslığımsı ses.
  entry_channels: anket
- entry_name: |
    kanser hastalarıyla dalga geçen insanlar
  entry_id:  92903176
  entry_writer: corsairsgold35
  entry_fav_count: 0
  entry_content: |
    (bkz: <a class="b" href="/?q=orospu+%c3%a7ocuklar%c4%b1">orospu çocukları</a>)
  entry_channels: ilişkiler,sağlık,haber
---
