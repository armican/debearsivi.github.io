---
layout: post
title: 21 Mayıs 2020 Ekşi Sözlük Debe
data:
- entry_name: |
    hakan ural
  entry_id:  107491017
  entry_writer: tirizettin
  entry_fav_count: 1410
  entry_content: |
    şu an canlı yayında ekşi sözlük'e saydırıyor. kimsiniz ulan sıfatınız ne, korkaksınız vs.. <br/><br/>keşke şu 1 ayda getirilen zamlardan bahsedebilse. varsa yoksa goy goy. her şeyi her konuyu yorumlayabiliyor, çıksın mesela 800'den fazla ürüne ek ilave gümrük vergisi neden getirilmiş konuşsun, mesela medikal bir ürün olan kontak lensin yerli üretimi var da biz mi almadık? yerli üretim böyle mi korunuyor desin. <br/><br/>bunları konuşsan goy goy yapmasan halkın yanında olsan merak etme seni burada bir allahın kulu eleştirmez hakan ural. eleştireni de sustururlar.
  entry_channels: tv,magazin
- entry_name: |
    arda turan'ın eşinin delikanlı açıklamaları
  entry_id:  107500282
  entry_writer: guerlain
  entry_fav_count: 1250
  entry_content: |
    millet sığ siz donanımlısınız evet. yahu insan kendinden bu kadar mı bihaber olur? senin kocan barda kavga etmiş yetmemiş eline silah alıp hastaneye gitmiş insan. biz sığ isek siz sanıyorum zemin kattaki otopark falansınız. para=kültür kafası bunlar hep işte. türkiye'deki varoşluğun kaynağı bu düşünce zaten. neyse klavye başından olmaz falan demiş. kocan görüntülü arasın onun suratına anlatıyım istersen.( muhtemelen etrafınızda yeteri kadar rafine insan olmadığı için size bunları söyleyecek insan da yok. haliyle cesaretle alakası yok beraber olduğunuz insanların seviyesiyle ilgisi var.) <br/><br/>en nihayetinde senin kocan futbolcu, kendini ne kadar geliştirmiş olabilir ki? ufak yaşta milyonlarca lira para, hayata karşı hiçbir sorumluluğu yok. mücadele duygusu yok. üniversite okuyup iş bulmaya çalışan, asgari ücretle çalışan insanlardan daha mı donanımlı senin kocan? yahu senin kocan parasına güvenip eski sevgilisi için sinema kapatan biri. sığlık konusunda en son konuşacak insan da sensin. böyle bir adamla evlenip çocuk yaptın. senden sığ çok az kişi var.<br/><br/>bu ülkede insanların kendini her konuda haklı görmesi, her zaman söyleyecekleri bir şey olması beni çıldırtıyor. arkadaş kimse " ya galiba bu sefer haksızım" " sanırım ben bu konuda hatalıyım" vs demiyor. ulan her konuda haklı olamazsınız ya. bi kere susun artık. ben yanıldım sanırım diyin. ya da en kötü ihtimalle insanlar beni yanlış anlıyor falan diyin. ulan 7 den 70 e herkes her konuda haklı ve her boka cevapları var.<br/><br/>kendini bilen adam bunların aksine daha ağırbaşlı ve öz eleştiri yapıyor. ona da bu özelliğinden dolayı ezik gözüyle bakıyorlar. varoşluğun ve kendini bilmezliğin egemenliğinden bıktım ben artık. cehaletin durmadan kazandığı, yüzsüzlüğün ve utanmazlığın bu kadar yüceltildiği bir ortamda yaşamak midemi bulandırıyor. <br/><br/>kocan sabah akşam adamım ben goy goyu yaparsa, her fırsatta yerli ve milli goy goyu yapıp siyasi iktidara göz kırparsa, milliyetçi bir çizgide görünüp vatan millet sakarya edebiyatı yapıp sonra da bu ülkenin bağımsızlık mücadelesinin başladığı bir tarihi unutup komik duruma düşerse, insanlar tabii ki dalga geçer. bunun sığlıkla ne ilgisi var?<br/><br/>tanım: abuk subuk açıklamalar.
  entry_channels: spor,magazin
- entry_name: |
    ankara'da otobüste çift oturmaktan çıkan kavga
  entry_id:  107513794
  entry_writer: camide bir ateist
  entry_fav_count: 643
  entry_content: |
    sizi efendi birisi olarak gördükleri için topluca linç etmeye çalışmışlar. bu toplumun huyudur bu, biri ezilirse bir tekme daha atmak için sıraya girerler. en başından "siktir git amına kodumun çocuğu" gibi medenice bir tepki verseydiniz hepsi suspus olurdu.
  entry_channels: seyahat
- entry_name: |
    trt 1 yayınındaki tuhaf erdoğan fotoğrafı
  entry_id:  107508754
  entry_writer: tekkullanimlikhesap
  entry_fav_count: 557
  entry_content: |
    (bkz: <a class="b" href="/?q=toplanak+allah+diyek">toplanak allah diyek</a>)
  entry_channels: siyaset,tv
- entry_name: |
    ingiliz bayraklı havlu asan kişinin tutuklanması
  entry_id:  107475235
  entry_writer: turkiyeden bikan pilot
  entry_fav_count: 552
  entry_content: |
    hayır velev ki adam balkona ingiliz bayrağı astı ne olacak amk. avrupadaki türkler konvoy yapıyor anasını sikiyor ortalığın sonra avrupa ırkçı. <br/><br/>saçmalık.
  entry_channels: siyaset,haber
- entry_name: |
    20 mayıs 2020 800 ürüne ek gümrük vergisi
  entry_id:  107473108
  entry_writer: zengin pijj
  entry_fav_count: 473
  entry_content: |
    (bkz: <a class="b" href="/?q=halk+a%c4%9f%c4%b1r+vergiler+alt%c4%b1nda+sikiliyordu">halk ağır vergiler altında sikiliyordu</a>)
  entry_channels: haber,ekonomi
- entry_name: |
    türkiye'deki abur cuburların bi boka benzememesi
  entry_id:  107499858
  entry_writer: jamirikua
  entry_fav_count: 417
  entry_content: |
    yahu sen 3. dünya vatandaşısın alman çikolatasını sana yedirirler mi?
  entry_channels: anket,yeme-içme
- entry_name: |
    oyun bağımlısı erkekle evlenmek
  entry_id:  107468629
  entry_writer: shinobigami
  entry_fav_count: 387
  entry_content: |
    kadınlar, hanımlar, eşler, dişi bireyler;<br/>erkekleri oyun üzerinden darlamayı bırakın artık iki saat bile hobisine zaman ayırmasına izin veremeyecek kadar ilgi orospususu iseniz lütfen evlenmeyin bulaşmayın bize.. bi rahat verin ya amk
  entry_channels: ilişkiler,oyun
- entry_name: |
    mardin'in 1.3 milyar lira elektrik borcunun olması
  entry_id:  107507416
  entry_writer: stefan braun
  entry_fav_count: 346
  entry_content: |
    kürtçe eğitimle elektrik çalmanın ne alakası var amk!
  entry_channels: haber,ekonomi
- entry_name: |
    buz gibi bira + sosis tava + patates kızartması
  entry_id:  107505163
  entry_writer: jamirikua
  entry_fav_count: 333
  entry_content: |
    75 tl.
  entry_channels: anket,yeme-içme
- entry_name: |
    kürtlerin ana dilde eğitim hakkı
  entry_id:  107513038
  entry_writer: the vys
  entry_fav_count: 283
  entry_content: |
    vay amk. bir kürt olarak ben karşıyken adamlara bak. kardeş derdiniz sikim. lütfen gidin anadili kürtçe olan bir ülkede yaşayın. bizim türkiye'de daha büyük problemlerimiz var.<br/>tanım: yallah barzanistana.<br/>edit: özelden amk kürdü diye küfür edenler. öperim. dikkat edin.<br/>edit: tribüne oynama demiş biri. yavrum aklınızı başınıza toplayın. ırak'ı, suriye'yi, libya'yı görün. aptal olmayın birlikte yaşayın. dil insanlar anlaşsın diye vardır, kavga etsin ayrışsın diye değil. şu kısacık ömrümüzü zehir etmeyin, bırakın yaşayalım.
  entry_channels: siyaset,eğitim
- entry_name: |
    20 mayıs 2020 amazon.com.tr'nin yaptığı şov
  entry_id:  107507797
  entry_writer: monkeyisgonetoheaven
  entry_fav_count: 258
  entry_content: |
    alıp alıp iade edecek çakallar yüzünden hızla iptal edilecek uygulamadır. boşuna sevinmeyin bu ülkede bu tip hareketler fazla sürmez.
  entry_channels: teknoloji
- entry_name: |
    20 mayıs 2020 izmir camiilerinde çav bella çalması
  entry_id:  107507759
  entry_writer: mecburen geldim
  entry_fav_count: 245
  entry_content: |
    ben artık camilerden yükselen hiçbir sesi duymak istemiyorum. yeter yahu. bangır bangır. sabah, akşam, gece fark etmiyor, sürekli bangır bangır. insani seviyelere çekin artık şu hoparlör ayarlarını.
  entry_channels: siyaset,müzik,haber
- entry_name: |
    20 mayıs 2020 berat albayrak açıklamaları
  entry_id:  107506324
  entry_writer: turkiyeden bikan pilot
  entry_fav_count: 229
  entry_content: |
    18 yıl sonra yerli üretim akıllarına gelmiş. zahmet etmeseydin kral ne gerek var hazır halde başka ülkelerden alıyoruz.
  entry_channels: siyaset,haber,ekonomi
- entry_name: |
    fuckbuddy arayan 40 yaş üstü evli kadın
  entry_id:  107506824
  entry_writer: h stone
  entry_fav_count: 218
  entry_content: |
    (bkz: <a class="b" href="/?q=yine+seks+hikayesi+mi+yaz%c4%b1yorsun+feridun+abi">yine seks hikayesi mi yazıyorsun feridun abi</a>)
  entry_channels: ilişkiler
- entry_name: |
    götüm var pozu
  entry_id:  107506533
  entry_writer: cathars
  entry_fav_count: 210
  entry_content: |
    <a rel="nofollow noopener" class="url" target="_blank" href="https://soz.lk/i/9e5h4yox" title="https://soz.lk/i/9e5h4yox">görsel</a>
  entry_channels: anket,ilişkiler
- entry_name: |
    hitler'in en büyük hatası
  entry_id:  107472244
  entry_writer: elgatooo
  entry_fav_count: 209
  entry_content: |
    avusturyadaki sanat okulunu kazanamaması
  entry_channels: anket,tarih
- entry_name: |
    sadece adı ve soyadıyla e-mail alabilen insan
  entry_id:  107493080
  entry_writer: selfsoul
  entry_fav_count: 187
  entry_content: |
    artık pek mümkün olmayandır. tabi isminiz sıbgetullah incebıyık değilse.
  entry_channels: teknoloji
- entry_name: |
    20 mayıs 2020 twitter aktroll kavgası
  entry_id:  107485326
  entry_writer: moraigne
  entry_fav_count: 173
  entry_content: |
    şu ibneler birde dava mava yazmıyorlar mı, ulan ibne neyin davası amacınız para işte, iki kemiğin adı dava olmuş.
  entry_channels: siyaset,haber
- entry_name: |
    öznur güven'in efsanevi karantina paylaşımı
  entry_id:  107469137
  entry_writer: cevirimanyagi
  entry_fav_count: 134
  entry_content: |
    milyonlarca yürek, tek bir soru: <br/><br/>(bkz: <a class="b" href="/?q=%c3%b6znur+g%c3%bcven+kim+amk">öznur güven kim amk</a>)
  entry_channels: magazin
- entry_name: |
    iphone'dan bilgisayara fotoğraf aktaramamak
  entry_id:  107493823
  entry_writer: samimi panda
  entry_fav_count: 116
  entry_content: |
    olm cidden bu aktaramıyorum diyenler trollüyor mu?<br/><br/>mac kullanıyorsan<br/>seç.<br/>gönder.<br/>airdrop.<br/>bitti.<br/><br/>windows kullanıyorsan<br/>telefonu bağla<br/>fotoğrafların olduğu klasörü aç<br/>kopyala<br/>yapıştır <br/><br/>bitti gitti amına koyim? <br/><br/>ne itunes'u?<br/>ne üçüncü parti programı?<br/>kafayı mı yediniz lan?
  entry_channels: teknoloji
- entry_name: |
    survivor 2020 ünlüler-gönüllüler
  entry_id:  107469125
  entry_writer: iyotsuz tuz
  entry_fav_count: 112
  entry_content: |
    baraka bana emanet diyen aycan, adamın yaptığı barakada oturmuş gitmesi iyi oldu diyor. <sup class="ab"><a title="(bkz: swh)" href="/?q=swh" data-query="swh">*</a></sup>
  entry_channels: anket,tv,magazin,spoiler
- entry_name: |
    yolda 100 bin dolar bulsanız geri verir misiniz
  entry_id:  107504041
  entry_writer: hedefe kosan adam
  entry_fav_count: 104
  entry_content: |
    geri vermem için birinden almam gerek. ne yapayım, kaldırıma mı geri vereyim? <br/><br/>soruda mantık hatası var.
  entry_channels: anket,ekonomi
- entry_name: |
    20 mayıs 2020 covid-19 vakasının 152587'ye çıkması
  entry_id:  107512829
  entry_writer: yea1453
  entry_fav_count: 76
  entry_content: |
    ya şu 200 satır sayısal veri verenler artık hergun aynı şeyleri yazıp durmayın amk
  entry_channels: siyaset,sağlık,haber
- entry_name: |
    suriye'de uzman çavuşu döven paşa
  entry_id:  107468004
  entry_writer: bu adam adam olmaz
  entry_fav_count: 71
  entry_content: |
    daha evvel suriye'de de görev yapmış olan şu anda binbaşı rütbesindeki subay bir arkadaşımla aramızda şöyle bir diyalog geçti;<br/><br/>- kral iyi akşamlar. ercan pürsunlu'yu tanıyor musun?<br/>* yok, kim bu?<br/>- tuğgeneralmiş.<br/>* hayırdır, nerede generalmiş?<br/>- ya bi uzman çavuşu dövmüş de suriye'de, twitter'da gündem olmuş.<br/>* iyi yapmış, general rütbesindeki biri uzman dövmüşse uzman dayaktan fazlasını hak etmiştir. "profesyonel ordu" diye çıkılan yol, disiplinsizlik abidesi sivilde bir baltaya sap olamayacak ipsiz sapsız bir sürü adamın silahlı kuvvetler'e doldurulması ile sonuçlandı. canımı tereddüt etmeden emanet edeceğim birçok subaydan daha yetenekli ve karakterli uzmanlarla da çalıştım ama istisnalar kaideyi bozmuyor.<br/><br/>özetle gram kompleksi bulunmayan bir subayın konuya ilişkin yorumu bu.
  entry_channels: siyaset,haber
- entry_name: |
    nasadan paralel evrene kanıt bulduk açıklaması
  entry_id:  107515376
  entry_writer: neyini tiryaki eden adam
  entry_fav_count: 63
  entry_content: |
    işte o paralel evrenler var ya onların inlerine girecegiz inlerine.
  entry_channels: bilim,haber
- entry_name: |
    ideal kadın boyu
  entry_id:  107473096
  entry_writer: haklisin hande
  entry_fav_count: 51
  entry_content: |
    sanırsın boy seçebilecek durumda müptezel sümsüqler. ciddili yazmışlar bir de. ‘bana şu boy olur, ona bu boy olur, berikine bir karış daa kısası kafi.’ neyse.
  entry_channels: anket,ilişkiler
- entry_name: |
    hakan ural'ın mesleği
  entry_id:  107514949
  entry_writer: dunya kupasina sarilan dede
  entry_fav_count: 51
  entry_content: |
    sabahın köründe dedikoducu kadınlarla birlikte ünlülerin dedikoduculuğunu yapma. <br/><br/>katma değeri oldukça yüksek, ülkemizin cari açığını kapatma yolunda önemli işler başardığını düşünmekteyim bu mesleği ile.
  entry_channels: anket,magazin
- entry_name: |
    çin'de covid-19'un mutasyona uğraması
  entry_id:  107511988
  entry_writer: liku it
  entry_fav_count: 46
  entry_content: |
    bir tuhaflık sezdiğim haber. 100 milyon kişi karantina altına alınmış ve tüm dünyanın sonradan mı haberi oluyor yani? bakın 100 milyon diyorum! ya haberde bir yanlışlık var ya da işin içinde bir pislik var.
  entry_channels: bilim,sağlık,haber
- entry_name: |
    doktorluğu en zor tıbbi branş
  entry_id:  107511654
  entry_writer: pageplant
  entry_fav_count: 40
  entry_content: |
    tıp okumadım, doğal olarak uygulamada en zor branşın ne olduğunu bilemem. ama hem annesini hem de babasını kanser/metastazdan kaybetmiş ve bundan dolayı birçok onkolog ile muhatap olmuş biri olarak en zor branşın onkoloji özellikle de çocuk onkolojisi olduğunu düşünüyorum. keşke çocuklar asla kanser olmasa...
  entry_channels: anket,bilim,sağlık
- entry_name: |
    kılıçdaroğlu'nun istifa etmesi gerekliliği
  entry_id:  107491773
  entry_writer: sodyumklorur
  entry_fav_count: 36
  entry_content: |
    chp'nin başına hz. muhammed gelse cehape dinsizdir deyip oyunu vermeyecek insanların yaşadığı memlekette trollükten başka bir şey olamayan gereklilik.
  entry_channels: siyaset
- entry_name: |
    birini unutmanın en iyi yolu
  entry_id:  107467980
  entry_writer: kafasizkalmakisteyenbeyin
  entry_fav_count: 36
  entry_content: |
    sana ne hayrı dokundu ki unutmanı engelliyo veya dokunduysa neden unutman gerekiyo. demek ki zararlı biri sana çok daa iyi geldiği söylenemez. hayatınızdan birilerini çıkarmak zor olmamalı ya. şahsen ben zorlanmıyorum çabalarım, karşılığı yoksa hadi güle güle bitti. ya kısa bir hayatın içindeyiz. allah kahretsin bide birini unutmaya mı çalışalım. giden gitsin, kalan kalsın. sen gidiyosan unut gitsin. <br/><br/>yani kısaca siz en değerli olansınız o birilerini de yerin dibine gömme zamanıı
  entry_channels: anket,ilişkiler
- entry_name: |
    25 yaşına gelip hala kariyer planı olmayan insan
  entry_id:  107512310
  entry_writer: outdoor
  entry_fav_count: 33
  entry_content: |
    42 yaşını devirmek üzereyim, kariyer planım yok. çok şükür muhtaç değilim, açıkta değilim. babadan dededen kalma zırnık yok. genel olarak mutlu bi insan sayılırım. yani çok da kasmayın amk. ölümlü dünya.
  entry_channels: 
- entry_name: |
    davutoğlu gül ve babacan'ın yöneteceği ülke
  entry_id:  107515007
  entry_writer: renksiz hayat
  entry_fav_count: 31
  entry_content: |
    ikisi de hala cesur ve temiz bir özeleştiri yapıp özür dilemiyor. haberimiz olmayan ne yolsuzluklar ve çarpıklıklar görmüşlerdir allah bilir.
  entry_channels: siyaset
- entry_name: |
    2500 dolarlık viskiyi lavaboya döken kadın
  entry_id:  107510965
  entry_writer: girdapmc5
  entry_fav_count: 27
  entry_content: |
    oyuncuları da tebrik ederim,çok güzel oynamışlar.
  entry_channels: ilişkiler,yeme-içme
- entry_name: |
    beşiktaş vs galatasaray
  entry_id:  107474827
  entry_writer: tapirmating
  entry_fav_count: 24
  entry_content: |
    değişik kafalar yaşatan bir versustur. yukarıdaki bir yorumu aynen kopyalıyorum, ne kafasiyla yazıldığını siz tahmin edersiniz artık.<br/><br/>"bu iki kulup sportif basarida birbirlerine yakin olsalar da..."<br/><br/>adam bunu cidden yazmış, inanarak yazmış. başka sözüm yok.
  entry_channels: spor,anket
- entry_name: |
    deniz mi daha çok huzur verir orman mı sorunsalı
  entry_id:  107468215
  entry_writer: turanacagri
  entry_fav_count: 22
  entry_content: |
    sağlık, ekonomi huzur verir insana.<br/><br/>hayat kötüyse ne orman huzur verir okyanus. <br/><br/>eğer kafan rahatsa çukurova'da 45 derecede pamuk toplarken de türkü söylersin.
  entry_channels: anket,seyahat
- entry_name: |
    kavgaya çağrılabilecek dizi karakterleri
  entry_id:  107468055
  entry_writer: saladdays
  entry_fav_count: 20
  entry_content: |
    breaking bad - better call saul / mike<br/><br/>edit: gelen mesajlar ile ilgili yorum yapmak gerekirse, yumruk yumruğa kavgadan ziyade sistemli bir kavga düşünmüştüm, yoksa çağırılmaz tabi. (neden öyle düşündüysem)
  entry_channels: anket,tv
- entry_name: |
    sihir ve büyüden korunmanın yolları
  entry_id:  107482560
  entry_writer: kenasuka
  entry_fav_count: 19
  entry_content: |
    ortalama iq yeterli
  entry_channels: anket
- entry_name: |
    hes kodu ile seyahat dönemi
  entry_id:  107508730
  entry_writer: enigma84
  entry_fav_count: 11
  entry_content: |
    55 milyon seçmenin tüm bilgilerinin koca bir dosya halinde internette yayınlandığı ülke sanırım patagonya idi.<br/><br/>ne gizliliği, gizlilik mi kaldı diye sorası geliyor insanın.
  entry_channels: teknoloji,sağlık,seyahat
- entry_name: |
    ege ve akdeniz bölgesinde kürt mafya yapılanması
  entry_id:  107473398
  entry_writer: beyler derebeyi
  entry_fav_count: 10
  entry_content: |
    egeliyim ve özeleştiri yapacağım: madem bu adamları sevmiyorsunuz neden dükkanınızı, arsanızı satıyorsunuz?<br/><br/>ben çok görüyorum, arsasını fahiş fiyata satıp koşa koşa mersedes bayisine gidip en pahalisindan araba alan hemşehrilerimi. memleketinizi çok seviyorsanız satmayacaksınız kardeşim! ondan sonra ziril ziril ağlıyorlar kürtler şöyle kürtler böyle diye. o kürtler sabahın yedisinde eşek gibi inşaata koşuyorlar, peki sen hemşehrim? sen mersedesinle limon ağacı diplerinde bira içerken neden ikiyüzlü tavırlar sergiliyorsun?<br/><br/>iğneyi kendine çuvaldızı başkasına batır demişler. arsayı satıp parasını yerken iyi, ama sonra para bitince kötü. olmaz.<br/><br/>kürtlerin mafya şeklinde yapılandigi doğrudur ama buna izin veren de sensin!
  entry_channels: siyaset,seyahat
- entry_name: |
    dünya tarihinde en fazla tanınan türkler
  entry_id:  107516950
  entry_writer: golorucu
  entry_fav_count: 10
  entry_content: |
    tanrı'nın kırbacı <a class="b" href="/?q=attila">attila</a>
  entry_channels: tarih
- entry_name: |
    kürtlerin mezopotamya tarihini sahiplenme çabası
  entry_id:  107515660
  entry_writer: nedemekulan
  entry_fav_count: 9
  entry_content: |
    bakın <a rel="nofollow noopener" class="url" target="_blank" href="https://twitter.com/nomadontheroadd/status/1065601623395848192" title="https://twitter.com/nomadontheroadd/status/1065601623395848192">şu</a> harita marco polo'nun gezerken çizdiği harita. bu adam anadolu'da hatrı sayılı zaman geçirmiş ve şunları gözlemlemiş ;<br/><br/>"turcomania'da üç kavim vardır. bunlardan ilki muhammed'e tapan ve onun koyduğu kanunları takip eden türkmenlerdir, bunlar kaba bir dil konuşan sade insanlardır. güzel otlakların olduğu dağlarda ve bozkırlarda yaşarlar, zira hayvancılıkla geçinirler"<br/><br/>"burada türki denen mükemmel bir at ırkı ve ayrıca kıymetli katırlar vardır."<br/><br/>"diğerleri (diğer iki kavim) ise şehirlerde ve kasabalarda yaşayan rumlar ve ermenilerdir, bunlar ticaret ve zanaat ile uğraşırlar."<br/><br/>"en iyi ve gösterişli halılar, kızıl ipekler burada (turcomania) imal edilir. en önemli şehirleri konya, kayseri ve sivas'tır"<br/><br/>"bunların hepsi, tatarların (moğolların) ulu hanına tabidir."<br/><br/>marco polo'nun "tatarların (moğolların) ulu hanı" dediği kişi ise en doğudaki hanlığın hükümdarı kubilay han'dır.<br/>"<br/>ne kürtlerden bahseder ne de başkalarından. çünkü adam gördüğünü söylemiş. bu gezi 13. yüzyılda geçiyor. gerisi size kalmış.<br/><br/>bana göre kürtler ayrı bir ırk yada kavim değil. irani bir topluluk. kendilerini farklı görüp kendilerine suni bir tarih yaratmak istiyorlar.
  entry_channels: siyaset,tarih
- entry_name: |
    ekşi itiraf
  entry_id:  107505383
  entry_writer: prokeyfsiyonel
  entry_fav_count: 8
  entry_content: |
    ulan para kazanmaya çalışıyorsun, gusto geliştirecek zaman kalmıyor. gustoya abanayım diyosun, işleri kim yapacak? zevk sahibi bir zengin olamayacağımı artık kabul etmem gerek sanırım ve bu beni çok üzüyor derdimi skem.<br/><br/>ayrıcaaaaa... aman o kitap da okunsun, bu film de çok güzelmiş, ooo şu diziyi nasıl izlemezsin, haydaa bu poskesi hiç mi dinlemedin, 1657 senesi kuş gözlem müzesini gezdin mi, gotik davlumbazlarla müzik yapan grubu gördün mü, ebeyin yutup kanalına baktın mı? olm bu zamanını her boka yetiremeyen bir ben miyim yaa? oh be rahatladım.
  entry_channels: anket,ilişkiler,ekşi-sözlük
- entry_name: |
    dünyada en fazla tanınan türkler
  entry_id:  107516681
  entry_writer: corsairsgold35
  entry_fav_count: 7
  entry_content: |
    (bkz: <a class="b" href="/?q=mustafa+kemal+atat%c3%bcrk">mustafa kemal atatürk</a>)
  entry_channels: 
- entry_name: |
    sürat kargo
  entry_id:  107491629
  entry_writer: bicaremadenmuhendisi
  entry_fav_count: 4
  entry_content: |
    12 mayısta kargoya verilen ürünüm ankara aktarmada araçtan indirilmiş ve indirildiği yerde kalmış sanırım. bir daha da araca bindirilmemiş.<br/><br/>yol mu tuttu ki benim ürünümü. ikna mı edemiyorlar binmek için araca ne oldu ki? <br/><br/>bir de sloganları var süratliyiz diye. bence sürat felakettir yazsalar daha bir gerçekçi olur.
  entry_channels: ekonomi
- entry_name: |
    rte ve bahçeli'nin siyasetten silinme tarihi
  entry_id:  107516546
  entry_writer: cirkinlik ne kadar kolay
  entry_fav_count: 4
  entry_content: |
    beklediğimiz tarihtir. <br/>deistim ama böyle duaya amin denir.
  entry_channels: siyaset,anket
- entry_name: |
    20 mayıa 2020 kızılayın fidye istemesi
  entry_id:  107517517
  entry_writer: ozmencoskun
  entry_fav_count: 3
  entry_content: |
    tanım: kızılay’ın kaçırması gereken yazarın açtığı başlık.<br/><br/>deprem ya da koronavirüs başlığı değil ki başkası kapsın. neye acele ettin de “mayıa” yazdın amk? elin mi doluydu?
  entry_channels: 
- entry_name: |
    serdar ali çelikler
  entry_id:  107517389
  entry_writer: cey30
  entry_fav_count: 0
  entry_content: |
    maç izlemez, avrupa futbolunu bilmez yemediğini indirdiği gibi programa çıkar sinameki gibi abuk sabuk ezber kelimelerle kahve ağzıyla konuşur , yurdum futbolseveri de bayıla bayıla izler bu adamı. <br/>her zaman diyorum bu ülkede futbolu gerçekten seven insan yok denecek kadar az.
  entry_channels: spor,tv
- entry_name: |
    hayatı anlamlı kılan üç şey
  entry_id:  107517669
  entry_writer: zaman sokaklarda kayboluyordu
  entry_fav_count: 0
  entry_content: |
    sağlıklı olmak.<br/>vicdan ve akıl sahibi olmak.<br/>sevmek.<br/><br/>bunlar yoksa, hayatın anlamı da yoktur.
  entry_channels: anket
---
